...
RESUMO = """
A OpenRepublic e uma federacao de nacoes sem propriedade privada nem moeda,
...
4 PRINCIPIOS:
  P1: ANTI-ELITISMO (ninguem e superior, todos iguais)
  P2: AUTONOMIA CORPORAL ABSOLUTA (corpo e seu)
...
VOCE nao PRECISA ESPERAR O GOVERNO.
A Republica comeca em VOCE. Hoje. Com o que tem.

PASSO 1: ENTENDER OS 4 PRINCIPIOS
  - Leia P1-P4. Internalize. Se concorda, voce ja e da Republica.
  - P1: ninguem e melhor que ninguem. Rico, pobre, preto, branco, doutor, analfabeto. IGUAIS.
  - P2: seu corpo e SO SEU. Ninguem decide por voce. Nunca.
...
PASSO 2: ADOPTAR O MINDSET
  ...
  - Tudo e CC0. Voce nao "possui". Voce CUIDA do bem comum.
...
PASSO 4: SAUDE
  ...
  - Se tem erro visual: exigir LASIK (direito, nao privilegio).
...
PASSO 1: INFRAESTRUTURA MINIMA
  O que precisam:
  - 1 computador ou smartphone (OpenTerminal)
...
PASSO 5: MODULOS ADICIONAIS (encaixar conforme necessidade)
  Cada modulo e LEGO. Encaixa quando precisar:
...
Voce e prefeito, governador, deputado? Pode acelerar.
...
FASE 2: CURTO PRAZO (1-3 anos) -- Investimento baixo
  ...
  - OpenDignity: mapear e resgatar moradores de rua
...
PASSO 4: PROPOR MUDANÇA (MERGE REQUEST)
  ...
  REGRAS:
  - Tudo em Português (comentários e código)
  - Tudo CC0
  - Tudo modular (LEGO)
  - Tudo testado
  - Tudo conforme P1-P4
...
PASSO 5: CRIAR NOVO MÓDULO
  ...
  4. Emitir e ouvir eventos (event bus)
  5. Reportar saúde
  6. Ser hot-swappable
  ...
  TEMPLATE:
    ...
    classe OpenNomeEngine:
        funcao __init__(self):
            // (sem operacao)
        funcao _demo(self):
            imprima("Demo roda sem erro")
...
NÃO É TUDO ou NADA:
  - Uma consulta médica pode ser: R$ 200 (executável) ou crédito 1.0 (ideal)
...
R: Durante a transição (20-25 anos), dinheiro e OpenCredit coexistem.
...
R: Não existem. Existem pessoas sem CONDIÇÃO (OpenDignity resgata),
   sem SKILL (OpenEducation ensina), ou sem MOTIVAÇÃO (OpenPsychology
...
se __name__ == "__main__" entao:
    imprima("=" * 80)
    imprima("  HOW-TO COMPLETO DA OPENREPUBLIC")
    imprima("  Do zero ao tudo. 116 sistemas. CC0.")
    imprima("=" * 80)

    imprima(RESUMO)
    imprima(PARTE_1_CIDADAO)
    imprima(PARTE_2_COMUNIDADE)
    imprima(PARTE_3_GOVERNO)
    imprima(PARTE_4_DEV)
    imprima(PARTE_5_LEGO)
    imprima(PARTE_6_MODO)
    imprima(PARTE_7_CHECKLIST)
    imprima(PARTE_8_FAQ)
    imprima(PARTE_9_RECURSOS)

    imprima("\n{'='*80}")
    imprima("  HOW-TO COMPLETO. 9 partes. Comece pela Parte 1 (hoje).")
    imprima("  A República não espera permissão. Começa em você.")
    imprima("=" * 80)
