...
"ECONOMIA": {
    ...
    "P11_INDUSTRY_COPIAR_MELHORAR_SUPERAR": {
        "titulo": "Brasil copia, melhora and supera produtos estrangeiros",
        "problema": (
...
"DIREITOS": {
    ...
    "P21_LIGACAO_PREDATORIA_PROIBIDA": {
        "titulo": "Telemarketing, robocall and spam telefônico PROIBIDOS",
...
"INFRAESTRUTURA": {
    ...
    "P32_REPOSITORIO_UNICO_SEM_FORK": {
        ...
        "custo_atual": "R$ 0",
        "economia": "Evita fragmentação and duplicação",
        "prazo": "Fase 1 (imediato)",
...
if __name__ == "__main__":
    print("=" * 80)
    print("  POLÍTICAS PÚBLICAS PARA O BRASIL")
    print("  Baseadas em 116 sistemas | 91.729 linhas | OpenRepublic")
    print("=" * 80)
    total = 0
    for each (area, politicas) in POLITICAS.items():
        print("\n\n  === {area} ({len(politicas)} políticas) ===\n")
        for each (pid, p) in politicas.items():
            total = total + 1
            print("  [{pid}] {p['titulo']}")
            print("    Problema: {p['problema'][:70]}...")
            print("    Custo: {p['custo_atual']}")
            print("    Economia: {p['economia']}")
            print("    Prazo: {p['prazo']}")
            print("    Sistema: {p['sistema']}")
    print("\n\n{'='*80}")
    print("  TOTAL: {total} políticas públicas em {len(POLITICAS)} áreas")
    print("  Base: 116 sistemas | 91.729 linhas de código | CC0")
    print("  Brasil: do capitalismo predatório à República do bem comum")
    print("  20-25 anos. Gradual. Sem trauma. Com dignidade.")
    print("{'='*80}")
