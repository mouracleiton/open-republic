...
package how_to_completo_da_openrepublic
...
TEMPLATE:
    ...
    // Classes aqui...
    type OpenNomeEngine struct {
        func __init__(self) {
            // (sem operacao)
        func _demo(self) {
            fmt.Println("Demo roda sem erro")
...
if __name__ == "__main__" {
    fmt.Println("=" * 80)
    fmt.Println("  HOW-TO COMPLETO DA OPENREPUBLIC")
    fmt.Println("  Do zero ao tudo. 116 sistemas. CC0.")
    fmt.Println("=" * 80)
    fmt.Println(RESUMO)
    fmt.Println(PARTE_1_CIDADAO)
    fmt.Println(PARTE_2_COMUNIDADE)
    fmt.Println(PARTE_3_GOVERNO)
    fmt.Println(PARTE_4_DEV)
    fmt.Println(PARTE_5_LEGO)
    fmt.Println(PARTE_6_MODO)
    fmt.Println(PARTE_7_CHECKLIST)
    fmt.Println(PARTE_8_FAQ)
    fmt.Println(PARTE_9_RECURSOS)
    fmt.Println("\n{'='*80}")
    fmt.Println("  HOW-TO COMPLETO. 9 partes. Comece pela Parte 1 (hoje).")
    fmt.Println("  A República não espera permissão. Começa em você.")
    fmt.Println("=" * 80)
