...
if __name__ == "__main__" {
    println!("=" * 80);
    println!("  POLÍTICAS PÚBLICAS PARA O BRASIL");
    println!("  Baseadas em 116 sistemas | 91.729 linhas | OpenRepublic");
    println!("=" * 80);
    total = 0;
    para cada (area, politicas) em POLITICAS.items(): {
        ...
        para cada (pid, p) em politicas.items(): {
            total = total + 1;
            println!("  [{pid}] {p['titulo']}");
            println!("    Problema: {p['problema'][:70]}...");
            println!("    Custo: {p['custo_atual']}");
            println!("    Economia: {p['economia']}");
            println!("    Prazo: {p['prazo']}");
            println!("    Sistema: {p['sistema']}");
    println!("\n\n{'='*80}");
    println!("  TOTAL: {total} políticas públicas em {len(POLITICAS)} áreas");
    println!("  Base: 116 sistemas | 91.729 linhas de código | CC0");
    println!("  Brasil: do capitalismo predatório à República do bem comum");
    println!("  20-25 anos. Gradual. Sem trauma. Com dignidade.");
    println!("{'='*80}");
