...
let Razao: overload cognitivo de multiplas redes = dano mental = dano corporal.;
...
if __name__ == "__main__" {
    println!("=" * 60);
    println!("  OPENFOCUS -- POLITICA DE FOCO ESTRATEGICO");
    println!("  'Um canal. Uma mente. Saude primeiro.'");
    println!("=" * 60);
    println!();
    for p in PLATFORMS {
        emoji = {"ativa": "[FOCO]", "abandonada": "[CORT]",;
                "nunca": "[----]"};
        println!("  {emoji.get(p.status.value, '[?]')} {p.name:<16} {p.reason}");
    println!();
    println!("=" * 60);
    println!("  X/Twitter: negociar features + opensoftware + social unico");
    println!("  Tudo o mais: ruido. Ruido = dano. Cortar.");
    println!("=" * 60);
