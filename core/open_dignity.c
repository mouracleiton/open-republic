...
#ifndef OPENDIGNITY_ERRADICACAO_DA_POBREZA_E_MORADORES_DE_RUA_H
#define OPENDIGNITY_ERRADICACAO_DA_POBREZA_E_MORADORES_DE_RUA_H
...
// ============================================================================
typedef struct VulnerabilityType {
...
// ============================================================================
// decorador: @dataclass
typedef struct VulnerablePerson {
...
typedef struct DignityColony {
    ...
    // Infraestrutura
    bool has_housing = true;
    bool has_kitchen = true;
    bool has_clinic = true;
    bool has_school = true;
    bool has_fablab = false;
    bool has_garden = true // horta comunitaria;
    bool has_terminal = true // OpenTerminal;
    // Servicos
    int medical_staff = 2;
    int social_workers = 3;
    int mentors = 5;
    char* security = "comunitaria"  // ! policial -- OpenMartialArts;
    // Progresso
...
typedef struct DignityEngine {
    ...
    {texto: qualquer} immediate_rescue(self, person_id: texto) {
    ...
    {texto: qualquer} assign_to_colony(self, person_id: texto) {
    ...
    {texto: qualquer} restore(self, person_id: texto) {
    ...
    {texto: qualquer} reintegrate(self, person_id: texto) {
    ...
    {texto: qualquer} report_relapse(self, person_id: texto, reason: texto) {
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENDIGNITY -- ERRADICACAO DA POBREZA E MORADORES DE RUA");
    printf("  Ninguem dorme na rua. Ninguem passa fome. Nunca mais.");
    printf("=" * 80);
    ...
    /* TODO: iterador C manual para c em colonies_data */
        engine.register_colony(c);
        printf("  [{c.colony_id}] {c.name:<30} {c.colony_type.value:<20} ";
    ...
    /* para name, age, vuln, loc, addic, mental, days, story in persons_data: */
        result = engine.identify_person(name, age, vuln, loc, addic, mental,;
                                        days, story);
        printf("  [{result['person_id']}] {result['name']:<25} ";
            "{vuln.value:<25} {loc}");
    // === 3. RESGATE IMEDIATO ===
    printf("\n\n  === 3. RESGATE IMEDIATO (comer + abrigo + saude) ===\n");
    /* TODO: iterador C manual para pid em list(engine.persons.keys()) */
        result = engine.immediate_rescue(pid);
        if (result.get("rescued")) {
            printf("  {result['person']:<25} -> RESGATADO (comeu, abrigo, saude)");
    ...
    /* TODO: iterador C manual para pid em list(engine.persons.keys()) */
        ...
        if (result.get("assigned")) {
            ...
            printf("    Moradia: {result['housing']}");
            if (result['treatment']) {
                printf("    Tratamento: {', '.join(result['treatment'][:3])}");
            printf("    Skills: {', '.join(result['skills'][:3])}");
    // === 5. RESTAURACAO ===
    printf("\n\n  === 5. RESTAURACAO (tempo, tratamento, oficio) ===\n");
    /* TODO: iterador C manual para pid em list(engine.persons.keys())[:5] */
        result = engine.restore(pid);
        if ("person" in result) {
            printf("  {result['person']:<25} -> {result['message']}");
    // === 6. REINTEGRACAO ===
    printf("\n\n  === 6. REINTEGRACAO (primeiros casos) ===\n");
    /* TODO: iterador C manual para pid em list(engine.persons.keys())[:3] */
        result = engine.reintegrate(pid);
        if (result.get("status") == "REINTEGRADO") {
            printf("\n  {result['person']:<25} -> {result['status']}");
            printf("    {result['housing']}");
            printf("    {result['employment']}");
            printf("    {result['record']}");
            printf("    {result['message']}");
    // === 7. CASO DE RECAIDA ===
    printf("\n\n  === 7. RECAIDA = FALHA DO SISTEMA ===\n");
    if (sizeof(engine.persons) > 5) {
        pids = list(engine.persons.keys());
        result = engine.report_relapse(pids[5], "Tratamento de adicao incompleto");
        printf("  {result['person']} voltou para rua.");
        printf("  Motivo: {result['reason']}");
        printf("  {result['system_failure']}");
    // === 8. STATS ===
    printf("\n\n  === 8. ESTATISTICAS ===\n");
    s = engine.stats();
    ...
    // === FILOSOFIA ===
    printf("\n\n{'='*80}");
    printf("  FILOSOFIA DO OPENDIGNITY");
    printf("{'='*80}");
    printf(""";
A REPUBLICA ! ACEITA:;
...
// )
    printf("{'='*80}");
    printf("  OpenDignity: {s['total_identified']} pessoas mapeadas, ";
        "{s['total_rescued']} resgatadas, ";
        "{s['total_integrated']} reintegradas.");
    printf("  Ninguem na rua. Nunca mais.");
    printf("{'='*80}");

#endif // OPENDIGNITY_ERRADICACAO_DA_POBREZA_E_MORADORES_DE_RUA_H
