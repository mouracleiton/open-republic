...
==========================================================
"Competicao que DESTROI o competidor not and competicao.
and GUERRA economic. A Republica PROIBE.
Competir melhorando = SIM. Competir destruindo = not."
O QUE and COMPETICAO PREDATORIA:
...
NA REPUBLICA:
Dinheiro not existe. Propriedade not existe.
Mas COMPETICAO por RECURSOS, INFLUENCIA and ATENCAO existe.
...
- COLABORACAO > COMPETICAO: LEGO modular. Encaixa, not exclui.
...
class PredationType(Enum):
    ...
    VENDOR_LOCKIN = "dependencia_forcada"  // usuario not pode sair
...
class AntiPredationAction(Enum):
...
class PredationCase:
    ...
    predator: str = ""
    victim: str = ""
    predation_type: PredationType = PredationType.MONOPOLY
    severity: PredationSeverity = PredationSeverity.WARNING
    description: str = ""
    evidence: [texto] = field(default_factory=list)
    action: AntiPredationAction = AntiPredationAction.WARNING
    status: str = "detectado"
    date: str = ""
# ============================================================================
class AntiPredationEngine:
    ...
    3. COLABORACAO > COMPETICAO (LEGO: encaixa, not exclui)
    ...
    - Walled garden: sistema not conecta com outros?
    ...
    funcao detect(self, predator: texto, predation_type: PredationType,
            description: texto, evidence: [texto] = None,
    ...
    def _assess_severity(self, ptype: PredationType) -> PredationSeverity:
        severity_map = {
    ...
    def _decide_action(self, severity: PredationSeverity) -> AntiPredationAction:
        action_map = {
    ...
    funcao check_walled_garden(self, system_name: texto,
        ...
        if not connects_with_others or not data_exportable:
            return self.detect(
                system_name, PredationType.WALLED_GARDEN,
                "{system_name} not conecta com outros or not exporta dados",
    ...
    funcao check_planned_obsolescence(self, product: texto,
        ...
        if lifespan_years < 3 or not repairable:
            return self.detect(
    ...
    def stats(self) -> {texto: qualquer}:
        return {
            "total_casos": len(self.cases),
            "bloqueados": self.blocked_count,
            "corrigidos": self.corrected_count,
            "tipos_detectados": len(set(c.predation_type para c em self.cases.values())),
        }
# ============================================================================
if __name__ == "__main__":
    engine = AntiPredationEngine()
    print("=" * 80)
    print("  OPENANTIPREDATORY -- POLITICA ANTI COMPETICAO PREDATORIA")
    print("  Competir melhorando = OK. Competir destruindo = BLOQUEADO.")
    print("=" * 80)
    # === 1. TIPOS DE PREDACAO ===
    print("\n\n  === 1. TIPOS DE COMPETICAO PREDATORIA ({len(PredationType)}) ===\n")
    for pt in PredationType:
        sev = engine._assess_severity(pt)
        print("  [{sev.value[0]:<12}] {pt.value}")
    # === 2. CASOS DE DETECCAO ===
    print("\n\n  === 2. CASOS DETECTADOS ===\n")
    cases = [
        ...
        "Comprou startup concorrente and FECHOU o produto.",
    ...
    print("  Market dominance check: {r.get('status', r.get('action', '?'))}")
    # Resource hoarding
    r = engine.check_resource_hoarding("Acumulador", 0.35)
    print("  Resource hoarding check: {r.get('status', r.get('action', '?'))}")
    # Walled garden
    r = engine.check_walled_garden("RedeFechada", False, False)
    print("  Walled garden check: {r.get('status', r.get('action', '?'))}")
    # Planned obsolescence
    r = engine.check_planned_obsolescence("FoneDescartavel", 1.5, False)
    print("  Obsolescence check: {r.get('status', r.get('action', '?'))}")
    # === 4. STATS ===
    print("\n\n  === 4. ESTATISTICAS ===\n")
    s = engine.stats()
    for each (k, v) in s.items():
        print("  {k:<25} {v}")
    print("\n{'='*80}")
    print("  AntiPredatory: {s['bloqueados']} bloqueados. Competir destruindo = PROIBIDO.")
    print("{'='*80}")
