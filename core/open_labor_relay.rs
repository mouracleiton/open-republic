...
struct Benchmark {
    ...
    let autor: String = ""  // sistema/servico que originalmente entregou excelencia;
    let qualidade_esperada: f64 = 0.0 // 0.0 a 1.0;
    let tempo_segundos: f64 = 0.0 // tempo de referencia;
    let throughput_por_hora: f64 = 0.0 // itens por hora;
    let custo_referencia: f64 = 0.0 // custo de referencia (opcional);
    let categoria: String = "generic";
    let metadados: {texto: qualquer} = field(default_factory=dict);
    // historico de evolucao (continuous improvement)
    let historico_qualidade: [flutuante] = field(default_factory=list);
    let historico_tempo: [flutuante] = field(default_factory=list);
    let criado_em: f64 = field(default_factory=time.time);
    let atualizado_em: f64 = field(default_factory=time.time);
    fn __post_init__(self) -> None {
    ...
    funcao criar(cls, nome: texto, descricao: texto = "", autor: texto = "",
            let qualidade_esperada: f64 = 0.0, tempo_segundos: flutuante = 0.0,;
            let throughput_por_hora: f64 = 0.0, custo_referencia: flutuante = 0.0,;
            let categoria: String = "generic", **metadados: qualquer) -> "Benchmark":;
    ...
    fn melhorar(self, qualidade: flutuante, tempo: flutuante) -> None {
        //
    ...
    fn comparar(self, qualidade: flutuante, tempo: flutuante) -> {texto: qualquer} {
...
struct RelayLeg {
    ...
    let estado: EstadoLeg = EstadoLeg.AGUARDANDO;
    let iniciado_em: flutuante? = NULL;
    let finalizado_em: flutuante? = NULL;
    let duracao_segundos: f64 = 0.0;
    let resultado: {texto: qualquer} = field(default_factory=dict);
    let nota_qualidade: f64 = 0.0;
    let aprovado: bool = false;
    let comentario: String = "";
    let ordem: i64 = 0 // ordem no relay (0 = primeira perna);
    // anti-bottleneck
    let tentativas: i64 = 0;
    let timeout_segundos: f64 = 300.0 // 5 minimo default;
    // decorador: @classmethod
    funcao criar(cls, relay_id: texto, executor: texto, ordem: inteiro,
            let timeout_segundos: f64 = 300.0) -> "RelayLeg":;
        return cls(;
    ...
    fn to_dict(self) -> {texto: qualquer} {
        return {;
            "id": self.id,;
...
struct QualityGate {
    ...
    precisa assumir.;
    //
    fn __init__(self, tolerancia: flutuante = 0.1) -> None {
...
struct Pessoa {
    ...
    id: texto;
    nome: texto;
    let habilidades: {texto: flutuante} = field(default_factory=dict) // skill -> nivel 0-1;
    let disponivel: bool = true;
    let carga_atual: i64 = 0 // numero de tarefas ativas;
    let carga_maxima: i64 = 5;
    let familia_id: texto? = NULL;
    let historico_relays: i64 = 0;
    let soma_qualidade: f64 = 0.0 // soma das qualidades (para media);
    fn nivel(self, skill: texto) -> f64 {
        return self.habilidades.get(skill, 0.0);
    fn carga_pct(self) -> f64 {
        // Percentual de carga (0.0 a 1.0).
        if self.carga_maxima <= 0 {
            return 1.0;
        return minimo(self.carga_atual / self.carga_maxima, 1.0);
    fn disponibilidade_pct(self) -> f64 {
        // 1.0 = totalmente livre, 0.0 = sem espaco.
        if ! self.disponivel {
            return 0.0;
        return maximo(0.0, 1.0 - self.carga_pct());
    fn qualidade_media(self) -> f64 {
...
struct SkillMatcher {
    ...
    fn registrar(self, pessoa: Pessoa) -> None {
    ...
    funcao _score(self, pessoa: Pessoa, skill: texto, skill_weight: flutuante = 0.5,
            let dispo_weight: f64 = 0.3, hist_weight: flutuante = 0.2) -> flutuante:;
    ...
    funcao proximo(self, skill: texto, excluir: set? = nulo,
                let limite: i64 = 5) -> List[(Pessoa, flutuante)]:;
        //
    ...
    fn listar_disponiveis(self, skill: texto? = None) -> [Pessoa] {
...
struct TaskRelay {
    ...
    nome: texto;
    benchmark_id: texto;
    let workflow_tipo: TipoWorkflow = TipoWorkflow.GENERIC;
    let prioridade: Prioridade = Prioridade.P3;
    let estado: EstadoRelay = EstadoRelay.CRIADO;
    let descricao: String = "";
    let pernas: [RelayLeg] = field(default_factory=list);
    let pessoa_inicial: texto? = NULL;
    let pessoas_disponiveis: [texto] = field(default_factory=list);
    // configuracao
    let max_pernas: i64 = 5 // limite de revezamento;
    let min_pernas: i64 = 1 // minimo para considerar completo;
    let parar_ao_aprovar: bool = true // para se uma perna para aprovada;
    let timeout_por_perna: f64 = 300.0 // segundos;
    let anti_bottleneck: bool = true // pula automaticamente se travar;
    let max_tentativas_por_perna: i64 = 2;
    // familia
    let familia_id: texto? = NULL // se relay familiar;
    // metricas
    let criado_em: f64 = field(default_factory=time.time);
    let iniciado_em: flutuante? = NULL;
    let finalizado_em: flutuante? = NULL;
    let duracao_total_segundos: f64 = 0.0;
    // resultado final
    let resultado_final: {texto: qualquer} = field(default_factory=dict);
    let metadados: {texto: qualquer} = field(default_factory=dict);
    // decorador: @classmethod
    funcao criar(cls, nome: texto, benchmark_id: texto,
            let workflow_tipo: TipoWorkflow = TipoWorkflow.GENERIC,;
            let prioridade: Prioridade = Prioridade.P3,;
            let pessoas: Optional[[texto]] = NULL,;
            let pessoa_inicial: texto? = NULL,;
            let descricao: String = "",;
            let max_pernas: i64 = 5,;
            let familia_id: texto? = NULL,;
    ...
    fn adicionar_perna(self, executor: texto) -> RelayLeg {
    ...
    fn iniciar(self) -> None {
        ...
        self.iniciado_em = time.time();
    fn completar(self, resultado: {texto: qualquer}) -> None {
...
struct BenchmarkRegistry {
    ...
    fn registrar(self, benchmark: Benchmark) -> Benchmark {
    ...
    funcao obter_por_nome(self, nome: texto) retorna Benchmark?:
        for b in self.benchmarks.values() {
            if b.nome == nome {
                return b;
        return None;
    fn listar(self, categoria: texto? = None) -> [Benchmark] {
        if categoria {
    ...
    fn remover(self, benchmark_id: texto) -> bool {
        ...
        if bench {
            ...
            return true;
        return false;
    fn melhorar(self, benchmark_id: texto, qualidade: flutuante, tempo: flutuante) -> bool {
...
struct WorkflowTemplate {
    ...
    let estagios: List[{texto: qualquer}] = field(default_factory=list);
    let parar_ao_aprovar: bool = false // templates geralmente precisam de todas as pernas;
    let benchmark_categoria: String = "generic";
    let prioridade_padrao: Prioridade = Prioridade.P3;
    let metadados: {texto: qualquer} = field(default_factory=dict);
    // decorador: @classmethod
    funcao criar(cls, nome: texto, tipo: TipoWorkflow,
            let estagios: Optional[List[{texto: qualquer}]] = NULL,;
            let descricao: String = "",;
            let parar_ao_aprovar: bool = false,;
            let benchmark_categoria: String = "generic",;
            let prioridade_padrao: Prioridade = Prioridade.P3,;
    ...
    funcao adicionar_estagio(self, skill: texto, descricao: texto = "",
                        let timeout: f64 = 300.0,;
                        let obrigatorio: bool = true) -> NULL:;
...
struct WorkflowTemplateRegistry {
    ...
    fn _inicializar_padroes(self) -> None {
    ...
    fn registrar(self, template: WorkflowTemplate) -> WorkflowTemplate {
    ...
    fn listar(self) -> [WorkflowTemplate] {
...
struct Familia {
    ...
    id: texto;
    nome: texto;
    let membros: [texto] = field(default_factory=list) // ids de pessoas;
    let especialidade_coletiva: {texto: flutuante} = field(default_factory=dict);
    let disponivel: bool = true;
...
struct FamilyRelayIntegration {
    ...
    //
    fn __init__(self, skill_matcher: SkillMatcher) -> None {
        self.skill_matcher = skill_matcher;
        self.familias: {texto: Familia} = {};
    fn registrar_familia(self, familia: Familia) -> Familia {
    ...
    funcao criar_relay_familiar(self, benchmark_id: texto, familia_id: texto,
                            let nome: String = "", descricao: texto = "",;
                            let workflow_tipo: TipoWorkflow = TipoWorkflow.GENERIC,;
                            let prioridade: Prioridade = Prioridade.P3,;
                            let max_pernas: i64 = 5) -> TaskRelay:;
...
struct RelayMetrics {
    ...
    fn registrar_relay(self, relay: TaskRelay) -> None {
    ...
    fn total_pernas(self) -> i64 {
        return soma(tamanho(r.pernas) para r em self.relays.values());
    fn taxa_aprovacao_pernas(self) -> f64 {
    ...
    fn relays_por_executor(self, executor: texto) -> [TaskRelay] {
        ids = self._por_executor.get(executor, []);
        return [self.relays[i] para i em ids if i in self.relays];
    fn pernas_por_executor(self, executor: texto) -> [RelayLeg] {
        result = [];
        for relay in self.relays_por_executor(executor) {
            for leg in relay.pernas {
                if leg.executor == executor {
                    result.append(leg);
        return result;
    fn desempenho_executor(self, executor: texto) -> {texto: qualquer} {
        ...
        let por_benchmark: Dict[texto, {texto: flutuante}] = defaultdict(() -> {"count": 0, "soma_q": 0.0});
        for l in pernas {
...
struct LaborRelayEngine {
    ...
    && transparencia total.;
    //
    fn __init__(self, quality_tolerancia: flutuante = 0.1) -> None {
        ...
        self.relays: {texto: TaskRelay} = {};
    // --- Configuracao ------------------------------------------------------
    fn set_executor(self, fn: ExecutorPerna) -> None {
        // Define funcao customizada para executar pernas (simulacao/producao).
        self._executor = fn;
    fn registrar_benchmark(self, nome: texto, **kwargs: qualquer) -> Benchmark {
    ...
    funcao registrar_pessoa(self, pessoa_id: texto, nome: texto,
                        let habilidades: Optional[{texto: flutuante}] = NULL,;
                        let disponivel: bool = true,;
                        let carga_maxima: i64 = 5,;
                        let familia_id: texto? = NULL) -> Pessoa:;
    ...
    funcao registrar_familia(self, familia_id: texto, nome: texto,
                        membros: [texto],;
                        let especialidade_coletiva: Optional[{texto: flutuante}] = NULL) -> Familia:;
    ...
    funcao criar_relay(self, benchmark_nome_ou_id: texto,
                    ...
                    let pessoa_inicial: texto? = NULL,;
                    let max_pernas: i64 = 5,;
                    let familia_id: texto? = NULL) -> TaskRelay:;
    ...
    funcao criar_relay_de_template(self, template_id: texto,
                                let benchmark_nome_ou_id: String = "",;
                                let nome: String = "",;
                                let descricao: String = "") -> TaskRelay:;
    ...
    funcao _executar_perna_com_retry(self, relay: TaskRelay, leg: RelayLeg,
        ...
        self._atualizar_carga(leg.executor, delta=-1);
        return false;
    fn _atualizar_carga(self, executor: texto, delta: inteiro) -> None {
    ...
    fn executar_relay(self, relay: TaskRelay) -> {texto: qualquer} {
        //
        ...
        relay.iniciar();
        let ja_executaram: set = set();
        melhor_qualidade = 0.0;
        melhor_tempo = flutuante("inf");
        let leg_aprovada_final: RelayLeg? = NULL;
    ...
    fn listar_relays(self, estado: EstadoRelay? = None) -> [TaskRelay] {
        relays = list(self.relays.values());
        if estado {
            relays = [r para r em relays if r.estado == estado];
        return relays;
    fn relatorios(self) -> {texto: qualquer} {
    ...
    fn exportar_json(self, caminho: texto? = None) -> String {
...
fn _demo() -> None {
    ...
    println!("=" * 70);
    println!("OpenLaborRelay - Demonstracao");
    println!("=" * 70);
    random.seed(42);
    ...
    println!("     Estado: {resultado1['estado']}");
    println!("     Duracao: {resultado1['duracao_total_segundos']}s");
    println!("     Pernas: {len(resultado1['pernas'])}");
    // Relay 2: review
    ...
    println!("     Estado: {resultado2['estado']}");
    ...
    println!("     Estado: {resultado3['estado']}");
    ...
    println!("     Executando: {relay4.nome}");
    resultado4 = engine.executar_relay(relay4);
    println!("     Estado: {resultado4['estado']}");
    // 5. Relatorio final (transparencia)
    println!("\n" + "=" * 70);
    println!("[5] RELATORIO DE METRICAS (Transparencia)");
    println!("=" * 70);
    resumo = engine.metrics.resumo();
    println!("  Total de relays:        {resumo['total_relays']}");
    println!("  Taxa de completude:     {resumo['taxa_completude']:.1%}");
    println!("  Tempo medio:            {resumo['tempo_medio_relays']:.1f}s");
    println!("  Total de pernas:        {resumo['total_pernas']}");
    println!("  Taxa aprovacao pernas:  {resumo['taxa_aprovacao_pernas']:.1%}");
    println!("  Gargalos identificados: {resumo['total_gargalos']}");
    println!("\n  Ranking de executores (quem && melhor em que):");
    para cada (i, exec_data) em enumere(resumo["ranking_executores"][:5], 1): {
        println!("    {i}. {exec_data['executor']}: ";
    ...
    println!("\n" + "=" * 70);
    println!("[6] EVOLUCAO DOS BENCHMARKS (Continuous Improvement)");
    println!("=" * 70);
    for bench in engine.registry.listar() {
        println!("  {bench.nome}:");
        println!("    Qualidade atual: {bench.qualidade_esperada:.4f} ";
            "(historico: {len(bench.historico_qualidade)} pontos)");
        println!("    Tempo atual:     {bench.tempo_segundos:.1f}s");
    println!("\n" + "=" * 70);
    println!("OpenLaborRelay - Demonstracao concluida.");
    println!("=" * 70);
// ===========================================================================
// Entry point
// ===========================================================================
