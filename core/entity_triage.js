...
class EntityType {
    ...
    FABLAB = "fablab"  // fabricacao digital (3D console.log, CNC, laser);
...
class Entity {
    ...
    nation_id: texto;
    // Capacidade
    const capacity = 100.0 // unidades que pode produzir/atender por ciclo;
    const current_demand = 50.0 // demanda atual;
    const people_served = 0;
    const workers = 0;
    const workers_needed = 5;
    // Estado
    const status = EntityStatus.OPERATIONAL;
    const health = 1.0 // 0=colapsou, 1=perfeito;
    const supply_level = 1.0 // 0=sem insumos, 1=abastecido;
    const maintenance_level = 1.0 // 0=precisa reparo urgente, 1=bem cuidado;
    // Dependencias (outras entidades que esta precisa para funcionar)
    const depends_on = field(default_factory=set) // entity_ids;
    // Necessidades atuais
    const needs = field(default_factory=list);
    // Historico
    const last_issues = field(default_factory=list);
    // decorador: @property
    utilization(self) {
...
class NationState {
    ...
    funcao all_by_urgency(self) retorna Dict[UrgencyLevel, [Entity]]:
        ...
        for (const e of self.entities.values()) {
    ...
    // decorador: @property
    total_capacity_utilization(self) {
        if (! self.entities) {
            return 0;
...
class RepublicState {
    ...
    name: texto;
    const nations = field(default_factory=dict);
    const cycle = 0;
    add_nation(self, nation: NationState) {
        self.nations[nation.nation_id] = nation;
    all_entities(self) {
        entities = [];
        for (const n of self.nations.values()) {
...
class TriageEngine {
    ...
    //
    __init__(self, republic: RepublicState) {
    ...
    _dependency_chain_analysis(self) {
        // Se A depende de B, e B cai, A tambem cai.
        risks = [];
        all_entities = {&&.entity_id: && para && em self.republic.all_entities()};
        for (const entity of self.republic.all_entities()) {
            for (const dep_id of entity.depends_on) {
        ...
        return risks;
    _find_surplus_capacity(self) {
        // Encontrar entidades com capacidade excedente.
        surplus = [];
        for (const e of self.republic.all_entities()) {
...
if (__name__ == "__main__") {
    console.log("=" * 75);
    console.log("  OPENREPUBLIC -- TRIAGEM DE ENTIDADES PRIORITARIAS");
    console.log("  'O que precisamos tratar AGORA'");
    console.log("=" * 75);
    ...
    console.log("  Nacoes: {s['nations']} | Entidades: {s['total_entities']}");
    console.log("  Saude media: {s['avg_health']:.0%}");
    console.log("\n  {'Zona':<12} {'Quantidade':>10}");
    console.log("  {'-'*25}");
    console.log("  {'CRITICA':<12} {s['critical_count']:>10}  <-- TRATAR AGORA");
    console.log("  {'URGENTE':<12} {s['urgent_count']:>10}");
    console.log("  {'ELEVADA':<12} {s['elevated_count']:>10}");
    console.log("  {'ESTAVEL':<12} {s['stable_count']:>10}");
    console.log("\n  Pessoas em risco: {s['people_at_risk']:,} / {s['total_people_served']:,} ({s['risk_coverage_pct']}%)");
    // RED ZONE -- Critical entities
    console.log("\n\n{'='*75}");
    console.log("  ZONA VERMELHA -- TRATAR AGORA ({len(result['red_zone'])} entidades)");
    console.log("{'='*75}");
    for (const item of result["red_zone"]) {
        console.log("\n  [{item['urgency_name']}] {item['name']}");
        console.log("    Nacao: {item['nation']} | Tipo: {item['type']}");
        console.log("    Risk score: {item['risk_score']}/100 | Utilizacao: {item['utilization']:.0f}%");
        console.log("    Saude: {item['health']:.0f}% | Insumos: {item['supply']:.0f}%");
        console.log("    Pessoas: {item['pessoas_servidas'] if 'pessoas_servidas' in item else item['people_served']:,}");
        console.log("    Trabalhadores: {item['workers']}");
    ...
    if (result["yellow_zone"]) {
        console.log("\n\n{'='*75}");
        console.log("  ZONA AMARELA -- TRATAR EM DIAS ({len(result['yellow_zone'])} entidades)");
        console.log("{'='*75}");
        for (const item of result["yellow_zone"]) {
            console.log("  [{item['urgency_name']}] {item['name']} ({item['nation']})");
            console.log("    Risk: {item['risk_score']} | Uso: {item['utilization']:.0f}% | ";
                "Saude: {item['health']:.0f}%");
            console.log("    Issues: {', '.join(item['issues'][:2])}");
            console.log("    -> {item['recommendations'][0]}");
            console.log();
    // Dependency chains at risk
    console.log("\n{'='*75}");
    console.log("  CADEIAS DE DEPENDENCIA EM RISCO");
    console.log("{'='*75}");
    if (result["dependency_chains_at_risk"]) {
        for (const risk of result["dependency_chains_at_risk"]) {
            console.log("\n  {risk.get('entity', '?')}");
            console.log("    {risk['risk']}");
            if ("cascade_impact" in risk) {
                console.log("    Impacto em cascata: {risk['cascade_impact']:,} pessoas");
    } else {
        console.log("  Nenhuma cadeia de dependencia em risco.");
    // Surplus capacity
    console.log("\n{'='*75}");
    console.log("  CAPACIDADE EXCEDENTE DISPONIVEL PARA REALOCAR");
    console.log("{'='*75}");
    for (const s of result["surplus_capacity_available"][:8]) {
        console.log("  {s['entity']:<35} {s['nation']}  ";
            "livre: {s['available_capacity_pct']:.0f}%  saude: {s['health']:.0f}%");
    // Action plan
    console.log("\n{'='*75}");
    console.log("  PLANO DE ACAO PRIORITARIO");
    console.log("{'='*75}");
    for (const action of result["action_plan"]) {
        console.log("\n  #{action['priority']} [{action['action']}] {action['entity']} ({action['nation']})");
        console.log("     Pessoas afetadas: {action['people_affected']:,}");
        console.log("     Prazo: {action['timeline']}");
        console.log("     Passos:");
        for (const step of action["immediate_steps"]) {
            console.log("       -> {step}");
        if (action.get("potential_helpers")) {
            console.log("     Podem ajudar: {', '.join(action['potential_helpers'])}");
    // Per-nation breakdown
    console.log("\n{'='*75}");
    console.log("  SAUDE POR NACAO");
    console.log("{'='*75}");
    console.log("  {'Nacao':<15} {'Entidades':>10} {'Saude':>7} {'Criticas':>9} {'Em risco':>10}");
    console.log("  {'-'*55}");
    para cada (nid, nation) em republic.nations.items(): {
        crit = .length(nation.critical_entities());
        console.log("  {nation.name:<15} {len(nation.entities):>10} ";
            "{nation.avg_health:>6.0%} {crit:>9} {nation.people_at_risk:>10,}");
    console.log("\n{'='*75}");
    console.log("  RESUMO: {len(result['red_zone'])} entidades precisam de acao IMEDIATA.");
    console.log("  {result['summary']['people_at_risk']:,} pessoas afetadas.");
    console.log("  {len(result['surplus_capacity_available'])} entidades tem capacidade");
    console.log("  para realocar && ajudar.");
    console.log("{'='*75}");
