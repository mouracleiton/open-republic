...
class LegoPiece {
    ...
    const description = "";
    // Encaixes
    const input_connector = null;
    const output_connector = null;
    // Configuracao
    const config = field(default_factory=dict);
    // Estado
    const executed = false;
    const result = field(default_factory=dict);
    can_connect_to(self, other: "LegoPiece") {
...
class LegoChain {
    ...
    chain_id: texto;
    name: texto;
    const pieces = field(default_factory=list);
    const executed = false;
    const total_impact = field(default_factory=dict);
    // decorador: @property
    is_valid(self) {
        // Verifica se todos os encaixes sao compativeis.
        for (const i of intervalo(.length(self.pieces) - 1)) {
...
class Competition {
    ...
    const description = "";
    const organizer = "";
    const participants = field(default_factory=list);
    const winners = field(default_factory=list);
    const prize_pool_money = 0.0 // dinheiro (durante transicao);
    const prize_pool_credit = 0.0 // credito da Republica;
    const ngo_share_pct = 0.3 // % que vai para ONG;
    const ngo_verified = "";
    const start_date = "";
    const end_date = "";
// decorador: @dataclass
class NGOAllocation {
    ...
    ngo_id: texto;
    ngo_name: texto;
    const verified = false // OpenHistory fact-check;
    const verification_source = "";
    const causes = field(default_factory=list);
    const amount_received = 0.0;
    const amount_used = 0.0;
    const people_helped = 0;
    const transparent = true // OpenSymbolRevision: ONG transparente;
// ============================================================================
...
class BusinessModelEngine {
    ...
    funcao create_competition(self, title: texto, comp_type: CompetitionType,
                        const prize_money = 0.0,;
                        const prize_credit = 0.0,;
                        const ngo_share = 0.3,;
                        const ngo_id = "",;
                        const organizer = "",;
                        const description = "") -> {texto: qualquer}:;
    ...
    funcao execute_chain(self, chain_id: texto,
                    const winners = null) -> {texto: qualquer}:;
        ...
        chain.total_impact = {
            "winners": winners  ||  [],;
            "pieces_executed": .length(results),;
        };
        return {;
            "chain_id": chain_id,;
            "name": chain.name,;
    ...
    stats(self) {
        return {;
            "total_competitions": .length(self.competitions),;
            "total_chains": .length(self.chains),;
            "total_ngos": .length(self.ngos),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENBUSINESSMODEL -- MODELO HIBRIDO DA TRANSICAO");
    console.log("  Competicao -> Premiacao -> ONG -> Impacto -> Credito (LEGO chain)");
    console.log("=" * 80);
    // === 1. ONGs VERIFICADAS ===
    console.log("\n\n  === 1. ONGs VERIFICADAS ===\n");
    for (const ngo of engine.list_ngos()) {
        status = ngo["verified"] ? "OK" : "BLOQ";
        console.log("  [{status}] {ngo['name']:<35} causas: {', '.join(ngo['causes'][:2])}");
    ...
    para title, ctype, money, credit, ngo_share, ngo_id in comps: {
        ...
        if (r.get("created")) {
            console.log("\n  {r['title']}");
            console.log("    Premio: {r['prize_money']} + {r['prize_credit']}");
            console.log("    ONG: {r['ngo']} ({r['ngo_share']})");
            console.log("    Chain LEGO: {r['chain_pieces']} pecas (valida: {r['chain_valid']})");
        } else {
            console.log("\n  {title}: {r.get('error', 'ERRO')}");
            console.log("    {r.get('message', '')}");
    ...
    para cada (chain_id, chain) em engine.chains.items(): {
        ...
        if (result.get("executed")) {
            ...
            for (const p of result["results"][:3]) {
                console.log("    [{p['type']}] {p['piece']}");
                if (p["result"]) {
                    para cada (k, v) em list(p["result"].items())[:2]: {
                        console.log("      {k}: {v}");
    // === 4. DISTRIBUICAO DO PREMIO ===
    console.log("\n\n  === 4. DISTRIBUICAO DO PREMIO (modelo hibrido) ===\n");
    console.log("  Para cada R$ 1.000 de premio:");
    console.log("    R$ 500 (50%) -> Vencedor (dinheiro durante transicao)");
    console.log("    R$ 300 (30%) -> ONG verificada (ajuda pessoas reais)");
    console.log("    R$ 200 (20%) -> Reinvestido na Republica (infraestrutura)");
    console.log("    + Credito da Republica para Vencedor");
    console.log("\n  DURANTE A TRANSICAO: dinheiro + credito coexistem");
    console.log("  DEPOIS DA TRANSICAO: so credito. Dinheiro extinto.");
    // === 5. ONG IMPACTO ===
    console.log("\n\n  === 5. IMPACTO DAS ONGs ===\n");
    for (const ngo of engine.list_ngos()) {
        if (ngo["verified"]  &&  ngo["received"] > 0) {
            console.log("  {ngo['name']:<35} recebeu R$ {ngo['received']:,.0f} ";
    ...
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA DO OPENBUSINESSMODEL");
    console.log("{'='*80}");
    console.log(""";
...
// )
    console.log("{'='*80}");
    console.log("  OpenBusinessModel: {s['total_competitions']} competicoes, ";
        "{s['total_donated']:,.0f} doado, ";
        "{s['people_helped']} pessoas ajudadas.");
    console.log("  Modelo LEGO: pecas encaixam em cadeia.");
    console.log("{'='*80}");
