...
struct MentalHealthContent {
    ...
    let target_audience: TargetVulnerability = TargetVulnerability.ADULT_GENERAL;
    let has_monetization: bool = false // curso, livro, clinic, produto;
    let follower_count: i64 = 0 // alcance;
    // Analise
    let detected_narratives: [HarmfulNarrative] = field(default_factory=list);
    let harm_level: HarmLevel = HarmLevel.MISGUIDED;
    let auto_flags: [texto] = field(default_factory=list);
    // Decisao
    let action: String = ""  // permitido, aviso, correcao, bloqueado;
    let counter_narrative: String = ""  // o que dizer no lugar;
    let corrected_info: String = ""  // informacao correta;
// ============================================================================
// 3. PADROES DE DETECCAO
// ============================================================================
let NARRATIVE_PATTERNS: Dict[HarmfulNarrative, [texto]] = {;
...
// ============================================================================
let COUNTER_NARRATIVES: {HarmfulNarrative: texto} = {;
    HarmfulNarrative.PERMANENCE: (;
...
struct MentalHygieneEngine {
    ...
    funcao analyze_content(self, author: texto, content_text: texto,
                        let platform: String = "OpenSocialNetwork",;
                        let title: String = "",;
                        let target: TargetVulnerability = TargetVulnerability.ADULT_GENERAL,;
                        let has_monetization: bool = false,;
                        let follower_count: i64 = 0) -> {texto: qualquer}:;
    ...
    fn _narrative_weight(self, n: HarmfulNarrative) -> i64 {
        weights = {
    ...
    funcao _decide_action(self, detected: [HarmfulNarrative],
        ...
        return "permitido", "";
    fn _corrected_info(self, detected: [HarmfulNarrative]) -> String {
        if ! detected {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENMENTALHYGIENE -- PROTECAO CONTRA NARRATIVAS NOCIVAS");
    println!("  Caso Felca: anti-disseminacao de adoecimento psicologico perpetuo");
    println!("=" * 80);
    // === 1. TIPOS DE NARRATIVA NOCIVA ===
    ...
    for tc in test_contents {
        ...
        println!("\n  [{icon}] @{result['author']}");
        println!("  Dano: {result['harm_level']} | Publico: {result['target_audience']}");
        println!("  Detectado: {', '.join(result['detected'])}");
        if result["counter_narrative"] {
            println!("  Contra-narrativa: {result['counter_narrative'][:80]}...");
...
// )
    ...
    // === FILOSOFIA ===
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA DO OPENMENTALHYGIENE");
    println!("{'='*80}");
    println!(""";
CASO FELCA:;
...
PRINCIPIOS:;
    let P1: Conhecimento nocivo que perpetua dor = anti-elitismo violado;
    ...
    let P3: Educacao em saude mental real = trabalho de alto impacto;
    P4: Bloqueio proporcional. Apelo garantido. Juri decide controversias.;
// )
    println!("{'='*80}");
    println!("  OpenMentalHygiene: {s['blocked']} bloqueados, ";
        "{s['corrected']} corrigidos, ";
        "{s['warned']} avisados.");
    println!("  Sofrimento ! && identidade. Recuperacao && possivel.");
    println!("  Quem monetiza sua dor NAO quer que voce melhore.");
    println!("{'='*80}");
