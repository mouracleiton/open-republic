...
public class OpenvaluesimulationSimulacoesComPessoasReaisNomesObfuscados {
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class RealWorker {
    ...
    // 2. SIMULADOR
    // ============================================================================
    public static class ValueSimulator {
        ...
        public {texto: qualquer} simulate_worker(self, worker_id: texto) {
        ...
        public String _life_impact(self, sobra_cap: flutuante, sobra_rep: flutuante) {
        ...
        public {texto: qualquer} aggregate_stats(self) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENVALUESIMULATION -- PESSOAS REAIS, SALARIOS REAIS");
        System.out.println("  Nomes obfuscados. Dados reais do mercado brasileiro.");
        System.out.println("=" * 80);
        // === 1. CADA TRABALHADOR ===
        System.out.println("\n\n  === SIMULACAO POR TRABALHADOR ===\n");
        /* TODO: for-each Java para wid em ordene(sim.workers.keys()) */
            r = sim.simulate_worker(wid);
            w = sim.workers[wid];
            System.out.println("\n  {'='*70}");
            System.out.println("  {r['alias']} | {r['profissao']} | {r['empresa']} | {r['local']}");
            System.out.println("  Familia: {r['familia']} | Horas/mes: {r['horas_mes']}");
            System.out.println("  {'='*70}");
            System.out.println("\n  CAPITALISMO (hoje):");
            System.out.println("    Produz:           {r['CAPITALISMO']['produz']}");
            System.out.println("    Recebe:           {r['CAPITALISMO']['recebe']} ({r['CAPITALISMO']['fica_com']})");
            System.out.println("    Empresa suga:     {r['CAPITALISMO']['empresa_suga']}");
            System.out.println("    Banco suga:       {r['CAPITALISMO']['banco_suga']}");
            System.out.println("    Intermediario:    {r['CAPITALISMO']['intermediario_suga']}");
            System.out.println("    TOTAL EXTRAIDO:   {r['CAPITALISMO']['total_extraido']}");
            System.out.println("    Razao:            {r['CAPITALISMO']['razao_exploracao']}");
            System.out.println("    Hora vale:        {r['CAPITALISMO']['hora_valor']}");
            System.out.println("    Sobra (apos ali+comida): {r['CAPITALISMO']['sobra_apos_aluguel_comida']}");
            System.out.println("\n  REPUBLICA EXECUTAVEL:");
            System.out.println("    Produz:           {r['REPUBLICA_EXECUTAVEL']['produz']}");
            System.out.println("    Recebe:           {r['REPUBLICA_EXECUTAVEL']['recebe']} ({r['REPUBLICA_EXECUTAVEL']['fica_com']})");
            System.out.println("    Empresa suga:     {r['REPUBLICA_EXECUTAVEL']['empresa_suga']}");
            System.out.println("    Banco suga:       {r['REPUBLICA_EXECUTAVEL']['banco_suga']}");
            System.out.println("    Intermediario:    {r['REPUBLICA_EXECUTAVEL']['intermediario_suga']}");
            System.out.println("    Pool 5%:          {r['REPUBLICA_EXECUTAVEL']['pool_5pct']}");
            System.out.println("    Operacional real: {r['REPUBLICA_EXECUTAVEL']['operacional_real']}");
            System.out.println("    Hora vale:        {r['REPUBLICA_EXECUTAVEL']['hora_valor']}");
            System.out.println("    Sobra (apos ali+comida): {r['REPUBLICA_EXECUTAVEL']['sobra_apos_aluguel_comida']}");
            System.out.println("\n  GANHO:");
            System.out.println("    Extra/mes:  {r['GANHO']['extra_por_mes']}");
            System.out.println("    Extra/ano:  {r['GANHO']['extra_por_ano']}");
            System.out.println("    Multiplicador: {r['GANHO']['multiplicador']}");
            System.out.println("    Impacto: {r['GANHO']['mudanca_vida']}");
        // === 2. AGREGADO ===
        System.out.println("\n\n  {'='*70}");
        System.out.println("  === AGREGADO: TODOS OS {len(sim.workers)} TRABALHADORES ===");
        System.out.println("  {'='*70}\n");
        agg = sim.simulate_all();
        System.out.println("  {agg['message']}");
        System.out.println("\n  Extracao mensal total: {agg['total_extraido_mensal']}");
        System.out.println("  Extracao anual total:  {agg['total_extraido_anual']}");
        System.out.println("  Ganho mensal Republica: {agg['total_ganho_republica_mensal']}");
        System.out.println("  Ganho anual Republica:  {agg['total_ganho_republica_anual']}");
        // === 3. STATS ===
        System.out.println("\n\n  === ESTATISTICAS BASE ===\n");
        ...
        System.out.println("\n\n  === RESUMO (todos) ===\n");
        System.out.println("  {'Alias':<10} {'Profissao':<25} {'Hoje':>10} {'Republica':>10} {'Ganho':>10} {'X':>5} {'Impacto'}");
        ...
        /* TODO: for-each Java para wid em ordene(sim.workers.keys()) */
            ...
            System.out.println("  {r['alias']:<10} {r['profissao'][:24]:<25} ";
                "R${int(hoje):>8,} R${int(rep):>8,} R${int(gain):>8,} ";
                "{mult:>4} {impacto}");
        System.out.println("\n{'='*80}");
        System.out.println("  Simulacao: {len(sim.workers)} trabalhadores reais.");
        System.out.println("  Extracao anual: {agg['total_extraido_anual']}.");
        System.out.println("  Se virasse Republica: {agg['total_ganho_republica_anual']} voltam.");
        System.out.println("  Para trabalhadores. Nao parasitas.");
        System.out.println("{'='*80}");
}
