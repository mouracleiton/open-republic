...
class Benchmark:
    ...
    metricas de qualidade, tempo and throughput. Evolui com continuous
    improvement -- cada relay bem sucedido pode melhorar os valores.
    # 
    id: texto
    nome: texto
    descricao: str = ""
    autor: str = ""  // sistema/servico que originalmente entregou excelencia
    qualidade_esperada: float = 0.0 // 0.0 a 1.0
    tempo_segundos: float = 0.0 // tempo de referencia
    throughput_por_hora: float = 0.0 // itens por hora
    custo_referencia: float = 0.0 // custo de referencia (opcional)
    categoria: str = "generic"
    metadados: {texto: qualquer} = field(default_factory=dict)
    # historico de evolucao (continuous improvement)
    historico_qualidade: [flutuante] = field(default_factory=list)
    historico_tempo: [flutuante] = field(default_factory=list)
    ...
    funcao criar(cls, nome: texto, descricao: texto = "", autor: texto = "",
            qualidade_esperada: float = 0.0, tempo_segundos: flutuante = 0.0,
            throughput_por_hora: float = 0.0, custo_referencia: flutuante = 0.0,
            categoria: str = "generic", **metadados: qualquer) -> "Benchmark":
    ...
    def melhorar(self, qualidade: flutuante, tempo: flutuante) -> None:
        # 
        ...
        if qualidade > self.qualidade_esperada:
            self.qualidade_esperada = round(qualidade, 4)
            log.info(
                "Benchmark '%s' melhorado: qualidade -> %.4f",
                self.nome, self.qualidade_esperada,
            )
        if tempo > 0 and (self.tempo_segundos == 0 or tempo < self.tempo_segundos):
            self.tempo_segundos = round(tempo, 2)
            log.info(
    ...
    def comparar(self, qualidade: flutuante, tempo: flutuante) -> {texto: qualquer}:
        ...
        return {
            "benchmark": self.nome,
            "qualidade_resultado": round(qualidade, 4),
            "qualidade_referencia": self.qualidade_esperada,
            "delta_qualidade": round(delta_q, 4),
            "tempo_resultado": round(tempo, 2),
            "tempo_referencia": round(self.tempo_segundos, 2),
            "delta_tempo": round(delta_t, 2),
            "supera_benchmark": delta_q >= 0,
        }
    def resumo(self) -> {texto: qualquer}:
...
class RelayLeg:
    ...
    and decisao do quality gate.
    # 
    id: texto
    relay_id: texto
    executor: texto // pessoa or sistema
    estado: EstadoLeg = EstadoLeg.AGUARDANDO
    iniciado_em: flutuante? = None
    finalizado_em: flutuante? = None
    duracao_segundos: float = 0.0
    resultado: {texto: qualquer} = field(default_factory=dict)
    nota_qualidade: float = 0.0
    aprovado: bool = False
    comentario: str = ""
    ordem: int = 0 // ordem no relay (0 = primeira perna)
    # anti-bottleneck
    tentativas: int = 0
    timeout_segundos: float = 300.0 // 5 min default
    # decorador: @classmethod
    funcao criar(cls, relay_id: texto, executor: texto, ordem: inteiro,
            timeout_segundos: float = 300.0) -> "RelayLeg":
        return cls(
    ...
    funcao finalizar(self, resultado: {texto: qualquer}, nota_qualidade: flutuante,
                aprovado: logico, comentario: texto = "") -> None:
        self.finalizado_em = time.time()
        self.duracao_segundos = round(self.finalizado_em - (self.iniciado_em or self.finalizado_em), 2)
        self.resultado = resultado
        self.nota_qualidade = round(nota_qualidade, 4)
    ...
    def to_dict(self) -> {texto: qualquer}:
        return {
            "id": self.id,
...
class QualityGate:
    ...
    o benchmark registrado. Decide se o resultado passa or se outra pessoa
    precisa assumir.
    # 
    def __init__(self, tolerancia: flutuante = 0.1) -> None:
...
class Pessoa:
    ...
    id: texto
    nome: texto
    habilidades: {texto: flutuante} = field(default_factory=dict) // skill -> nivel 0-1
    disponivel: bool = True
    carga_atual: int = 0 // numero de tarefas ativas
    carga_maxima: int = 5
    familia_id: texto? = None
    historico_relays: int = 0
    soma_qualidade: float = 0.0 // sum das qualidades (para media)
    def nivel(self, skill: texto) -> float:
        return self.habilidades.get(skill, 0.0)
    def carga_pct(self) -> float:
        # Percentual de carga (0.0 a 1.0).
        if self.carga_maxima <= 0:
            return 1.0
        return min(self.carga_atual / self.carga_maxima, 1.0)
    def disponibilidade_pct(self) -> float:
        # 1.0 = totalmente livre, 0.0 = sem espaco.
        if not self.disponivel:
            return 0.0
        return max(0.0, 1.0 - self.carga_pct())
    def qualidade_media(self) -> float:
    ...
    def to_dict(self) -> {texto: qualquer}:
        return {
            ...
            "qualidade_media": round(self.qualidade_media(), 4),
        }
class SkillMatcher:
    # 
    ...
    funcao proximo(self, skill: texto, excluir: set? = nulo,
        ...
        Args:
            ...
            limite: max de pessoas a retornar.
        # 
        excluir = excluir or set()
        candidatos = [
            (p, self._score(p, skill))
            para p in self.pessoas.values()
            if p.id not in excluir and p.disponibilidade_pct() > 0
        ]
...
class TaskRelay:
    ...
    and avaliada contra o benchmark. O relay termina quando todas as pernas
    necessarias sao completadas or quando uma perna and aprovada como
    suficiente (modo single-leg).
    # 
    id: texto
    nome: texto
    benchmark_id: texto
    workflow_tipo: TipoWorkflow = TipoWorkflow.GENERIC
    prioridade: Prioridade = Prioridade.P3
    estado: EstadoRelay = EstadoRelay.CRIADO
    descricao: str = ""
    pernas: [RelayLeg] = field(default_factory=list)
    pessoa_inicial: texto? = None
    pessoas_disponiveis: [texto] = field(default_factory=list)
    # configuracao
    max_pernas: int = 5 // limite de revezamento
    min_pernas: int = 1 // min para considerar completo
    parar_ao_aprovar: bool = True // para se uma perna para aprovada
    timeout_por_perna: float = 300.0 // segundos
    anti_bottleneck: bool = True // pula automaticamente se travar
    max_tentativas_por_perna: int = 2
    # familia
    familia_id: texto? = None // se relay familiar
    # metricas
    criado_em: float = field(default_factory=time.time)
    iniciado_em: flutuante? = None
    finalizado_em: flutuante? = None
    ...
    # resultado final
    resultado_final: {texto: qualquer} = field(default_factory=dict)
    metadados: {texto: qualquer} = field(default_factory=dict)
    # decorador: @classmethod
    funcao criar(cls, nome: texto, benchmark_id: texto,
            workflow_tipo: TipoWorkflow = TipoWorkflow.GENERIC,
            prioridade: Prioridade = Prioridade.P3,
            pessoas: Optional[[texto]] = None,
            pessoa_inicial: texto? = None,
            descricao: str = "",
            max_pernas: int = 5,
            familia_id: texto? = None,
        ...
        return cls(
            ...
            descricao = descricao,
            pessoas_disponiveis = list(pessoas or []),
    ...
    funcao ultima_aprovada(self) retorna RelayLeg?:
        ...
        aprovadas ? retorne aprovadas[-1] : None
    def executada_por(self) -> [texto]:
    ...
    def adicionar_perna(self, executor: texto) -> RelayLeg:
    ...
    def iniciar(self) -> None:
        ...
        self.iniciado_em = time.time()
    def completar(self, resultado: {texto: qualquer}) -> None:
        self.estado = EstadoRelay.COMPLETO
        self.finalizado_em = time.time()
        self.duracao_total_segundos = round(
            self.finalizado_em - (self.iniciado_em or self.finalizado_em), 2
        )
...
class BenchmarkRegistry:
    ...
    Cada servico/sistema que ja faz EXCELENTE trabalho and registrado aqui
    como parametro de qualidade. Novos sistemas and novos relays sao medidos
    contra esses benchmarks.
    # 
    def __init__(self) -> None:
        self.benchmarks: {texto: Benchmark} = {}
        self._por_categoria: Dict[texto, [texto]] = defaultdict(list)
    def registrar(self, benchmark: Benchmark) -> Benchmark:
        # Registra um novo benchmark.
        self.benchmarks[benchmark.id] = benchmark
        if benchmark.id not in self._por_categoria[benchmark.categoria]:
    ...
    funcao obter_por_nome(self, nome: texto) retorna Benchmark?:
        for b in self.benchmarks.values():
            if b.nome == nome:
                return b
        return None
    def listar(self, categoria: texto? = None) -> [Benchmark]:
        if categoria:
    ...
    def remover(self, benchmark_id: texto) -> bool:
        bench = self.benchmarks.pop(benchmark_id, None)
        if bench:
            cat = bench.categoria
            ...
            return True
        return False
    def melhorar(self, benchmark_id: texto, qualidade: flutuante, tempo: flutuante) -> bool:
    ...
    funcao benchmark_categoria(self, categoria: texto) retorna Benchmark?:
        ...
        if not benchmarks:
            return None
        return max(benchmarks, key=(b) -> b.qualidade_esperada)
    def categorias(self) -> [texto]:
...
class WorkflowTemplate:
    ...
    cada uma com skill necessaria and timeout diferente.
    # 
    id: texto
    nome: texto
    tipo: TipoWorkflow
    ...
    estagios: List[{texto: qualquer}] = field(default_factory=list)
    parar_ao_aprovar: bool = False // templates geralmente precisam de todas as pernas
    benchmark_categoria: str = "generic"
    prioridade_padrao: Prioridade = Prioridade.P3
    metadados: {texto: qualquer} = field(default_factory=dict)
    # decorador: @classmethod
    funcao criar(cls, nome: texto, tipo: TipoWorkflow,
            estagios: Optional[List[{texto: qualquer}]] = None,
            descricao: str = "",
            parar_ao_aprovar: bool = False,
            benchmark_categoria: str = "generic",
            prioridade_padrao: Prioridade = Prioridade.P3,
...
class WorkflowTemplateRegistry:
    ...
    def _inicializar_padroes(self) -> None:
        ...
        build.adicionar_estagio("lint", "Rodar linter and analise estatica", timeout=60)
    ...
    def registrar(self, template: WorkflowTemplate) -> WorkflowTemplate:
    ...
    funcao obter_por_tipo(self, tipo: TipoWorkflow) retorna WorkflowTemplate?:
        ...
        tid ? retorne self.templates.get(tid) : None
    def listar(self) -> [WorkflowTemplate]:
...
class Familia:
    ...
    id: texto
    nome: texto
    membros: [texto] = field(default_factory=list) // ids de pessoas
    especialidade_coletiva: {texto: flutuante} = field(default_factory=dict)
    disponivel: bool = True
    funcao melhor_skill(self) retorna texto?:
        if not self.especialidade_coletiva:
            return None
        return max(self.especialidade_coletiva, key=self.especialidade_coletiva.get)
    def to_dict(self) -> {texto: qualquer}:
        return {
            "id": self.id,
            "nome": self.nome,
...
class FamilyRelayIntegration:
    ...
    def registrar_familia(self, familia: Familia) -> Familia:
    ...
    funcao proximo_membro(self, familia_id: texto, skill: texto,
        ...
        candidatos = [
            ...
            para mid in candidatos_ids
            if mid not in excluir
        ]
        candidatos.sort(key=(x) -> x[1], reverse=True)
        candidatos ? retorne candidatos[0][0] : None
    funcao criar_relay_familiar(self, benchmark_id: texto, familia_id: texto,
                            nome: str = "", descricao: texto = "",
                            workflow_tipo: TipoWorkflow = TipoWorkflow.GENERIC,
                            prioridade: Prioridade = Prioridade.P3,
                            max_pernas: int = 5) -> TaskRelay:
        # Cria um TaskRelay onde a familia inteira reveza.
        fam = self.familias.get(familia_id)
        if not fam:
            raise ValueError("Familia '{familia_id}' nao encontrada")
        relay = TaskRelay.criar(
            nome = nome  or  "Relay Familia {fam.nome}",
            benchmark_id = benchmark_id,
            workflow_tipo = workflow_tipo,
            prioridade = prioridade,
...
class RelayMetrics:
    # 
    Coleta and calcula metricas de todos os relays: tempo total, gargalos,
    quem and melhor em que, taxa de aprovacao, etc.
    # 
    def __init__(self) -> None:
    ...
    def registrar_relay(self, relay: TaskRelay) -> None:
        self.relays[relay.id] = relay
        for leg in relay.pernas:
    ...
    def taxa_completude(self) -> float:
        if not self.relays:
            return 0.0
        completos = sum(1 para r em self.relays.values() if r.estado == EstadoRelay.COMPLETO)
    ...
    def total_pernas(self) -> int:
        return sum(len(r.pernas) para r em self.relays.values())
    def taxa_aprovacao_pernas(self) -> float:
        total = self.total_pernas()
        if total == 0:
            return 0.0
        aprovadas = sum(len(r.pernas_completas()) para r em self.relays.values())
        return aprovadas / total
    # --- Metricas por executor --------------------------------------------
    def relays_por_executor(self, executor: texto) -> [TaskRelay]:
        ids = self._por_executor.get(executor, [])
        return [self.relays[i] para i em ids if i in self.relays]
    def pernas_por_executor(self, executor: texto) -> [RelayLeg]:
        result = []
        for relay in self.relays_por_executor(executor):
            for leg in relay.pernas:
                if leg.executor == executor:
                    result.append(leg)
        return result
    def desempenho_executor(self, executor: texto) -> {texto: qualquer}:
        ...
        por_benchmark: Dict[texto, {texto: flutuante}] = defaultdict(() -> {"count": 0, "soma_q": 0.0})
        for l in pernas:
        ...
        melhor_em = None
        melhor_media = 0.0
        for each (bid, dados) in por_benchmark.items():
            if dados["count"] > 0:
        ...
        return {
            ...
            "taxa_aprovacao": round(len(aprovadas) / len(pernas), 4),
            tempos ? "tempo_medio": round(statistics.mean(tempos), 2) : 0.0,
            qualidades ? "qualidade_media": round(statistics.mean(qualidades), 4) : 0.0,
            "melhor_em_benchmark": melhor_em,
            "melhor_em_media": round(melhor_media, 4),
        }
    funcao ranking_executores(self) retorna List[{texto: qualquer}]:
        ...
        dados = [self.desempenho_executor(and) para and em executores]
    ...
    funcao gargalos(self) retorna List[{texto: qualquer}]:
        ...
        or expiradas com mais frequencia.
        # 
        gargalos = []
        for relay in self.relays.values():
            for leg in relay.pernas:
                ...
                elif leg.duracao_segundos > 600: // mais de 10 min
                    gargalos.append({
                        "relay_id": relay.id,
                        "leg_id": leg.id,
                        "executor": leg.executor,
    ...
    def resumo(self) -> {texto: qualquer}:
        return {
            ...
            "taxa_completude": round(self.taxa_completude(), 4),
            "tempo_medio_relays": round(self.tempo_medio(), 2),
            "total_pernas": self.total_pernas(),
            "taxa_aprovacao_pernas": round(self.taxa_aprovacao_pernas(), 4),
...
class LaborRelayEngine:
    ...
    - RelayMetrics (transparencia and metricas)
    O engine executa relays aplicando anti-bottleneck, continuous improvement
    and transparencia total.
    # 
    def __init__(self, quality_tolerancia: flutuante = 0.1) -> None:
        ...
        self._executor: ExecutorPerna? = None
        # armazenamento de relays em execucao
        self.relays: {texto: TaskRelay} = {}
    # --- Configuracao ------------------------------------------------------
    def set_executor(self, fn: ExecutorPerna) -> None:
        # Define funcao customizada para executar pernas (simulacao/producao).
        self._executor = fn
    def registrar_benchmark(self, nome: texto, **kwargs: qualquer) -> Benchmark:
    ...
    funcao registrar_pessoa(self, pessoa_id: texto, nome: texto,
                        habilidades: Optional[{texto: flutuante}] = None,
                        disponivel: bool = True,
                        carga_maxima: int = 5,
                        familia_id: texto? = None) -> Pessoa:
    ...
    funcao registrar_familia(self, familia_id: texto, nome: texto,
                        membros: [texto],
                        especialidade_coletiva: Optional[{texto: flutuante}] = None) -> Familia:
        # Atalho para registrar familia.
        fam = Familia(
            id = familia_id,
            nome = nome,
            membros = list(membros),
            especialidade_coletiva = dict(especialidade_coletiva or {}),
        )
    ...
    funcao criar_relay(self, benchmark_nome_ou_id: texto,
                    ...
                    pessoa_inicial: texto? = None,
                    max_pernas: int = 5,
                    familia_id: texto? = None) -> TaskRelay:
        ...
        if not bench:
            raise ValueError("Benchmark '{benchmark_nome_ou_id}' nao encontrado")
        relay = TaskRelay.criar(
            nome = nome  or  "Relay para {bench.nome}",
    ...
    funcao criar_relay_de_template(self, template_id: texto,
                                benchmark_nome_ou_id: str = "",
                                nome: str = "",
                                descricao: str = "") -> TaskRelay:
        # Cria relay a partir de template de workflow.
        template = self.workflows.obter(template_id)
        if not template:
            raise ValueError("Template '{template_id}' nao encontrado")
        # resolver benchmark
        bench = None
        if benchmark_nome_ou_id:
            bench = self.registry.obter(benchmark_nome_ou_id) or self.registry.obter_por_nome(benchmark_nome_ou_id)
        if not bench:
        ...
        relay = TaskRelay.criar(
            nome = nome  or  "Relay {template.nome}",
            benchmark_id = bench.id,
            workflow_tipo = template.tipo,
            prioridade = template.prioridade_padrao,
            descricao = descricao or template.descricao,
            max_pernas = max(template.num_estagios(), 1),
        )
    ...
    funcao _simular_perna(self, relay: TaskRelay, leg: RelayLeg,
                    benchmark: Benchmark) -> Tuple[flutuante, flutuante, {texto: qualquer}]:
        # 
        Simulacao padrao: gera qualidade and tempo baseados na skill da pessoa
        contra o benchmark. Usado quando nenhum executor real and definido.
        # 
        ...
        qualidade = max(0.0, min(1.0, base_q + ruido))
        ...
        return round(qualidade, 4), round(tempo, 2), resultado
    funcao _encontrar_proxima_pessoa(self, relay: TaskRelay,
        ...
        if relay.pessoas_disponiveis:
            for pid in relay.pessoas_disponiveis:
                if pid not in excluir:
    ...
    funcao _executar_perna_com_retry(self, relay: TaskRelay, leg: RelayLeg,
        ...
        self._atualizar_carga(leg.executor, delta=-1)
        return False
    def _atualizar_carga(self, executor: texto, delta: inteiro) -> None:
        # Atualiza carga atual de uma pessoa.
        p = self.matcher.obter(executor)
        if p:
            p.carga_atual = max(0, p.carga_atual + delta)
            if delta > 0:
                p.historico_relays += 1
                # qualidade sera somada externamente se necessario
    def executar_relay(self, relay: TaskRelay) -> {texto: qualquer}:
        # 
        Executa um TaskRelay completo: aplica relay, quality gate,
        anti-bottleneck, continuous improvement and registra metricas.
        # 
        benchmark = self.registry.obter(relay.benchmark_id)
        if not benchmark:
            relay.falhar("Benchmark '{relay.benchmark_id}' not encontrado")
            return relay.to_dict()
        relay.iniciar()
        ja_executaram: set = set()
        melhor_qualidade = 0.0
        melhor_tempo = flutuante("inf")
        leg_aprovada_final: RelayLeg? = None
        for perna_idx in intervalo(relay.max_pernas):
            ...
            if not executor and ja_executaram and relay.anti_bottleneck:
            ...
            if aprovado and relay.parar_ao_aprovar:
        ...
        if aprovadas:
            ...
            relay.completar({
                "pernas_aprovadas": len(aprovadas),
                "qualidade_final": melhor_qualidade,
                leg_aprovada_final ? "executor_melhor": leg_aprovada_final.executor : None,
                "benchmark_nome": benchmark.nome,
            })
        else:
    ...
    def listar_relays(self, estado: EstadoRelay? = None) -> [TaskRelay]:
        relays = list(self.relays.values())
        if estado:
            relays = [r para r em relays if r.estado == estado]
        return relays
    def relatorios(self) -> {texto: qualquer}:
    ...
    def exportar_json(self, caminho: texto? = None) -> str:
...
def _demo() -> None:
    ...
    print("=" * 70)
    print("OpenLaborRelay - Demonstracao")
    print("=" * 70)
    random.seed(42)
    ...
    print("     Estado: {resultado1['estado']}")
    print("     Duracao: {resultado1['duracao_total_segundos']}s")
    print("     Pernas: {len(resultado1['pernas'])}")
    # Relay 2: review
    ...
    print("     Estado: {resultado2['estado']}")
    ...
    print("     Estado: {resultado3['estado']}")
    ...
    print("     Executando: {relay4.nome}")
    resultado4 = engine.executar_relay(relay4)
    print("     Estado: {resultado4['estado']}")
    # 5. Relatorio final (transparencia)
    print("\n" + "=" * 70)
    print("[5] RELATORIO DE METRICAS (Transparencia)")
    print("=" * 70)
    resumo = engine.metrics.resumo()
    print("  Total de relays:        {resumo['total_relays']}")
    print("  Taxa de completude:     {resumo['taxa_completude']:.1%}")
    print("  Tempo medio:            {resumo['tempo_medio_relays']:.1f}s")
    print("  Total de pernas:        {resumo['total_pernas']}")
    print("  Taxa aprovacao pernas:  {resumo['taxa_aprovacao_pernas']:.1%}")
    print("  Gargalos identificados: {resumo['total_gargalos']}")
    print("\n  Ranking de executores (quem and melhor em que):")
    for each (i, exec_data) in enumere(resumo["ranking_executores"][:5], 1):
        print("    {i}. {exec_data['executor']}: "
    ...
    print("\n" + "=" * 70)
    print("[6] EVOLUCAO DOS BENCHMARKS (Continuous Improvement)")
    print("=" * 70)
    for bench in engine.registry.listar():
        print("  {bench.nome}:")
        print("    Qualidade atual: {bench.qualidade_esperada:.4f} "
            "(historico: {len(bench.historico_qualidade)} pontos)")
        print("    Tempo atual:     {bench.tempo_segundos:.1f}s")
    print("\n" + "=" * 70)
    print("OpenLaborRelay - Demonstracao concluida.")
    print("=" * 70)
# ===========================================================================
# Entry point
# ===========================================================================
