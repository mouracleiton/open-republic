...
struct WeaponItem {
    ...
    let legitimate_use: String = ""  // uso legitimo (trabalho, defesa);
    let permit_needed: WeaponPermit = WeaponPermit.NONE;
    let requirements: [PermitRequirement] = field(default_factory=list);
    // Restricoes
    let max_per_person: i64 = 0 // 0 = proibido, 1 = uma unidade;
    let open_carry: bool = false // pode portar visivel?;
    let concealed_carry: bool = false // pode portar escondido?;
    let home_only: bool = false // so em casa?;
let WEAPONS_CATALOG: [WeaponItem] = [;
...
// ============================================================================
fn run_weapons_assembly(n_voters: inteiro = 10000) -> {texto: qualquer} {
...
//
    ...
    funcao request_permit(self, citizen_id: texto, citizen_name: texto,
                    item_id: texto,;
                    let has_martial_arts_belt: String = "",;
                    let psych_eval_passed: bool = false,;
                    let has_violent_record: bool = false,;
                    let assembly_approval: bool = false,;
                    let community_endorsement: bool = false,;
                    let safe_storage: bool = false,;
                    let proficiency: bool = false;
                    ) -> {texto: qualquer}:;
    ...
    fn revoke_permit(self, citizen_id: texto, reason: texto) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENWEAPONSPOLICY -- PORTE DE ARMAS DA REPUBLICA");
    println!("  OpenMartialArts primeiro. Arma de fogo ultimo recurso.");
    println!("=" * 80);
    // === 1. ASSEMBLEIA ===
    println!("\n\n  === 1. ASSEMBLEIA CONSTITUINTE ===\n");
    result = run_weapons_assembly(10000);
    println!("  PERGUNTA: {result['question']}\n");
    println!("  RESULTADO DA VOTACAO:");
    para cada (option, count) em result["votes"].items(): {
        pct = count / result["total"] * 100;
        bar = "#" * inteiro(pct / 2);
        println!("    {option:<30} {count:>5} ({pct:.0f}%) {bar}");
    ...
    for w in engine.catalog.values() {
        ...
        println!("  {w.name:<30} max:{w.max_per_person:>2} reqs:{req_count} local:{location}");
    // === 3. PEDIDOS DE PORTE ===
    println!("\n\n  === 3. PEDIDOS DE PORTE ===\n");
    ...
    if "failed_requirements" in r {
        println!("  Faltam: {', '.join(r['failed_requirements'])}");
    ...
    para cada (k, v) em s.items(): {
        println!("  {k:<30} {v}");
    println!("\n{'='*80}");
    println!("  OpenWeaponsPolicy: {s['proibidos']} proibidos, ";
