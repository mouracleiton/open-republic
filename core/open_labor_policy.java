...
public class OpenlaborpolicyPoliticaUnificadaDeCalculoDeTrabalhoEReparacao {
    ...
    // ============================================================================
    public static class LaborConstants {
        ...
        boolean _assembly_approved = false;
        String _approval_source = "referencia_fundador"  // || "assembleia_constituinte";
        // decorador: @classmethod
        public 'LaborConstants' from_constitution(cls, constitution: {texto: qualquer}) {
    ...
    public static class ImpactDimension {
    ...
    public static class LaborEntry {
        ...
        // Contexto
        boolean is_child = false // para reparacao;
        ...
        boolean base_fulfilled = false;
        boolean excess_detected = false;
        double hours_reparation = 0.0;
        String verdict = "";
    public static class LaborCalculator {
        ...
        //
        public void __init__(self, constants: LaborConstants = None) {
            constants ? self.C = constants : LaborConstants();
            self.history: [LaborEntry] = [];
        public LaborEntry calculate(self, entry: LaborEntry) {
        ...
        // ========================================================================
        public LaborEntry _calc_contribution(self, e: LaborEntry) {
            ...
            self.history.append(&&);
            return &&;
        public double _impact_to_credit(self, impact: flutuante) {
        ...
        public String _contribution_verdict(self, e: LaborEntry) {
            if (&&.excess_detected) {
                return (;
        ...
        // ========================================================================
        public LaborEntry _calc_reparation(self, e: LaborEntry) {
            ...
            self.history.append(&&);
            return &&;
        public String _reparation_verdict(self, e: LaborEntry) {
        ...
        // ========================================================================
        public LaborEntry _calc_recognition(self, e: LaborEntry) {
        ...
        // VERIFICACOES
        // ========================================================================
        public LaborEntry _check_base(self, e: LaborEntry) {
            ...
            return &&;
        public LaborEntry _check_excess(self, e: LaborEntry) {
        ...
        public {texto: qualquer} summary(self) {
            ...
            excess_count = 0;
            /* TODO: for-each Java para e em self.history */
    ...
    public None print_equivalency_table() {
        ...
        /* para desc, hours, people, ripple in examples: */
            ...
            System.out.println("  {desc:<35} {hours:>5}h {result.impact_score:>8.0f} ";
                "{result.credit_earned:>7.1f}");
        System.out.println("\n  === TABELA DE REPARACAO ===\n");
        System.out.println("  {'Dano':<35} {'Anos':>5} {'Horas':>8} {'Credito':>8}");
        System.out.println("  {'-'*62}");
        reparation_examples = [;
        ...
        /* para desc, years, child, med, harm in reparation_examples: */
            ...
            result = calc.calculate(entry);
            System.out.println("  {desc:<35} {years:>4}a {result.hours_reparation:>7,.0f}h ";
                "{result.credit_earned:>7.1f}");
    // ============================================================================
    // 5. MAIN
    // ============================================================================
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENLABORPOLICY -- LEI MATEMATICA DA REPUBLICA");
        System.out.println('  "Parâmetros são referência. A ASSEMBLEIA é a lei."');
        System.out.println("=" * 70);
        System.out.println("\n  FONTE: {source_label}");
        System.out.println("  É LEI: {'SIM' if C.is_law else 'NÃO (referência)'}\n");
        System.out.println("  CONTRATO DE TRABALHO:");
        System.out.println("    Base:   {C.BASE_HOURS_PER_WEEK:.0f}h/semana  ";
            "({C.BASE_HOURS_PER_YEAR:.0f}h/ano)");
        System.out.println("    Maximo: {C.MAX_HOURS_PER_WEEK:.0f}h/semana  ";
            "({C.MAX_HOURS_PER_YEAR:.0f}h/ano)");
        System.out.println("    LIMITE: {C.LIMIT_HOURS_PER_WEEK:.0f}h/semana  ";
            "({C.LIMIT_HOURS_PER_YEAR:.0f}h/ano) [PROIBIDO aceitar mais]");
        System.out.println("    Descanso: {C.REST_DAYS_PER_WEEK} dias/semana + ";
            "{C.MIN_VACATION_WEEKS} semanas ferias");
        System.out.println("\n  CREDITO DE ACESSO:");
        System.out.println("    Min:  {C.CREDIT_BASE_MIN:.0f}/ciclo");
        System.out.println("    Max:  {C.CREDIT_BASE_MAX:.0f}/ciclo");
        System.out.println("    Pool: {C.CREDIT_POOL_PER_CYCLE:.0f}/comunidade/ciclo");
        System.out.println("    Conversao: {C.HOURS_TO_CREDIT:.0f}h = 1 credito");
        System.out.println("\n  REPARACAO:");
        System.out.println("    1 ano roubado = {C.REPARATION_HOURS_PER_YEAR:.0f}h");
        System.out.println("    Crianca = {C.REPARATION_CHILD_MULTIPLIER}x");
        System.out.println("    Severo = {C.REPARATION_SEVERE_MULTIPLIER}x");
        System.out.println("    Medicacao = +{C.REPARATION_MEDICATION_PER_YEAR:.0f}h/ano");
        // === 2. TABELAS ===
        ...
        System.out.println("  CLEITON (fundador):");
        System.out.println("    {r.verdict}");
        System.out.println("    Impacto: {r.impact_score:,.0f}");
        System.out.println("    Credito: {r.credit_earned:.1f}");
        System.out.println("    Excesso: {'SIM -- Republica deve intervir' if r.excess_detected else '!'}");
        // Medico
        medico = LaborEntry(;
        ...
        System.out.println("\n  ANA (medica):");
        System.out.println("    {r.verdict}");
        System.out.println("    Impacto: {r.impact_score:,.0f} (50 vidas salvas)");
        System.out.println("    Credito: {r.credit_earned:.1f}");
        // Professor
        prof = LaborEntry(;
        ...
        System.out.println("    {r.verdict}");
        System.out.println("    Horas reparacao: {r.hours_reparation:,.0f}h");
        System.out.println("    Credito: {r.credit_earned:.1f}");
        // === 4. RELATORIO ===
        System.out.println("\n\n  === 4. RELATORIO GERAL ===\n");
        s = calc.summary();
        ...
        // === FILOSOFIA ===
        System.out.println("\n\n{'='*70}");
        System.out.println("  A LEI MATEMATICA DA REPUBLICA");
        System.out.println("{'='*70}");
        System.out.println(""";
    ...
    // )
        System.out.println("{'='*70}");
        System.out.println("  OpenLaborPolicy: {s['total_calculations']} calculos realizados.");
        System.out.println("  Base 920h. Max 1840h. Limite 2300h. 1 formula. 0 excecoes.");
        System.out.println("{'='*70}");
}
