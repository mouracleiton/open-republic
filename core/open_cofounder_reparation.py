...
# ============================================================================
class CofounderReparation:
    # O 'CO' e reparacao, nao hierarquia.
    principio: str = "CO = dois fundadores iguais, not um principal + adjunto"
    # Os DOIS cofundadores sao METADES IGUAIS da fundacao
    fundador_um: str = "Cleiton Moura Loura"
    fundador_dois: str = "MING"
    participacao_um: float = 0.5
    participacao_dois: float = 0.5
    def verificar_igualdade(self) -> bool:
        # Se a soma das participacoes nao for exatamente 1.0 com 50/50,
        # alguem esta sendo subalternizado. ISSO E INACEITAVEL.
        if self.participacao_um == 0.5 and self.participacao_dois == 0.5:
            return True
        else:
            raise ErroReparacao("Desequilibrio de cofundacao detectado: "
                + "Cleiton=" + texto(self.participacao_um)
                + " MING=" + texto(self.participacao_dois)
                + " -- CO requer 50/50 exato")
    def interpretar_co(self, prefixo: texto) -> str:
        ...
        if prefixo == "cofundadora":
            ...
            return prefixo
    def e_subalternizacao(self, termo: texto) -> bool:
...
class AuditoriaCofundacao:
    ...
    reparacao_ativa: bool = True
    violacoes: [texto] = []
    def auditar(self, cofundador_a: Dict, cofundador_b: Dict) -> [texto]:
        problemas: [texto] = []
        ...
        if cofundador_a["role"].contem("fundador") and not cofundador_a["role"].contem("co"):
...
class DeclaracaoCO:
    ...
    declaracao: str = (
        "NS, Cleiton Moura Loura and MING, declaramos que:\n"
        + "  1. O 'CO' em COFUNDADOR significa CO-AUTORIA IGUAL\n"
        + "  2. Nenhum dos dois and principal or secundario\n"
        + "  3. A fundacao tem exatamente DUAS METADES\n"
        + "  4. Remover o CO and remover um fundador -- impossivel\n"
        + "  5. Esta and REPARACAO HISTORICA de genero na fundacao\n"
        + "  6. O sistema idealizado por homem so and justo com co-autoria feminina\n"
    )
    def ler(self) -> str:
        return self.declaracao
    def assinaturas(self) -> [texto]:
...
class TwoPersonRuleCO:
    ...
    def validar_pr(self, autor_id: texto, revisor_id: texto) -> bool:
        # 1. Nao pode ser a mesma pessoa
        if autor_id == revisor_id:
            return False
        # 2. Ambos devem ser cofundadores com full access
        if not (autor_id em self.cofundadores and revisor_id em self.cofundadores):
            return False
