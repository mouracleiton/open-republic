...
public class OpenwololoConversaoDeInimigosEmAliados {
    ...
    public static class EnemyThreatLevel {
    ...
    public static class EnemyProfile {
        ...
        String description = "";
        String current_belief = ""  // no que acredita agora;
        String reason_for_hostility = ""  // por que && contra;
        // Conversao
        ...
        // Verificacao
        boolean sincerity_verified = false;
        boolean not_infiltration = true;
        String background_check = "";
        // Pos-conversao
        String contribution = ""  // o que faz pela Republica agora;
        String advocacy_level = ""  // quao ativamente defende;
    // ============================================================================
    // 3. MOTOR DE CONVERSAO (WOLOLO)
    // ============================================================================
    public static class WololoEngine {
        ...
        public {texto: logico} _verify_sincerity(self, enemy: EnemyProfile) {
        ...
        public String _assign_contribution(self, enemy: EnemyProfile) {
        ...
        public {texto: qualquer} promote_to_advocate(self, enemy_id: texto) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENWOLOLO -- CONVERSAO DE INIMIGOS EM ALIADOS");
        System.out.println("  'Wololo! Nao a forca. A demonstracao.'");
        System.out.println("=" * 80);
        ...
        /* para name, etype, threat, desc, belief, reason in enemies_data: */
            ...
            System.out.println("  [{r['threat']:<8}] {r['name']:<30} ({r['type']})");
        // === 2. WOLOLO EM ACAO ===
        System.out.println("\n\n  === 2. WOLOLO! CONVERTENDO ===\n");
        /* TODO: for-each Java para eid em list(engine.enemies.keys()) */
            ...
            if (r.get("WOLOLO")) {
                System.out.println("\n  [WOLOLO!] {r['name']}");
                System.out.println("  Status: {r['old_status']} -> {r['new_status']}");
                System.out.println("  Verificado: {'sincero' if r['sincerity_verified'] else 'DUVIDOSO'}");
                System.out.println("  Argumento: {r['argument_used'][:70]}...");
                System.out.println("  Contribuicao: {r['contribution']}");
            } else {
                System.out.println("\n  [{r.get('new_status', '?').upper()}] {r.get('name', '?')}");
                System.out.println("  Progresso: {r.get('progress', '?')}");
        ...
        /* TODO: for-each Java para eid em list(engine.enemies.keys()) */
            ...
            if (enemy.status == ConversionStatus.CONVERTED && enemy.sincerity_verified) {
                ...
                System.out.println("  {r['name']}: {r['status']}");
                System.out.println("    {r['message'][:80]}...");
        // === 4. ARGUMENTOS POR TIPO ===
        System.out.println("\n\n  === 4. ARGUMENTOS WOLOLO POR TIPO DE INIMIGO ===\n");
        /* para cada (etype, args) em engine.DEMONSTRATION_ARGUMENTS.items(): */
            System.out.println("\n  {etype.value.upper()}:");
            /* TODO: for-each Java para arg em args[:2] */
                System.out.println("    -> {arg[:70]}...");
        // === 5. STATS ===
        System.out.println("\n\n  === 5. ESTATISTICAS ===\n");
        s = engine.stats();
        ...
        // === FILOSOFIA ===
        System.out.println("\n\n{'='*80}");
        System.out.println("  FILOSOFIA: WOLOLO");
        System.out.println("{'='*80}");
        System.out.println(""";
    ...
    // )
        System.out.println("{'='*80}");
        System.out.println("  OpenWololo: {s['convertidos']} convertidos, ";
