...
class GoodOrService {
    ...
    name: texto;
    tier: AccessTier;
    const credit_cost = 0.0 // 0 para garantidos;
    const description = "";
    const category = "";
    const scarce = false // oferta limitada?;
...
class CreditAccount {
    ...
    citizen_id: texto;
    name: texto;
    const current_credits = 0.0;
    const credits_this_cycle = 0.0;
    const credits_spent = 0.0;
    const credits_expired = 0.0 // ! usados, expiraram;
    const contribution_hours = 0.0;
    const contributions = field(default_factory=list);
    const community_recognition = 0.0 // avaliacao pelos pares;
    const cycle = 0 // ciclo atual;
    // decorador: @property
    can_spend(self) {
        // Quanto pode gastar neste ciclo.
        return maximo(0, self.current_credits);
    spend(self, amount: flutuante, good_name: texto) {
...
class Citizen {
    ...
    name: texto;
    age: inteiro;
    const skills = field(default_factory=list);
    const account = field(default_factory=() -> CreditAccount("", ""));
class DemocraticCreditSystem {
    ...
    //
    __init__(self, pool_per_cycle: flutuante = 1000.0) {
    ...
    add_citizen(self, citizen: Citizen) {
    ...
    funcao register_contribution(self, citizen_id: texto,
                            ...
                            const people_impacted = 1,;
                            const ripple_factor = 1.0):;
    ...
    run_cycle(self) {
        ...
        result = {
            ...
            "proposals_voted": .length(proposals),;
        ...
        return result;
    _generate_proposals(self, contributions: Dict, total_weighted: flutuante) {
        ...
        return proposals;
    _simulate_vote(self, proposals: [Dict]) {
    ...
    access_good(self, citizen_id: texto, good_name: texto) {
...
if (__name__ == "__main__") {
    random.seed(42);
    console.log("=" * 75);
    console.log("  OPENCREDIT -- CREDITO DEMOCRATICO DE ACESSO");
    console.log("  'Nao && moeda. E acesso. Nao acumula. A comunidade decide.'");
    console.log("=" * 75);
    ...
    // === Catalog ===
    console.log("\n\n  === CATALOGO DE BENS ===\n");
    console.log("  DIREITOS GARANTIDOS (sem credito, sempre):");
    ...
    for (const cycle_num of intervalo(1, 4)) {
        ...
        for (const c of system.citizens.values()) {
        ...
        console.log("    Proposta vencedora: {result['winning_proposal']}");
        console.log("    {result['winning_description'][:70]}...");
        console.log("    Pool: {result['pool_total']:.0f} | Cidadaos: {result['citizens']}");
        console.log("    Min: {result['min_given']} | Max: {result['max_given']} ";
            "| Media: {result['avg_given']}");
        console.log("    Expirado ciclo anterior: {result['expired_from_last_cycle']}");
        ...
        para cada (name, credits) em ordene(result["distribution"].items()): {
            ...
            console.log("    {name:<10} {c.account.contribution_hours:>5.0f}h {credits:>8.1f}");
        ...
        for (const c of spenders) {
            ...
            console.log("      {c.name:<10} -> {good.name[:25]:<25} ";
                "custo={cost:>3} restante={remaining:>5.1f} [{status}]");
        console.log();
    // === Explicar por que NAO e moeda ===
    console.log("\n{'='*75}");
    console.log("  POR QUE ISSO NAO E DINHEIRO");
    console.log("{'='*75}");
    console.log(""";
...
// )
    // === Resumo final ===
    console.log("{'='*75}");
    console.log("  RESUMO DO OPENCREDIT");
    console.log("{'='*75}\n");
    console.log("  1. ESSNCIAIS sao DIREITOS (sem credito):");
    console.log("     Comida, agua, moradia, saude, educacao, transporte");
    console.log("     -> Voce ACESSE. Nao compra. Nao gasta credito.\n");
    console.log("  2. BENS ESCASSOS precisam de CREDITO:");
    console.log("     Instrumentos, viagens, eletronicos especiais");
    console.log("     -> Voce gasta credito de acesso do ciclo\n");
    console.log("  3. CREDITO && distribuido por TODOS:");
    console.log("     A comunidade vota como dividir o pool");
    console.log("     Base igual para todos + bonus por contribuicao");
    console.log("     -> Quem contribui mais, tem mais acesso ao que && escasso\n");
    console.log("  4. CREDITO NAO E MOEDA:");
    console.log("     Expira todo ciclo (! acumula)");
    console.log("     E pessoal (! transfere)");
    console.log("     Tem piso (ninguem fica sem) && teto (ninguem domina)");
    console.log("     Nao compra essencial (essencial && direito)\n");
    console.log("  5. CONTRIBUICAO && reconhecida pela comunidade:");
    console.log("     Nao && 'salario'. E reconhecimento.");
    console.log("     Saude, educacao && comida valem mais que arte.");
    console.log("     Mas arte tambem conta.\n");
    console.log("  RESULTADO:");
    console.log("  Ninguem passa fome porque 'acabou o credito'.");
    console.log("  Ninguem acumula fortuna porque 'trabalhou mais'.");
    console.log("  Todos tem o essencial garantido.");
    console.log("  O escasso && distribuido democraticamente.");
    console.log("  A cada ciclo, recomeca. Sem aculumulo. Sem heranca.");
    console.log("  Sem classe. Sem exploracao. Sem dominacao.\n");
    console.log("{'='*75}");
