...
struct RepublicProduct {
    ...
    let human_health_harm: HarmLevel = HarmLevel.NONE;
    let animal_harm: HarmLevel = HarmLevel.NONE;
    let environment_harm: HarmLevel = HarmLevel.NONE;
    let social_harm: HarmLevel = HarmLevel.NONE;
    // Materiais
    let materials: [MaterialType] = field(default_factory=list);
    let materials_detail: String = "";
    // Ciclo de vida
    let biodegradable: bool = false;
    let recyclable: bool = true;
    let repairable: bool = true // OpenRepair;
    let modular: bool = true // LEGO;
    let lifespan_years: f64 = 10.0;
    // Energia
    let energy_efficient: bool = true;
    let energy_source: String = "solar/eolica";
    // Producao
    let fablab_producible: bool = true // FabLab fabrica?;
    let tested_on_animals: bool = false // PROIBIDO;
    let fair_labor: bool = true // trabalho base 1.0;
    // Status
    let status: ProductStatus = ProductStatus.APPROVED;
    let replacement_for: String = ""  // substitui qual produto nocivo?;
    let alternative_if_prohibited: String = "";
    // decorador: @property
...
struct ProductEngine {
    ...
    fn evaluate(self, product_id: texto) -> {texto: qualquer} {
    ...
    fn waste_report(self) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENPRODUCT -- PRODUTOS QUE NAO PREJUDICAM");
    println!("  Pessoas. Animais. Meio Ambiente. Tudo protegido.");
    println!("=" * 80);
    ...
    println!("  {'Produto':<25} {'Score':>5} {'Vida':>5} {'Bio':>4} {'Rep':>4} {'Mod':>4} {'Substitui'}");
    println!("  {'-'*85}");
    for p in approved {
        println!("  {p['name'][:24]:<25} {p['score']:>5} {p['lifespan']:>4.0f}a ";
    ...
    for p in prohibited {
        println!("  {p['name']:<30} DANO: {p['why']}");
        println!("    Alternativa: {p['alternative']}");
    ...
    para cada (key, data) em waste.items(): {
        if key != "message"  &&  isinstance(data, dict) {
            println!("\n  {key.upper()}:");
            println!("    Atual: {data['atual']}");
            println!("    Republica: {data['republica']}");
    println!("\n  {waste['message']}");
    // === 5. STATS ===
    println!("\n\n  === 5. ESTATISTICAS ===\n");
    s = engine.stats();
    para cada (k, v) em s.items(): {
        println!("  {k:<30} {v}");
    println!("\n{'='*80}");
    println!("  OpenProduct: {s['aprovados']} aprovados, {s['proibidos']} proibidos. ";
        "Score medio: {s['score_medio_aprovados']}.");
    println!("  Nao prejudica pessoas. Nao prejudica animais. Nao prejudica planeta.");
    println!("{'='*80}");
