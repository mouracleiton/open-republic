...
class DeterministicNarrative {
    ...
    const real_example = ""  // exemplo real de mudanca;
// As 12 narrativas
const NARRATIVES = [;
...
class AntiDeterminismEngine {
    ...
    fact_check_determinism(self, phrase: texto) {
        // Verifica se frase e determinista e corrige.
        phrase_lower = phrase.lower();
        for (const narrative of self.narratives.values()) {
    ...
    get_narrative(self, determinism_type: DeterminismType) {
        // Retorna a verdade sobre um tipo de determinismo.
        for (const n of self.narratives.values()) {
    ...
    stats(self) {
        return {;
            "narrativas_derrubadas": .length(self.narratives),;
            "principio": "O passado && contexto. O futuro && escolha.",;
            "sistemas_integrados": .length(set(;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENANTIDETERMINISM -- O FUTURO NAO ESTA ESCRITO");
    console.log("  O passado && contexto. O futuro && escolha.");
    console.log("=" * 80);
    // === 1. AS 12 MENTIRAS DETERMINISTAS ===
    console.log("\n\n  === 12 NARRATIVAS DETERMINISTAS DERRUBADAS ===\n");
    for (const n of NARRATIVES) {
        console.log("\n  [{n.narrative_id}] {n.determinism_type.value.upper()}");
        console.log("  A MENTIRA: {n.the_lie[:70]}...");
        console.log("  A VERDADE: {n.the_truth[:70]}...");
        console.log("  CIENCIA: {n.scientific_evidence[:70]}...");
        console.log("  SISTEMA: {n.republic_system}");
        console.log("  EXEMPLO: {n.real_example[:70]}...");
    ...
    for (const phrase of test_phrases) {
        ...
        if (result["identified"]) {
            console.log("  Tipo: {result['determinism_type']}");
            console.log("  Verdade: {result['the_truth'][:70]}...");
    // === 3. STATS ===
    console.log("\n\n  === ESTATISTICAS ===\n");
    s = engine.stats();
    ...
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA: ANTI-DETERMINISMO");
    console.log("{'='*80}");
    console.log(""";
...
// )
    console.log("{'='*80}");
    console.log("  OpenAntiDeterminism: {s['narrativas_derrubadas']} narrativas derrubadas.");
