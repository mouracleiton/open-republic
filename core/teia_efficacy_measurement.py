...
class ModelBacktest:
    ...
    def grade(self) -> str:
        if self.error_pct < 5:
            return "A (excelente)"
        elif self.error_pct < 10:
            return "B (bom)"
        elif self.error_pct < 15:
            return "C (aceitavel)"
        elif self.error_pct < 25:
...
BACKTESTS: [ModelBacktest] = [
    ...
    ModelBacktest(
        ...
        actual_outcome = 25_200_000_000, // ainda not implementado, valor projetado
...
class ArtifactQualityScore:
    ...
    4. Painel de 5 avaliadores pontua sem saber qual and qual
    5. Criterios: acuracia, clareza, profundidade, acionabilidade, fontes
    # 
    artifact_id: texto
    artifact_type: texto   // "teia_generated" or "human_made"
    evaluator: texto
    evaluator_background: texto
    # Scores 0-10
    accuracy: float = 0 // dados estao corretos?
    clarity: float = 0 // and legivel?
    depth: float = 0 // tem profundidade analitica?
    actionability: float = 0 // da para tomar decisao com isso?
    sourcing: float = 0 // fontes sao confiaveis and verificaveis?
    completeness: float = 0 // cobre o topico inteiro?
    # decorador: @property
    ...
    def grade(self) -> str:
        ...
        if s >= 9:
            ...
            return "F"
def generate_blind_test_simulation() -> [ArtifactQualityScore]:
    ...
    for each (name, bg) in evaluators:
        para artifact_type, base_scores in [
            ...
            scores = {k: max(0, min(10, v + rng.uniform(-0.8, 0.8)))
...
class UsabilityTest:
    ...
    def usability_score(self) -> float:
        ...
        if not self.completed:
            return 0
        time_score = max(0, 10 - (self.time_minutes - 15) * 0.2) // 15min = 10, +1min = -0.2
        error_score = max(0, 10 - self.errors_count * 1.5)
        quality = self.quality_score
        frustration_score = 10 - (self.frustration_level - 1) * 2
        help_score = max(0, 10 - self.needed_help * 2)
        return (time_score + error_score + quality + frustration_score + help_score) / 5
def simulate_usability_tests() -> [UsabilityTest]:
    ...
    results = []
    for each (tid, profile) in testers:
...
class MarketAcceptanceTest:
    ...
    def nps_category(self) -> str:
        if self.nps_score >= 9:
            ...
            return "DETRATOR"
def simulate_market_tests() -> [MarketAcceptanceTest]:
    ...
    para cid, ctype, org in clients:
        results.append(MarketAcceptanceTest(
            ...
            client_type = ctype,
            client_org = org,
            artifact_shown = "Dossie TEIA: Fome and Saneamento",
...
class LaunchGate(Enum):
    ...
    def __init__(self, label: texto, threshold: flutuante, metric: texto):
        self.label = label
...
class GateResult:
    ...
    passed: logico
    details: str = ""
def evaluate_gates() -> [GateResult]:
    # Avalia todos os gates.
    results = []
    # GATE A: Accuracia de dados
    accuracy_tests = [t para t em ACCURACY_TESTS if "TESTE DE ERRO" not  in t.metric_name]
    accuracy_passed = sum(1 para t em accuracy_tests if t.passed)
    accuracy_rate = accuracy_passed / len(accuracy_tests) * 100
    ...
    # GATE B: Backtesting
    model_passed = sum(1 para b em BACKTESTS if b.passed)
    model_rate = model_passed / len(BACKTESTS) * 100
    results.append(GateResult(
    ...
    teia_avg = teia_scores ? sum(teia_scores) / len(teia_scores) : 0
    human_avg = human_scores ? sum(human_scores) / len(human_scores) : 0
    results.append(GateResult(
    ...
    usability_avg = usability_scores ? sum(usability_scores) / len(usability_scores) : 0
    results.append(GateResult(
    ...
    promoters = sum(1 para m em market if m.nps_score >= 9)
    detractors = sum(1 para m em market if m.nps_score < 7)
    nps = (promoters - detractors) / len(market) * 100
    results.append(GateResult(
...
def print_efficacy_report() -> str:
    ...
    lines.append("")
    # === METODOLOGIA ===
    lines.append("5 FASES OBRIGATORIAS (cada uma and um GATE):")
    lines.append("-" * 110)
    lines.append("")
    ...
    lines.append("")
    lines.append("  Se qualquer GATE not passa -> CORRIGIR antes de avancar.")
    ...
    valid = [t para t em ACCURACY_TESTS if "TESTE DE ERRO" not  in t.metric_name]
    passed = sum(1 para t em valid if t.passed)
    ...
    teia_avg = sum(b.total_score para b em teia) / len(teia)
    human_avg = sum(b.total_score para b em human) / len(human)
    ...
    u_avg = sum(u.usability_score para u em usability) / len(usability)
    ...
    promoters = sum(1 para m em market if m.nps_score >= 9)
    detractors = sum(1 para m em market if m.nps_score < 7)
    neutrals = sum(1 para m em market if 7 <= m.nps_score < 9)
    nps = (promoters - detractors) / len(market) * 100
    avg_pay = sum(m.willingness_to_pay_brl para m em market if m.willingness_to_pay_brl > 0) / max(1, sum(1 para m em market if m.willingness_to_pay_brl > 0))
    ...
    feedbacks_seen = set()
    for m in market:
        if m.feedback not in feedbacks_seen:
    ...
    lines.append("-" * 110)
    lines.append("O QUE FAZER AGORA (not and simulacao):")
    lines.append("-" * 110)
    lines.append("")
    ...
    lines.append("     NPS. Se NPS < 50, not lancar. Ouvir feedback and melhorar.")
    lines.append("")
    lines.append("=" * 110)
    return "\n".join(lines)
# ============================================================================
