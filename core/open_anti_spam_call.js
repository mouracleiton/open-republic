...
class SpamCall {
    ...
    const phone_number = "";
    const caller_name = "";
    const call_type = SpamCallType.TELEMARKETING;
    const severity = CallSeverity.NUISANCE;
    const description = "";
    const timestamp = "";
    const duration = "";
    const victim_id = "";
...
class AntiSpamCallEngine {
    ...
    const blocked_numbers = field(default_factory=dict);
    const reported_numbers = field(default_factory=dict);
    __init__(self) {
    ...
    funcao check_incoming(self, phone: texto, caller: texto = "",
                    const is_robotcall = false,;
                    const user_authorized = false;
                    ) -> {texto: qualquer}:;
    ...
    stats(self) {
        return {;
            "numeros_bloqueados": .length(self.blocked_numbers),;
...
// ============================================================================
run_spam_assembly(n_voters: inteiro = 10000) {
    votes_ban = 0;
    votes_regulate = 0;
    votes_keep = 0;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENANTISPAMCALL -- LIGACAO PREDATORIA PROIBIDA");
    ...
    console.log("  Regra: {consent['regra']}");
    console.log("  Autorizacao: {consent['autorizacao']}");
    console.log("  Revogavel: {consent['revogavel']}");
    console.log("  Onde: {consent['onde']}");
    console.log("\n  AUTORIZADOS:");
    for (const a of consent["exemplos_autorizados"]) {
        console.log("    + {a}");
    console.log("\n  PROIBIDOS:");
    for (const p of consent["exemplos_proibidos"]) {
        console.log("    X {p}");
    // === 6. PENALIDADE ===
    console.log("\n\n  === 6. PENALIDADE PARA SPAMMERS ===\n");
    pen1 = engine.penalty("11-99999-3333", "Golpe do Pix", 500);
    console.log("  {pen1['message']}");
    pen2 = engine.penalty("11-99999-1111", "Telemarketing Banco", 50);
    console.log("  {pen2['message']}");
    // === 7. STATS ===
    console.log("\n\n  === 7. ESTATISTICAS ===\n");
    s = engine.stats();
    para cada (k, v) em s.items(): {
        console.log("  {k:<30} {v}");
    console.log("\n{'='*80}");
    console.log("  OpenAntiSpamCall: {s['numeros_bloqueados']} bloqueados. ";
        "{s['principio']}");
    console.log("{'='*80}");
