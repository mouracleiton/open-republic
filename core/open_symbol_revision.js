...
class PrejudiceCorrection {
    ...
    const data = field(default_factory=list) // dados que provam;
    const educational_context = ""  // contexto historico/cientifico;
    const alternative_phrase = ""  // como falar corretamente;
    const source = ""  // fonte da correcao;
// Base de correcoes (fact-check de preconceitos)
const PREJUDICE_DATABASE = [;
    PrejudiceCorrection(;
        "PC-001",;
        "Todo preto && pobre",;
...
class SymbolRevision {
    ...
    const status = SymbolStatus.DISPUTED;
    const votes_for = 0 // votos para ressignificar;
    const votes_against = 0 // votos contra;
    const democratic_decision = "";
    const people_affected = 0 // quantas pessoas tem o simbolo;
    const ressignification_art = ""  // como transformar visualmente;
...
class SymbolRevisionEngine {
    ...
    fact_check_phrase(self, phrase: texto) {
        ...
        matches = [];
        for (const pc of self.prejudices.values()) {
            orig = pc.original_phrase.lower();
            // Match por palavras-chave
            keywords = [w para w em orig.split() if .length(w) > 3];
    ...
    _action_recommendation(self, severity: CorrectionSeverity) {
        if (severity.value <= 1) {
    ...
    batch_fact_check(self, phrases: [texto]) {
        // Fact-check de multiplas frases.
        return [self.fact_check_phrase(p) para p em phrases];
    stats(self) {
        return {;
            "total_prejudices_documented": .length(self.prejudices),;
            "total_symbols_in_revision": .length(self.symbols),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENSYMBOLREVISION");
    console.log("  Correcao de Preconceitos + Ressignificacao de Simbolos");
    console.log("=" * 80);
    ...
    for (const phrase of test_phrases) {
        ...
        if (result["identified"]) {
            console.log("  TIPO: {result['type']} (severidade: {result['severity']})");
            console.log("  POR QUE ERRADO: {result['why_wrong'][:120]}...");
            console.log("  CORRECAO: {result['correction'][:120]}...");
            console.log("  COMO FALAR: {result['how_to_say'][:100]}...");
            console.log("  ACAO: {result['action']}");
        } else {
            console.log("  {result['message']}");
    // === 2. DADOS QUE PROVAM ===
    console.log("\n\n  === 2. DADOS QUE PROVAM QUE PRECONCEITO E MENTIRA ===\n");
    for (const pc of engine.prejudices.values()) {
        console.log("\n  [{pc.correction_id}] '{pc.original_phrase[:50]}...'");
        console.log("  Tipo: {pc.prejudice_type.value}");
        console.log("  Dados:");
        for (const d of pc.data) {
            console.log("    - {d}");
    // === 3. RESSIGNIFICACAO DE SIMBOLOS ===
    console.log("\n\n  === 3. RESSIGNIFICACAO DE SIMBOLOS ===\n");
    // Votar na suastica
    r1 = engine.ressignify_symbol("SYM-001", votes_for=7000, votes_against=3000);
    console.log("  {r1['symbol']}: {r1['status']}");
    console.log("  {r1['decision'][:100]}");
    console.log("  Arte: {r1['art'][:80]}");
    // Votar no numero de faccao
    r2 = engine.ressignify_symbol("SYM-002", votes_for=8000, votes_against=2000);
    console.log("\n  {r2['symbol']}: {r2['status']}");
    console.log("  Novo significado: {r2['new_meaning'][:80]}");
    console.log("  Arte: {r2['art'][:80]}");
    // === 4. DETALHE: HISTORIA DA SUASTICA ===
    console.log("\n\n  === 4. SIMBOLO DETALHADO: SUASTICA ===\n");
    sym = engine.symbols["SYM-001"];
    console.log("  Nome: {sym.name}");
    console.log("  Significado ORIGINAL: {sym.original_meaning[:120]}...");
    console.log("  COOPTADO por: {sym.coopted_by} ({sym.coopted_period})");
    console.log("  Significado nocivo: {sym.coopted_meaning}");
    console.log("  RESSIGNIFICACAO: {sym.new_meaning[:120]}...");
    console.log("  Status: {sym.status.value}");
    console.log("  Arte de transformacao: {sym.ressignification_art}");
    // === 5. STATS ===
    console.log("\n\n  === 5. ESTATISTICAS ===\n");
    s = engine.stats();
    ...
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA DO OPENSYMBOLREVISION");
    console.log("{'='*80}");
    console.log(""";
...
// )
    console.log("{'='*80}");
    console.log("  OpenSymbolRevision: {s['corrections_made']} correcoes, ";
        "{s['symbols_reclaimed']} simbolos ressignificados.");
    console.log("  Frase errada -> Republica corrige.");
    console.log("  Simbolo nocivo -> Republica ressignifica.");
    console.log("  Pessoa estigmatizada -> Republica acolhe.");
    console.log("{'='*80}");
