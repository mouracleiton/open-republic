...
package openseniority_niveis_de_senioridade_por_impacto_nao_hierarquia
...
type PromotionCriteria struct {
    ...
    requirements := field(default_factory=list) // [texto]
    evidence_required := field(default_factory=list) // [texto]
    review_by := "" // string
...
// 3. TABELA DE NIVEIS
// ============================================================================
func print_seniority_table() string {
...
// ============================================================================
// decorador: @dataclass
type CompensationCalc struct {
    // Calcula compensacao por nivel, horas, e modo (Ideal vs Executavel).
    level: SeniorityLevel
    hours_per_week := 20.0 // Base 1.0 = 20h/semana // float64
    weeks_per_year := 46 // int64
    // decorador: @property
    func hours_per_year(self) float64 {
        return self.hours_per_week * self.weeks_per_year
    // decorador: @property
    func hours_per_month(self) float64 {
...
// ============================================================================
func map_to_labor_tier(level: SeniorityLevel) string {
...
// ============================================================================
func anti_elitism_check(level: SeniorityLevel, stated_by_self: logico = False) retorna (logico, texto) {
...
if __name__ == "__main__" {
    fmt.Println(print_seniority_table())
    fmt.Println()
    // Demo: compensacao em cada nivel
    fmt.Println()
    fmt.Println("DEMO -- Compensacao por nivel (20h/semana, Base 1.0):")
    fmt.Println("-" * 70)
    for _, level := range SeniorityLevel {
    ...
    fmt.Println()
    fmt.Println("DEMO -- Compensacao N6 em carga cheia (40h/semana):")
    fmt.Println("-" * 70)
    calc_n6 = CompensationCalc(level=SeniorityLevel.N6, hours_per_week=40.0)
    fmt.Println(calc_n6.summary())
    fmt.Println()
    fmt.Println("DEMO -- Anti-elitismo:")
    fmt.Println("-" * 70)
    for _, level := range SeniorityLevel {
        desempacote approved, msg = anti_elitism_check(level, stated_by_self=true)
        status = approved ? "OK" : "BLOQUEADO"
        fmt.Println("  N{level.name[1]} autodeclarado: [{status}] {msg}")
