...
print_evaluation_template() {
    ...
    lines.append("");
    for (const artefato of ARTEFATOS_PARA_TESTE) {
        lines.append("-" * 80);
...
// ============================================================================
analyze_results(evaluations: [BlindEvaluation], identity_map: {texto: texto}) {
    ...
    for (const tema of ordene(temas)) {
        lines.append("-" * 110);
        ...
        lines.append("  " + "-" * 85);
        for (const e of a_evals + b_evals) {
        ...
        // Médias
        a_avg = a_evals ? soma(&&.score_total para && em a_evals) / .length(a_evals) : 0;
        b_avg = b_evals ? soma(&&.score_total para && em b_evals) / .length(b_evals) : 0;
    ...
    teia_avg = teia_evals ? soma(&&.score_total para && em teia_evals) / .length(teia_evals) : 0;
    mercado_avg = mercado_evals ? soma(&&.score_total para && em mercado_evals) / .length(mercado_evals) : 0;
    ...
    para crit, attr in [("Acurácia", "acuracia"), ("Clareza", "clareza"), {
        ...
        t_avg = teia_evals ? soma(getattr(&&, attr) para && em teia_evals) / .length(teia_evals) : 0;
        m_avg = mercado_evals ? soma(getattr(&&, attr) para && em mercado_evals) / .length(mercado_evals) : 0;
        diff = t_avg - m_avg;
...
if (__name__ == "__main__") {
    console.log("=" * 110);
    console.log("GATE C: QUALIDADE DO ARTEFATO — PAINEL CEGO");
    console.log("=" * 110);
    console.log();
    // Mostrar template
    console.log("TEMPLATE DE AVALIAÇÃO (para impressão):");
    console.log();
    // print(print_evaluation_template())  // Uncomment para imprimir
    console.log("  [Template disponível no arquivo]");
    console.log();
    // Rodar simulação
    console.log();
    console.log(demo_simulation());
    console.log();
    console.log("=" * 110);
    console.log("COMO EXECUTAR O TESTE REAL:");
    console.log("=" * 110);
    console.log(""";
1. GERAR ARTEFATOS:;
