...
class SurplusMetric {
    ...
    name: texto;
    // No modo EXECUTAVEL
    const custo_operacional_pct = 0.0 // % da producao gasto em operacao;
    const investimento_justo_pct = 0.0 // % para melhorar;
    const reserva_pct = 0.0 // % para emergencia;
    const comunitario_pct = 0.0 // % para comunidade;
    // No modo IDEAL
    const ideal_equivalent = ""  // como funciona sem excedente;
    // Custo operacional detalhado
    const custo_aluguel_pct = 0.0;
    const custo_equipamento_pct = 0.0;
    const custo_insumos_pct = 0.0;
    const custo_energia_pct = 0.0;
    const custo_manutencao_pct = 0.0;
    // Limite predatorio
    const max_excedente_justo_pct = 0.0 // acima disso = predatorio;
    const excedente_predatorio_exemplo = "";
    // decorador: @property
    excedente_necessario_total(self) {
        return (self.custo_operacional_pct + self.investimento_justo_pct;
                + self.reserva_pct + self.comunitario_pct);
    // decorador: @property
    max_predatorio_pct(self) {
        return maximo(0.0, 1.0 - self.excedente_necessario_total);
    // decorador: @property
    trabalhador_recebe_pct(self) {
...
class SurplusCalculation {
    ...
    // Dados reais
    const workers = 1;
    const hours_per_worker_month = 160.0 // 40h/sem * 4;
    const value_per_hour = 0.0 // valor que cada hora gera;
    const worker_salary_month = 0.0 // quanto o trabalhador recebe;
    const establishment_revenue_month = 0.0 // receita total;
    const operational_cost_month = 0.0 // custo operacional real;
    // Resultados
    const production_total = 0.0 // valor total produzido;
    const worker_receives_total = 0.0 // total pago a trabalhadores;
    const surplus_extracted = 0.0 // excedente extraido;
    const surplus_pct = 0.0 // % de excedente;
    const surplus_type = SurplusType.NECESSARIO;
    // decorador: @property
    is_predatory(self) {
...
class SurplusEngine {
    ...
    get_metric(self, est_type: EstablishmentType) {
    ...
    funcao calculate(self, est_type: EstablishmentType,
                ...
                const operational_cost_month = 0.0;
                ) -> {texto: qualquer}:;
    ...
    ideal_mode_equivalent(self, est_type: EstablishmentType) {
    ...
    stats(self) {
        predatory = soma(1 para c em self.calculations.values();
                        if c.is_predatory);
        return {;
            "tipos_cadastrados": .length(self.metrics),;
            "calculos_feitos": .length(self.calculations),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENSURPLUS -- METRICA DE EXCEDENTE POR ESTABELECIMENTO");
    console.log("  Quanto excedente && NECESSARIO vs PREDATORIO?");
    console.log("=" * 80);
    // === 1. METRICAS POR TIPO ===
    console.log("\n\n  === 1. LIMITES DE EXCEDENTE POR TIPO ===\n");
    console.log("  {'Tipo':<25} {'Necessario':>12} {'Max Justo':>12} {'Trab. recebe':>14} {'Acima &&':>10}");
    console.log("  {'-'*75}");
    for (const c of engine.compare_all()) {
        console.log("  {c['tipo']:<25} {c['necessario']:>12} {c['max_justo']:>12} ";
            "{c['trabalhador_recebe']:>14} {c['acima_e_predatorio']:>10}");
    // === 2. DETALHE: HOSPITAL ===
    console.log("\n\n  === 2. DETALHE: HOSPITAL/CLINICA ===\n");
    hosp = engine.get_metric(EstablishmentType.SAUDE);
    console.log("  Excedente necessario: {hosp['excedente_necessario']}");
    console.log("  Trabalhador recebe: {hosp['trabalhador_recebe']}");
    console.log("  Limite predatorio: {hosp['limite_predatorio']}");
    console.log("\n  Custo operacional:");
    ...
    console.log("  Cada trabalhador: produz {r1['cada_trabalhador']['produz']}, ";
    ...
    console.log("  Razao exploracao: {r2['cada_trabalhador']['razao_exploracao']}");
    ...
    para et em [EstablishmentType.SAUDE, EstablishmentType.SOFTWARE, {
        ...
        console.log("\n  {ideal['tipo']}:");
        console.log("    Excedente: {ideal['excedente']}");
        console.log("    Trabalhador: {ideal['trabalhador_recebe']}");
        console.log("    {ideal['ideal'][:70]}...");
    // === 5. STATS ===
    console.log("\n\n  === 5. ESTATISTICAS ===\n");
    s = engine.stats();
    para cada (k, v) em s.items(): {
        console.log("  {k:<30} {v}");
    console.log("\n{'='*80}");
    console.log("  OpenSurplus: {s['tipos_cadastrados']} tipos, ";
