...
public class OpencreditSistemaDeCreditoDemocratico {
    ...
    public static class ContributionType {
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class GoodOrService {
        // Um bem ou servico na Republica.
        name: texto;
        tier: AccessTier;
        double credit_cost = 0.0 // 0 para garantidos;
        String description = "";
        String category = "";
        boolean scarce = false // oferta limitada?;
    ...
    public static class CreditAccount {
        ...
        citizen_id: texto;
        name: texto;
        double current_credits = 0.0;
        double credits_this_cycle = 0.0;
        double credits_spent = 0.0;
        double credits_expired = 0.0 // ! usados, expiraram;
        double contribution_hours = 0.0;
        List[(ContributionType, flutuante)] contributions = field(default_factory=list);
        double community_recognition = 0.0 // avaliacao pelos pares;
        int cycle = 0 // ciclo atual;
        // decorador: @property
        public double can_spend(self) {
            // Quanto pode gastar neste ciclo.
            return maximo(0, self.current_credits);
        public {texto: qualquer} spend(self, amount: flutuante, good_name: texto) {
        ...
        public {texto: qualquer} end_cycle(self) {
    ...
    public static class Citizen {
        ...
        name: texto;
        age: inteiro;
        [texto] skills = field(default_factory=list);
        CreditAccount account = field(default_factory=() -> CreditAccount("", ""));
    public static class DemocraticCreditSystem {
        ...
        //
        public void __init__(self, pool_per_cycle: flutuante = 1000.0) {
        ...
        public void add_citizen(self, citizen: Citizen) {
        ...
        funcao register_contribution(self, citizen_id: texto,
                                ...
                                int people_impacted = 1,;
                                double ripple_factor = 1.0):;
        ...
        public {texto: qualquer} run_cycle(self) {
            ...
            expired_total = 0;
            /* TODO: for-each Java para c em self.citizens.values() */
            ...
            return result;
        public [Dict] _generate_proposals(self, contributions: Dict, total_weighted: flutuante) {
            ...
            return proposals;
        public Dict _simulate_vote(self, proposals: [Dict]) {
        ...
        public {texto: qualquer} access_good(self, citizen_id: texto, good_name: texto) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENCREDIT -- CREDITO DEMOCRATICO DE ACESSO");
        System.out.println("  'Nao && moeda. E acesso. Nao acumula. A comunidade decide.'");
        System.out.println("=" * 75);
        ...
        // === Catalog ===
        System.out.println("\n\n  === CATALOGO DE BENS ===\n");
        System.out.println("  DIREITOS GARANTIDOS (sem credito, sempre):");
        /* TODO: for-each Java para g em CATALOG */
        ...
        System.out.println("  {'Bem':<35} {'Credito':>8}");
        System.out.println("  {'-'*45}");
        /* TODO: for-each Java para g em CATALOG */
            if (g.tier == AccessTier.CREDIT) {
                System.out.println("  {g.name:<35} {g.credit_cost:>7.0f}");
        // === Simular 3 ciclos ===
        System.out.println("\n\n  === SIMULACAO: 3 CICLOS DE CREDITO DEMOCRATICO ===\n");
        contribution_types = list(ContributionType);
        /* TODO: for-each Java para cycle_num em intervalo(1, 4) */
            ...
            /* TODO: for-each Java para c em system.citizens.values() */
            ...
            System.out.println("  CICLO {result['cycle']}:");
            System.out.println("    Proposta vencedora: {result['winning_proposal']}");
            System.out.println("    {result['winning_description'][:70]}...");
            System.out.println("    Pool: {result['pool_total']:.0f} | Cidadaos: {result['citizens']}");
            System.out.println("    Min: {result['min_given']} | Max: {result['max_given']} ";
                "| Media: {result['avg_given']}");
            System.out.println("    Expirado ciclo anterior: {result['expired_from_last_cycle']}");
            ...
            /* para cada (name, credits) em ordene(result["distribution"].items()): */
                ...
                System.out.println("    {name:<10} {c.account.contribution_hours:>5.0f}h {credits:>8.1f}");
            // Simular alguns gastos
            System.out.println("\n    Gastos no ciclo:");
            spenders = random.sample(list(system.citizens.values()), 4);
            /* TODO: for-each Java para c em spenders */
                ...
                System.out.println("      {c.name:<10} -> {good.name[:25]:<25} ";
                    "custo={cost:>3} restante={remaining:>5.1f} [{status}]");
            System.out.println();
        // === Explicar por que NAO e moeda ===
        System.out.println("\n{'='*75}");
        System.out.println("  POR QUE ISSO NAO E DINHEIRO");
        System.out.println("{'='*75}");
        System.out.println(""";
    ...
    // )
        // === Resumo final ===
        System.out.println("{'='*75}");
        System.out.println("  RESUMO DO OPENCREDIT");
        System.out.println("{'='*75}\n");
        System.out.println("  1. ESSNCIAIS sao DIREITOS (sem credito):");
        System.out.println("     Comida, agua, moradia, saude, educacao, transporte");
        System.out.println("     -> Voce ACESSE. Nao compra. Nao gasta credito.\n");
        System.out.println("  2. BENS ESCASSOS precisam de CREDITO:");
        System.out.println("     Instrumentos, viagens, eletronicos especiais");
        System.out.println("     -> Voce gasta credito de acesso do ciclo\n");
        System.out.println("  3. CREDITO && distribuido por TODOS:");
        System.out.println("     A comunidade vota como dividir o pool");
        System.out.println("     Base igual para todos + bonus por contribuicao");
        System.out.println("     -> Quem contribui mais, tem mais acesso ao que && escasso\n");
        System.out.println("  4. CREDITO NAO E MOEDA:");
        System.out.println("     Expira todo ciclo (! acumula)");
        System.out.println("     E pessoal (! transfere)");
        System.out.println("     Tem piso (ninguem fica sem) && teto (ninguem domina)");
        System.out.println("     Nao compra essencial (essencial && direito)\n");
        System.out.println("  5. CONTRIBUICAO && reconhecida pela comunidade:");
        System.out.println("     Nao && 'salario'. E reconhecimento.");
        System.out.println("     Saude, educacao && comida valem mais que arte.");
        System.out.println("     Mas arte tambem conta.\n");
        System.out.println("  RESULTADO:");
        System.out.println("  Ninguem passa fome porque 'acabou o credito'.");
        System.out.println("  Ninguem acumula fortuna porque 'trabalhou mais'.");
        System.out.println("  Todos tem o essencial garantido.");
        System.out.println("  O escasso && distribuido democraticamente.");
        System.out.println("  A cada ciclo, recomeca. Sem aculumulo. Sem heranca.");
        System.out.println("  Sem classe. Sem exploracao. Sem dominacao.\n");
        System.out.println("{'='*75}");
}
