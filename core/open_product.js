...
class RepublicProduct {
    ...
    const human_health_harm = HarmLevel.NONE;
    const animal_harm = HarmLevel.NONE;
    const environment_harm = HarmLevel.NONE;
    const social_harm = HarmLevel.NONE;
    // Materiais
    const materials = field(default_factory=list);
    const materials_detail = "";
    // Ciclo de vida
    const biodegradable = false;
    const recyclable = true;
    const repairable = true // OpenRepair;
    const modular = true // LEGO;
    const lifespan_years = 10.0;
    // Energia
    const energy_efficient = true;
    const energy_source = "solar/eolica";
    // Producao
    const fablab_producible = true // FabLab fabrica?;
    const tested_on_animals = false // PROIBIDO;
    const fair_labor = true // trabalho base 1.0;
    // Status
    const status = ProductStatus.APPROVED;
    const replacement_for = ""  // substitui qual produto nocivo?;
    const alternative_if_prohibited = "";
    // decorador: @property
    overall_score(self) {
...
class ProductEngine {
    ...
    evaluate(self, product_id: texto) {
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENPRODUCT -- PRODUTOS QUE NAO PREJUDICAM");
    console.log("  Pessoas. Animais. Meio Ambiente. Tudo protegido.");
    console.log("=" * 80);
    // === 1. AVALIAR TODOS OS PRODUTOS ===
    console.log("\n\n  === 1. AVALIACAO DE PRODUTOS ===\n");
    for (const pid of engine.products) {
        r = engine.evaluate(pid);
    // === 2. PRODUTOS APROVADOS ===
    console.log("\n\n  === 2. PRODUTOS APROVADOS ===\n");
    approved = engine.list_approved();
    console.log("  {'Produto':<25} {'Score':>5} {'Vida':>5} {'Bio':>4} {'Rep':>4} {'Mod':>4} {'Substitui'}");
    console.log("  {'-'*85}");
    for (const p of approved) {
        console.log("  {p['name'][:24]:<25} {p['score']:>5} {p['lifespan']:>4.0f}a ";
    ...
    for (const p of prohibited) {
        console.log("  {p['name']:<30} DANO: {p['why']}");
        console.log("    Alternativa: {p['alternative']}");
    ...
    para cada (key, data) em waste.items(): {
        if (key != "message"  &&  isinstance(data, dict)) {
            console.log("\n  {key.upper()}:");
            console.log("    Atual: {data['atual']}");
            console.log("    Republica: {data['republica']}");
    console.log("\n  {waste['message']}");
    // === 5. STATS ===
    console.log("\n\n  === 5. ESTATISTICAS ===\n");
    s = engine.stats();
    para cada (k, v) em s.items(): {
        console.log("  {k:<30} {v}");
    console.log("\n{'='*80}");
    console.log("  OpenProduct: {s['aprovados']} aprovados, {s['proibidos']} proibidos. ";
        "Score medio: {s['score_medio_aprovados']}.");
    console.log("  Nao prejudica pessoas. Nao prejudica animais. Nao prejudica planeta.");
    console.log("{'='*80}");
