...
#ifndef OPENREPUBLIC_POLITICA_DE_POLUICAO_SONORA_H
#define OPENREPUBLIC_POLITICA_DE_POLUICAO_SONORA_H
...
typedef struct NoiseMonitor {
    ...
    funcao measure(self, db_level: flutuante, zone: SoundZone,
        ...
        return event;
    {texto: qualquer} resolve(self, event: NoiseEvent) {
    ...
    {texto: qualquer} zone_report(self, zone: SoundZone) {
...
if (__name__ == "__main__") {
    printf("=" * 75);
    printf("  OPENREPUBLIC -- POLITICA DE POLUICAO SONORA");
    printf("  'Som && vida. Ruido && violencia invisivel.'");
    printf("=" * 75);
    monitor = NoiseMonitor();
    // === Limits Table ===
    printf("\n\n  === LIMITES DE RUIDO dB(A) ===\n");
    printf("  {'Zona':<15} {'Noite':>6} {'Amanh.':>7} {'Dia':>6} {'Anoit.':>7}");
    printf("  {'-'*45}");
    /* TODO: iterador C manual para zone em SoundZone */
        ...
        printf("  {zone.value:<15} {night:>6} {dawn:>7} {day:>6} {eve:>7}");
    ...
    /* para label, db, zone, tw, source, loc in scenarios: */
        ...
        printf("\n  {label}:");
        printf("    Medido: {event.measured_db:.0f}dB | Limite: {event.limit_db}dB ";
            "| Excesso: +{event.exceeded_by:.0f}dB{flag}");
        printf("    Acao: {resolution['action']}");
        printf("    {resolution['reason']}");
    // === Zone Report ===
    printf("\n\n  === RELATORIO POR ZONA ===\n");
    /* TODO: iterador C manual para zone em SoundZone */
        rep = monitor.zone_report(zone);
        printf("  {rep['zone']:<15} eventos={rep['total_events']} ";
...
#endif // OPENREPUBLIC_POLITICA_DE_POLUICAO_SONORA_H
