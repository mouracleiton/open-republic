...
class Representative {
    ...
    // Mandato
    const mandate_start = field(default_factory=time.time);
    const mandate_duration_days = 180 // 6 meses;
    const status = MandateStatus.ACTIVE;
    // Performance
    const votes_represented = 0 // quantas vezes votou conforme setor;
    const votes_against_sector = 0 // votou CONTRA o setor (traicao);
    const recall_requests = 0 // quantos pedidos de revogacao;
    const community_rating = 50.0 // 0-100;
    // Transparencia
    const actions_log = field(default_factory=list);
    const meetings_attended = 0;
    const meetings_missed = 0;
    is_expired(self) {
        // Verificar se o mandato expirou.
        elapsed = (time.time() - self.mandate_start) / 86400;
        return elapsed > self.mandate_duration_days;
    accountability_score(self) {
...
class SectorDecision {
    ...
    const options = field(default_factory=list);
    const sector_vote = field(default_factory=dict) // option -> votes;
    const winning_option = "";
    // Mandato do representante
    const representative_id = "";
    const rep_voted_as_sector = true // votou conforme setor?;
...
class RepresentativeSystem {
    ...
    __init__(self) {
        ...
        self._rep_counter = 0;
        self._dec_counter = 0;
    register_citizen_sector(self, citizen_id: texto, sector: Sector) {
    ...
    funcao elect_representative(self, sector: Sector, level: RepresentationLevel,
                            ...
                            votes: {texto: inteiro},;
                            const method = ElectionMethod.APPROVAL;
                            ) -> Representative:;
        ...
        self.elections.append({
            ...
            "winner": winner[1], "candidates": .length(candidates),;
    ...
    funcao recall(self, rep_id: texto, signatures: {texto},
        ...
        threshold = .length(sector_members) * 0.25;
    ...
    sector_report(self, sector: Sector) {
        ...
        members = .length(self.sector_members.get(sector, set()));
        return {;
            "sector": sector.value,;
            "members": members,;
            "active_representatives": .length(reps),;
            "total_representatives": .length(all_reps),;
            "decisions_made": .length(decisions),;
...
if (__name__ == "__main__") {
    // importa numpy as np
    console.log("=" * 80);
    console.log("  OPENREPRESENTATIVE -- REPRESENTACAO SETORIAL");
    console.log("  'Representante ! && chefe. E correio.'");
    console.log("=" * 80);
    ...
    para cid, name, sectors in citizens: {
        ...
        console.log("  {name}: {', '.join(s.value for s in sectors)}");
    // === 3. Election ===
    ...
    console.log("  Membros do setor {sector.value}: {len(health_members)}");
    ...
    console.log("  Eleito: {rep.name} ({rep.rep_id})");
    console.log("  Mandato: {rep.mandate_duration_days} dias");
    console.log("  Setor: {rep.sector.value} | Nivel: {rep.level.value}");
    console.log("  Status: {rep.status.value}");
    console.log("  AVISO: RECALL a qualquer momento com 25% do setor");
    // === 4. Decision flow ===
    console.log("\n\n  === FLUXO DE DECISAO ===\n");
    decision = system.submit_decision(;
        ...
        rep_id = rep.rep_id);
    console.log("  Decisao: {decision.title}");
    console.log("  Votos do setor: {decision.sector_vote}");
    console.log("  Decisao do setor: {decision.winning_option.upper()}");
    console.log("  Representante transportou: SIM (fiel ao setor)");
    console.log("  Representante: {rep.name}");
    ...
    result = system.representative_breaks_trust(;
        ...
        sector_decision = "sim",;
        rep_actual_vote = "!");
    console.log("  Setor decidiu: SIM (construir clinica)");
    console.log("  Representante votou: NAO");
    ...
    console.log("  Eleito: {rep2.name} ({rep2.rep_id})");
    // === 6. Recall ===
    console.log("\n\n  === RECALL (REVOGACAO) ===\n");
    ...
    console.log("  Representante: {rep2.name}");
    console.log("  Assinaturas: {recall_result['signatures']}");
    console.log("  Necessario (25%): {recall_result['threshold_needed']}");
    console.log("  Revogado: {recall_result['recalled']}");
    if (recall_result['recalled']) {
        console.log("  {recall_result['reason']}");
    // === 7. Sector Report ===
    console.log("\n\n  === RELATORIO DO SETOR SAUDE ===\n");
    report = system.sector_report(sector);
    console.log("  Setor: {report['sector']}");
    console.log("  Membros: {report['members']}");
    console.log("  Representantes ativos: {report['active_representatives']}");
    console.log("  Total representantes (historico): {report['total_representatives']}");
    console.log("  Decisoes tomadas: {report['decisions_made']}");
    console.log("  Score medio de accountability: {report['avg_accountability']}");
    // === Philosophy ===
    POLITICIANS_REP = "Politicos profissionais";
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA: REPRESENTACAO");
    console.log("{'='*80}");
    console.log(""";
