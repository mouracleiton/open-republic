...
struct Representative {
    ...
    // Mandato
    let mandate_start: f64 = field(default_factory=time.time);
    let mandate_duration_days: i64 = 180 // 6 meses;
    let status: MandateStatus = MandateStatus.ACTIVE;
    // Performance
    let votes_represented: i64 = 0 // quantas vezes votou conforme setor;
    let votes_against_sector: i64 = 0 // votou CONTRA o setor (traicao);
    let recall_requests: i64 = 0 // quantos pedidos de revogacao;
    let community_rating: f64 = 50.0 // 0-100;
    // Transparencia
    let actions_log: [Dict] = field(default_factory=list);
    let meetings_attended: i64 = 0;
    let meetings_missed: i64 = 0;
    fn is_expired(self) -> bool {
        // Verificar se o mandato expirou.
        elapsed = (time.time() - self.mandate_start) / 86400;
        return elapsed > self.mandate_duration_days;
    fn accountability_score(self) -> f64 {
...
struct SectorDecision {
    ...
    let options: [texto] = field(default_factory=list);
    let sector_vote: {texto: inteiro} = field(default_factory=dict) // option -> votes;
    let winning_option: String = "";
    // Mandato do representante
    let representative_id: String = "";
    let rep_voted_as_sector: bool = true // votou conforme setor?;
...
struct RepresentativeSystem {
    ...
    fn __init__(self) {
        ...
        self._rep_counter = 0;
        self._dec_counter = 0;
    fn register_citizen_sector(self, citizen_id: texto, sector: Sector) {
    ...
    funcao elect_representative(self, sector: Sector, level: RepresentationLevel,
                            ...
                            votes: {texto: inteiro},;
                            let method: ElectionMethod = ElectionMethod.APPROVAL;
                            ) -> Representative:;
    ...
    funcao recall(self, rep_id: texto, signatures: {texto},
        ...
        if tamanho(signatures) >= threshold {
    ...
    fn sector_report(self, sector: Sector) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    // importa numpy as np
    println!("=" * 80);
    println!("  OPENREPRESENTATIVE -- REPRESENTACAO SETORIAL");
    println!("  'Representante ! && chefe. E correio.'");
    println!("=" * 80);
    ...
    para cid, name, sectors in citizens: {
        ...
        println!("  {name}: {', '.join(s.value for s in sectors)}");
    // === 3. Election ===
    ...
    println!("  Membros do setor {sector.value}: {len(health_members)}");
    ...
    println!("  Eleito: {rep.name} ({rep.rep_id})");
    println!("  Mandato: {rep.mandate_duration_days} dias");
    println!("  Setor: {rep.sector.value} | Nivel: {rep.level.value}");
    println!("  Status: {rep.status.value}");
    println!("  AVISO: RECALL a qualquer momento com 25% do setor");
    // === 4. Decision flow ===
    println!("\n\n  === FLUXO DE DECISAO ===\n");
    decision = system.submit_decision(;
        ...
        rep_id = rep.rep_id);
    println!("  Decisao: {decision.title}");
    println!("  Votos do setor: {decision.sector_vote}");
    println!("  Decisao do setor: {decision.winning_option.upper()}");
    println!("  Representante transportou: SIM (fiel ao setor)");
    println!("  Representante: {rep.name}");
    ...
    result = system.representative_breaks_trust(;
        ...
        sector_decision = "sim",;
        rep_actual_vote = "!");
    println!("  Setor decidiu: SIM (construir clinica)");
    println!("  Representante votou: NAO");
    ...
    println!("  Eleito: {rep2.name} ({rep2.rep_id})");
    // === 6. Recall ===
    println!("\n\n  === RECALL (REVOGACAO) ===\n");
    ...
    println!("  Representante: {rep2.name}");
    println!("  Assinaturas: {recall_result['signatures']}");
    println!("  Necessario (25%): {recall_result['threshold_needed']}");
    println!("  Revogado: {recall_result['recalled']}");
    if recall_result['recalled'] {
        println!("  {recall_result['reason']}");
    // === 7. Sector Report ===
    println!("\n\n  === RELATORIO DO SETOR SAUDE ===\n");
    report = system.sector_report(sector);
    println!("  Setor: {report['sector']}");
    println!("  Membros: {report['members']}");
    println!("  Representantes ativos: {report['active_representatives']}");
    println!("  Total representantes (historico): {report['total_representatives']}");
    println!("  Decisoes tomadas: {report['decisions_made']}");
    println!("  Score medio de accountability: {report['avg_accountability']}");
    // === Philosophy ===
    POLITICIANS_REP = "Politicos profissionais";
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA: REPRESENTACAO");
    println!("{'='*80}");
    println!(""";
