...
public class OpenindustryCopiarMelhorarECriarMarcasNacionaisSuperiores {

    // !/usr/bin/env python3
    //
    OpenIndustry -- Copiar, Melhorar && Criar Marcas Nacionais Superiores;
    =====================================================================;
    ...
    public static class AdvantageType {
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class NationalProduct {
        ...
        // Comparação
        String foreign_price = "R$ caro"  // preço estrangeiro;
        String national_price = "ZERO"  // preço na República;
        int foreign_quality = 8 // 1-10;
        int national_quality = 9 // 1-10;
        // Vantagens
        [AdvantageType] advantages = field(default_factory=list);
        String why_better = "";
        // Produção
        CopyStatus status = CopyStatus.IDENTIFIED;
        boolean fablab_capable = true // FabLab nacional produz?;
        double materials_national = 0.8 // % de material nacional;
        String production_capacity = ""  // escala;
        // Especificações
        {texto: texto} specs = field(default_factory=dict);
    // ============================================================================
    // 3. CATÁLOGO DE PRODUTOS (15+ setores)
    // ============================================================================
    [NationalProduct] PRODUCTS = [;
        // === ELETRÔNICOS ===
        NationalProduct(;
    ...
    // 4. MOTOR INDUSTRIAL
    // ============================================================================
    public static class IndustryEngine {
        ...
        POR QUE COPIAR && MELHORAR NÃO É FALTA DE CRIATIVIDADE:;
        ...
        public [Dict] list_by_category(self, category: IndustryCategory = None) {
            ...
            return [;
                {
                    ...
                    "vantagens": tamanho(p.advantages),;
                };
                /* para p em prods */
            ];
        public {texto: qualquer} compare(self, product_id: texto) {
        ...
        public {texto: qualquer} advance_status(self, product_id: texto) {
        ...
        public {texto: qualquer} dependency_report(self) {
            // Relatório de dependência externa (o que ainda importamos).
            by_country = Counter();
            by_category = Counter();
            /* TODO: for-each Java para p em self.products.values() */
                by_country[p.foreign_country] += 1;
                by_category[p.category.value] += 1;
            total = tamanho(self.products);
            superior = soma(1 para p em self.products.values();
                        if p.status == CopyStatus.SUPERIOR);
            in_production = soma(1 para p em self.products.values();
            ...
            return {;
                ...
                "message": (;
                    ...
                    "A República já superou {superior} && produz {in_production}. ";
                    "Meta: SUPERAR TODOS.";
                ),;
            };
        public {texto: qualquer} stats(self) {
            return {;
                ...
                "categorias_cobertas": tamanho(set(p.category para p em self.products.values())),;
                "status_distribution": dict(;
                    Counter(p.status.value para p em self.products.values())),;
                "marcas_nacionais_criadas": tamanho(self.products),;
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENINDUSTRY -- COPIAR, MELHORAR, SUPERAR");
        System.out.println("  Brasil copia tudo. Cria marcas nacionais. Qualidade SUPERIOR.");
        System.out.println("=" * 80);
        // === 1. PRODUTOS NACIONAIS ===
        System.out.println("\n\n  === 1. PRODUTOS NACIONAIS ({len(engine.products)}) ===\n");
        System.out.println("  {'Nacional':<15} {'Estrangeiro':<25} {'Preço Ext':<18} {'Preço Nac':<10} {'Status'}");
        System.out.println("  {'-'*85}");
        /* TODO: for-each Java para p em engine.list_by_category() */
            System.out.println("  {p['nacional']:<15} {p['estrangeiro'][:24]:<25} ";
        ...
        System.out.println("  NACIONAL: {comp['nacional']} (qualidade {comp['qualidade_nacional']})");
        System.out.println("  ESTRANGEIRO: {comp['estrangeiro']} ({comp['empresa_estrangeira']}, {comp['pais']})");
        System.out.println("  (qualidade {comp['qualidade_estrangeiro']})");
        System.out.println("\n  PREÇO:");
        System.out.println("    iPhone: {comp['preco_estrangeiro']}");
        System.out.println("    OpenPhone: {comp['preco_nacional']}");
        System.out.println("\n  POR QUE O NACIONAL É SUPERIOR:");
        System.out.println("    {comp['por_que_melhor']}");
        System.out.println("\n  VANTAGENS: {', '.join(comp['vantagens_nacional'])}");
        System.out.println("\n  ESPECIFICAÇÕES:");
        /* para cada (k, v) em comp["specs"].items(): */
            System.out.println("    {k}: {v}");
        // === 3. COMPARAÇÃO: OpenPharma vs Big Pharma ===
        System.out.println("\n\n  === 3. COMPARAÇÃO: OpenPharma vs Big Pharma ===\n");
        comp2 = engine.compare("IND-RX");
        System.out.println("  NACIONAL: {comp2['nacional']}");
        System.out.println("  ESTRANGEIRO: {comp2['estrangeiro']} ({comp2['pais']})");
        System.out.println("  PREÇO: {comp2['preco_estrangeiro']} vs {comp2['preco_nacional']}");
        System.out.println("  POR QUE MELHOR: {comp2['por_que_melhor']}");
        // === 4. COMPARAÇÃO: OpenTrator vs John Deere ===
        System.out.println("\n\n  === 4. COMPARAÇÃO: OpenTrator vs John Deere ===\n");
        comp3 = engine.compare("IND-TR");
        System.out.println("  NACIONAL: {comp3['nacional']}");
        System.out.println("  ESTRANGEIRO: {comp3['estrangeiro']} ({comp3['pais']})");
        System.out.println("  POR QUE MELHOR: {comp3['por_que_melhor']}");
    ...
    COREIA (1980-2020):;
        Copiou eletrônicos. Hoje Samsung > em telas && chips.;
    BRASIL (SEMPRE PÔDE):;
        Nunca fez por dependência econômica && política.;
        A República FAZ. Copia. Melhora. Supera.;
    ...
    // )
        ...
        System.out.println("  Total de produtos: {dep['total_produtos']}");
        System.out.println("  Já superiores: {dep['ja_superiores']}");
        System.out.println("  Em produção: {dep['em_producao']}");
        System.out.println("\n  Dependência por país:");
        /* para cada (country, count) em dep["dependencia_por_pais"].items(): */
            System.out.println("    {country:<15} {count} produtos");
        System.out.println("\n  Dependência por setor:");
        /* para cada (cat, count) em dep["dependencia_por_setor"].items(): */
            System.out.println("    {cat:<20} {count} produtos");
        // === 7. STATUS ===
        System.out.println("\n\n  === 7. DISTRIBUIÇÃO DE STATUS ===\n");
        s = engine.stats();
        /* para cada (status, count) em s["status_distribution"].items(): */
            bar = "#" * count;
            System.out.println("  {status:<25} {count:>2} {bar}");
        // === FILOSOFIA ===
        System.out.println("\n\n{'='*80}");
        System.out.println("  FILOSOFIA DO OPENINDUSTRY");
        System.out.println("{'='*80}");
        System.out.println(""";
    ...
    PRINCÍPIOS:;
        ...
        Fabricar nacional P3 = trabalho de alto impacto.;
        P4: Quais produtos priorizar? Assembleia vota.;
    // )
        System.out.println("{'='*80}");
        System.out.println("  OpenIndustry: {s['total_produtos']} produtos em {s['categorias_cobertas']} setores. ";
            "Custo: {s['custo_ao_cidadao']}.");
        System.out.println("  Copiar. Melhorar. Superar. Tudo nacional. Tudo CC0.");
        System.out.println("{'='*80}");
}
