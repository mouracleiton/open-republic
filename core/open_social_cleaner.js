...
class SocialContact {
    ...
    const last_interaction_days = 999 // ha quantos dias interagiu;
    const mutual_likes = 0 // curtidas reciprocas nos ultimos 90 dias;
    const mutual_replies = 0 // respostas reciprocas;
    const dms_exchanged = 0;
    // Classificacao
    const category = ContactCategory.UNKNOWN;
    const action = CleanAction.UNFOLLOW;
    const reason = "";
    // Metadados
    const is_steward_of = ""  // se para guardiao, de qual sistema;
    const manually_kept = false // marcado manualmente para manter;
// ============================================================================
// 2. MOTOR DE LIMPEZA
// ============================================================================
class SocialCleaner {
    ...
    6. Reporta;
    //
    __init__(self, platform: texto = "x") {
    ...
    load_stewards(self, stewards_data: {texto: qualquer}) {
    ...
    mark_keep(self, handles: [texto]) {
        // Marca contatos para manter manualmente.
        for (const h of handles) {
            self.keep_list.add(h.lower().lstrip("@"));
    classify_contact(self, contact: SocialContact) {
    ...
    classify_all(self) {
        ...
        return {;
            "total_contacts": .length(self.contacts),;
    ...
    generate_plan(self) {
        ...
        return plan;
    execute_xurl(self, handle: texto, action: texto) {
    ...
    execute_plan(self, confirm: logico = False) {
        ...
        for (const item of plan) {
            ...
            time.sleep(1);
        return results;
    import_from_xurl(self, limit: inteiro = 200) {
        ...
        tente:;
            ...
            if (result.returncode != 0) {
                console.log("  [ERRO] xurl following falhou: {result.stderr[:200]}");
                return 0;
            ...
            console.log("  [ERRO] Nao consegui parsear saida do xurl");
            return 0;
        capture Exception como &&:;
            console.log("  [ERRO] {&&}");
            return 0;
    report(self) {
        ...
        if (unfollow) {
            ...
            for (const c of unfollow[:20]) {
                lines.append("    @{c.handle:<25} {c.reason}");
            if (.length(unfollow) > 20) {
...
if (__name__ == "__main__") {
    console.log("=" * 70);
    console.log("  OPENSOCIALCLEANER -- LIMPEZA DE REDE SOCIAL");
    console.log('  "Sua rede && extensao do seu corpo digital."');
    console.log("=" * 70);
    ...
    console.log("  Total: {stats['total_contacts']} contatos");
    console.log("  Manter:    {stats['keep_count']}");
    console.log("  Desseguir: {stats['unfollow_count']}");
    console.log("  Silenciar: {stats['mute_count']}");
    console.log("  Bloquear:  {stats['block_count']}");
    // === RELATORIO ===
    console.log();
    ...
    for (const p of plan) {
        console.log("    [{p['action'].upper():>9}] @{p['handle']:<25} ";
            "-- {p['reason']}");
    // === EXECUTAR (dry run) ===
    console.log("\n\n  EXECUCAO (DRY RUN -- sem alteracoes reais)\n");
    results = cleaner.execute_plan(confirm=false);
    console.log("  Dry run completo. {len(plan)} acoes planejadas.");
    console.log("  Para executar de verdade: execute_plan(confirm=True)");
    // === INSTRUCOES ===
    console.log("\n\n{'='*70}");
    console.log("  COMO USAR COM CONTA REAL (X/Twitter)");
    console.log("{'='*70}");
    console.log(""";
...
2. EXECUTAR LIMPEZA REAL:;
    ...
    console.log(cleaner.report()) // revisar;
...
// )
    console.log("{'='*70}");
    console.log("  OpenSocialCleaner: {stats['total_contacts']} contatos -> ";
        "{stats['keep_count']} mantidos.");
    console.log("  Guardioes sempre seguidos. Resto && ruido.");
    console.log("{'='*70}");
