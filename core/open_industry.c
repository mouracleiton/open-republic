...
#ifndef OPENINDUSTRY_COPIAR_MELHORAR_E_CRIAR_MARCAS_NACIONAIS_SUPERIORES_H
#define OPENINDUSTRY_COPIAR_MELHORAR_E_CRIAR_MARCAS_NACIONAIS_SUPERIORES_H
...
// ============================================================================
typedef struct IndustryCategory {
...
// ============================================================================
// decorador: @dataclass
typedef struct NationalProduct {
    ...
    // Comparação
    char* foreign_price = "R$ caro"  // preço estrangeiro;
    char* national_price = "ZERO"  // preço na República;
    ...
    bool fablab_capable = true // FabLab nacional produz?;
    double materials_national = 0.8 // % de material nacional;
    char* production_capacity = ""  // escala;
    // Especificações
...
typedef struct IndustryEngine {
    ...
    [Dict] list_by_category(self, category: IndustryCategory = None) {
        ...
        return [;
            {
                ...
                "vantagens": sizeof(p.advantages),;
            };
            /* para p em prods */
        ];
    {texto: qualquer} compare(self, product_id: texto) {
    ...
    {texto: qualquer} advance_status(self, product_id: texto) {
    ...
    {texto: qualquer} dependency_report(self) {
        // Relatório de dependência externa (o que ainda importamos).
        by_country = Counter();
        by_category = Counter();
        /* TODO: iterador C manual para p em self.products.values() */
            by_country[p.foreign_country] += 1;
            by_category[p.category.value] += 1;
        total = sizeof(self.products);
    ...
    {texto: qualquer} stats(self) {
        return {;
            ...
            "categorias_cobertas": sizeof(set(p.category para p em self.products.values())),;
            "status_distribution": dict(;
                Counter(p.status.value para p em self.products.values())),;
            "marcas_nacionais_criadas": sizeof(self.products),;
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENINDUSTRY -- COPIAR, MELHORAR, SUPERAR");
    printf("  Brasil copia tudo. Cria marcas nacionais. Qualidade SUPERIOR.");
    printf("=" * 80);
    // === 1. PRODUTOS NACIONAIS ===
    printf("\n\n  === 1. PRODUTOS NACIONAIS ({len(engine.products)}) ===\n");
    printf("  {'Nacional':<15} {'Estrangeiro':<25} {'Preço Ext':<18} {'Preço Nac':<10} {'Status'}");
    printf("  {'-'*85}");
    /* TODO: iterador C manual para p em engine.list_by_category() */
        printf("  {p['nacional']:<15} {p['estrangeiro'][:24]:<25} ";
    ...
    printf("  NACIONAL: {comp['nacional']} (qualidade {comp['qualidade_nacional']})");
    printf("  ESTRANGEIRO: {comp['estrangeiro']} ({comp['empresa_estrangeira']}, {comp['pais']})");
    printf("  (qualidade {comp['qualidade_estrangeiro']})");
    printf("\n  PREÇO:");
    printf("    iPhone: {comp['preco_estrangeiro']}");
    printf("    OpenPhone: {comp['preco_nacional']}");
    printf("\n  POR QUE O NACIONAL É SUPERIOR:");
    printf("    {comp['por_que_melhor']}");
    ...
    printf("  NACIONAL: {comp2['nacional']}");
    printf("  ESTRANGEIRO: {comp2['estrangeiro']} ({comp2['pais']})");
    printf("  PREÇO: {comp2['preco_estrangeiro']} vs {comp2['preco_nacional']}");
    printf("  POR QUE MELHOR: {comp2['por_que_melhor']}");
    // === 4. COMPARAÇÃO: OpenTrator vs John Deere ===
    printf("\n\n  === 4. COMPARAÇÃO: OpenTrator vs John Deere ===\n");
    comp3 = engine.compare("IND-TR");
    printf("  NACIONAL: {comp3['nacional']}");
    printf("  ESTRANGEIRO: {comp3['estrangeiro']} ({comp3['pais']})");
    printf("  POR QUE MELHOR: {comp3['por_que_melhor']}");
...
// )
    ...
    printf("  Total de produtos: {dep['total_produtos']}");
    printf("  Já superiores: {dep['ja_superiores']}");
    printf("  Em produção: {dep['em_producao']}");
    printf("\n  Dependência por país:");
    /* para cada (country, count) em dep["dependencia_por_pais"].items(): */
        printf("    {country:<15} {count} produtos");
    printf("\n  Dependência por setor:");
    /* para cada (cat, count) em dep["dependencia_por_setor"].items(): */
        printf("    {cat:<20} {count} produtos");
    // === 7. STATUS ===
    printf("\n\n  === 7. DISTRIBUIÇÃO DE STATUS ===\n");
    s = engine.stats();
    /* para cada (status, count) em s["status_distribution"].items(): */
        bar = "#" * count;
        printf("  {status:<25} {count:>2} {bar}");
    // === FILOSOFIA ===
    printf("\n\n{'='*80}");
    printf("  FILOSOFIA DO OPENINDUSTRY");
    printf("{'='*80}");
    printf(""";
...
// )
    printf("{'='*80}");
    printf("  OpenIndustry: {s['total_produtos']} produtos em {s['categorias_cobertas']} setores. ";
        "Custo: {s['custo_ao_cidadao']}.");
    printf("  Copiar. Melhorar. Superar. Tudo nacional. Tudo CC0.");
    printf("{'='*80}");

#endif // OPENINDUSTRY_COPIAR_MELHORAR_E_CRIAR_MARCAS_NACIONAIS_SUPERIORES_H
