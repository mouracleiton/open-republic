...
#ifndef OPENREPUBLIC_POLITICA_FIM_DAS_BATERIAS_DESCARTAVEIS_H
#define OPENREPUBLIC_POLITICA_FIM_DAS_BATERIAS_DESCARTAVEIS_H
...
typedef struct BatterySpec {
    ...
    bool repairable = false // pode trocar celula individual?;
...
typedef struct Product {
    ...
    int battery_count = 1;
    bool battery_replaceable = false;
    bool glued = false;
    double expected_life_years = 0;
typedef struct BatteryPolicy {
    ...
    {texto: qualquer} check_product(self, product: Product) {
    ...
    {texto: qualquer} recommend_replacement(self, prohibited_type: BatteryType) {
    ...
    {texto: qualquer} impact_report(self, population: inteiro = 1_000_000) {
...
if (__name__ == "__main__") {
    printf("=" * 75);
    printf("  OPENREPUBLIC -- FIM DAS BATERIAS DESCARTAVEIS");
    printf("  'Pilha no lixo = 100 anos de veneno.'");
    printf("  'Bateria reparavel = 15 anos de uso.'");
    printf("=" * 75);
    policy = BatteryPolicy();
    // === 1. Battery Database ===
    printf("\n\n  === BANCO DE BATERIAS ===\n");
    printf("  {'Tipo':<20} {'Status':<14} {'Vida':>5} {'Ciclos':>7} {'Recicl.':>8} {'Toxico'}");
    printf("  {'-'*75}");
    /* para cada (bt, spec) em BATTERY_DATABASE.items(): */
        toxic = spec.toxic_materials ? "SIM" : "!";
        printf("  {bt.value:<20} {spec.status.value:<14} ";
    ...
    /* TODO: iterador C manual para p em products */
        result = policy.check_product(p);
        verdict = result["verdict"];
        ok = result["allowed"] ? "OK" : "BLOQUEADO";
        printf("\n  {p.name}:");
        printf("    Bateria: {p.battery_type.value} x{p.battery_count}");
        printf("    Substituivel: {'sim' if p.battery_replaceable else 'NAO'} | ";
            "Colada: {'sim' if p.glued else '!'}");
        printf("    Veredito: {ok} ({verdict})");
        printf("    Razao: {result['reason'][:80]}");
    ...
    /* TODO: iterador C manual para pt em prohibited_types */
        rec = policy.recommend_replacement(pt);
        printf("\n  {rec['prohibited']:<20} -> {rec['replacement']}");
        printf("    Como: {rec['how']}");
        printf("    Ganho: {rec['lifespan_gain']}");
        printf("    Reciclavel: {rec['recycle_gain']}");
    // === 4. Impact Report ===
    printf("\n\n  === IMPACTO AMBIENTAL ===\n");
    /* TODO: iterador C manual para pop em [10_000, 100_000, 1_000_000] */
        impact = policy.impact_report(pop);
        printf("\n  Populacao: {pop:,}");
        printf("    {impact['comparacao']}");
        printf("    CO2 evitado: {impact['co2_evitado_ton']:,} toneladas/ano");
        printf("    Material toxico evitado: {impact['material_toxico_evitado_ton']} ton/ano");
    // === Philosophy ===
    printf("\n\n{'='*75}");
    printf("  PRINCIPIOS");
    printf("{'='*75}");
...
#endif // OPENREPUBLIC_POLITICA_FIM_DAS_BATERIAS_DESCARTAVEIS_H
