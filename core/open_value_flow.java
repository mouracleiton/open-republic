...
public class OpenvalueflowEliminarEmpresaEBancoDaCadeiaDeValor {
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class ValueChainAnalysis {
        // Analisa para onde vai cada R$ 1 produzido pelo trabalhador.
        analysis_id: texto;
        scenario: texto;
        mode: FlowMode;
        // Valor produzido
        double total_value = 100.0 // valor total produzido;
        // Para onde vai (em R$)
        double worker_receives = 0.0;
        double company_profit = 0.0 // lucro do dono;
        double company_operational = 0.0 // custo real de operar;
        double bank_interest = 0.0 // juros;
        double bank_transaction = 0.0 // taxas de transacao;
        double bank_spread = 0.0 // spread cambial;
        double tax = 0.0 // impostos;
        double intermediary = 0.0 // intermediarios;
        // Pool coletivo (Republica)
        double collective_pool = 0.0 // 5% que volta;
        // decorador: @property
        public double worker_pct(self) {
            return self.worker_receives / self.total_value * 100;
        // decorador: @property
        public double extracted_pct(self) {
    ...
    // ============================================================================
    public static class ValueFlowEngine {
        ...
        funcao analyze_capitalism(self, total_value: flutuante = 100.0,
                            double company_surplus_pct = 0.60,;
                            double bank_interest_pct = 0.15,;
                            double bank_transaction_pct = 0.03,;
                            double bank_spread_pct = 0.02,;
                            double intermediary_pct = 0.00;
                            ) -> {texto: qualquer}:;
        ...
        public {texto: qualquer} bank_extraction_detail(self) {
        ...
        public {texto: qualquer} company_extraction_detail(self) {
        ...
        public {texto: qualquer} full_comparison(self, total_value: flutuante = 100.0) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENVALUEFLOW -- ELIMINAR EMPRESA E BANCO DA CADEIA");
        System.out.println("  100% pra quem trabalha. ZERO parasita.");
        System.out.println("=" * 80);
        // === 1. CAPITALISMO ATUAL ===
        System.out.println("\n\n  === 1. ONDE VAI O VALOR NO CAPITALISMO ===\n");
        cap = engine.analyze_capitalism(100.0);
        System.out.println("  Produzido: {cap['total_produzido']}");
        System.out.println("\n  PARASITAS:");
        ...
        System.out.println("  Extracao total: {cap['EXTRACAO_TOTAL']}");
        // === 2. COMO O BANCO SUGA ===
        System.out.println("\n\n  === 2. COMO O BANCO SUGA VALOR ===\n");
        bank = engine.bank_extraction_detail();
        System.out.println("  {bank['total_extraido_banco']}\n");
        /* TODO: for-each Java para forma em bank["formas"] */
            System.out.println("\n  {forma['nome']}");
            System.out.println("    Como: {forma['como'][:80]}...");
            System.out.println("    Exemplo: {forma['exemplo'][:80]}...");
            System.out.println("    Republica: {forma['republica']}");
        // === 3. COMO A EMPRESA SUGA ===
        System.out.println("\n\n  === 3. COMO A EMPRESA (DONO) SUGA VALOR ===\n");
        comp = engine.company_extraction_detail();
        System.out.println("  Como: {comp['como'][:100]}...");
        System.out.println("  Exemplo: {comp['exemplo']}");
        System.out.println("  Solucao: {comp['solution']}");
        // === 4. REPUBLICA EXECUTAVEL ===
        System.out.println("\n\n  === 4. REPUBLICA EXECUTAVEL (sem parasitas) ===\n");
        rep = engine.analyze_republic_executavel(100.0);
        System.out.println("  Produzido: {rep['total_produzido']}");
        System.out.println("\n  SEM PARASITAS:");
        /* para cada (k, v) em rep["SEM_PARASITAS"].items(): */
            System.out.println("    {k:<20} {v}");
        System.out.println("\n  Operacional real: {rep['OPERACIONAL_REAL']}");
        System.out.println("  Pool coletivo (5%): {rep['POOL_COLETIVO_5PCT']}");
        System.out.println("\n  >>> TRABALHADOR RECEBE: {rep['TRABALHADOR_RECEBE']} <<<");
        System.out.println("  Extracao total: {rep['EXTRACAO_TOTAL']}");
        // === 5. COMPARACAO ===
        System.out.println("\n\n  === 5. COMPARACAO: CAPITALISMO vs REPUBLICA ===\n");
        comp_final = engine.full_comparison(100.0);
        System.out.println("  {comp_final['message']}");
    ...
    // )
        ...
        System.out.println("\n{'='*80}");
        System.out.println("  OpenValueFlow: {s['analises_feitas']} analises.");
