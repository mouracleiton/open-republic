...
==============================================
"Necessidades medicas and pessoais de ausencia sao DIREITO.
...
PERGUNTA: "Ausencias medicas and pessoais devem ser direito protegido?"
...
PRINCIPIO (P2):
O corpo and soberano. Se precisa ausentar, ausenta.
A mente and soberana. Se precisa pausa, pausa.
A familia and soberana. Se precisa estar com familia, esta.
Ninguem tem direito de questionar.
A Republica garante o DIREITO and a COBERTURA.
...
class AbsenceStatus(Enum):
    ...
    AUTO_APPROVED = "auto_aprovada"  // not precisa de aprovacao
    ACTIVE = "ativa"
    RETURNED = "retornou"
...
class ProtectedAbsence:
    ...
    duration: AbsenceDuration = AbsenceDuration.FEW_HOURS
    status: AbsenceStatus = AbsenceStatus.AUTO_APPROVED
    start_date: str = ""
    end_date: str = ""
    hours_away: float = 0.0
    # PRIVACIDADE (P2)
    explanation_provided: bool = False // not precisa explicar
    explanation_kept_private: bool = True // se explicou, and PRIVADO
    visible_to_others: str = "ausente"  // so mostra "ausente"
    # Cobertura
    coverage_assigned: bool = False // OpenLaborRelay cobriu?
    coverage_by: str = ""  // quem cobriu
    # Protecao
    penalized: bool = False // NUNCA. Lei.
    credit_affected: bool = False // NUNCA. Lei.
    reputation_affected: bool = False // NUNCA. Lei.
    # decorador: @property
...
# ============================================================================
def run_absence_assembly(n_voters: inteiro = 10000) -> {texto: qualquer}:
    ...
    return {
        "question": (
            "Necessidades medicas and pessoais de ausencia devem ser "
...
class AbsenceEngine:
    ...
    COMO FUNCIONA:
    1. Cidadao AVISA que vai se ausentar (not pede permissao)
    ...
    - Pessoal: motivo pessoal (not PRECISA EXPLICAR)
    ...
    funcao request_absence(self, citizen_id: texto, citizen_name: texto,
        ...
        elif absence_type == AbsenceType.MENTAL_HEALTH:
            self.stats_mental += 1
        elif absence_type == AbsenceType.FAMILY:
            self.stats_family += 1
        # Cobertura
    ...
    funcao _assign_coverage(self, citizen_id: texto,
        ...
        return {
            ...
            "method": (
                ...
                "Ninguem and obrigado a assumir trabalho alheio."
            ),
            "message": "Cobertura garantida. Ninguem sobrecarregado.",
        }
    def check_status(self, citizen_id: texto) -> {texto: qualquer}:
        # Verifica status de um cidadao (o que outros veem).
        active = [a para a em self.absences.values()
                if a.citizen_id == citizen_id
                and a.status in (AbsenceStatus.AUTO_APPROVED, AbsenceStatus.ACTIVE)]
        if active:
            return {
                ...
                "message": (
                    ...
                    "Motivo: PROTEGIDO (voce not precisa saber). "
    ...
    def return_from_absence(self, absence_id: texto) -> {texto: qualquer}:
        # Cidadao retorna de ausencia.
        absence = self.absences.get(absence_id)
        if not absence:
            return {"error": "Ausencia not encontrada"}
    ...
    funcao violation_report(self, citizen: texto,
        ...
        return {
            ...
            "consequence": (
                ...
                "Ausencia protegida and LEI. Nao sugestao."
            ),
            "law": (
                "Constituicao da Republica: Ausencia medica and pessoal "
                "and DIREITO PROTEGIDO. Assembleia aprovou (88%). "
                "Penalizar ausencia and INCONSTITUCIONAL."
            ),
        }
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("  OPENABSENCE -- DIREITO DE AUSENCIA PROTEGIDO")
    print("  Assembleia Constituinte: ausencia and direito. Sem perguntas.")
    print("=" * 80)
    # === 1. ASSEMBLEIA ===
    ...
    print("  COM CONDICOES:              {result['votes_conditional']:>6} ({result['votes_conditional']/100:.0f}%)")
    print("  CONTRA:                     {result['votes_against']:>6} ({result['votes_against']/100:.0f}%)")
    print("\n  RESULTADO: {result['result']}")
    print("\n  DECISAO DA ASSEMBLEIA:")
    for each (key, val) in result["decision"].items():
        if isinstance(val, logico):
            print("    {key:<35} {'SIM' if val else 'NAO'}")
        else:
            print("    {key:<35} {val}")
    ...
    para cid, name, atype, dur in absences_data:
        ...
        print("\n  {r['citizen']:<12} -> {r['type']:<20} {r['duration']:<15} "
            "[{r['status']}]")
        print("    Visivel: '{r['visible_to_others']}'  Penalizada: {r['penalized']}")
        print("    Cobertura: {r['coverage']['covered']}")
    # === 3. VERIFICAR STATUS (o que outros veem) ===
    print("\n\n  === 3. O QUE OUTROS VEEM (privacidade) ===\n")
    for cid in ["C-001", "C-002", "C-005", "C-999"]:
        status = engine.check_status(cid)
        print("  {status.get('citizen', cid):<12} -> {status['status']}")
        if "message" in status  and  "ausente" in status.get("status", "").lower():
            print("    {status['message'][:70]}")
    ...
    print("  QUEM: {viol['citizen']}")
    print("  O QUE: {viol['violation']}")
    print("  TIPO: {viol['type']}")
    print("  CONSEQUENCIA: {viol['consequence'][:80]}")
    print("  LEI: {viol['law'][:80]}")
    # === 7. COMPARACAO ===
    ...
    print("  {'Aspecto':<20} {'Sistema Atual':<35} {'Republica'}")
    print("  {'-'*80}")
    para aspect, atual, rep in comparisons:
        print("  {aspect:<20} {atual:<35} {rep}")
    # === 8. STATS ===
    print("\n\n  === 8. ESTATISTICAS ===\n")
    s = engine.stats()
    ...
    # === FILOSOFIA ===
    print("\n\n{'='*80}")
    print("  FILOSOFIA: DIREITO DE AUSENCIA")
    print("{'='*80}")
    print("""
ASSEMBLEIA APROVOU (88%):
    Ausencia medica and pessoal and DIREITO PROTEGIDO.
...
8 TIPOS DE AUSENCIA PROTEGIDA:
    ...
    5. PESSOAL: motivo pessoal (not PRECISA EXPLICAR)
...
O QUE and PROTEGIDO:
    - not PRECISA EXPLICAR (P2 privacidade)
    - not PRECISA ATESTADO
    - not PENALIZADA (credito, reputacao, prontuario)
    - COBERTURA GARANTIDA (OpenLaborRelay)
    - VISIVEL SO COMO 'AUSENTE' (sem motivo)
    - VOLTA QUANDO ESTIVER PRONTO (sem pressao)
O QUE not EXISTE MAIS:
    - Chefe aprovando ausencia
...
SAUDE MENTAL:
    ...
    Republica: DIREITO. Pausa mental and saude.
...
PRINCIPIOS:
    ...
    P2: Corpo and mente soberanos. Ninguem explica o que faz com o corpo.
    P3: Cobertura por OpenLaborRelay. Ninguem sobrecarregado.
    P4: Assembleia votou (88%). Direito inalienavel.
# )
    print("{'='*80}")
    print("  OpenAbsence: {s['total_ausencias']} ausencias protegidas. "
        "Penalizadas: {s['penalizadas']}. Credito afetado: {s['credito_afetado']}.")
    print("  Ausencia and direito. Sem perguntas. Sem pressao.")
    print("{'='*80}")
