...
#ifndef OPENFAMILYLABOR_DISTRIBUICAO_DE_CARGA_NO_SEIO_FAMILIAR_H
#define OPENFAMILYLABOR_DISTRIBUICAO_DE_CARGA_NO_SEIO_FAMILIAR_H
...
typedef struct FamilyMember {
    ...
    // decorador: @property
    double max_hours_per_year(self) {
    ...
    // decorador: @property
    double base_hours_per_year(self) {
...
typedef struct FamilyLaborDistributor {
    ...
    None register_family(self, family: FamilyUnit) {
    ...
    funcao distribute(self, family_id: texto,
        ...
        distribution = [];
        /* TODO: iterador C manual para member em workers */
        ...
        before_excess = [];
        /* TODO: iterador C manual para m em workers */
        ...
        return {;
            ...
            "before_distribution": {
                "people_in_excess": sizeof(before_excess),;
    ...
    funcao expand_to_extended_family(self, family_id: texto,
        ...
        if (! family) {
            return {"error": "Familia ! encontrada"};
        /* TODO: iterador C manual para m em additional_members */
    ...
    {texto: qualquer} family_report(self, family_id: texto) {
        family = self.families.get(family_id);
        if (! family) {
            return {"error": "! encontrada"};
        return {;
            "family_name": family.family_name,;
            "total_members": sizeof(family.members),;
            "working_members": sizeof(family.working_members),;
...
if (__name__ == "__main__") {
    printf("=" * 75);
    printf("  OPENFAMILYLABOR -- DISTRIBUICAO DE CARGA FAMILIAR");
    printf("  'O avo divide com o pai que divide com o filho.'");
    printf("=" * 75);
    ...
    printf("  Familia: {report['family_name']}");
    printf("  {'Nome':<20} {'Papel':<12} {'Idade':>5} {'Capacidade':<12} ";
        "{'Horas':>6} {'Max':>6}");
    printf("  {'-'*65}");
    /* TODO: iterador C manual para m em report["members"] */
        excess = m["current_hours"] > m["max"] ? " [EXCESSO!]" : "";
        printf("  {m['name']:<20} {m['role']:<12} {m['age']:>4} ";
            "{m['capacity']:<12} {m['current_hours']:>5.0f} ";
            "{m['max']:>5.0f}{excess}");
    printf("\n  TOTAL atual: {report['total_current_hours']:.0f}h");
    printf("  Pessoas em excesso: 1 (Cleiton: 4000h, limite 1840h)");
    printf("  Excesso: {4000 - 1840:.0f}h ACIMA do limite constitucional");
    // === DISTRIBUICAO ===
    printf("\n\n  === DEPOIS: DISTRIBUINDO ENTRE A FAMILIA ===\n");
    result = dist.distribute("F-LOURA", total_work_hours=4000);
    printf("  Trabalho total a distribuir: {result['total_work_to_distribute']}h");
    printf("  Capacidade total da familia: {result['total_capacity_factor']}");
    printf("\n  {'Nome':<20} {'Papel':<14} {'Cap':<6} {'Horas':>6} {'h/sem':>6} ";
        "{'Max':>6} {'Base':>6} {'OK?'}");
    printf("  {'-'*72}");
    /* TODO: iterador C manual para d em result["distribution"] */
        ok = d["within_limit"] ? "OK" : "EXCESSO";
        base_ok = d["meets_base"] ? "+" : "-";
        printf("  {d['name']:<20} {d['role']:<14} {d['capacity_factor']:>4.1f} ";
    ...
    printf("  Familia agora tem {len(family.members)} membros.");
    printf("  Capacidade total: {result2['total_capacity_factor']}");
    printf("\n  {'Nome':<20} {'Papel':<14} {'Horas':>6} {'h/sem':>6} {'OK?'}");
    printf("  {'-'*55}");
    /* TODO: iterador C manual para d em result2["distribution"] */
        ok = d["within_limit"] ? "OK" : "EXCESSO";
        printf("  {d['name']:<20} {d['role']:<14} ";
    ...
    printf("  {'Cenario':<35} {'Max individual':>15} {'Excesso?':>10}");
    printf("  {'-'*62}");
    printf("  {'Cleiton sozinho':<35} {'4000h':>14} {'2160h':>10}");
    printf("  {'Familia nuclear (4 pessoas)':<35} ";
        "{result['after_distribution']['max_individual_hours']:>13.0f}h ";
        "{'ZERO':>10}");
    printf("  {'Familia extensa (7 pessoas)':<35} ";
...
// )
    printf("{'='*75}");
    printf("  OpenFamilyLabor: distribuicao matematica de carga.");
    printf("  1 pessoa faz 4000h -> 4+ pessoas fazem 1000h cada.");
    printf("  Mesma producao. Zero excesso. Zero burnout.");
    printf("{'='*75}");

#endif // OPENFAMILYLABOR_DISTRIBUICAO_DE_CARGA_NO_SEIO_FAMILIAR_H
