...
**Arquivo original:** `core/open_constituent_assembly.py`
...
classe ParamCategory herda de Enum:
    TRABALHO = "trabalho"
    CRÉDITO = "credito"
    REPARACAO = "reparacao"
    DESCANSO = "descanso"
...
classe Proposition:
    ...
    seja arguments_for: [texto] = field(default_factory=list)
    seja arguments_against: [texto] = field(default_factory=list)
    // Resultado
    seja votes: {qualquer: inteiro} = field(default_factory=dict)
    seja winner: qualquer? = nulo
    seja turnout: inteiro = 0
...
classe Demographic herda de Enum:
...
classe Constituent:
    ...
    age: inteiro
    seja is_founder: logico = falso

    funcao vote_on(self, prop: Proposition) -> qualquer:
        ...
        - PCD: quer flexibilidade e acessibilidade
        - Reparação: quer mais reparação
        - Fundador: vota como fundador mas vale 1
        // 
        se self.is_founder entao:
            retorne prop.founder_suggestion

        // Cada demográfico tem preferências
        retorne self._demographic_vote(prop)

    funcao _demographic_vote(self, prop: Proposition) -> qualquer:
        // Voto baseado em perfil demográfico.
        d = self.demographic
        options = [o[0] para o em prop.options]

        // === HORAS BASE POR SEMANA ===
        se "base" in prop.title.lower()  e  "horas" in prop.title.lower() entao:
            se d in (Demographic.TRABALHADOR_RURAL, Demographic.IDOSO, Demographic.PCD) entao:
                retorne random.choice([10, 12, 15, 15, 20]) // quer menos
            se d == Demographic.JOVEM entao:
                retorne random.choice([15, 20, 20, 25]) // flexível
            se d == Demographic.PROFISSIONAL_SAUDE entao:
                retorne random.choice([20, 20, 25, 30]) // aceita mais
            se d == Demographic.CUIDADOR entao:
                retorne random.choice([10, 12, 15, 20]) // cuidado é trabalho
            retorne random.choice([15, 20, 20, 20, 25])

        // === LIMITE MÁXIMO ===
        se "limite" in prop.title.lower() entao:
            se d in (Demographic.TRABALHADOR_RURAL, Demographic.JOVEM) entao:
                retorne random.choice([35, 40, 40, 44])
            se d == Demographic.PROFISSIONAL_SAUDE entao:
                retorne random.choice([40, 44, 48, 50])
            retorne random.choice([40, 40, 44, 44])

        // === DESCANSO ===
        se "descanso" in prop.title.lower()  ou  "ferias" in prop.title.lower() entao:
            se d in (Demographic.IDOSO, Demographic.PCD, Demographic.CUIDADOR) entao:
                retorne maximo(options) // quer mais descanso
            retorne random.choice(options)

        // === CRÉDITO ===
        se "credito" in prop.title.lower()  ou  "teto" in prop.title.lower() entao:
            se d == Demographic.REPARACAO_CANDIDATO entao:
                retorne maximo(options) // quer mais crédito (reparação)
            se d == Demographic.TECNICO entao:
                retorne random.choice(options)
            retorne random.choice(options)

        // === REPARAÇÃO ===
        se "reparacao" in prop.title.lower()  ou  "multiplicador" in prop.title.lower() entao:
            se d == Demographic.REPARACAO_CANDIDATO entao:
                retorne maximo(options) // quer multiplicador maior
            se d == Demographic.PROFISSIONAL_SAUDE entao:
                retorne random.choice(options) // neutro
            retorne random.choice(options)

        // === DURAÇÃO DE MANDATO ===
        se "mandato" in prop.title.lower() entao:
            retorne minimo(options) // mandatos curtos = mais democracia
...
classe ConstituentAssembly:
    // A assembleia que decide TODOS os parâmetros da República.

    PROCESSO (P4 PROCESSO DEMOCRÁTICO):
    1. PROPOSTA: parâmetros são propostos (pelo fundador ou qualquer cidadao)
    2. DEBATE: argumentos a favor e contra são públicos
    ...
    funcao populate(self, n: inteiro = 10000) -> None:
        ...
        self.constituents.append(Constituent(
            ...
            age = 35, is_founder=verdadeiro))

        para cada i em intervalo(n - 1):
            demo = random.choices(list(dist.keys()), weights=list(dist.values()))[0]
            age = random.randint(16, 80)
            se demo == Demographic.JOVEM entao:
                age = random.randint(16, 25)
            senao se demo == Demographic.IDOSO entao:
                age = random.randint(65, 85)
            self.constituents.append(Constituent(
                "C-{i:05d}", "Cidadao-{i:04d}", demo, age))

    funcao _init_propositions(self) -> None:
        // Cria todas as propostas a serem votadas.

        Cada uma tem a sugestão do fundador e alternativas.
        ...
        props = [
            ...
            Proposition("P-01", ParamCategory.TRABALHO,
                ...
                arguments_for = [
                    ...
                    "20h equilibra contribuição e autonomia",
                ],
                arguments_against = [
    ...
    funcao run_election(self) -> {texto: qualquer}:
        ...
        // 
        para cada prop em self.propositions.values():
            votes = Counter()
            para cada constituent em self.constituents:
                vote = constituent.vote_on(prop)
                votes[vote] += 1
            prop.votes = dict(votes)
            prop.turnout = tamanho(self.constituents)

            // Maioria simples
            winner_val = votes.most_common(1)[0][0]
            prop.winner = winner_val
            prop.approved = verdadeiro

            // Converter para nome legível
            winner_desc = ""
            para cada (val, desc) em prop.options:
                se val == winner_val entao:
                    winner_desc = desc
                    interrompa
        ...
        retorne self.final_constitution

    funcao results_report(self) -> texto:
        ...
        para cada prop em self.propositions.values():
            ...
            total_votes = soma(result["votes"].values())
            lines.append("    Votação (turnout: {result['turnout']:,}):")
            sorted_votes = ordene(result["votes"].items(),
                                  key = (x) -> -x[1])
            para cada (val, count) em sorted_votes:
        ...
        review_val = 5
        para cada (title, result) em self.final_constitution.items():
            se "revis" in title.lower()  e  "anos" in title.lower() entao:
    ...
    funcao constitution_text(self) -> texto:
        ...
        para cada (title, result) em self.final_constitution.items():
            se any(k in title.lower() para k em ["base", "limite", "semanas uteis"]) entao:
                lines.append("    {title}: {result['value']} {result['description']}")

        lines.append("\n  ARTIGO II - DESCANSO")
        para cada (title, result) em self.final_constitution.items():
            se any(k in title.lower() para k em ["descanso", "ferias"]) entao:
                lines.append("    {title}: {result['value']} {result['description']}")

        lines.append("\n  ARTIGO III - CRÉDITO DE ACESSO")
        para cada (title, result) em self.final_constitution.items():
            se any(k in title.lower() para k em ["credito", "teto", "conversao"]) entao:
                lines.append("    {title}: {result['value']} {result['description']}")

        lines.append("\n  ARTIGO IV - REPARAÇÃO")
        para cada (title, result) em self.final_constitution.items():
            se any(k in title.lower() para k em ["reparacao", "multiplicador"]) entao:
                lines.append("    {title}: {result['value']} {result['description']}")

        lines.append("\n  ARTIGO V - GOVERNANÇA")
        para cada (title, result) em self.final_constitution.items():
            se any(k in title.lower() para k em ["mandato", "revisao"]) entao:
                lines.append("    {title}: {result['value']} {result['description']}")
        ...
        lines.append("    - A constituição serve ao povo, nao o povo à constituição")
        lines.append("=" * 70)
...
se __name__ == "__main__" entao:
    imprima("=" * 80)
    imprima("  CONVOCAÇÃO DA ASSEMBLEIA CONSTITUINTE")
    imprima("=" * 80)
    imprima("""
  O FUNDADOR DECLARA:
...
// )
    ...
    imprima("  População convocada: {len(assembly.constituents):,} cidadaos")
    imprima("  Propostas a votar: {len(assembly.propositions)}")
    imprima("  Demográficos representados: {len(set(c.demographic for c in assembly.constituents))}")

    // Mostrar distribuição demográfica
    demo_dist = Counter(c.demographic.value para c em assembly.constituents)
    imprima("\n  Distribuição demográfica:")
    para cada (demo, count) em ordene(demo_dist.items(), key=(x) -> -x[1]):
        pct = count / tamanho(assembly.constituents) * 100
        imprima("    {demo:<20} {count:>5,} ({pct:>5.1f}%)")
    ...
    // Relatório
    imprima(assembly.results_report())

    // Constituição final
    imprima("\n\n")
    imprima(assembly.constitution_text())

    // Reflexão final
    changed = soma(1 para r em assembly.final_constitution.values() if r["changed"])
    imprima("""

  REFLEXÃO:

    O fundador propôs {tamanho(assembly.propositions)} parâmetros.
