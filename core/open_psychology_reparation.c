...
#ifndef OPENPSYCHOLOGYREPARATION_REAVALIACAO_E_REPARACAO_DE_DIAGNOSTICOS_H
#define OPENPSYCHOLOGYREPARATION_REAVALIACAO_E_REPARACAO_DE_DIAGNOSTICOS_H
...
// ============================================================================
// decorador: @dataclass
typedef struct DiagnosisCase {
    // Um caso de diagnostico a ser reavaliado.
    case_id: texto;
    citizen_id: texto;
    citizen_name: texto;
    original_diagnosis: texto // diagnostico original;
    char* diagnosing_doctor = "";
    char* diagnosing_institution = "";
    char* diagnosis_date = ""  // quando foi diagnosticado;
    int diagnosis_age = 0 // idade na epoca;
    int current_age = 0;
    char* medication_prescribed = "";
    ...
    bool apology_required = true;
    bool public_record_correction = true // apagar rotulo do prontuario;
    // Status
    char* status = "pendente"  // pendente, reavaliado, reparado;
...
// ============================================================================
typedef struct DiagnosisReassessment {
    ...
    {texto: qualquer} submit_case(self, case: DiagnosisCase) {
    ...
    double _calculate_harm(self, case: DiagnosisCase) {
        ...
        /* TODO: iterador C manual para h em case.harm_categories */
    ...
    void _calculate_reparation(self, case: DiagnosisCase) retorna (flutuante, flutuante) {
        ...
        return hours, credit;
    char* _verdict(self, case: DiagnosisCase) {
        // Veredicto textual.
    ...
    {texto: qualquer} report(self) {
        ...
        total_hours = 0;
        /* TODO: iterador C manual para case em self.cases.values() */
...
// ============================================================================
typedef struct SufferingCompensation {
    ...
    {texto: qualquer} process_compensation(self, case_id: texto) {
        ...
        return compensation;
    char* _who_apologizes(self, case: DiagnosisCase) {
    ...
    {texto: texto} _support_plan(self, case: DiagnosisCase) {
...
// ============================================================================
DiagnosisReassessment _seed_cases(void) {
    ...
    /* TODO: iterador C manual para case em cases */
        r.submit_case(case);
    return r;
// ============================================================================
// 6. MAIN
...
if (__name__ == "__main__") {
    ...
    comp.reassessor = r;
    printf("=" * 80);
    printf("  OPENPSYCHOLOGYREPARATION");
    printf("  Reavaliacao de Diagnosticos + Pagamento por Sofrimento");
    printf("=" * 80);
    // === 1. CASOS SUBMETIDOS ===
    printf("\n\n  === 1. CASOS PARA REAVALIACAO: {len(r.cases)} ===\n");
    printf("  {'ID':<10} {'Cidadao':<14} {'Diagnostico':<20} ";
        "{'Idade':>6} {'Anos rotulo':>11} {'Medicado'}");
    printf("  {'-'*80}");
    /* TODO: iterador C manual para c em r.cases.values() */
        printf("  {c.case_id:<10} {c.citizen_name:<14} ";
    ...
    r.reassess("CASE-006", ErrorType.CHILD_LABEL,;
            ...
            "Infancia roubada.");
    /* TODO: iterador C manual para cid em r.cases */
        c = r.cases[cid];
        printf("\n  [{c.case_id}] {c.citizen_name}");
        printf("  Diagnostico original: {c.original_diagnosis}");
        printf("  Erro: {c.error_type.value}");
        printf("  Dano: {c.harm_severity_score:.0f}/100");
        printf("  Responsavel: {c.liable.value}");
        printf("  Danos: {c.harm_description}");
        printf("  VEREDICTO: {r._verdict(c)}");
    // === 3. COMPENSACAO ===
    ...
    /* TODO: iterador C manual para cid em r.cases */
        ...
        printf("  Diagnostico: {result['diagnosis']}");
        comp_components = result["components"];
        printf("  Trabalho reconhecido: ";
            "{comp_components['trabalho_reconhecido_horas']:.0f}h ";
            "({comp_components['trabalho_reconhecido_anos']:.1f} anos)");
        printf("  Credito de acesso: {comp_components['credito_acesso']}");
        printf("  Retificacao: {comp_components['retificacao_prontuario'][:60]}");
        printf("  Desculpas: {comp_components['pedido_desculpas'][:80]}");
        plan = comp_components['apoio_continuo'];
        printf("  Apoio: {', '.join(plan.keys())}");
    // === 4. RELATORIO GERAL ===
    printf("\n\n  === 4. RELATORIO GERAL ===\n");
    rep = r.report();
    printf("  Casos reavaliados:     {rep['reassessed']}");
    printf("  Diagnosticos revogados:{rep['overturned']}");
    printf("  Dano medio:            {rep['avg_harm_score']:.0f}/100");
    printf("  Reparacao total:       {rep['total_reparation_hours']:,.0f}h ";
    ...
    /* para liable, count in ordene(rep["by_liable"].items(), */
                                key = (x) -> -x[1]):;
        printf("    {liable:<25} {count}");
    printf("\n  Por tipo de dano:");
    /* para harm, count in ordene(rep["by_harm"].items(), */
                            key = (x) -> -x[1]):;
        printf("    {harm:<25} {count}");
    // === FILOSOFIA ===
    printf("\n\n{'='*80}");
    printf("  POLITICA DE REPARACAO PSICOLOGICA");
    printf("{'='*80}");
    printf(""";
...
// )
    printf("{'='*80}");
    printf("  Reavaliados: {rep['reassessed']} | ";
        "Revogados: {rep['overturned']} | ";
        "Reparacao: {rep['total_reparation_hours']:,.0f}h");
    printf("{'='*80}");

#endif // OPENPSYCHOLOGYREPARATION_REAVALIACAO_E_REPARACAO_DE_DIAGNOSTICOS_H
