...
#ifndef OPENREPUBLIC_INTERFACE_DE_REALIDADE_AUMENTADA_COM_WEBCAM_H
#define OPENREPUBLIC_INTERFACE_DE_REALIDADE_AUMENTADA_COM_WEBCAM_H
...
typedef struct GestureRecognizer {
    ...
    [HandState] process(self, hands_data: [Dict]) {
    ...
    GestureType _recognize(self, lm: np.ndarray) {
    ...
    int _count_fingers(lm: np.ndarray) {
        ...
        return count;
    // decorador: @staticmethod
    bool _is_finger_extended(lm: np.ndarray, finger: texto) {
    ...
    // decorador: @staticmethod
    double _pinch_strength(lm: np.ndarray) {
    ...
    // decorador: @staticmethod
    void _palm_center(lm: np.ndarray) retorna (flutuante, flutuante) {
        // Centro da palma.
    ...
    // decorador: @staticmethod
    double _stability(hand: Handedness, landmarks: np.ndarray) {
...
typedef struct VirtualObject {
    ...
    bool grabbed = false // sendo segurado por gestos;
    char* grabbed_by = ""  // qual mao;
    bool visible = true;
    bool interactive = true;
    char* label = "";
    {texto: qualquer} metadata = field(default_factory=dict);
typedef struct ARScene {
    ...
    funcao add_object(self, obj_type: ObjectType,
        ...
        return obj;
    void remove_object(self, obj_id: texto) {
        self.objects.pop(obj_id, NULL);
    void update(self, hands: [HandState]) {
        // Atualizar cena com base nos gestos detectados.
        self.active_hands = hands;
        /* TODO: iterador C manual para hand em hands */
            /* TODO: iterador C manual para obj em self.objects.values() */
        ...
        // Escala de duas maos
        if sizeof(hands) == 2 && all(h.gesture == GestureType.TWO_HAND_SCALE;
                                /* TODO: iterador C manual para h em hands) */
            /* TODO: iterador C manual para obj em self.objects.values() */
                if (obj.grabbed) {
                    dist = math.sqrt(;
...
typedef struct ARApplication {
    ...
    void load_scenario(self, scenario: ARScenario) {
        ...
        if (scenario == ARScenario.EDUCATION_ATOM) {
            ...
            nucleus.scale = 2.0;
            /* TODO: iterador C manual para i em intervalo(3) */
            ...
            /* TODO: iterador C manual para b em [block1, block2, block3] */
                b.color = (200, 150, 50);
    ...
    {texto: qualquer} process_frame(self, hands_data: [Dict]) {
...
if (__name__ == "__main__") {
    printf("=" * 75);
    printf("  OPENREPUBLIC -- REALIDADE AUMENTADA COM WEBCAM");
    printf("  'Sem oculos. Sem headset. So webcam && gestos.'");
    printf("=" * 75);
    app = ARApplication();
    // Simulate hand data (MediaPipe-style output)
    void make_hand(handedness="direita", gesture="open", pos=(0.5, 0.5)) {
        ...
        if (gesture == "fist") {
            ...
            /* TODO: iterador C manual para tip em [4, 8, 12, 16, 20] */
                base[tip] = (base[tip][0], base[tip][1] + 0.1, 0);
            /* TODO: iterador C manual para pip em [6, 10, 14, 18] */
            ...
            /* TODO: iterador C manual para tip em [12, 16, 20] */
                base[tip] = (base[tip][0], base[tip][1] + 0.1, 0);
            /* TODO: iterador C manual para pip em [10, 14, 18] */
                base[pip] = (base[pip][0], base[pip][1] + 0.05, 0);
            base[4] = (base[4][0], base[4][1] + 0.05, 0);
        // Offset por position
        /* TODO: iterador C manual para i em intervalo(21) */
    ...
    printf("  Objetos na cena: {len(app.scene.objects)}");
    /* TODO: iterador C manual para o em app.scene.objects.values() */
        printf("    {o.label}: pos={o.position} scale={o.scale}");
    ...
    printf("  Gesto: {frame['gestures'][0]['gesture']}");
    printf("  Pinca: {frame['gestures'][0]['pinch']}");
    grabbed = [o para o em frame['objects'] if o['grabbed']];
    printf("  Objetos grabbados: {len(grabbed)}");
    // Move
    printf("\n  Frame 2: move mao para esquerda");
    frame = app.process_frame([make_hand("direita", "pinch", (0.4, 0.5))]);
    /* TODO: iterador C manual para o em frame['objects'] */
        if (o['grabbed']) {
            printf("    {o['label']}: nova pos={o['pos']}");
    // Release
...
#endif // OPENREPUBLIC_INTERFACE_DE_REALIDADE_AUMENTADA_COM_WEBCAM_H
