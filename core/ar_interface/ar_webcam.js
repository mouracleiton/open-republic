...
class GestureRecognizer {
    ...
    _count_fingers(lm: np.ndarray) {
        ...
        return count;
    // decorador: @staticmethod
    _is_finger_extended(lm: np.ndarray, finger: texto) {
    ...
    // decorador: @staticmethod
    _palm_center(lm: np.ndarray) retorna (flutuante, flutuante) {
        // Centro da palma.
    ...
    // decorador: @staticmethod
    _stability(hand: Handedness, landmarks: np.ndarray) {
...
class VirtualObject {
    ...
    obj_id: texto;
    obj_type: ObjectType;
    const position = (0.5, 0.5, 0.0) // x,y,z normalizado;
    const rotation = (0.0, 0.0, 0.0) // graus;
    const scale = 1.0;
    const color = (100, 200, 100);
    const grabbed = false // sendo segurado por gestos;
    const grabbed_by = ""  // qual mao;
    const visible = true;
    const interactive = true;
    const label = "";
    const metadata = field(default_factory=dict);
class ARScene {
    ...
    funcao add_object(self, obj_type: ObjectType,
                const position = (0.5, 0.5, 0.0),;
                const label = "") -> VirtualObject:;
        self._obj_counter += 1;
        ...
        return obj;
    remove_object(self, obj_id: texto) {
        self.objects.pop(obj_id, null);
    update(self, hands: [HandState]) {
        ...
        // Escala de duas maos
        if .length(hands) == 2 && all(h.gesture == GestureType.TWO_HAND_SCALE;
...
class ARApplication {
    ...
    load_scenario(self, scenario: ARScenario) {
    ...
    process_frame(self, hands_data: [Dict]) {
...
if (__name__ == "__main__") {
    console.log("=" * 75);
    console.log("  OPENREPUBLIC -- REALIDADE AUMENTADA COM WEBCAM");
    console.log("  'Sem oculos. Sem headset. So webcam && gestos.'");
    console.log("=" * 75);
    app = ARApplication();
    // Simulate hand data (MediaPipe-style output)
    make_hand(handedness="direita", gesture="open", pos=(0.5, 0.5)) {
        ...
        if (gesture == "fist") {
            ...
            for (const tip of [12, 16, 20]) {
                base[tip] = (base[tip][0], base[tip][1] + 0.1, 0);
            for (const pip of [10, 14, 18]) {
                base[pip] = (base[pip][0], base[pip][1] + 0.05, 0);
            base[4] = (base[4][0], base[4][1] + 0.05, 0);
        // Offset por position
        for (const i of intervalo(21)) {
    ...
    console.log("  Objetos na cena: {len(app.scene.objects)}");
    for (const o of app.scene.objects.values()) {
        console.log("    {o.label}: pos={o.position} scale={o.scale}");
    ...
    console.log("  Gesto: {frame['gestures'][0]['gesture']}");
    console.log("  Pinca: {frame['gestures'][0]['pinch']}");
    grabbed = [o para o em frame['objects'] if o['grabbed']];
    console.log("  Objetos grabbados: {len(grabbed)}");
    // Move
    console.log("\n  Frame 2: move mao para esquerda");
    frame = app.process_frame([make_hand("direita", "pinch", (0.4, 0.5))]);
    for (const o of frame['objects']) {
        if (o['grabbed']) {
            console.log("    {o['label']}: nova pos={o['pos']}");
    // Release
    ...
    console.log("  Pecas para montar: {len(app.scene.objects)}");
    for (const o of app.scene.objects.values()) {
        console.log("    {o.label}: pos={o.position}");
    console.log("\n  Frame: pega a Base");
    frame = app.process_frame([make_hand("direita", "pinch", (0.3, 0.3))]);
    grabbed = [o para o em frame['objects'] if o['grabbed']];
    console.log("  Grabbed: {[o['label'] for o in grabbed]}");
    // === Test 3: Governance Vote ===
    console.log("\n\n  === CENARIO: VOTACAO POR GESTO ===\n");
    app.load_scenario(ARScenario.GOVERNANCE_VOTE);
    console.log("  Proposta na tela");
    for (const o of app.scene.objects.values()) {
        console.log("    {o.label}");
    // Vote YES
