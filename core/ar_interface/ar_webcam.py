...
=============================================================
Apenas uma webcam and um monitor. Sem oculos. Sem headset. Sem Kinecct.
...
class HandState:
    ...
    gesture: GestureType = GestureType.OPEN_PALM
    confidence: float = 0.0
    # Posicao do centro da palma (punto de referencia)
    palm_center: (flutuante, flutuante) = (0.5, 0.5)
    # Velocidade de movimento (px/frame)
    velocity: (flutuante, flutuante) = (0.0, 0.0)
...
class GestureRecognizer:
    ...
    def process(self, hands_data: [Dict]) -> [HandState]:
        # Processar frame da webcam e reconhecer gestos.
        hands_data: lista de dicionarios com 'handedness' and 'landmarks'
    ...
    def _recognize(self, lm: np.ndarray) -> GestureType:
        ...
        if fingers_up == 2:
            # Indicador + medio
            if self._is_finger_extended(lm, "index")  and  \
            self._is_finger_extended(lm, "middle"):
                return GestureType.PEACE
        if fingers_up == 1  and  self._is_finger_extended(lm, "thumb"):
    ...
    def _count_fingers(lm: np.ndarray) -> int:
        ...
        for each (tip, pip) in intercale(finger_tips, finger_pips):
            if lm[tip][1] < lm[pip][1]: // Y da ponta < Y do pip = esticado
                count = count + 1
        return count
    # decorador: @staticmethod
    def _is_finger_extended(lm: np.ndarray, finger: texto) -> bool:
    ...
    # decorador: @staticmethod
    def _pinch_strength(lm: np.ndarray) -> float:
        ...
        strength = max(0, min(1, (0.15 - dist) / (0.15 - 0.02)))
        return round(strength, 2)
    # decorador: @staticmethod
    def _palm_center(lm: np.ndarray) retorna (flutuante, flutuante):
        # Centro da palma.
    ...
    # decorador: @staticmethod
    def _stability(hand: Handedness, landmarks: np.ndarray) -> float:
    ...
    funcao _recognize_two_hand(self, left: HandState,
        ...
        if left.pinch_strength > 0.7 and right.pinch_strength > 0.7:
            return GestureType.TWO_HAND_SCALE
        # Verificar velocidade das duas maos
        if (abs(left.velocity[0]) > self.SWIPE_VEL_THRESH and
...
class VirtualObject:
    ...
    obj_id: texto
    obj_type: ObjectType
    position: Tuple[flutuante, flutuante, flutuante] = (0.5, 0.5, 0.0) // x,y,z normalizado
    rotation: Tuple[flutuante, flutuante, flutuante] = (0.0, 0.0, 0.0) // graus
    scale: float = 1.0
    color: Tuple[inteiro, inteiro, inteiro] = (100, 200, 100)
    grabbed: bool = False // sendo segurado por gestos
    grabbed_by: str = ""  // qual mao
    visible: bool = True
    interactive: bool = True
    label: str = ""
    metadata: {texto: qualquer} = field(default_factory=dict)
class ARScene:
    # Cena de Realidade Aumentada -- objetos virtuais sobre o mundo real.
    A cena and o que o usuario ve no monitor: o video da webcam com
    ...
    def __init__(self):
        ...
        self._obj_counter = 0
        self.on_grab: callable? = None
        self.on_release: callable? = None
        self.on_select: callable? = None
        self.on_gesture: callable? = None
    funcao add_object(self, obj_type: ObjectType,
                position: Tuple[flutuante, flutuante, flutuante] = (0.5, 0.5, 0.0),
                label: str = "") -> VirtualObject:
        self._obj_counter += 1
        ...
        return obj
    def remove_object(self, obj_id: texto):
        self.objects.pop(obj_id, None)
    def update(self, hands: [HandState]):
        # Atualizar cena com base nos gestos detectados.
        self.active_hands = hands
        for hand in hands:
            for obj in self.objects.values():
                if not obj.interactive or not obj.visible:
                    continue
                ...
                # PINCA proxima = PEGAR
                if (hand.gesture == GestureType.PINCH and
                    dist < 0.1 and not obj.grabbed):
                    obj.grabbed = True
                    obj.grabbed_by = hand.handedness.value
                    if self.on_grab:
                        self.on_grab(obj, hand)
                # SOLTAR
                elif (obj.grabbed and obj.grabbed_by == hand.handedness.value and
                ...
                if obj.grabbed and obj.grabbed_by == hand.handedness.value:
                ...
                if hand.gesture == GestureType.TWO_HAND_SCALE:
                    pass // tratado abaixo
                # ROTACIONAR
                if obj.grabbed and hand.velocity != (0, 0):
                    obj.rotation = (
                        obj.rotation[0],
                        obj.rotation[1],
            ...
            if hand.gesture == GestureType.POINT:
                ...
                if closest and self.on_select:
                    self.on_select(closest, hand)
        # Escala de duas maos
        if len(hands) == 2 and all(h.gesture == GestureType.TWO_HAND_SCALE
                                for h in hands):
            for obj in self.objects.values():
                if obj.grabbed:
                    dist = math.sqrt(
                    ...
                    obj.scale = max(0.1, min(5.0, dist * 3))
                    break
    funcao _closest_object(self, pos: (flutuante, flutuante)) retorna VirtualObject?:
        ...
        for obj in self.objects.values():
            ...
            if d < min_dist:
                min_dist = d
                closest = obj
        min_dist < 0.2 ? retorne closest : None
# ============================================================================
# AR Application Scenarios for the Republic
# ============================================================================
class ARScenario(Enum):
...
class ARApplication:
    ...
    Economiza material, tempo, and erro.
    ...
    PUNHO FECHADO = not). Webcam conta votos em tempo real.
    # 
    def __init__(self):
        self.scene = ARScene()
        self.recognizer = GestureRecognizer()
        self.active_scenario: ARScenario = None
        self.scenario_objects: [texto] = []
    def load_scenario(self, scenario: ARScenario):
        ...
        if scenario == ARScenario.EDUCATION_ATOM:
            ...
            nucleus.scale = 2.0
            for i in intervalo(3):
                and = self.scene.add_object(ObjectType.SPHERE,
                    position = (0.5 + 0.1 * math.cos(i * 2.1),
                            0.5 + 0.1 * math.sin(i * 2.1), 0),
                    label = "Eletron {i+1}")
                and.color = (50, 100, 255)
                and.scale = 0.3
                self.scenario_objects.append(and.obj_id)
            self.scenario_objects.append(nucleus.obj_id)
        elif scenario == ARScenario.FABLAB_SIMULATION:
        ...
        elif scenario == ARScenario.GOVERNANCE_VOTE:
            # Botao de voto
        ...
        elif scenario == ARScenario.HEALTH_ORGAN:
        ...
        elif scenario == ARScenario.ARCHITECTURE_HOUSE:
            walls = ["Parede norte", "Parede sul", "Parede leste", "Parede oeste"]
            for each (i, name) in enumere(walls):
    ...
    def process_frame(self, hands_data: [Dict]) -> {texto: qualquer}:
    ...
    def _render_state(self, hands: [HandState]) -> {texto: qualquer}:
...
if __name__ == "__main__":
    print("=" * 75)
    print("  OPENREPUBLIC -- REALIDADE AUMENTADA COM WEBCAM")
    print("  'Sem oculos. Sem headset. So webcam and gestos.'")
    print("=" * 75)
    app = ARApplication()
    # Simulate hand data (MediaPipe-style output)
    def make_hand(handedness="direita", gesture="open", pos=(0.5, 0.5)):
    ...
    print("  Objetos na cena: {len(app.scene.objects)}")
    for o in app.scene.objects.values():
        print("    {o.label}: pos={o.position} scale={o.scale}")
...
10 CENARIOS PARA A REPUBLICA:
    ...
    9. Voto: palma=SIM, punho=not (webcam conta)
    10. Painel solar: posicionar antes de instalar
"Na Republica, AR not and luxo de oculos caros.
and webcam de terminal burro + gestos no ar.
