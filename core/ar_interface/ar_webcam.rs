...
struct GestureRecognizer {
    ...
    fn process(self, hands_data: [Dict]) -> [HandState] {
    ...
    fn _recognize(self, lm: np.ndarray) -> GestureType {
    ...
    fn _count_fingers(lm: np.ndarray) -> i64 {
        ...
        return count;
    // decorador: @staticmethod
    fn _is_finger_extended(lm: np.ndarray, finger: texto) -> bool {
    ...
    // decorador: @staticmethod
    fn _pinch_strength(lm: np.ndarray) -> f64 {
    ...
    // decorador: @staticmethod
    fn _palm_center(lm: np.ndarray) retorna (flutuante, flutuante) {
        // Centro da palma.
    ...
    // decorador: @staticmethod
    fn _stability(hand: Handedness, landmarks: np.ndarray) -> f64 {
...
struct VirtualObject {
    ...
    obj_id: texto;
    obj_type: ObjectType;
    let position: Tuple[flutuante, flutuante, flutuante] = (0.5, 0.5, 0.0) // x,y,z normalizado;
    let rotation: Tuple[flutuante, flutuante, flutuante] = (0.0, 0.0, 0.0) // graus;
    let scale: f64 = 1.0;
    let color: Tuple[inteiro, inteiro, inteiro] = (100, 200, 100);
    let grabbed: bool = false // sendo segurado por gestos;
    let grabbed_by: String = ""  // qual mao;
    let visible: bool = true;
    let interactive: bool = true;
    let label: String = "";
    let metadata: {texto: qualquer} = field(default_factory=dict);
#[derive(Debug, Clone)]
struct ARScene {
    ...
    funcao add_object(self, obj_type: ObjectType,
                let position: Tuple[flutuante, flutuante, flutuante] = (0.5, 0.5, 0.0),;
                let label: String = "") -> VirtualObject:;
        self._obj_counter += 1;
        ...
        return obj;
    fn remove_object(self, obj_id: texto) {
        self.objects.pop(obj_id, NULL);
    fn update(self, hands: [HandState]) {
...
struct ARApplication {
    ...
    fn load_scenario(self, scenario: ARScenario) {
    ...
    fn process_frame(self, hands_data: [Dict]) -> {texto: qualquer} {
    ...
    fn _render_state(self, hands: [HandState]) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    println!("=" * 75);
    println!("  OPENREPUBLIC -- REALIDADE AUMENTADA COM WEBCAM");
    println!("  'Sem oculos. Sem headset. So webcam && gestos.'");
    println!("=" * 75);
    app = ARApplication();
    // Simulate hand data (MediaPipe-style output)
    fn make_hand(handedness="direita", gesture="open", pos=(0.5, 0.5)) {
    ...
    println!("  Objetos na cena: {len(app.scene.objects)}");
    for o in app.scene.objects.values() {
        println!("    {o.label}: pos={o.position} scale={o.scale}");
    ...
    println!("  Gesto: {frame['gestures'][0]['gesture']}");
    println!("  Pinca: {frame['gestures'][0]['pinch']}");
    grabbed = [o para o em frame['objects'] if o['grabbed']];
    println!("  Objetos grabbados: {len(grabbed)}");
    // Move
    println!("\n  Frame 2: move mao para esquerda");
    frame = app.process_frame([make_hand("direita", "pinch", (0.4, 0.5))]);
    for o in frame['objects'] {
        if o['grabbed'] {
            println!("    {o['label']}: nova pos={o['pos']}");
    // Release
