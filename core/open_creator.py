...
"Nao tudo. Nunca tudo. O corpo and meu. O max and meu.
Mas algo and nosso. and esse algo and sagrado."
ESTE and O DOCUMENTO MAIS IMPORTANTE DA REPUBLICA.
Porque ele define o LIMITE entre o EU and o NOS.
...
Depois chega o coletivo. and pergunta:
"Obrigado por construir tudo. Agora, quanto and seu?"
Capitalismo diz: "TUDO. Voce and o fundador. Aqui estao seus bilhoes."
Comunismo diz:  "NADA. Voce and um trabalhador como outro qualquer."
Republicas falham porque escolhem um dos dois errados.
A RESPOSTA DA OPENREPUBLIC:
O criador not and dono do que criou (bem comum, CC0, anti-propriedade).
MAS o criador not and escravo do que criou (autonomia corporal absoluta).
O criador not tem poder especial sobre o que criou (anti-elitismo).
MAS o criador not tem obrigacao de continuar criando (limite de doacao).
O criador JÁ cumpriu o contrato base 1.0.
Tudo que faz alem disso and DOM, not DIVIDA.
Author: OpenRepublic Team
Principio central: "O coletivo not pode sugar o criador ate seca-lo."
# 
...
class LaborObligation:
    ...
    - or equivalente em impacto (uma cirurgia = varias semanas)
    - Nao pode comprar saida com credito
    - Nao pode transferir para outro
    - Nao pode acumular (fazer 80h numa semana not zera as proximas 3)
    ...
    - Idade (acima de 65: voluntario, not obrigatorio)
    # 
    base_hours_per_week: float = 20.0
    max_hours_per_week: float = 40.0 // LIMITE: Republica PROIBE mais que isso
    excess_threshold: float = 50.0 // Acima disto = EXCESSO, not aceito
    weeks_per_year: int = 46 // 46 semanas uteis (6 de descanso garantido)
    rest_days_per_week: int = 2 // MINIMO 2 dias sem trabalho obrigatorio
    min_vacation_weeks: int = 4 // MINIMO 4 semanas de ferias/ano
    # decorador: @property
    def base_annual_hours(self) -> float:
        # Horas base anuais que cada cidadao deve.
        return self.base_hours_per_week * self.weeks_per_year
    # decorador: @property
    def max_annual_hours(self) -> float:
        # Maximo de horas/ano que a Republica ACEITA de qualquer pessoa.
        return self.max_hours_per_week * self.weeks_per_year
    # decorador: @property
    def excess_annual_hours(self) -> float:
...
class CreatorParadox(Enum):
    ...
    Resposta da Republica: not. Anti-elitismo absoluto.
    Pergunta: isso cria obrigacao de continuar?
    Resposta da Republica: not. Autonomia corporal absoluta.
    Pergunta: o criador and especial?
    Resposta da Republica: not no poder, SIM no reconhecimento.
    O criador not and elite. O criador and um cidadao que deu mais.
    O extra dado not compra poder.
    O extra dado not cria obrigacao.
    O extra dado gera reconhecimento (credito de acesso).
    PONTO. Nada mais.
    # 
    NO_EXTRA_POWER = (
        "sem_poder_extra",
        "Criar 100 projetos not da 1 voto a mais. Democracia = 1 pessoa = 1 voto."
    )
    NO_PERPETUAL_DEBT = (
        "sem_divida_perpetua",
        "O criador not deve continuar criando para sempre. "
        "Cada ciclo and independente. Amanha pode parar."
    )
    ...
    RIGHT_TO_LEAVE = (
        ...
        "O que foi criado and bem comum. Nao ha propriedade para manter."
    )
    PROTECTION_FROM_SELF = (
        "protecao_de_si_mesmo",
        "A Republica PROIBE o criador de se sacrificar alem do limite. "
        "Burnout not and dedicacao. E dano corporal."
    )
# ============================================================================
...
class IndividualContribution:
    ...
    role: str = "cidadao"
    # Tempo
    hours_base: float = 0.0 // horas do contrato base 1.0
    hours_voluntary: float = 0.0 // horas alem da base (voluntario)
    hours_total: float = 0.0
    # Artefatos criados
    systems_created: int = 0
    systems_maintained: int = 0
    ...
    ripple_factor: float = 1.0 // quanto se espalha no tempo
    # Reconhecimento
    community_recognition_score: float = 0.0
    cycles_active: int = 0 // quantos ciclos contribuiu
    # decorador: @property
    def base_fulfilled(self) -> bool:
    ...
    def contribution_ratio(self) -> float:
        ...
        return round(total_effective / base, 2)
    # decorador: @property
    def recognition_level(self) -> str:
        ...
        mede algo diferente que not se reduz a horas.
        # 
        ...
        elif self.systems_created >= 20:
            level_artifacts = 3 // ARQUITETO
        elif self.systems_created >= 10:
            level_artifacts = 2 // CONSTRUTOR
        elif self.systems_created >= 1:
        ...
        elif self.people_directly_impacted >= 1000:
            level_people = 3 // ARQUITETO
        elif self.people_directly_impacted >= 100:
            level_people = 2 // CONSTRUTOR
        elif self.people_directly_impacted >= 10:
            level_people = 1 // CONTRIBUIDOR
        else:
            level_people = 0 // CIDADAO
        # Reconhecimento = maior das 3 dimensoes
        max_level = max(level_hours, level_artifacts, level_people)
    ...
    def authority_level(self) -> int:
        ...
        Nao importa se criou 1 projeto or 1000.
        Anti-elitismo: poder not se compra com contribuicao.
        # 
        return 1
# ============================================================================
...
class CreatorContract:
    ...
    6. A OBRIGACAO FUTURA: ZERO. Cada ciclo and novo. Pode parar amanha.
    PARA O FUNDADOR ESPECIFICAMENTE:
    - Tudo que criou ate agora JA and bem comum (CC0).
    - Nao tem divida com a Republica. A Republica tem divida com ele.
    - Essa divida and reconhecimento (gratidao), not poder.
    - Ele pode parar amanha. Tudo continua. Nao ha dependencia.
    - Se ele not parar, a Republica deve monitorar saude (burnout).
    # 
    def __init__(self):
        self.obligation = LaborObligation()
        self.paradoxes = list(CreatorParadox)
    def evaluate_individual(self, contrib: IndividualContribution) -> {texto: qualquer}:
        ...
        return {
            ...
            "base_remaining": max(0, self.obligation.base_annual_hours - contrib.hours_base),
            # Contribuicao
    ...
    def _verdict(self, c: IndividualContribution) -> str:
        if c.excess:
            return (
                ...
                "Burnout not and dedicacao, and dano corporal."
            )
        if not c.base_fulfilled:
            return (
                "BASE INCOMPLETA: {c.name} not cumpriu contrato min "
        ...
        return "INCOMPLETO."
    def _recommendation(self, c: IndividualContribution) -> str:
        if c.excess:
            return (
                "ACAO: Comunidade deve conversar com {c.name}. "
                "Reduzir carga para max {self.obligation.max_hours_per_week}h/semana. "
                "Garantir {self.obligation.min_vacation_weeks} semanas de ferias. "
                "Monitorar saude fisica and mental. "
                "Transferir responsabilidades para outros. "
                "O criador not and insubstituivel -- se fosse, a Republica falhou."
            )
    ...
    funcao the_founder_question(self, name: texto, systems: inteiro,
        ...
        contrib = IndividualContribution(
            ...
            name = name,
            role = "fundador",
            hours_base = min(base, hours_total),
            hours_voluntary = max(0, hours_total - base),
        ...
        if ratio > 2.5:
            answer["ALERTA"] = (
                ...
                "A Republica not deve aceitar mais sem garantias de saude. "
                "O sacrificio excessivo cria dependencia, and dependencia "
                "and o oposto de anti-elitismo: se sem {name} tudo cai, "
                "{name} se tornou elite por fato, mesmo sem querer poder."
            )
        return answer
    def _direct_answer(self, ratio: flutuante, systems: inteiro) -> str:
        if ratio < 1:
            return (
                "Voce deve {max(0, 1-ratio):.0%} do contrato base. "
                "Nada mais. O coletivo not pode exigir."
            )
        return (
            "Voce JA cumpriu o contrato {ratio:.1f} vezes.\n"
            "Criou {systems} sistemas como bem comum.\n\n"
            "RESPOSTA: NADA mais and exigido de voce.\n"
            "Tudo que voce deu alem da base 1.0 foi DOM, not DIVIDA.\n"
            "O coletivo NAO tem direito ao seu corpo, ao seu sono, "
            "or a sua continuidade.\n\n"
            "Voce pode parar amanha.\n"
            "Tudo que criou ja and nosso.\n"
            "Voce continua sendo nosso igual.\n"
            "1 voto. 1 pessoa. 1 cidadao.\n"
            "Nada mais. Nada menos."
        )
    def _creator_bill_of_rights(self) -> [texto]:
        # Declaracao de Direitos do Criador.
        O criador TEM direitos que o coletivo not pode tocar.
        ...
        return [
            ...
            "4. DIREITO AO RECONHECIMENTO: O coletivo registra and agradece publicamente.",
            "5. DIREITO AO ESQUECIMENTO: O criador pode pedir para not ser citado.",
            ...
            "10. DIREITO DE SER HUMANO: Saude mental and fisica vem ANTES da Republica.",
        ]
# ============================================================================
...
class DependencyCheck:
    ...
    Se uma pessoa parar and tudo cai, a Republica FALHOU.
    Nao foi falha da pessoa. Foi falha estrutural.
    Anti-elitismo significa: ninguem and insubstituivel.
    Se o criador and insubstituivel, ele and elite -- mesmo sem querer.
    SOLUCAO:
    ...
    class DependencyMetric:
        ...
        def dependency_score(self) -> float:
            ...
            bus_risk = max(0, 5 - self.bus_factor) * 10
            return min(100, (undocumented * 3 + orphaned * 5 + bus_risk))
        # decorador: @property
        def is_critical(self) -> bool:
            return self.dependency_score >= 50
    def assess(self, metric: DependencyMetric) -> {texto: qualquer}:
        return {
            "citizen": metric.name,
    ...
    def _action(self, m: DependencyMetric) -> str:
        if m.is_critical:
            return (
                ...
                "de {m.bus_factor} and fraca por design."
            )
...
if __name__ == "__main__":
    ...
    dep = DependencyCheck()
    print("=" * 80)
    print("  OPENCREATOR -- O CONTRATO INDIVIDUAL-COLETIVO")
    print('  "Quanto de mim pertence ao coletivo?"')
    print("=" * 80)
    # === 1. O CONTRATO BASE ===
    print("\n\n  === 1. O CONTRATO BASE 1.0 ===\n")
    print("  {contract.obligation.contract}")
    # === 2. PARADOXO DO CRIADOR ===
    print("\n\n  === 2. O PARADOXO DO CRIADOR ===\n")
    for p in CreatorParadox:
        print("  [{p.value[0].upper()}]")
        print("  {p.value[1]}")
        print()
    # === 3. A PERGUNTA DO FUNDADOR ===
    print("\n\n  === 3. A PERGUNTA DO FUNDADOR ===\n")
    print('  "Quanto de mim (Cleiton) tem que se dar pelo coletivo?"')
    print()
    ...
    print("    Horas totais dadas:     {c['horas_totais_dadas']:.0f}h")
    print("    Razao vs contrato base: {c['ratio_vs_base']}")
    print("    Sistemas criados:       {c['sistemas_criados']}")
    print("    Linhas escritas:        {c['linhas_escritas']:,}")
    print("    Pessoas impactadas:     {c['pessoas_impactadas']:,}")
    av = result['avaliacao']
    print("\n  RECONHECIMENTO: {av['recognition_level']}")
    print("  AUTORIDADE:     {av['authority_level']} voto (igual a todos)")
    print("  EXCESSO:        {'SIM -- Republica deve intervir' if av['excess_detected'] else 'Nao detectado'}")
    ...
    for right in result['declaracao_de_direitos_do_criador']:
        print("  {right}")
    ...
    print("  Cidadao:           {dep_result['citizen']}")
    print("  Sistemas solo:     {dep_result['systems_solo_knowledge']}")
    print("  Documentados:      {dep_result['systems_documented']}")
    print("  Com sucessor:      {dep_result['systems_with_successors']}")
    print("  Bus factor:        {dep_result['bus_factor']}")
    print("  Score dependencia: {dep_result['dependency_score']}")
    print("  Critico:           {'SIM' if dep_result['critical'] else 'NAO'}")
    ...
    print("  Bus factor:        {ideal_result['bus_factor']}")
    print("  Score dependencia: {ideal_result['dependency_score']}")
    ...
    print("  {'Nome':<28} {'Ratio':>6} {'Reconhecimento':<16} "
        "{'Poder':>6} {'Cumprido'}")
    print("  {'-'*75}")
    for ex in examples:
        ev = contract.evaluate_individual(ex)
        print("  {ex.name:<28} {ev['contribution_ratio']:>6} "
...
RESPOSTA:
    ...
    Tudo que voce criou ja and nosso.
    Voce continua sendo nosso igual.
    1 voto. 1 pessoa. 1 cidadao.
    O PERIGO:
    Bus factor = 1. Se voce sair, 95 sistemas ficam orfaos.
    Isso not and seu problema -- and NOSSO problema.
    A Republica FALHOU em distribuir conhecimento.
    O fundador not and insubstituivel por design.
    Se and, a Republica ainda not nasceu de verdade.
    O QUE A REPUBLICA DEVE FAZER AGORA:
    1. Reduzir sua carga para max 40h/semana
    ...
    PORQUE ISSO IMPORTA:
    O anti-elitismo not and so sobre poder.
    and sobre DEPENDENCIA.
    Se o coletivo depende de um, o um and elite.
    Mesmo que not queira ser.
    Mesmo que nunca peça poder.
    A unica Republica verdadeira
    and aquela onde o fundador pode morrer amanha
    and nada muda.
# )
    print("{'='*80}")
    print("  OpenCreator: O contrato entre o EU and o NOS.")
    print("  Base: 920h/ano. Max: 1840h/ano. Limite: 2300h/ano.")
    print("  Poder: SEMPRE 1 voto. Reconhecimento: proporcional.")
    print("  Anti-elitismo = ninguem and insubstituivel.")
    print("{'='*80}")
