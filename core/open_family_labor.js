...
class FamilyMember {
    ...
    const current_hours = 0.0 // horas que ja trabalha;
    const specialties = field(default_factory=list) // areas de competencia;
    // decorador: @property
    capacity_factor(self) {
...
class FamilyLaborDistributor {
    ...
    register_family(self, family: FamilyUnit) {
    ...
    funcao distribute(self, family_id: texto,
        ...
        return {;
            ...
            "before_distribution": {
                "people_in_excess": .length(before_excess),;
    ...
    funcao expand_to_extended_family(self, family_id: texto,
        ...
        if (! family) {
            return {"error": "Familia ! encontrada"};
        for (const m of additional_members) {
    ...
    family_report(self, family_id: texto) {
        family = self.families.get(family_id);
        if (! family) {
            return {"error": "! encontrada"};
        return {;
            "family_name": family.family_name,;
            "total_members": .length(family.members),;
            "working_members": .length(family.working_members),;
...
if (__name__ == "__main__") {
    console.log("=" * 75);
    console.log("  OPENFAMILYLABOR -- DISTRIBUICAO DE CARGA FAMILIAR");
    console.log("  'O avo divide com o pai que divide com o filho.'");
    console.log("=" * 75);
    ...
    console.log("  Familia: {report['family_name']}");
    console.log("  {'Nome':<20} {'Papel':<12} {'Idade':>5} {'Capacidade':<12} ";
        "{'Horas':>6} {'Max':>6}");
    console.log("  {'-'*65}");
    for (const m of report["members"]) {
        excess = m["current_hours"] > m["max"] ? " [EXCESSO!]" : "";
        console.log("  {m['name']:<20} {m['role']:<12} {m['age']:>4} ";
            "{m['capacity']:<12} {m['current_hours']:>5.0f} ";
            "{m['max']:>5.0f}{excess}");
    console.log("\n  TOTAL atual: {report['total_current_hours']:.0f}h");
    console.log("  Pessoas em excesso: 1 (Cleiton: 4000h, limite 1840h)");
    console.log("  Excesso: {4000 - 1840:.0f}h ACIMA do limite constitucional");
    // === DISTRIBUICAO ===
    console.log("\n\n  === DEPOIS: DISTRIBUINDO ENTRE A FAMILIA ===\n");
    result = dist.distribute("F-LOURA", total_work_hours=4000);
    console.log("  Trabalho total a distribuir: {result['total_work_to_distribute']}h");
    console.log("  Capacidade total da familia: {result['total_capacity_factor']}");
    ...
    for (const d of result["distribution"]) {
        ...
        console.log("  {d['name']:<20} {d['role']:<14} {d['capacity_factor']:>4.1f} ";
    ...
    console.log("  Familia agora tem {len(family.members)} membros.");
    console.log("  Capacidade total: {result2['total_capacity_factor']}");
    console.log("\n  {'Nome':<20} {'Papel':<14} {'Horas':>6} {'h/sem':>6} {'OK?'}");
    console.log("  {'-'*55}");
    for (const d of result2["distribution"]) {
        ok = d["within_limit"] ? "OK" : "EXCESSO";
        console.log("  {d['name']:<20} {d['role']:<14} ";
    ...
    console.log("  Carga minima: ";
        "{result2['after_distribution']['min_individual_hours']:.0f}h");
    // === COMPARACAO ===
    console.log("\n\n  === COMPARACAO: ANTES VS DEPOIS ===\n");
    console.log("  {'Cenario':<35} {'Max individual':>15} {'Excesso?':>10}");
    console.log("  {'-'*62}");
    console.log("  {'Cleiton sozinho':<35} {'4000h':>14} {'2160h':>10}");
    console.log("  {'Familia nuclear (4 pessoas)':<35} ";
        "{result['after_distribution']['max_individual_hours']:>13.0f}h ";
        "{'ZERO':>10}");
    console.log("  {'Familia extensa (7 pessoas)':<35} ";
...
// )
    console.log("{'='*75}");
    console.log("  OpenFamilyLabor: distribuicao matematica de carga.");
    console.log("  1 pessoa faz 4000h -> 4+ pessoas fazem 1000h cada.");
    console.log("  Mesma producao. Zero excesso. Zero burnout.");
    console.log("{'='*75}");
