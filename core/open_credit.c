...
#ifndef OPENCREDIT_SISTEMA_DE_CREDITO_DEMOCRATICO_H
#define OPENCREDIT_SISTEMA_DE_CREDITO_DEMOCRATICO_H
...
typedef struct ContributionType {
...
typedef struct GoodOrService {
    ...
    char* description = "";
    char* category = "";
    bool scarce = false // oferta limitada?;
...
typedef struct CreditAccount {
    ...
    // decorador: @property
    double can_spend(self) {
        // Quanto pode gastar neste ciclo.
        return maximo(0, self.current_credits);
    {texto: qualquer} spend(self, amount: flutuante, good_name: texto) {
...
typedef struct DemocraticCreditSystem {
    ...
    //
    void __init__(self, pool_per_cycle: flutuante = 1000.0) {
    ...
    void add_citizen(self, citizen: Citizen) {
    ...
    {texto: qualquer} run_cycle(self) {
        ...
        self.cycle += 1;
        n = sizeof(self.citizens);
        if (n == 0) {
            return {"error": "no citizens"};
        // 1. Expirar creditos do ciclo anterior
        expired_total = 0;
        /* TODO: iterador C manual para c em self.citizens.values() */
        ...
        result = {
            ...
            "proposals_voted": sizeof(proposals),;
        ...
        return result;
    [Dict] _generate_proposals(self, contributions: Dict, total_weighted: flutuante) {
        // Gerar propostas de distribuicao para votacao.
        n = sizeof(contributions);
        proposals = [];
        ...
        return proposals;
    Dict _simulate_vote(self, proposals: [Dict]) {
    ...
    {texto: qualquer} access_good(self, citizen_id: texto, good_name: texto) {
...
if (__name__ == "__main__") {
    random.seed(42);
    printf("=" * 75);
    printf("  OPENCREDIT -- CREDITO DEMOCRATICO DE ACESSO");
    printf("  'Nao && moeda. E acesso. Nao acumula. A comunidade decide.'");
    printf("=" * 75);
    ...
    // === Catalog ===
    printf("\n\n  === CATALOGO DE BENS ===\n");
    printf("  DIREITOS GARANTIDOS (sem credito, sempre):");
    /* TODO: iterador C manual para g em CATALOG */
    ...
    printf("  {'Bem':<35} {'Credito':>8}");
    printf("  {'-'*45}");
    /* TODO: iterador C manual para g em CATALOG */
        if (g.tier == AccessTier.CREDIT) {
            printf("  {g.name:<35} {g.credit_cost:>7.0f}");
    // === Simular 3 ciclos ===
    printf("\n\n  === SIMULACAO: 3 CICLOS DE CREDITO DEMOCRATICO ===\n");
    contribution_types = list(ContributionType);
    /* TODO: iterador C manual para cycle_num em intervalo(1, 4) */
        ...
        /* TODO: iterador C manual para c em system.citizens.values() */
        ...
        printf("  CICLO {result['cycle']}:");
        printf("    Proposta vencedora: {result['winning_proposal']}");
        printf("    {result['winning_description'][:70]}...");
        printf("    Pool: {result['pool_total']:.0f} | Cidadaos: {result['citizens']}");
        printf("    Min: {result['min_given']} | Max: {result['max_given']} ";
            "| Media: {result['avg_given']}");
        printf("    Expirado ciclo anterior: {result['expired_from_last_cycle']}");
        ...
        /* para cada (name, credits) em ordene(result["distribution"].items()): */
            ...
            printf("    {name:<10} {c.account.contribution_hours:>5.0f}h {credits:>8.1f}");
        // Simular alguns gastos
        printf("\n    Gastos no ciclo:");
        spenders = random.sample(list(system.citizens.values()), 4);
        /* TODO: iterador C manual para c em spenders */
...
// )
    // === Resumo final ===
    printf("{'='*75}");
    printf("  RESUMO DO OPENCREDIT");
    printf("{'='*75}\n");
    printf("  1. ESSNCIAIS sao DIREITOS (sem credito):");
    printf("     Comida, agua, moradia, saude, educacao, transporte");
    printf("     -> Voce ACESSE. Nao compra. Nao gasta credito.\n");
    printf("  2. BENS ESCASSOS precisam de CREDITO:");
    printf("     Instrumentos, viagens, eletronicos especiais");
    printf("     -> Voce gasta credito de acesso do ciclo\n");
    printf("  3. CREDITO && distribuido por TODOS:");
    printf("     A comunidade vota como dividir o pool");
    printf("     Base igual para todos + bonus por contribuicao");
    printf("     -> Quem contribui mais, tem mais acesso ao que && escasso\n");
    printf("  4. CREDITO NAO E MOEDA:");
    printf("     Expira todo ciclo (! acumula)");
    printf("     E pessoal (! transfere)");
    printf("     Tem piso (ninguem fica sem) && teto (ninguem domina)");
    printf("     Nao compra essencial (essencial && direito)\n");
    printf("  5. CONTRIBUICAO && reconhecida pela comunidade:");
    printf("     Nao && 'salario'. E reconhecimento.");
    printf("     Saude, educacao && comida valem mais que arte.");
    printf("     Mas arte tambem conta.\n");
    printf("  RESULTADO:");
    printf("  Ninguem passa fome porque 'acabou o credito'.");
    printf("  Ninguem acumula fortuna porque 'trabalhou mais'.");
    printf("  Todos tem o essencial garantido.");
    printf("  O escasso && distribuido democraticamente.");
    printf("  A cada ciclo, recomeca. Sem aculumulo. Sem heranca.");
    printf("  Sem classe. Sem exploracao. Sem dominacao.\n");
    printf("{'='*75}");

#endif // OPENCREDIT_SISTEMA_DE_CREDITO_DEMOCRATICO_H
