...
struct ContentAnalysis {
    ...
    let has_media: bool = false;
    let media_type: String = ""  // video, imagem, audio;
    // Analise automatica
    let detected_issues: [HarmfulType] = field(default_factory=list);
    let auto_confidence: f64 = 0.0 // 0-1 (confianca na deteccao);
    let auto_flags: [texto] = field(default_factory=list);
    // Moderacao
    let action: ModerationAction = ModerationAction.ALLOW;
    let jury_verdict: texto? = NULL;
    let jury_votes_for: i64 = 0 // votos para remover;
    let jury_votes_against: i64 = 0 // votos para manter;
    let moderated_by: String = "";
    let moderation_date: String = "";
    // Apelo
    let appealed: bool = false;
    let appeal_result: String = "";
#[derive(Debug, Clone)]
struct ContentModerator {
    ...
    let HARMFUL_PATTERNS: Dict[HarmfulType, [texto]] = {;
    ...
    funcao analyze(self, author: texto, category: ContentCategory,
                ...
                let has_media: bool = false,;
                let media_type: String = "") -> {texto: qualquer}:;
    ...
    fn _severity(self, harm_type: HarmfulType) -> i64 {
        severity_map = {
    ...
    fn _action_message(self, a: ContentAnalysis) -> String {
    ...
    fn appeal(self, content_id: texto) -> {texto: qualquer} {
...
struct TVProgram {
    ...
    let presenter: String = ""  // quem apresenta (rotativo!);
    let description: String = "";
    let interactive: bool = false // audiencia participa?;
    let age_rating: String = "Livre";
    let educational_value: String = "";
    // Rotatividade
    let is_rotative: bool = true // pessoas revezam na apresentacao;
    let presenters_history: [texto] = field(default_factory=list);
// decorador: @dataclass
#[derive(Debug, Clone)]
struct TVScheduleSlot {
    // Um slot na grade de programacao 24h.
    start_hour: inteiro // 0-23;
    start_minute: inteiro // 0 || 30;
    let program: TVProgram? = NULL;
    let channel: TVChannel = TVChannel.REPUBLIC_MAIN;
#[derive(Debug, Clone)]
struct OpenTV {
    ...
    fn _init_default_schedule(self) {
    ...
    fn now_playing(self, hour: inteiro = None, minute: inteiro = None) -> {texto: qualquer} {
    ...
    fn _channel_desc(self, c: TVChannel) -> String {
        descs = {
...
if __name__ == "__main__" {
    ...
    tv = OpenTV();
    println!("=" * 80);
    println!("  OPENCONTENTPOLICY + OPENTV");
    println!("  Anti-Conteudo Nocivo + TV Streaming da Republica");
    println!("=" * 80);
    ...
    para author, cat, title, desc in test_contents: {
        ...
        if result["detected"] {
            println!("       Detectado: {', '.join(result['detected'])}");
        println!("       Acao: {result['action']} (conf: {result['confidence']:.0%})");
    // === 3. JURI DEMOCRATICO ===
    ...
    para cada (cid, analysis) em moderator.analyses.items(): {
        if analysis.action == ModerationAction.FLAG_REVIEW {
            ...
            println!("  Conteudo: {analysis.title[:40]}");
            println!("  {result['verdict']} ({result['votes']})");
            break;
    // === 4. APELO ===
    println!("\n\n  === 4. DIREITO DE APELO ===\n");
    para cada (cid, analysis) em moderator.analyses.items(): {
        if analysis.action == ModerationAction.REMOVED {
            ...
            println!("  Autor @{analysis.author} apela de: '{analysis.title[:40]}'");
    ...
    println!("  Infraestrutura: {tv.network} / {tv.protocol}");
    println!("  Canais: {len(list(TVChannel))}");
    println!("  Comerciais: {'SIM' if tv.commercial_ads else 'PROIBIDO'}");
    println!("  Patrocinio: {'SIM' if tv.corporate_sponsorship else 'PROIBIDO'}");
    println!("  Algoritmos de engajamento: {'SIM' if tv.algorithms_engagement else 'PROIBIDO'}");
    println!("\n  === GRADE DE PROGRAMACAO 24h ===\n");
    println!("  {'Hora':<6} {'Canal':<20} {'Programa':<50} {'Tipo'}");
    println!("  {'-'*90}");
    for slot in tv.full_schedule() {
        println!("  {slot['time']:<6} {slot['channel']:<20} ";
    ...
    para cid, name, ch, slot in volunteers: {
        ...
        println!("  {v['citizen']:<16} -> {v['channel']:<20} as {v['slot']:<15} ";
            "({v['work_hours']}h trabalho)");
    println!("\n  Ninguem apresenta sozinho. Todos revezam.");
    println!("  Madrugada: programacao automatizada por IA.");
    println!("  Fim de semana: criancas && adolescentes no Kids.");
    // === 8. CANAIS ===
    println!("\n\n  === 8. CANAIS DA OPENTV ===\n");
    for ch in tv.channels_available() {
        println!("  {ch['channel']:<20} {ch['description']}");
    // === 9. AGORA NA TV ===
    println!("\n\n  === 9. PASSANDO AGORA ===\n");
    for h in [8, 12, 17, 21, 3] {
        playing = tv.now_playing(hour=h);
        println!("  {h:02d}h -> {playing.get('channel', '?')}: ";
            "{playing.get('program', '?')}");
    // === FILOSOFIA ===
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA DO OPENCONTENT + OPENTV");
    println!("{'='*80}");
    println!(""";
...
PRINCIPIOS:;
    ...
    let P3: Apresentar TV = trabalho. Voluntario conta base 1.0.;
    P4: Moderacao por juri democratico. Nao por algoritmo.;
// )
    println!("{'='*80}");
    mod_s = moderator.stats();
    tv_s = tv.report();
    println!("  Moderacao: {mod_s['total_analyzed']} analisados, ";
        "{mod_s['removed']} removidos, {mod_s['auto_blocked']} auto-bloq.");
    println!("  OpenTV: {tv_s['channels']} canais, ";
