...
struct QueueItem {
    ...
    sender_handle: texto;
    sender_name: texto;
    let sender_relation: ContactRelation = ContactRelation.UNKNOWN;
    let content_preview: String = ""  // primeiros 100 chars;
    let content_url: String = ""  // link profundo pro modulo;
    let timestamp: f64 = field(default_factory=time.time);
    // Prioridade
    let base_priority: Priority = Priority.NOISE;
    let final_priority: Priority = Priority.NOISE;
    let priority_reason: String = "";
    // Estado
    let read: bool = false;
    let archived: bool = false;
    let acted_upon: bool = false // usuario respondeu/interagiu;
    let auto_archived: bool = false // sistema arquivou (expirou);
    // Contexto
    let requires_response: bool = false // precisa de acao do usuario;
    let response_deadline: f64 = 0.0 // prazo para responder (0 = sem prazo);
    // Metadados anti-engajamento
    let has_media: bool = false;
    let is_sensational: bool = false // sistema detecta clickbait;
    let engagement_bait: bool = false // sistema detecta tentativa de viciar;
    // decorador: @property
...
struct AttentionPrioritizer {
    ...
    fn prioritize(self, item: QueueItem) -> QueueItem {
    ...
    fn detect_engagement_bait(text: texto) -> bool {
    ...
    // decorador: @staticmethod
    fn detect_sensational(text: texto) -> bool {
...
struct UnifiedInbox {
    ...
    fn mark_steward(self, handles: [texto]) -> None {
        for h in handles {
            self.steward_handles.add(h.lower().lstrip("@"));
    fn _resolve_relation(self, handle: texto) -> ContactRelation {
    ...
    fn _reset_daily_if_needed(self) -> None {
    ...
    funcao enqueue(self, source: SourceModule, event_type: EventType,
                ...
                let requires_response: bool = false,;
                let response_deadline: f64 = 0.0,;
                let has_media: bool = false) -> {texto: qualquer}:;
    ...
    fn process_batch(self, limit: inteiro = 20) -> [QueueItem] {
    ...
    fn _auto_archive(self) -> None {
        ...
        for item in self.queue {
            if item.is_expired && ! item.archived {
                ...
                item.auto_archived = true;
                self.total_archived += 1;
    fn act_on(self, item_id: texto) -> bool {
        ...
        for item in self.queue {
            if item.item_id == item_id {
                ...
                return true;
        return false;
    fn dismiss(self, item_id: texto) -> bool {
    ...
    fn digest_24h(self) -> {texto: qualquer} {
...
// ============================================================================
fn simulate_five_apps_one_inbox() -> None {
    ...
    println!("=" * 75);
    println!("  OPENINBOX -- A FILA UNIFICADA DE ATENCAO");
    println!('  "5 apps competindo = 5 ladroes de tempo. 1 fila = 1 cerebro."');
    println!("=" * 75);
    ...
    para source, etype, handle, name, preview, requires_resp, *rest in events: {
        ...
        println!("  [{status:>8}] [{prio:>9}] {source.value:<12} ";
            "@{handle:<20} {preview[:40]}");
    // === Estatisticas ===
    println!("\n\n  === FILA APOS INGESTAO ===\n");
    stats = inbox.stats();
    println!("  Total na fila:     {stats['total_in_queue']}");
    println!("  Nao lidos:         {stats['unread']}");
    println!("  Por fonte:         {stats['by_source']}");
    println!("  Por prioridade:    {stats['by_priority']}");
    println!("  Bait bloqueado:    {stats['bait_blocked']}");
    println!("  Limite diario:     {stats['items_today']}/{stats['daily_limit']}");
    // === Processar lote ===
    println!("\n\n  === PROCESSANDO LOTE (batch, ! ping) ===\n");
    batch = inbox.process_batch(limit=10);
    println!("  {'Prioridade':<12} {'Fonte':<12} {'De':<22} {'Preview'}");
    println!("  {'-'*75}");
    for item in batch {
        println!("  [{item.final_priority.name:<10}] {item.source.value:<12} ";
            "{item.sender_name:<22} {item.content_preview[:35]}");
        if item.priority_reason  &&  item.priority_reason != "padrao" {
            println!("  {'':>25} razao: {item.priority_reason}");
    // === Modo Foco ===
    println!("\n\n  === MODO FOCO ATIVADO ===\n");
    inbox.mode = InboxMode.FOCUS;
    println!("  So EMERGENCIES + FAMILIA + SAUDE passam. Resto arquivado.\n");
    inbox.queue.clear();
    ...
    for item in batch {
        println!("  [{item.final_priority.name:<10}] {item.source.value:<12} ";
            "{item.sender_name:<22} {item.content_preview[:35]}");
    if ! batch {
        println!("  (fila vazia -- tudo foi arquivado pelo Modo Foco)");
    stats = inbox.stats();
    ...
    println!("  Categorias: {digest['categories']}");
    println!("  Total: {digest['total']} itens nas ultimas 24h");
    para cada (cat, data) em digest["digest"].items(): {
        hl = "; ".join(h["from"] para h em data.get("highlights", []));
        println!("    {cat:<14} {data['count']} itens";
            hl ? + (" (destaques: {hl})" : ""));
    // === Filosofia ===
    println!("\n\n{'='*75}");
    println!("  COMO O OPENINBOX PROTEGE SEU CEREBRO");
    println!("{'='*75}");
    println!(""";
...
// )
    println!("{'='*75}");
    println!("  OpenInbox: {stats['total_enqueued']} eventos -> ";
        "{stats['total_processed']} processados -> ";
        "{stats['bait_blocked']} bait bloqueado.");
    println!("  5 apps. 1 fila. 0 ansiedade.");
    println!("{'='*75}");
// ============================================================================
