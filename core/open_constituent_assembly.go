...
package openconstituentassembly_assembleia_constituinte
...
type Proposition struct {
    ...
    arguments_for := field(default_factory=list) // [texto]
    arguments_against := field(default_factory=list) // [texto]
    // Resultado
    votes := field(default_factory=dict) // {qualquer: inteiro}
    winner := nil // qualquer?
    turnout := 0 // int64
    approved := false // bool
// ============================================================================
...
type Constituent struct {
    ...
    age: inteiro
    is_founder := false // bool
    func vote_on(self, prop: Proposition) interface{} {
    ...
    func _demographic_vote(self, prop: Proposition) interface{} {
...
// ============================================================================
type ConstituentAssembly struct {
    ...
    func populate(self, n: inteiro = 10000) None {
    ...
    func _init_propositions(self) None {
    ...
    func run_election(self) {texto: qualquer} {
        ...
        //
        for _, prop := range self.propositions.values() {
            votes = Counter()
            for _, constituent := range self.constituents {
    ...
    func results_report(self) string {
        ...
        lines.append("  {'='*80}")
        return "\n".join(lines)
    func constitution_text(self) string {
...
if __name__ == "__main__" {
    fmt.Println("=" * 80)
    fmt.Println("  CONVOCAÇÃO DA ASSEMBLEIA CONSTITUINTE")
    fmt.Println("=" * 80)
    fmt.Println("""
O FUNDADOR DECLARA:
...
// )
    ...
    fmt.Println("  População convocada: {len(assembly.constituents):,} cidadaos")
    fmt.Println("  Propostas a votar: {len(assembly.propositions)}")
    fmt.Println("  Demográficos representados: {len(set(c.demographic for c in assembly.constituents))}")
    ...
    para cada (demo, count) em ordene(demo_dist.items(), key=(x) -> -x[1]): {
        ...
        fmt.Println("    {demo:<20} {count:>5,} ({pct:>5.1f}%)")
    // Votação
    fmt.Println("\n\n  VOTAÇÃO EM ANDAMENTO...\n")
    assembly.run_election()
    // Relatório
    fmt.Println(assembly.results_report())
    // Constituição final
    fmt.Println("\n\n")
    fmt.Println(assembly.constitution_text())
    // Reflexão final
