...
package openterritory_redistribuicao_populacional_e_novas_metropoles
...
type PopulationCenter struct {
    ...
    population: inteiro
    area_km2: flutuante
    current_density := 0.0 // float64
    target_density := 0.0 // meta da Republica // float64
    status := "desconhecido"  // esvaziar, manter, crescer, criar // string
    // decorador: @property
    func density_class(self) DensityClass {
    ...
    // decorador: @property
    func overpopulated(self) bool {
        return self.current_density > 2000
    // decorador: @property
    func underpopulated(self) bool {
        return self.current_density < 50
    // decorador: @property
    func surplus_population(self) int64 {
    ...
    // decorador: @property
    func capacity_to_grow(self) int64 {
...
type NewMetropolis struct {
    ...
    current_population := 0 // geralmente <50k (interior) // int64
    area_km2 := 500.0 // float64
    // Infraestrutura planejada
    has_healthcare := false // nivel Sirio-Libanes // bool
    has_education := false // universidade + escolas // bool
    has_transport := false // OpenMobility (metro, VLT, aeroporto) // bool
    has_energy := false // OpenEnergy (solar/eolica) // bool
    has_network := false // OpenNetwork/OpenProtocol // bool
    has_fablab := false // FabLab industrial // bool
    has_terminal := false // OpenTerminal em todo estabelecimento // bool
    has_housing := false // OpenCivilConstruction // bool
    has_food_production := false // agricultura urbana periurbana // bool
    has_water := false // saneamento + reservatorio // bool
    // Status de construcao
    construction_phase := "planejamento"  // planejamento, obras, habitavel, ativa // string
    construction_progress := 0.0 // 0-1 // float64
    est_completion_months := 36 // int64
    // Atrativos (por que morar aqui?)
    incentives := field(default_factory=list) // [texto]
    // decorador: @property
    func infrastructure_ready(self) bool {
...
type TerritoryEngine struct {
    ...
    func _init_current_population(self) {
    ...
    func diagnosis(self) {texto: qualquer} {
    ...
    func new_metroplan_summary(self) {texto: qualquer} {
    ...
    func build_infrastructure(self, metro_id: texto) {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENTERRITORY -- REDISTRIBUICAO POPULACIONAL E NOVAS METROPOLES")
    fmt.Println("  'SP ta pequeno demais. O interior ! existe mais.'")
    fmt.Println("=" * 80)
    // === 1. DIAGNOSTICO ===
    fmt.Println("\n\n  === 1. DIAGNOSTICO: DESEQUILIBRIO TERRITORIAL ===\n")
    fmt.Println("  {'Cidade':<25} {'Regiao':<15} {'Populacao':>12} {'Densidade':>12} {'Status'}")
    fmt.Println("  {'-'*80}")
    para c em ordene(engine.current_centers.values(), {
                    key = (x) -> -x.current_density):
        fmt.Println("  {c.name:<25} {c.region:<15} {c.population:>12,} "
            "{c.current_density:>10.0f}/km2  {c.status.upper()}")
    // === 2. DESEQUILIBRIO ===
    fmt.Println("\n\n  === 2. O PROBLEMA ===\n")
    diag = engine.diagnosis()
    fmt.Println("  Hiper-saturadas: {diag['hiper_saturadas']}")
    fmt.Println("  Saturadas: {diag['saturadas']}")
    fmt.Println("  Vazios demograficos: {diag['vazios_demograficos']}")
    fmt.Println("  Populacao excedente (precisa sair): {diag['populacao_excedente']:,}")
    fmt.Println("  Pior caso: {diag['pior_caso']['nome']} "
        "({diag['pior_caso']['densidade']}, "
        "excedente: {diag['pior_caso']['excedente']})")
    fmt.Println("  Desequilibrio: {diag['desequilibrio']}")
    // === 3. PLANO DE NOVAS METROPOLES ===
    fmt.Println("\n\n  === 3. PLANO: {len(engine.new_metropolises)} NOVAS METROPOLES ===\n")
    plan = engine.new_metroplan_summary()
    fmt.Println("  Por regiao:")
    para cada (region, count) em ordene(plan["por_regiao"].items()): {
        fmt.Println("    {region:<15} {count} metropoles")
    fmt.Println("\n  Por len:")
    fmt.Println("    ALPHA (1M-2M):    {plan['alfa_1m_2m']}")
    fmt.Println("    BETA (500k-1M):   {plan['beta_500k_1m']}")
    fmt.Println("    GAMMA (200k-500k):{plan['gamma_200k_500k']}")
    fmt.Println("\n  Capacidade total: {plan['capacidade_total']:,} habitantes")
    // === 4. TODAS AS NOVAS METROPOLES ===
    fmt.Println("\n\n  === 4. NOVAS METROPOLES POR REGIAO ===\n")
    by_region = defaultdict(list)
    for _, m := range engine.new_metropolises.values() {
        by_region[m.region].append(m)
    for _, region := range ordene(by_region.keys()) {
        fmt.Println("\n  {region.upper()}:")
        for _, m := range by_region[region] {
            fmt.Println("    [{m.metro_id}] {m.name:<25} {m.state}  "
                "meta: {m.target_population:,}  ({m.tier.value})")
    // === 5. CONSTRUIR INFRAESTRUTURA ===
    fmt.Println("\n\n  === 5. CONSTRUCAO DE INFRAESTRUTURA ===\n")
    // Construir todas
    for _, mid := range list(engine.new_metropolises.keys()) {
        result = engine.build_infrastructure(mid)
    fmt.Println("  {len(engine.new_metropolises)} metropoles construidas.")
    fmt.Println("  Cada uma tem:")
    fmt.Println("    [OK] Hospital nivel Sirio-Libanes")
    fmt.Println("    [OK] Universidade + escolas")
    fmt.Println("    [OK] Metro/VLT + OpenMobility")
    fmt.Println("    [OK] Energia solar/eolica")
    fmt.Println("    [OK] OpenNetwork/OpenProtocol")
    fmt.Println("    [OK] FabLab industrial")
    fmt.Println("    [OK] OpenTerminal em todo lugar")
    fmt.Println("    [OK] Moradia (OpenCivilConstruction)")
    fmt.Println("    [OK] Agricultura periurbana")
    fmt.Println("    [OK] Saneamento + agua")
    ...
    para cid, name, from_city, to_metro in migrations: {
        ...
        fmt.Println("    Status saida: {result['from_status']}")
        fmt.Println("    Bonus: +{result['bonus_credit']} credito")
        fmt.Println("    Voluntario: SIM (P2)")
    // === 7. SP SENDO ESVAZIADA PARA OBRAS ===
    fmt.Println("\n\n  === 7. SAO PAULO: ESVAZIAR PARA REFORMAR ===\n")
    sp = engine.current_centers.get("Sao Paulo")
    if sp {
        fmt.Println("  ANTES:")
        fmt.Println("    Populacao: {sp.population:,}")
        fmt.Println("    Densidade: {sp.current_density:.0f} hab/km2")
        fmt.Println("    Excedente: {sp.surplus_population:,} pessoas precisam sair")
        fmt.Println("\n  DEPOIS (meta):")
        fmt.Println("    Populacao meta: {int(2000 * sp.area_km2):,}")
        fmt.Println("    Densidade meta: 2.000 hab/km2 (ainda urbano, mas respiravel)")
        fmt.Println("    Reducao: {sp.surplus_population:,} pessoas ({sp.surplus_population/sp.population*100:.0f}%)")
        fmt.Println("\n  O QUE FAZER COM ESPACO LIBERADO:")
        fmt.Println("    - Reformar infraestrutura antiga")
        fmt.Println("    - Criar espacos verdes")
        fmt.Println("    - Reconstruir moradias dignas")
        fmt.Println("    - Reduzir transito")
        fmt.Println("    - Limpar rios (Tiete, Pinheiros)")
        fmt.Println("    - Descongestionar hospitais")
    // === 8. STATS ===
    fmt.Println("\n\n  === 8. ESTATISTICAS ===\n")
    s = engine.stats()
    fmt.Println("  Novas metropoles: {s['total_metropoles']}")
    fmt.Println("  Construidas: {s['construidas']}")
    fmt.Println("  Migracoes oferecidas: {s['migracoes_oferecidas']}")
    fmt.Println("  Capacidade total: {s['plano']['capacidade_total']:,}")
    fmt.Println("  Populacao excedente: {s['diagnostico']['populacao_excedente']:,}")
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  FILOSOFIA DO OPENTERRITORY")
    fmt.Println("{'='*80}")
    fmt.Println("""
O PROBLEMA:
...
PRINCIPIOS:
    ...
    P3 := trabalho massivo (OpenLaborRelay). // Construir novas metropoles
    P4: Plano votado pela assembleia. Regioes decidem suas prioridades.
// )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenTerritory: {s['total_metropoles']} novas metropoles, "
        "{s['plano']['capacidade_total']:,} de capacidade.")
    fmt.Println("  'Interior' ! existe mais. Todo ponto && centro.")
    fmt.Println("{'='*80}")
