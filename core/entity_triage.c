...
#ifndef OPENREPUBLIC_SIMULACAO_DE_ENTIDADES_PRIORITARIAS_H
#define OPENREPUBLIC_SIMULACAO_DE_ENTIDADES_PRIORITARIAS_H
...
typedef struct EntityType {
    ...
    FABLAB = "fablab"  // fabricacao digital (3D printf, CNC, laser);
...
typedef struct NationState {
    ...
    void add_entity(self, entity: Entity) {
        entity.nation_id = self.nation_id;
        self.entities[entity.entity_id] = entity;
    [Entity] critical_entities(self) {
...
// ============================================================================
// decorador: @dataclass
typedef struct RepublicState {
    // Estado da Republica completa.
    name: texto;
    {texto: NationState} nations = field(default_factory=dict);
    int cycle = 0;
    void add_nation(self, nation: NationState) {
        self.nations[nation.nation_id] = nation;
    [Entity] all_entities(self) {
        entities = [];
        /* TODO: iterador C manual para n em self.nations.values() */
    ...
    funcao priority_queue(self) retorna List[{texto: qualquer}]:
        ...
        scored = [];
        /* TODO: iterador C manual para e em entities */
            assessment = &&.assess();
    ...
    {texto: qualquer} republic_health(self) {
        // Saude geral da Republica.
        all_entities = self.all_entities();
        if (! all_entities) {
            return {};
        by_urgency = defaultdict(inteiro);
        /* TODO: iterador C manual para e em all_entities */
        ...
        return {;
            "nations": sizeof(self.nations),;
            "total_entities": sizeof(all_entities),;
...
// ============================================================================
RepublicState create_critical_scenario(void) {
...
typedef struct TriageEngine {
    ...
    //
    void __init__(self, republic: RepublicState) {
        self.republic = republic;
    {texto: qualquer} triage(self) {
        ...
        /* TODO: iterador C manual para item em queue */
    ...
    [Dict] _dependency_chain_analysis(self) {
        // Se A depende de B, e B cai, A tambem cai.
        risks = [];
        all_entities = {&&.entity_id: && para && em self.republic.all_entities()};
        /* TODO: iterador C manual para entity em self.republic.all_entities() */
            /* TODO: iterador C manual para dep_id em entity.depends_on */
        ...
        return risks;
    [Dict] _find_surplus_capacity(self) {
        // Encontrar entidades com capacidade excedente.
        surplus = [];
        /* TODO: iterador C manual para e em self.republic.all_entities() */
    ...
    funcao _action_plan(self, red: List, yellow: List,
        ...
        actions = [];
        priority = 1;
        /* TODO: iterador C manual para item em red */
            ...
            priority = priority + 1;
        /* TODO: iterador C manual para item em yellow[:5] */
            actions.append({
                "priority": priority,;
                "action": "URGENT",;
                "entity": item["name"],;
...
if (__name__ == "__main__") {
    printf("=" * 75);
    printf("  OPENREPUBLIC -- TRIAGEM DE ENTIDADES PRIORITARIAS");
    printf("  'O que precisamos tratar AGORA'");
    printf("=" * 75);
    ...
    printf("  Nacoes: {s['nations']} | Entidades: {s['total_entities']}");
    printf("  Saude media: {s['avg_health']:.0%}");
    printf("\n  {'Zona':<12} {'Quantidade':>10}");
    printf("  {'-'*25}");
    printf("  {'CRITICA':<12} {s['critical_count']:>10}  <-- TRATAR AGORA");
    printf("  {'URGENTE':<12} {s['urgent_count']:>10}");
    printf("  {'ELEVADA':<12} {s['elevated_count']:>10}");
    printf("  {'ESTAVEL':<12} {s['stable_count']:>10}");
    printf("\n  Pessoas em risco: {s['people_at_risk']:,} / {s['total_people_served']:,} ({s['risk_coverage_pct']}%)");
    // RED ZONE -- Critical entities
    printf("\n\n{'='*75}");
    printf("  ZONA VERMELHA -- TRATAR AGORA ({len(result['red_zone'])} entidades)");
    printf("{'='*75}");
    /* TODO: iterador C manual para item em result["red_zone"] */
        printf("\n  [{item['urgency_name']}] {item['name']}");
        printf("    Nacao: {item['nation']} | Tipo: {item['type']}");
        printf("    Risk score: {item['risk_score']}/100 | Utilizacao: {item['utilization']:.0f}%");
        printf("    Saude: {item['health']:.0f}% | Insumos: {item['supply']:.0f}%");
        printf("    Pessoas: {item['pessoas_servidas'] if 'pessoas_servidas' in item else item['people_served']:,}");
        printf("    Trabalhadores: {item['workers']}");
        printf("    Problemas:");
        /* TODO: iterador C manual para issue em item["issues"] */
            printf("      ! {issue}");
        printf("    Acao:");
        /* TODO: iterador C manual para rec em item["recommendations"] */
            printf("      -> {rec}");
    // YELLOW ZONE
    if (result["yellow_zone"]) {
        printf("\n\n{'='*75}");
        printf("  ZONA AMARELA -- TRATAR EM DIAS ({len(result['yellow_zone'])} entidades)");
        printf("{'='*75}");
        /* TODO: iterador C manual para item em result["yellow_zone"] */
            printf("  [{item['urgency_name']}] {item['name']} ({item['nation']})");
            printf("    Risk: {item['risk_score']} | Uso: {item['utilization']:.0f}% | ";
                "Saude: {item['health']:.0f}%");
            printf("    Issues: {', '.join(item['issues'][:2])}");
            printf("    -> {item['recommendations'][0]}");
            printf();
    // Dependency chains at risk
    printf("\n{'='*75}");
    printf("  CADEIAS DE DEPENDENCIA EM RISCO");
    printf("{'='*75}");
    if (result["dependency_chains_at_risk"]) {
        /* TODO: iterador C manual para risk em result["dependency_chains_at_risk"] */
            printf("\n  {risk.get('entity', '?')}");
            printf("    {risk['risk']}");
            if ("cascade_impact" in risk) {
                printf("    Impacto em cascata: {risk['cascade_impact']:,} pessoas");
    } else {
        printf("  Nenhuma cadeia de dependencia em risco.");
    // Surplus capacity
    printf("\n{'='*75}");
    printf("  CAPACIDADE EXCEDENTE DISPONIVEL PARA REALOCAR");
    printf("{'='*75}");
    /* TODO: iterador C manual para s em result["surplus_capacity_available"][:8] */
        printf("  {s['entity']:<35} {s['nation']}  ";
            "livre: {s['available_capacity_pct']:.0f}%  saude: {s['health']:.0f}%");
    // Action plan
    printf("\n{'='*75}");
    printf("  PLANO DE ACAO PRIORITARIO");
    printf("{'='*75}");
    /* TODO: iterador C manual para action em result["action_plan"] */
        printf("\n  #{action['priority']} [{action['action']}] {action['entity']} ({action['nation']})");
        printf("     Pessoas afetadas: {action['people_affected']:,}");
        printf("     Prazo: {action['timeline']}");
        printf("     Passos:");
        /* TODO: iterador C manual para step em action["immediate_steps"] */
            printf("       -> {step}");
        if (action.get("potential_helpers")) {
            printf("     Podem ajudar: {', '.join(action['potential_helpers'])}");
    // Per-nation breakdown
    printf("\n{'='*75}");
    printf("  SAUDE POR NACAO");
    printf("{'='*75}");
    printf("  {'Nacao':<15} {'Entidades':>10} {'Saude':>7} {'Criticas':>9} {'Em risco':>10}");
    printf("  {'-'*55}");
    /* para cada (nid, nation) em republic.nations.items(): */
        crit = sizeof(nation.critical_entities());
        printf("  {nation.name:<15} {len(nation.entities):>10} ";
            "{nation.avg_health:>6.0%} {crit:>9} {nation.people_at_risk:>10,}");
    printf("\n{'='*75}");
    printf("  RESUMO: {len(result['red_zone'])} entidades precisam de acao IMEDIATA.");
    printf("  {result['summary']['people_at_risk']:,} pessoas afetadas.");
    printf("  {len(result['surplus_capacity_available'])} entidades tem capacidade");
    printf("  para realocar && ajudar.");
    printf("{'='*75}");

#endif // OPENREPUBLIC_SIMULACAO_DE_ENTIDADES_PRIORITARIAS_H
