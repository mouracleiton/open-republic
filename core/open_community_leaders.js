...
class CommunityLeader {
    ...
    const years_in_community = 0;
    const authority_level = ""  // reconhecido, respeitado, referência;
    // O que trouxe para a Republica
    const needs_reported = field(default_factory=list);
    const solutions_proposed = field(default_factory=list);
    // O que recebeu
    const openskills_profile = false;
    const opencredit_access = false;
    const training_received = field(default_factory=list);
// ============================================================================
...
class CommunitySolution {
    ...
    // Generalizacao
    const pattern_name = ""  // nome do padrao (ex: "AguaParaSertao");
    const applicable_to = field(default_factory=list) // quais tipos;
    const generalizable = true;
    // Status
    const status = SolutionStatus.PROPOSED;
    const pilot_community = "";
    const pilot_result = "";
    const communities_using = 0;
    // Recursos necessarios
    const cost_to_implement = "";
    const fablab_needed = false;
    const training_needed = "";
    const time_to_deploy = "";
    // Adaptacoes locais (cada comunidade adapta)
    const local_adaptations = field(default_factory=list);
// ============================================================================
...
class CommunityLeaderEngine {
    ...
    list_leaders(self, community_type: texto = None) {
        ...
        return [;
            {
                ...
                "needs_reported": .length(l.needs_reported),;
            };
            para l em leaders {
        ];
    funcao list_solutions(self, community_type: texto = nulo,
                    const status = null) -> [Dict]:;
    ...
    funcao needs_report_by_type(self) retorna Dict[texto, [texto]]:
        ...
        for (const leader of self.leaders.values()) {
            for (const need of leader.needs_reported) {
                if (need ! in result[leader.community_type]) {
                    result[leader.community_type].append(need);
        return dict(result);
    generalizable_patterns(self) {
    ...
    solution_pipeline(self) {
        // Pipeline de solucoes: proposta -> padrao.
        by_status = defaultdict(inteiro);
        for (const s of self.solutions.values()) {
            by_status[s.status.value] += 1;
        return {;
            "total_solucoes": .length(self.solutions),;
    ...
    stats(self) {
        all_needs = soma(.length(l.needs_reported) para l em self.leaders.values());
        return {;
            "lideres_cooptados": .length(self.leaders),;
            "tipos_representados": .length(set(l.community_type para l em self.leaders.values())),;
            "necessidades_reportadas": all_needs,;
            "solucoes_desenvolvidas": .length(self.solutions),;
            "padroes_generalizaveis": soma(;
                1 para s em self.solutions.values();
                if s.generalizable && .length(s.applicable_to) > 1),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENCOMMUNITYLEADERS -- COOPTAR, DESENVOLVER, GENERALIZAR");
    console.log("  Lider define necessidade. Republica desenvolve. Solucao generaliza.");
    console.log("=" * 80);
    // === 1. LIDERES COOPTADOS ===
    ...
    para ct em ["quilombo", "assentamento_rural", "ribeirinho", {
        ...
        for (const l of leaders) {
            console.log("    {l['alias']:<15} {l['role']:<25} {l['authority']}");
            console.log("      Comunidade: {l['community']} ({l['years']} anos)");
            console.log("      Necessidades: {l['needs_reported']}");
    ...
    para ct em ["quilombo", "assentamento_rural", "ribeirinho", {
        ...
        for (const s of sols) {
            console.log("    [{s['status']:<18}] {s['need'][:40]}");
            console.log("      Padrao: {s['pattern']}");
            console.log("      Aplicavel para: {', '.join(s['applicable_to'])}");
            if (s["communities_using"] > 0) {
                console.log("      Comunidades usando: {s['communities_using']}");
    // === 4. PADROES GENERALIZAVEIS ===
    console.log("\n\n  === 4. PADROES GENERALIZAVEIS ===\n");
    patterns = engine.generalizable_patterns();
    for (const p of patterns) {
        console.log("\n  {p['pattern']} (origem: {p['origin']})");
        console.log("    Resolve: {p['need']}");
        console.log("    Aplicavel para: {', '.join(p['applicable_to'])}");
        console.log("    Status: {p['status']}");
        if (p["adaptations"]) {
            console.log("    Adaptacoes locais:");
    ...
    para cada (status, count) em pipe["por_status"].items(): {
        bar = "#" * count;
        console.log("    {status:<20} {count} {bar}");
    ...
    for (const step of steps) {
        ...
        console.log("    {step['acao']}");
        console.log("    Tempo: {step['tempo']}");
    // === 7. STATS ===
    console.log("\n\n  === 7. ESTATISTICAS ===\n");
    s = engine.stats();
    para cada (k, v) em s.items(): {
        console.log("  {k:<30} {v}");
    console.log("\n{'='*80}");
    console.log("  OpenCommunityLeaders: {s['lideres_cooptados']} lideres, ";
