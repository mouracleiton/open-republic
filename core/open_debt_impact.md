...
**Arquivo original:** `open-republic/core/open_debt_impact.py`
...
classe SeverityLevel herda de Enum:
...
classe AreaImpact:
    ...
    declare unit_name: str  <- ""  // nome da unidade
    declare units_not_delivered_per_year: int  <- 0  // escolas/casas/km nao construidos

    // Descricao humanizada
    declare description: str  <- ""
    declare human_cost: str  <- ""  // o que significa na vida real

    funcao budget_gap_percentage(self) retorna float:
        // Retorna o gap orcamentario como porcentagem.
        se self.annual_budget_needed_brl <= 0 entao:
            retorne 0
        retorne (self.annual_budget_gap_brl / self.annual_budget_needed_brl) * 100

    funcao units_lost_per_billion(self) retorna float:
        // Quantas unidades se perdem por R$ 1 bilhao de juros.
        se self.unit_cost_brl <= 0 entao:
            retorne 0
        retorne 1e9 / self.unit_cost_brl
...
declare AREA_IMPACTS: List[AreaImpact]  <- [
    AreaImpact(
...
classe YearImpact:
    ...
    declare area_details: Dict[str, Dict[str, Any]]  <- field(default_factory=dict)
    declare cumulative_gap_brl: float  <- 0.0
    declare cumulative_people_affected: int  <- 0


classe ImpactSimulator:
    // 
    ...
    funcao simulate(self) retorna List[YearImpact]:
        ...
        para cada i em range(self.years + 1):
            ...
            cumulative_gap <- cumulative_gap + total_gap
            cumulative_people <- cumulative_people + total_people
    ...
    funcao total_gap_all_years(self) retorna float:
        retorne self.simulations[-1].cumulative_gap_brl if self.simulations else 0

    funcao total_interest_all_years(self) retorna float:
...
funcao render_area_chart(simulations: List[YearImpact]) retorna str:
    ...
    areas_sorted <- sorted(s.area_details.items(), key=funcao anonima(x): x[1]["gap_brl"], reverse=VERDADEIRO)
    max_gap <- max(v["gap_brl"] for _, v in areas_sorted) if areas_sorted else 1

    para cada (area_key, details) em areas_sorted:
...
funcao render_cumulative_chart(simulations: List[YearImpact]) retorna str:
...
funcao render_human_cost() retorna str:
...
funcao render_equivalence_table() retorna str:
    ...
    para cada (label, qty) em equivalences:
        se qty >= 1e9 entao:
...
funcao render_comparison_other_countries() retorna str:
...
funcao render_narrative(simulations: List[YearImpact]) retorna str:
