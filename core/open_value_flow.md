# OpenValueFlow -- Eliminar Empresa e Banco da Cadeia de Valor

**Linguagem:** Portugol++ (PPM)

**Arquivo original:** `core/open_value_flow.py`
...
O PROBLEMA:
  ...
  DOIS parasitas na cadeia: empresa E banco.
  Resolver um sem o outro nao basta.
...
classe ValueSink herda de Enum:
...
classe ValueChainAnalysis:
    ...
    // Valor produzido
    seja total_value: flutuante = 100.0 // valor total produzido

    // Para onde vai (em R$)
    seja worker_receives: flutuante = 0.0
    seja company_profit: flutuante = 0.0 // lucro do dono
    seja company_operational: flutuante = 0.0 // custo real de operar
    seja bank_interest: flutuante = 0.0 // juros
    seja bank_transaction: flutuante = 0.0 // taxas de transacao
    seja bank_spread: flutuante = 0.0 // spread cambial
    seja tax: flutuante = 0.0 // impostos
    seja intermediary: flutuante = 0.0 // intermediarios

    // Pool coletivo (Republica)
    seja collective_pool: flutuante = 0.0 // 5% que volta

    // decorador: @property
    funcao worker_pct(self) -> flutuante:
        retorne self.worker_receives / self.total_value * 100

    // decorador: @property
    funcao extracted_pct(self) -> flutuante:
        // Quanto foi EXTRAIDO do trabalhador.
        retorne (self.company_profit + self.bank_interest
...
classe ValueFlowEngine:
    ...
    funcao analyze_capitalism(self, total_value: flutuante = 100.0,
                           seja company_surplus_pct: flutuante = 0.60,
                           seja bank_interest_pct: flutuante = 0.15,
                           seja bank_transaction_pct: flutuante = 0.03,
                           seja bank_spread_pct: flutuante = 0.02,
                           seja intermediary_pct: flutuante = 0.00
                           ) -> {texto: qualquer}:
        // Analisa cadeia de valor no CAPITALISMO atual.

        Mostra exatamente quanto e EXTRAIDO em cada ponto.
        // 
        ...
        retorne {
            ...
            "message": (
                ...
                "Empresa (dono) e Banco sugar {analysis.extracted_pct:.0f}%."
            ),
        }

    funcao analyze_republic_executavel(self, total_value: flutuante = 100.0,
                                    seja operational_pct: flutuante = 0.30,
                                    ) -> {texto: qualquer}:
        ...
        - Intermediario: nao existe (OpenMarketplace direto).
        ...
        worker = total_value - operational // operacional e REAL (energia, insumos)
        ...
        retorne {
            ...
            "total_produzido": "R$ {total_value:.2f}",
            "SEM_PARASITAS": {
                "lucro_dono": "R$ 0,00 (dono nao existe)",
    ...
    funcao analyze_republic_ideal(self, total_value: flutuante = 100.0
        ...
        retorne {
            ...
            "pool": "Nao existe (5% e durante transicao)",
    ...
    funcao bank_extraction_detail(self) -> {texto: qualquer}:
    ...
    funcao company_extraction_detail(self) -> {texto: qualquer}:
        // Detalha como a empresa extrai valor.
        retorne {
            "titulo": "COMO A EMPRESA (DONO) SUGA VALOR",
            "como": (
                "Dono nao pede tijolo. Nao escreve codigo. Nao opera maquina. "
            ...
            "solution": (
                ...
                "Operacional e CUSTO REAL (energia, insumos). "
                "Resto vai pra trabalhador. 100%."
            ),
        }

    funcao full_comparison(self, total_value: flutuante = 100.0) -> {texto: qualquer}:
    ...
    funcao stats(self) -> {texto: qualquer}:
        retorne {
            "analises_feitas": tamanho(self.analyses),
  ...
  REPUBLICA EXECUTAVEL (R$ 100 produzidos):
    ...
    Trabalhador -----> R$ 100 valor
                  ...
                  -R$ 30 -R$ 5 (volta)
                   ...
                   (Operacional e CUSTO REAL, nao extracao)
...
// )
    ...
    imprima("\n{'='*80}")
    imprima("  OpenValueFlow: {s['analises_feitas']} analises.")
    imprima("  {s['principio']}")
    imprima("{'='*80}")
