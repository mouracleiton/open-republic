...
**Arquivo original:** `core/constituent_assembly.py`

**Descricao:** =============================================================
"O fundador propoe. O povo dispoe.
 12 das 13 propostas do fundador foram REJEITADAS.
 Isso nao e falha. E DEMOCRACIA funcionando."
A Assembleia Constituinte e o ORGAO MAXO da Republica.
...
classe Citizen:
    ...
    sector: Sector
    age: inteiro
    background: texto // quem e na vida real

    // Tendencias de voto (0-1)
    seja pragmatism: flutuante = 0.5 // quao pragmatico vs idealista
    seja risk_tolerance: flutuante = 0.5 // tolerancia a risco
    seja equity_focus: flutuante = 0.5 // foco em igualdade vs meritocracia
    seja autonomy_focus: flutuante = 0.5 // foco em autonomia individual
    ...
    funcao vote(self, proposal: 'AssemblyProposal',
        ...
        se score > 0.55 entao:
            ...
            reasoning = "Alinhado com meus valores e setor."
        senao se score < 0.35 entao:
            choice = VoteChoice.NO
            reasoning = "Conflita com meus valores ou setor."
        senao:
            choice = VoteChoice.ABSTAIN
            reasoning = "Preciso de mais informacao ou sou ambivalente."
...
classe VoteChoice herda de Enum:
    YES = "sim"
    NO = "nao"
    ABSTAIN = "abstencao"
...
classe AssemblyProposal:
    ...
    seja stances: {texto: flutuante} = field(default_factory=dict)

    // Parametros da votacao
    seja quorum_pct: flutuante = 0.50 // 50% + 1 para aprovar
    seja constitutional: logico = falso // se verdadeiro, quorum = 60%

    // Resultado
    seja votes: [Vote] = field(default_factory=list)
    result: 'VoteResult'? = nulo

    funcao cast_votes(self, citizens: [Citizen], rng: random.Random):
        // Todos os cidadaos votam.
        self.votes = [c.vote(self, rng) para c em citizens]

    // decorador: @property
    funcao yes_count(self) -> inteiro:
        retorne soma(1 para v em self.votes if v.choice == VoteChoice.YES)

    // decorador: @property
    funcao no_count(self) -> inteiro:
        retorne soma(1 para v em self.votes if v.choice == VoteChoice.NO)

    // decorador: @property
    funcao abstain_count(self) -> inteiro:
        retorne soma(1 para v em self.votes if v.choice == VoteChoice.ABSTAIN)

    // decorador: @property
    funcao total_votes(self) -> inteiro:
        retorne tamanho(self.votes)

    // decorador: @property
    funcao decided_votes(self) -> inteiro:
        retorne self.yes_count + self.no_count

    // decorador: @property
    funcao approved(self) -> logico:
        se self.decided_votes == 0 entao:
            retorne falso
        threshold = self.constitutional ? 0.60 : 0.50
        retorne (self.yes_count / self.decided_votes) >= threshold

    // decorador: @property
    funcao yes_pct(self) -> flutuante:
        self.decided_votes ? retorne self.yes_count / self.decided_votes * 100 : 0

    // decorador: @property
    funcao margin(self) -> texto:
...
classe VoteResult:
    ...
    total: inteiro
    margin: texto
    seja by_sector: Dict[texto, {texto: inteiro}] = field(default_factory=dict)
    seja note: texto = ""
...
classe ConstituentAssembly:
    ...
    funcao __init__(self, seed: inteiro = 42):
    ...
    funcao _populate_citizens(self):
    ...
    funcao submit_proposal(self, proposal: AssemblyProposal):
        // Submete proposta para votacao.
        se proposal.constitutional entao:
    ...
    funcao run_election(self) -> {texto: qualquer}:
        ...
        para cada prop em self.proposals:
            prop.cast_votes(self.citizens, self.rng)

            // Analise por setor
            seja by_sector: Dict[texto, {texto: inteiro}] = {}
            para cada v em prop.votes:
                sname = v.sector.value
                se sname nao in by_sector entao:
                    by_sector[sname] = {"sim": 0, "nao": 0, "abstencao": 0}
        ...
        retorne constitution

    funcao print_results(self) -> texto:
        ...
        para cada (i, result) em enumere(self.results, 1):
            ...
            lines.append("  STATUS: {status} ({result.yes} sim, {result.no} nao, {result.abstain} abstencao)")
            ...
            para cada (sector_name, counts) em result.by_sector.items():
                ...
                total = s + n + a
                se total > 0 entao:
                    lines.append("    {sector_name:<20} sim={s} nao={n} abst={a}")

            se result.approved entao:
        ...
        senao:
            ...
            lines.append("  O fundador ouviu o povo e o povo concordou.")
        lines.append("")
...
funcao session_financing() -> ConstituentAssembly:
    // Sessao da Assembleia sobre a PERGUNTA CRITICA:

    'Como financiar a Republica e seus cidadaos durante a transicao?'
    ...
    assembly.submit_proposal(AssemblyProposal(
        ...
        description = (
            ...
            "A Republica e financiada pelo salario do fundador durante a transicao."
        ),
    ...
    assembly.submit_proposal(AssemblyProposal(
        ...
        description = (
            "Contrato part-time ou consultoria ($300-400k/ano). "
    ...
    assembly.submit_proposal(AssemblyProposal(
        ...
        description = (
            ...
            "Cliente paga em R$ mas estrutura interna e da Republica. "
...
se __name__ == "__main__" entao:
    ...
    imprima(assembly.print_results())

    imprima()
    imprima("=" * 100)
    imprima("DECISOES APROVADAS PELA ASSEMBLEIA:")
    imprima("=" * 100)
    se constitution entao:
        para cada (title, details) em constitution.items():
            imprima("  >> {title}")
            imprima("     Valor: {details['value']}")
            imprima("     Votos: {details['votes']} ({details['yes_pct']:.0f}% sim)")
            imprima()
    senao:
        imprima("  Nenhuma proposta aprovada. O povo rejeitou tudo.")
        imprima("  O fundador precisa ouvir e reformular.")
    imprima("=" * 100)
