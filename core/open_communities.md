...
**Arquivo original:** `core/open_communities.py`

**Descricao:** ============================================================
"O modelo NAO se impoe. Se ADAPTA.
...
PRINCIPIO (P2 + P4):
  ...
  - Se nao quiserem: respeito. OpenWololo depois.
AS 6 COMUNIDADES:
...
classe CommunityType herda de Enum:
...
classe CulturalPillar herda de Enum:
    ...
    ORALIDADE = "oralidade"  // tradicao oral (nao escrita)
...
classe CommunityProfile:
    ...
    seja cultural_pillars: [CulturalPillar] = field(default_factory=list)
    seja existing_practices: {texto: texto} = field(default_factory=dict)
    ...
    seja adapted_systems: {texto: texto} = field(default_factory=dict)
    // adapted_systems = {"sistema_republica": "como_adapta_para_esta_comunidade"}

    // O que NÃO se importa (respeito cultural)
    seja do_not_import: [texto] = field(default_factory=list)

    // Governanca adaptada
    seja governance_model: texto = ""
    seja decision_making: texto = ""  // como decidem (consenso, voto, conselho)

    // Lingua
    seja primary_language: texto = "pt-BR"
    seja secondary_languages: [texto] = field(default_factory=list)

    // Isolamento geografico
    seja isolation_level: texto = ""  // urbano, semi-isolado, isolado, muito isolado
    seja access_challenge: texto = ""  // barreira de acesso (rio, estrada, etc)
...
seja PROFILES: [CommunityProfile] = [
    ...
    CommunityProfile(
        ...
        description = (
            ...
            "que resistiram e fundaram comunidades livres. "
        ...
        existing_practices = {
            ...
            "capoeira": "OpenMartialArts (ja e defesa + cultura + corpo)",
    ...
    CommunityProfile(
        ...
        description = (
            ...
            "A Republica chega como FERRAMENTA, nao como ideia."
        ),
        cultural_pillars = [
            CulturalPillar.TERRA,
            CulturalPillar.MUTIRAO,
        ...
        needs = [
            ...
            "Agroindustria (beneficiar produto: nao vender materia prima)",
        ...
        adapted_systems = {
            ...
            "OpenIndustry": (
                ...
                "graos (moinho). "
                "Vender processado, nao materia prima."
            ),
            "OpenEducation": (
            ...
            "OpenLaborRelay": (
                "Mutirao registrado e recompensado. "
            ...
            "OpenRepair": (
                ...
                "OpenTrator e OpenHardware: FabLab fabrica peça. "
    ...
    CommunityProfile(
        ...
        description = (
            ...
            "Beira-rio. Vida no ciclo das aguas (cheia e seca). "
        ...
        adapted_systems = {
            ...
            "OpenEducation": (
                ...
                "Professor mora na comunidade (nao itinerante). "
    ...
    CommunityProfile(
        ...
        needs = [
            "Demarcação e proteção de terra (invasão constante)",
        ...
        adapted_systems = {
            "OpenConstituentAssembly": (
                ...
                "Decisao por CONSENSO (nao maioria 51%). "
            ...
            "OpenEducation": (
                ...
                "Currículo da etnia (nao do dominador). "
                "Matematica com padroes indigenas. "
                "Ciencias com conhecimento ancestral. "
                "OpenTerminal como ferramenta (nao como substituicao)."
            ),
            "OpenHistory": (
    ...
    CommunityProfile(
        ...
        name = "OpenFavela",
        description = (
            "Favelas e periferias urbanas. 17 milhoes de brasileiros. "
        ...
        existing_practices = {
            ...
            "balada_funk": "OpenMusic + OpenNightLife (funk e patrimônio)",
        ...
        adapted_systems = {
            ...
            "OpenRepair": (
                ...
                "Gato de energia -> OpenSolar (seguro e legal). "
    ...
    CommunityProfile(
        ...
        description = (
            ...
            "A Republica chega com AGUA e CONEXAO."
        ),
        cultural_pillars = [
            CulturalPillar.ANCESTRALIDADE,
            CulturalPillar.MUTIRAO,
        ...
        needs = [
            ...
            "Combate ao êxodo (jovem sai, nao volta)",
            "Armazenagem de agua (cisterna de placa)",
            "Refrigeração (conservar comida no calor extremo)",
        ],
        adapted_systems = {
            "OpenWater": (
                "EIXO CENTRAL. Sem agua, nao ha Republica no sertao. "
                ...
                "Irrigacao gota-a-gota (minimo desperdicio)."
            ),
            "OpenSolar": (
            ...
            "OpenAgrarian": (
                "Agricultura ADAPTADA AO SEMI-ARIDO (nao irrigada). "
            ...
            "OpenHealth": (
                ...
                "Parteira tradicional integrada. "
                "Rezador/benzedeira integrado (nao combatido). "
...
classe CommunityEngine:
    ...
    3. A COMUNIDADE DECIDE. O que nao quer, nao entra.
    4. nao IMPOR. Demonstrar (OpenWololo).
    5. RESPEITAR LIDERANCA TRADICIONAL. Sempre.

    COMO CHEGAR:
    1. Contato via liderança tradicional (NUNCA bypass)
    2. Apresentar sistemas RELEVANTES (nao todos)
    ...
    7. Sair quando nao precisarem mais (meta: autonomia total)

    O QUE O SISTEMA FAZ:
    - Mapeia necessidades especificas de cada comunidade
    - Cruza com sistemas da Republica disponíveis
    - Adapta (nao copia) cada sistema a cultura local
    ...
    funcao get_profile(self, community_type: CommunityType) -> {texto: qualquer}:
        // Retorna perfil completo de uma comunidade.
        p = self.profiles.get(community_type.value[0])
        se nao p entao:
            retorne {"error": "Comunidade nao encontrada"}
        ...
        retorne {
            ...
            "message": (
                ...
                "ADAPTACAO, nao imposicao."
            ),
        }

    funcao adaptation_matrix(self) -> [Dict]:
        ...
        matrix = []
        para cada system em ordene(systems):
            row = {"sistema": system}
            para cada p em self.profiles.values():
                se system in p.adapted_systems entao:
    ...
    funcao what_already_exists(self) -> {texto: Dict}:
    ...
    funcao respect_rules(self) retorna Dict[texto, [texto]]:
        ...
        retorne {
            ...
            para p em self.profiles.values()
        }

    funcao compare_communities(self) -> [Dict]:
        // Compara necessidades entre comunidades.
        retorne [
            {
                "comunidade": p.name,
                "necessidades": tamanho(p.needs),
                "sistemas_adaptados": tamanho(p.adapted_systems),
                "respeitos": tamanho(p.do_not_import),
                "pilares_culturais": tamanho(p.cultural_pillars),
                "praticas_existentes": tamanho(p.existing_practices),
                "isolamento": p.isolation_level,
            }
            para p em self.profiles.values()
        ]
    ...
    funcao stats(self) -> {texto: qualquer}:
        retorne {
            "comunidades": tamanho(self.profiles),
            "total_necessidades": soma(tamanho(p.needs) para p em self.profiles.values()),
            "total_sistemas_adaptados": soma(tamanho(p.adapted_systems) para p em self.profiles.values()),
            "total_respeitos_culturais": soma(tamanho(p.do_not_import) para p em self.profiles.values()),
...
se __name__ == "__main__" entao:
    ...
    imprima("=" * 80)
    imprima("  OPENCOMMUNITIES -- 6 ADAPTACOES COMUNITARIAS")
    imprima("  O modelo se ADAPTA. Nao se impoe.")
    imprima("=" * 80)

    // === 1. COMPARACAO GERAL ===
    imprima("\n\n  === 1. COMPARACAO GERAL ===\n")
    imprima("  {'Comunidade':<25} {'Necessidades':>13} {'Sistemas':>10} {'Respeitos':>10} {'Praticas':>10}")
    imprima("  {'-'*70}")
    para cada c em engine.compare_communities():
        imprima("  {c['comunidade']:<25} {c['necessidades']:>13} "
              "{c['sistemas_adaptados']:>10} {c['respeitos']:>10} "
              "{c['praticas_existentes']:>10}")

    // === 2. PERFIL COMPLETO DE CADA COMUNIDADE ===
    para cada ct em CommunityType:
        imprima("\n\n  {'='*70}")
        p = engine.get_profile(ct)
        imprima("  {'='*70}")
        imprima("  {p['comunidade'].upper()}")
        imprima("  {p['descricao'][:80]}...")
        imprima("  {'='*70}\n")

        imprima("  PILARES CULTURAIS:")
        para cada pillar em p["pilares_culturais"]:
            imprima("    + {pillar}")

        imprima("\n  PRATICAS QUE JA EXISTEM (equivalente Republica):")
        para cada (practice, equivalent) em p["praticas_existentes"].items():
            imprima("    {practice:<25} -> {equivalent}")

        imprima("\n  NECESSIDADES ({len(p['necessidades'])}):")
        para cada need em p["necessidades"]:
            imprima("    - {need}")

        imprima("\n  SISTEMAS ADAPTADOS ({len(p['sistemas_adaptados'])}):")
        para cada (system, adaptation) em p["sistemas_adaptados"].items():
            imprima("\n    {system}:")
            imprima("      {adaptation[:100]}...")

        imprima("\n  O QUE NUNCA IMPORTAR ({len(p['o_que_nao_importar'])}):")
        para cada rule em p["o_que_nao_importar"]:
            imprima("    X {rule}")

        imprima("\n  GOVERNANCA: {p['governanca']}")
        imprima("  DECISAO: {p['tomada_decisao']}")
        imprima("  LINGUA: {p['lingua_principal']} {p['linguas_secundarias']}")
        imprima("  ISOLAMENTO: {p['isolamento']}")
        imprima("  ACESSO: {p['acesso']}")

    // === 3. STATS ===
    imprima("\n\n  {'='*70}")
    imprima("  === ESTATISTICAS ===\n")
    s = engine.stats()
    para cada (k, v) em s.items():
        imprima("  {k:<30} {v}")

    imprima("\n{'='*80}")
    imprima("  OpenCommunities: {s['comunidades']} comunidades adaptadas.")
    imprima("  {s['total_necessidades']} necessidades mapeadas.")
    imprima("  {s['total_sistemas_adaptados']} sistemas adaptados.")
    imprima("  {s['total_respeitos_culturais']} respeitos culturais.")
    imprima("  {s['principio']}")
    imprima("{'='*80}")
