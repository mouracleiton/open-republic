...
class PropagationEngine {
    ...
    stats(self) {
        return {;
            "pilotos_planejados": .length(self.pilots),;
            "pecas_conteudo": .length(self.content),;
            "estrategias_comunitarias": .length(self.communities),;
            "aliancas_potenciais": .length(self.alliances),;
            "produtos_publicos": .length(self.products),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENPROPAGATION -- ESTRATEGIA DE DIVULGACAO MASSIVA");
    console.log("  Nao grite. Demonstre. 1 prova > 1000 teorias.");
    console.log("=" * 80);
    // === 1. AS 5 FRENTES ===
    console.log("\n\n  === AS 5 FRENTES DE PROPAGACAO ===\n");
    console.log("  1. DEMONSTRACAO REAL (piloto funcionando) -- A QUE MATRIA");
    console.log("  2. CONTEUDO VIRAL (X/Twitter, video, infografico)");
    console.log("  3. COMUNIDADES REAIS (CONAQ, MST, G10, ASA)");
    console.log("  4. ALIANCAS (artistas, ONGs, jornalistas)");
    console.log("  5. PRODUTO PUBLICO (simulador, app, landing page)");
    // === 2. PILOTOS ===
    console.log("\n\n  === PILOTOS REAIS ({len(engine.pilots)}) ===\n");
    for (const p of engine.pilots) {
        console.log("\n  {p.name} ({p.location})");
        console.log("    Demonstra: {p.what_demonstrates}");
        console.log("    Impacto: {p.expected_impact}");
        console.log("    Custo: {p.cost}");
        console.log("    Prazo: {p.timeline}");
        console.log("    Viral: {p.viral_potential}");
        console.log("    Doc: {p.documentation}");
    // === 3. CONTEUDO ===
    console.log("\n\n  === CONTEUDO VIRAL ({len(engine.content)}) ===\n");
    for (const c of engine.content) {
        console.log("\n  [{c.format}] {c.topic}");
        console.log("    Hook: '{c.hook}'");
        console.log("    Plataforma: {c.platform}");
        console.log("    CTA: {c.call_to_action}");
        console.log("    Esperado: {c.expected_reach}");
    // === 4. COMUNIDADES ===
    console.log("\n\n  === EXPANSAO COMUNITARIA ({len(engine.communities)}) ===\n");
    for (const c of engine.communities) {
        console.log("  {c.strategy}");
        console.log("    Metodo: {c.method}");
        console.log("    Fator: {c.expansion_factor}");
    // === 5. ALIANCAS ===
    console.log("\n\n  === ALIANCAS ({len(engine.alliances)}) ===\n");
    for (const a of engine.alliances) {
        console.log("  [{a.priority}] {a.ally_type}: {a.who}");
        console.log("    Eles ganham: {a.what_they_gain[:60]}");
        console.log("    Republica ganha: {a.what_republica_gains[:60]}");
    // === 6. PRODUTOS PUBLICOS ===
    console.log("\n\n  === PRODUTOS PUBLICOS USAVEIS ({len(engine.products)}) ===\n");
    for (const p of engine.products) {
        console.log("  [{p.status}] {p.name}");
        console.log("    O que: {p.what}");
        console.log("    Viraliza porque: {p.why_viral}");
    // === 7. PLANO DE EXECUCAO ===
    console.log("\n\n  === PLANO DE EXECUCAO ===\n");
    ...
    for (const fase of plan) {
        ...
        for (const acao of fase["acoes"]) {
            console.log("    -> {acao}");
        console.log("    Custo: {fase['custo']}");
        console.log("    Resultado: {fase['resultado']}");
    // === 8. CALENDARIO ===
    console.log("\n\n  === CALENDARIO DE CONTEUDO (X/Twitter) ===\n");
    cal = engine.content_calendar();
    for (const day of cal) {
        console.log("  {day['dia']:<12} {day['tipo']}: {day['exemplo']}");
    // === 9. METRICAS ===
    console.log("\n\n  === METRICAS ===\n");
    for (const m of engine.key_metrics()) {
        console.log("  {m['metrica']}");
        console.log("    3 meses: {m['meta_3meses']}");
        console.log("    6 meses: {m['meta_6meses']}");
        console.log("    12 meses: {m['meta_12meses']}");
    // === 10. O QUE NAO FAZER ===
    console.log("\n\n  === O QUE NAO FAZER (ERROS QUE MATAM) ===\n");
    for (const dont of engine.what_NOT_to_do()) {
        console.log("  X {dont}");
    // === STATS ===
    console.log("\n\n  === ESTATISTICAS ===\n");
    s = engine.stats();
    para cada (k, v) em s.items(): {
        console.log("  {k:<30} {v}");
    console.log("\n{'='*80}");
    console.log("  OpenPropagation: {s['pilotos_planejados']} pilotos, ";
