# OpenProduct -- Produtos Otimizados para Meio Ambiente, Pessoas e Animais

**Linguagem:** Portugol++ (PPM)

**Arquivo original:** `core/open_product.py`

**Descricao:** =========================================================================
"Todo produto da Republica e projetado para:
 1. NAO prejudicar PESSOAS (toxico, perigoso, explorador)
 2. NAO prejudicar ANIMAIS (teste, crueldade, extincao)
 3. NAO prejudicar MEIO AMBIENTE (poluicao, desperdicio, extracao)
 4. SER REPARAVEL (OpenRepair, nao descartavel)
 5. SER MODULAR (LEGO, troca peca, nao produto inteiro)
 ...
 Produto que e descartavel? SUBSTITUIDO."
...
classe ImpactArea herda de Enum:
...
classe ProductStatus herda de Enum:
...
classe MaterialType herda de Enum:
    RECYCLED = "reciclado"
...
classe RepublicProduct:
    ...
    seja human_health_harm: HarmLevel = HarmLevel.NONE
    seja animal_harm: HarmLevel = HarmLevel.NONE
    seja environment_harm: HarmLevel = HarmLevel.NONE
    seja social_harm: HarmLevel = HarmLevel.NONE

    // Materiais
    seja materials: [MaterialType] = field(default_factory=list)
    seja materials_detail: texto = ""

    // Ciclo de vida
    seja biodegradable: logico = falso
    seja recyclable: logico = verdadeiro
    seja repairable: logico = verdadeiro // OpenRepair
    seja modular: logico = verdadeiro // LEGO
    seja lifespan_years: flutuante = 10.0

    // Energia
    seja energy_efficient: logico = verdadeiro
    seja energy_source: texto = "solar/eolica"

    // Producao
    seja fablab_producible: logico = verdadeiro // FabLab fabrica?
    seja tested_on_animals: logico = falso // PROIBIDO
    seja fair_labor: logico = verdadeiro // trabalho base 1.0

    // Status
    seja status: ProductStatus = ProductStatus.APPROVED
    seja replacement_for: texto = ""  // substitui qual produto nocivo?
    seja alternative_if_prohibited: texto = ""

    // decorador: @property
    funcao overall_score(self) -> inteiro:
        ...
        bonus = 0
        se self.biodegradable entao:
            bonus = bonus + 5
        se self.recyclable entao:
            bonus = bonus + 5
        se self.repairable entao:
            bonus = bonus + 10
        se self.modular entao:
            bonus = bonus + 10
        se self.energy_efficient entao:
            bonus = bonus + 5
        se nao self.tested_on_animals entao:
            bonus = bonus + 10
        se self.fablab_producible entao:
            bonus = bonus + 5
        retorne maximo(0, minimo(100, 100 - penalty + bonus))
...
seja PRODUCTS: [RepublicProduct] = [
    ...
    RepublicProduct(
        ...
        biodegradable = verdadeiro, recyclable=verdadeiro, repairable=verdadeiro,
        lifespan_years = 5.0,
    ...
    RepublicProduct(
        ...
        biodegradable = falso, recyclable=verdadeiro, repairable=verdadeiro, modular=verdadeiro,
        lifespan_years = 8.0,
    ...
    RepublicProduct(
        ...
        biodegradable = verdadeiro, repairable=verdadeiro,
        lifespan_years = 15.0,
    ...
    RepublicProduct(
        ...
        recyclable = verdadeiro, repairable=verdadeiro, modular=verdadeiro,
        lifespan_years = 20.0,
        energy_efficient = verdadeiro,
    ...
    RepublicProduct(
        ...
        biodegradable = verdadeiro, recyclable=verdadeiro, repairable=verdadeiro, modular=verdadeiro,
        lifespan_years = 30.0,
    ...
    RepublicProduct(
        ...
        biodegradable = verdadeiro, repairable=verdadeiro, modular=verdadeiro,
        lifespan_years = 25.0,
    ),
    ...
    RepublicProduct(
        ...
        biodegradable = falso, recyclable=verdadeiro,
        lifespan_years = 20.0,
    ...
    RepublicProduct(
        ...
        biodegradable = verdadeiro, tested_on_animals=falso,
        lifespan_years = 1.0,
    ...
    RepublicProduct(
        ...
        biodegradable = verdadeiro, recyclable=verdadeiro,
        lifespan_years = 10.0,
        replacement_for = "Sacola plastica (1 TRILHAO/ano no planeta)",
    ),
    RepublicProduct(
        "PRD-BOTTLE", "OpenBottle (garrafa)", "embalagem",
        "Garrafa de aco inoxidavel ou vidro. Reutilizavel eternamente.",
        materials = [MaterialType.MINERAL],
        materials_detail = "Aco inoxidavel reciclavel ou vidro. Sem plastico.",
        recyclable = verdadeiro, repairable=verdadeiro,
        lifespan_years = 30.0,
    ...
    RepublicProduct(
        ...
        "PROIBIDO. Teste em animais e CRUELDADE desnecessaria.",
...
classe ProductEngine:
    // Motor que aprova, reprova e reformula produtos.

    CRITERIOS DE APROVACAO (TODOS devem passar):
    1. nao prejudica saude humana (sem toxico, sem cancerigeno)
    2. nao testa em animais (PROIBIDO)
    3. nao prejudica meio ambiente (biodegradavel/reciclavel)
    4. REPARAVEL (OpenRepair, nao descartavel)
    5. MODULAR (LEGO, troca peca)
    6. FABLAB PRODUCIVEL (producao nacional)
    7. TRABALHO JUSTO (base 1.0, sem exploracao)

    SE FALHA EM UM CRITERIO: reformular ou proibir.
    ...
    funcao evaluate(self, product_id: texto) -> {texto: qualquer}:
        // Avalia produto contra todos os criterios.
        p = self.products.get(product_id)
        se nao p entao:
            retorne {"error": "Produto nao encontrado"}

        checks = {
            "saude_humana": p.human_health_harm.value[1] <= 1,
            "sem_teste_animal": nao  p.tested_on_animals,
            ...
            "sem_toxico": MaterialType.TOXIC nao  in p.materials,
        }

        failed = [k para k, v in checks.items() if nao v]
        any_critical = any([
        ...
        se any_critical entao:
            p.status = ProductStatus.PROHIBITED
        senao se failed entao:
    ...
    funcao list_approved(self) -> [Dict]:
        retorne [
    ...
    funcao list_prohibited(self) -> [Dict]:
        retorne [
            {"id": p.product_id, "name": p.name,
    ...
    funcao waste_report(self) -> {texto: qualquer}:
        ...
        retorne {
            ...
            "message": (
                ...
                "Descartavel nao existe. Lixo nao existe."
            ),
        }
    ...
    funcao stats(self) -> {texto: qualquer}:
        ...
        retorne {
            "total_produtos": tamanho(self.products),
            "aprovados": approved,
...
se __name__ == "__main__" entao:
    ...
    imprima("=" * 80)
    imprima("  OPENPRODUCT -- PRODUTOS QUE NAO PREJUDICAM")
    imprima("  Pessoas. Animais. Meio Ambiente. Tudo protegido.")
    imprima("=" * 80)

    // === 1. AVALIAR TODOS OS PRODUTOS ===
    imprima("\n\n  === 1. AVALIACAO DE PRODUTOS ===\n")
    para cada pid em engine.products:
        r = engine.evaluate(pid)

    // === 2. PRODUTOS APROVADOS ===
    imprima("\n\n  === 2. PRODUTOS APROVADOS ===\n")
    approved = engine.list_approved()
    imprima("  {'Produto':<25} {'Score':>5} {'Vida':>5} {'Bio':>4} {'Rep':>4} {'Mod':>4} {'Substitui'}")
    imprima("  {'-'*85}")
    para cada p em approved:
        imprima("  {p['name'][:24]:<25} {p['score']:>5} {p['lifespan']:>4.0f}a "
    ...
    para cada p em prohibited:
        imprima("  {p['name']:<30} DANO: {p['why']}")
        imprima("    Alternativa: {p['alternative']}")

    // === 4. RELATORIO DE RESIDUOS ===
    imprima("\n\n  === 4. RESIDUOS EVITADOS ===\n")
    waste = engine.waste_report()
    para cada (key, data) em waste.items():
        se key != "message"  e  isinstance(data, dict) entao:
            imprima("\n  {key.upper()}:")
            imprima("    Atual: {data['atual']}")
            imprima("    Republica: {data['republica']}")
    imprima("\n  {waste['message']}")
    ...
    imprima("\n{'='*80}")
    imprima("  OpenProduct: {s['aprovados']} aprovados, {s['proibidos']} proibidos. "
          "Score medio: {s['score_medio_aprovados']}.")
    imprima("  Nao prejudica pessoas. Nao prejudica animais. Nao prejudica planeta.")
    imprima("{'='*80}")
