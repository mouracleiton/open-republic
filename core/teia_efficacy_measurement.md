...
**Arquivo original:** `core/teia_efficacy_measurement.py`
...
classe DataAccuracyTest:
    ...
    seja variance_pct: flutuante = 0.0

    funcao __post_init__(self):
        se self.official_value != 0 entao:
            self.variance_pct = abs(self.teia_value - self.official_value) / self.official_value * 100

    // decorador: @property
    funcao passed(self) -> logico:
        // Passa se variancia < 2% (tolerancia para arredondamento/atualizacao).
        retorne self.variance_pct < 2.0

    // decorador: @property
    funcao status(self) -> texto:
        se self.variance_pct < 1.0 entao:
            retorne "EXATO"
        senao se self.variance_pct < 2.0 entao:
            retorne "ACEITAVEL"
        senao se self.variance_pct < 5.0 entao:
            retorne "DIVERGENCIA"
        senao:
            retorne "ERRO CRITICO"


// Simulacao de testes de acuracia (valores ilustrativos)
// Na pratica: cruzar com IBGE/SNIS/CAGED reais
seja ACCURACY_TESTS: [DataAccuracyTest] = [
...
classe ModelBacktest:
    ...
    seja error_pct: flutuante = 0.0

    funcao __post_init__(self):
        se self.actual_outcome != 0 entao:
            self.error_pct = abs(self.model_prediction - self.actual_outcome) / self.actual_outcome * 100

    // decorador: @property
    funcao passed(self) -> logico:
        retorne self.error_pct < 15.0 // 15% de margem para modelos socioeconomicos

    // decorador: @property
    funcao grade(self) -> texto:
        se self.error_pct < 5 entao:
            retorne "A (excelente)"
        senao se self.error_pct < 10 entao:
            retorne "B (bom)"
        senao se self.error_pct < 15 entao:
            retorne "C (aceitavel)"
        senao se self.error_pct < 25 entao:
            retorne "D (precisa revisao)"
        senao:
            retorne "F (reprovado)"


seja BACKTESTS: [ModelBacktest] = [
    ...
    ModelBacktest(
        ...
        actual_outcome = 25_200_000_000, // ainda nao implementado, valor projetado
...
classe ArtifactQualityScore:
    ...
    4. Painel de 5 avaliadores pontua sem saber qual e qual
    5. Criterios: acuracia, clareza, profundidade, acionabilidade, fontes
    // 

    artifact_id: texto
    artifact_type: texto   // "teia_generated" ou "human_made"
    evaluator: texto
    evaluator_background: texto

    // Scores 0-10
    seja accuracy: flutuante = 0 // dados estao corretos?
    seja clarity: flutuante = 0 // e legivel?
    seja depth: flutuante = 0 // tem profundidade analitica?
    seja actionability: flutuante = 0 // da para tomar decisao com isso?
    seja sourcing: flutuante = 0 // fontes sao confiaveis e verificaveis?
    seja completeness: flutuante = 0 // cobre o topico inteiro?

    // decorador: @property
    funcao total_score(self) -> flutuante:
        retorne (self.accuracy + self.clarity + self.depth +
...
funcao generate_blind_test_simulation() -> [ArtifactQualityScore]:
...
classe UsabilityTest:
    ...
    funcao usability_score(self) -> flutuante:
        ...
        retorne (time_score + error_score + quality + frustration_score + help_score) / 5


funcao simulate_usability_tests() -> [UsabilityTest]:
...
classe MarketAcceptanceTest:
    ...
    // decorador: @property
    funcao nps_category(self) -> texto:
        se self.nps_score >= 9 entao:
            retorne "PROMOTOR"
        senao se self.nps_score >= 7 entao:
            retorne "NEUTRO"
        senao:
            retorne "DETRATOR"


funcao simulate_market_tests() -> [MarketAcceptanceTest]:
    ...
    para cid, ctype, org in clients:
        results.append(MarketAcceptanceTest(
            ...
            client_type = ctype,
            client_org = org,
            artifact_shown = "Dossie TEIA: Fome e Saneamento",
...
classe LaunchGate herda de Enum:
    ...
    funcao __init__(self, label: texto, threshold: flutuante, metric: texto):
        self.label = label
...
funcao evaluate_gates() -> [GateResult]:
    // Avalia todos os gates.

    results = []

    // GATE A: Accuracia de dados
    accuracy_tests = [t para t em ACCURACY_TESTS if "TESTE DE ERRO" nao  in t.metric_name]
    accuracy_passed = soma(1 para t em accuracy_tests if t.passed)
    accuracy_rate = accuracy_passed / tamanho(accuracy_tests) * 100
    ...
    model_rate = model_passed / tamanho(BACKTESTS) * 100
    results.append(GateResult(
    ...
    teia_avg = teia_scores ? soma(teia_scores) / tamanho(teia_scores) : 0
    human_avg = human_scores ? soma(human_scores) / tamanho(human_scores) : 0
    results.append(GateResult(
    ...
    usability_avg = usability_scores ? soma(usability_scores) / tamanho(usability_scores) : 0
    results.append(GateResult(
    ...
    nps = (promoters - detractors) / tamanho(market) * 100
    results.append(GateResult(
...
funcao print_efficacy_report() -> texto:
    lines = []
    ...
    // === METODOLOGIA ===
    lines.append("5 FASES OBRIGATORIAS (cada uma e um GATE):")
    lines.append("-" * 110)
    lines.append("")
    ...
    lines.append("")
    lines.append("  Se qualquer GATE nao passa -> CORRIGIR antes de avancar.")
    ...
    valid = [t para t em ACCURACY_TESTS if "TESTE DE ERRO" nao  in t.metric_name]
    ...
    teia_avg = soma(b.total_score para b em teia) / tamanho(teia)
    human_avg = soma(b.total_score para b em human) / tamanho(human)
    ...
    u_avg = soma(u.usability_score para u em usability) / tamanho(usability)
    ...
    feedbacks_seen = set()
    para cada m em market:
        se m.feedback nao in feedbacks_seen entao:
    ...
    all_passed = verdadeiro
    para cada g em gates:
        status = g.passed ? "PASS" : "FAIL"
        se nao g.passed entao:
            all_passed = falso
        lines.append(
            "  {g.gate.label:<45} "
            "{g.threshold:>8.0f} "
    ...
    lines.append("-" * 110)
    lines.append("O QUE FAZER AGORA (nao e simulacao):")
    lines.append("-" * 110)
    lines.append("")
    ...
    lines.append("     NPS. Se NPS < 50, nao lancar. Ouvir feedback e melhorar.")
    lines.append("")
