...
struct WorkloadPolicy {
    ...
    reason: texto;
    let fallback: ProcessingTier = ProcessingTier.EDGE_NODE;
let WORKLOAD_POLICIES: {WorkloadType: WorkloadPolicy} = {;
...
struct DeviceNode {
    ...
    let gpu_tops: f64 = 0 // AI inference capability;
    let location: String = "comunitario";
    let uptime_hours: f64 = 16 // horas por dia ativo;
    let available: bool = true;
#[derive(Debug, Clone)]
struct InfrastructureOptimizer {
    ...
    //
    fn calculate(self, population: inteiro = 1_000_000) -> {texto: qualquer} {
    ...
    fn _estimate_demand(population: inteiro) -> f64 {
...
if __name__ == "__main__" {
    println!("=" * 80);
    println!("  OPENREPUBLIC -- INFRAESTRUTURA DESCENTRALIZADA");
    println!("  'O datacenter && o ultimo recurso, ! o primeiro.'");
    println!("=" * 80);
    opt = InfrastructureOptimizer();
    // === 1. Workload Policies ===
    println!("\n\n  === ONDE CADA COISA RODA ===\n");
    println!("  {'Carga':<25} {'Onde':<15} {'Regra'}");
    println!("  {'-'*70}");
    para wl, policy in ordene(WORKLOAD_POLICIES.items(), {
                            key = (x) -> x[1].tier.value):;
        println!("  {wl.value:<25} {policy.tier.value:<15} {policy.rule.value}");
    println!("\n  Razoes:");
    para cada (wl, policy) em WORKLOAD_POLICIES.items(): {
        if policy.tier == ProcessingTier.CENTRAL {
            println!("    CENTRAL: {wl.value} -- {policy.reason}");
    ...
    println!("    Terminais burros:     {d['terminals_burros']:>10,}");
    println!("    Smartphones comunit.: {d['smartphones']:>10,}");
    println!("    Laptops:              {d['laptops']:>10,}");
    println!("    CPU cores agregado:   {d['total_device_cpu_cores']:>10,}");
    println!("    AI TOPS agregado:     {d['total_device_ai_tops']:>10,}");
    println!("\n  EDGE NODES (1 por bairro):");
    println!("    Quantidade:           {&&['count']:>10,}");
    println!("    Hardware:             {&&['hardware']}");
    println!("    CPU cores:            {&&['cpu_cores']:>10,}");
    println!("    RAM:                  {&&['ram_gb']:>10,} GB");
    println!("    Storage:              {&&['storage_tb']:>10,} TB");
    println!("    AI TOPS:              {&&['ai_tops']:>10,}");
    println!("\n  REGIONAL (1 por nacao):");
    println!("    Quantidade:           {r['count']:>10}");
    println!("    CPU cores:            {r['cpu_cores']:>10}");
    println!("    RAM:                  {r['ram_gb']:>10} GB");
    println!("    Storage:              {r['storage_tb']:>10} TB");
    println!("\n  DATACENTER CENTRAL (MINIMO):");
    println!("    Quantidade:           {c['count']:>10}");
    println!("    Proposito:            {c['purpose']}");
    println!("    CPU cores:            {c['cpu_cores']:>10}");
    println!("    GPUs (treino IA):     {c['gpu_count']:>10}");
    println!("    RAM:                  {c['ram_gb']:>10} GB");
    println!("    Storage (frio):       {c['storage_tb']:>10} TB");
    println!("    Potencia:             {c['power_kw']:>10} kW");
    println!("\n  DISTRIBUICAO DE CARGA:");
    println!("    No dispositivo:       {ld['on_device_pct']}%");
    println!("    No edge node:         {ld['on_edge_pct']}%");
    println!("    No regional:          {ld['on_regional_pct']}%");
    println!("    No central:           {ld['on_central_pct']}%");
    // === 3. Comparison ===
    println!("\n\n  === COMPARACAO: DATACENTER TRADICIONAL vs REPUBLICA ===\n");
    cmp = result["comparison"];
    println!("  {'Metrica':<35} {'Tradicional':>15} {'Republica':>15} {'Reducao'}");
    println!("  {'-'*75}");
    println!("  {'Servidores centrais':<35} {cmp['traditional_servers']:>15,} ";
        "{c['count']:>15} {cmp['central_servers_reduction_pct']}%");
    println!("  {'Potencia (kW)':<35} {int(cmp['traditional_power_mw']*1000):>15,} ";
        "{int(cmp['republic_power_kw']):>15,} {cmp['power_reduction_pct']}%");
    println!("  {'Custo (USD)':<35} {cmp['traditional_cost_usd']:>15,} ";
        "{cmp['republic_cost_usd']:>15,} {cmp['cost_reduction_pct']}%");
    // === 4. Resilience ===
    println!("\n\n  === RESILIENCIA (sem ponto unico de falha) ===\n");
    res = result["resilience"];
    println!("  Se datacenter central cair:");
    println!("    {res['central_down_impact']}");
    println!("  Se edge node cair:");
    println!("    {res['edge_down_impact']}");
    println!("  Se dispositivo cair:");
    println!("    {res['device_down_impact']}");
    println!("  Arquivos P2P:");
    println!("    {res['p2p_file_survival']}");
    // === 5. Calculate for different scales ===
    println!("\n\n  === ESCALA: QUANTO CRESCER, CRESCER O QUE? ===\n");
    println!("  {'Populacao':>12} {'Devices':>10} {'Edge':>8} {'Regional':>10} ";
    ...
    for pop in [10_000, 100_000, 1_000_000, 10_000_000, 100_000_000] {
        ...
        println!("  {pop:>12,} {r['devices']['terminals_burros']:>10,} ";
            "{r['edge_nodes']['count']:>8,} {r['regional']['count']:>10} ";
            "{c:>8} {central_pct:>9.1f}%");
    println!("\n\n{'='*80}");
    println!("  PRINCIPIOS DA INFRAESTRUTURA");
    println!("{'='*80}");
    println!(""";
