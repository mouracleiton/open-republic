...
WORKERS: [RealWorker] = [
    ...
    RealWorker(
        ...
        monthly_salary = 1518, hours_per_month=220, // salario min
    ...
    RealWorker(
        ...
        monthly_salary = 1412, hours_per_month=200, // salario min
...
class ValueSimulator:
    ...
    4. GANHO: diferenca (R$ and multiplicador)
    5. IMPACTO NA VIDA: o que muda
    # 
    def __init__(self):
        self.workers = {w.worker_id: w para w em WORKERS}
    def simulate_worker(self, worker_id: texto) -> {texto: qualquer}:
        ...
        company_extracted = max(0, company_profit)
        ...
        multiplier = worker_total_republic / max(w.monthly_salary, 1)
        # Impacto na vida
    ...
    def simulate_all(self) -> {texto: qualquer}:
    ...
    def aggregate_stats(self) -> {texto: qualquer}:
...
if __name__ == "__main__":
    sim = ValueSimulator()
    print("=" * 80)
    print("  OPENVALUESIMULATION -- PESSOAS REAIS, SALARIOS REAIS")
    print("  Nomes obfuscados. Dados reais do mercado brasileiro.")
    print("=" * 80)
    # === 1. CADA TRABALHADOR ===
    print("\n\n  === SIMULACAO POR TRABALHADOR ===\n")
    for wid in ordene(sim.workers.keys()):
        r = sim.simulate_worker(wid)
        w = sim.workers[wid]
        print("\n  {'='*70}")
        print("  {r['alias']} | {r['profissao']} | {r['empresa']} | {r['local']}")
        print("  Familia: {r['familia']} | Horas/mes: {r['horas_mes']}")
        print("  {'='*70}")
        print("\n  CAPITALISMO (hoje):")
        print("    Produz:           {r['CAPITALISMO']['produz']}")
        print("    Recebe:           {r['CAPITALISMO']['recebe']} ({r['CAPITALISMO']['fica_com']})")
        print("    Empresa suga:     {r['CAPITALISMO']['empresa_suga']}")
        print("    Banco suga:       {r['CAPITALISMO']['banco_suga']}")
        print("    Intermediario:    {r['CAPITALISMO']['intermediario_suga']}")
        print("    TOTAL EXTRAIDO:   {r['CAPITALISMO']['total_extraido']}")
        print("    Razao:            {r['CAPITALISMO']['razao_exploracao']}")
        print("    Hora vale:        {r['CAPITALISMO']['hora_valor']}")
        print("    Sobra (apos ali+comida): {r['CAPITALISMO']['sobra_apos_aluguel_comida']}")
        print("\n  REPUBLICA EXECUTAVEL:")
        print("    Produz:           {r['REPUBLICA_EXECUTAVEL']['produz']}")
        print("    Recebe:           {r['REPUBLICA_EXECUTAVEL']['recebe']} ({r['REPUBLICA_EXECUTAVEL']['fica_com']})")
        print("    Empresa suga:     {r['REPUBLICA_EXECUTAVEL']['empresa_suga']}")
        print("    Banco suga:       {r['REPUBLICA_EXECUTAVEL']['banco_suga']}")
        print("    Intermediario:    {r['REPUBLICA_EXECUTAVEL']['intermediario_suga']}")
        print("    Pool 5%:          {r['REPUBLICA_EXECUTAVEL']['pool_5pct']}")
        print("    Operacional real: {r['REPUBLICA_EXECUTAVEL']['operacional_real']}")
        print("    Hora vale:        {r['REPUBLICA_EXECUTAVEL']['hora_valor']}")
        print("    Sobra (apos ali+comida): {r['REPUBLICA_EXECUTAVEL']['sobra_apos_aluguel_comida']}")
        print("\n  GANHO:")
        print("    Extra/mes:  {r['GANHO']['extra_por_mes']}")
        print("    Extra/ano:  {r['GANHO']['extra_por_ano']}")
        print("    Multiplicador: {r['GANHO']['multiplicador']}")
        print("    Impacto: {r['GANHO']['mudanca_vida']}")
    # === 2. AGREGADO ===
    print("\n\n  {'='*70}")
    print("  === AGREGADO: TODOS OS {len(sim.workers)} TRABALHADORES ===")
    print("  {'='*70}\n")
    agg = sim.simulate_all()
    print("  {agg['message']}")
    print("\n  Extracao mensal total: {agg['total_extraido_mensal']}")
    print("  Extracao anual total:  {agg['total_extraido_anual']}")
    print("  Ganho mensal Republica: {agg['total_ganho_republica_mensal']}")
    print("  Ganho anual Republica:  {agg['total_ganho_republica_anual']}")
    # === 3. STATS ===
    print("\n\n  === ESTATISTICAS BASE ===\n")
    ...
    print("\n\n  === RESUMO (todos) ===\n")
    print("  {'Alias':<10} {'Profissao':<25} {'Hoje':>10} {'Republica':>10} {'Ganho':>10} {'X':>5} {'Impacto'}")
    print("  {'-'*95}")
    for wid in ordene(sim.workers.keys()):
        r = sim.simulate_worker(wid)
        ...
        print("  {r['alias']:<10} {r['profissao'][:24]:<25} "
            "R${int(hoje):>8,} R${int(rep):>8,} R${int(gain):>8,} "
            "{mult:>4} {impacto}")
    print("\n{'='*80}")
    print("  Simulacao: {len(sim.workers)} trabalhadores reais.")
    print("  Extracao anual: {agg['total_extraido_anual']}.")
    print("  Se virasse Republica: {agg['total_ganho_republica_anual']} voltam.")
    print("  Para trabalhadores. Nao parasitas.")
    print("{'='*80}")
