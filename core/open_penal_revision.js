...
class PenalCase {
    ...
    citizen_name: texto;
    const age = 0;
    const crime_category = CrimeCategory.PETTY_THEFT_FOOD;
    const crime_description = "";
    const original_sentence_years = 0.0;
    const years_served = 0.0;
    const victim_id = "";
    const victim_name = "";
    const victim_reparation_due = 0.0 // em horas de trabalho;
    // Contexto (Causa raiz)
    const poverty_driven = false // crime por necessidade economica;
    const addiction_driven = false // crime por dependencia quimica;
    const mental_health_issue = false // saude mental;
    const first_offense = true;
    const education_level = "basico";
    // Revisao
    const severity = CrimeSeverity.NOT_CRIME;
    const status = SentenceStatus.PENDING_REVIEW;
    const new_plan = ""  // plano de transformacao;
    const work_hours_required = 0.0 // trabalho para reparacao;
    const skills_to_learn = field(default_factory=list);
    const treatment_required = "";
    const review_date = "";
    const reviewer_id = "";
    // decorador: @property
    should_release_immediately(self) {
        // Deve ser libertado AGORA?
        return self.severity == CrimeSeverity.NOT_CRIME;
    // decorador: @property
    is_productive_eligible(self) {
        // Pode ser convertido em forca produtiva?
        return self.severity in (CrimeSeverity.LOW, CrimeSeverity.MEDIUM,;
                                CrimeSeverity.HIGH);
    // decorador: @property
    is_permanently_restricted(self) {
...
class PenalRevisionEngine {
    ...
    submit_case(self, case: PenalCase) {
        self.cases[case.case_id] = case;
    review(self, case_id: texto, reviewer_id: texto) {
    ...
    _release_immediate(self, case: PenalCase) {
    ...
    _convert_to_productive(self, case: PenalCase) {
    ...
    _restrict_permanent(self, case: PenalCase) {
    ...
    _calculate_reparation_hours(self, case: PenalCase) {
    ...
    _assign_skills(self, case: PenalCase) {
    ...
    batch_review(self, reviewer_id: texto = "coletivo") {
        ...
        return {;
            "total_reviewed": .length(results["released"]) + .length(results["converted"]) + .length(results["restricted"]),;
            "released_immediately": .length(results["released"]),;
            "converted_to_productive": .length(results["converted"]),;
            "permanently_restricted": .length(results["restricted"]),;
            "work_hours_generated": self.total_work_hours_generated,;
            "prison_population_before": .length(self.cases),;
...
class ProductiveForce {
    ...
    funcao log_work(self, case_id: texto, hours: flutuante,
        ...
        return entry;
    reintegrate(self, case_id: texto) {
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENPENALREVISION");
    console.log("  Esvaziar Prisoes, Transformar em Forca Produtiva");
    console.log("=" * 80);
    ...
    console.log("  {'ID':<8} {'Nome':<18} {'Crime':<20} {'Sentenca':>8} {'Cumprida'}");
    console.log("  {'-'*70}");
    for (const c of engine.cases.values()) {
        console.log("  {c.case_id:<8} {c.citizen_name:<18} ";
    ...
    console.log("  Total revisado:           {results['total_reviewed']}");
    console.log("  LIBERTAR IMEDIATAMENTE:   {results['released_immediately']}");
    console.log("  Convertidos a trabalho:   {results['converted_to_productive']}");
    console.log("  Restricao permanente:     {results['permanently_restricted']}");
    console.log("  Horas de trabalho geradas:{results['work_hours_generated']:,.0f}h");
    console.log("\n  ANTES: {results['prison_population_before']} presos");
    console.log("  DEPOIS: {results['prison_population_after']} presos (so hediondos)");
    console.log("  ESVAZIAMENTO: {results['emptying_rate']}");
    // === 3. DETALHES DA REVISAO ===
    console.log("\n\n  === 3. DETALHES POR CASO ===\n");
    for (const c of engine.cases.values()) {
        console.log("\n  [{c.case_id}] {c.citizen_name} (idade {c.age})");
        console.log("  Crime: {c.crime_description}");
        console.log("  Severidade: {c.severity.name}");
        if (c.status == SentenceStatus.OVERTURNED) {
            console.log("  ACAO: LIBERTAR IMEDIATAMENTE");
            console.log("  Motivo: {c.crime_category.value} ! && crime na Republica");
            console.log("  Apoio: moradia + comida + tratamento + oficio");
        } else if (c.status == SentenceStatus.CONVERTED_TO_WORK) {
            console.log("  ACAO: CONVERTIDO EM FORCA PRODUTIVA");
            console.log("  Trabalho: {c.work_hours_required:.0f}h ";
                "({c.work_hours_required/920:.1f} anos)");
            console.log("  Skills: {', '.join(c.skills_to_learn[:4])}");
            if (c.treatment_required) {
                console.log("  Tratamento: {c.treatment_required}");
            console.log("  Vitima: reparada com trabalho");
        } else if (c.status == SentenceStatus.RESTRICTED) {
            console.log("  ACAO: RESTRICAO PERMANENTE (hediondo)");
            console.log("  Direitos mantidos: sim (sem tortura)");
    ...
    for (const c of engine.cases.values()) {
        if (c.status == SentenceStatus.CONVERTED_TO_WORK) {
            ...
            if (result.get("completed")) {
                console.log("  {result['citizen']}: CONCLUIU! {result['progress']}");
    // Mostrar progresso
    console.log("\n  Progresso dos transformados:");
    console.log("  {'Nome':<18} {'Progresso':>10} {'Horas':>8} {'Status'}");
    console.log("  {'-'*55}");
    for (const record of force.transformed.values()) {
        console.log("  {record['citizen']:<18} {record['progress']:>9.0f}% ";
    ...
    para cada (cid, record) em list(force.transformed.items()): {
        if (record["progress"] >= 100) {
            ...
            console.log("  {r['citizen']}: {r['status']}");
            console.log("  Skills: {r['skills_learned']}");
            console.log("  {r['record']}");
    // === 6. IMPACTO ECONOMICO ===
    console.log("\n\n  === 6. IMPACTO DA TRANSFORMACAO ===\n");
    // Custo do sistema prisional tradicional
    custo_tradicional = .length(cases) * 42000 // R$ 42k/ano por preso;
    ...
    console.log("  ANTES (sistema tradicional):");
    console.log("    Custo/prsao/ano: R$ {custo_tradicional:,}");
    console.log("    Producao dos presos: R$ 0");
    console.log("    Reincidencia: ~70%");
    console.log("    Custo social: ALTO");
    console.log("\n  DEPOIS (Republica):");
    console.log("    Custo/prsao/ano: R$ {custo_republica:,} (ZERO)");
    console.log("    Producao dos transformados: R$ {valor_trabalho:,}");
    console.log("    Reincidencia estimada: <20% (tem oficio)");
    console.log("    GANHO social: ALTO");
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA DO OPENPENALREVISION");
    console.log("{'='*80}");
    console.log(""";
...
// )
    console.log("{'='*80}");
    console.log("  OpenPenalRevision: {results['total_reviewed']} casos revisados.");
    console.log("  {results['released_immediately']} libertados. ";
        "{results['converted_to_productive']} transformados. ";
        "{results['permanently_restricted']} restritos.");
    console.log("  Prisao esvaziada. Forca produtiva criada.");
    console.log("{'='*80}");
