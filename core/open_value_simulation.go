...
package openvaluesimulation_simulacoes_com_pessoas_reais_nomes_obfuscados
...
type ValueSimulator struct {
    ...
    func simulate_worker(self, worker_id: texto) {texto: qualquer} {
    ...
    func _life_impact(self, sobra_cap: flutuante, sobra_rep: flutuante) string {
    ...
    func aggregate_stats(self) {texto: qualquer} {
        ...
        return {
            "trabalhadores": len(self.workers),
...
if __name__ == "__main__" {
    sim = ValueSimulator()
    fmt.Println("=" * 80)
    fmt.Println("  OPENVALUESIMULATION -- PESSOAS REAIS, SALARIOS REAIS")
    fmt.Println("  Nomes obfuscados. Dados reais do mercado brasileiro.")
    fmt.Println("=" * 80)
    // === 1. CADA TRABALHADOR ===
    fmt.Println("\n\n  === SIMULACAO POR TRABALHADOR ===\n")
    for _, wid := range ordene(sim.workers.keys()) {
        r = sim.simulate_worker(wid)
        w = sim.workers[wid]
        fmt.Println("\n  {'='*70}")
        fmt.Println("  {r['alias']} | {r['profissao']} | {r['empresa']} | {r['local']}")
        fmt.Println("  Familia: {r['familia']} | Horas/mes: {r['horas_mes']}")
        fmt.Println("  {'='*70}")
        fmt.Println("\n  CAPITALISMO (hoje):")
        fmt.Println("    Produz:           {r['CAPITALISMO']['produz']}")
        fmt.Println("    Recebe:           {r['CAPITALISMO']['recebe']} ({r['CAPITALISMO']['fica_com']})")
        fmt.Println("    Empresa suga:     {r['CAPITALISMO']['empresa_suga']}")
        fmt.Println("    Banco suga:       {r['CAPITALISMO']['banco_suga']}")
        fmt.Println("    Intermediario:    {r['CAPITALISMO']['intermediario_suga']}")
        fmt.Println("    TOTAL EXTRAIDO:   {r['CAPITALISMO']['total_extraido']}")
        fmt.Println("    Razao:            {r['CAPITALISMO']['razao_exploracao']}")
        fmt.Println("    Hora vale:        {r['CAPITALISMO']['hora_valor']}")
        fmt.Println("    Sobra (apos ali+comida): {r['CAPITALISMO']['sobra_apos_aluguel_comida']}")
        fmt.Println("\n  REPUBLICA EXECUTAVEL:")
        fmt.Println("    Produz:           {r['REPUBLICA_EXECUTAVEL']['produz']}")
        fmt.Println("    Recebe:           {r['REPUBLICA_EXECUTAVEL']['recebe']} ({r['REPUBLICA_EXECUTAVEL']['fica_com']})")
        fmt.Println("    Empresa suga:     {r['REPUBLICA_EXECUTAVEL']['empresa_suga']}")
        fmt.Println("    Banco suga:       {r['REPUBLICA_EXECUTAVEL']['banco_suga']}")
        fmt.Println("    Intermediario:    {r['REPUBLICA_EXECUTAVEL']['intermediario_suga']}")
        fmt.Println("    Pool 5%:          {r['REPUBLICA_EXECUTAVEL']['pool_5pct']}")
        fmt.Println("    Operacional real: {r['REPUBLICA_EXECUTAVEL']['operacional_real']}")
        fmt.Println("    Hora vale:        {r['REPUBLICA_EXECUTAVEL']['hora_valor']}")
        fmt.Println("    Sobra (apos ali+comida): {r['REPUBLICA_EXECUTAVEL']['sobra_apos_aluguel_comida']}")
        fmt.Println("\n  GANHO:")
        fmt.Println("    Extra/mes:  {r['GANHO']['extra_por_mes']}")
        fmt.Println("    Extra/ano:  {r['GANHO']['extra_por_ano']}")
        fmt.Println("    Multiplicador: {r['GANHO']['multiplicador']}")
        fmt.Println("    Impacto: {r['GANHO']['mudanca_vida']}")
    // === 2. AGREGADO ===
    fmt.Println("\n\n  {'='*70}")
    fmt.Println("  === AGREGADO: TODOS OS {len(sim.workers)} TRABALHADORES ===")
    fmt.Println("  {'='*70}\n")
    agg = sim.simulate_all()
    fmt.Println("  {agg['message']}")
    fmt.Println("\n  Extracao mensal total: {agg['total_extraido_mensal']}")
    fmt.Println("  Extracao anual total:  {agg['total_extraido_anual']}")
    fmt.Println("  Ganho mensal Republica: {agg['total_ganho_republica_mensal']}")
    fmt.Println("  Ganho anual Republica:  {agg['total_ganho_republica_anual']}")
    // === 3. STATS ===
    fmt.Println("\n\n  === ESTATISTICAS BASE ===\n")
    ...
    fmt.Println("\n\n  === RESUMO (todos) ===\n")
    fmt.Println("  {'Alias':<10} {'Profissao':<25} {'Hoje':>10} {'Republica':>10} {'Ganho':>10} {'X':>5} {'Impacto'}")
    fmt.Println("  {'-'*95}")
    for _, wid := range ordene(sim.workers.keys()) {
        r = sim.simulate_worker(wid)
        ...
        fmt.Println("  {r['alias']:<10} {r['profissao'][:24]:<25} "
            "R${int(hoje):>8,} R${int(rep):>8,} R${int(gain):>8,} "
            "{mult:>4} {impacto}")
    fmt.Println("\n{'='*80}")
    fmt.Println("  Simulacao: {len(sim.workers)} trabalhadores reais.")
    fmt.Println("  Extracao anual: {agg['total_extraido_anual']}.")
    fmt.Println("  Se virasse Republica: {agg['total_ganho_republica_anual']} voltam.")
    fmt.Println("  Para trabalhadores. Nao parasitas.")
    fmt.Println("{'='*80}")
