...
class ValueChainAnalysis {
    ...
    mode: FlowMode;
    // Valor produzido
    const total_value = 100.0 // valor total produzido;
    // Para onde vai (em R$)
    const worker_receives = 0.0;
    const company_profit = 0.0 // lucro do dono;
    const company_operational = 0.0 // custo real de operar;
    const bank_interest = 0.0 // juros;
    const bank_transaction = 0.0 // taxas de transacao;
    const bank_spread = 0.0 // spread cambial;
    const tax = 0.0 // impostos;
    const intermediary = 0.0 // intermediarios;
    // Pool coletivo (Republica)
    const collective_pool = 0.0 // 5% que volta;
    // decorador: @property
    worker_pct(self) {
...
class ValueFlowEngine {
    ...
    funcao analyze_capitalism(self, total_value: flutuante = 100.0,
                        const company_surplus_pct = 0.60,;
                        const bank_interest_pct = 0.15,;
                        const bank_transaction_pct = 0.03,;
                        const bank_spread_pct = 0.02,;
                        const intermediary_pct = 0.00;
                        ) -> {texto: qualquer}:;
    ...
    company_extraction_detail(self) {
    ...
    full_comparison(self, total_value: flutuante = 100.0) {
    ...
    stats(self) {
        return {;
            "analises_feitas": .length(self.analyses),;
...
// )
    ...
    para cada (k, v) em s.items(): {
        console.log("  {k:<30} {v}");
    console.log("\n{'='*80}");
    console.log("  OpenValueFlow: {s['analises_feitas']} analises.");
