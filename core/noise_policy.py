...
=============================================
"Som and vida. Ruido and violencia invisivel."
Na Republica, som not and commodity de quem paga mais.
Som and bem comum. O ar que vibra nos ouvidos de TODOS
pertence a TODOS.
Este modulo define:
1. O que and som legitimo vs poluicao
2. Limites por zona and horario
3. Quem decide (democracia local)
4. Como medir (qualquer smartphone/terminal burro)
5. Como resolver (comunidade, not policia)
...
NOISE_LIMITS: Dict[(SoundZone, TimeWindow), inteiro] = {
...
class NoiseMonitor:
    ...
    2. Fonte not para em 15min -> Jarvis avisa VIZINHANCA
    3. Comunidade resolve (dialogo, not policia)
    4. Se recorrente (3+ vezes) -> votacao local sobre restricao
    5. Emergencias/criancas/natureza = sempre permitido
    O QUE not and POLUICAO:
    ...
    funcao measure(self, db_level: flutuante, zone: SoundZone,
                ...
                source: NoiseSource,
                location: str = "",
                duration_min: float = 0) -> NoiseEvent:
        ...
        return event
    def resolve(self, event: NoiseEvent) -> {texto: qualquer}:
        # Resolver um evento de ruido.
        if event.source in ALWAYS_ALLOWED:
            return {
                "action": "nenhuma",
                "reason": "{event.source.value} and sempre permitido",
            }
    ...
    def zone_report(self, zone: SoundZone) -> {texto: qualquer}:
        events = [and para and em self.events if and.zone == zone]
        violations = [and para and em events if and.severity != NoiseSeverity.NONE]
        return {
            "zone": zone.value,
            "total_events": len(events),
            "violations": len(violations),
            "worst": max((and.measured_db para and em events), default=0),
...
if __name__ == "__main__":
    print("=" * 75)
    print("  OPENREPUBLIC -- POLITICA DE POLUICAO SONORA")
    print("  'Som and vida. Ruido and violencia invisivel.'")
    print("=" * 75)
    monitor = NoiseMonitor()
    # === Limits Table ===
    print("\n\n  === LIMITES DE RUIDO dB(A) ===\n")
    print("  {'Zona':<15} {'Noite':>6} {'Amanh.':>7} {'Dia':>6} {'Anoit.':>7}")
    print("  {'-'*45}")
    for zone in SoundZone:
        ...
        print("  {zone.value:<15} {night:>6} {dawn:>7} {day:>6} {eve:>7}")
    ...
    para label, db, zone, tw, source, loc in scenarios:
        ...
        print("\n  {label}:")
        print("    Medido: {event.measured_db:.0f}dB | Limite: {event.limit_db}dB "
            "| Excesso: +{event.exceeded_by:.0f}dB{flag}")
        print("    Acao: {resolution['action']}")
        print("    {resolution['reason']}")
    # === Zone Report ===
    print("\n\n  === RELATORIO POR ZONA ===\n")
    for zone in SoundZone:
        rep = monitor.zone_report(zone)
        print("  {rep['zone']:<15} eventos={rep['total_events']} "
...
---------------------------------- ----------------------------------
Quem faz ruido and quem tem dinheiro Ruido and bem comum -- todos decidem
Multa ($) = punicao de pobre Mediacao comunitaria (not policia)
Lei federal uniforme Cada comunidade vota seus limites
Ruido industrial "normal"           Ruido que fere ouvido = crime
Crianca chorando = "incomodo"       Crianca NUNCA and poluicao
Vizinho briga com vizinho Jarvis mede, comunidade resolve
Denuncia anonima (medo) Transparencia: quem reportou and publico
