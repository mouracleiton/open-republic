...
package opencoin_sistema_que_substitui_bets_cassinos_e_criptos
...
type CoinTransaction struct {
    ...
    amount: flutuante
    description := "" // string
    timestamp := field(default_factory=() -> datetime.now().timestamp()) // float64
type OpenCoin struct {
    ...
    func convert_to_credit(self, citizen: texto, amount: flutuante) {texto: qualquer} {
    ...
    funcao attempt_bet(self, citizen: texto, amount: flutuante,
        ...
        replacement = next((r para r em REPLACEMENTS if r.vice == bet_type), nil)
        return {
            "bet_attempted": false,
            "blocked": true,
    ...
    func expire_all(self) {texto: qualquer} {
    ...
    func balance(self, citizen: texto) {texto: qualquer} {
        return {
            "citizen": citizen,
            "balance": self.balances[citizen],
...
type DemocraticLottery struct {
    ...
    func register_all(self, citizens: [texto]) None {
        // Todos cidadaos sao automaticamente registrados.
        self.participants = list(citizens)
    func draw(self, prize: texto, n_winners: inteiro = 1) {texto: qualquer} {
        // Realiza sorteio com chance igual para todos.
        winners = random.sample(self.participants, minimo(n_winners, len(self.participants)))
        result = {
            "prize": prize,
            "winners": winners,
...
if __name__ == "__main__" {
    fmt.Println("=" * 75)
    fmt.Println("  OPENCOIN -- A MOEDA QUE NAO E MOEDA")
    fmt.Println("  Substitui bets, cassinos && criptos")
    fmt.Println("=" * 75)
    coin = OpenCoin()
    lottery = DemocraticLottery()
    // === 1. O QUE SUBSTITUI ===
    fmt.Println("\n\n  === 1. VICIOS SUBSTITUIDOS ===\n")
    fmt.Println("  {'Vicio':<25} {'Substituido por':<30} {'Recompensa'}")
    fmt.Println("  {'-'*75}")
    for _, rep := range REPLACEMENTS {
        fmt.Println("  {rep.vice.value:<25} {rep.replacement_activity:<30} "
            "{rep.reward_type}")
    // === 2. GANHAR OPENCOIN ===
    ...
    fmt.Println("  Pedro tenta apostar no jogo:")
    fmt.Println("  Bloqueado: {bet['blocked']}")
    fmt.Println("  Redirecionado para: {bet['redirect']}")
    fmt.Println("  {bet['message']}")
    fmt.Println()
    bet2 = coin.attempt_bet("Carlos", 50, ViceType.CASINO)
    fmt.Println("  Carlos tenta entrar no cassino:")
    fmt.Println("  Redirecionado para: {bet2['redirect']}")
    fmt.Println("  {bet2['message']}")
    fmt.Println()
    bet3 = coin.attempt_bet("Lucas", 100, ViceType.CRYPTO_TRADING)
    fmt.Println("  Lucas tenta especular cripto:")
    fmt.Println("  Redirecionado para: {bet3['redirect']}")
    // === 4. CONVERTER EM CREDITO ===
    fmt.Println("\n\n  === 4. CONVERTENDO EM CREDITO DE ACESSO ===\n")
    conv = coin.convert_to_credit("Cleiton", 10)
    fmt.Println("  Cleiton converte 10 OpenCoin:")
    fmt.Println("  Taxa: {conv['rate']} | Credito recebido: {conv['credit_earned']}")
    fmt.Println("  Saldo restante: {coin.balances['Cleiton']}")
    ...
    fmt.Println("  Premio: {draw['prize']}")
    fmt.Println("  Ganhador: {draw['winners'][0]}")
    fmt.Println("  Participantes: {draw['participants']}")
    fmt.Println("  Custo do bilhete: {draw['ticket_cost']}")
    fmt.Println("  Chance: {draw['chance_per_person']}")
    // === 6. EXPIRACAO ===
    fmt.Println("\n\n  === 6. EXPIRACAO (! acumula) ===\n")
    exp = coin.expire_all()
    fmt.Println("  Total expirado: {exp['expired_total']}")
    fmt.Println("  Cidadaos afetados: {exp['expired_citizens']}")
    fmt.Println("  {exp['note']}")
    // === 7. STATS ===
    fmt.Println("\n\n  === 7. ESTATISTICAS ===\n")
    s = coin.stats()
    ...
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*75}")
    fmt.Println("  FILOSOFIA DO OPENCOIN")
    fmt.Println("{'='*75}")
    fmt.Println("""
...
PRINCIPIO:
    ...
    P4 := chance igual para todos. // Sorteio democratico
// )
    fmt.Println("{'='*75}")
    fmt.Println("  OpenCoin: a moeda que ! && moeda.")
    fmt.Println("  Zero aposta. Zero cassino. Zero cripto. Zero exploracao.")
    fmt.Println("{'='*75}")
