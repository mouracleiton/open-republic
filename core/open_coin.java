...
public class OpencoinSistemaQueSubstituiBetsCassinosECriptos {
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class ViceReplacement {
    ...
    // ============================================================================
    public static class CoinTransactionType {
    ...
    public static class CoinTransaction {
        ...
        amount: flutuante;
        String description = "";
        double timestamp = field(default_factory=() -> datetime.now().timestamp());
    public static class OpenCoin {
        ...
        public {texto: qualquer} convert_to_credit(self, citizen: texto, amount: flutuante) {
        ...
        public {texto: qualquer} expire_all(self) {
        ...
        public {texto: qualquer} balance(self, citizen: texto) {
            return {;
                "citizen": citizen,;
                "balance": self.balances[citizen],;
    ...
    // ============================================================================
    public static class DemocraticLottery {
        ...
        public None register_all(self, citizens: [texto]) {
            // Todos cidadaos sao automaticamente registrados.
            self.participants = list(citizens);
        public {texto: qualquer} draw(self, prize: texto, n_winners: inteiro = 1) {
            ...
            /* TODO: for-each Java para w em winners */
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENCOIN -- A MOEDA QUE NAO E MOEDA");
        System.out.println("  Substitui bets, cassinos && criptos");
        System.out.println("=" * 75);
        coin = OpenCoin();
        lottery = DemocraticLottery();
        // === 1. O QUE SUBSTITUI ===
        System.out.println("\n\n  === 1. VICIOS SUBSTITUIDOS ===\n");
        System.out.println("  {'Vicio':<25} {'Substituido por':<30} {'Recompensa'}");
        System.out.println("  {'-'*75}");
        /* TODO: for-each Java para rep em REPLACEMENTS */
            System.out.println("  {rep.vice.value:<25} {rep.replacement_activity:<30} ";
                "{rep.reward_type}");
        // === 2. GANHAR OPENCOIN ===
        ...
        System.out.println("  Cleiton: {r1['reason']} +{r1['amount']} (Construiu horta comunitaria)");
        r2 = coin.earn("Ana", 10, CoinTransactionType.SKILL_REWARD,;
                    "Venceu torneio de xadrez da comunidade");
        System.out.println("  Ana: {r2['reason']} +{r2['amount']} (Venceu torneio de xadrez)");
        r3 = coin.earn("Joao", 8, CoinTransactionType.LEARNING_REWARD,;
                    "Completou curso de Rust basico");
        System.out.println("  Joao: {r3['reason']} +{r3['amount']} (Completou curso de Rust)");
        r4 = coin.earn("Maria", 5, CoinTransactionType.CHALLENGE_REWARD,;
                    "Organizou mutirao de limpeza do rio");
        System.out.println("  Maria: {r4['reason']} +{r4['amount']} (Mutirao de limpeza do rio)");
        ...
        System.out.println("  Pedro tenta apostar no jogo:");
        System.out.println("  Bloqueado: {bet['blocked']}");
        System.out.println("  Redirecionado para: {bet['redirect']}");
        System.out.println("  {bet['message']}");
        System.out.println();
        bet2 = coin.attempt_bet("Carlos", 50, ViceType.CASINO);
        System.out.println("  Carlos tenta entrar no cassino:");
        System.out.println("  Redirecionado para: {bet2['redirect']}");
        System.out.println("  {bet2['message']}");
        System.out.println();
        bet3 = coin.attempt_bet("Lucas", 100, ViceType.CRYPTO_TRADING);
        System.out.println("  Lucas tenta especular cripto:");
        System.out.println("  Redirecionado para: {bet3['redirect']}");
        // === 4. CONVERTER EM CREDITO ===
        System.out.println("\n\n  === 4. CONVERTENDO EM CREDITO DE ACESSO ===\n");
        conv = coin.convert_to_credit("Cleiton", 10);
        System.out.println("  Cleiton converte 10 OpenCoin:");
        System.out.println("  Taxa: {conv['rate']} | Credito recebido: {conv['credit_earned']}");
        System.out.println("  Saldo restante: {coin.balances['Cleiton']}");
        ...
        System.out.println("  Premio: {draw['prize']}");
        System.out.println("  Ganhador: {draw['winners'][0]}");
        System.out.println("  Participantes: {draw['participants']}");
        System.out.println("  Custo do bilhete: {draw['ticket_cost']}");
        System.out.println("  Chance: {draw['chance_per_person']}");
        // === 6. EXPIRACAO ===
        System.out.println("\n\n  === 6. EXPIRACAO (! acumula) ===\n");
        exp = coin.expire_all();
        System.out.println("  Total expirado: {exp['expired_total']}");
        System.out.println("  Cidadaos afetados: {exp['expired_citizens']}");
        System.out.println("  {exp['note']}");
        // === 7. STATS ===
        System.out.println("\n\n  === 7. ESTATISTICAS ===\n");
        s = coin.stats();
        ...
        // === FILOSOFIA ===
        System.out.println("\n\n{'='*75}");
        System.out.println("  FILOSOFIA DO OPENCOIN");
        System.out.println("{'='*75}");
        System.out.println(""";
    ...
    PRINCIPIO:;
        ...
        Sorteio democratico P4 = chance igual para todos.;
    // )
        System.out.println("{'='*75}");
        System.out.println("  OpenCoin: a moeda que ! && moeda.");
        System.out.println("  Zero aposta. Zero cassino. Zero cripto. Zero exploracao.");
        System.out.println("{'='*75}");
}
