...
class CofounderReparation {
    ...
    const principio = "CO = dois fundadores iguais, ! um principal + adjunto";
    // Os DOIS cofundadores sao METADES IGUAIS da fundacao
    const fundador_um = "Cleiton Moura Loura";
    const fundador_dois = "MING";
    const participacao_um = 0.5;
    const participacao_dois = 0.5;
    verificar_igualdade(self) {
    ...
    interpretar_co(self, prefixo: texto) {
        ...
        if (prefixo == "cofundadora") {
            ...
            return prefixo;
    e_subalternizacao(self, termo: texto) {
...
// ============================================================================
class ErroReparacao extends Erro {
...
class AuditoriaCofundacao {
    ...
    const reparacao_ativa = true;
    const violacoes = [];
    auditar(self, cofundador_a: Dict, cofundador_b: Dict) {
        const problemas = [];
        ...
        return problemas;
    relatorio(self) {
        if (.length(self.violacoes) == 0) {
            return "REPARACAO CO: OK. Dois cofundadores iguais confirmados.";
        } else {
            return "REPARACAO CO: " + texto(.length(self.violacoes)) + " violacoes detectadas";
// ============================================================================
...
class TwoPersonRuleCO {
    ...
    validar_pr(self, autor_id: texto, revisor_id: texto) {
