...
struct SoundAlert {
    ...
    let default_sound: String = ""  // som original;
    let action: AlertAction = AlertAction.DISABLED;
    let replacement: String = ""  // o que existe no lugar;
    let pressure_level: i64 = 0 // 0-5 (quanto pressiona);
// ============================================================================
// 3. CATÁLOGO DE ALERTAS DE PRESSÃO (TODOS DESATIVADOS)
// ============================================================================
let PRESSURE_ALERTS: [SoundAlert] = [;
    // === TRABALHO ===
...
struct SilenceEngine {
    ...
    fn disable_all_pressure(self) -> {texto: qualquer} {
    ...
    fn user_opt_in(self, user_id: texto, alert_id: texto) -> {texto: qualquer} {
    ...
    fn check_alert(self, user_id: texto, alert_id: texto) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENSILENCEPOLICY -- ALERTAS DE PRESSAO DESATIVADOS");
    ...
    println!("  {r['message']}");
    println!("  Som padrao no sistema: {r['som_no_sistema']}");
    ...
    for alert in engine.alerts.values() {
        ...
        println!("  [{action_icon:<10}] {alert.name[:45]:<46} pressao:{alert.pressure_level}");
    // === 3. O QUE SUBSTITUI ===
    println!("\n\n  === 3. O QUE EXISTE NO LUGAR ===\n");
    replacements = engine.what_replaces_sounds();
    for r in replacements {
        println!("\n  {r['categoria'].upper()}:");
        println!("    Antes:  {r['antes']}");
        println!("    Depois: {r['depois']}");
    // === 4. USUARIO OPT-IN ===
    println!("\n\n  === 4. USUARIO PODE OPTAR (se quiser) ===\n");
    opt = engine.user_opt_in("cleiton", "AL-H01");
    println!("  {opt['message']}");
    println!("  (Pode revogar a qualquer momento. Padrao = desativado.)");
    // === 5. HONRA AUSENCIA ===
    println!("\n\n  === 5. HONRA OPENSABSENCE ===\n");
    engine.absent_users.append("cleiton");
    check = engine.check_alert("cleiton", "AL-W01");
    println!("  Cleiton (ausente): {check['action']}");
    println!("  {check.get('message', check.get('reason', ''))}");
    // === 6. STATS ===
    println!("\n\n  === 6. ESTATISTICAS ===\n");
    s = engine.stats();
    para cada (k, v) em s.items(): {
        println!("  {k:<35} {v}");
    println!("\n{'='*80}");
    println!("  OpenSilencePolicy: {s['desativados']} desativados, ";
