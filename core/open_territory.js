...
class PopulationCenter {
    ...
    population: inteiro;
    area_km2: flutuante;
    const current_density = 0.0;
    const target_density = 0.0 // meta da Republica;
    const status = "desconhecido"  // esvaziar, manter, crescer, criar;
    // decorador: @property
    density_class(self) {
...
class NewMetropolis {
    ...
    const current_population = 0 // geralmente <50k (interior);
    const area_km2 = 500.0;
    // Infraestrutura planejada
    const has_healthcare = false // nivel Sirio-Libanes;
    const has_education = false // universidade + escolas;
    const has_transport = false // OpenMobility (metro, VLT, aeroporto);
    const has_energy = false // OpenEnergy (solar/eolica);
    const has_network = false // OpenNetwork/OpenProtocol;
    const has_fablab = false // FabLab industrial;
    const has_terminal = false // OpenTerminal em todo estabelecimento;
    const has_housing = false // OpenCivilConstruction;
    const has_food_production = false // agricultura urbana periurbana;
    const has_water = false // saneamento + reservatorio;
    // Status de construcao
    const construction_phase = "planejamento"  // planejamento, obras, habitavel, ativa;
    const construction_progress = 0.0 // 0-1;
    const est_completion_months = 36;
    // Atrativos (por que morar aqui?)
    const incentives = field(default_factory=list);
    // decorador: @property
    infrastructure_ready(self) {
...
class TerritoryEngine {
    ...
    diagnosis(self) {
        ...
        return {;
            "hiper_saturadas": .length(hyper),;
            "saturadas": .length(saturated),;
            "vazios_demograficos": .length(empty),;
    ...
    new_metroplan_summary(self) {
        ...
        return {;
            "total_novas_metropoles": .length(self.new_metropolises),;
    ...
    build_infrastructure(self, metro_id: texto) {
    ...
    stats(self) {
        ...
        return {;
            ...
            "plano": plan,;
            "construidas": built,;
            "migracoes_oferecidas": .length(self.migrations),;
            "total_metropoles": .length(self.new_metropolises),;
        };
// ============================================================================
// 5. MAIN
// ============================================================================
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENTERRITORY -- REDISTRIBUICAO POPULACIONAL E NOVAS METROPOLES");
    console.log("  'SP ta pequeno demais. O interior ! existe mais.'");
    console.log("=" * 80);
    // === 1. DIAGNOSTICO ===
    console.log("\n\n  === 1. DIAGNOSTICO: DESEQUILIBRIO TERRITORIAL ===\n");
    console.log("  {'Cidade':<25} {'Regiao':<15} {'Populacao':>12} {'Densidade':>12} {'Status'}");
    console.log("  {'-'*80}");
    para c em ordene(engine.current_centers.values(), {
                    key = (x) -> -x.current_density):;
        console.log("  {c.name:<25} {c.region:<15} {c.population:>12,} ";
            "{c.current_density:>10.0f}/km2  {c.status.upper()}");
    // === 2. DESEQUILIBRIO ===
    console.log("\n\n  === 2. O PROBLEMA ===\n");
    diag = engine.diagnosis();
    console.log("  Hiper-saturadas: {diag['hiper_saturadas']}");
    console.log("  Saturadas: {diag['saturadas']}");
    console.log("  Vazios demograficos: {diag['vazios_demograficos']}");
    console.log("  Populacao excedente (precisa sair): {diag['populacao_excedente']:,}");
    console.log("  Pior caso: {diag['pior_caso']['nome']} ";
        "({diag['pior_caso']['densidade']}, ";
        "excedente: {diag['pior_caso']['excedente']})");
    console.log("  Desequilibrio: {diag['desequilibrio']}");
    // === 3. PLANO DE NOVAS METROPOLES ===
    console.log("\n\n  === 3. PLANO: {len(engine.new_metropolises)} NOVAS METROPOLES ===\n");
    plan = engine.new_metroplan_summary();
    console.log("  Por regiao:");
    para cada (region, count) em ordene(plan["por_regiao"].items()): {
        console.log("    {region:<15} {count} metropoles");
    console.log("\n  Por .length:");
    console.log("    ALPHA (1M-2M):    {plan['alfa_1m_2m']}");
    console.log("    BETA (500k-1M):   {plan['beta_500k_1m']}");
    console.log("    GAMMA (200k-500k):{plan['gamma_200k_500k']}");
    console.log("\n  Capacidade total: {plan['capacidade_total']:,} habitantes");
    // === 4. TODAS AS NOVAS METROPOLES ===
    console.log("\n\n  === 4. NOVAS METROPOLES POR REGIAO ===\n");
    by_region = defaultdict(list);
    for (const m of engine.new_metropolises.values()) {
        by_region[m.region].append(m);
    for (const region of ordene(by_region.keys())) {
        console.log("\n  {region.upper()}:");
        for (const m of by_region[region]) {
            console.log("    [{m.metro_id}] {m.name:<25} {m.state}  ";
                "meta: {m.target_population:,}  ({m.tier.value})");
    // === 5. CONSTRUIR INFRAESTRUTURA ===
    console.log("\n\n  === 5. CONSTRUCAO DE INFRAESTRUTURA ===\n");
    // Construir todas
    for (const mid of list(engine.new_metropolises.keys())) {
        result = engine.build_infrastructure(mid);
    console.log("  {len(engine.new_metropolises)} metropoles construidas.");
    console.log("  Cada uma tem:");
    console.log("    [OK] Hospital nivel Sirio-Libanes");
    console.log("    [OK] Universidade + escolas");
    console.log("    [OK] Metro/VLT + OpenMobility");
    console.log("    [OK] Energia solar/eolica");
    console.log("    [OK] OpenNetwork/OpenProtocol");
    console.log("    [OK] FabLab industrial");
    console.log("    [OK] OpenTerminal em todo lugar");
    console.log("    [OK] Moradia (OpenCivilConstruction)");
    console.log("    [OK] Agricultura periurbana");
    console.log("    [OK] Saneamento + agua");
    ...
    para cid, name, from_city, to_metro in migrations: {
        ...
        console.log("    Status saida: {result['from_status']}");
        console.log("    Bonus: +{result['bonus_credit']} credito");
        console.log("    Voluntario: SIM (P2)");
    // === 7. SP SENDO ESVAZIADA PARA OBRAS ===
    console.log("\n\n  === 7. SAO PAULO: ESVAZIAR PARA REFORMAR ===\n");
    sp = engine.current_centers.get("Sao Paulo");
    if (sp) {
        console.log("  ANTES:");
        console.log("    Populacao: {sp.population:,}");
        console.log("    Densidade: {sp.current_density:.0f} hab/km2");
        console.log("    Excedente: {sp.surplus_population:,} pessoas precisam sair");
        console.log("\n  DEPOIS (meta):");
        console.log("    Populacao meta: {int(2000 * sp.area_km2):,}");
        console.log("    Densidade meta: 2.000 hab/km2 (ainda urbano, mas respiravel)");
        console.log("    Reducao: {sp.surplus_population:,} pessoas ({sp.surplus_population/sp.population*100:.0f}%)");
        console.log("\n  O QUE FAZER COM ESPACO LIBERADO:");
        console.log("    - Reformar infraestrutura antiga");
        console.log("    - Criar espacos verdes");
        console.log("    - Reconstruir moradias dignas");
        console.log("    - Reduzir transito");
        console.log("    - Limpar rios (Tiete, Pinheiros)");
        console.log("    - Descongestionar hospitais");
    // === 8. STATS ===
    console.log("\n\n  === 8. ESTATISTICAS ===\n");
    s = engine.stats();
    console.log("  Novas metropoles: {s['total_metropoles']}");
    console.log("  Construidas: {s['construidas']}");
    console.log("  Migracoes oferecidas: {s['migracoes_oferecidas']}");
    console.log("  Capacidade total: {s['plano']['capacidade_total']:,}");
    console.log("  Populacao excedente: {s['diagnostico']['populacao_excedente']:,}");
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA DO OPENTERRITORY");
    console.log("{'='*80}");
    console.log(""";
O PROBLEMA:;
...
PRINCIPIOS:;
    ...
    const P3 = trabalho massivo (OpenLaborRelay).;
    P4: Plano votado pela assembleia. Regioes decidem suas prioridades.;
// )
    console.log("{'='*80}");
    console.log("  OpenTerritory: {s['total_metropoles']} novas metropoles, ";
        "{s['plano']['capacidade_total']:,} de capacidade.");
    console.log("  'Interior' ! existe mais. Todo ponto && centro.");
    console.log("{'='*80}");
