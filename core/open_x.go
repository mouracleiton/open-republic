...
package openx_estrategia_para_x_twitter
...
if __name__ == "__main__" {
    fmt.Println("=" * 70)
    fmt.Println("  OPENX -- ESTRATEGIA PARA X/TWITTER")
    fmt.Println("  'Um canal. Opensoftware para estender. Sem prisao.'")
    fmt.Println("=" * 70)
    for _, strat := range XStrategy {
        fmt.Println("\n  === {strat.value.upper()} ===\n")
        items = [x para x em X_STRATEGY_PLAN if x.strategy == strat]
        items.sort(key=(x) -> x.priority)
        for _, x := range items {
            fmt.Println("  [{x.id}] P{x.priority} {x.title}")
            fmt.Println("    {x.description}")
            if x.estimated_effort {
                fmt.Println("    Esforco: {x.estimated_effort}")
            fmt.Println()
    fmt.Println("\n{'='*70}")
    fmt.Println("  {len(X_STRATEGY_PLAN)} acoes. 3 estrategias. 1 canal.")
    fmt.Println("  X: usar, estender, libertar. Nunca depender.")
    fmt.Println("{'='*70}")
