...
struct RepublicAsset {
    ...
    let files: i64 = 0;
    let lines: i64 = 0;
    let languages: [texto] = field(default_factory=list);
    let status: String = "operational";
    let dependencies: [texto] = field(default_factory=list);
    let provides: [texto] = field(default_factory=list);
    let maintainer: String = "comunidade";
// Registry of all assets
let REPUBLIC_REGISTRY: [RepublicAsset] = [;
    // INFRASTRUCTURE
...
fn count_asset(asset: RepublicAsset, base_path: Path) -> {texto: qualquer} {
...
fn full_inventory(base_path: Path = None) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    println!("=" * 75);
    println!("  OPENREPUBLIC -- REGISTRO CENTRAL DE BENS COMUNS");
    println!("  'Tudo que existe na Republica && bem publico && cadastrado.'");
    println!("=" * 75);
    inv = full_inventory();
    println!("\n  TOTAL: {inv['total_assets']} sistemas | ";
        "{inv['total_files']} arquivos | ";
        "{inv['total_lines']:,} linhas\n");
    println!("  {'Dominio':<16} {'Sistemas':>8} {'Arquivos':>8} {'Linhas':>8}");
    println!("  {'-'*44}");
    para cada (domain, data) em ordene(inv["by_domain"].items()): {
        println!("  {domain:<16} {len(data['assets']):>8} ";
    ...
    para cada (domain, data) em ordene(inv["by_domain"].items()): {
        for asset in ordene(data["assets"], key=(x) -> -x["lines"]) {
            ...
            println!("  {asset['name']:<28} {domain:<14} ";
                "{asset['lines']:>7,}{marker}");
    println!("\n{'='*75}");
    println!("  8 dominios. {inv['total_assets']} sistemas integrados.");
    println!("  {inv['total_lines']:,} linhas de codigo aberto.");
    println!("  Tudo bem comum. Tudo da Republica.");
    println!("{'='*75}");
