...
#ifndef OPENCOIN_SISTEMA_QUE_SUBSTITUI_BETS_CASSINOS_E_CRIPTOS_H
#define OPENCOIN_SISTEMA_QUE_SUBSTITUI_BETS_CASSINOS_E_CRIPTOS_H
...
// ============================================================================
// decorador: @dataclass
typedef struct ViceReplacement {
...
// ============================================================================
typedef struct CoinTransactionType {
...
// decorador: @dataclass
typedef struct CoinTransaction {
    tx_id: texto;
    citizen: texto;
...
typedef struct OpenCoin {
    ...
    {texto: qualquer} convert_to_credit(self, citizen: texto, amount: flutuante) {
    ...
    {texto: qualquer} expire_all(self) {
    ...
    {texto: qualquer} balance(self, citizen: texto) {
        return {;
            "citizen": citizen,;
            "balance": self.balances[citizen],;
    ...
    {texto: qualquer} stats(self) {
        return {;
            "total_citizens_with_coin": sizeof([b para b em self.balances.values() if b > 0]),;
            "total_coin_in_circulation": soma(self.balances.values()),;
            "total_transactions": sizeof(self.transactions),;
...
// ============================================================================
typedef struct DemocraticLottery {
    ...
    None register_all(self, citizens: [texto]) {
        // Todos cidadaos sao automaticamente registrados.
        self.participants = list(citizens);
    {texto: qualquer} draw(self, prize: texto, n_winners: inteiro = 1) {
        // Realiza sorteio com chance igual para todos.
        winners = random.sample(self.participants, minimo(n_winners, sizeof(self.participants)));
        result = {
            "prize": prize,;
            "winners": winners,;
            "participants": sizeof(self.participants),;
            "ticket_cost": "ZERO",;
            "chance_per_person": "1 em {len(self.participants)}",;
            "note": "Chance IGUAL para todos. Sem bilhete. Sem custo.",;
        };
        /* TODO: iterador C manual para w em winners */
...
if (__name__ == "__main__") {
    printf("=" * 75);
    printf("  OPENCOIN -- A MOEDA QUE NAO E MOEDA");
    printf("  Substitui bets, cassinos && criptos");
    printf("=" * 75);
    coin = OpenCoin();
    lottery = DemocraticLottery();
    // === 1. O QUE SUBSTITUI ===
    printf("\n\n  === 1. VICIOS SUBSTITUIDOS ===\n");
    printf("  {'Vicio':<25} {'Substituido por':<30} {'Recompensa'}");
    printf("  {'-'*75}");
    /* TODO: iterador C manual para rep em REPLACEMENTS */
        printf("  {rep.vice.value:<25} {rep.replacement_activity:<30} ";
            "{rep.reward_type}");
    // === 2. GANHAR OPENCOIN ===
    ...
    printf("  Pedro tenta apostar no jogo:");
    printf("  Bloqueado: {bet['blocked']}");
    printf("  Redirecionado para: {bet['redirect']}");
    printf("  {bet['message']}");
    printf();
    bet2 = coin.attempt_bet("Carlos", 50, ViceType.CASINO);
    printf("  Carlos tenta entrar no cassino:");
    printf("  Redirecionado para: {bet2['redirect']}");
    printf("  {bet2['message']}");
    printf();
    bet3 = coin.attempt_bet("Lucas", 100, ViceType.CRYPTO_TRADING);
    printf("  Lucas tenta especular cripto:");
    printf("  Redirecionado para: {bet3['redirect']}");
    // === 4. CONVERTER EM CREDITO ===
    printf("\n\n  === 4. CONVERTENDO EM CREDITO DE ACESSO ===\n");
    conv = coin.convert_to_credit("Cleiton", 10);
    printf("  Cleiton converte 10 OpenCoin:");
    printf("  Taxa: {conv['rate']} | Credito recebido: {conv['credit_earned']}");
    printf("  Saldo restante: {coin.balances['Cleiton']}");
    ...
    printf("  Premio: {draw['prize']}");
    printf("  Ganhador: {draw['winners'][0]}");
    printf("  Participantes: {draw['participants']}");
    printf("  Custo do bilhete: {draw['ticket_cost']}");
    printf("  Chance: {draw['chance_per_person']}");
    // === 6. EXPIRACAO ===
    printf("\n\n  === 6. EXPIRACAO (! acumula) ===\n");
    exp = coin.expire_all();
    printf("  Total expirado: {exp['expired_total']}");
    printf("  Cidadaos afetados: {exp['expired_citizens']}");
    printf("  {exp['note']}");
    // === 7. STATS ===
    printf("\n\n  === 7. ESTATISTICAS ===\n");
    s = coin.stats();
...
// )
    printf("{'='*75}");
    printf("  OpenCoin: a moeda que ! && moeda.");
    printf("  Zero aposta. Zero cassino. Zero cripto. Zero exploracao.");
    printf("{'='*75}");

#endif // OPENCOIN_SISTEMA_QUE_SUBSTITUI_BETS_CASSINOS_E_CRIPTOS_H
