...
#ifndef OPENREPOPOLICY_OPENSKILLS_REPOSITORIO_UNICO_E_SKILLS_COMPROVADAS_H
#define OPENREPOPOLICY_OPENSKILLS_REPOSITORIO_UNICO_E_SKILLS_COMPROVADAS_H
...
typedef struct MergeProposal {
    ...
    bool passes_constitution = true;
    char* fails_reason = "";
    bool tested = false // rodou sem erro?;
    char* test_result = "";
    char* created_date = "";
    char* merged_date = "";
typedef struct RepoEngine {
    ...
    {texto: qualquer} pull(self, citizen_id: texto) {
    ...
    funcao propose(self, author: texto, title: texto,
                ...
                bool passes_constitution = true,;
                char* fails_reason = "",;
                bool tested = true) -> {texto: qualquer}:;
    ...
    None _merge(self, proposal: MergeProposal) {
    ...
    {texto: qualquer} reject_fork(self, citizen: texto, reason: texto) {
    ...
    {texto: qualquer} repo_status(self) {
        return {;
            "repositorio": "UNICO (main)",;
            "forks_permitidos": 0,;
            "fonte_de_verdade": "REPOSITORIO PRINCIPAL",;
            **self.repo_stats,;
            "propostas_totais": sizeof(self.proposals),;
...
typedef struct VerifiedSkill {
    ...
    // decorador: @property
    bool is_verified(self) {
        return sizeof(self.proofs) > 0 && self.confidence > 0.3;
// decorador: @dataclass
typedef struct SkillProfile {
    ...
    int age = 0;
    char* profession = "";
    char* profession_level = ""  // OpenProfessions (aprendiz -> mestre);
    ...
    // decorador: @property
    int skill_count(self) {
        return sizeof(self.skills);
    // decorador: @property
    int verified_count(self) {
        return soma(1 para s em self.skills.values() if s.is_verified);
typedef struct SkillsEngine {
    ...
    funcao add_proof(self, citizen_id: texto, skill_name: texto,
        ...
        total_proofs = sizeof(skill.proofs);
        ...
        skill.level = levels[minimo(total_proofs, sizeof(levels) - 1)];
        // Confianca
        weights = {
    ...
    {texto: qualquer} get_profile(self, citizen_id: texto) {
    ...
    funcao search_by_skill(self, skill_name: texto,
        ...
        results = [];
        /* TODO: iterador C manual para profile em self.profiles.values() */
            /* TODO: iterador C manual para skill em profile.skills.values() */
    ...
    {texto: qualquer} stats(self) {
        ...
        return {;
            "total_perfis": sizeof(self.profiles),;
            "total_skills_registradas": sizeof(all_skills),;
            "skills_unicas": sizeof(set(all_skills)),;
...
if (__name__ == "__main__") {
    ...
    skills = SkillsEngine();
    printf("=" * 80);
    printf("  OPENREPOPOLICY + OPENSKILLS");
    printf("  Repositorio unico + Skills comprovadas pelo sistema");
    printf("=" * 80);
    // === PARTE 1: REPOSITORIO ===
    printf("\n\n  {'='*40}");
    printf("  PARTE 1: OPENREPOPOLICY");
    printf("  {'='*40}\n");
    ...
    printf("  {fork['citizen']}: {fork['action']}");
    printf("  Politica: {fork['policy']}");
    printf("  Alternativa: {fork['alternative']}");
    // Status repositorio
    printf("\n\n  === STATUS DO REPOSITORIO ===\n");
    ...
    // === PARTE 2: OPENSKILLS ===
    printf("\n\n  {'='*40}");
    printf("  PARTE 2: OPENSKILLS");
    printf("  {'='*40}\n");
    // Criar perfis
    printf("  === CRIANDO PERFIS ===\n");
    ...
    /* para skill, cat, proof, detail, level in proofs_cleiton: */
        ...
        printf("  {r['citizen']}: {r['skill']} -> {r['level']} ";
    ...
    /* TODO: iterador C manual para cid em ["C-001", "C-002", "C-003", "C-004"] */
        profile = skills.get_profile(cid);
        printf("\n  {profile['citizen']} ({profile['profession']})");
        printf("  Skills verificadas: {profile['verified_skills']}/{profile['total_skills']}");
        /* TODO: iterador C manual para s em profile["skills"][:3] */
            printf("    [{s['level']:<13}] {s['skill']:<25} ";
                "conf: {s['confidence']} provas: {s['proofs']}");
            /* TODO: iterador C manual para d em s["proofs_detail"][:1] */
                printf("      -> {d[:60]}");
    // === BUSCAR POR SKILL ===
    printf("\n\n  === BUSCAR: quem sabe Rust? ===\n");
    rust_devs = skills.search_by_skill("programacao_rust", min_level=2);
    /* TODO: iterador C manual para dev em rust_devs */
        printf("  {dev['citizen']:<15} {dev['level']:<13} ";
            "conf: {dev['confidence']} provas: {dev['proofs']}");
    printf("\n  === BUSCAR: quem sabe alvenaria? ===\n");
    builders = skills.search_by_skill("alvenaria", min_level=2);
    /* TODO: iterador C manual para b em builders */
        printf("  {b['citizen']:<15} {b['level']:<13} ";
            "conf: {b['confidence']} provas: {b['proofs']}");
    // === COMPARACAO CV vs OPENSKILLS ===
    printf("\n\n  === CV TRADICIONAL vs OPENSKILLS ===\n");
    comp = skills.compare_to_cv();
    printf("  {'Aspecto':<15} {'CV Tradicional':<30} {'OpenSkills'}");
    printf("  {'-'*75}");
    /* TODO: iterador C manual para c em comp */
        printf("  {c['aspecto']:<15} {c['cv_tradicional'][:29]:<30} {c['openskills'][:35]}");
    // === STATS ===
    printf("\n\n  === ESTATISTICAS ===\n");
    s = skills.stats();
    ...
    // === FILOSOFIA ===
    printf("\n\n{'='*80}");
    printf("  FILOSOFIA: REPOSITORIO UNICO + SKILLS COMPROVADAS");
    printf("{'='*80}");
    printf(""";
...
// )
    printf("{'='*80}");
    rs = repo.repo_status();
    printf("  Repo: {rs['repositorio']} | {rs['propostas_integradas']} integradas | ";
        "{rs['forks_rejeitados']} forks rejeitados.");
    printf("  Skills: {s['total_perfis']} perfis, {s['skills_unicas']} skills unicas.");
    printf("  Sem fork. Sem CV. So Republica.");
    printf("{'='*80}");

#endif // OPENREPOPOLICY_OPENSKILLS_REPOSITORIO_UNICO_E_SKILLS_COMPROVADAS_H
