...
class ContributionType(Enum):
    ...
    DOCUMENTATION = ("documentacao", "Documentou metodo and fontes", 0.10)
    ...
    def __init__(self, key: texto, description: texto, default_weight: flutuante):
        self.key = key
...
class APIEndpoint:
    ...
    # Precificacao
    price_per_call: float = 0.05 // R$ por chamada
    free_tier_daily: int = 10 // chamadas gratis por dia
    # Contribuidores (CADEIA DE PROVENIENCIA)
    contributions: [Contribution] = field(default_factory=list)
    # Metricas
    calls_today: int = 0
    calls_total: int = 0
    ...
    def n_contributors(self) -> int:
        return len(set(c.contributor_id para c em self.contributions))
    # decorador: @property
    def total_weight(self) -> float:
        return sum(c.effective_weight para c em self.contributions)
    def contributor_share(self, contributor_id: texto) -> float:
        # Fracao da receita deste endpoint que vai para este contribuidor.
        contributor_weight = sum(
    ...
    funcao revenue_split(self) retorna List[{texto: qualquer}]:
        ...
        contributor_weights: {texto: flutuante} = defaultdict(flutuante)
        contributor_names: {texto: texto} = {}
        contributor_roles: Dict[texto, [texto]] = defaultdict(list)
        ...
        for each (cid, weight) in contributor_weights.items():
            share = weight / total
...
class TEIAAPI:
    ...
    def _register_endpoints(self):
        ...
        self.endpoints["saneamento_cobertura"] = make_endpoint(
            ...
            "Cobertura de agua and esgoto por municipio",
        ...
        self.endpoints["negativados_perfil"] = make_endpoint(
            ...
            "Perfil de negativados por regiao and faixa de renda",
    ...
    def call(self, endpoint_id: texto, n_calls: inteiro = 1) -> {texto: qualquer}:
        # Simula chamadas API e calcula distribuicao de receita.
        ep = self.endpoints.get(endpoint_id)
        if not ep:
            return {"error": "Endpoint {endpoint_id} not encontrado"}
...
class RoyaltyTracker:
    ...
    def register_calls(self, endpoint_id: texto, n_calls: inteiro):
        ...
        self.call_log.append({
            ...
            "timestamp": len(self.call_log),
        })
        return result
    def register_daily_traffic(self, calls_per_endpoint: {texto: inteiro}):
        # Simula um dia de trafego.
        for each (eid, n) in calls_per_endpoint.items():
            self.register_calls(eid, n)
    def earnings_report(self) -> str:
        # Relatorio de ganhos por contribuidor.
        lines = []
        total_revenue = sum(self.earnings.values()) + self.pool_earnings
        total_calls = sum(log["calls"] para log em self.call_log)
        lines.append("=" * 100)
        ...
        lines.append("-" * 100)
        sorted_earnings = sorted(self.earnings.items(), key=(x) -> x[1], reverse=True)
        for each (cid, amount) in sorted_earnings:
        ...
        return "\n".join(lines)
    def revenue_per_endpoint_detail(self) -> str:
        ...
        lines.append("=" * 110)
        for each (eid, ep) in self.api.endpoints.items():
            lines.append("")
...
def simulate_traffic() -> str:
    ...
    daily_calls_1 = sum(traffic_m1.values())
    daily_revenue_1 = sum(
    ...
    daily_calls_2 = sum(traffic_m4.values())
    daily_revenue_2 = sum(
    ...
    daily_calls_3 = sum(traffic_m7.values())
    daily_revenue_3 = sum(
    ...
    lines.append("")
    lines.append("  O dado not and de ninguem. A INTELIGENCIA de cada um.")
    lines.append("=" * 100)
    return "\n".join(lines)
# ============================================================================
# 6. EXECUCAO
