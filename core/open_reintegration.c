...
#ifndef OPENREINTEGRATION_INFRAESTRUTURA_COMPLETA_PARA_EX_PENITENCIARIOS_H
#define OPENREINTEGRATION_INFRAESTRUTURA_COMPLETA_PARA_EX_PENITENCIARIOS_H
...
// ============================================================================
typedef struct ReintegrationPhase {
...
// ============================================================================
// decorador: @dataclass
typedef struct ReintegrationPlan {
    ...
    char* housing_location = "";
    char* assigned_mentor = "";
    char* assigned_mentor_id = "";
    char* community_group = "";
    char* work_assignment = "";
    // Historico
    ...
    // Reincidencia
    bool recidivated = false;
    char* recidivism_reason = ""  // se voltou, POR QUE? (culpa do sistema);
    void init_needs(self) {
        // Inicializa todas as necessidades como pendentes.
        /* TODO: iterador C manual para need em SupportNeed */
            self.needs[need] = SupportStatus.PENDING;
    // decorador: @property
    bool all_needs_met(self) {
        return all(s in (SupportStatus.PROVIDED, SupportStatus.ONGOING);
                /* para s em self.needs.values()) */
    // decorador: @property
    double readiness_score(self) {
...
// ============================================================================
typedef struct InfrastructureProvider {
    ...
    {texto: qualquer} provide_openkit(self, citizen_id: texto) {
    ...
    funcao provide_employment(self, citizen_id: texto,
        ...
        /* TODO: iterador C manual para skill em skills */
    ...
    funcao assign_community(self, citizen_id: texto,
                        char* group_name = "") -> {texto: qualquer}:;
        ...
        /* TODO: iterador C manual para g em self.community_groups */
            if (g["name"] == group) {
...
// ============================================================================
typedef struct RecordExpungement {
    ...
    funcao check_eligibility(self, citizen_id: texto,
                        bool was_hediondous = false) -> {texto: qualquer}:;
    ...
    {texto: qualquer} verify_clean_record(self, citizen_id: texto) {
...
// ============================================================================
typedef struct ReintegrationEngine {
    ...
    funcao prepare_release(self, plan_id: texto,
                        char* mentor_name = "Mentor Voluntario",;
                        char* mentor_id = "M-001") -> {texto: qualquer}:;
    ...
    {texto: qualquer} release(self, plan_id: texto) {
    ...
    {texto: qualquer} report_recidivism(self, plan_id: texto, reason: texto) {
    ...
    {texto: qualquer} stats(self) {
        ...
        in_progress = soma(1 para p em self.plans.values();
                        ...
                        && ! p.recidivated);
        total_outcome = sizeof(self.plans);
        rate = total_outcome > 0 ? (recidivated / total_outcome * 100) : 0;
        return {;
            "total_plans": sizeof(self.plans),;
            ...
            "expunged_records": sizeof(self.expungement.expunged),;
            "housing_provided": sizeof(self.infra.housing_units),;
            "mentors_active": sizeof(self.infra.mentors),;
            "community_groups": sizeof(self.infra.community_groups),;
        };
// ============================================================================
// 6. MAIN
// ============================================================================
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENREINTEGRATION");
    printf("  Infraestrutura Completa para Ex-Penitenciarios");
    printf("  'Para que ! precisem cometer erros.'");
    printf("=" * 80);
    ...
    /* para cid, name, age, crime, served, skills, *treatments in cases: */
        ...
        printf("  [{plan.plan_id}] {name} ({age} anos)");
        printf("    Crime original: {crime}");
        printf("    Tempo cumprido: {served:.1f} anos");
        printf("    Skills: {', '.join(skills[:3])}");
        if (treatment) {
            printf("    Tratamento: {', '.join(treatment)}");
        printf("    Readiness inicial: {plan.readiness_score:.0f}%");
    ...
    /* para cid, name, *_ in cases: */
        ...
        if (plans) {
            ...
            printf("    Moradia: {prep['details']['housing']['location']}");
            printf("    Trabalho: {prep['details']['employment']['job']}");
            printf("    Mentor: {prep['details']['mentor']['mentor']}");
            printf("    Comunidade: {prep['details']['community']['group']}");
            printf("    Saude: {prep['details']['health']['level']}");
            printf("    OpenKit: {prep['details']['kit']['cost']}");
            printf("    Pode liberar: {'SIM' if prep['can_release'] else 'NAO'}");
    // === 3. LIBERACAO ===
    ...
    /* para cid, name, *_ in cases: */
        ...
        if (plans) {
            ...
            if (release.get("released")) {
                printf("    LIBERADO com infraestrutura completa");
                printf("    Na rua: NAO -- {release['housing']}");
                printf("    Abandonado: NAO -- mentor + comunidade");
                printf("    Trabalho: {release['work']}");
                ex = release.get("prontuario", {});
                if (ex.get("expunged")) {
                    printf("    Prontuario: {ex['after']}");
                    printf("    Visivel a: {ex['visible']}");
            } else {
                printf("    {release.get('error', 'ERRO')}");
    ...
    /* para cid, name, *_ in cases: */
        ...
        if (plans) {
            ...
            /* TODO: iterador C manual para _ em intervalo(4) */
                ci = engine.check_in(plans[0].plan_id, "bom", "");
            printf("  {name}: {ci['days_since_release']} dias -> {ci['phase']}");
    ...
    /* para cid, name, *_ in cases: */
        ...
        printf("  {name:<20} -> {check['record']} ({check['status']})");
    ...
    if (plans_list) {
        ...
        printf("  {rec['citizen']} reincidiu.");
        printf("  Motivo: {rec['reason']}");
        printf("  {rec['system_failure']}");
        printf("  Suportes que falharam: {rec['failed_supports']}");
        printf("  Acao: {rec['action']}");
    // === 7. COMPARACAO ===
    printf("\n\n  === 7. COMPARACAO: SISTEMA ATUAL vs REPUBLICA ===\n");
    s = engine.stats();
    printf("  {'Metrica':<30} {'Sistema Atual':>15} {'Republica':>15}");
    printf("  {'-'*62}");
    printf("  {'Reincidencia':<30} {'70%':>14} {s['recidivism_rate']:>14}");
    printf("  {'Moradia ao sair':<30} {'Rua/favela':>15} {'Garantida':>15}");
    printf("  {'Trabalho ao sair':<30} {'Nenhum':>15} {'Garantido':>15}");
    printf("  {'Mentor':<30} {'Nenhum':>15} {'1 ano':>15}");
    printf("  {'Saude':<30} {'SUS fila':>15} {'Sirio-Lib.':>15}");
    printf("  {'Prontuario':<30} {'Marcado':>15} {'LIMPO':>15}");
    printf("  {'Comunidade':<30} {'Isolado':>15} {'Apoio mutuo':>15}");
    printf("  {'Custo':<30} {'R$ 42k/ano':>15} {'ZERO':>15}");
    // === FILOSOFIA ===
    printf("\n\n{'='*80}");
    printf("  FILOSOFIA DA REINTEGRACAO");
    printf("{'='*80}");
    printf(""";
...
// )
    printf("{'='*80}");
    printf("  OpenReintegration: {s['total_plans']} planos, ";
        "{s['fully_reintegrated']} integrados, ";
        "{s['expunged_records']} prontuarios limpos.");
    printf("  Reincidencia: {s['recidivism_rate']} (vs 70% atual).");
    printf("  Prevenir > Punir. Sempre.");
    printf("{'='*80}");

#endif // OPENREINTEGRATION_INFRAESTRUTURA_COMPLETA_PARA_EX_PENITENCIARIOS_H
