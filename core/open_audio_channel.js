...
class AudioSegment {
    ...
    const confidence = 0.0 // 0-1;
    // Speaker info (se for voz)
    const speaker_id = ""  // quem esta falando;
    const is_user = false // && o dono do dispositivo?;
    const speaker_name = "";
    // Audio features
    const volume_db = 0.0 // volume em dB;
    const snr_db = 0.0 // signal-to-noise ratio;
    const overlap = false // sobreposto com outro canal?;
    // Contexto
    const source_device = ""  // "microfone_notebook" / "smartphone_mesa";
    const room_estimate = ""  // estimativa do ambiente;
    // decorador: @property
    is_speech(self) {
...
class VoicePrint {
    ...
    user_id: texto;
    user_name: texto;
    const embedding = field(default_factory=list) // placeholder;
    const enrolled_at = "";
    const samples_count = 0;
    // Caracteristicas vocais (para demo sem modelo real)
    const pitch_range = (80.0, 200.0) // Hz (masculino tipico);
    const speaking_rate = 150.0 // palavras por minuto;
    const accent = "brasileiro";
    funcao identify(self, speaker_embedding: [flutuante],
                const pitch = 0.0) -> (logico, flutuante):;
        ...
        if (.length(self.embedding) > 0 && .length(speaker_embedding) > 0) {
...
class AudioChannelSeparator {
    ...
    funcao enroll_user(self, user_id: texto, user_name: texto,
                    const pitch_low = 80.0, pitch_high: flutuante = 200.0) -> VoicePrint:;
    ...
    funcao enroll_speaker(self, speaker_id: texto, name: texto,
                    const pitch_low = 0, pitch_high: flutuante = 0) -> VoicePrint:;
    ...
    funcao process_frame(self, audio_data: bytes,
                    const device = "microfone_notebook",;
                    const simulated_segments = null) -> [AudioSegment]:;
    ...
    stats(self) {
        return {;
            ...
            "known_speakers": .length(self.known_speakers),;
...
class AudioSource {
    ...
    const quality = 0.8 // 0-1 (quao bom && o microfone);
    const active = true;
    const latency_ms = 50 // latencia estimada;
class MultiDeviceFusion {
    ...
    add_source(self, source: AudioSource) {
    ...
    funcao best_source_for(self, channel_type: AudioChannelType) retorna AudioSource?:
        ...
        for (const dev_type of preferred) {
            for (const source of self.sources.values()) {
                if (source.device_type == dev_type && source.active) {
                    return source;
        return null;
    fusion_report(self) {
        return {;
            "total_sources": .length(self.sources),;
...
if (__name__ == "__main__") {
    console.log("=" * 75);
    console.log("  OPENAUDIOCHANNEL -- SEPARACAO EM MICRO-CANAIS");
    console.log("  'O microfone capta tudo. O cerebro separa.'");
    console.log("=" * 75);
    ...
    console.log("  Usuario: {vp.user_name}");
    console.log("  Pitch range: {vp.pitch_range[0]:.0f}-{vp.pitch_range[1]:.0f} Hz");
    console.log("  Amostras: {vp.samples_count}s de audio");
    console.log("  Voiceprint: criado (192 dimensoes)");
    ...
    for (const s of separator.known_speakers.values()) {
        console.log("    {s.user_name} (pitch {s.pitch_range[0]:.0f}-{s.pitch_range[1]:.0f} Hz)");
    // === 2. DISPOSITIVOS ===
    console.log("\n\n  === 2. MULTI-DISPOSITIVO ===\n");
    ...
    console.log("  Dispositivos: {report['total_sources']}");
    for (const d of report["devices"]) {
        console.log("    {d['name']:<20} {d['type']:<20} {d['location']:<10} Q:{d['quality']:.2f}");
    ...
    para ch em [AudioChannelType.USER_VOICE, AudioChannelType.OTHER_VOICE, {
        ...
        console.log("    {ch.value:<20} -> {best.device_name if best else 'N/A'}");
    // === 3. SIMULACAO: CENARIO REAL ===
    console.log("\n\n  === 3. CENARIO REAL: AUDIO MISTURADO ===\n");
    console.log("  Situacao: Cleiton no escritorio. Esposa conversa ao fundo.");
    console.log("  Musica toca no Spotify. Transito na rua.\n");
    ...
    console.log("  Segmentos detectados: {len(segments)}\n");
    console.log("  {'Canal':<20} {'Quem':<12} {'Prioridade':<12} {'Dur':>5} {'Texto'}");
    console.log("  {'-'*75}");
    for (const seg of segments) {
        text = seg.transcript ? seg.transcript[ : 35] : "(som)";
        marker = seg.is_command ? "[CMD]" : "";
        console.log("  {seg.channel_type.value:<20} {seg.speaker_name or '-':<12} ";
    ...
    para cada (ch_type, ch_segments) em channels.items(): {
        ...
        for (const s of ch_segments) {
            marker = s["is_user"] ? " <-- USUARIO" : "";
            text = s["transcript"]  ||  "(som ambiente)";
            console.log("    [{s['duration']}] {s['speaker'] or '-'}: ";
                "{text}{marker}");
    ...
    console.log("  NAO recebe: audio bruto de outras pessoas (privacidade)");
        ...
        if (! is_user) {
            para cada (sid, sp) em separator.known_speakers.items(): {
                if (sp.pitch_range[0] <= pitch <= sp.pitch_range[1]) {
                    ...
                    break;
        console.log("  {label} ({desc}): pitch {pitch}Hz -> {identity} ";
            "(confianca {conf:.0%})");
    // === 7. PRIVACIDADE ===
    console.log("\n\n  === 7. POLITICA DE PRIVACIDADE ===\n");
    console.log("  Tudo processa LOCALMENTE: {separator.local_only}");
    console.log("  Audio bruto NUNCA enviado: {separator.privacy_mode}");
    console.log("  So transcritos do USUARIO vao pro Hermes");
    console.log("  Outras vozes: so contexto (nome, ! conteudo)");
    console.log("  Musica/TV: so tipo, ! conteudo");
    console.log("  Ambiente: so classificacao, ! audio");
    // === 8. STATS ===
    console.log("\n\n  === 8. ESTATISTICAS ===\n");
    s = separator.stats();
    ...
    // === ARQUITETURA ===
    console.log("\n\n{'='*75}");
    console.log("  ARQUITETURA DO OPENAUDIOCHANNEL");
    console.log("{'='*75}");
    console.log(""";
...
// )
    console.log("{'='*75}");
    console.log("  OpenAudioChannel: {s['total_segments']} segmentos processados.");
    console.log("  {s['user_commands']} comandos do usuario identificados.");
    console.log("  {s['other_voices']} vozes de outros separadas (contexto only).");
    console.log("  Tudo local. Zero nuvem. Zero audio bruto enviado.");
    console.log("{'='*75}");
