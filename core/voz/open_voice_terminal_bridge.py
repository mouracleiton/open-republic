...
class CategoriaComandoShell(Enum):
...
class TraducaoComando:
    ...
    perigoso: bool = False      # rm -rf, dd, mkfs
    requer_sudo: bool = False
    descricao: str = ""
    exemplo_voz: str = ""
    exemplo_shell: str = ""
...
class ResultadoExecucao:
    ...
    tipo_saida: TipoSaida = TipoSaida.TEXTO_LIMPO
    texto_para_tts: str = ""
    # metadados
    timestamp: str = ""
...
class RegraTraducaoErro:
    ...
    sugestao: str = ""         # "Quer que eu rode com sudo?"
...
def _init_traducoes_comandos() -> List[TraducaoComando]:
    ...
    return [
        ...
        TraducaoComando("arq_rm", CategoriaComandoShell.ARQUIVO,
            ...
            "rm -i {arquivo}",
            ["arquivo"],
            perigoso=False,  # -i pede confirmacao
...
def _init_traducoes_erros() -> List[RegraTraducaoErro]:
...
class LimpadorSaida:
    ...
    @classmethod
    def detectar_tipo(cls, saida_bruta: bytes) -> TipoSaida:
        """Detecta o tipo de saida (texto, binario, vazio, etc.)."""
        if not saida_bruta:
            return TipoSaida.VAZIO
        ...
        try:
            ...
            return TipoSaida.BINARIO

        # verificar proporcao de bytes nao-texto
        nao_texto = sum(1 for b in saida_bruta if b < 9 or (13 < b < 32) or b == 127)
        if len(saida_bruta) > 0 and nao_texto / len(saida_bruta) > 0.1:
            return TipoSaida.BINARIO
        ...
        if len(linhas) > cls.MAX_LINHAS_TTS or len(texto_limpo) > cls.MAX_CHARS_TTS:
            return TipoSaida.TEXTO_LONGO

        return TipoSaida.TEXTO_LIMPO

    @classmethod
    def resumir(cls, texto: str, num_linhas: int = 5) -> str:
        """
...
class TradutorErros:
    ...
    def traduzir(self, stderr: str) -> Optional[Tuple[str, str]]:
        """
        Tenta traduzir o stderr.
        Retorna (traducao, sugestao) ou None se nao reconhecer.
        """
        stderr_lower = stderr.lower()
        for regra in self.regras:
            if re.search(regra.padrao_erro, stderr_lower, re.IGNORECASE):
                return (regra.traducao_humana, regra.sugestao)
        return None
...
class TradutorComandos:
    ...
    def traduzir(self, texto_voz: str) -> Tuple[Optional[TraducaoComando], Dict[str, str]]:
        """
        Converte texto de voz em comando shell.
        Retorna (traducao, parametros) ou (None, {}).
        """
        texto = texto_voz.strip().lower()
        if not texto:
            return (None, {})

        for tr in self.traducoes:
            for padrao in tr.padrao_voz:
                match = re.match(padrao, texto, re.IGNORECASE)
                if match:
                    params: Dict[str, str] = {}
                    for i, nome in enumerate(tr.parametros):
    ...
    def _sanitizar_parametro(self, valor: str) -> str:
    ...
    def construir_shell(self, traducao: TraducaoComando, params: Dict[str, str]) -> str:
...
class ExecutorComando:
    ...
    def executar(self, cmd_shell: str, timeout: int = TIMEOUT_PADRAO,
                 dry_run: bool = True) -> ResultadoExecucao:
        """
        Executa um comando. dry_run=True simula (para o demo).
        dry_run=False executa de verdade (perigoso -- so em producao).
        """
        inicio = datetime.now()

        if dry_run:
            return self._simular(cmd_shell, inicio)
    ...
    def _simular(self, cmd: str, inicio: datetime) -> ResultadoExecucao:
        ...
        if cmd.startswith("ls"):
            ...
            status = StatusExecucao.SUCESSO
            exit_code = 0
        elif "command not found" in cmd or "naoexiste" in cmd:
            stderr = f"{cmd.split()[0]}: command not found"
            status = StatusExecucao.NAO_ENCONTRADO
            exit_code = 127
            stdout = ""
        elif cmd.startswith("DITAR_TEXTO:"):
...
class VoiceTerminalBridgeEngine:
    """
    ...
    def voz_para_terminal(self, texto_voz: str, dry_run: bool = True) -> ResultadoExecucao:
        """
        Pipeline completo: texto de voz -> comando shell -> execucao -> resultado.
        """
        # 0. VERIFICAR PERIGO ANTES DE TUDO (mesmo antes de traduzir)
        if self.tradutor_cmd.e_perigoso(texto_voz.lower()):
        ...
        traducao, params = self.tradutor_cmd.traduzir(texto_voz)
        ...
        if self.tradutor_cmd.e_perigoso(cmd_shell):
            return ResultadoExecucao(
    ...
    def _processar_saida(self, resultado: ResultadoExecucao) -> ResultadoExecucao:
        """
        ...
        if resultado.status != StatusExecucao.SUCESSO and resultado.stderr:
            traducao = self.tradutor_erro.traduzir(resultado.stderr)
            if traducao:
                msg, sugestao = traducao
                resultado.texto_para_tts = msg
                if sugestao and self.verbosidade != NivelVerbosidade.MINIMO:
        ...
        if tipo == TipoSaida.VAZIO or not texto_limpo:
            resultado.texto_para_tts = "Feito."
            resultado.tipo_saida = TipoSaida.VAZIO
            return resultado

        if tipo == TipoSaida.TEXTO_LONGO:
            # resumir para o TTS
            if self.verbosidade == NivelVerbosidade.COMPLETO:
    ...
    def ciclo_completo(self, texto_voz: str, dry_run: bool = True) -> Dict[str, Any]:
        """
...
""")
    ...
    for cat in CategoriaComandoShell:
    ...
    for r in e.tradutor_erro.regras:
    ...
    for cmd in sorted(COMANDOS_PERIGOSOS):
    ...
    comandos_teste = [
        ...
        "instalar pacote_inexistente",  # erro: not found
        "rm -rf /",                  # BLOQUEADO (perigoso)
        "buscar relatorio",          # find
    ]

    for cmd_voz in comandos_teste:
