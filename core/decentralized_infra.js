...
class WorkloadPolicy {
    ...
    reason: texto;
    const fallback = ProcessingTier.EDGE_NODE;
...
class DeviceNode {
    ...
    const gpu_tops = 0 // AI inference capability;
    const location = "comunitario";
    const uptime_hours = 16 // horas por dia ativo;
...
class InfrastructureOptimizer {
    ...
    //
    calculate(self, population: inteiro = 1_000_000) {
    ...
    _estimate_demand(population: inteiro) {
...
if (__name__ == "__main__") {
    console.log("=" * 80);
    console.log("  OPENREPUBLIC -- INFRAESTRUTURA DESCENTRALIZADA");
    console.log("  'O datacenter && o ultimo recurso, ! o primeiro.'");
    console.log("=" * 80);
    opt = InfrastructureOptimizer();
    // === 1. Workload Policies ===
    console.log("\n\n  === ONDE CADA COISA RODA ===\n");
    console.log("  {'Carga':<25} {'Onde':<15} {'Regra'}");
    console.log("  {'-'*70}");
    para wl, policy in ordene(WORKLOAD_POLICIES.items(), {
                            key = (x) -> x[1].tier.value):;
        console.log("  {wl.value:<25} {policy.tier.value:<15} {policy.rule.value}");
    console.log("\n  Razoes:");
    para cada (wl, policy) em WORKLOAD_POLICIES.items(): {
        if (policy.tier == ProcessingTier.CENTRAL) {
            console.log("    CENTRAL: {wl.value} -- {policy.reason}");
    ...
    console.log("    Terminais burros:     {d['terminals_burros']:>10,}");
    console.log("    Smartphones comunit.: {d['smartphones']:>10,}");
    console.log("    Laptops:              {d['laptops']:>10,}");
    console.log("    CPU cores agregado:   {d['total_device_cpu_cores']:>10,}");
    console.log("    AI TOPS agregado:     {d['total_device_ai_tops']:>10,}");
    console.log("\n  EDGE NODES (1 por bairro):");
    console.log("    Quantidade:           {&&['count']:>10,}");
    console.log("    Hardware:             {&&['hardware']}");
    console.log("    CPU cores:            {&&['cpu_cores']:>10,}");
    console.log("    RAM:                  {&&['ram_gb']:>10,} GB");
    console.log("    Storage:              {&&['storage_tb']:>10,} TB");
    console.log("    AI TOPS:              {&&['ai_tops']:>10,}");
    console.log("\n  REGIONAL (1 por nacao):");
    console.log("    Quantidade:           {r['count']:>10}");
    console.log("    CPU cores:            {r['cpu_cores']:>10}");
    console.log("    RAM:                  {r['ram_gb']:>10} GB");
    console.log("    Storage:              {r['storage_tb']:>10} TB");
    console.log("\n  DATACENTER CENTRAL (MINIMO):");
    console.log("    Quantidade:           {c['count']:>10}");
    console.log("    Proposito:            {c['purpose']}");
    console.log("    CPU cores:            {c['cpu_cores']:>10}");
    console.log("    GPUs (treino IA):     {c['gpu_count']:>10}");
    console.log("    RAM:                  {c['ram_gb']:>10} GB");
    console.log("    Storage (frio):       {c['storage_tb']:>10} TB");
    console.log("    Potencia:             {c['power_kw']:>10} kW");
    console.log("\n  DISTRIBUICAO DE CARGA:");
    console.log("    No dispositivo:       {ld['on_device_pct']}%");
    console.log("    No edge node:         {ld['on_edge_pct']}%");
    console.log("    No regional:          {ld['on_regional_pct']}%");
    console.log("    No central:           {ld['on_central_pct']}%");
    // === 3. Comparison ===
    console.log("\n\n  === COMPARACAO: DATACENTER TRADICIONAL vs REPUBLICA ===\n");
    cmp = result["comparison"];
    console.log("  {'Metrica':<35} {'Tradicional':>15} {'Republica':>15} {'Reducao'}");
    console.log("  {'-'*75}");
    console.log("  {'Servidores centrais':<35} {cmp['traditional_servers']:>15,} ";
        "{c['count']:>15} {cmp['central_servers_reduction_pct']}%");
    console.log("  {'Potencia (kW)':<35} {int(cmp['traditional_power_mw']*1000):>15,} ";
        "{int(cmp['republic_power_kw']):>15,} {cmp['power_reduction_pct']}%");
    console.log("  {'Custo (USD)':<35} {cmp['traditional_cost_usd']:>15,} ";
        "{cmp['republic_cost_usd']:>15,} {cmp['cost_reduction_pct']}%");
    // === 4. Resilience ===
    console.log("\n\n  === RESILIENCIA (sem ponto unico de falha) ===\n");
    res = result["resilience"];
    console.log("  Se datacenter central cair:");
    console.log("    {res['central_down_impact']}");
    console.log("  Se edge node cair:");
    console.log("    {res['edge_down_impact']}");
    console.log("  Se dispositivo cair:");
    console.log("    {res['device_down_impact']}");
    console.log("  Arquivos P2P:");
    console.log("    {res['p2p_file_survival']}");
    // === 5. Calculate for different scales ===
    console.log("\n\n  === ESCALA: QUANTO CRESCER, CRESCER O QUE? ===\n");
    console.log("  {'Populacao':>12} {'Devices':>10} {'Edge':>8} {'Regional':>10} ";
        "{'Central':>8} {'Central %':>10}");
    console.log("  {'-'*60}");
    for (const pop of [10_000, 100_000, 1_000_000, 10_000_000, 100_000_000]) {
        r = opt.calculate(pop);
        c = r["central"]["count"];
        ...
        console.log("  {pop:>12,} {r['devices']['terminals_burros']:>10,} ";
            "{r['edge_nodes']['count']:>8,} {r['regional']['count']:>10} ";
            "{c:>8} {central_pct:>9.1f}%");
    console.log("\n\n{'='*80}");
    console.log("  PRINCIPIOS DA INFRAESTRUTURA");
    console.log("{'='*80}");
    console.log(""";
