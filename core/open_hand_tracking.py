...
===================================================================
"O controle do futuro not tem botoes.
and a sua MAO.
...
class HandState:
    ...
    landmarks: List[(inteiro, inteiro)] = field(default_factory=list)
    center: (inteiro, inteiro) = (0, 0)
    wrist_angle: float = 0.0
    fingers_up: [logico] = field(default_factory=() -> [False]*5)
    pinch_distance: float = 0.0
    prev_center: (inteiro, inteiro) = (0, 0)
...
class HandTracker:
    ...
    def _classify_gesture(self) -> Gesture:
        ...
        if f[0] and f[1] and not f[2] and not f[3] and not f[4]:
            return Gesture.GUN
        return Gesture.FLAT
    def _buffer_gesture(self, gesture: Gesture):
...
class VirtualUI:
    ...
    Elementos:
    - Cursor: segue a mao
    - Botoes: clicar com pinca or punho fechado
    - Slider: arrastar
    - Objetos: pegar and mover
    # 
    # decorador: @dataclass
    class Button:
        x: inteiro; y: inteiro; w: inteiro; h: inteiro
        label: texto
        color: Tuple[inteiro, inteiro, inteiro] = (100, 100, 100)
        hover_color: Tuple[inteiro, inteiro, inteiro] = (0, 255, 0)
        pressed: bool = False
        on_click: Any = None
    # decorador: @dataclass
    class GameObject:
        ...
        vx: float = 0.0
        vy: float = 0.0
    def __init__(self, width: inteiro = 640, height: inteiro = 480):
        self.width = width
        self.height = height
    ...
    def update(self, hand: HandState):
        ...
        if hand.detected:
            ...
            for obj in self.objects:
                obj.grabbed = False
    def _update_menu(self, hand: HandState):
        ...
        for btn in self.buttons:
            ...
            hover = (bx <= self.cursor_pos[0] <= bx + bw and
                    by <= self.cursor_pos[1] <= by + bh)
            btn.pressed = hover and click
            if btn.pressed and btn.on_click and not None:
                btn.on_click()
    def _update_catch(self, hand: HandState):
        ...
        while len(self.objects) < 5:
            self._spawn_fruit()
    def _update_paint(self, hand: HandState):
        # Pintar: apontar para desenhar.
        if not  hasattr(self, 'paint_points'):
            self.paint_points = []
        if hand.gesture == Gesture.POINT:
            ...
            if len(self.paint_points) > 1000:
                self.paint_points.pop(0)
    def _update_free(self, hand: HandState):
    ...
    funcao render(self, frame: np.ndarray, hand: HandState) retorna np.ndarray:
        ...
        for btn in self.buttons:
            color = btn.hover_color if (
                btn.x <= self.cursor_pos[0] <= btn.x + btn.w and
                btn.y <= self.cursor_pos[1] <= btn.y + btn.h and hand.detected
            ) else btn.color
            if btn.pressed:
                color = (255, 255, 255)
        ...
        # Pintura
        if self.game_mode == "paint"  and  hasattr(self, 'paint_points'):
        ...
        for each (i, line) in enumere(hud_lines):
            cv2.putText(frame, line, (10, 25 * (i + 1)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        # Instrucoes
        instr = {
            "menu": "PINCA or PUNHO para clicar. ESC sai.",
...
class HandTrackingApp:
    ...
    # 
    def __init__(self, camera_index: inteiro = 0):
    ...
    def run(self):
        ...
        if not cap.isOpened():
            print("ERRO: webcam not disponivel")
            return None
        # Configurar camera
        ...
        self.running = True
        print("=" * 50)
        print("  OpenHandTracking")
        print("  Mostre a mao para a camera.")
        print("  PINCA/PUNHO = clicar. ESC = sair. M = menu.")
        print("=" * 50)
        prev_time = time.time()
        while self.running:
