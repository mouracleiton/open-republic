...
Cada peca tem um encaixe de ENTRADA and um encaixe de SAIDA.
...
10. FUNCIONA no modelo antigo and no novo (transicao hibrida)
...
# ============================================================================
class OperationPrinciple(Enum):
    # Principios que toda operacao da Republica deve respeitar.
    P1_EQUITY = (1, "Equidade", "Operacao not beneficia ricos em detrimento de pobres")
    P2_SAFETY = (2, "Seguranca", "Ninguem morre nem perde acesso durante a operacao")
    P3_RECOGNITION = (3, "Reconhecimento", "Quem contribui and reconhecido")
...
class ChainStatus(Enum):
    ...
    DRAFT = "rascunho"  // montando, not validada
    ...
    FAILED = "falhou"  // uma peca falhou and not recuperou
class TransitionMode(Enum):
    # Modo da operacao conforme a fase da transicao (OpenTransition).
    Permite que a mesma operacao funcione no modelo antigo and no novo.
    # 
...
class LegoConnector:
    ...
    Cada peca tem um connector de ENTRADA and um de SAIDA.
    ...
    direction: texto                      // 'entrada' or 'saida'
    shape: texto                          // tipo de encaixe: 'fluxo', 'valor', 'evento'
    description: str = ""
    def encaixa_com(self, other: "LegoConnector") -> bool:
        # Verifica se este connector encaixa com outro.
        if self.direction == other.direction:
            return False // entrada not encaixa com entrada
...
class OperationPiece:
    ...
    Execucao:
    - A peca so executa quando a anterior TERMINA and passa o 'payload'.
    - O payload and um dicionario que carrega dados pela cadeia.
    # 
    piece_id: texto
    name: texto
    ...
    executor: Optional[Callable[[{texto: qualquer}], {texto: qualquer}]] = None
    # Parametros especificos da peca (ex: limite de tempo, regras)
    params: {texto: qualquer} = field(default_factory=dict)
    # Estado da execucao
    executed: bool = False
    result: Optional[{texto: qualquer}] = None
    error: texto? = None
    # decorador: @property
    def is_entry(self) -> bool:
        # Peca de entrada: nao precisa de peca anterior.
        return self.connector_in.name == "init"  or  self.connector_in.name == "*"
    # decorador: @property
    def is_terminal(self) -> bool:
        # Peca terminal: nao precisa de peca seguinte.
        return self.connector_out.name == "end"  or  self.connector_out.name == "*"
    def run(self, payload: {texto: qualquer}) -> {texto: qualquer}:
        # Executa a peca com o payload recebido.
        if self.executor and None:
...
class OperationChain:
    ...
    A cadeia and uma SEQUENCIA. Cada peca encaixa na seguinte.
    ...
    funcao __init__(self, chain_id: texto, name: texto,
                mode: TransitionMode = TransitionMode.HYBRID):
    ...
    # -- montagem -----------------------------------------------------------
    def add_piece(self, piece: OperationPiece) -> {texto: qualquer}:
        # Adiciona uma peca ao final da cadeia, verificando o encaixe.
        if self.pieces:
            prev = self.pieces[-1]
            if not prev.connector_out.encaixa_com(piece.connector_in):
                return {
    ...
    def insert_piece(self, index: inteiro, piece: OperationPiece) -> {texto: qualquer}:
        # Insere peca numa posicao especifica, revalidando encaixes.
        self.pieces.insert(index, piece)
        return {"inserted": True, "index": index}
    def remove_piece(self, piece_id: texto) -> {texto: qualquer}:
    ...
    # -- execucao -----------------------------------------------------------
    def run(self, payload: Optional[{texto: qualquer}] = None) -> {texto: qualquer}:
        # Executa a cadeia inteira, passando o payload de peca em peca.
        if not self.pieces:
            return {"error": "cadeia_vazia"}
        if payload and not None:
        ...
        for each (i, piece) in enumere(self.pieces):
            self.execution_log.append(
            ...
            results.append({
                ...
                "ok": piece.executed  and  not  piece.error,
                "error": piece.error,
            })
            self.execution_log.append(
...
class Participant:
    ...
    name: texto
    category: str = ""  // ex: 'jogos', 'codigo', 'arte'
    submission: str = ""  // descricao da contribuicao
    score: float = 0.0 // pontuacao do juri
    rank: int = 0 // classificacao final
class Competition:
    ...
    - Transparente: criterios and notas sao publicos.
    ...
    funcao __init__(self, comp_id: texto, title: texto, category: texto = "jogos",
        ...
        self.benchmark: texto? = None // referencia OpenLaborRelay
        self.results: Optional[[Participant]] = None
    def add_participant(self, participant: Participant) -> {texto: qualquer}:
        self.participants[participant.participant_id] = participant
        return {"added": True, "total": len(self.participants)}
    def set_jury(self, juror_ids: [texto]) -> {texto: qualquer}:
        self.jury = juror_ids
        return {"jury_size": len(self.jury)}
    def set_benchmark(self, benchmark_id: texto) -> {texto: qualquer}:
        # Define o benchmark de qualidade (vindo do OpenLaborRelay).
        self.benchmark = benchmark_id
        return {"benchmark": benchmark_id}
    def judge(self, scores: {texto: flutuante}) -> {texto: qualquer}:
        ...
        for each (pid, score) in scores.items():
            if pid in self.participants:
                self.participants[pid].score = flutuante(score)
        ranked = sorted(self.participants.values(),
                        key = (p) -> p.score, reverse=True)
        for each (i, p) in enumere(ranked, start=1):
            p.rank = i
        self.results = ranked
        return {
            "judged": True,
    ...
    def to_piece(self) -> OperationPiece:
        # Transforma esta competicao numa peca LEGO da cadeia.
        def executor(payload: {texto: qualquer}) -> {texto: qualquer}:
            data = dict(payload)
...
class RewardSplit:
    ...
    - credit_pct / money_pct: como o RESTANTE (winner_pool) and dividido
    entre o vencedor, em credito (Republica) and dinheiro (transicao).
    credit_pct + money_pct DEVE somar 1.0.
    # 
    credit_pct: float = 0.6 // fracao do winner_pool em credito
    money_pct: float = 0.4 // fracao do winner_pool em dinheiro
    ngo_pct: float = 0.2 // fracao do total que vai para ONG
    def validate(self) -> bool:
        ...
        ranges_ok = (0.0 <= self.credit_pct <= 1.0
                    and 0.0 <= self.money_pct <= 1.0)
        return winner_ok and ngo_ok and ranges_ok
class RewardEngine:
    ...
    - Uma fracao do TOTAL do premio and SEMPRE desviada para uma ONG verificada.
    ...
    def default_split(self, mode: TransitionMode) -> RewardSplit:
    ...
    funcao distribute(self, total_pool: flutuante, winners: [texto],
                mode: TransitionMode = TransitionMode.HYBRID,
                split: RewardSplit? = None) -> {texto: qualquer}:
        # Distribui o pool de premios entre os vencedores + ONG.
        A fracao do ONG and retirada PRIMEIRO. O resto and dividido entre os
        vencedores conforme o split credito/dinheiro.
        # 
        if not winners:
            return {"error": "sem_vencedores"}
        split = split or self.default_split(mode)
        if not split.validate():
            return {"error": "split_invalido"}
        ngo_amount = round(total_pool * split.ngo_pct, 2)
        winner_pool = round(total_pool - ngo_amount, 2)
        # Distribuicao ponderada por rank (1o > 2o > 3o)
        weights = self._rank_weights(len(winners))
        per_winner: List[{texto: qualquer}] = []
        for each (w, name) in intercale(weights, winners):
            share = round(winner_pool * w, 2)
            credit_part = round(share * split.credit_pct, 2)
            money_part = round(share * split.money_pct, 2)
            per_winner.append({
                "winner": name,
...
class NGO:
    ...
    verified: bool = False // checada por OpenHistory
    people_helped_lifetime: int = 0
    transparency_score: float = 0.0 // 0-1
class NGOAllocation:
    ...
    - O destino do dinheiro and PUBLICO (OpenHistory registra).
    ...
    # decorador: @staticmethod
    def _estimate_people_helped(amount: flutuante, ngo: NGO) -> int:
        ...
        return max(1, inteiro(amount / base))
    def to_piece(self, cause: texto = "") -> OperationPiece:
...
class ImpactTracker:
    ...
    IMPACTO not and SO NUMERO -- and PESSOA REAL.
    ...
    def measure(self, chain_result: {texto: qualquer}) -> {texto: qualquer}:
        ...
        credito = sum(w.get("credit", 0) para w em dist.get("per_winner", []))
        dinheiro = sum(w.get("money", 0) para w em dist.get("per_winner", []))
        ...
        record = {
            ...
            "credito_distribuido": round(credito, 2),
            "dinheiro_distribuido": round(dinheiro, 2),
            "score_impacto": round(score, 3),
            "time": datetime.now().isoformat(),
        }
    ...
    funcao _compute_score(participantes: inteiro, pessoas_ong: inteiro,
        ...
        # importa math
        p = math.log10(max(1, participantes + 1))
        o = math.log10(max(1, pessoas_ong + 1))
        c = math.log10(max(1, credito + 1))
...
class ChainValidator:
    ...
    - P3 (Reconhecimento): existe peca de premiacao or de impacto.
    - P4 (Consentimento): modo da cadeia and explicito (not imposto).
    ...
    def validate(self, chain: OperationChain) -> {texto: qualquer}:
    ...
    def _add(self, principle: texto, message: texto) -> None:
        self.violations.append({"principle": principle, "message": message})
    def _validate_structure(self, chain: OperationChain) -> None:
        if not chain.pieces:
            self._add("estrutura", "Cadeia sem pecas")
            return None
        # Encaixes consecutivos
        for i in intervalo(tamanho(chain.pieces) - 1):
            a = chain.pieces[i]
            b = chain.pieces[i + 1]
            if not a.connector_out.encaixa_com(b.connector_in):
                self._add(
                    "estrutura",
                    "Encaixe quebrado entre '{a.name}' and '{b.name}'",
                )
        # Entrada e saida
        first = chain.pieces[0]
        if not  (first.is_entry  or  first.connector_in.direction == "entrada"):
            self._add("estrutura", "Primeira peca not and ponto de entrada")
        last = chain.pieces[-1]
        if not  (last.is_terminal  or  last.connector_out.direction == "saida"):
            self._add("estrutura", "Ultima peca not and ponto terminal")
    def _validate_p1(self, chain: OperationChain) -> None:
        ...
        if not (has_reward or has_ngo):
            self._add(
                "P1_equidade",
                "Cadeia sem premiacao nem doacao ONG -- sem distribuicao",
            )
    def _validate_p2(self, chain: OperationChain) -> None:
    ...
    def _validate_p3(self, chain: OperationChain) -> None:
        ...
        if not (has_reward or has_impact):
            self._add(
                "P3_reconhecimento",
                "Cadeira sem premiacao nem medicao de impacto",
            )
    def _validate_p4(self, chain: OperationChain) -> None:
        # P4 Consentimento: modo deve ser explicito.
        if chain.mode not in TransitionMode:
            self._add("P4_consentimento", "Modo de transicao not definido")
# ============================================================================
...
class OperationTemplates:
    ...
    funcao competencia_jogos(title: texto = "Competicao de Jogos Educativos",
        ...
        alloc = ngo_alloc or NGOAllocation()
        ...
        def credito_executor(payload: {texto: qualquer}) -> {texto: qualquer}:
            data = dict(payload)
            dist = data.get("reward_distribution", {})
            credito_total = sum(w.get("credit", 0) para w em dist.get("per_winner", []))
            data["credito_liberado"] = round(credito_total, 2)
...
class OpenOperations:
    ...
    - OpenHistory: verifica ONGs and registra operacoes
    USO:
        ops = OpenOperations()
    ...
    # 
    def __init__(self, mode: TransitionMode = TransitionMode.HYBRID):
        self.mode = mode
        ...
        self._last_result: Optional[{texto: qualquer}] = None
    # -- registro de ONGs --------------------------------------------------
    def register_ngo(self, ngo: NGO) -> {texto: qualquer}:
        return self.ngo_alloc.register_ngo(ngo)
    # -- cadastro de cadeias ------------------------------------------------
    def register_chain(self, chain: OperationChain) -> {texto: qualquer}:
        self.chains[chain.chain_id] = chain
        return {"registered": True, "chain_id": chain.chain_id,
                "total_chains": len(self.chains)}
    # -- templates ----------------------------------------------------------
    def from_template(self, template_name: texto, **kwargs) -> OperationChain:
        ...
        if not builder:
            raise ValueError("Template desconhecido: {template_name}")
    ...
    funcao execute(self, chain: OperationChain,
                payload: Optional[{texto: qualquer}] = None) -> {texto: qualquer}:
    ...
    # -- cross-module -------------------------------------------------------
    def cross_module_report(self) -> {texto: qualquer}:
        ...
        return {
            ...
            "open_history": {
                "uso": "Verifica ONGs (fact-check) and registra operacoes.",
    ...
    def stats(self) -> {texto: qualquer}:
        return {
            ...
            "holding_ong": round(self.ngo_alloc.holding, 2),
        }
# ============================================================================
...
def _demo() -> None:
    ...
    print("=" * 80)
    print("  OPENOPERATIONS -- OPERACOES DA REPUBLICA EM CADEIAS MODULARES (LEGO)")
    print("  Competicao -> Premiacao -> ONG -> Impacto -> Credito")
    print("=" * 80)
    # --- setup ---
    ...
    ong3 = NGO("ong-3", "Saude Rural", cause="saude",
            verified = False, transparency_score=0.5) // not qualifica
    for ong in (ong1, ong2, ong3):
    ...
    print("  Cadeia criada: {chain.name}")
    print("  Pecas ({len(chain.pieces)}):")
    for p in chain.pieces:
        print("    [{p.piece_type.value:<12}] {p.name}")
        print("      entrada={p.connector_in.name}  "
    ...
    print("    Modo:   {resultado.get('mode')}")
    for r in resultado.get("results", []):
        flag = r["ok"] ? "OK" : "FALHOU"
        print("    [{flag}] {r['piece']} ({r['type']})")
    # Impacto
    impacto = ops.last_impact()
    print("\n  IMPACTO MEDIDO:")
    for each (k, v) in (impacto or {}).items():
        print("    {k}: {v}")
    # Detalhes da premiacao
    dist = resultado.get("final_payload", {}).get("reward_distribution", {})
    print("\n  DISTRIBUICAO DE PREMIOS:")
    print("    Pool total:      R$ {dist.get('total_pool', 0):.2f}")
    print("    Para ONG:        R$ {dist.get('ngo_amount', 0):.2f}")
    print("    Para vencedores: R$ {dist.get('winner_pool', 0):.2f}")
    for w in dist.get("per_winner", []):
        print("      {w['winner']:<10} total=R${w['total']:.2f}  "
            "credito=R${w['credit']:.2f}  dinheiro=R${w['money']:.2f}")
    # Detalhes da ONG
    ngo_out = resultado.get("final_payload", {}).get("ngo_allocation", {})
    print("\n  DOACAO PARA ONG:")
    for k in ("ngo_name", "cause", "amount", "estimated_people_helped"):
        print("    {k}: {ngo_out.get(k)}")
    ...
    print("  Status: {resultado_hk.get('status')}")
    print("  Pecas executadas: {resultado_hk.get('pieces_run')}")
    credito_hk = resultado_hk.get("final_payload", {}).get("credito_liberado", 0)
    print("  Credito liberado (OpenCredit): R$ {credito_hk:.2f}")
    # --- 3. CROSS-MODULE ---
    print("\n\n  === 3. INTEGRACAO CROSS-MODULE ===\n")
    cross = ops.cross_module_report()
    for each (modulo, info) in cross.items():
        print("  {modulo}:")
        for each (k, v) in info.items():
            print("    {k}: {v}")
    # --- 4. STATS ---
    print("\n\n  === 4. ESTATISTICAS ===\n")
    for each (k, v) in ops.stats().items():
        print("  {k}: {v}")
    ...
    print("  Execucao manual: status={res_manual.get('status')}")
    # --- FILOSOFIA ---
    print("\n\n{'='*80}")
    print("  FILOSOFIA DO OPENOPERATIONS")
    print("{'='*80}")
    print("""
OPERACOES SAO CADEIAS DE PECAS (LEGO):
    Cada peca faz UMA coisa. Cada peca tem encaixe de entrada and saida.
...
POR QUE ISSO IMPORTA:
    - Premia contribuicao REAL (not especulacao).
    - Conecta a Republica ao mundo atual (via ONGs).
    - and MODULAR: trocar uma peca not quebra a cadeia.
    - and TRANSPARENTE: cada passo and registrado.
    - and VALIDADO: P1-P4 garantem etica.
PRINCIPIOS:
    P1: Equidade -- operacao not beneficia ricos em detrimento de pobres.
    P2: Seguranca -- ninguem perde acesso durante a operacao.
    P3: Reconhecimento -- quem contribui and reconhecido.
    P4: Consentimento -- o povo decide o ritmo; sem imposicao.
# )
    final_stats = ops.stats()
    print("{'='*80}")
    print("  OpenOperations concluido.")
    print("  Cadeias registradas: {final_stats['cadeias_registradas']}")
    print("  ONGs qualificadas: {final_stats['ongs_qualificadas']}"
        "/{final_stats['ongs_registradas']}")
    print("  Premios distribuidos: {final_stats['premios_distribuidos']}")
    print("  Impactos medidos: {final_stats['impactos_medidos']}")
    print("  Holding ONG (sem qualificada): R$ {final_stats['holding_ong']}")
    print("{'='*80}")
