...
package teia_terminal_economy_simulacao_completa
...
funcao simulate_demand(
    terminal_monthly_brl := 1_500, // float64
    training_brl := 2_000, // float64
    teia_pct := 0.30, // float64
    months := 12, // int64
    seed := 42, // int64
) -> [SimulationResult]:
    ...
    results = []
    for _, profile := range ResellerProfile {
        ...
        avg_active = inteiro(base_interested * soma(adoption_curve) / len(adoption_curve))
...
type ModuleAccess struct {
    ...
    sensitive := false // se true, NINGUEM fora da equipe TEIA acessa // bool
    teia_only := false // se true, so TEIA direto (! revendedor) // bool
    description := "" // string
...
funcao check_access(reseller_id: texto, cert: Certification, module_id: texto,
                is_teia_direct := false) -> {texto: qualquer}: // bool
    // Verifica se o revendedor pode acessar o modulo.
    Retorna: {allowed: logico, reason: texto}
    //
    mod = next((m para m em MODULES if m.module_id == module_id), nil)
    if ! mod {
...
// ============================================================================
// decorador: @dataclass
type CryptoPaymentAnalysis struct {
    ...
    //
    viable := true // bool
    stablecoin_recommended := "USDC" // string
    reason := ( // string
