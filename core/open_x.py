...
===================================
"X and o unico canal social. Tudo que tocar X tem que ser:
...
X_STRATEGY_PLAN = [
    ...
    XAction("X-010", XStrategy.BUILD,
        ...
        "Sistema que classifica contatos and limpa rede automaticamente. "
        "Ja desenvolvido. So precisa auth.",
        1, "2 dias"),
    XAction("X-011", XStrategy.BUILD,
        "OpenXBridge -- X <-> Nostr",
        "Ponte bidirecional: post no X aparece no Nostr and vice-versa. "
    ...
    XAction("X-014", XStrategy.BUILD,
        ...
        "Agendar threads and posts. Compose offline, publica quando voltar online. "
    ...
    XAction("X-021", XStrategy.INTEGRATE,
        ...
        "Bot que monitora politicos no X and verifica conformidade com "
...
if __name__ == "__main__":
    print("=" * 70)
    print("  OPENX -- ESTRATEGIA PARA X/TWITTER")
    print("  'Um canal. Opensoftware para estender. Sem prisao.'")
    print("=" * 70)
    for strat in XStrategy:
        ...
        for x in items:
            print("  [{x.id}] P{x.priority} {x.title}")
            print("    {x.description}")
            if x.estimated_effort:
                print("    Esforco: {x.estimated_effort}")
            print()
    print("\n{'='*70}")
    print("  {len(X_STRATEGY_PLAN)} acoes. 3 estrategias. 1 canal.")
    print("  X: usar, estender, libertar. Nunca depender.")
    print("{'='*70}")
