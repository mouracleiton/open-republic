...
public class OpenrepublicComunicacaoPorProximidadeProximityMesh {
    ...
    public static class Device {
        ...
        device_id: texto;
        name: texto;
        String device_type = ""  // fone, oculos, terminal, tablet, ring, glove;
        // Canais suportados
    ...
    public static class ProximityMesh {
        ...
        public void __init__(self) {
            ...
            self._counter = 0;
        public void register(self, device: Device) {
            self.devices[device.device_id] = device;
        public {texto: qualquer} discover(self, device_a: texto, device_b: texto) {
            ...
            available = [];
            /* TODO: for-each Java para ch em common_channels */
        ...
        // decorador: @staticmethod
        public String _context_label(dist: flutuante, channel: Channel) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  COMUNICACAO POR PROXIMIDADE (PROXIMITY MESH)");
        System.out.println("  'Hardware se conhece pelo corpo, ! pelo IP.'");
        System.out.println("=" * 75);
        mesh = ProximityMesh();
        // === 1. Channels ===
        System.out.println("\n\n  === CANAIS DE COMUNICACAO ===\n");
        System.out.println("  {'Canal':<15} {'Alcance':>8} {'Banda':>10} {'Latencia':>9} ";
        ...
        /* para cada (ch, spec) em CHANNEL_SPECS.items(): */
            ...
            System.out.println("  {ch.value:<15} {spec.range_m:>7.2f}m {spec.bandwidth_kbps:>8.0f}kb/s ";
        ...
        /* TODO: for-each Java para d em devices */
            mesh.register(d);
            System.out.println("  {d.name:<20} ({d.device_type}) @ {d.pos} | ";
        ...
        /* para label, a, b in scenarios: */
            ...
            System.out.println("\n  {label}:");
            System.out.println("    Distancia: {result['distance_m']}m");
            System.out.println("    Canais disponiveis: {len(result['available_channels'])}");
            if (result["available_channels"]) {
                /* TODO: for-each Java para ch em result["available_channels"][:3] */
                    System.out.println("      {ch['channel']:<12} {ch['bandwidth']:.0f}kb/s ";
                        "{ch['latency']:.0f}ms {ch['power']:.0f}mW");
                System.out.println("    MELHOR: {result['best_channel']}");
            } else {
                System.out.println("    Fora de alcance de todos os canais");
        // === 4. Transfers ===
        System.out.println("\n\n  === TRANSFERENCIAS ===\n");
        transfers = [;
        ...
        /* para label, a, b, payload in transfers: */
            ...
            if (result.get("ok")) {
                ...
                System.out.println("    Canal: {result['channel']}{fb}");
                System.out.println("    Tamanho: {result['payload_size_bytes']} bytes");
                System.out.println("    Banda: {result['bandwidth_kbps']:.0f} kb/s");
                System.out.println("    Tempo: {result['transfer_time_ms']}ms");
                System.out.println("    Contexto: {result['method']}");
            } else {
                System.out.println("    ERRO: {result.get('error', '?')}");
        // === Philosophy ===
        p = ("=== " "F" "I" "L" "O" "S" "O" "F" "I" "A" " ===");
        System.out.println("\n\n{'='*75}");
