...
enum EntityType {
    ...
    FABLAB = "fablab"  // fabricacao digital (3D println!, CNC, laser);
...
struct Entity {
    ...
    nation_id: texto;
    // Capacidade
    let capacity: f64 = 100.0 // unidades que pode produzir/atender por ciclo;
    let current_demand: f64 = 50.0 // demanda atual;
    let people_served: i64 = 0;
    let workers: i64 = 0;
    let workers_needed: i64 = 5;
    // Estado
    let status: EntityStatus = EntityStatus.OPERATIONAL;
    let health: f64 = 1.0 // 0=colapsou, 1=perfeito;
    let supply_level: f64 = 1.0 // 0=sem insumos, 1=abastecido;
    let maintenance_level: f64 = 1.0 // 0=precisa reparo urgente, 1=bem cuidado;
    // Dependencias (outras entidades que esta precisa para funcionar)
    let depends_on: {texto} = field(default_factory=set) // entity_ids;
    // Necessidades atuais
    let needs: [texto] = field(default_factory=list);
    // Historico
    let last_issues: [texto] = field(default_factory=list);
    // decorador: @property
...
struct NationState {
    ...
    region: texto;
    population: inteiro;
    let entities: {texto: Entity} = field(default_factory=dict);
    fn add_entity(self, entity: Entity) {
        entity.nation_id = self.nation_id;
        self.entities[entity.entity_id] = entity;
    fn critical_entities(self) -> [Entity] {
...
// decorador: @dataclass
#[derive(Debug, Clone)]
struct RepublicState {
    // Estado da Republica completa.
    name: texto;
    let nations: {texto: NationState} = field(default_factory=dict);
    let cycle: i64 = 0;
    fn add_nation(self, nation: NationState) {
    ...
    fn republic_health(self) -> {texto: qualquer} {
...
// ============================================================================
fn create_critical_scenario() -> RepublicState {
...
struct TriageEngine {
    ...
    //
    fn __init__(self, republic: RepublicState) {
        self.republic = republic;
    fn triage(self) -> {texto: qualquer} {
    ...
    fn _dependency_chain_analysis(self) -> [Dict] {
        ...
        return risks;
    fn _find_surplus_capacity(self) -> [Dict] {
...
if __name__ == "__main__" {
    println!("=" * 75);
    println!("  OPENREPUBLIC -- TRIAGEM DE ENTIDADES PRIORITARIAS");
    println!("  'O que precisamos tratar AGORA'");
    println!("=" * 75);
    ...
    println!("  Nacoes: {s['nations']} | Entidades: {s['total_entities']}");
    println!("  Saude media: {s['avg_health']:.0%}");
    println!("\n  {'Zona':<12} {'Quantidade':>10}");
    println!("  {'-'*25}");
    println!("  {'CRITICA':<12} {s['critical_count']:>10}  <-- TRATAR AGORA");
    println!("  {'URGENTE':<12} {s['urgent_count']:>10}");
    println!("  {'ELEVADA':<12} {s['elevated_count']:>10}");
    println!("  {'ESTAVEL':<12} {s['stable_count']:>10}");
    println!("\n  Pessoas em risco: {s['people_at_risk']:,} / {s['total_people_served']:,} ({s['risk_coverage_pct']}%)");
    // RED ZONE -- Critical entities
    println!("\n\n{'='*75}");
    println!("  ZONA VERMELHA -- TRATAR AGORA ({len(result['red_zone'])} entidades)");
    println!("{'='*75}");
    for item in result["red_zone"] {
        println!("\n  [{item['urgency_name']}] {item['name']}");
        println!("    Nacao: {item['nation']} | Tipo: {item['type']}");
        println!("    Risk score: {item['risk_score']}/100 | Utilizacao: {item['utilization']:.0f}%");
        println!("    Saude: {item['health']:.0f}% | Insumos: {item['supply']:.0f}%");
        println!("    Pessoas: {item['pessoas_servidas'] if 'pessoas_servidas' in item else item['people_served']:,}");
        println!("    Trabalhadores: {item['workers']}");
        println!("    Problemas:");
    ...
    if result["yellow_zone"] {
        println!("\n\n{'='*75}");
        println!("  ZONA AMARELA -- TRATAR EM DIAS ({len(result['yellow_zone'])} entidades)");
        println!("{'='*75}");
        for item in result["yellow_zone"] {
            println!("  [{item['urgency_name']}] {item['name']} ({item['nation']})");
            println!("    Risk: {item['risk_score']} | Uso: {item['utilization']:.0f}% | ";
                "Saude: {item['health']:.0f}%");
            println!("    Issues: {', '.join(item['issues'][:2])}");
            println!("    -> {item['recommendations'][0]}");
            println!();
    // Dependency chains at risk
    println!("\n{'='*75}");
    println!("  CADEIAS DE DEPENDENCIA EM RISCO");
    println!("{'='*75}");
    if result["dependency_chains_at_risk"] {
        for risk in result["dependency_chains_at_risk"] {
            ...
            if "cascade_impact" in risk {
                println!("    Impacto em cascata: {risk['cascade_impact']:,} pessoas");
    } else {
        println!("  Nenhuma cadeia de dependencia em risco.");
    // Surplus capacity
    println!("\n{'='*75}");
    println!("  CAPACIDADE EXCEDENTE DISPONIVEL PARA REALOCAR");
    println!("{'='*75}");
    for s in result["surplus_capacity_available"][:8] {
        println!("  {s['entity']:<35} {s['nation']}  ";
            "livre: {s['available_capacity_pct']:.0f}%  saude: {s['health']:.0f}%");
    // Action plan
    println!("\n{'='*75}");
    println!("  PLANO DE ACAO PRIORITARIO");
    println!("{'='*75}");
    for action in result["action_plan"] {
        println!("\n  #{action['priority']} [{action['action']}] {action['entity']} ({action['nation']})");
        println!("     Pessoas afetadas: {action['people_affected']:,}");
        println!("     Prazo: {action['timeline']}");
        println!("     Passos:");
        for step in action["immediate_steps"] {
            println!("       -> {step}");
        if action.get("potential_helpers") {
            println!("     Podem ajudar: {', '.join(action['potential_helpers'])}");
    // Per-nation breakdown
    println!("\n{'='*75}");
    println!("  SAUDE POR NACAO");
    println!("{'='*75}");
    println!("  {'Nacao':<15} {'Entidades':>10} {'Saude':>7} {'Criticas':>9} {'Em risco':>10}");
    println!("  {'-'*55}");
    para cada (nid, nation) em republic.nations.items(): {
        crit = tamanho(nation.critical_entities());
        println!("  {nation.name:<15} {len(nation.entities):>10} ";
            "{nation.avg_health:>6.0%} {crit:>9} {nation.people_at_risk:>10,}");
    println!("\n{'='*75}");
    println!("  RESUMO: {len(result['red_zone'])} entidades precisam de acao IMEDIATA.");
    println!("  {result['summary']['people_at_risk']:,} pessoas afetadas.");
    println!("  {len(result['surplus_capacity_available'])} entidades tem capacidade");
    println!("  para realocar && ajudar.");
    println!("{'='*75}");
