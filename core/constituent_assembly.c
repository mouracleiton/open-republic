...
#ifndef CONSTITUENTASSEMBLY_A_VOZ_DO_POVO_SOBRE_A_LEI_MATEMATICA_H
#define CONSTITUENTASSEMBLY_A_VOZ_DO_POVO_SOBRE_A_LEI_MATEMATICA_H
...
typedef struct Vote {
    ...
    reasoning: texto;
// decorador: @dataclass
typedef struct AssemblyProposal {
    ...
    bool constitutional = false // se true, quorum = 60%;
    // Resultado
    [Vote] votes = field(default_factory=list);
    result: 'VoteResult'? = NULL;
    void cast_votes(self, citizens: [Citizen], rng: random.Random) {
    ...
    // decorador: @property
    int abstain_count(self) {
        return soma(1 para v em self.votes if v.choice == VoteChoice.ABSTAIN);
    // decorador: @property
    int total_votes(self) {
        return sizeof(self.votes);
    // decorador: @property
    int decided_votes(self) {
...
typedef struct ConstituentAssembly {
    ...
    void submit_proposal(self, proposal: AssemblyProposal) {
    ...
    {texto: qualquer} run_election(self) {
        ...
        constitution = {};
        /* TODO: iterador C manual para prop em self.proposals */
            prop.cast_votes(self.citizens, self.rng);
            // Analise por setor
            Dict[texto, {texto: inteiro}] by_sector = {};
            /* TODO: iterador C manual para v em prop.votes */
    ...
    char* print_results(self) {
        ...
        lines.append("-" * 100);
        /* TODO: iterador C manual para c em self.citizens */
...
// ============================================================================
ConstituentAssembly session_financing(void) {
...
if (__name__ == "__main__") {
    ...
    printf(assembly.print_results());
    printf();
    printf("=" * 100);
    printf("DECISOES APROVADAS PELA ASSEMBLEIA:");
    printf("=" * 100);
    if (constitution) {
        /* para cada (title, details) em constitution.items(): */
            printf("  >> {title}");
            printf("     Valor: {details['value']}");
            printf("     Votos: {details['votes']} ({details['yes_pct']:.0f}% sim)");
            printf();
    } else {
        printf("  Nenhuma proposta aprovada. O povo rejeitou tudo.");
        printf("  O fundador precisa ouvir && reformular.");
    printf("=" * 100);

#endif // CONSTITUENTASSEMBLY_A_VOZ_DO_POVO_SOBRE_A_LEI_MATEMATICA_H
