...
// decorador: @dataclass
#[derive(Debug, Clone)]
struct HarmfulBusiness {
    // Um negocio nocivo identificado.
    business_id: texto;
    name: texto // nome do negocio;
    business_type: HarmfulBusinessType;
    let harm_level: HarmLevel = HarmLevel.HARMFUL;
    let description: String = "";
    let victims_estimated: i64 = 0 // quantas pessoas afeta;
    let money_drained_monthly: String = ""  // quanto suga por mes;
    // Acao
    let action: BusinessAction = BusinessAction.PROHIBIT;
    let replacement: String = ""  // open equivalente;
    let conversion_target: String = ""  // para que converter;
    // Status
    let status: String = "identificado"  // identificado, votado, fechado, convertido;
    let closed_date: String = "";
// ============================================================================
// 3. CATÁLOGO DE NEGÓCIOS NOCIVOS
// ============================================================================
let HARMFUL_BUSINESSES: [HarmfulBusiness] = [;
    // === JOGO DE AZAR ===
    HarmfulBusiness(;
...
// ============================================================================
fn run_business_assembly(n_voters: inteiro = 10000) -> {texto: qualquer} {
...
struct ProhibitionEngine {
    ...
    fn list_harmful(self, severity: inteiro = 0) -> [Dict] {
    ...
    fn close_business(self, business_id: texto) -> {texto: qualquer} {
    ...
    fn _convert_space(self, biz: HarmfulBusiness) -> {texto: texto} {
    ...
    fn financial_impact(self) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENPROHIBITEDBUSINESS -- PROIBICAO DE ESTABELECIMENTOS NOCIVOS");
    println!("  Quem destroi vida ! tem lugar. Fechado. Convertido. Substituido.");
    println!("=" * 80);
    // === 1. ASSEMBLEIA ===
    ...
    println!("  {'ID':<8} {'Negocio':<35} {'Dano':<15} {'Vitimas':>12} {'Acao'}");
    println!("  {'-'*85}");
    para b em ordene(engine.businesses.values(), {
                    key = (x) -> -x.harm_level.value[1]):;
        println!("  {b.business_id:<8} {b.name[:34]:<35} {b.harm_level.value[0]:<15} ";
    ...
    println!("  {close1['business']}: {close1['status']}");
    println!("  Vitimas protegidas: {close1['victims_protected']}");
    println!("  Substituido por: {close1['replacement']}");
    println!("  Espaco convertido: {close1['space_converted']['new_use']}");
    // === 4. FECHAR BANCOS PREDATORIOS ===
    println!("\n\n  === 4. FECHANDO: BANCOS PREDATORIOS ===\n");
    close2 = engine.close_business("HB-004");
    println!("  {close2['business']}: {close2['status']}");
    println!("  Vitimas: {close2['victims_protected']}");
    println!("  Substituido: {close2['replacement']}");
    println!("  Espaco: {close2['space_converted']['new_use']}");
    // === 5. FECHAR TODOS ===
    ...
    for bid in list(engine.businesses.keys()) {
        if engine.businesses[bid].status != "fechado" {
            ...
            println!("  [FECHADO] {r['business']:<35} -> {r['space_converted']['new_use']}");
    ...
    // === FILOSOFIA ===
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA: PROIBICAO DE NOCIVOS");
    println!("{'='*80}");
    println!(""";
...
PRINCIPIOS:;
    let P1: Negocio que destrui vida = contra P1 (anti-elitismo). Proibido.;
...
// )
    println!("{'='*80}");
    println!("  OpenProhibitedBusiness: {s['fechados']}/{s['total_identificados']} fechados. ";
        "Vitimas protegidas: {s['vitimas_protegidas_total']:,}.");
    println!("  Quem destroi vida ! tem lugar.");
    println!("{'='*80}");
