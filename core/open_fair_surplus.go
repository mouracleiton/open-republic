...
package openfairsurplus_5_que_volta_pra_quem_trabalha
...
INCENTIVO := mais 5% circulando = mais se sustentando // mais trabalhadores
A MATEMATICA:
...
type FairSurplusCalc struct {
    ...
    // Producao
    workers := 0 // int64
    output_value_month := 0.0 // valor total produzido/mes // float64
    worker_pay_month := 0.0 // quanto cada trabalhador recebe // float64
    // Os 5%
    surplus_5pct := 0.0 // 5% do valor produzido // float64
    surplus_per_worker := 0.0 // quanto cada trabalhador recebe do 5% // float64
    surplus_flow := SurplusFlow.COLLECTIVE_POOL // SurplusFlow
    // Comparacao com predatorio
    predatory_90pct := 0.0 // o que o dono levaria no capitalismo // float64
    worker_gain_vs_predatory := 0.0 // quanto o trabalhador GANHA trocando // float64
    // Escala
    surplus_at_10_workers := 0.0 // float64
    surplus_at_100_workers := 0.0 // float64
    surplus_at_1000_workers := 0.0 // float64
    // decorador: @property
    func worker_total_month(self) float64 {
...
type FairSurplusEngine struct {
    ...
    funcao calculate(self, establishment: texto, est_type: texto,
                ...
                flow := SurplusFlow.COLLECTIVE_POOL // SurplusFlow
                ) -> {texto: qualquer}:
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENFAIRSURPLUS -- 5% QUE VOLTA PRA QUEM TRABALHA")
    fmt.Println("  Mais gente trabalhando = mais se sustentando")
    fmt.Println("=" * 80)
    // === 1. A DESCOBERTA ===
    fmt.Println("""
A DESCOBERTA:
...
// )
    ...
    fmt.Println("  Producao por trabalhador: {scale['output_por_trabalhador']}")
    fmt.Println("  Surplus: {scale['surplus_pct']}\n")
    fmt.Println("  {'Trabalhadores':>15} {'Producao Total':>20} {'5% Circulando':>20} {'Por Trabalhador':>20}")
    fmt.Println("  {'-'*80}")
    for _, r := range scale["escala"] {
        fmt.Println("  {r['trabalhadores']:>15} {r['producao_total']:>20} "
    ...
    fmt.Println("\n  {r1['message']}")
    fmt.Println("  Trabalhador total: {r1['trabalhador_recebe_total']}")
    fmt.Println("  No capitalismo, dono levaria: {r1['COMPARACAO_PREDATORIA']['capitalismo_dono_leva']}")
    // FabLab
    r2 = engine.calculate("FabLab Central", "construcao",
                        workers = 20, output_per_worker_month=6000,
                        worker_pay_month = 3500)
    fmt.Println("\n  {r2['message']}")
    fmt.Println("  100 trabalhadores: {r2['ESCALA']['100_trabalhadores']}")
    // Software
    ...
    for _, f := range flow {
        fmt.Println("  {f['destino']:<25} {f['pct']:<15} {f['descricao']}")
    // === 5. COMPARACAO CAPITALISMO vs REPUBLICA ===
    fmt.Println("\n\n  === 5. CAPITALISMO vs REPUBLICA ===\n")
    comp = engine.compare_capitalism_vs_republic(4000.0)
    fmt.Println("  Producao: {comp['producao_por_trabalhador']}\n")
    fmt.Println("  CAPITALISMO:")
    ...
    para cada (k, v) em s.items(): {
        fmt.Println("  {k:<25} {v}")
    fmt.Println("\n{'='*80}")
    fmt.Println("  OpenFairSurplus: {s['surplus_definido']} que volta. {s['principio']}")
    fmt.Println("{'='*80}")
