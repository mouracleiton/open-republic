...
public class OpenreintegrationInfraestruturaCompletaParaExPenitenciarios {
    ...
    // ============================================================================
    public static class ReintegrationPhase {
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class ReintegrationPlan {
        ...
        [texto] skills_learned = field(default_factory=list);
        [texto] treatment_needs = field(default_factory=list);
        // Fase atual
        ReintegrationPhase phase = ReintegrationPhase.PRE_RELEASE;
        String release_date = "";
        int days_since_release = 0;
        // Suporte
        {SupportNeed: SupportStatus} needs = field(default_factory=dict);
        String housing_location = "";
        String assigned_mentor = "";
        String assigned_mentor_id = "";
        String community_group = "";
        String work_assignment = "";
        // Historico
        [Dict] check_ins = field(default_factory=list);
        [texto] milestones_achieved = field(default_factory=list);
        [texto] flags = field(default_factory=list) // alertas (! infracoes);
        // Reincidencia
        boolean recidivated = false;
        String recidivism_reason = ""  // se voltou, POR QUE? (culpa do sistema);
        public void init_needs(self) {
            // Inicializa todas as necessidades como pendentes.
            /* TODO: for-each Java para need em SupportNeed */
                self.needs[need] = SupportStatus.PENDING;
        // decorador: @property
        public boolean all_needs_met(self) {
            return all(s in (SupportStatus.PROVIDED, SupportStatus.ONGOING);
                    /* para s em self.needs.values()) */
        // decorador: @property
        public double readiness_score(self) {
        ...
        // decorador: @property
        public boolean can_release(self) {
    ...
    public static class InfrastructureProvider {
        ...
        public {texto: qualquer} provide_openkit(self, citizen_id: texto) {
        ...
        funcao provide_employment(self, citizen_id: texto,
            ...
            /* TODO: for-each Java para skill em skills */
        ...
        funcao assign_community(self, citizen_id: texto,
                            String group_name = "") -> {texto: qualquer}:;
            ...
            /* TODO: for-each Java para g em self.community_groups */
                if (g["name"] == group) {
        ...
        funcao provide_healthcare(self, citizen_id: texto,
                            [texto] needs_treatment = null) -> {texto: qualquer}:;
    ...
    // ============================================================================
    public static class RecordExpungement {
        ...
        funcao check_eligibility(self, citizen_id: texto,
                            boolean was_hediondous = false) -> {texto: qualquer}:;
        ...
        public {texto: qualquer} verify_clean_record(self, citizen_id: texto) {
    ...
    // ============================================================================
    public static class ReintegrationEngine {
        ...
        funcao create_plan(self, citizen_id: texto, citizen_name: texto, age: inteiro,
                        ...
                        [texto] treatment_needs = null) -> ReintegrationPlan:;
        ...
        funcao prepare_release(self, plan_id: texto,
                            String mentor_name = "Mentor Voluntario",;
                            String mentor_id = "M-001") -> {texto: qualquer}:;
        ...
        public {texto: qualquer} release(self, plan_id: texto) {
        ...
        public {texto: qualquer} report_recidivism(self, plan_id: texto, reason: texto) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENREINTEGRATION");
        System.out.println("  Infraestrutura Completa para Ex-Penitenciarios");
        System.out.println("  'Para que ! precisem cometer erros.'");
        System.out.println("=" * 80);
        ...
        /* para cid, name, age, crime, served, skills, *treatments in cases: */
            ...
            System.out.println("  [{plan.plan_id}] {name} ({age} anos)");
            System.out.println("    Crime original: {crime}");
            System.out.println("    Tempo cumprido: {served:.1f} anos");
            System.out.println("    Skills: {', '.join(skills[:3])}");
            if (treatment) {
                System.out.println("    Tratamento: {', '.join(treatment)}");
            System.out.println("    Readiness inicial: {plan.readiness_score:.0f}%");
        ...
        /* para cid, name, *_ in cases: */
            ...
            if (plans) {
                ...
                System.out.println("    Moradia: {prep['details']['housing']['location']}");
                System.out.println("    Trabalho: {prep['details']['employment']['job']}");
                System.out.println("    Mentor: {prep['details']['mentor']['mentor']}");
                System.out.println("    Comunidade: {prep['details']['community']['group']}");
                System.out.println("    Saude: {prep['details']['health']['level']}");
                System.out.println("    OpenKit: {prep['details']['kit']['cost']}");
                System.out.println("    Pode liberar: {'SIM' if prep['can_release'] else 'NAO'}");
        // === 3. LIBERACAO ===
        ...
        /* para cid, name, *_ in cases: */
            ...
            if (plans) {
                ...
                if (release.get("released")) {
                    System.out.println("    LIBERADO com infraestrutura completa");
                    System.out.println("    Na rua: NAO -- {release['housing']}");
                    System.out.println("    Abandonado: NAO -- mentor + comunidade");
                    System.out.println("    Trabalho: {release['work']}");
                    ex = release.get("prontuario", {});
                    if (ex.get("expunged")) {
                        System.out.println("    Prontuario: {ex['after']}");
                        System.out.println("    Visivel a: {ex['visible']}");
                } else {
                    System.out.println("    {release.get('error', 'ERRO')}");
        ...
        /* para cid, name, *_ in cases: */
            ...
            if (plans) {
                ...
                System.out.println("  {name}: {ci['days_since_release']} dias -> {ci['phase']}");
        ...
        /* para cid, name, *_ in cases: */
            ...
            System.out.println("  {name:<20} -> {check['record']} ({check['status']})");
        ...
        if (plans_list) {
            ...
            System.out.println("  {rec['citizen']} reincidiu.");
            System.out.println("  Motivo: {rec['reason']}");
            System.out.println("  {rec['system_failure']}");
            System.out.println("  Suportes que falharam: {rec['failed_supports']}");
            System.out.println("  Acao: {rec['action']}");
        // === 7. COMPARACAO ===
        System.out.println("\n\n  === 7. COMPARACAO: SISTEMA ATUAL vs REPUBLICA ===\n");
        s = engine.stats();
        System.out.println("  {'Metrica':<30} {'Sistema Atual':>15} {'Republica':>15}");
        System.out.println("  {'-'*62}");
        System.out.println("  {'Reincidencia':<30} {'70%':>14} {s['recidivism_rate']:>14}");
        System.out.println("  {'Moradia ao sair':<30} {'Rua/favela':>15} {'Garantida':>15}");
        System.out.println("  {'Trabalho ao sair':<30} {'Nenhum':>15} {'Garantido':>15}");
        System.out.println("  {'Mentor':<30} {'Nenhum':>15} {'1 ano':>15}");
        System.out.println("  {'Saude':<30} {'SUS fila':>15} {'Sirio-Lib.':>15}");
        System.out.println("  {'Prontuario':<30} {'Marcado':>15} {'LIMPO':>15}");
        System.out.println("  {'Comunidade':<30} {'Isolado':>15} {'Apoio mutuo':>15}");
        System.out.println("  {'Custo':<30} {'R$ 42k/ano':>15} {'ZERO':>15}");
        // === FILOSOFIA ===
        System.out.println("\n\n{'='*80}");
        System.out.println("  FILOSOFIA DA REINTEGRACAO");
        System.out.println("{'='*80}");
        System.out.println(""";
    ...
    // )
        System.out.println("{'='*80}");
        System.out.println("  OpenReintegration: {s['total_plans']} planos, ";
            "{s['fully_reintegrated']} integrados, ";
            "{s['expunged_records']} prontuarios limpos.");
        System.out.println("  Reincidencia: {s['recidivism_rate']} (vs 70% atual).");
        System.out.println("  Prevenir > Punir. Sempre.");
        System.out.println("{'='*80}");
}
