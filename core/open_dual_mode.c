...
#ifndef OPENDUALMODE_O_IDEAL_COLETIVO_E_O_EXECUTAVEL_AGORA_H
#define OPENDUALMODE_O_IDEAL_COLETIVO_E_O_EXECUTAVEL_AGORA_H
...
typedef struct Commodity {
    ...
    // No IDEAL
    char* ideal_equivalent = ""  // como funciona sem dinheiro;
    ...
    // Quem produz
    char* skill_required = ""  // OpenSkills necessaria;
...
typedef struct DualModeEngine {
    ...
    //
    void __init__(self, current_phase: inteiro = 0) {
    ...
    {texto: flutuante} _calculate_mode_mix(self, phase: inteiro) {
    ...
    funcao transact(self, from_citizen: texto, to_citizen: texto,
        ...
        if (mode == OperatingMode.EXECUTAVEL) {
            ...
            impact = hours * (1 + sizeof(c.name) % 5) // simplificado;
            txn = Transaction(;
                transaction_id = tid, from_citizen=from_citizen,;
                to_citizen = to_citizen, commodity=c.name,;
    ...
    {texto: qualquer} labor_as_commodity_report(self) {
    ...
    {texto: qualquer} advance_phase(self) {
    ...
    {texto: qualquer} stats(self) {
        ...
        return {;
            ...
            "mercadorias_catalogadas": sizeof(self.commodities),;
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENDUALMODE -- O IDEAL COLETIVO E O EXECUTAVEL AGORA");
    printf("  Toda atividade humana = MERCADORIA. Trabalho = tipo de mercadoria.");
    printf("=" * 80);
    // === 1. OS DOIS MODOS ===
    printf("\n\n  === 1. OS DOIS MODOS ===\n");
    printf("  MODO IDEAL (destino):");
    printf("    Sem dinheiro. Sem propriedade. CC0.");
    printf("    Trabalho = base 1.0 + impacto. Credito expira.");
    printf("    Tudo gratis. Tudo bem comum.");
    printf("");
    printf("  MODO EXECUTAVEL (agora):");
    printf("    Dinheiro existe. Mercado existe.");
    printf("    Toda atividade = MERCADORIA. Trabalho = tipo de mercadoria.");
    printf("    OpenERP processa. OpenBusinessModel distribui.");
    printf("    Preco justo (sem exploracao). Fair price.");
    // === 2. TRABALHO COMO MERCADORIA ===
    printf("\n\n  === 2. TRABALHO COMO MERCADORIA (EXECUTAVEL) ===\n");
    report = engine.labor_as_commodity_report();
    printf("  EXECUTAVEL: {report['modo_executavel']['trabalho_e']}");
    ...
    /* TODO: iterador C manual para cid em ["CMD-L01", "CMD-L02", "CMD-L04", "CMD-C02"] */
        p = engine.price_commodity(cid);
        printf("\n  {p['mercadoria']} ({p['tipo']}):");
        printf("    EXECUTAVEL: {p['MODELO_EXECUTAVEL']['preco_brl']} ";
            "(mercado: {p['MODELO_EXECUTAVEL']['valor_mercado']})");
        printf("    IDEAL: {p['MODELO_IDEAL']['credito']} ";
    ...
    /* TODO: iterador C manual para _ em intervalo(4) */
        ...
        printf("  Fase {r['new_phase']}: {r['mode']} ";
    ...
    /* para cada (k, v) em s.items(): */
        printf("  {k:<30} {v}");
    printf("\n{'='*80}");
    printf("  OpenDualMode: fase {s['fase_transicao']}, ";
        "modo {s['modo_predominante']}.");
    printf("  {s['principio']}");
    printf("{'='*80}");

#endif // OPENDUALMODE_O_IDEAL_COLETIVO_E_O_EXECUTAVEL_AGORA_H
