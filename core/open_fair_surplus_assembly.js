...
run_assembly(n_voters: inteiro = 10000) {
...
if (__name__ == "__main__") {
    console.log("=" * 80);
    console.log("  ASSEMBLEIA CONSTITUINTE -- VOTACAO: EXCEDENTE DE 5%");
    console.log("=" * 80);
    result = run_assembly(10000);
    console.log("\n  PERGUNTA:\n    {result['question']}\n");
    console.log("  VOTACAO ({result['total']:,} cidadaos):\n");
    para option, count in ordene(result["votes"].items(), {
                                key = (x) -> -x[1]):;
        pct = count / result["total"] * 100;
        bar = "#" * inteiro(pct / 2);
        winner_mark = option == result["winner"] ? " <--- VENCEDOR" : "";
        console.log("    {option:<25} {count:>6} ({pct:.0f}%) {bar}{winner_mark}");
    console.log("\n  RESULTADO: {result['decision']}");
    console.log("  Vencedor: {result['winner']} ({result['winner_percent']})");
    console.log("  Runner-up: {result['runner_up']} ({result['runner_up_votes']/result['total']*100:.0f}%)");
    console.log("\n  LEI RATIFICADA:");
    para cada (key, val) em result["law"].items(): {
        if (isinstance(val, flutuante)) {
            console.log("    {key:<30} {val:.0%}");
        } else {
            console.log("    {key:<30} {val}");
    console.log("\n{'='*80}");
    console.log("  ASSEMBLEIA DECIDIU: 5% de excedente. LEI.");
    console.log("  62% dos cidadaos concordaram.");
    console.log("  Acima de 5% = PREDATORIO = BLOQUEADO.");
    console.log("  O 5% volta pra quem trabalha. Sempre.");
    console.log("{'='*80}");
