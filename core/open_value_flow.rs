...
// decorador: @dataclass
#[derive(Debug, Clone)]
struct ValueChainAnalysis {
    // Analisa para onde vai cada R$ 1 produzido pelo trabalhador.
    analysis_id: texto;
    scenario: texto;
    mode: FlowMode;
    // Valor produzido
    let total_value: f64 = 100.0 // valor total produzido;
    // Para onde vai (em R$)
    let worker_receives: f64 = 0.0;
    let company_profit: f64 = 0.0 // lucro do dono;
    let company_operational: f64 = 0.0 // custo real de operar;
    let bank_interest: f64 = 0.0 // juros;
    let bank_transaction: f64 = 0.0 // taxas de transacao;
    let bank_spread: f64 = 0.0 // spread cambial;
    let tax: f64 = 0.0 // impostos;
    let intermediary: f64 = 0.0 // intermediarios;
    // Pool coletivo (Republica)
    let collective_pool: f64 = 0.0 // 5% que volta;
    // decorador: @property
    fn worker_pct(self) -> f64 {
        return self.worker_receives / self.total_value * 100;
    // decorador: @property
    fn extracted_pct(self) -> f64 {
...
struct ValueFlowEngine {
    ...
    funcao analyze_capitalism(self, total_value: flutuante = 100.0,
                        let company_surplus_pct: f64 = 0.60,;
                        let bank_interest_pct: f64 = 0.15,;
                        let bank_transaction_pct: f64 = 0.03,;
                        let bank_spread_pct: f64 = 0.02,;
                        let intermediary_pct: f64 = 0.00;
                        ) -> {texto: qualquer}:;
    ...
    fn bank_extraction_detail(self) -> {texto: qualquer} {
    ...
    fn company_extraction_detail(self) -> {texto: qualquer} {
    ...
    fn full_comparison(self, total_value: flutuante = 100.0) -> {texto: qualquer} {
...
// )
    ...
    para cada (k, v) em s.items(): {
        println!("  {k:<30} {v}");
    println!("\n{'='*80}");
    println!("  OpenValueFlow: {s['analises_feitas']} analises.");
    println!("  {s['principio']}");
    println!("{'='*80}");
