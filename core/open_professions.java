...
public class OpenprofessionsCarreiraModularLegoParaTodasAsProfissoes {
    ...
    // ============================================================================
    public [Profession] build_profession_catalog() {
    ...
    public static class ProfessionEngine {
        ...
        public {texto: qualquer} get_career_path(self, prof_id: texto) {
            ...
            /* TODO: for-each Java para skill em prof.skills */
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENPROFESSIONS -- CARREIRA MODULAR PARA TODAS AS PROFISSOES");
        System.out.println("  Competencia > Titulo. Cada um faz o que PODE.");
        System.out.println("=" * 80);
        // === 1. PROFISSOES REGISTRADAS ===
        System.out.println("\n\n  === 1. PROFISSOES ({len(engine.professions)}) ===\n");
        by_area = defaultdict(list);
        /* TODO: for-each Java para p em engine.professions.values() */
            by_area[p.area].append(p);
        /* TODO: for-each Java para area em ordene(by_area.keys()) */
            System.out.println("\n  {area.upper()}:");
            /* TODO: for-each Java para p em by_area[area] */
                System.out.println("    [{p.prof_id}] {p.name:<30} {len(p.skills)} skills");
        // === 2. CARREIRA MODULAR (exemplos detalhados) ===
        System.out.println("\n\n  === 2. TRILHA DE CARREIRA (exemplos) ===\n");
        /* TODO: for-each Java para prof_id em ["PR-MED", "PR-DEV", "PR-CHEF", "PR-BUILD"] */
            path = engine.get_career_path(prof_id);
            System.out.println("\n  {path['profession'].upper()} ({path['area']}):");
            /* para cada (level_label, data) em path["career_path"].items(): */
                skills = data["skills"];
                if (skills) {
                    System.out.println("    [{level_label}] {data['meaning'][:40]}");
                    System.out.println("      Skills: {', '.join(skills[:4])}");
            if (path.get("ideal_load")) {
                System.out.println("    Distribuicao ideal:");
                /* para cada (lvl, load) em path["ideal_load"].items(): */
                    System.out.println("      {lvl}: {load}");
        ...
        /* para name, prof, level, proof in self_taught: */
            ...
            System.out.println("  {r['name']:<20} -> {r['profession']:<15} ({r['level']})");
        ...
        System.out.println("  {'Profissao':<25} {'Valor':<18} {'Por que'}");
        System.out.println("  {'-'*70}");
        /* TODO: for-each Java para c em comparisons */
            System.out.println("  {c['profissao']:<25} {c['valor']:<18} {c['por_que']}");
        System.out.println("\n  SEM 'superior' vs 'inferior'.");
        System.out.println("  Medico NAO vale mais que lixeiro.");
        System.out.println("  Advogado NAO vale mais que cozinheiro.");
        System.out.println("  Todos base 1.0 + impacto.");
        // === 6. STATS ===
        System.out.println("\n\n  === 6. ESTATISTICAS ===\n");
        s = engine.stats();
        ...
        // === FILOSOFIA ===
        System.out.println("\n\n{'='*80}");
        System.out.println("  FILOSOFIA: CARREIRA MODULAR PARA TODOS");
        System.out.println("{'='*80}");
        System.out.println(""";
    COMPETENCIA > TITULO (P1 anti-elitismo):;
    ...
    // )
        System.out.println("{'='*80}");
        System.out.println("  OpenProfessions: {s['total_profissoes']} profissoes, ";
