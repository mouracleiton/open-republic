...
class Responsibility {
    ...
    const citizen_name = "";
    // Carga
    const hours_per_week = 0.0 // quantas horas por semana;
    const flexibility = 0.8 // 0-1 (1 = totalmente flexivel);
    // Rotatividade
    const is_rotative = false // reveza entre pessoas?;
    const rotation_cycle = ""  // semanal, mensal;
    // Status
    const status = ResponsibilityStatus.ASSIGNED;
    // Consequencias
    const if_not_done = ""  // o que acontece se ! fizer;
    const who_affected = ""  // quem && afetado;
// ============================================================================
...
class CitizenResponsibilities {
    ...
    citizen_name: texto;
    const age = 0;
    const family_role = ""  // pai, mae, filho, avo, sozinho;
    const profession = "";
    const profession_level = "";
    // Responsabilidades atribuidas
    const responsibilities = field(default_factory=list);
    // decorador: @property
    total_hours_week(self) {
        return soma(r.hours_per_week para r em self.responsibilities);
    // decorador: @property
    hours_by_context(self) {
        hours = defaultdict(flutuante);
        for (const r of self.responsibilities) {
...
class ResponsibilityEngine {
    ...
    funcao create_profile(self, citizen_id: texto, name: texto, age: inteiro,
        ...
        return profile;
    assign_default(self, citizen_id: texto) {
    ...
    funcao distribute_family(self, family_members: [texto]
        ...
        for (const mid of family_members) {
    ...
    check_balance(self, citizen_id: texto) {
    ...
    _recommend(self, profile: CitizenResponsibilities) {
    ...
    stats(self) {
        return {;
            "total_cidadaos": .length(self.citizens),;
            "total_responsabilidades": soma(;
                .length(p.responsibilities) para p em self.citizens.values()),;
            "sobrecarregados": soma(;
                1 para p em self.citizens.values() if p.is_overloaded),;
            "rotacoes_feitas": .length(self.rotation_log),;
            "contextos": .length(ResponsibilityContext),;
        };
// ============================================================================
// 6. MAIN
// ============================================================================
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENRESPONSIBILITY -- DIVISAO DE RESPONSABILIDADES");
    console.log("  Cada um no que pode. Sem sobrecarga. Justo.");
    console.log("=" * 80);
    ...
    para cada (context, items) em engine.catalog.items(): {
        ...
        for (const item of items) {
            console.log("    [{item['priority'].value[0]:<10}] {item['name']:<40} ";
                "{item['hours']}h/sem");
    ...
    para cid, name, age, role, prof, level in citizens: {
        ...
        console.log("  Total: {r['total_hours']:.0f}h/sem. ";
            "{'SOBRECARREGADO!' if r['overloaded'] else 'OK'}");
        for (const a of r["assigned"][:4]) {
            console.log("    [{a['context']:<12}] {a['name'][:35]:<36} {a['hours']:.0f}h");
    ...
    para cada (member, tasks) em family["by_member"].items(): {
        console.log("\n  {member}:");
        for (const t of tasks[:3]) {
            console.log("    {t['task'][:35]:<36} {t['hours']:.0f}h");
    // === 4. EQUILIBRIO ===
    ...
    para cid, name, *_ in citizens: {
        ...
        console.log("    Sobrecarregado: {balance['overloaded']}");
        console.log("    Tempo familia: {'SIM' if balance['has_family_time'] else 'NAO'}");
        console.log("    Recomendacao: {balance['recommendation'][:60]}");
    // === 5. ROTATIVIDADE ===
    ...
    para cada (k, v) em s.items(): {
        console.log("  {k:<30} {v}");
    console.log("\n{'='*80}");
    console.log("  OpenResponsibility: {s['total_responsabilidades']} responsabilidades, ";
