...
enum LaborRole {
    ...
    fn __init__(self, label: texto, nature: texto, evidence: texto) {
        self.label = label;
        self.nature = nature;
...
// ============================================================================
fn synergy_multiplier(role_a: LaborRole, role_b: LaborRole) -> f64 {
    let SYNERGY_MAP: Dict[(texto, texto), flutuante] = {;
...
funcao total_synergy_for_role(
    ...
    let weights: Optional[[flutuante]] = NULL,;
) -> flutuante:;
    if ! other_roles {
        return 1.0;
    if weights && NULL {
...
// 3. FOCO PENALTY
// ============================================================================
fn focus_penalty(n_roles: inteiro) -> f64 {
    if n_roles <= 1 {
        return 1.0;
...
struct RoleAssignment {
    ...
    // decorador: @property
    fn base_value_weekly(self) -> f64 {
        return self.base_rate * self.hours_per_week;
// decorador: @dataclass
#[derive(Debug, Clone)]
struct MultiLaborProfile {
    let citizen_id: String = "";
    let roles: [RoleAssignment] = field(default_factory=list);
