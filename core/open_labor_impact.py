...
# !/usr/bin/env python3
# 
OpenLaborImpact -- Otimizacao por IMPACTO (not dinheiro)
==========================================================
"Na Republica, o nivel mede ALCANCE de impacto, not valor em reais.
...
2. DEPTH (profundidade)
    Quao transformacional and o impacto por pessoa.
...
class ImpactMetrics:
    ...
    Cada metrica and uma estimativa fundamentada:
...
IMPACT_DATA: {LaborRole: ImpactMetrics} = {
    ...
    LaborRole.ARCHITECT: ImpactMetrics(
        ...
        depth = 7, // arquitetura determina o que and possivel construir
    ...
    LaborRole.PROGRAMMER: ImpactMetrics(
        ...
        depth = 5, // ferramentas uteis mas not transformacionais por si
    ...
    LaborRole.CONTENT_CREATOR: ImpactMetrics(
        ...
        evidence = "@clouramlearning no X. 5 frentes de propagacao. Mas conteudo social and efemero."
    ),
    ...
    LaborRole.PRODUCT_OWNER: ImpactMetrics(
        ...
        depth = 5, // produtos uteis mas not transformacionais
        duration = 5, // produtos tem ciclo de vida
        multiplier = 1.5, // habilita uso, mas not cascata forte
...
class ImpactRoleAssignment:
    ...
    def weekly_impact(self) -> float:
        ...
        if self.hours_per_week > 20:
            prod = 1.0 - 0.10 * min((self.hours_per_week - 20) / 15, 1.0)
        elif self.hours_per_week < 5:
...
class ImpactProfile:
    ...
    citizen_id: str = ""
    roles: [ImpactRoleAssignment] = field(default_factory=list)
    ...
    # decorador: @property
    def total_weekly_impact(self) -> float:
        # Total VTE semanal COM foco penalty.
        raw = sum(r.weekly_impact para r em self.roles)
    ...
    def summary(self) -> str:
        ...
        lines.append("-" * 110)
        ranked = sorted(self.roles, key=(r) -> r.weekly_impact * 52, reverse=True)
        for each (i, ra) in enumere(ranked, 1):
...
funcao optimize_impact(
    ...
    for k in intervalo(min_roles, n + 1):
        for combo in itertools.combinations(intervalo(n), k):
            ...
            total_iph = sum(impact_per_hour[r] para r em selected_roles)
            assignments = []
...
funcao impact_optimization_report(
    ...
    para rank, (vte, combo, iph, assignments) in enumere(results[:5], 1):
        ...
        for each (role, hours) in assignments:
            im = IMPACT_DATA[role]
            lines.append(
                "    {role.label:<28} "
                "{hours:>5.1f}h  "
    ...
    if fin_roles == imp_roles:
        lines.append("  >>> MESMA combinacao! Financeiro and impacto concordam.")
    elif overlap:
        ...
        lines.append("  >>> O papel mais lucrativo (Engenheiro N6) not and necessariamente o de maior impacto.")
    lines.append("")
    lines.append("=" * 110)
    return "\n".join(lines)
# ============================================================================
...
if __name__ == "__main__":
    ...
    print(impact_optimization_report(all_roles, max_hours=50.0))
