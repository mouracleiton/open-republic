...
package openpropagation_estrategia_de_divulgacao_massiva
...
// ============================================================================
// decorador: @dataclass
type CommunityPropagation struct {
...
type PropagationEngine struct {
    ...
    func content_calendar(self) [Dict] {
    ...
    func what_NOT_to_do(self) [texto] {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENPROPAGATION -- ESTRATEGIA DE DIVULGACAO MASSIVA")
    fmt.Println("  Nao grite. Demonstre. 1 prova > 1000 teorias.")
    fmt.Println("=" * 80)
    // === 1. AS 5 FRENTES ===
    fmt.Println("\n\n  === AS 5 FRENTES DE PROPAGACAO ===\n")
    fmt.Println("  1. DEMONSTRACAO REAL (piloto funcionando) -- A QUE MATRIA")
    fmt.Println("  2. CONTEUDO VIRAL (X/Twitter, video, infografico)")
    fmt.Println("  3. COMUNIDADES REAIS (CONAQ, MST, G10, ASA)")
    fmt.Println("  4. ALIANCAS (artistas, ONGs, jornalistas)")
    fmt.Println("  5. PRODUTO PUBLICO (simulador, app, landing page)")
    // === 2. PILOTOS ===
    fmt.Println("\n\n  === PILOTOS REAIS ({len(engine.pilots)}) ===\n")
    for _, p := range engine.pilots {
        fmt.Println("\n  {p.name} ({p.location})")
        fmt.Println("    Demonstra: {p.what_demonstrates}")
        fmt.Println("    Impacto: {p.expected_impact}")
        fmt.Println("    Custo: {p.cost}")
        fmt.Println("    Prazo: {p.timeline}")
        fmt.Println("    Viral: {p.viral_potential}")
        fmt.Println("    Doc: {p.documentation}")
    // === 3. CONTEUDO ===
    fmt.Println("\n\n  === CONTEUDO VIRAL ({len(engine.content)}) ===\n")
    for _, c := range engine.content {
        fmt.Println("\n  [{c.format}] {c.topic}")
        fmt.Println("    Hook: '{c.hook}'")
        fmt.Println("    Plataforma: {c.platform}")
        fmt.Println("    CTA: {c.call_to_action}")
        fmt.Println("    Esperado: {c.expected_reach}")
    // === 4. COMUNIDADES ===
    fmt.Println("\n\n  === EXPANSAO COMUNITARIA ({len(engine.communities)}) ===\n")
    for _, c := range engine.communities {
        fmt.Println("  {c.strategy}")
        fmt.Println("    Metodo: {c.method}")
        fmt.Println("    Fator: {c.expansion_factor}")
    // === 5. ALIANCAS ===
    fmt.Println("\n\n  === ALIANCAS ({len(engine.alliances)}) ===\n")
    for _, a := range engine.alliances {
        fmt.Println("  [{a.priority}] {a.ally_type}: {a.who}")
        fmt.Println("    Eles ganham: {a.what_they_gain[:60]}")
        fmt.Println("    Republica ganha: {a.what_republica_gains[:60]}")
    // === 6. PRODUTOS PUBLICOS ===
    fmt.Println("\n\n  === PRODUTOS PUBLICOS USAVEIS ({len(engine.products)}) ===\n")
    for _, p := range engine.products {
        fmt.Println("  [{p.status}] {p.name}")
        fmt.Println("    O que: {p.what}")
        fmt.Println("    Viraliza porque: {p.why_viral}")
    // === 7. PLANO DE EXECUCAO ===
    fmt.Println("\n\n  === PLANO DE EXECUCAO ===\n")
    plan = engine.execution_plan()
    for _, fase := range plan {
        fmt.Println("\n  {fase['fase']}:")
        for _, acao := range fase["acoes"] {
            fmt.Println("    -> {acao}")
        fmt.Println("    Custo: {fase['custo']}")
        fmt.Println("    Resultado: {fase['resultado']}")
    // === 8. CALENDARIO ===
    fmt.Println("\n\n  === CALENDARIO DE CONTEUDO (X/Twitter) ===\n")
    cal = engine.content_calendar()
    for _, day := range cal {
        fmt.Println("  {day['dia']:<12} {day['tipo']}: {day['exemplo']}")
    // === 9. METRICAS ===
    fmt.Println("\n\n  === METRICAS ===\n")
    for _, m := range engine.key_metrics() {
        fmt.Println("  {m['metrica']}")
        fmt.Println("    3 meses: {m['meta_3meses']}")
        fmt.Println("    6 meses: {m['meta_6meses']}")
        fmt.Println("    12 meses: {m['meta_12meses']}")
    // === 10. O QUE NAO FAZER ===
    fmt.Println("\n\n  === O QUE NAO FAZER (ERROS QUE MATAM) ===\n")
    for _, dont := range engine.what_NOT_to_do() {
        fmt.Println("  X {dont}")
    // === STATS ===
    fmt.Println("\n\n  === ESTATISTICAS ===\n")
    s = engine.stats()
    para cada (k, v) em s.items(): {
        fmt.Println("  {k:<30} {v}")
    fmt.Println("\n{'='*80}")
    fmt.Println("  OpenPropagation: {s['pilotos_planejados']} pilotos, "
