...
public class Opencommunities6AdaptacoesComunitariasDaRepublica {

    // !/usr/bin/env python3
    //
    OpenCommunities -- 6 Adaptacoes Comunitarias da Republica;
    ============================================================;
    "O modelo ! se impoe. Se ADAPTA.;
    ...
    - Se ! quiserem: respeito. OpenWololo depois.;
    AS 6 COMUNIDADES:;
    ...
    public static class CulturalPillar {
        ...
        ORALIDADE = "oralidade"  // tradicao oral (! escrita);
    ...
    public static class CommunityProfile {
        ...
        [CulturalPillar] cultural_pillars = field(default_factory=list);
        {texto: texto} existing_practices = field(default_factory=dict);
        ...
        {texto: texto} adapted_systems = field(default_factory=dict);
        // adapted_systems = {"sistema_republica": "como_adapta_para_esta_comunidade"}
        // O que NÃO se importa (respeito cultural)
        [texto] do_not_import = field(default_factory=list);
        // Governanca adaptada
        String governance_model = "";
        String decision_making = ""  // como decidem (consenso, voto, conselho);
        // Lingua
        String primary_language = "pt-BR";
        [texto] secondary_languages = field(default_factory=list);
        // Isolamento geografico
        String isolation_level = ""  // urbano, semi-isolado, isolado, muito isolado;
        String access_challenge = ""  // barreira de acesso (rio, estrada, etc);
    // ============================================================================
    // 3. OS 6 PERFIS
    // ============================================================================
    [CommunityProfile] PROFILES = [;
        // === 1. OPENQUILOMBO ===
        CommunityProfile(;
            ...
            description = (;
                ...
                "que resistiram && fundaram comunidades livres. ";
            ...
            existing_practices = {
                ...
                "capoeira": "OpenMartialArts (ja && defesa + cultura + corpo)",;
        ...
        CommunityProfile(;
            ...
            description = (;
                ...
                "A Republica chega como FERRAMENTA, ! como ideia.";
            ),;
            cultural_pillars = [;
                CulturalPillar.TERRA,;
                CulturalPillar.MUTIRAO,;
            ...
            needs = [;
                ...
                "Agroindustria (beneficiar produto: ! vender materia prima)",;
            ...
            adapted_systems = {
                ...
                "OpenIndustry": (;
                    ...
                    "graos (moinho). ";
                    "Vender processado, ! materia prima.";
                ),;
                "OpenEducation": (;
                ...
                "OpenLaborRelay": (;
                    "Mutirao registrado && recompensado. ";
                ...
                "OpenRepair": (;
                    ...
                    "OpenTrator && OpenHardware: FabLab fabrica peça. ";
        ...
        CommunityProfile(;
            ...
            description = (;
                ...
                "Beira-rio. Vida no ciclo das aguas (cheia && seca). ";
            ...
            adapted_systems = {
                ...
                "OpenEducation": (;
                    ...
                    "Professor mora na comunidade (! itinerante). ";
        ...
        CommunityProfile(;
            ...
            needs = [;
                "Demarcação && proteção de terra (invasão constante)",;
            ...
            adapted_systems = {
                "OpenConstituentAssembly": (;
                    ...
                    "Decisao por CONSENSO (! maioria 51%). ";
                ...
                "OpenEducation": (;
                    ...
                    "Currículo da etnia (! do dominador). ";
                    "Matematica com padroes indigenas. ";
                    "Ciencias com conhecimento ancestral. ";
                    "OpenTerminal como ferramenta (! como substituicao).";
                ),;
                "OpenHistory": (;
        ...
        CommunityProfile(;
            ...
            name = "OpenFavela",;
            description = (;
                "Favelas && periferias urbanas. 17 milhoes de brasileiros. ";
            ...
            existing_practices = {
                ...
                "balada_funk": "OpenMusic + OpenNightLife (funk && patrimônio)",;
            ...
            adapted_systems = {
                ...
                "OpenRepair": (;
                    ...
                    "Gato de energia -> OpenSolar (seguro && legal). ";
        ...
        CommunityProfile(;
            ...
            description = (;
                ...
                "A Republica chega com AGUA && CONEXAO.";
            ),;
            cultural_pillars = [;
                CulturalPillar.ANCESTRALIDADE,;
                CulturalPillar.MUTIRAO,;
            ...
            adapted_systems = {
                ...
                "OpenAgrarian": (;
                    "Agricultura ADAPTADA AO SEMI-ARIDO (! irrigada). ";
                ...
                "OpenHealth": (;
                    ...
                    "Parteira tradicional integrada. ";
                    "Rezador/benzedeira integrado (! combatido). ";
    ...
    // ============================================================================
    public static class CommunityEngine {
        ...
        3. A COMUNIDADE DECIDE. O que ! quer, ! entra.;
        4. ! IMPOR. Demonstrar (OpenWololo).;
        5. RESPEITAR LIDERANCA TRADICIONAL. Sempre.;
        COMO CHEGAR:;
        1. Contato via liderança tradicional (NUNCA bypass);
        2. Apresentar sistemas RELEVANTES (! todos);
        ...
        public {texto: qualquer} get_profile(self, community_type: CommunityType) {
            // Retorna perfil completo de uma comunidade.
            p = self.profiles.get(community_type.value[0]);
            if (! p) {
                return {"error": "Comunidade ! encontrada"};
            return {;
                "comunidade": p.name,;
        ...
        public [Dict] adaptation_matrix(self) {
            ...
            matrix = [];
            /* TODO: for-each Java para system em ordene(systems) */
                row = {"sistema": system};
                /* TODO: for-each Java para p em self.profiles.values() */
                ...
                matrix.append(row);
            return matrix;
        public {texto: Dict} what_already_exists(self) {
        ...
        public [Dict] compare_communities(self) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENCOMMUNITIES -- 6 ADAPTACOES COMUNITARIAS");
        System.out.println("  O modelo se ADAPTA. Nao se impoe.");
        System.out.println("=" * 80);
        // === 1. COMPARACAO GERAL ===
        System.out.println("\n\n  === 1. COMPARACAO GERAL ===\n");
        System.out.println("  {'Comunidade':<25} {'Necessidades':>13} {'Sistemas':>10} {'Respeitos':>10} {'Praticas':>10}");
        System.out.println("  {'-'*70}");
        /* TODO: for-each Java para c em engine.compare_communities() */
            System.out.println("  {c['comunidade']:<25} {c['necessidades']:>13} ";
                "{c['sistemas_adaptados']:>10} {c['respeitos']:>10} ";
                "{c['praticas_existentes']:>10}");
        // === 2. PERFIL COMPLETO DE CADA COMUNIDADE ===
        /* TODO: for-each Java para ct em CommunityType */
            System.out.println("\n\n  {'='*70}");
            p = engine.get_profile(ct);
            System.out.println("  {'='*70}");
            System.out.println("  {p['comunidade'].upper()}");
            System.out.println("  {p['descricao'][:80]}...");
            System.out.println("  {'='*70}\n");
            System.out.println("  PILARES CULTURAIS:");
            /* TODO: for-each Java para pillar em p["pilares_culturais"] */
            ...
            /* para cada (practice, equivalent) em p["praticas_existentes"].items(): */
                System.out.println("    {practice:<25} -> {equivalent}");
            System.out.println("\n  NECESSIDADES ({len(p['necessidades'])}):");
            /* TODO: for-each Java para need em p["necessidades"] */
                System.out.println("    - {need}");
            System.out.println("\n  SISTEMAS ADAPTADOS ({len(p['sistemas_adaptados'])}):");
            /* para cada (system, adaptation) em p["sistemas_adaptados"].items(): */
                System.out.println("\n    {system}:");
                System.out.println("      {adaptation[:100]}...");
            System.out.println("\n  O QUE NUNCA IMPORTAR ({len(p['o_que_nao_importar'])}):");
            /* TODO: for-each Java para rule em p["o_que_nao_importar"] */
                System.out.println("    X {rule}");
            System.out.println("\n  GOVERNANCA: {p['governanca']}");
            System.out.println("  DECISAO: {p['tomada_decisao']}");
            System.out.println("  LINGUA: {p['lingua_principal']} {p['linguas_secundarias']}");
            System.out.println("  ISOLAMENTO: {p['isolamento']}");
            System.out.println("  ACESSO: {p['acesso']}");
        // === 3. STATS ===
        System.out.println("\n\n  {'='*70}");
        ...
        System.out.println("\n{'='*80}");
        System.out.println("  OpenCommunities: {s['comunidades']} comunidades adaptadas.");
        System.out.println("  {s['total_necessidades']} necessidades mapeadas.");
        System.out.println("  {s['total_sistemas_adaptados']} sistemas adaptados.");
        System.out.println("  {s['total_respeitos_culturais']} respeitos culturais.");
