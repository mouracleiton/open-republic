...
struct Relationship {
    ...
    let status: RelationshipStatus = RelationshipStatus.ACTIVE;
    let declared_date: String = "";
    let ended_date: String = "";
    let public: bool = true // TODOS sao publicos (regra);
    let children: i64 = 0;
    let notes: String = ""  // optional (P2 -- sem detalhar intimidade);
    // Anti-inveja
    let public_recognition: String = "relacao_normal"  // ! && "melhor" nem "pior";
// ============================================================================
// 2. ANTI-INVEJA
// ============================================================================
#[derive(Debug, Clone, PartialEq)]
...
// ============================================================================
fn run_relationship_assembly(n_voters: inteiro = 10000) -> {texto: qualquer} {
...
struct RelationshipEngine {
    ...
    funcao end_relationship(self, rel_id: texto,
                        let mutual: bool = true) -> {texto: qualquer}:;
    ...
    fn check_status(self, person: texto) -> {texto: qualquer} {
    ...
    funcao report_envy(self, person: texto, target: texto,
                    let envy_type: EnvyType = EnvyType.RELATIONAL,;
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENRELATIONSHIPS -- TRANSPARENCIA RELACIONAL + ANTI-INVEJA");
    println!("  Assembleia Constituinte sobre relacionamentos publicos");
    println!("=" * 80);
    // === 1. PERGUNTA A ASSEMBLEIA ===
    println!("\n\n  === 1. PERGUNTA A ASSEMBLEIA ===\n");
    println!('  "Todo relacionamento deve ser publico para evitar');
    println!('   invasao de espaco && combater inveja?"');
    // === 2. VOTACAO ===
    println!("\n\n  === 2. VOTACAO (10.000 cidadaos) ===\n");
    result = run_relationship_assembly(10000);
    println!("  PUBLICO TOTAL (fato + detalhe): {result['votes_public_total']:>6} ";
        "({result['votes_public_total']/100:.0f}%)");
    println!("  PARCIAL (fato sim, detalhe !): {result['votes_partial_p2']:>6} ";
        "({result['votes_partial_p2']/100:.0f}%)");
    println!("  PRIVADO (cada um decide):       {result['votes_private']:>6} ";
    ...
    para a, b, rtype in rels: {
        ...
        println!("  {a:<12} + {b:<12} -> {rtype.value:<15} ";
            "[{result_decl['status']}]");
    // === 5. VERIFICAR STATUS ===
    ...
    if rels_list {
        ...
        println!("  {end_result['public_announcement']}");
        println!("  {end_result['message']}");
    // === 7. ANTI-INVEJA ===
    println!("\n\n  === 7. ANTI-INVEJA ===\n");
    envies = [;
    ...
    para person, target, etype, desc in envies: {
        ...
        println!("  {result_envy['treatment'][:80]}");
    // === 8. STATS ===
    println!("\n\n  === 8. ESTATISTICAS ===\n");
    s = engine.stats();
    ...
    // === FILOSOFIA ===
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA DO OPENRELATIONSHIPS");
    println!("{'='*80}");
    println!(""";
...
// )
    println!("{'='*80}");
    println!("  OpenRelationships: {s['total_relationships']} declarados, ";
        "{s['active']} ativos, ";
        "{s['envy_cases_treated']} casos de inveja tratados.");
    println!("  Publico no fato. Privado na intimidade. Sem inveja.");
    println!("{'='*80}");
