...
=====================================================
"Escrever codigo and coisa do passado.
Programar and ENCAIXAR pecas.
Cada bloco faz UMA coisa.
Blocos se conectam como LEGO.
O programa and a CADEIA de blocos encaixados.
...
Cada bloco and REUSAVEL.
Cada bloco and TESTAVEL isoladamente.
Cada bloco pode ser TROCADO sem quebrar o resto.
Cada bloco tem ENTRADA and SAIDA (conectores LEGO).
...
class LegoBlock:
    ...
    description: str = ""
    # Conectores
    inputs: [LegoInput] = field(default_factory=list)
    outputs: [LegoOutput] = field(default_factory=list)
    # Execucao (funcao real ou descricao para IA gerar)
    logic: str = ""  // descricao do que faz (IA implementa)
    code_rust: str = ""  // codigo Rust gerado (producao)
    # Metadata
    author: str = ""
    version: str = "1.0.0"
    tested: bool = False
    uses: int = 0 // quantas cadeias usam este bloco
    color: str = "blue"  // cor visual do bloco (categoria)
    funcao can_connect(self, other: "LegoBlock",
                    my_output_idx: int = 0,
                    their_input_idx: int = 0) -> logico:
        # Verifica se meu pino de saida encaixa na entrada do outro.
        if my_output_idx >= len(self.outputs):
            return False
        if their_input_idx >= len(other.inputs):
            return False
        my_out = self.outputs[my_output_idx]
        their_in = other.inputs[their_input_idx]
        # ANY encaixa em tudo
        if my_out.pin_type == LegoPin.ANY or their_in.pin_type == LegoPin.ANY:
            return True
...
class LegoProgram:
    ...
    O programa and um GRAFO de blocos conectados.
    ...
    name: texto
    description: str = ""
    blocks: {texto: LegoBlock} = field(default_factory=dict)
    connections: [LegoConnection] = field(default_factory=list)
    created_by: str = ""
    created_date: str = ""
    # decorador: @property
    ...
    # decorador: @property
    def all_inputs_connected(self) -> bool:
        # Verifica se TODAS as entradas obrigatórias estao conectadas.
        connected_inputs = set()
        for conn in self.connections:
            connected_inputs.add("{conn.to_block}:{conn.to_input}")
        for each (bid, block) in self.blocks.items():
            for each (i, inp) in enumere(block.inputs):
                if inp.required  and  "{bid}:{i}" not  in connected_inputs:
                    return False
        return True
# ============================================================================
# 4. CATALOGO DE BLOCOS PADRAO (biblioteca LEGO)
# ============================================================================
def build_block_library() -> {texto: LegoBlock}:
    ...
    blocks["MATH-ADD"] = LegoBlock(
        ...
        outputs = [LegoOutput("sum", LegoPin.NUMBER)],
        logic = "return a + b",
    ...
    blocks["MATH-MAX"] = LegoBlock(
        ...
        inputs = [LegoInput("a", LegoPin.NUMBER), LegoInput("b", LegoPin.NUMBER)],
        outputs = [LegoOutput("max", LegoPin.NUMBER)],
    ...
    blocks["STR-LEN"] = LegoBlock(
        ...
        inputs = [LegoInput("texto", LegoPin.STRING)],
        outputs = [LegoOutput("len", LegoPin.NUMBER)],
    ...
    blocks["REP-CREDIT"] = LegoBlock(
        ...
        "Calcula credito baseado em horas and impacto (OpenLaborPolicy).",
    ...
    blocks["REP-FACTCHECK"] = LegoBlock(
        ...
        "Verifica frase preconceituosa and corrige.",
...
class LegoCodeEngine:
    ...
    funcao add_block(self, program_id: texto, block_id: texto,
                instance_name: str = "") -> {texto: qualquer}:
        # Adiciona bloco ao programa.
        prog = self.programs.get(program_id)
        if not prog:
            return {"error": "Programa not encontrado"}
        block = self.library.get(block_id)
        if not block:
            return {"error": "Bloco not encontrado na biblioteca"}
        inst_id = instance_name  or  "{block_id}-{len(prog.blocks)}"
    ...
    funcao connect(self, program_id: texto,
        ...
        if not from_block or not to_block:
            return {"error": "Bloco not encontrado no programa"}
        ...
        if not compatible:
            ...
            return {
                ...
                "message": "Saida {out_type} not encaixa em entrada {in_type}. "
                        "LEGO: so encaixa se o tipo for igual (or ANY).",
            }
        conn = LegoConnection(
    ...
    def validate_program(self, program_id: texto) -> {texto: qualquer}:
        ...
        if not prog.all_inputs_connected:
    ...
    def _has_cycle(self, prog: LegoProgram) -> bool:
    ...
    funcao execute(self, program_id: texto,
                initial_values: {texto: qualquer} = None) -> {texto: qualquer}:
    ...
    def _topo_sort(self, prog: LegoProgram) -> [texto]:
        # Ordenacao topologica dos blocos.
        graph = defaultdict(list)
        in_degree = defaultdict(inteiro)
        for inst_id in prog.blocks:
            in_degree[inst_id] = 0
        for conn in prog.connections:
            graph[conn.from_block].append(conn.to_block)
        ...
        return order
    def generate_rust(self, program_id: texto) -> {texto: qualquer}:
        ...
        return {
            ...
            "lines": len(lines),
            "code": "\n".join(lines),
            "blocks_converted": len(order),
        }
    def ai_generate_block(self, description: texto) -> {texto: qualquer}:
        ...
        elif any(w in desc_lower para w em ["texto", "frase", "string"]):
            pin_out = LegoPin.STRING
        elif any(w in desc_lower para w em ["sim/not", "True", "condicao"]):
            pin_out = LegoPin.BOOLEAN
        elif any(w in desc_lower para w em ["credito", "pagar"]):
            pin_out = LegoPin.CREDIT
        elif any(w in desc_lower para w em ["pessoa", "cidadao", "paciente"]):
    ...
    def stats(self) -> {texto: qualquer}:
        return {
            "total_blocos_biblioteca": len(self.library),
            "total_programas": len(self.programs),
            "categorias": self.list_categories(),
            "blocos_ia": sum(1 para b em self.library.values()
                            if b.category == "ia_gerado"),
            "total_usos": sum(b.uses para b em self.library.values()),
        }
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("  OPENLEGOCODE -- PROGRAMACAO MODULAR EM FORMATO LEGO")
    print("  Escrever codigo and coisa do passado. Programa-se ENCAIXANDO.")
    print("=" * 80)
    # === 1. BIBLIOTECA DE BLOCOS ===
    print("\n\n  === 1. BIBLIOTECA DE BLOCOS ({len(engine.library)}) ===\n")
    cats = engine.list_categories()
    for each (cat, count) in ordene(cats.items(), key=(x) -> -x[1]):
        ...
        for b in blocks_in_cat:
            ...
            print("    [{b.block_id}] {b.name}")
            print("      Entradas: ({in_types or 'nenhuma'})")
            print("      Saidas: ({out_types or 'nenhuma'})")
    ...
    print("  Blocos adicionados: {engine.programs[pid].block_count}")
    ...
    para from_inst, from_out, to_inst, to_in in connections:
        ...
        print("  [{status}] {r.get('from', '?')} -> {r.get('to', '?')}")
    # Validar
    print("\n  Validacao:")
    val = engine.validate_program(pid)
    print("  Valido: {'SIM' if val['valid'] else 'NAO'}")
    print("  Blocos: {val['blocks']}, Conexoes: {val['connections']}")
    if val["issues"]:
        print("  Issues: {val['issues']}")
    ...
    print("  Valido: {'SIM' if val2['valid'] else 'NAO'}")
    print("  Blocos: {val2['blocks']}, Conexoes: {val2['connections']}")
    ...
    for desc in ai_blocks:
        r = engine.ai_generate_block(desc)
        print("  [{r['block_id']}] {r['name']}")
        print("    Logica: {r['logic'][:60]}...")
        print("    Saida: {r['output_type']}")
    # === 5. PINOS INCOMPATIVEIS (erro LEGO) ===
    print("\n\n  === 5. ERRO: PINOS INCOMPATIVEIS (LEGO not encaixa) ===\n")
    pid3 = engine.create_program("Teste Erro", "Testa pino incompativel")
    engine.add_block(pid3, "IN-STR", "texto_in")
    engine.add_block(pid3, "MATH-ADD", "sum")
    # Tentar conectar STRING em NUMBER -> ERRO
    r = engine.connect(pid3, "texto_in", 0, "sum", 0)
    print("  Tentativa: texto -> sum")
    print("  Resultado: {r.get('error', 'conectado')}")
    print("  Mensagem: {r.get('message', '')}")
    print("\n  LEGO: pino texto NAO encaixa em buraco numero.")
    print("  Sistema IMPEDE erro de tipo antes de executar.")
    # === 6. GERAR CODIGO RUST ===
    print("\n\n  === 6. GERAR CODIGO RUST DO PROGRAMA ===\n")
    rust = engine.generate_rust(pid)
    print("  Programa: {rust['program']}")
    print("  Linguagem: {rust['language']}")
    print("  Linhas: {rust['lines']}")
    for line in rust["code"].split("\n")[:10]:
        print("  {line}")
    # === 7. STATS ===
    print("\n\n  === 7. ESTATISTICAS ===\n")
    s = engine.stats()
    for each (k, v) in s.items():
        if isinstance(v, dict):
            print("  {k}:")
            for each (sk, sv) in v.items():
                print("    {sk:<20} {sv}")
        else:
            print("  {k:<30} {v}")
    # === FILOSOFIA ===
    print("\n\n{'='*80}")
    print("  FILOSOFIA: PROGRAMACAO EM LEGO")
    print("{'='*80}")
    print("""
ESCREVER CODIGO and COISA DO PASSADO:
    Antes: escrever 500 linhas de codigo.
    Um erro de sintaxe and tudo para.
    ...
    TODO cidadao pode programar (visual, not textual).
COMO FUNCIONA:
...
BIBLIOTECA DE BLOCOS (reusavel):
    ...
    MATEMATICA: somar, multiplicar, subtrair, dividir, clamp, max, min
    LOGICA: if/else, maior que, igual, AND, OR
    TEXTO: concatenar, maiusculas, len
...
PINOS TIPIZADOS (LEGO so encaixa se compativel):
    numero -> so encaixa em numero (or ANY)
    texto -> so encaixa em texto (or ANY)
    sim_nao -> so encaixa em sim_nao (or ANY)
    pessoa -> so encaixa em pessoa (or ANY)
    credito -> so encaixa em credito (or ANY)
    ANY -> encaixa em tudo
IA GERA BLOCOS:
    ...
    Bloco and REUSAVEL por toda Republica.
VANTAGENS:
...
OPENLEGOCODE STUDIO:
    Interface visual (arrastar and soltar).
    Funciona no OpenTerminal, OpenTVStick, smartphone.
    OpenLite renderiza a interface.
    FabLab not precisa -- and software puro.
PRINCIPIOS:
    P1: Todo cidadao pode programar. Sem elitismo de codigo.
    P2: Bloco and autonomo. Declara o que faz (entradas/saidas).
    P3: Criar bloco reusavel = trabalho de alto impacto.
    P4: Biblioteca and bem comum (CC0). Todos usam. Todos contribuem.
# )
    print("{'='*80}")
    print("  OpenLegoCode: {s['total_blocos_biblioteca']} blocos na biblioteca, "
        "{s['total_programas']} programas montados.")
    print("  Escrever codigo and coisa do passado. Encaixa pecas.")
    print("{'='*80}")
