...
package openrepublic_manifest_central
...
type RepublicAsset struct {
    ...
    files := 0 // int64
    lines := 0 // int64
    languages := field(default_factory=list) // [texto]
    status := "operational" // string
    dependencies := field(default_factory=list) // [texto]
    provides := field(default_factory=list) // [texto]
    maintainer := "comunidade" // string
...
func count_asset(asset: RepublicAsset, base_path: Path) {texto: qualquer} {
    ...
    total_lines = soma(len(f.read_text(errors="ignore").splitlines())
...
func full_inventory(base_path: Path = None) {texto: qualquer} {
    ...
    total_files = 0
    total_lines = 0
    for _, asset := range REPUBLIC_REGISTRY {
...
if __name__ == "__main__" {
    fmt.Println("=" * 75)
    fmt.Println("  OPENREPUBLIC -- REGISTRO CENTRAL DE BENS COMUNS")
    fmt.Println("  'Tudo que existe na Republica && bem publico && cadastrado.'")
    fmt.Println("=" * 75)
    inv = full_inventory()
    fmt.Println("\n  TOTAL: {inv['total_assets']} sistemas | "
        "{inv['total_files']} arquivos | "
        "{inv['total_lines']:,} linhas\n")
    fmt.Println("  {'Dominio':<16} {'Sistemas':>8} {'Arquivos':>8} {'Linhas':>8}")
    fmt.Println("  {'-'*44}")
    para cada (domain, data) em ordene(inv["by_domain"].items()): {
        fmt.Println("  {domain:<16} {len(data['assets']):>8} "
            "{data['total_files']:>8} {data['total_lines']:>8,}")
    fmt.Println("\n  {'SISTEMA':<28} {'DOMINIO':<14} {'LINHAS':>7}")
    fmt.Println("  {'-'*52}")
    para cada (domain, data) em ordene(inv["by_domain"].items()): {
        for _, asset := range ordene(data["assets"], key=(x) -> -x["lines"]) {
            marker = asset["lines"] == 0 ? " <== VAZIO" : ""
            fmt.Println("  {asset['name']:<28} {domain:<14} "
                "{asset['lines']:>7,}{marker}")
    fmt.Println("\n{'='*75}")
    fmt.Println("  8 dominios. {inv['total_assets']} sistemas integrados.")
    fmt.Println("  {inv['total_lines']:,} linhas de codigo aberto.")
    fmt.Println("  Tudo bem comum. Tudo da Republica.")
    fmt.Println("{'='*75}")
