...
**Arquivo original:** `core/open_labor_optimizer.py`

**Descricao:** ====================================================================
v1 assumia valor linear (mais horas = mais valor proporcional).
REALIDADE: retornos decrescentes. 40h de engenharia nao produzem 4x de 10h.
- Primeiras horas sao super-produtivas (foco profundo)
- Horas adicionais tem produtividade decrescente
- 20h/semana e o pico de produtividade por hora (base: Base 1.0)
...
seja Formula: valor_hora = taxa * sinergia * foco * CURVA(horas)
  CURVA(horas) = 1.0 ate 20h, depois decai
...
funcao productivity_curve(hours: flutuante) -> flutuante:
    ...
    se hours <= 20 entao:
        retorne 1.0
    senao se hours <= 35 entao:
        // Decaimento suave: 20h=1.0, 35h=0.90
        retorne 1.0 - 0.10 * (hours - 20) / 15
    senao se hours <= 50 entao:
        // Decaimento acelerado: 35h=0.90, 50h=0.70
        retorne 0.90 - 0.20 * (hours - 35) / 15
    senao:
        // Alem de 50h: burnout zone
        retorne maximo(0.50, 0.70 - 0.20 * (hours - 50) / 10)


funcao productivity_label(hours: flutuante) -> texto:
    se hours <= 20 entao:
        retorne "PEAK"
    senao se hours <= 35 entao:
        retorne "FADIGA"
...
classe RoleAssignmentV2:
    ...
    // decorador: @property
    funcao productivity_factor(self) -> flutuante:
        retorne productivity_curve(self.hours_per_week)

    // decorador: @property
    funcao effective_rate(self) -> flutuante:
        // Taxa efetiva COM curva de produtividade (sem sinergia/foco).
        retorne self.base_rate * self.productivity_factor

    // decorador: @property
    funcao label_productivity(self) -> texto:
        retorne productivity_label(self.hours_per_week)


// decorador: @dataclass
classe MultiLaborProfileV2:
    seja citizen_id: texto = ""
    seja roles: [RoleAssignmentV2] = field(default_factory=list)
    ...
    funcao role_effective_rate(self, idx: inteiro) -> flutuante:
        ...
        retorne role.base_rate * syn * self.focus_factor * role.productivity_factor
...
funcao redistribute_hours_v2(
    ...
    se base_alloc * n >= max_hours entao:
        retorne [
...
funcao evaluate_combo_v2(
    ...
    retorne (profile.total_weekly_value, profile.total_hours, profile.effective_hourly_rate, selected)


funcao optimize_v2(
    all_roles: [RoleAssignmentV2],
    seja max_hours: flutuante = 50.0,
    seja min_roles: inteiro = 2,
...
se __name__ == "__main__" entao:
    ...
    imprima(optimization_report_v2(all_roles, max_hours=50.0))
