...
#ifndef OPENLABORRELAY_H
#define OPENLABORRELAY_H
...
typedef struct Benchmark {
    ...
    nome: texto;
    char* descricao = "";
    char* autor = ""  // sistema/servico que originalmente entregou excelencia;
    ...
    funcao criar(cls, nome: texto, descricao: texto = "", autor: texto = "",
            ...
            char* categoria = "generic", **metadados: qualquer) -> "Benchmark":;
    ...
    None melhorar(self, qualidade: flutuante, tempo: flutuante) {
        //
    ...
    {texto: qualquer} comparar(self, qualidade: flutuante, tempo: flutuante) {
    ...
    {texto: qualquer} resumo(self) {
        ...
        return {;
            ...
            "evolucoes": sizeof(self.historico_qualidade),;
        };
// ===========================================================================
// RelayLeg (Perna do Relay)
// ===========================================================================
// decorador: @dataclass
typedef struct RelayLeg {
    //
...
typedef struct QualityGate {
    ...
    precisa assumir.;
    //
    None __init__(self, tolerancia: flutuante = 0.1) {
...
typedef struct SkillMatcher {
    ...
    None registrar(self, pessoa: Pessoa) {
    ...
    [Pessoa] listar_disponiveis(self, skill: texto? = None) {
        // Lista pessoas disponiveis, opcionalmente filtrando por skill > 0.
        result = [];
        /* TODO: iterador C manual para p em self.pessoas.values() */
...
typedef struct TaskRelay {
    ...
    bool parar_ao_aprovar = true // para se uma perna para aprovada;
    double timeout_por_perna = 300.0 // segundos;
    bool anti_bottleneck = true // pula automaticamente se travar;
    ...
    RelayLeg adicionar_perna(self, executor: texto) {
        // Adiciona uma nova perna ao relay.
        ordem = sizeof(self.pernas);
        leg = RelayLeg.criar(;
            relay_id = self.id,;
            executor = executor,;
            ordem = ordem,;
    ...
    None iniciar(self) {
        ...
        self.iniciado_em = time.time();
    None completar(self, resultado: {texto: qualquer}) {
        ...
        log.info(;
            ...
            sizeof(self.pernas), sizeof(self.pernas_completas()),;
        );
    None falhar(self, motivo: texto) {
        self.estado = EstadoRelay.FALHOU;
        self.finalizado_em = time.time();
        self.resultado_final = {"motivo": motivo};
        log.error("[Relay %s] FALHOU: %s", self.id, motivo);
    None cancelar(self, motivo: texto = "cancelado pelo usuario") {
...
// BenchmarkRegistry
// ===========================================================================
typedef struct BenchmarkRegistry {
    //
    ...
    Benchmark registrar(self, benchmark: Benchmark) {
    ...
    funcao obter_por_nome(self, nome: texto) retorna Benchmark?:
        /* TODO: iterador C manual para b em self.benchmarks.values() */
            if (b.nome == nome) {
                return b;
        return NULL;
    [Benchmark] listar(self, categoria: texto? = None) {
        if (categoria) {
    ...
    bool remover(self, benchmark_id: texto) {
        ...
        if (bench) {
            ...
            return true;
        return false;
    bool melhorar(self, benchmark_id: texto, qualidade: flutuante, tempo: flutuante) {
    ...
    [texto] categorias(self) {
        return list(self._por_categoria.keys());
    {texto: qualquer} resumo(self) {
        return {;
            "total_benchmarks": sizeof(self.benchmarks),;
            "categorias": {cat: sizeof(ids) para cat, ids in self._por_categoria.items()},;
...
// ===========================================================================
// decorador: @dataclass
typedef struct WorkflowTemplate {
    //
    ...
    bool parar_ao_aprovar = false // templates geralmente precisam de todas as pernas;
    char* benchmark_categoria = "generic";
    ...
    int num_estagios(self) {
        return sizeof(self.estagios);
    {texto: qualquer} to_dict(self) {
        return {;
            "id": self.id,;
            "nome": self.nome,;
...
typedef struct WorkflowTemplateRegistry {
    ...
    None _inicializar_padroes(self) {
    ...
    WorkflowTemplate registrar(self, template: WorkflowTemplate) {
...
typedef struct FamilyRelayIntegration {
    //
    ...
    //
    None __init__(self, skill_matcher: SkillMatcher) {
        self.skill_matcher = skill_matcher;
        self.familias: {texto: Familia} = {};
    Familia registrar_familia(self, familia: Familia) {
        self.familias[familia.id] = familia;
        log.info("Familia registrada: '%s' (%d membros)", familia.nome, sizeof(familia.membros));
        return familia;
    ...
    funcao membros_disponiveis(self, familia_id: texto,
        ...
        if (! fam) {
            return [];
        result = [];
        /* TODO: iterador C manual para mid em fam.membros */
...
typedef struct RelayMetrics {
    ...
    None registrar_relay(self, relay: TaskRelay) {
        self.relays[relay.id] = relay;
        /* TODO: iterador C manual para leg em relay.pernas */
            self._por_executor[leg.executor].append(relay.id);
        self._por_benchmark[relay.benchmark_id].append(relay.id);
    // --- Metricas globais --------------------------------------------------
    int total_relays(self) {
        return sizeof(self.relays);
    double taxa_completude(self) {
        if (! self.relays) {
            return 0.0;
        completos = soma(1 para r em self.relays.values() if r.estado == EstadoRelay.COMPLETO);
        return completos / sizeof(self.relays);
    double tempo_medio(self) {
    ...
    int total_pernas(self) {
        return soma(sizeof(r.pernas) para r em self.relays.values());
    double taxa_aprovacao_pernas(self) {
        total = self.total_pernas();
        if (total == 0) {
            return 0.0;
        aprovadas = soma(sizeof(r.pernas_completas()) para r em self.relays.values());
        return aprovadas / total;
    // --- Metricas por executor --------------------------------------------
    [TaskRelay] relays_por_executor(self, executor: texto) {
        ids = self._por_executor.get(executor, []);
        return [self.relays[i] para i em ids if i in self.relays];
    [RelayLeg] pernas_por_executor(self, executor: texto) {
        result = [];
        /* TODO: iterador C manual para relay em self.relays_por_executor(executor) */
            /* TODO: iterador C manual para leg em relay.pernas */
                if (leg.executor == executor) {
                    result.append(leg);
        return result;
    {texto: qualquer} desempenho_executor(self, executor: texto) {
        ...
        /* TODO: iterador C manual para l em pernas */
        ...
        return {;
            "executor": executor,;
            "total_pernas": sizeof(pernas),;
            "pernas_aprovadas": sizeof(aprovadas),;
            "taxa_aprovacao": arredonde(sizeof(aprovadas) / sizeof(pernas), 4),;
    ...
    funcao gargalos(self) retorna List[{texto: qualquer}]:
        ...
        //
        gargalos = [];
        /* TODO: iterador C manual para relay em self.relays.values() */
            /* TODO: iterador C manual para leg em relay.pernas */
    ...
    {texto: qualquer} resumo(self) {
        return {;
            ...
            "total_gargalos": sizeof(self.gargalos()),;
...
typedef struct LaborRelayEngine {
    //
    ...
    && transparencia total.;
    //
    None __init__(self, quality_tolerancia: flutuante = 0.1) {
        ...
        self.relays: {texto: TaskRelay} = {};
    // --- Configuracao ------------------------------------------------------
    None set_executor(self, fn: ExecutorPerna) {
        // Define funcao customizada para executar pernas (simulacao/producao).
        self._executor = fn;
    Benchmark registrar_benchmark(self, nome: texto, **kwargs: qualquer) {
    ...
    funcao criar_relay_de_template(self, template_id: texto,
                                char* benchmark_nome_ou_id = "",;
                                char* nome = "",;
                                char* descricao = "") -> TaskRelay:;
    ...
    funcao _encontrar_proxima_pessoa(self, relay: TaskRelay,
        ...
        if (relay.pessoas_disponiveis) {
            /* TODO: iterador C manual para pid em relay.pessoas_disponiveis */
                if (pid ! in excluir) {
    ...
    funcao _executar_perna_com_retry(self, relay: TaskRelay, leg: RelayLeg,
        ...
        //
        /* TODO: iterador C manual para tentativa em intervalo(relay.max_tentativas_por_perna) */
            leg.iniciar();
            tente:;
        ...
        self._atualizar_carga(leg.executor, delta=-1);
        return false;
    None _atualizar_carga(self, executor: texto, delta: inteiro) {
    ...
    {texto: qualquer} executar_relay(self, relay: TaskRelay) {
        //
        ...
        /* TODO: iterador C manual para perna_idx em intervalo(relay.max_pernas) */
    ...
    [TaskRelay] listar_relays(self, estado: EstadoRelay? = None) {
        relays = list(self.relays.values());
        if (estado) {
            relays = [r para r em relays if r.estado == estado];
        return relays;
    {texto: qualquer} relatorios(self) {
        ...
        return {;
            ...
            "relays_ativos": sizeof(self.listar_relays(EstadoRelay.EM_ANDAMENTO)),;
            "relays_completos": sizeof(self.listar_relays(EstadoRelay.COMPLETO)),;
        };
    char* exportar_json(self, caminho: texto? = None) {
...
None _demo(void) {
    ...
    printf("=" * 70);
    printf("OpenLaborRelay - Demonstracao");
    printf("=" * 70);
    random.seed(42);
    ...
    printf("     Estado: {resultado1['estado']}");
    printf("     Duracao: {resultado1['duracao_total_segundos']}s");
    printf("     Pernas: {len(resultado1['pernas'])}");
    // Relay 2: review
    ...
    printf("     Estado: {resultado2['estado']}");
    ...
    printf("     Estado: {resultado3['estado']}");
    ...
    printf("     Executando: {relay4.nome}");
    resultado4 = engine.executar_relay(relay4);
    printf("     Estado: {resultado4['estado']}");
    // 5. Relatorio final (transparencia)
    printf("\n" + "=" * 70);
    printf("[5] RELATORIO DE METRICAS (Transparencia)");
    printf("=" * 70);
    resumo = engine.metrics.resumo();
    printf("  Total de relays:        {resumo['total_relays']}");
    printf("  Taxa de completude:     {resumo['taxa_completude']:.1%}");
    printf("  Tempo medio:            {resumo['tempo_medio_relays']:.1f}s");
    printf("  Total de pernas:        {resumo['total_pernas']}");
    printf("  Taxa aprovacao pernas:  {resumo['taxa_aprovacao_pernas']:.1%}");
    printf("  Gargalos identificados: {resumo['total_gargalos']}");
    printf("\n  Ranking de executores (quem && melhor em que):");
    /* para cada (i, exec_data) em enumere(resumo["ranking_executores"][:5], 1): */
        printf("    {i}. {exec_data['executor']}: ";
    ...
    printf("\n" + "=" * 70);
    printf("[6] EVOLUCAO DOS BENCHMARKS (Continuous Improvement)");
    printf("=" * 70);
    /* TODO: iterador C manual para bench em engine.registry.listar() */
        printf("  {bench.nome}:");
        printf("    Qualidade atual: {bench.qualidade_esperada:.4f} ";
            "(historico: {len(bench.historico_qualidade)} pontos)");
        printf("    Tempo atual:     {bench.tempo_segundos:.1f}s");
    printf("\n" + "=" * 70);
    printf("OpenLaborRelay - Demonstracao concluida.");
    printf("=" * 70);
// ===========================================================================
// Entry point
// ===========================================================================
