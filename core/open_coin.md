# OpenCoin -- Sistema que Substitui Bets, Cassinos e Criptos

**Linguagem:** Portugol++ (PPM)

**Arquivo original:** `core/open_coin.py`

**Descricao:** =============================================================
"Na Republica nao ha aposta. Nao ha cassino. Nao ha cripto.
 Ha OpenCoin: a unica moeda que NAO e moeda."
...
O QUE O OPENCOIN FAZ:
  ...
  - Substitui cripto por CREDITO DE ACESSO (que expira, nao acumula)
  - Adrenalina vem de CONQUISTA REAL, nao de perda financeira
...
classe ViceHook herda de Enum:
    ...
    SUNK_COST = "custo_afundado"  // ja investi muito, nao posso parar
...
seja REPLACEMENTS: [ViceReplacement] = [
    ViceReplacement(
        ...
        "A adrenalina vem de jogar de verdade, nao de assistir.",
        "credito + saude",
    ...
    ViceReplacement(
        ...
        "Sem aposta. A recompensa e MELHORAR. Adrenalina de征服 real.",
    ...
    ViceReplacement(
        ...
        [ViceHook.DOPAMINE_UNPREDICTABLE, ViceHook.SOCIAL_PROOF],
        "Sorteio Democratico",
        "Sorteio real onde TODOS tem chance igual (nao proporcional a bilhete). "
    ...
    ViceReplacement(
        ...
        "FabLab, OpenProduct. A recompensa de fabricar e sua.",
    ...
    ViceReplacement(
        ...
        "com dados reais. Pontua por acerto de previsao, nao por dinheiro. "
...
classe CoinTransactionType herda de Enum:
    ...
    EXPIRED = "expirado"  // nao usado no ciclo
    CONVERTED_TO_CREDIT = "convertido"  // virou credito de acesso


// decorador: @dataclass
classe CoinTransaction:
    tx_id: texto
    citizen: texto
    type: CoinTransactionType
    amount: flutuante
    seja description: texto = ""
    seja timestamp: flutuante = field(default_factory=() -> datetime.now().timestamp())
...
classe OpenCoin:
    ...
    - nao acumula indefinidamente (expira)
    - nao e minerada (ganha-se por acao real)
    - nao tem valor especulativo (1 OpenCoin = 1 credito de acesso)
    - nao e transferivel livremente (so em casos especificos)
    - nao pode ser apostada (aposta nao existe)
    - nao gera juros (guardar nao da nada)
    ...
    - nao pode: apostar, especular, acumular, transferir para acumular
    // 

    funcao __init__(self):
        self.balances: {texto: flutuante} = defaultdict(flutuante)
        self.transactions: [CoinTransaction] = []
        self.expiry_cycles_days: inteiro = 90 // expira em 90 dias se nao usar
        self.mining_possible: logico = falso // NUNCA
        self.staking_possible: logico = falso // NUNCA
        self.betting_possible: logico = falso // NUNCA
        self.speculation_possible: logico = falso // NUNCA
    ...
    funcao earn(self, citizen: texto, amount: flutuante,
        ...
        retorne {
            ...
            "note": "Ganho por ACAO REAL. NAO por sorte ou aposta.",
        }

    funcao convert_to_credit(self, citizen: texto, amount: flutuante) -> {texto: qualquer}:
        // Converte OpenCoin em credito de acesso (1:1).
        se self.balances[citizen] < amount entao:
            retorne {"error": "Saldo insuficiente"}
        self.balances[citizen] -= amount
    ...
    funcao attempt_bet(self, citizen: texto, amount: flutuante,
        ...
        replacement = next((r para r em REPLACEMENTS if r.vice == bet_type), nulo)
    ...
    funcao expire_all(self) -> {texto: qualquer}:
        ...
        para cada (citizen, balance) em list(self.balances.items()):
            se balance > 0 entao:
                ...
                self.transactions.append(CoinTransaction(
                    ...
                    amount = balance, description="Expirado (nao usado no ciclo)",
                ))
                self.balances[citizen] = 0
        retorne {
            "expired_total": expired_total,
            "expired_citizens": expired_citizens,
            "note": "OpenCoin NAO acumula. Usar ou perder.",
        }

    funcao balance(self, citizen: texto) -> {texto: qualquer}:
        retorne {
            "citizen": citizen,
            "balance": self.balances[citizen],
...
classe DemocraticLottery:
    ...
    funcao register_all(self, citizens: [texto]) -> None:
        // Todos cidadaos sao automaticamente registrados.
        self.participants = list(citizens)

    funcao draw(self, prize: texto, n_winners: inteiro = 1) -> {texto: qualquer}:
...
se __name__ == "__main__" entao:
    imprima("=" * 75)
    imprima("  OPENCOIN -- A MOEDA QUE NAO E MOEDA")
    imprima("  Substitui bets, cassinos e criptos")
    imprima("=" * 75)
    ...
    imprima("  {'Vicio':<25} {'Substituido por':<30} {'Recompensa'}")
    imprima("  {'-'*75}")
    para cada rep em REPLACEMENTS:
        imprima("  {rep.vice.value:<25} {rep.replacement_activity:<30} "
              "{rep.reward_type}")
    ...
    imprima("  Cleiton: {r1['reason']} +{r1['amount']} (Construiu horta comunitaria)")

    r2 = coin.earn("Ana", 10, CoinTransactionType.SKILL_REWARD,
                   "Venceu torneio de xadrez da comunidade")
    imprima("  Ana: {r2['reason']} +{r2['amount']} (Venceu torneio de xadrez)")

    r3 = coin.earn("Joao", 8, CoinTransactionType.LEARNING_REWARD,
                   "Completou curso de Rust basico")
    imprima("  Joao: {r3['reason']} +{r3['amount']} (Completou curso de Rust)")

    r4 = coin.earn("Maria", 5, CoinTransactionType.CHALLENGE_REWARD,
                   "Organizou mutirao de limpeza do rio")
    imprima("  Maria: {r4['reason']} +{r4['amount']} (Mutirao de limpeza do rio)")

    // === 3. TENTATIVA DE APOSTA (BLOQUEADA) ===
    imprima("\n\n  === 3. TENTATIVA DE APOSTA -> REDIRECIONADA ===\n")

    bet = coin.attempt_bet("Pedro", 20, ViceType.SPORTS_BETING
                           if falso else ViceType.SPORTS_BETTING)
    imprima("  Pedro tenta apostar no jogo:")
    imprima("  Bloqueado: {bet['blocked']}")
    imprima("  Redirecionado para: {bet['redirect']}")
    imprima("  {bet['message']}")

    imprima()
    bet2 = coin.attempt_bet("Carlos", 50, ViceType.CASINO)
    imprima("  Carlos tenta entrar no cassino:")
    imprima("  Redirecionado para: {bet2['redirect']}")
    imprima("  {bet2['message']}")

    imprima()
    bet3 = coin.attempt_bet("Lucas", 100, ViceType.CRYPTO_TRADING)
    imprima("  Lucas tenta especular cripto:")
    imprima("  Redirecionado para: {bet3['redirect']}")

    // === 4. CONVERTER EM CREDITO ===
    imprima("\n\n  === 4. CONVERTENDO EM CREDITO DE ACESSO ===\n")
    conv = coin.convert_to_credit("Cleiton", 10)
    imprima("  Cleiton converte 10 OpenCoin:")
    imprima("  Taxa: {conv['rate']} | Credito recebido: {conv['credit_earned']}")
    imprima("  Saldo restante: {coin.balances['Cleiton']}")
    ...
    imprima("  Premio: {draw['prize']}")
    imprima("  Ganhador: {draw['winners'][0]}")
    imprima("  Participantes: {draw['participants']}")
    imprima("  Custo do bilhete: {draw['ticket_cost']}")
    imprima("  Chance: {draw['chance_per_person']}")

    // === 6. EXPIRACAO ===
    imprima("\n\n  === 6. EXPIRACAO (nao acumula) ===\n")
    exp = coin.expire_all()
    imprima("  Total expirado: {exp['expired_total']}")
    imprima("  Cidadaos afetados: {exp['expired_citizens']}")
    imprima("  {exp['note']}")
  ...
  O QUE O OPENCOIN SUBSTITUI:
    ...
    Bitcoin/Cripto -> OpenCoin (credito que expira, nao acumula)
  ...
  PORQUE ISSO FUNCIONA:
    ...
    Porque nao ha nada a perder. So ha a ganhar.
    e o que ganha é REAL: habilidade, conhecimento, credito, comunidade.

  PRINCIPIO:
    P1: Ninguem lucra com vicio alheio. Anti-elitismo.
    P2: Corpo-mente protegido da exploracao. Autonomia.
    P3: Recompensa por trabalho/habilidade, nao por sorte.
    seja P4: Sorteio democratico = chance igual para todos.
// )
    imprima("{'='*75}")
    imprima("  OpenCoin: a moeda que nao e moeda.")
    imprima("  Zero aposta. Zero cassino. Zero cripto. Zero exploracao.")
    imprima("{'='*75}")
