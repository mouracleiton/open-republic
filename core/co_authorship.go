...
package coauthorship_two_person_rule_para_produ_o
...
type Contributor struct {
    ...
    full_access := false // bool
    can_author := true // bool
    can_review := true // bool
    joined_at := 0.0 // float64
...
type CodeChange struct {
    ...
    author_id := "" // string
    reviewer_id := "" // string
    author_approved := false // bool
    reviewer_approved := false // bool
    status := PRStatus.DRAFT // PRStatus
    comments := field(default_factory=list) // [Dict]
    created_at := 0.0 // float64
    updated_at := 0.0 // float64
    // decorador: @property
    func can_deploy(self) bool {
...
type CoAuthorshipEngine struct {
    ...
    func add_contributor(self, c: Contributor) {
    ...
    funcao create_pr(
        ...
        return pr
    func author_approve(self, change_id: texto, contributor_id: texto) {texto: qualquer} {
    ...
    func reviewer_approve(self, change_id: texto, contributor_id: texto) {texto: qualquer} {
        ...
        pr.updated_at = datetime.now().timestamp()
        return self._check_both_approved(pr)
    func reject(self, change_id: texto, contributor_id: texto, reason: texto) {texto: qualquer} {
    ...
    func add_comment(self, change_id: texto, contributor_id: texto, comment: texto) {
    ...
    func deploy(self, change_id: texto, requested_by: texto) {texto: qualquer} {
    ...
    func _check_both_approved(self, pr: CodeChange) {texto: qualquer} {
    ...
    func status_report(self) string {
        ...
        lines.append("-" * 100)
        for _, pr := range self.prs.values() {
...
if __name__ == "__main__" {
    ...
    fmt.Println("PR-002: MING rejeita -> {r}")
    fmt.Println()
    // Relatório final
    fmt.Println()
    fmt.Println(engine.status_report())
    // Tentar criar PR com mesma pessoa como autor E revisor
    fmt.Println()
    fmt.Println("=" * 100)
    fmt.Println("TESTE: AUTOR = REVISOR (DEVE FALHAR)")
    fmt.Println("=" * 100)
    tente:
        engine.create_pr(
            change_id = "PR-FAIL",
    ...
    fmt.Println()
    fmt.Println("=" * 100)
    fmt.Println("PRINCÍPIO DE COAUTORIA")
    fmt.Println("=" * 100)
    fmt.Println("""
