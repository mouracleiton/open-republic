...
# !/usr/bin/env python3
# 
OpenDualMode -- O Ideal Coletivo and o Executavel Agora
========================================================
...
MODO EXECUTAVEL (O CAMINHO):
    ...
    TODA atividade humana and MERCADORIA.
    TRABALHO and um tipo de mercadoria.
    OpenERP processa. OpenBusinessModel distribui.
    Transicao gradual (OpenTransition 7 fases).
A DIFERENCA:
No IDEAL, trabalho not and mercadoria. and DEVER + IMPACTO.
No EXECUTAVEL, trabalho and mercadoria. Tem preco. and negociado.
...
Porque no mundo REAL, agora, trabalho ja and mercadoria.
Ignorar isso and fingir que o mundo not existe.
A Republica not finge. ENCARA a realidade.
...
class CommodityType(Enum):
    ...
    NO MODO EXECUTAVEL:
    Toda atividade humana que gera valor and MERCADORIA.
    Trabalho and um tipo. Mas not o unico.
    # 
...
class Commodity:
    ...
    NO MODO EXECUTAVEL:
    - Tudo que gera valor and mercadoria.
    - Tem preco. and negociado. and transacionado.
    ...
    description: str = ""
    # Preco no EXECUTAVEL
    price_model: PricingModel = PricingModel.HOURLY
    price_brl: float = 0.0 // preco em dinheiro (transicao)
    price_credit: float = 0.0 // equivalente em credito Republica
    # No IDEAL
    ideal_equivalent: str = ""  // como funciona sem dinheiro
    ideal_credit_per_hour: float = 1.0 // credito base no modo ideal
    # Valor real
    market_value: float = 0.0 // valor de mercado real
    production_cost: float = 0.0 // custo de producao
    fair_price: float = 0.0 // preco justo (sem exploracao)
    # Quem produz
    skill_required: str = ""  // OpenSkills necessaria
    time_to_produce: float = 0.0 // horas para produzir
# ============================================================================
...
COMMODITIES: [Commodity] = [
    ...
    Commodity(
        ...
        "Medico diagnostica and trata.",
        price_model = PricingModel.FIXED, price_brl=200.0, price_credit=1.0,
        ideal_equivalent = "Base 1.0/hora + impacto (vida salva peso max)",
    ...
    Commodity(
        ...
        ideal_equivalent = "Gratis. Conhecimento and bem comum. CC0.",
    ...
    Commodity(
        ...
        price_model = PricingModel.ZERO, price_brl=0.0, price_credit=0.0,
        ideal_equivalent = "Gratis. Saude and direito. Sirio-Libanes.",
        market_value = 500.0, fair_price=0.0,
    ),
    Commodity(
    ...
    Commodity(
        ...
        "Musico cria and distribui musica.",
...
class DualModeEngine:
    ...
    - TRABALHO = contribuicao, not mercadoria.
    ...
    O QUE O MODO EXECUTAVEL FAZ QUE O IDEAL not:
    ...
    O QUE O MODO IDEAL FAZ QUE O EXECUTAVEL not:
    ...
    - IDEAL: credito base 1.0 (quem and da Republica usa credito)
    O SISTEMA ACEITA OS DOIS. Simultaneamente.
    # 
    def __init__(self, current_phase: inteiro = 0):
    ...
    def _calculate_mode_mix(self, phase: inteiro) -> {texto: flutuante}:
    ...
    def price_commodity(self, commodity_id: texto) -> {texto: qualquer}:
        # Precifica mercadoria em ambos os modos.
        c = self.commodities.get(commodity_id)
        if not c:
            return {"error": "Mercadoria not encontrada"}
        return {
            "mercadoria": c.name,
            ...
            "MODELO_IDEAL": {
                ...
                "negociavel": False,
                "moeda": "OpenCredit (expira, not acumula)",
            },
    ...
    funcao transact(self, from_citizen: texto, to_citizen: texto,
        ...
        if mode == OperatingMode.EXECUTAVEL:
            brl = amount_brl is not None ? amount_brl : c.price_brl * hours
            ...
            impact = hours * (1 + len(c.name) % 5) // simplificado
            txn = Transaction(
                transaction_id = tid, from_citizen=from_citizen,
                to_citizen = to_citizen, commodity=c.name,
    ...
    def labor_as_commodity_report(self) -> {texto: qualquer}:
        ...
        return {
            ...
            "modo_ideal": {
                ...
                "exemplos": {
                    ...
                    "medico": "1.0/h + impacto (vidas * peso max)",
            ...
            "transicao": (
                ...
                "{self.mode_mix['ideal']:.0%} ideal. "
                "Conforme avanca, executavel diminui and ideal aumenta."
            ),
        }
    def advance_phase(self) -> {texto: qualquer}:
    ...
    def stats(self) -> {texto: qualquer}:
        exec_txns = sum(1 para t em self.transactions
                        if t.mode == OperatingMode.EXECUTAVEL)
        ideal_txns = sum(1 para t em self.transactions
        ...
        return {
            ...
            "mercadorias_catalogadas": len(self.commodities),
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("  OPENDUALMODE -- O IDEAL COLETIVO E O EXECUTAVEL AGORA")
    print("  Toda atividade humana = MERCADORIA. Trabalho = tipo de mercadoria.")
    print("=" * 80)
    # === 1. OS DOIS MODOS ===
    print("\n\n  === 1. OS DOIS MODOS ===\n")
    print("  MODO IDEAL (destino):")
    print("    Sem dinheiro. Sem propriedade. CC0.")
    print("    Trabalho = base 1.0 + impacto. Credito expira.")
    print("    Tudo gratis. Tudo bem comum.")
    print("")
    print("  MODO EXECUTAVEL (agora):")
    print("    Dinheiro existe. Mercado existe.")
    print("    Toda atividade = MERCADORIA. Trabalho = tipo de mercadoria.")
    print("    OpenERP processa. OpenBusinessModel distribui.")
    print("    Preco justo (sem exploracao). Fair price.")
    # === 2. TRABALHO COMO MERCADORIA ===
    print("\n\n  === 2. TRABALHO COMO MERCADORIA (EXECUTAVEL) ===\n")
    report = engine.labor_as_commodity_report()
    print("  EXECUTAVEL: {report['modo_executavel']['trabalho_e']}")
    for each (job, price) in report["modo_executavel"]["exemplos"].items():
        print("    {job:<25} {price}")
    print("\n  IDEAL: {report['modo_ideal']['trabalho_e']}")
    for each (job, price) in report["modo_ideal"]["exemplos"].items():
        print("    {job:<25} {price}")
    print("\n  {report['transicao']}")
    # === 3. PRECIFICACAO DUAL ===
    print("\n\n  === 3. PRECIFICACAO DUAL (mercadorias) ===\n")
    for cid in ["CMD-L01", "CMD-L02", "CMD-L04", "CMD-C02"]:
        p = engine.price_commodity(cid)
        print("\n  {p['mercadoria']} ({p['tipo']}):")
        print("    EXECUTAVEL: {p['MODELO_EXECUTAVEL']['preco_brl']} "
            "(mercado: {p['MODELO_EXECUTAVEL']['valor_mercado']})")
        print("    IDEAL: {p['MODELO_IDEAL']['credito']} "
    ...
    for each (k, v) in s.items():
        print("  {k:<30} {v}")
    print("\n{'='*80}")
    print("  OpenDualMode: fase {s['fase_transicao']}, "
