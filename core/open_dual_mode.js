...
class Commodity {
    ...
    const description = "";
    // Preco no EXECUTAVEL
    const price_model = PricingModel.HOURLY;
    const price_brl = 0.0 // preco em dinheiro (transicao);
    const price_credit = 0.0 // equivalente em credito Republica;
    // No IDEAL
    const ideal_equivalent = ""  // como funciona sem dinheiro;
    const ideal_credit_per_hour = 1.0 // credito base no modo ideal;
    // Valor real
    const market_value = 0.0 // valor de mercado real;
    const production_cost = 0.0 // custo de producao;
    const fair_price = 0.0 // preco justo (sem exploracao);
    // Quem produz
    const skill_required = ""  // OpenSkills necessaria;
    const time_to_produce = 0.0 // horas para produzir;
// ============================================================================
...
class Transaction {
    ...
    mode: OperatingMode;
    // EXECUTAVEL
    const amount_brl = 0.0;
    const amount_credit = 0.0;
    // IDEAL
...
class DualModeEngine {
    ...
    //
    __init__(self, current_phase: inteiro = 0) {
    ...
    _calculate_mode_mix(self, phase: inteiro) {
    ...
    price_commodity(self, commodity_id: texto) {
    ...
    funcao transact(self, from_citizen: texto, to_citizen: texto,
        ...
        if (mode == OperatingMode.EXECUTAVEL) {
            ...
            impact = hours * (1 + .length(c.name) % 5) // simplificado;
            txn = Transaction(;
                transaction_id = tid, from_citizen=from_citizen,;
                to_citizen = to_citizen, commodity=c.name,;
    ...
    labor_as_commodity_report(self) {
    ...
    stats(self) {
        ...
        return {;
            ...
            "mercadorias_catalogadas": .length(self.commodities),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENDUALMODE -- O IDEAL COLETIVO E O EXECUTAVEL AGORA");
    console.log("  Toda atividade humana = MERCADORIA. Trabalho = tipo de mercadoria.");
    console.log("=" * 80);
    // === 1. OS DOIS MODOS ===
    console.log("\n\n  === 1. OS DOIS MODOS ===\n");
    console.log("  MODO IDEAL (destino):");
    console.log("    Sem dinheiro. Sem propriedade. CC0.");
    console.log("    Trabalho = base 1.0 + impacto. Credito expira.");
    console.log("    Tudo gratis. Tudo bem comum.");
    console.log("");
    console.log("  MODO EXECUTAVEL (agora):");
    console.log("    Dinheiro existe. Mercado existe.");
    console.log("    Toda atividade = MERCADORIA. Trabalho = tipo de mercadoria.");
    console.log("    OpenERP processa. OpenBusinessModel distribui.");
    console.log("    Preco justo (sem exploracao). Fair price.");
    // === 2. TRABALHO COMO MERCADORIA ===
    console.log("\n\n  === 2. TRABALHO COMO MERCADORIA (EXECUTAVEL) ===\n");
    report = engine.labor_as_commodity_report();
    console.log("  EXECUTAVEL: {report['modo_executavel']['trabalho_e']}");
    para cada (job, price) em report["modo_executavel"]["exemplos"].items(): {
        console.log("    {job:<25} {price}");
    ...
    for (const cid of ["CMD-L01", "CMD-L02", "CMD-L04", "CMD-C02"]) {
        p = engine.price_commodity(cid);
        console.log("\n  {p['mercadoria']} ({p['tipo']}):");
        console.log("    EXECUTAVEL: {p['MODELO_EXECUTAVEL']['preco_brl']} ";
            "(mercado: {p['MODELO_EXECUTAVEL']['valor_mercado']})");
        console.log("    IDEAL: {p['MODELO_IDEAL']['credito']} ";
    ...
    for (const _ of intervalo(4)) {
        ...
        console.log("  Fase {r['new_phase']}: {r['mode']} ";
    ...
    para cada (k, v) em s.items(): {
        console.log("  {k:<30} {v}");
    console.log("\n{'='*80}");
    console.log("  OpenDualMode: fase {s['fase_transicao']}, ";
