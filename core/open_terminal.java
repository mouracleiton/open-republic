...
public class OpenterminalTerminaisDaRepublicaEmTodoEstabelecimento {
    ...
    public static class Terminal {
        ...
        String hardware_spec = ""  // CPU, RAM, tela;
        TerminalStatus status = TerminalStatus.AVAILABLE;
        String current_user = ""  // quem esta usando agora;
        double session_start = 0.0 // quando comecou a sessao atual;
        // Capacidades
        boolean has_internet = true;
        boolean has_camera = false // para telemedicina/OpenHealth;
        boolean has_microphone = false // para OpenAudioChannel/voz;
        boolean has_printer = false // para documentos;
        boolean has_scanner = false // para documentos/receitas;
        boolean has_card_reader = false // leitor de cartao (! credito -- ID);
        // Stats
        int total_sessions = 0;
        double total_hours_used = 0.0;
        int total_tasks_completed = 0;
        double total_learning_hours = 0.0;
        // Apps disponiveis neste terminal
        [texto] apps = field(default_factory=list);
        // decorador: @property
        public boolean is_available(self) {
            return self.status == TerminalStatus.AVAILABLE;
        // decorador: @property
        public boolean supports_voice(self) {
            return self.has_microphone;
        // decorador: @property
        public boolean supports_video_call(self) {
            return self.has_camera && self.has_internet;
        public [texto] default_apps(self) {
    ...
    public static class TerminalSession {
        ...
        // O que fez
        [SessionActivity] activities = field(default_factory=list);
        int tasks_completed = 0 // tarefas de OpenLaborRelay;
        int learning_modules = 0 // modulos de OpenEducation;
        int votes_cast = 0 // votacoes;
        double credit_earned = 0.0 // credito ganho;
        [texto] skills_practiced = field(default_factory=list);
        // Privacidade
        boolean data_left_on_terminal = false // SEMPRE false (efemero);
        // decorador: @property
        public boolean was_productive(self) {
        ...
        // decorador: @property
        public double counts_as_work(self) {
    ...
    public static class TerminalRegistry {
        ...
        public {texto: qualquer} register_terminal(self, terminal: Terminal) {
        ...
        funcao find_nearby(self, location: texto,
                        TerminalType? terminal_type = null,;
                        boolean available_only = true) -> [Terminal]:;
            // Encontra terminais proximos disponiveis.
            results = [];
            /* TODO: for-each Java para t em self.terminals.values() */
        ...
        funcao end_session(self, session_id: texto,
                        ...
                        int tasks = 0, learning: inteiro = 0,;
                        int votes = 0, credit: flutuante = 0.0,;
                        [texto] skills = null) -> {texto: qualquer}:;
        ...
        public {texto: qualquer} idle_terminal_report(self) {
    ...
    // ============================================================================
    public static class IdleTimePolicy {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENTERMINAL -- TERMINAIS DA REPUBLICA EM TODO LUGAR");
        System.out.println("  'Toda TV ociosa && um terminal. Todo laborante contribui.'");
        System.out.println("=" * 75);
        ...
        /* TODO: for-each Java para t em terminals_to_register */
            result = registry.register_terminal(t);
            System.out.println("  [{t.terminal_id}] {t.name:<35} {t.terminal_type.value:<15} ";
        ...
        /* para cada (name, has) em checks: */
            ...
            System.out.println("  {name:<25} {status}");
            if (!  result["compliant"]) {
        ...
        System.out.println("  CENARIO: Maria esta na sala de espera do hospital.");
        System.out.println("  Fila de 45 min. Terminal T-002 disponivel.\n");
        s1 = registry.start_session("T-002", "L-001", "Maria Silva");
        System.out.println("  {s1['message']}");
        System.out.println("  Apps: {', '.join(s1['apps_available'][:6])}");
        ...
        System.out.println("\n  {end1['message']}");
        System.out.println("  Tempo: {end1['duration']}");
        System.out.println("  Produtivo: {'sim' if end1['productive'] else '!'}");
        System.out.println("  Dados no terminal: {end1['data_left']}");
        ...
        System.out.println("  {end2['message']}");
        ...
        System.out.println("  {end3['message']}");
        // === 4. CENARIOS DE TEMPO OCIOSO ===
        System.out.println("\n\n  === 4. TEMPO OCIOSO = OPORTUNIDADE ===\n");
        System.out.println("  {'Local':<35} {'Tempo':>12} {'Terminal':<20} {'Melhor uso'}");
        System.out.println("  {'-'*90}");
        /* TODO: for-each Java para sc em policy.SCENARIOS */
            term = sc.get("terminal", "?");
            uso = sc.get("melhor_uso", "?")[:30];
            System.out.println("  {sc['local']:<35} {sc['tempo_disponivel']:>12} ";
                "{term:<20} {uso}");
        // === 5. TERMINAIS PROXIMOS ===
        System.out.println("\n\n  === 5. ENCONTRAR TERMINAL PROXIMO ===\n");
        nearby = registry.find_nearby("Centro");
        System.out.println("  Busca 'Centro': {len(nearby)} terminais");
        /* TODO: for-each Java para t em nearby */
            status = t.is_available ? "LIVRE" : "OCUPADO";
            System.out.println("    [{t.terminal_id}] {t.name:<35} {status}");
        ...
        // === FILOSOFIA ===
        System.out.println("\n\n{'='*75}");
        System.out.println("  FILOSOFIA DO OPENTERMINAL");
        System.out.println("{'='*75}");
        System.out.println(""";
    ...
    // )
        System.out.println("{'='*75}");
        System.out.println("  OpenTerminal: {s['total_terminals']} terminais, ";
            "{s['total_sessions']} sessoes, ";
            "{s['total_hours_used']:.0f}h usadas.");
        System.out.println("  Toda tela ociosa && um terminal da Republica.");
        System.out.println("{'='*75}");
}
