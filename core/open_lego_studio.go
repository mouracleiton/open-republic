...
package openlegostudio_interface_visual_de_programacao_lego
...
type VisualBlock struct {
    ...
    color: BlockColor
    shape: BlockShape
    x := 0 // posicao no canvas // int64
    y := 0 // int64
    width := 20 // largura visual // int64
    height := 5 // altura visual // int64
    // Pinos
    input_pins := field(default_factory=list)  // [{"name", "type", "connected"}] // [Dict]
    output_pins := field(default_factory=list) // [Dict]
    // Estado
    executing := false // bool
    has_error := false // bool
    error_msg := "" // string
    result_value := nil // interface{}
    // decorador: @property
    func center_x(self) int64 {
        return self.x + self.width // 2
    // decorador: @property
    func center_y(self) int64 {
...
type VisualConnection struct {
    ...
    to_pin_idx: inteiro
    valid := true // bool
    color := "verde"  // verde=valido, vermelho=invalido // string
    // Renderizacao
    wire_path := field(default_factory=list) // List[(inteiro, inteiro)]
// ============================================================================
...
type Canvas struct {
    ...
    width := 80 // int64
    height := 40 // int64
    grid := field(default_factory=list) // List[[texto]]
    blocks := field(default_factory=dict) // {texto: VisualBlock}
    connections := field(default_factory=list) // [VisualConnection]
    cursor_x := 0 // int64
    cursor_y := 0 // int64
    selected_block := nil // texto?
    dragging_block := nil // texto?
    snap_target := CanvasSlot.EMPTY // CanvasSlot?
    func __post_init__(self) {
        self.grid = [[" " para _ em intervalo(self.width)] para _ em intervalo(self.height)]
    func place_block(self, block: VisualBlock) None {
    ...
    func render(self) string {
        ...
        for _, block := range self.blocks.values() {
        ...
        lines.append("+" + "-" * self.width + "+")
        return "\n".join(lines)
    func _render_block(self, block: VisualBlock) None {
        ...
        if b.y >= self.height || b.x >= self.width {
            return nil
        // Moldura superior
        for _, i := range intervalo(minimo(b.width, self.width - b.x)) {
        ...
        if block.executing {
            if b.y + 1 < self.height && b.x < self.width {
                self.grid[b.y + 1][b.x] = ">"
    func _render_wire(self, conn: VisualConnection) None {
...
type BlockPalette struct {
    ...
    //
    categories := field(default_factory=dict) // Dict[texto, [Dict]]
    func render(self) string {
        ...
        return "\n".join(lines)
func build_palette() BlockPalette {
...
type LegoAssistant struct {
    ...
    func understand(self, description: texto) {texto: qualquer} {
    ...
    func auto_connect(self, template: [texto]) [Dict] {
        // Gera conexoes automaticas baseado no template.
        connections = []
        for _, i := range intervalo(len(template) - 1) {
            connections.append({
                "from": template[i],
                "from_pin": 0,
                "to": template[i + 1],
...
type GalleryProgram struct {
    ...
    author: texto
    description := "" // string
    blocks := field(default_factory=list) // [texto]
    downloads := 0 // int64
    rating := 0.0 // float64
func build_gallery() [GalleryProgram] {
    return [
...
type LegoStudio struct {
    ...
    func new_program(self, name: texto) {texto: qualquer} {
        // Cria novo programa LEGO.
        self.current_program = name
        self.canvas = Canvas(width=80, height=36)
        return {"created": true, "program": name, "message": "Novo programa: {name}"}
    func drag_drop_block(self, block_id: texto, x: inteiro, y: inteiro) {texto: qualquer} {
    ...
    funcao draw_wire(self, from_inst: texto, from_pin: inteiro,
                ...
                valid := true) -> {texto: qualquer}: // bool
        ...
        if from_inst in self.canvas.blocks {
            for _, p := range self.canvas.blocks[from_inst].output_pins {
                p["connected"] = true
        if to_inst in self.canvas.blocks {
            for _, p := range self.canvas.blocks[to_inst].input_pins {
                p["connected"] = true
        return {
            "wired": true,
            "from": from_inst,
    ...
    func validate(self) {texto: qualquer} {
    ...
    func execute(self) {texto: qualquer} {
    ...
    func ai_assemble(self, description: texto) {texto: qualquer} {
...
if __name__ == "__main__" {
    studio = LegoStudio()
    fmt.Println("=" * 80)
    fmt.Println("  OPENLEGOSTUDIO -- INTERFACE VISUAL DE PROGRAMACAO LEGO")
    fmt.Println("  Arrastar. Soltar. Encaixar. Sem texto. Sem sintaxe.")
    fmt.Println("=" * 80)
    // === 1. IA MONTA PROGRAMA POR DESCRICAO ===
    fmt.Println("\n\n  === 1. IA MONTA PROGRAMA ===\n")
    fmt.Println("  Cidadao diz: 'Quero calcular credito de trabalho'\n")
    r = studio.ai_assemble("calcular credito de trabalho baseado em horas")
    fmt.Println("  IA: {r['message']}")
    fmt.Println("  Blocos colocados: {r['blocks_placed']}")
    fmt.Println("  Conexoes feitas: {r['connections_made']}")
    // === 2. RENDERIZAR CANVAS ===
    fmt.Println("\n\n  === 2. CANVAS (ASCII art) ===\n")
    ...
    fmt.Println("  Arrastando blocos da paleta:")
    studio.drag_drop_block("IN-STR", 2, 8)
    fmt.Println("    [OK] IN-STR em (2, 8)")
    studio.drag_drop_block("REP-FACTCHECK", 24, 8)
    fmt.Println("    [OK] REP-FACTCHECK em (24, 8)")
    studio.drag_drop_block("OUT", 50, 8)
    fmt.Println("    [OK] OUT em (50, 8)")
    fmt.Println("\n  Conectando:")
    blocks = list(studio.canvas.blocks.keys())
    r1 = studio.draw_wire(blocks[0], 0, blocks[1], 0)
    fmt.Println("    {r1['from']} --> {r1['to']} ({r1['color']})")
    r2 = studio.draw_wire(blocks[1], 0, blocks[2], 0)
    fmt.Println("    {r2['from']} --> {r2['to']} ({r2['color']})")
    // === 4. RENDERIZAR ===
    fmt.Println("\n\n  === 4. CANVAS DO FACT-CHECK ===\n")
    fmt.Println(studio.render())
    // === 5. VALIDAR E EXECUTAR ===
    fmt.Println("\n\n  === 5. VALIDAR E EXECUTAR ===\n")
    val = studio.validate()
    fmt.Println("  Valido: {'SIM' if val['valid'] else 'NAO'}")
    fmt.Println("  Pronto: {'SIM' if val['ready_to_run'] else 'NAO'}")
    if val["ready_to_run"] {
        exec_result = studio.execute()
        fmt.Println("  {exec_result['message']}")
        for _, log := range exec_result["log"] {
            fmt.Println("    {log}")
    ...
    fmt.Println("  Visual: cidadao ve o erro SEM precisar ler codigo.")
    // === 7. GALERIA COMUNITARIA ===
    fmt.Println("\n\n  === 7. GALERIA COMUNITARIA ===\n")
    for _, prog := range studio.gallery {
        stars = "*" * inteiro(prog.rating)
        fmt.Println("  [{prog.program_id}] {prog.name:<30} por {prog.author:<12} "
            "{prog.downloads:>4} downloads  {stars} ({prog.rating})")
        fmt.Println("    {prog.description}")
        fmt.Println("    Blocos: {' -> '.join(prog.blocks)}")
    ...
    for _, q := range queries {
        ...
        fmt.Println("  '{q}'")
        fmt.Println("    -> IA entendeu. {result['block_count']} blocos.")
        fmt.Println("    -> Cadeia: {' -> '.join(result['template'])}")
        fmt.Println()
    // === FILOSOFIA ===
    fmt.Println("\n{'='*80}")
    fmt.Println("  FILOSOFIA DO OPENLEGOSTUDIO")
    fmt.Println("{'='*80}")
    fmt.Println("""
...
PRINCIPIOS:
    ...
    P3 := trabalho de alto impacto. // Criar bloco reusavel
    P4: Galeria && bem comum. Todos usam. Todos contribuem.
// )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenLegoStudio: interface visual para programar encaixando pecas.")
    fmt.Println("  Sem texto. Sem sintaxe. Sem erro. So LEGO.")
    fmt.Println("{'='*80}")
