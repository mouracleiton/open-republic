...
struct Proposition {
    ...
    let arguments_for: [texto] = field(default_factory=list);
    let arguments_against: [texto] = field(default_factory=list);
    // Resultado
    let votes: {qualquer: inteiro} = field(default_factory=dict);
    let winner: qualquer? = NULL;
    let turnout: i64 = 0;
...
struct Constituent {
    ...
    age: inteiro;
    let is_founder: bool = false;
    fn vote_on(self, prop: Proposition) -> dyn std::fmt::Debug {
    ...
    fn _demographic_vote(self, prop: Proposition) -> dyn std::fmt::Debug {
...
struct ConstituentAssembly {
    ...
    fn populate(self, n: inteiro = 10000) -> None {
    ...
    fn _init_propositions(self) -> None {
    ...
    fn results_report(self) -> String {
        ...
        lines.append("  {'='*80}");
        return "\n".join(lines);
    fn constitution_text(self) -> String {
...
if __name__ == "__main__" {
    println!("=" * 80);
    println!("  CONVOCAÇÃO DA ASSEMBLEIA CONSTITUINTE");
    println!("=" * 80);
    println!(""";
O FUNDADOR DECLARA:;
...
// )
    ...
    println!("  População convocada: {len(assembly.constituents):,} cidadaos");
    println!("  Propostas a votar: {len(assembly.propositions)}");
    println!("  Demográficos representados: {len(set(c.demographic for c in assembly.constituents))}");
    ...
    para cada (demo, count) em ordene(demo_dist.items(), key=(x) -> -x[1]): {
        ...
        println!("    {demo:<20} {count:>5,} ({pct:>5.1f}%)");
    // Votação
    println!("\n\n  VOTAÇÃO EM ANDAMENTO...\n");
    assembly.run_election();
    // Relatório
    println!(assembly.results_report());
    // Constituição final
    println!("\n\n");
    println!(assembly.constitution_text());
    // Reflexão final
