...
#ifndef OPENRESPONSIBILITY_DIVISAO_E_ESTABELECIMENTO_DE_RESPONSABILIDADES_H
#define OPENRESPONSIBILITY_DIVISAO_E_ESTABELECIMENTO_DE_RESPONSABILIDADES_H
...
// ============================================================================
typedef struct ResponsibilityContext {
...
typedef struct ResponsibilityPriority {
...
typedef struct ResponsibilityStatus {
...
typedef struct Responsibility {
    ...
    char* citizen_name = "";
    // Carga
    double hours_per_week = 0.0 // quantas horas por semana;
    double flexibility = 0.8 // 0-1 (1 = totalmente flexivel);
    // Rotatividade
    bool is_rotative = false // reveza entre pessoas?;
    char* rotation_cycle = ""  // semanal, mensal;
    // Status
    ResponsibilityStatus status = ResponsibilityStatus.ASSIGNED;
    // Consequencias
    char* if_not_done = ""  // o que acontece se ! fizer;
    char* who_affected = ""  // quem && afetado;
// ============================================================================
...
// ============================================================================
// decorador: @dataclass
typedef struct CitizenResponsibilities {
    // Perfil completo de responsabilidades de um cidadao.
    citizen_id: texto;
    citizen_name: texto;
    int age = 0;
    char* family_role = ""  // pai, mae, filho, avo, sozinho;
    char* profession = "";
    char* profession_level = "";
    // Responsabilidades atribuidas
    [Responsibility] responsibilities = field(default_factory=list);
    // decorador: @property
    double total_hours_week(self) {
        return soma(r.hours_per_week para r em self.responsibilities);
    // decorador: @property
    {texto: flutuante} hours_by_context(self) {
        hours = defaultdict(flutuante);
        /* TODO: iterador C manual para r em self.responsibilities */
...
// ============================================================================
typedef struct ResponsibilityEngine {
    ...
    funcao create_profile(self, citizen_id: texto, name: texto, age: inteiro,
        ...
        return profile;
    {texto: qualquer} assign_default(self, citizen_id: texto) {
        ...
        /* para cada (context, items) em self.catalog.items(): */
            /* TODO: iterador C manual para item em items */
    ...
    funcao distribute_family(self, family_members: [texto]
        ...
        /* TODO: iterador C manual para mid em family_members */
            profile = self.citizens.get(mid);
            if (! profile) {
                continue;
            /* TODO: iterador C manual para r em profile.responsibilities */
                if (r.context == ResponsibilityContext.FAMILY) {
    ...
    {texto: qualquer} check_balance(self, citizen_id: texto) {
    ...
    char* _recommend(self, profile: CitizenResponsibilities) {
    ...
    {texto: qualquer} stats(self) {
        return {;
            "total_cidadaos": sizeof(self.citizens),;
            "total_responsabilidades": soma(;
                sizeof(p.responsibilities) para p em self.citizens.values()),;
            "sobrecarregados": soma(;
                1 para p em self.citizens.values() if p.is_overloaded),;
            "rotacoes_feitas": sizeof(self.rotation_log),;
            "contextos": sizeof(ResponsibilityContext),;
        };
// ============================================================================
// 6. MAIN
// ============================================================================
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENRESPONSIBILITY -- DIVISAO DE RESPONSABILIDADES");
    printf("  Cada um no que pode. Sem sobrecarga. Justo.");
    printf("=" * 80);
    ...
    /* para cada (context, items) em engine.catalog.items(): */
        ...
        /* TODO: iterador C manual para item em items */
            printf("    [{item['priority'].value[0]:<10}] {item['name']:<40} ";
                "{item['hours']}h/sem");
    ...
    /* para cid, name, age, role, prof, level in citizens: */
        ...
        printf("  Total: {r['total_hours']:.0f}h/sem. ";
            "{'SOBRECARREGADO!' if r['overloaded'] else 'OK'}");
        /* TODO: iterador C manual para a em r["assigned"][:4] */
            printf("    [{a['context']:<12}] {a['name'][:35]:<36} {a['hours']:.0f}h");
    // === 3. DISTRIBUICAO FAMILIAR ===
    printf("\n\n  === 3. DISTRIBUICAO FAMILIAR (OpenFamilyLabor) ===\n");
    family = engine.distribute_family(["C-001", "C-002", "C-003", "C-004"]);
    printf("  {family['message']}");
    /* para cada (member, tasks) em family["by_member"].items(): */
        printf("\n  {member}:");
        /* TODO: iterador C manual para t em tasks[:3] */
            printf("    {t['task'][:35]:<36} {t['hours']:.0f}h");
    // === 4. EQUILIBRIO ===
    ...
    /* para cid, name, *_ in citizens: */
        ...
        printf("    Sobrecarregado: {balance['overloaded']}");
        printf("    Tempo familia: {'SIM' if balance['has_family_time'] else 'NAO'}");
        printf("    Recomendacao: {balance['recommendation'][:60]}");
    // === 5. ROTATIVIDADE ===
    ...
    /* para cada (k, v) em s.items(): */
        printf("  {k:<30} {v}");
    printf("\n{'='*80}");
    printf("  OpenResponsibility: {s['total_responsabilidades']} responsabilidades, ";
        "{s['sobrecarregados']} sobrecarregados.");
    printf("  Cada um no que pode. Sem sobrecarga. Justo.");
    printf("{'='*80}");

#endif // OPENRESPONSIBILITY_DIVISAO_E_ESTABELECIMENTO_DE_RESPONSABILIDADES_H
