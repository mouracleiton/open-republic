...
package openpenalrevision_esvaziar_o_sistema_prisional_transformar_em_forca_produtiva
...
// decorador: @dataclass
type PenalCase struct {
    // Um caso carcerario para revisao.
    case_id: texto
    citizen_id: texto
    citizen_name: texto
    age := 0 // int64
    crime_category := CrimeCategory.PETTY_THEFT_FOOD // CrimeCategory
    crime_description := "" // string
    original_sentence_years := 0.0 // float64
    years_served := 0.0 // float64
    victim_id := "" // string
    victim_name := "" // string
    victim_reparation_due := 0.0 // em horas de trabalho // float64
    // Contexto (Causa raiz)
    poverty_driven := false // crime por necessidade economica // bool
    addiction_driven := false // crime por dependencia quimica // bool
    mental_health_issue := false // saude mental // bool
    first_offense := true // bool
    education_level := "basico" // string
    // Revisao
    severity := CrimeSeverity.NOT_CRIME // CrimeSeverity
    status := SentenceStatus.PENDING_REVIEW // SentenceStatus
    new_plan := ""  // plano de transformacao // string
    work_hours_required := 0.0 // trabalho para reparacao // float64
    skills_to_learn := field(default_factory=list) // [texto]
    treatment_required := "" // string
    review_date := "" // string
    reviewer_id := "" // string
    // decorador: @property
    func should_release_immediately(self) bool {
        // Deve ser libertado AGORA?
        return self.severity == CrimeSeverity.NOT_CRIME
    // decorador: @property
    func is_productive_eligible(self) bool {
        // Pode ser convertido em forca produtiva?
        return self.severity in (CrimeSeverity.LOW, CrimeSeverity.MEDIUM,
                                CrimeSeverity.HIGH)
    // decorador: @property
    func is_permanently_restricted(self) bool {
        // Crimes irrecuperaveis (hediondos)?
        return self.severity == CrimeSeverity.HEDIONDOUS
// ============================================================================
// 3. MOTOR DE REVISAO PENAL
// ============================================================================
type PenalRevisionEngine struct {
    ...
    func submit_case(self, case: PenalCase) None {
        self.cases[case.case_id] = case
    func review(self, case_id: texto, reviewer_id: texto) {texto: qualquer} {
    ...
    func _release_immediate(self, case: PenalCase) {texto: qualquer} {
    ...
    func _convert_to_productive(self, case: PenalCase) {texto: qualquer} {
    ...
    func _restrict_permanent(self, case: PenalCase) {texto: qualquer} {
    ...
    func _calculate_reparation_hours(self, case: PenalCase) float64 {
    ...
    func _assign_skills(self, case: PenalCase) [texto] {
    ...
    func batch_review(self, reviewer_id: texto = "coletivo") {texto: qualquer} {
...
type ProductiveForce struct {
    ...
    func __init__(self) {
        ...
        self.work_log: [Dict] = []
    func enroll(self, case: PenalCase) {texto: qualquer} {
    ...
    funcao log_work(self, case_id: texto, hours: flutuante,
        ...
        return entry
    func reintegrate(self, case_id: texto) {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENPENALREVISION")
    fmt.Println("  Esvaziar Prisoes, Transformar em Forca Produtiva")
    fmt.Println("=" * 80)
    ...
    fmt.Println("  {'ID':<8} {'Nome':<18} {'Crime':<20} {'Sentenca':>8} {'Cumprida'}")
    fmt.Println("  {'-'*70}")
    for _, c := range engine.cases.values() {
        fmt.Println("  {c.case_id:<8} {c.citizen_name:<18} "
    ...
    fmt.Println("  Total revisado:           {results['total_reviewed']}")
    fmt.Println("  LIBERTAR IMEDIATAMENTE:   {results['released_immediately']}")
    fmt.Println("  Convertidos a trabalho:   {results['converted_to_productive']}")
    fmt.Println("  Restricao permanente:     {results['permanently_restricted']}")
    fmt.Println("  Horas de trabalho geradas:{results['work_hours_generated']:,.0f}h")
    fmt.Println("\n  ANTES: {results['prison_population_before']} presos")
    fmt.Println("  DEPOIS: {results['prison_population_after']} presos (so hediondos)")
    fmt.Println("  ESVAZIAMENTO: {results['emptying_rate']}")
    // === 3. DETALHES DA REVISAO ===
    fmt.Println("\n\n  === 3. DETALHES POR CASO ===\n")
    for _, c := range engine.cases.values() {
        fmt.Println("\n  [{c.case_id}] {c.citizen_name} (idade {c.age})")
        fmt.Println("  Crime: {c.crime_description}")
        fmt.Println("  Severidade: {c.severity.name}")
        if c.status == SentenceStatus.OVERTURNED {
            fmt.Println("  ACAO: LIBERTAR IMEDIATAMENTE")
            fmt.Println("  Motivo: {c.crime_category.value} ! && crime na Republica")
            fmt.Println("  Apoio: moradia + comida + tratamento + oficio")
        } else if c.status == SentenceStatus.CONVERTED_TO_WORK {
            fmt.Println("  ACAO: CONVERTIDO EM FORCA PRODUTIVA")
            fmt.Println("  Trabalho: {c.work_hours_required:.0f}h "
                "({c.work_hours_required/920:.1f} anos)")
            fmt.Println("  Skills: {', '.join(c.skills_to_learn[:4])}")
            if c.treatment_required {
                fmt.Println("  Tratamento: {c.treatment_required}")
            fmt.Println("  Vitima: reparada com trabalho")
        } else if c.status == SentenceStatus.RESTRICTED {
            fmt.Println("  ACAO: RESTRICAO PERMANENTE (hediondo)")
            fmt.Println("  Direitos mantidos: sim (sem tortura)")
    ...
    for _, c := range engine.cases.values() {
        if c.status == SentenceStatus.CONVERTED_TO_WORK {
            ...
            if result.get("completed") {
                fmt.Println("  {result['citizen']}: CONCLUIU! {result['progress']}")
    // Mostrar progresso
    fmt.Println("\n  Progresso dos transformados:")
    fmt.Println("  {'Nome':<18} {'Progresso':>10} {'Horas':>8} {'Status'}")
    fmt.Println("  {'-'*55}")
    for _, record := range force.transformed.values() {
        fmt.Println("  {record['citizen']:<18} {record['progress']:>9.0f}% "
    ...
    para cada (cid, record) em list(force.transformed.items()): {
        if record["progress"] >= 100 {
            ...
            fmt.Println("  {r['citizen']}: {r['status']}")
            fmt.Println("  Skills: {r['skills_learned']}")
    ...
    fmt.Println("  ANTES (sistema tradicional):")
    fmt.Println("    Custo/prsao/ano: R$ {custo_tradicional:,}")
    fmt.Println("    Producao dos presos: R$ 0")
    fmt.Println("    Reincidencia: ~70%")
    fmt.Println("    Custo social: ALTO")
    fmt.Println("\n  DEPOIS (Republica):")
    fmt.Println("    Custo/prsao/ano: R$ {custo_republica:,} (ZERO)")
    fmt.Println("    Producao dos transformados: R$ {valor_trabalho:,}")
    fmt.Println("    Reincidencia estimada: <20% (tem oficio)")
    fmt.Println("    GANHO social: ALTO")
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  FILOSOFIA DO OPENPENALREVISION")
    fmt.Println("{'='*80}")
    fmt.Println("""
...
// )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenPenalRevision: {results['total_reviewed']} casos revisados.")
    fmt.Println("  {results['released_immediately']} libertados. "
        "{results['converted_to_productive']} transformados. "
        "{results['permanently_restricted']} restritos.")
    fmt.Println("  Prisao esvaziada. Forca produtiva criada.")
    fmt.Println("{'='*80}")
