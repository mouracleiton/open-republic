...
migracao livre, resolucão de conflitos entre nacoes, and governanca federal
liquida.
"Varias OpenNations, uma OpenRepublic."
Cada nacao mantem autonomia cultural and decisoes locais.
A Republica coordena o que and transnacional:
- Infraestrutura continental (energia, transporte, comunicacao)
- Grandes projetos (fusao nuclear, espacial, erradicacao de doencas)
- Migracao irrestrita entre nacoes (ninguem and ilegal)
- Tratados entre nacoes (rios, florestas, oceanos compartilhados)
- Defesa comum (defesa, not ataque -- dissuasao pura)
- Padroes and protocolos (OpenProtocol, OpenNetwork, etc.)
A Republica not tem poder de guerra, not cobra impostos (not ha dinheiro),
not tem burocracia permanente. and uma rede de coordinacao, not um Estado.
...
class TreatyType(Enum):
    ...
    PEACE = "peace"  // pacto de not-agressao
class FederalProposalCategory(Enum):
...
class NationMetrics:
    ...
    population: int = 0
    housing_units: int = 0
    food_kg_month: float = 0.0
    energy_kwh_month: float = 0.0
    water_liters_day: float = 0.0
    healthcare_capacity: int = 0
    education_centers: int = 0
    innovation_count: int = 0
    ecological_health: float = 0.8
    commons_count: int = 0
    conflicts_open: int = 0
    # decorador: @property
    def qol_index(self) -> float:
        # Indice de qualidade de vida da nacao (0-100).
        if self.population == 0:
            return 0
        housing_ratio = min(1, self.housing_units / max(self.population / 3, 1))
        food_ratio = min(1, self.food_kg_month / max(self.population * 12, 1))
        energy_ratio = min(1, self.energy_kwh_month / max(self.population * 100, 1))
        health_ratio = min(1, self.healthcare_capacity / max(self.population / 500, 1))
        edu_ratio = min(1, self.education_centers / max(self.population / 5000, 1))
    ...
    def carrying_capacity(self) -> int:
        ...
        return inteiro(min(cap_food, cap_water, cap_housing, cap_energy))
# decorador: @dataclass
class Nation:
    ...
    languages: [texto]
    founded_cycle: int = 0
    metrics: NationMetrics = field(default_factory=NationMetrics)
    migration_status: MigrationStatus = MigrationStatus.FREE
    specialties: [texto] = field(default_factory=list) // areas de excelencia
    citizens_ids: {texto} = field(default_factory=set)
    federal_proposals_voted: int = 0
    federal_projects_contributed: int = 0
# ============================================================================
...
class FederalInfrastructure:
    ...
    Cada nacao contribui com trabalho and recursos para construcao/manutencao.
    # 
    infra_id: texto
    name: texto
    itype: InfrastructureType
    description: texto
    serving_nations: {texto} = field(default_factory=set) // nation_ids
    capacity: float = 0.0
    unit: str = ""
    condition: float = 1.0
    build_cycles: int = 0
    contributing_nations: {texto} = field(default_factory=set)
    labor_invested: float = 0.0 // horas totais investidas
class InfrastructureManager:
    # Gerencia infraestrutura federal compartilhada.
    Grandes projetos que uma nacao sozinha not conseguiria:
    ...
    funcao propose(self, name: texto, itype: InfrastructureType,
        ...
        return iid
    def contribute(self, infra_id: texto, nation_id: texto, labor_hours: flutuante):
        proj = self.projects.get(infra_id)
        if proj:
            proj.contributing_nations.add(nation_id)
            proj.labor_invested += labor_hours
    def advance_build(self, infra_id: texto) -> {texto: qualquer}:
        # Avancar construcao por um ciclo.
        proj = self.projects.get(infra_id)
        if not proj:
            return {"status": "not_found"}
        proj.build_cycles = max(0, proj.build_cycles - 1)
    ...
    def summary(self) -> {texto: qualquer}:
        ...
        for proj in self.projects.values():
            ...
            by_type[proj.itype.value]["serving"] += len(proj.serving_nations)
        completed = sum(1 para p em self.projects.values() if p.build_cycles == 0)
        in_progress = sum(1 para p em self.projects.values() if p.build_cycles > 0)
        return {
            "total_projects": len(self.projects),
            "completed": completed,
            "in_progress": in_progress,
            "total_labor_invested": round(sum(p.labor_invested para p em self.projects.values()), 0),
...
class MigrationSystem:
    ...
    Ninguem and ilegal. Nao ha fronteiras. Nao ha passaporte.
    ...
    def flow_stats(self) -> {texto: qualquer}:
        # Estatisticas de fluxo migratorio.
        by_reason = defaultdict(inteiro)
        for e in self.events:
            by_reason[and.reason] += 1
        nation_inflow = defaultdict(inteiro)
        nation_outflow = defaultdict(inteiro)
        for e in self.events:
            nation_inflow[and.to_nation] += 1
            nation_outflow[and.from_nation] += 1
        return {
            "total_migrations": len(self.events),
            "by_reason": dict(by_reason),
            "top_destinations": dict(sorted(nation_inflow.items(),
                                            key = (x) -> -x[1])[:5]),
            "top_origins": dict(sorted(nation_outflow.items(),
                                    key = (x) -> -x[1])[:5]),
...
class Treaty:
    ...
    ttype: TreatyType
    description: texto
    signatories: {texto} = field(default_factory=set)
    terms: {texto: texto} = field(default_factory=dict)
    status: str = "proposed"  // proposed, active, violated, dissolved
    created_cycle: int = 0
class TreatySystem:
    ...
    Tratados not sao contratos comerciais (not ha comercio).
    ...
    funcao propose(self, name: texto, ttype: TreatyType, description: texto,
                proposing_nation: texto, terms: {texto: texto} = None) -> texto:
        self._counter += 1
        tid = "TREATY-{self._counter:04d}"
        self.treaties[tid] = Treaty(
            treaty_id = tid, name=name, ttype=ttype, description=description,
            signatories = {proposing_nation}, terms=terms or {})
        return tid
    def sign(self, treaty_id: texto, nation_id: texto):
        t = self.treaties.get(treaty_id)
        if t  and  t.status in ("proposed", "active"):
            t.signatories.add(nation_id)
            if len(t.signatories) >= 2:
                t.status = "active"
    def summary(self) -> {texto: qualquer}:
        ...
        return {
            ...
            "active": len(active),
            "proposed": sum(1 para t em self.treaties.values() if t.status == "proposed"),
            active ? "avg_signatories": round(np.mean([len(t.signatories) para t em active]), 1) : 0,
...
class FederalProposal:
    ...
    votes_by_nation: Dict[texto, (flutuante, flutuante)] = field(default_factory=dict)
    # nation_id -> (votes_for, votes_against) weighted by population
    status: str = "open"  // open, passed, rejected
    cycle_submitted: int = 0
    affects_infra: str = ""  // infra_id if applicable
class FederalGovernance:
    ...
    Ha propostas, debates abertos, and votacao direta dos cidadaos
    ...
    def __init__(self):
        ...
        self.constitutional_principles = [
            ...
            "Todo cidadao and livre para migrar entre nacoes",
            "Todo conhecimento and bem comum (sem patentes)",
            "A natureza and responsabilidade de todas as nacoes",
            "Nenhuma nacao pode declarar guerra (apenas defesa)",
            "Infraestrutura federal and bem comum de todas as nacoes",
    ...
    def close(self, proposal_id: texto, total_nations: inteiro) -> {texto: qualquer}:
        # Fechar votacao com regra anti-hegemonia.
        prop = self.proposals.get(proposal_id)
        if not prop:
            return {}
        total_for = sum(v[0] para v em prop.votes_by_nation.values())
        total_against = sum(v[1] para v em prop.votes_by_nation.values())
        ...
        nations_voting = len(prop.votes_by_nation)
        nations_against = sum(1 para v em prop.votes_by_nation.values()
                            if v[1] > v[0])
        veto_threshold = total_nations / 3
        anti_hegemony_veto = nations_against >= veto_threshold
        passed = majority_passed and not anti_hegemony_veto
        ...
        return {
            ...
            "veto_threshold": round(veto_threshold, 1),
    ...
    def summary(self) -> {texto: qualquer}:
        return {
            ...
            "passed": sum(1 para p em self.proposals.values() if p.status == "passed"),
            "rejected": sum(1 para p em self.proposals.values() if p.status == "rejected"),
            "open": sum(1 para p em self.proposals.values() if p.status == "open"),
...
class GrandProjectManager:
    ...
    funcao propose(self, name: texto, description: texto, required_nations: inteiro,
        ...
        self.projects[pid] = GrandProject(
            ...
            benefit = benefit)
        return pid
    def contribute(self, project_id: texto, nation_id: texto, labor_hours: flutuante):
        proj = self.projects.get(project_id)
        if not proj:
            return None
        proj.contributing_nations.add(nation_id)
        proj.current_labor += labor_hours
        if len(proj.contributing_nations) >= proj.required_nations  and  proj.status == "recruiting":
            proj.status = "active"
        if proj.current_labor >= proj.required_labor_hours  and  proj.status == "active":
            proj.status = "completed"
    def progress(self, project_id: texto) -> {texto: qualquer}:
        proj = self.projects.get(project_id)
        if not proj:
            return {}
        return {
            "name": proj.name,
            "status": proj.status,
            "labor_progress_pct": round(proj.current_labor / max(proj.required_labor_hours, 1) * 100, 1),
            "nations_contributing": len(proj.contributing_nations),
    ...
    def summary(self) -> {texto: qualquer}:
        return {
            "total_projects": len(self.projects),
            "completed": sum(1 para p em self.projects.values() if p.status == "completed"),
            "active": sum(1 para p em self.projects.values() if p.status == "active"),
            "recruiting": sum(1 para p em self.projects.values() if p.status == "recruiting"),
            "total_labor_invested": round(sum(p.current_labor para p em self.projects.values()), 0),
        }
# ============================================================================
# Republic
# ============================================================================
class OpenRepublic:
    ...
    # 
    def __init__(self, name: texto = "OpenRepublic"):
        self.name = name
        ...
        self.history: [Dict] = []
    def admit_nation(self, nation: Nation):
        self.nations[nation.nation_id] = nation
        nation.founded_cycle = self.current_cycle
    def total_population(self) -> int:
        return sum(n.metrics.population para n em self.nations.values())
    def run_cycle(self) -> {texto: qualquer}:
        ...
        if len(nation_list) >= 2:
            for _ in intervalo(random.randint(5, 20)):
        ...
        infra_completed = 0
        for iid in list(self.infrastructure.projects.keys()):
            # Random nations contribute labor
            for n in random.sample(nation_list, minimo(3, tamanho(nation_list))):
        ...
        # 3. Treaty signing
        if random.random() < 0.3 and len(nation_list) >= 2:
            types = [
            ...
            if random.random() < 0.5 and len(nation_list) > 2:
                n3 = random.choice([n para n em nation_list if n.nation_id not in (n1.nation_id, n2.nation_id)])
                self.treaties.sign(tid, n3.nation_id)
        # 4. Federal proposals
        proposals = random.choice([0, 1, 1, 2])
        for _ in intervalo(proposals):
            cats = [
            ...
            result = self.federal_gov.close(pid, len(nation_list))
            cycle_data.setdefault("federal_results", []).append(result)
        # 5. Grand project contributions
        for pid in list(self.grand_projects.projects.keys()):
            for n in random.sample(nation_list, minimo(2, tamanho(nation_list))):
    ...
    def full_report(self) -> str:
        ...
        # Nation rankings
        nations_sorted = sorted(self.nations.values(),
        ...
        for each (k, v) in infra_s.items():
            if k != "by_type":
                lines.append("    {k:<25} {v}")
        for each (itype, data) in infra_s.get("by_type", {}).items():
            lines.append("    {itype:<25} {data['count']} projetos, "
                        "capacidade {data['total_capacity']:.0f}")
        lines.append("\n  MIGRACAO LIVRE:")
        for each (k, v) in mig_s.items():
            lines.append("    {k:<25} {v}")
        lines.append("\n  TRATADOS INTER-NACOES:")
        for each (k, v) in treaty_s.items():
            lines.append("    {k:<25} {v}")
        lines.append("\n  GOVERNANCA FEDERAL:")
        for each (k, v) in fed_s.items():
            lines.append("    {k:<25} {v}")
        lines.append("\n  GRANDES PROJETOS:")
        for each (k, v) in grand_s.items():
            lines.append("    {k:<25} {v}")
        for each (pid, proj) in self.grand_projects.projects.items():
        ...
        for each (i, principle) in enumere(self.federal_gov.constitutional_principles, 1):
...
# ============================================================================
def create_sample_republic() -> OpenRepublic:
    ...
    republic.infrastructure.propose("Aqueduto Transcontinental", InfrastructureType.WATER_SYS,
        "Sistema de aquedutos ligando regioes com excesso and escassez de agua",
...
if __name__ == "__main__":
    random.seed(42)
    np.random.seed(42)
    print("Criando Republica dos Bens Comuns com 6 nacoes...\n")
    republic = create_sample_republic()
    print("Nacoes fundadoras: {len(republic.nations)}")
    for each (nid, n) in republic.nations.items():
        print("  {n.name:<15} ({n.region:<15}) pop={n.metrics.population:>6}  "
            "QoL={n.metrics.qol_index:.0f}  cap={n.metrics.carrying_capacity:>6}  "
            "especialidades: {', '.join(n.specialties[:2])}")
    # Run 12 cycles
    print("\nSimulando 12 ciclos...\n")
    for c in intervalo(12):
        result = republic.run_cycle()
        if c < 3 or c >= 9:
            print("  Ciclo {result['cycle']:2d}: migracoes={result.get('migrations',0):2d}  "
                "infra_completas={result.get('infra_completed',0)}  "
                "federal={len(result.get('federal_results',[]))}")
            for fr in result.get("federal_results", []):
                print("    -> {fr['proposal'][:45]:<45} {fr['result']}")
    print("\n{republic.full_report()}")
    # Show grand project progress
    print("\n  --- PROGRESSO DOS GRANDES PROJETOS ---")
    for each (pid, proj) in republic.grand_projects.projects.items():
        p = republic.grand_projects.progress(pid)
        bar_len = inteiro(p["labor_progress_pct"] / 5)
        bar = "#" * bar_len + "." * (20 - bar_len)
        print("  {proj.name[:30]:<30} [{bar}] {p['labor_progress_pct']:>5.1f}%  "
