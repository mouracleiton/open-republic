#!/usr/bin/env python3
"""
OpenRepublic -- Solucoes para os 3 Gargalos Restantes do Sahel -- gerado de Portugol++
"""
...
3. Habitacao (167% sobrecarga, faltam unidades and construtores)
Em uma sociedade sem propriedade privada and sem dinheiro, as solucoes
not sao "contratar mais gente" or "comprar equipamento". Sao:
...
# ============================================================================
class ChildcareSolution:
    # Como cuidar de 300 criancas com poucos adultos dedicados.
    Resposta: not precisa de 30 adultos全职.
    Precisa de um SISTEMA onde cada adulto contribui um pouco
    and as proprias criancas se organizam em rede de mutualidade.
    # 
    ...
    funcao calculate(population: inteiro, children_0_12: inteiro,
        ...
        ai_stations = min(children_0_12 // 8, 40) // 1 estacao por 8 criancas
        ...
        ratio = total_equiv / max(needed_equiv, 1)
        gap = max(0, needed_equiv - total_equiv)
        return {
            ...
            "fontes_de_cuidado": {
                "trabalhadores_dedicados": {
                    ...
                    "equiv_fulltime": round(dedicated, 1),
                },
                "rotacao_parental_4h_semana": {
                    "pessoas": parents,
                    "horas_semana": parental_hours_week,
                    "equiv_fulltime": round(parental_equiv, 1),
                ...
                "idosos_ativos_mentores": {
                    ...
                    "equiv_fulltime": round(elderly_equiv, 1),
                    "principio": "60+ not se aposentam, mudam de funcao",
                },
                "adolescentes_mentores_13_17": {
                    "pessoas": teen_mentors,
                    "horas_semana": teen_hours_week,
                    "equiv_fulltime": round(teen_equiv, 1),
            ...
            "capacidade_total_equiv": round(total_equiv, 1),
            "demanda_total_equiv": round(needed_equiv, 1),
            "cobertura": "{min(100, ratio * 100):.0f}%",
            "gap_residual": round(gap, 1),
            "viavel": ratio >= 1.0,
        }
# ============================================================================
...
class AgricultureSolution:
    ...
    de agua para: sementes, maquinario, conhecimento, and pessoas.
        ...
        return {
            ...
            "estrategias": {
                "hidroponia_automatizada": {
                    "area_m2": round(hydro_needed_m2),
                    "producao_kg_mes": round(hydro_needed_m2 * hydro_yield_kg_m2_month),
                    "trabalhadores": hydro_workers,
                    "agua_litros_dia": round(hydro_water_liters_day),
                    "eficiencia": "10x mais que solo, 90% menos agua",
                },
                "permacultura_desertica": {
                    "area_hectares": round(perm_needed_hectares, 1),
                    "producao_kg_mes": round(perm_needed_hectares * perm_yield_kg_hectare_month),
            ...
            "producao_total_nova_kg_mes": round(total_new_production),
...
class HousingSolution:
    ...
    funcao calculate(population: inteiro, current_units: inteiro,
        ...
        months_to_fill_gap = math.ceil(gap_units / max(total_houses_month, 1))
        total_workers = (printers_3d * 2 + workers_adobe + workers_fablab +
                        swarm_participants / 4) // swarm and rotacional
        return {
            "gap_habitacional": "{gap_units} unidades",
            "estrategias": {
                "impressao_3d": {
                    "printers": printers_3d,
                    "casas_por_mes": round(houses_3d_month),
                ...
                "adobe_btc_prensa": {
                    ...
                    "casas_por_mes": round(houses_adobe_month),
            ...
            "casas_por_mes_total": round(total_houses_month),
...
if __name__ == "__main__":
    print("=" * 75)
    print("  SOLUCOES PARA OS 3 GARGALOS DO SAHEL")
    print("  Sem dinheiro. Sem empreiteira. Sem 'contratar'.")
    print("  Com automatizacao + comunidade + conhecimento aberto.")
    print("=" * 75)
    # === SOLUCAO 1: CUIDADO INFANTIL ===
    print("\n\n" + "=" * 75)
    print("  GARGALO 1: CUIDADO INFANTIL (300% sobrecarga)")
    print("  1 pessoa tentando cuidar de 300 criancas")
    print("=" * 75)
    ...
    print("  Demanda ajustada (IA reduz 25%): {childcare['demanda_ajustada_por_ia']:,}")
    print("\n  FONTES DE CUIDADO:")
    print("  {'Fonte':<35} {'Pessoas':>8} {'Equiv FT':>10}")
    print("  {'-'*55}")
    for each (fonte, dados) in childcare['fontes_de_cuidado'].items():
        if 'pessoas' in dados:
            print("  {fonte.replace('_', ' ').title():<35} {dados['pessoas']:>8} "
                "{dados['equiv_fulltime']:>10}")
        elif 'estacoes' in dados:
            print("  {fonte.replace('_', ' ').title():<35} {dados['estacoes']:>8} estacoes")
    print("  {'-'*55}")
    print("  {'CAPACIDADE TOTAL':<35} {'':>8} {childcare['capacidade_total_equiv']:>10}")
    print("  {'DEMANDA TOTAL':<35} {'':>8} {childcare['demanda_total_equiv']:>10}")
    print("\n  COBERTURA: {childcare['cobertura']}")
    print("  VIÁVEL: {'SIM' if childcare['viavel'] else 'NAO'}")
    print("\n  PRINCIPIO: Nao se trata de contratar 75 profissionais.")
    print("  Trata-se de ativar a COMUNIDADE INTEIRA:")
    print("    - Cada pai/mae: 4h/semana")
    print("    - Cada idoso ativo: 10h/semana (continuar contribuindo)")
    print("    - Cada adolescente: 6h/semana (mentoria + responsabilidade)")
    print("    - Estacoes de IA: 25% do aprendizado and autonomo")
    print("  Resultado: a comunidade SE cuida. Nao precisa de Estado.")
    # === SOLUCAO 2: AGRICULTURA ===
    print("\n\n" + "=" * 75)
    print("  GARGALO 2: AGRICULTURA (167% sobrecarga)")
    print("  Faltam 2.000 kg/mes de comida para 40.000 pessoas")
    print("=" * 75)
    ...
    print("\n  ESTRATEGIAS:")
    print("  {'Estrategia':<30} {'Escala':>15} {'Producao':>12} {'Pessoas':>8}")
    print("  {'-'*68}")
    for each (nome, dados) in agriculture['estrategias'].items():
        if isinstance(dados, dict):
            ...
            elif 'area_hectares' in dados:
                escala = "{dados['area_hectares']} ha"
                prod = "{dados['producao_kg_mes']} kg"
                pessoas = texto(dados.get('trabalhadores', ''))
            elif 'tanques' in dados:
                escala = "{dados['tanques']} tanques"
                prod = "{dados['producao_kg_mes']} kg"
                pessoas = texto(dados.get('trabalhadores', ''))
            elif 'sensores' in dados:
                escala = "{dados['sensores']} sensores"
                prod = dados.get('reducao_trabalho', '')
                pessoas = ""
            elif 'variedades' in dados:
                escala = "{dados['variedades']} variedades"
                prod = ""
            elif 'especialistas_enviados' in dados:
                escala = "{dados['especialistas_enviados']} de {dados['de'][:12]}"
                pessoas = texto(dados.get('pessoas_treinadas', ''))
            print("  {nome.replace('_', ' ').title():<30} {escala:>15} {prod:>12} {pessoas:>8}")
    print("\n  {'PRODUCAO NOVA TOTAL':<30} {'':>15} {agriculture['producao_total_nova_kg_mes']:>9} kg/mes")
    print("  {'TRABALHADORES':<30} {'':>15} {'':>12} {agriculture['trabalhadores_necessarios']:>8}")
    print("  COBRE O GAP: {'SIM' if agriculture['cobre_gap'] else 'NAO'}")
    print("  AUTO-SUSTENTAVEL APOS: {agriculture['auto_sustentavel_apos']}")
    print("\n  PRINCIPIO: Hidroponia (10x mais por m2) + permacultura")
    print("  (auto-sustentavel apos 3 anos) + aquaponia (proteina + vegetais)")
    print("  + automacao IoT (40% menos trabalho) + conhecimento da Amazonia")
    # === SOLUCAO 3: HABITACAO ===
    print("\n\n" + "=" * 75)
    print("  GARGALO 3: HABITACAO (167% sobrecarga)")
    print("  Faltam ~2.000 unidades para 40.000 pessoas")
    print("=" * 75)
    ...
    print("\n  ESTRATEGIAS:")
    print("  {'Estrategia':<25} {'Unid/mes':>10} {'Material':<25} {'Pessoas':>8}")
    print("  {'-'*70}")
    for each (nome, dados) in housing['estrategias'].items():
        if isinstance(dados, dict)  and  ('casas_por_mes' in dados  or  'kits_por_mes' in dados):
            ...
            print("  {nome.replace('_', ' ').title():<25} {unid:>10} {mat:<25} {pessoas:>8}")
        elif isinstance(dados, dict)  and  'massa_termica' in dados:
            print("  {'Design Passivo':<25} {'N/A':>10} {'Sem ar condicionado':<25}")
    print("\n  {'TOTAL':<25} {housing['casas_por_mes_total']:>10}")
    print("  MESES PARA COBRIR GAP: {housing['meses_para_cobrir_gap']}")
    print("  TRABALHADORES: {housing['trabalhadores_necessarios']}")
    print("  CUSTO: {housing['custo_monetario']}")
    print("  MATERIAL: {housing['material_principal']}")
    print("\n  PRINCIPIO: Terra local and gratuita. Prensa manual faz bloco.")
    print("  Impressora 3D faz casa em 30h. Mutirao constroi 10 casas/semana.")
    print("  Design passivo: 40cm de parede de terra = sem ar condicionado.")
    # === RESUMO INTEGRADO ===
    print("\n\n" + "=" * 75)
    print("  RESUMO: COMO RESOLVER OS 3 GARGALOS SEM DINHEIRO")
    print("=" * 75)
    print("""
+-----------+----------------+------------------+----------+--------+
...
| | automacao | | drones | |
+-----------+----------------+------------------+----------+--------+
| Habitacao | 3D print + | Terra local | ~40 + | 6-12 |
...
As pessoas existem. O que falta and ORGANIZACAO, not dinheiro.
