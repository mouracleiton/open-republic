...
package openrepublic_sistema_constitucional_representantes_por_sistema
...
// ============================================================================
type ConstitutionalPrinciple int
const (
...
type ConstitutionalEngine struct {
    ...
    funcao register_system(self, system_id: texto, name: texto,
                        ...
                        created_by := "comunidade", // string
                        approved_by_vote := false, // bool
                        vote_count := 0, // int64
                        affects_bodies := false, // bool
                        collects_data := false, // bool
                        opaque_decisions := false, // bool
                        single_point_of_failure := false, // bool
                        requires_bodily_consent := false, // bool
                        labor_base_respected := true, // bool
                        has_successor := false, // bool
                        public_docs := false, // bool
    ...
    func validate_system(self, system_id: texto) {texto: qualquer} {
        ...
        avg_score = soma(c.score para c em checks) / len(checks)
        ...
        return result
    func validate_all(self) {texto: qualquer} {
        // Valida TODOS os sistemas registrados.
        results = {}
        for _, sid := range self.systems_registry {
    ...
    func _check_anti_elitism(self, sys_data: Dict) ComplianceCheck {
    ...
    func _check_bodily_autonomy(self, sys_data: Dict) ComplianceCheck {
    ...
    func _check_equal_labor(self, sys_data: Dict) ComplianceCheck {
    ...
    func _check_democratic_process(self, sys_data: Dict) ComplianceCheck {
    ...
    func _status_from_severity(severity: ViolationSeverity) string {
...
type SystemSteward struct {
    ...
    citizen_id: texto
    citizen_name: texto
    role := StewardRole.STEWARD // StewardRole
    mandate_start := 0.0 // float64
    mandate_duration_days := 180 // 6 meses // int64
    cycles_served := 0 // int64
    max_consecutive := 2 // int64
    max_systems_per_person := 3 // int64
    // Demands pipeline
    pending_demands := field(default_factory=list) // [Dict]
    completed_demands := field(default_factory=list) // [Dict]
    rejected_demands := field(default_factory=list) // [Dict]
    // Reporting
    last_status_report := "" // string
    constitutional_alerts := 0 // int64
    // Successor
    apprentice_id := "" // string
    apprentice_name := "" // string
    // decorador: @property
    func can_accept_more_systems(self) bool {
        return self.cycles_served < self.max_consecutive
    funcao receive_demand(self, source: texto, demand: texto,
                    priority := "normal") -> {texto: qualquer}: // string
        ...
        return entry
    func execute_demand(self, demand_id: texto) {texto: qualquer} {
        // Marca demanda como executada.
        for _, d := range self.pending_demands {
    ...
    func report_status(self) {texto: qualquer} {
...
type StewardAssignment struct {
    ...
    funcao founder_batch_demands(self, demands: List[(texto, texto)]
        ...
        para cada (sys_id, demand_text) em demands: {
            ...
            results.append(r)
        return results
    func system_status_report(self) {texto: qualquer} {
    ...
    func list_systems_for_demand(self) [Dict] {
...
if __name__ == "__main__" {
    fmt.Println("=" * 80)
    fmt.Println("  OPENREPUBLIC -- MOTOR CONSTITUCIONAL + GUARDIOES")
    fmt.Println('  "Os principios ! sao adornos. Sao LEIS."')
    fmt.Println("=" * 80)
    mgr = StewardAssignment()
    // === 1. OS 4 PRINCIPIOS CODIFICADOS ===
    fmt.Println("\n\n  === 1. PRINCIPIOS CONSTITUCIONAIS CODIFICADOS ===\n")
    for _, p := range ConstitutionalPrinciple {
        fmt.Println("  P{p.value[0]}: {p.value[1].upper()}")
        fmt.Println("  {p.value[2]}")
        fmt.Println()
    // === 2. REGISTRO DE SISTEMAS ===
    fmt.Println("\n  === 2. SISTEMAS REGISTRADOS: "
        "{len(mgr.engine.systems_registry)} ===\n")
    fmt.Println("  {'ID':<12} {'Sistema':<24} {'Dominio':<16} {'Guardiao'}")
    fmt.Println("  {'-'*70}")
    ...
    for _, s := range mgr.list_systems_for_demand() {
        fmt.Println("  {s['system_id']:<12} {s['name']:<24} "
            "{s['domain']:<16} {s['steward']}")
    // === 3. VALIDACAO CONSTITUCIONAL ===
    fmt.Println("\n\n  === 3. AUDITORIA CONSTITUCIONAL COMPLETA ===\n")
    audit = mgr.engine.validate_all()
    fmt.Println("  Total de sistemas:     {audit['total_systems']}")
    fmt.Println("  Conformes:             {audit['fully_compliant']}")
    fmt.Println("  Suspensos:             {audit['suspended']}")
    fmt.Println("  Banidos:               {audit['banned']}")
    fmt.Println("  Taxa de conformidade:  {audit['compliance_rate']}")
    fmt.Println("  Precisam revisao:      {audit['needs_review']}")
    ...
    para cada (sid, demand) em demands: {
        ...
        if "error" in r {
            ...
            fmt.Println("  [{r['system']}] -> {r['steward']}")
            fmt.Println("    Demanda: {demand}")
            fmt.Println("    Status constitucional: {r['constitutional_status']}")
            fmt.Println("    Alertas: {r['constitutional_alerts']}")
            fmt.Println()
    // === 5. SISTEMAS SEM GUARDIAO ===
    report = mgr.system_status_report()
    fmt.Println("\n  === 5. SISTEMAS SEM GUARDIAO ===\n")
    fmt.Println("  Designados: {report['stewards_assigned']}/{report['total_systems']}")
    fmt.Println("  Sem guardiao: {report['unassigned_systems']}")
    if report['unassigned_list'] {
        fmt.Println("  IDs: {', '.join(report['unassigned_list'][:10])}...")
    // === 6. DETALHE: AUDITORIA DE UM SISTEMA ===
    fmt.Println("\n\n  === 6. AUDITORIA DETALHADA: OpenHealth ===\n")
    detail = mgr.engine.validate_system("R-HEA-01")
    fmt.Println("  Sistema: {detail['system_name']}")
    fmt.Println("  Score geral: {detail['overall_score']}/100")
    fmt.Println("  Status: {detail['status']}")
    fmt.Println("  Conforme: {'SIM' if detail['fully_compliant'] else 'NAO'}")
    fmt.Println("\n  verificacao por principio:")
    for _, c := range detail["checks"] {
        status = c["passed"] ? "PASS" : "FAIL"
        fmt.Println("    [{status}] {c['principle']}: {c['score']}/100")
        if c["notes"] != "Conforme."  &&  c["notes"] {
            fmt.Println("          -> {c['notes'][:100]}")
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  SISTEMA CONSTITUCIONAL OPERACIONAL")
    fmt.Println("{'='*80}")
    fmt.Println("""
...
// )
    fmt.Println("{'='*80}")
    fmt.Println("  ConstitutionalEngine + StewardAssignment operacional.")
    fmt.Println("  {len(REPUBLIC_SYSTEMS)} sistemas. 4 principios. 0 exencoes.")
    fmt.Println("{'='*80}")
