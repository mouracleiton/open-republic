...
**Arquivo original:** `core/entity_triage.py`
...
Uma OpenNation nao e um bloco homogeneo. E composta por ENTIDADES:
  - Cada entidade e um grupo/infraestrutura/instituicao que presta
    uma funcao essencial para a nacao.
  - As entidades tem estados de saude, capacidade, e necessidade.
  - A Republica precisa saber QUAIS entidades precisam de atencao
    AGORA para priorizar alocacao de trabalho e recursos.
Esta simulacao identifica e prioriza as entidades mais criticas.
...
classe EntityType herda de Enum:
    // Producao
    ...
    FABLAB = "fablab"  // fabricacao digital (3D imprima, CNC, laser)
...
classe EntityStatus herda de Enum:
    ...
    PLANNED = "planned"  // ainda nao existe, precisa ser criada
...
classe Entity:
    ...
    Cada entidade e um componente funcional da nacao.
    ...
    // Capacidade
    seja capacity: flutuante = 100.0 // unidades que pode produzir/atender por ciclo
    seja current_demand: flutuante = 50.0 // demanda atual
    seja people_served: inteiro = 0
    seja workers: inteiro = 0
    seja workers_needed: inteiro = 5

    // Estado
    seja status: EntityStatus = EntityStatus.OPERATIONAL
    seja health: flutuante = 1.0 // 0=colapsou, 1=perfeito
    seja supply_level: flutuante = 1.0 // 0=sem insumos, 1=abastecido
    seja maintenance_level: flutuante = 1.0 // 0=precisa reparo urgente, 1=bem cuidado

    // Dependencias (outras entidades que esta precisa para funcionar)
    seja depends_on: {texto} = field(default_factory=set) // entity_ids

    // Necessidades atuais
    seja needs: [texto] = field(default_factory=list)

    // Historico
    seja last_issues: [texto] = field(default_factory=list)

    // decorador: @property
    funcao utilization(self) -> flutuante:
        // Quao sobrecarregada esta (0=solta, 1=no limite, >1=estourou).
        retorne self.current_demand / maximo(self.capacity, 1)

    // decorador: @property
    funcao urgency(self) -> UrgencyLevel:
        // Nivel de urgencia calculado automaticamente.
        se self.status == EntityStatus.OFFLINE entao:
            retorne UrgencyLevel.CRITICAL
        se self.status == EntityStatus.CRITICAL entao:
            retorne UrgencyLevel.CRITICAL
        se self.health < 0.3 ou self.supply_level < 0.2 ou self.utilization > 1.5 entao:
            retorne UrgencyLevel.CRITICAL
        se self.health < 0.5 ou self.supply_level < 0.4 ou self.utilization > 1.2 entao:
            retorne UrgencyLevel.URGENT
        se self.health < 0.7 ou self.supply_level < 0.6 ou self.utilization > 1.0 entao:
            retorne UrgencyLevel.ELEVATED
        se self.health < 0.85 ou self.workers < self.workers_needed entao:
            retorne UrgencyLevel.MODERATE
        se self.utilization < 0.5 entao:
            retorne UrgencyLevel.THRIVING
        retorne UrgencyLevel.STABLE

    // decorador: @property
    funcao risk_score(self) -> flutuante:
        ...
        retorne arredonde(minimo(100, health_risk + supply_risk + overload_risk + staff_risk + maint_risk), 1)

    funcao assess(self) -> {texto: qualquer}:
        ...
        se self.maintenance_level < 0.5 entao:
...
classe NationState:
    ...
    region: texto
    population: inteiro
    seja entities: {texto: Entity} = field(default_factory=dict)

    funcao add_entity(self, entity: Entity):
        entity.nation_id = self.nation_id
        self.entities[entity.entity_id] = entity

    funcao critical_entities(self) -> [Entity]:
        // Entidades em estado critico.
        retorne ordene(
            [e para e em self.entities.values() if e.urgency.value >= 4],
            key = (e) -> -e.risk_score
        )
    ...
    funcao all_by_urgency(self) retorna Dict[UrgencyLevel, [Entity]]:
        ...
        para cada e em self.entities.values():
            groups[e.urgency].append(e)
        para cada level em groups:
            groups[level].sort(key=(e) -> -e.risk_score)
        retorne dict(groups)

    // decorador: @property
    funcao avg_health(self) -> flutuante:
        se nao self.entities entao:
            retorne 0
        retorne arredonde(flutuante(np.mean([e.health para e em self.entities.values()])), 2)

    // decorador: @property
    funcao total_capacity_utilization(self) -> flutuante:
        se nao self.entities entao:
            retorne 0
        retorne arredonde(flutuante(np.mean([e.utilization para e em self.entities.values()])), 2)

    // decorador: @property
    funcao people_at_risk(self) -> inteiro:
        // Pessoas afetadas por entidades em risco.
        retorne soma(e.people_served para e em self.entities.values() if e.urgency.value >= 4)
...
// decorador: @dataclass
classe RepublicState:
    // Estado da Republica completa.
    name: texto
    seja nations: {texto: NationState} = field(default_factory=dict)
    seja cycle: inteiro = 0

    funcao add_nation(self, nation: NationState):
        self.nations[nation.nation_id] = nation

    funcao all_entities(self) -> [Entity]:
        entities = []
        para cada n em self.nations.values():
    ...
    funcao priority_queue(self) retorna List[{texto: qualquer}]:
        ...
        scored = []
        para cada e em entities:
            assessment = e.assess()
            // Composite priority score
            priority = (
                e.urgency.value * 1000 +
                e.risk_score * 10 +
                e.people_served * 0.01
            )
            scored.append((priority, assessment))

        scored.sort(key=(x) -> -x[0])

        retorne [s[1] para s em scored]

    funcao republic_health(self) -> {texto: qualquer}:
        // Saude geral da Republica.
        all_entities = self.all_entities()
        se nao all_entities entao:
            retorne {}

        by_urgency = defaultdict(inteiro)
        para cada e em all_entities:
            by_urgency[e.urgency.name] += 1

        total_people_at_risk = soma(
            e.people_served para e em all_entities if e.urgency.value >= 4
        )
        total_people_served = soma(e.people_served para e em all_entities)

        retorne {
            "nations": tamanho(self.nations),
            "total_entities": tamanho(all_entities),
            "avg_health": arredonde(flutuante(np.mean([e.health para e em all_entities])), 2),
...
funcao create_critical_scenario() -> RepublicState:
    // Cria um cenario onde varias entidades precisam atencao AGORA.

    Cenario: A Republica esta no ciclo 18. Algumas nacoes estao
    bem estabelecidas, outras sao recentes e tem gargalos.
    O objetivo e identificar o que tratar primeiro.
    // 
...
classe TriageEngine:
    ...
    3. Se a falha e de insumo, pessoal, ou infraestrutura
    4. Se existe capacidade excedente em outra nacao para ajudar
    // 

    funcao __init__(self, republic: RepublicState):
        self.republic = republic

    funcao triage(self) -> {texto: qualquer}:
        ...
        para cada item em queue:
            urgency = item["urgency"]
            se urgency >= 4 entao:
                red.append(item)
            senao se urgency >= 3 entao:
                yellow.append(item)
            senao se urgency >= 2 entao:
                green.append(item)
            senao:
                blue.append(item)
    ...
    funcao _dependency_chain_analysis(self) -> [Dict]:
        // Se A depende de B, e B cai, A tambem cai.
        risks = []
        all_entities = {e.entity_id: e para e em self.republic.all_entities()}

        para cada entity em self.republic.all_entities():
            para cada dep_id em entity.depends_on:
                ...
                senao se dep.urgency.value >= 4 entao:
                    risks.append({
                        "entity": entity.name,
                        "depends_on": dep.name,
    ...
    funcao _find_surplus_capacity(self) -> [Dict]:
        // Encontrar entidades com capacidade excedente.
        surplus = []
        para cada e em self.republic.all_entities():
            se e.utilization < 0.7 e e.health > 0.85 entao:
                surplus.append({
                    "entity": e.name,
                    "nation": e.nation_id,
                    "type": e.etype.value,
                    "capacity_used_pct": arredonde(e.utilization * 100, 0),
                    "available_capacity_pct": arredonde((1 - e.utilization) * 100, 0),
                    "health": arredonde(e.health * 100, 0),
                    "can_help": verdadeiro,
                })
        retorne ordene(surplus, key=(x) -> x["available_capacity_pct"], reverse=verdadeiro)
...
se __name__ == "__main__" entao:
    imprima("=" * 75)
    imprima("  OPENREPUBLIC -- TRIAGEM DE ENTIDADES PRIORITARIAS")
    imprima("  'O que precisamos tratar AGORA'")
    imprima("=" * 75)
    ...
    imprima("  Nacoes: {s['nations']} | Entidades: {s['total_entities']}")
    imprima("  Saude media: {s['avg_health']:.0%}")
    imprima("\n  {'Zona':<12} {'Quantidade':>10}")
    imprima("  {'-'*25}")
    imprima("  {'CRITICA':<12} {s['critical_count']:>10}  <-- TRATAR AGORA")
    imprima("  {'URGENTE':<12} {s['urgent_count']:>10}")
    imprima("  {'ELEVADA':<12} {s['elevated_count']:>10}")
    imprima("  {'ESTAVEL':<12} {s['stable_count']:>10}")
    imprima("\n  Pessoas em risco: {s['people_at_risk']:,} / {s['total_people_served']:,} ({s['risk_coverage_pct']}%)")

    // RED ZONE -- Critical entities
    imprima("\n\n{'='*75}")
    imprima("  ZONA VERMELHA -- TRATAR AGORA ({len(result['red_zone'])} entidades)")
    imprima("{'='*75}")

    para cada item em result["red_zone"]:
        imprima("\n  [{item['urgency_name']}] {item['name']}")
        imprima("    Nacao: {item['nation']} | Tipo: {item['type']}")
        imprima("    Risk score: {item['risk_score']}/100 | Utilizacao: {item['utilization']:.0f}%")
        imprima("    Saude: {item['health']:.0f}% | Insumos: {item['supply']:.0f}%")
        imprima("    Pessoas: {item['pessoas_servidas'] if 'pessoas_servidas' in item else item['people_served']:,}")
        imprima("    Trabalhadores: {item['workers']}")
        imprima("    Problemas:")
        para cada issue em item["issues"]:
            imprima("      ! {issue}")
        imprima("    Acao:")
        para cada rec em item["recommendations"]:
            imprima("      -> {rec}")

    // YELLOW ZONE
    se result["yellow_zone"] entao:
        imprima("\n\n{'='*75}")
        imprima("  ZONA AMARELA -- TRATAR EM DIAS ({len(result['yellow_zone'])} entidades)")
        imprima("{'='*75}")
        para cada item em result["yellow_zone"]:
            imprima("  [{item['urgency_name']}] {item['name']} ({item['nation']})")
            imprima("    Risk: {item['risk_score']} | Uso: {item['utilization']:.0f}% | "
                  "Saude: {item['health']:.0f}%")
            imprima("    Issues: {', '.join(item['issues'][:2])}")
            imprima("    -> {item['recommendations'][0]}")
            imprima()

    // Dependency chains at risk
    imprima("\n{'='*75}")
    imprima("  CADEIAS DE DEPENDENCIA EM RISCO")
    imprima("{'='*75}")
    se result["dependency_chains_at_risk"] entao:
        para cada risk em result["dependency_chains_at_risk"]:
            imprima("\n  {risk.get('entity', '?')}")
            imprima("    {risk['risk']}")
            se "cascade_impact" in risk entao:
                imprima("    Impacto em cascata: {risk['cascade_impact']:,} pessoas")
    senao:
        imprima("  Nenhuma cadeia de dependencia em risco.")

    // Surplus capacity
    imprima("\n{'='*75}")
    imprima("  CAPACIDADE EXCEDENTE DISPONIVEL PARA REALOCAR")
    imprima("{'='*75}")
    para cada s em result["surplus_capacity_available"][:8]:
        imprima("  {s['entity']:<35} {s['nation']}  "
              "livre: {s['available_capacity_pct']:.0f}%  saude: {s['health']:.0f}%")

    // Action plan
    imprima("\n{'='*75}")
    imprima("  PLANO DE ACAO PRIORITARIO")
    imprima("{'='*75}")
    para cada action em result["action_plan"]:
        imprima("\n  #{action['priority']} [{action['action']}] {action['entity']} ({action['nation']})")
        imprima("     Pessoas afetadas: {action['people_affected']:,}")
        imprima("     Prazo: {action['timeline']}")
        imprima("     Passos:")
        para cada step em action["immediate_steps"]:
            imprima("       -> {step}")
        se action.get("potential_helpers") entao:
            imprima("     Podem ajudar: {', '.join(action['potential_helpers'])}")

    // Per-nation breakdown
    imprima("\n{'='*75}")
    imprima("  SAUDE POR NACAO")
    imprima("{'='*75}")
    imprima("  {'Nacao':<15} {'Entidades':>10} {'Saude':>7} {'Criticas':>9} {'Em risco':>10}")
    imprima("  {'-'*55}")
    para cada (nid, nation) em republic.nations.items():
        crit = tamanho(nation.critical_entities())
        imprima("  {nation.name:<15} {len(nation.entities):>10} "
              "{nation.avg_health:>6.0%} {crit:>9} {nation.people_at_risk:>10,}")

    imprima("\n{'='*75}")
    imprima("  RESUMO: {len(result['red_zone'])} entidades precisam de acao IMEDIATA.")
    imprima("  {result['summary']['people_at_risk']:,} pessoas afetadas.")
    imprima("  {len(result['surplus_capacity_available'])} entidades tem capacidade")
    imprima("  para realocar e ajudar.")
    imprima("{'='*75}")
