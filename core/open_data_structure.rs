...
struct PacketMetadata {
    ...
    let sources: [texto] = field(default_factory=list);
    let compression: String = "openpack_v1";
    let priority: DataPriority = DataPriority.NORMAL;
    let input_type: InputType = InputType.COMMAND;
    let context: String = ""  // o que o usuario esta fazendo;
    let location: String = ""  // onde (opcional);
    let device: String = ""  // smartphone, notebook, terminal;
    ...
    funcao create(cls, user_id: texto, sources: [texto],
            ...
            let input_type: InputType = InputType.COMMAND,;
            let context: String = "",;
            let device: String = "notebook") -> "PacketMetadata":;
        return cls(;
...
struct KeyboardData {
    ...
    let raw_text: String = "";
    let compressed_text: String = ""  // base64(zlib(text));
    let raw_length: i64 = 0;
    let compressed_length: i64 = 0;
    let compression_ratio: f64 = 0.0;
    let language: String = "pt-BR";
    let input_type: String = "command"  // command, question, context;
    let modifiers: [texto] = field(default_factory=list) // shift, ctrl, etc;
    let typing_speed_wpm: f64 = 0.0 // palavras por minuto;
    let autocorrect_used: bool = false;
    let emoji_detected: bool = false;
    let code_detected: bool = false // se digitou codigo;
    let url_detected: bool = false;
    // decorador: @classmethod
    funcao from_input(cls, text: texto, language: texto = "pt-BR",
                let input_type: String = "command",;
                let modifiers: [texto] = NULL) -> "KeyboardData":;
...
struct AudioData {
    ...
    //
    let compressed_audio: String = ""  // base64(opus);
    let channels: [texto] = field(default_factory=() -> ["user_voice"]);
    let duration_sec: f64 = 0.0;
    let format: String = "opus_16khz";
    let sample_rate: i64 = 16000;
    let bitrate_kbps: i64 = 24;
    let vad_detected: bool = false // voice activity detection;
    let diarization: {texto: flutuante} = field(default_factory=dict);
    let transcription: String = ""  // texto transcrito (STT);
    let transcription_confidence: f64 = 0.0;
    let ambient_type: String = ""  // rua, escritorio, silencio, musica;
    let noise_level_db: f64 = 0.0;
    // decorador: @classmethod
    funcao from_capture(cls, duration: flutuante = 3.0,
                    let transcription: String = "",;
                    let channels: [texto] = NULL,;
                    let vad: bool = true,;
                    let ambient: String = "escritorio",;
                    let noise_db: f64 = 35.0) -> "AudioData":;
...
struct WebcamData {
    ...
    //
    let compressed_frames: String = "";
    let frame_count: i64 = 0;
    let resolution: String = "640x480";
    let fps_captured: i64 = 1 // 1-3 frames por pacote;
    let face_detected: bool = false;
    let face_expression: String = ""  // neutro, feliz, focado, cansado;
    let gesture: String = "none"  // none, wave, point, thumbs_up;
    let gaze_direction: String = "screen"  // screen, away, phone;
    let posture: String = "upright"  // upright, leaning, standing;
    let screen_share: bool = false;
    let screen_content: String = ""  // texto detectado na tela (OCR);
    let objects_detected: [texto] = field(default_factory=list);
    // decorador: @classmethod
    funcao from_capture(cls, face: logico = verdadeiro,
                    let expression: String = "focado",;
                    let gesture: String = "none",;
                    let gaze: String = "screen",;
                    let posture: String = "upright",;
                    let screen_share: bool = false,;
                    let screen_text: String = "",;
                    let objects: [texto] = NULL) -> "WebcamData":;
...
struct OpenDataPacket {
    ...
    let keyboard: KeyboardData? = NULL;
    let audio: AudioData? = NULL;
    let webcam: WebcamData? = NULL;
    // Fusao multimodal
    let fusion_mode: String = "single"  // single, combo_multimodal;
    let primary_source: DataSource = DataSource.KEYBOARD;
    let secondary_sources: [DataSource] = field(default_factory=list);
    // decorador: @property
    ...
    // decorador: @property
    fn total_compressed_size(self) -> i64 {
        size = 0;
    ...
    fn summary(self) -> {texto: qualquer} {
...
struct DataPacketEngine {
    ...
    funcao create_combo_packet(self, user_id: texto,
        ...
        return packet;
    fn _register(self, packet: OpenDataPacket) -> None {
        self.packets_sent += 1;
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENDATASTRUCTURE -- PACOTE UNIFICADO DE DADOS");
    println!("  Keyboard + Audio + Webcam em UM pacote compactado");
    println!("=" * 80);
    ...
    s1 = p1.summary();
    println!("  Packet ID: {s1['packet_id']}");
    println!("  Fonte: {s1['sources']}");
    println!("  Texto: '{s1['keyboard_text']}'");
    println!("  Bruto: {s1['raw_size']}");
    println!("  Compactado: {s1['compressed']}");
    println!("  Compressao: {s1['compression']}");
    ...
    s2 = p2.summary();
    println!("  Packet ID: {s2['packet_id']}");
    println!("  Fonte: {s2['sources']}");
    println!("  Transcricao: '{s2['audio_transcription']}'");
    println!("  Duracao: {p2.audio.duration_sec}s");
    println!("  VAD: {p2.audio.vad_detected}");
    println!("  Diarizacao: {p2.audio.diarization}");
    println!("  Ambiente: {p2.audio.ambient_type}");
    println!("  Bruto: {s2['raw_size']}");
    println!("  Compactado: {s2['compressed']}");
    println!("  Compressao: {s2['compression']}");
    ...
    s3 = p3.summary();
    println!("  Packet ID: {s3['packet_id']}");
    println!("  Fonte: {s3['sources']}");
    println!("  Expressao: {s3['webcam_expression']}");
    println!("  Olhar: {p3.webcam.gaze_direction}");
    println!("  Postura: {p3.webcam.posture}");
    println!("  Tela (OCR): {p3.webcam.screen_content}");
    println!("  Bruto: {s3['raw_size']}");
    println!("  Compactado: {s3['compressed']}");
    println!("  Compressao: {s3['compression']}");
    ...
    s4 = p4.summary();
    println!("  Packet ID: {s4['packet_id']}");
    println!("  Fontes: {s4['sources']}");
    println!("  Multimodal: {s4['multimodal']}");
    println!("  Fusao: {s4['fusion']}");
    println!("  Primaria: {s4['primary']}");
    println!("  Texto: '{s4['keyboard_text']}'");
    println!("  Voz: '{s4['audio_transcription']}'");
    println!("  Expressao: {s4['webcam_expression']}");
    println!("  Ambiente: {p4.audio.ambient_type} ({p4.audio.noise_level_db}dB)");
    println!("  Bruto: {s4['raw_size']}");
    println!("  Compactado: {s4['compressed']}");
    println!("  Compressao: {s4['compression']}");
    ...
    if tamanho(json_lines) > 60 {
        println!("  ... ({len(json_lines) - 60} linhas omitidas)");
    // === 6. COMPRESSAO ===
    println!("\n\n  === 6. COMPACTACAO DE DADOS ===\n");
    println!("  {'Tipo':<25} {'Bruto':>15} {'Compactado':>15} {'Ratio'}");
    println!("  {'-'*65}");
    para label, p in [("So teclado", p1), ("So audio", p2), {
                    ("So webcam", p3), ("Combo (3 fontes)", p4)]:;
        println!("  {label:<25} {p.total_raw_size:>14,}B {p.total_compressed_size:>14,}B ";
    ...
    // === FILOSOFIA ===
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA DO OPENDATASTRUCTURE");
    println!("{'='*80}");
    println!(""";
O PROBLEMA:;
...
PRINCIPIOS:;
    ...
    let P3: Compactar dados = eficiencia (menos banda, mais velocidade).;
    P4: Metadados transparentes. Usuario sabe o que envia.;
// )
    println!("{'='*80}");
    println!("  OpenDataStructure: {s['packets_sent']} pacotes enviados. ";
        "Compressao media: {s['avg_compression_ratio']}.");
    println!("  Banda economizada: {s['bandwidth_saved']}.");
    println!("  Keyboard + Audio + Webcam = 1 pacote unificado.");
    println!("{'='*80}");
