...
class EstrategiaAdaptacao(Enum):
...
class NivelAcessibilidadeApp(Enum):
...
class AppDetectado:
    ...
    titulo_janela: str = ""  # "OpenRepublic IDE - main.py"
    tipo: TipoApp = TipoApp.PROPRIETARIO
    nivel_nat: NivelAcessibilidadeApp = NivelAcessibilidadeApp.DESCONHECIDO
    tem_atspi: bool = False
    tem_audio: bool = False
    tem_video: bool = False
    toolkit: str = ""        # "gtk3", "electron", "qt6"


@dataclass
class AdaptacaoAtiva:
    """Uma adaptacao ativa para um app."""
    app_pid: int
    estrategia: EstrategiaAdaptacao
    status: StatusAdaptacao = StatusAdaptacao.ATIVO
    # performance
    cpu_pct: float = 0.0
    ram_mb: float = 0.0
    latencia_ms: float = 0.0
    # resultados
    texto_ocr_ultima_linha: str = ""
    legenda_ultima: str = ""
    # config
    intervalo_ms: int = 500  # quanto tempo entre ciclos de OCR


@dataclass
class PerfilAdaptacao:
    """Perfil do usuario define quais adaptacoes ativar."""
    cego: bool = False
    baixa_visao: bool = False
    surdo: bool = False
    motora: bool = False
    daltonismo: str = ""  # "protanopia", "deuteranopia", "tritanopia"
    cognitiva: bool = False
    # preferencias
    ocr_idioma: str = "por"  # Tesseract lang
    contraste_nivel: str = "alto"  # baixo, medio, alto, maximo
    zoom_nivel: float = 2.0
    legenda_posicao: str = "baixo"


@dataclass
class RegraAdaptacao:
    """Regra: se app e tipo X e usuario e Y, ativar estrategia Z."""
    tipo_app: TipoApp
    deficiencia: DeficienciaAlvo
    estrategia: EstrategiaAdaptacao
    prioridade: int = 5  # 1 (alta) a 10 (baixa)
    condicao: str = ""   # "sempre", "se_sem_atspi", "se_tem_audio"
...
def _init_regras() -> List[RegraAdaptacao]:
...
class AccessibilityShimEngine:
    ...
    def decidir_adaptacoes(self, app: AppDetectado) -> List[EstrategiaAdaptacao]:
        ...
        if self.perfil.daltonismo:
            deficiencias_ativas.append(DeficienciaAlvo.DALTONISMO)
        if self.perfil.cognitiva:
            deficiencias_ativas.append(DeficienciaAlvo.COGNITIVA)

        # encontrar regras aplicaveis
        regras_match = [
            r for r in self.regras
            if r.tipo_app == app.tipo and r.deficiencia in deficiencias_ativas
        ]

        # ordenar por prioridade
        regras_match.sort(key=lambda r: r.prioridade)

        for regra in regras_match:
            # verificar condicao
            if regra.condicao == "se_sem_atspi" and app.tem_atspi:
                continue  # app tem AT-SPI, nao precisa de shim
            if regra.condicao == "se_tem_audio" and not app.tem_audio:
                continue  # app nao tem audio, nao precisa de legenda
            if regra.condicao == "se_baixo_contraste":
                pass  # simplificado: sempre ativa se baixa visao
            if regra.condicao == "se_fonte_pequena":
                pass  # simplificado

            if regra.estrategia not in estrategias:
    ...
    def ativar_adaptacao(self, app: AppDetectado) -> List[AdaptacaoAtiva]:
        """Ativa as adaptacoes decididas para o app."""
        estrategias = self.decidir_adaptacoes(app)
        adaptacoes: List[AdaptacaoAtiva] = []

        for est in estrategias:
            # simular custo de cada estrategia
            cpu, ram, lat = self._estimar_custo(est)
    ...
    def _estimar_custo(self, est: EstrategiaAdaptacao) -> Tuple[float, float, float]:
        ...
        return custos.get(est, (5.0, 50.0, 50.0))

    def _intervalo_recomendado(self, est: EstrategiaAdaptacao) -> int:
        ...
        return intervalos.get(est, 500)

    # -- OCR Bridge (simulado) ---------------------------------------------

    def ocr_extrair_texto(self, app: AppDetectado) -> List[str]:
        """
        ...
        return textos_simulados.get(app.nome.lower(), [
    ...
    def caption_extrair_legenda(self, app: AppDetectado) -> str:
        """
        ...
        return legendas.get(app.nome.lower(), "[audio do app detectado, transcrevendo...]")

    # -- relatorio ----------------------------------------------------------

    def relatorio_adaptacoes(self) -> Dict[str, Any]:
        """Gera relatorio de todas as adaptacoes ativas."""
        total_cpu = 0.0
        total_ram = 0.0
        por_estategia: Dict[str, int] = defaultdict(int)

        for pid, adaptacoes in self.adaptacoes_ativas.items():
            for a in adaptacoes:
                total_cpu += a.cpu_pct
                total_ram += a.ram_mb
...
def _demo() -> None:
    ...
    for est in EstrategiaAdaptacao:
        cpu, ram, lat = e._estimar_custo(est)
    ...
    e.configurar_perfil(cego=True, baixa_visao=True)
    ...
    discord = e.registrar_app(
        ...
        titulo="Discord - #geral", tem_atspi=False, tem_audio=True,
        toolkit="electron"
    )
    ...
    wine_app = e.registrar_app(
        ...
        titulo="Photoshop CS6 (Wine)", tem_atspi=False,
        toolkit="wine"
    )
    ...
    gtk_app = e.registrar_app(
        ...
        titulo="Calculadora", tem_atspi=True, toolkit="gtk3"
    )
