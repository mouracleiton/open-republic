...
class CategoriaDispositivo(Enum):
...
class NivelCusto(Enum):
    ...
    @property
    def min_real(self) -> int:
        return self.value[2]

    @property
    def max_real(self) -> int:
        return self.value[3]


class StatusSpec(Enum):
...
class Especificacao:
    ...
    justificativa: str = ""  # por que esta spec importa para acessibilidade
...
class ProdutoSuportado:
    ...
    status: StatusSpec
    deficiencias_atendidas: List[DeficienciaAlvo] = field(default_factory=list)
...
class SpecDispositivo:
    ...
    specs: List[Especificacao] = field(default_factory=list)
    produtos: List[ProdutoSuportado] = field(default_factory=list)
    custo_minimo_brl: float = 0.0
    observacao: str = ""
...
def _init_specs() -> List[SpecDispositivo]:
    ...
    return [
        ...
        SpecDispositivo(
            ...
            specs=[
                Especificacao("Bluetooth", "5.0 ou superior (LE Audio / aptX Adaptive)",
                              True, "LE Audio reduz latencia e permite multi-stream (2 fones no mesmo dispositivo)."),
                Especificacao("Latencia", "< 200ms (ideal < 150ms)",
                              True, "Latencia alta quebra TTS em tempo real e audio descricao sincronizada."),
                Especificacao("Bateria", "Minimo 8h uso continuo",
                              True, "Dia de trabalho/estudo sem recarregar. Cego depende do fone o dia todo."),
                Especificacao("Microfone integrado", "Sim, com cancelamento de ruido",
                              True, "Para comando de voz, chamadas, e Libras por contexto de audio."),
                Especificacao("Perfil Bluetooth", "A2DP + HFP + AVRCP",
                              True, "A2DP = audio de qualidade. HFP = viva-voz. AVRCP = controle remoto."),
                Especificacao("Multiponto", "Sim (conectar 2 dispositivos simultaneamente)",
                              True, "Cego pode estar conectado ao smartphone E ao computador ao mesmo tempo."),
                Especificacao("Controles fisicos", "Botoes reais (nao touch)",
                              True, "Cego nao pode usar controles capacitivos sem feedback visual."),
                Especificacao("Feedback de bateria", "Anuncio de bateria por voz ou tom",
                              True, "Cego precisa saber quando vai acabar sem ver o LED."),
                Especificacao("Conforto", "Ajustavel, leve (< 300g), alcolchoado",
                              True, "Uso prolongado (8h+) requer conforto. Pessoas com sensibilidade sensorial (TEA) precisam de materiais suaves."),
                Especificacao("Codec", "SBC minimo. AAC/SBC/LDAC desejavel",
                              False, "AAC melhora qualidade para usuarios de iPhone."),
                Especificacao("Resistencia", "IPX4 (suor e chuva)",
                              False, "Para uso em mobilidade urbana."),
                Especificacao("Jack 3.5mm", "Sim (backup com fio)",
                              True, "Se Bluetooth falha, fio e backup. Cego nao pode ficar sem audio."),
            ],
            produtos=[
        ...
        SpecDispositivo(
            ...
            specs=[
                Especificacao("Sistema operacional", "Android 14+ ou iOS 17+",
                              True, "Versoes antigas nao tem features de acessibilidade criticas (Live Listen, Voice Control, Eye Tracking)."),
                Especificacao("RAM", "Minimo 4GB (ideal 6GB+)",
                              True, "Apps de acessibilidade (TalkBack, VoiceOver, Libras) consomem RAM."),
                Especificacao("Armazenamento", "Minimo 64GB",
                              True, "Modelos de IA local + apps de CAA + mapas offline precisam de espaco."),
                Especificacao("Camera", "Minimo 12MP com autofocus + OIS",
                              True, "Visao computacional (OCR, descricao de cena, Libras) precisa de camera decente."),
                Especificacao("Bateria", "Minimo 4000mAh",
                              True, "Dia inteiro de uso com acessibilidade ativa (TTS + GPS + camera)."),
                Especificacao("TalkBack/VoiceOver", "Nativo e funcional",
                              True, "Leitor de tela nativo e OBRIGATORIO. Sem ele, cego nao usa o telefone."),
                Especificacao("Bluetooth", "5.3+ (LE Audio preferivel)",
                              True, "Para conectar headphones, hearing aids, braille displays. LE Audio = multi-stream e baixa latencia."),
                Especificacao("GPS", "Sim, com A-GPS",
                              True, "Navegacao para cego, rastreamento de crianca/idoso."),
                Especificacao("NFC", "Sim",
                              False, "Pagamento sem QR code (OpenAntiQRPayment)."),
                Especificacao("Acelerometro/Giroscopio", "Sim",
                              True, "Deteccao de queda (idoso), bussola para navegacao (cego)."),
                Especificacao("Slot microSD", "Desejavel",
                              False, "Armazenamento expansivel para mapas offline e modelos de IA."),
                Especificacao("Radio FM", "Desejavel (com fone como antena)",
                              False, "Emergencias: FM nao depende de internet."),
            ],
            produtos=[
        ...
        SpecDispositivo(
            ...
            specs=[
                Especificacao("Sensor cardiaco", "Sim (PPG ou ECG)",
                              True, "Deteccao de estresse, arritmia, taquicardia."),
                Especificacao("Acelerometro", "Sim (3 eixos)",
                              True, "Deteccao de queda (idoso), tremor (Parkinson)."),
                Especificacao("GPS", "Sim (integrado)",
                              True, "Rastreamento de crianca/idoso sem depender do smartphone."),
                Especificacao("Bateria", "Minimo 3 dias (ideal 7+)",
                              True, "Biometria continua requer uso 24h. Recarga diaria e falha."),
                Especificacao("Resistencia", "IP68 (agua e poeira)",
                              True, "Usa no banho, na chuva, na piscina. Biometria nao para."),
                Especificacao("Vibracao", "Motor haptico forte",
                              True, "Surdo nao ouve alerta. Cego nao ve. Vibration e universal."),
                Especificacao("Tela sempre ligada", "Desejavel",
                              False, "Idoso precisa ver as horas sem tocar na tela."),
                Especificacao("Temperatura corporea", "Desejavel",
                              False, "Predicao de convulsao epileptica (pesquisa Apple Watch)."),
                Especificacao("SpO2 (oximetria)", "Desejavel",
                              False, "Monitoramento respiratorio."),
            ],
            produtos=[
        ...
        SpecDispositivo(
            ...
            specs=[
                Especificacao("Precisao", "< 1 grau visual (ideal < 0.5)",
                              True, "Para clicar em botoes pequenos na tela. Precisao baixa = frustracao."),
                Especificacao("Frequencia", "Minimo 60Hz (ideal 120Hz+)",
                              True, "Taxa de atualizacao. Baixa = lag entre olhar e clique."),
                Especificacao("Calibracao", "Automatica ou 1-5 pontos",
                              True, "Calibracao complexa (16 pontos) e exaustiva para quem tem mobilidade limitada."),
                Especificacao("Compatibilidade", "Windows + macOS + Linux",
                              True, "Nao pode prender o usuario num SO."),
                Especificacao("Software inclusivo", "Teclado virtual + CAA",
                              True, "Eye tracker sem software de comunicacao e so hardware inutil."),
                Especificacao("Latencia", "< 50ms",
                              True, "Entre o olhar e o cursor responder. Alto = nauseas."),
                Especificacao("Trabalha com oculos", "Sim",
                              True, "Muitos usuarios de eye tracker usam oculos."),
                Especificacao("Headbox", "Minimo 25x25cm",
                              True, "Area de liberdade de movimento da cabeca. Pequeno = usuario precisa ficar imovel."),
            ],
            produtos=[
        ...
        SpecDispositivo(
            ...
            specs=[
                Especificacao("Celas", "Minimo 8 celas (ideal 40+)",
                              True, "8 celas = palavra por vez. 40+ = linha inteira. Mais celas = leitura fluida."),
                Especificacao("Tipo de cela", "Piezoeletrica (8 pinos por cela)",
                              True, "Braille de 8 pontos (computador). 6 pontos so perde informacao de formatacao."),
                Especificacao("Conexao", "Bluetooth + USB",
                              True, "Bluetooth para smartphone. USB para computador. Ambos necessarios."),
                Especificacao("Bateria", "Minimo 10h",
                              True, "Dia de trabalho/estudo. Recarga no meio do dia e falha."),
                Especificacao("Botoes de navegacao", "Sim (pan, rota, cursor routing)",
                              True, "Navegar pelo texto sem depender do smartphone."),
                Especificacao("Compatibilidade", "TalkBack (Android) + VoiceOver (iOS) + NVDA/JAWS (PC)",
                              True, "Display Braille sem leitor de tela e so pinos. Precisa do SOFTWARE."),
            ],
            produtos=[
        ...
        SpecDispositivo(
            ...
            specs=[
                Especificacao("Forca de ativacao", "Maximo 100g (ideal < 50g)",
                              True, "Usuario com paralisia cerebral pode nao conseguir apertar botao duro."),
                Especificacao("Tamanho da superficie", "Minimo 5cm diametro",
                              True, "Alvo grande para quem tem tremor ou movimento involuntario."),
                Especificacao("Feedback", "Tatil (click) + auditivo (click) + visual (LED)",
                              True, "O usuario PRECISA saber que ativou. Multi-modal."),
                Especificacao("Conexao", "Bluetooth e/ou Jack 3.5mm",
                              True, "Jack 3.5mm = compatibilidade universal (AT, switch-adapted toys). BT = sem fio."),
                Especificacao("Resistencia", "A prova de impacto e saliva",
                              True, "Usuario pode ter espasmo. Crianca pode morder/bater."),
                Especificacao("Montagem", "Compativel com braços articulados e suportes",
                              True, "Switch precisa ser posicionado onde o usuario tem movimento."),
                Especificacao("Bateria", "Minimo 100h (ideal: meses)",
                              True, "Switch com fio nao precisa. Switch BT precisa durar semanas."),
            ],
            produtos=[
        ...
        SpecDispositivo(
            ...
            specs=[
                Especificacao("Bluetooth", "5.0+",
                              True, "Conexao estavel para TTS e audio descricao."),
                Especificacao("Orelha aberta", "Sim (nao bloqueia canal auditivo)",
                              True, "Cego que usa conducao ossea PRECISA ouvir o ambiente tambem."),
                Especificacao("Bateria", "Minimo 6h",
                              True, "Dia de uso continuo."),
                Especificacao("Microfone", "Sim",
                              True, "Para comando de voz e chamadas."),
                Especificacao("Conforto", "Ajustavel, leve (< 40g)",
                              True, "Repouso na teca (osso temporal). Pressao inadequada = dor."),
                Especificacao("Resistencia", "IPX4+ (suor)",
                              True, "Uso em atividade fisica e mobilidade urbana."),
            ],
            produtos=[
        ...
        SpecDispositivo(
            ...
            specs=[
                Especificacao("Cancelamento de ruido", "Sim (DSP ou cardiode)",
                              True, "Comando de voz em ambiente ruidoso precisa de CNR."),
                Especificacao("Conexao", "USB ou Bluetooth",
                              True, "USB = zero latencia. BT = mobilidade."),
                Especificacao("Padrao polar", "Cardiode ou unidirecional",
                              True, "Captura a voz do usuario, nao o ambiente inteiro."),
        ...
        SpecDispositivo(
            ...
            specs=[
                Especificacao("Resolucao", "Minimo 720p (ideal 1080p)",
                              True, "Libras requer resolucao suficiente para ver expressoes faciais e maos."),
                Especificacao("Frequencia", "Minimo 30fps (ideal 60fps)",
                              True, "Libras e RAPIDO. 15fps perde sinais. 60fps captura tudo."),
                Especificacao("Autofocus", "Sim (rapido)",
                              True, "Surdo que se move ao sinais nao pode ficar borrado."),
                Especificacao("Campo de visao", "Minimo 70 graus (ideal 90+)",
                              True, "Libras requer espaco para maos e bracos. Webcam estreita corta os sinais."),
                Especificacao("Baixa luz", "Funciona em < 50 lux",
                              True, "Nem todos tem iluminacao de estudio. Sala de casa a noite."),
                Especificacao("Microfone integrado", "Desejavel (backup)",
                              False, "Para videochamada com surdo (audio + Libras simultaneo)."),
            ],
            produtos=[
        ...
        SpecDispositivo(
            ...
            specs=[
                Especificacao("Tela", "Minimo 10 polegadas",
                              True, "Espaco para grade de simbolos CAA (minimo 8x8)."),
                Especificacao("Touch", "Multitoque capacitivo (sensivel)",
                              True, "Usuario com motora fina comprometida precisa de tela responsiva."),
                Especificacao("Bateria", "Minimo 8h",
                              True, "Dia de uso (escola, trabalho, terapia)."),
                Especificacao("Sintetizador de voz", "Nativo + apps CAA",
                              True, "Tablet sem TTS e so uma tela. Precisa FALAR."),
                Especificacao("Software CAA", "Compativel com: Avaz, Proloquo, TD Snap",
                              True, "Apps de CAA sao OBRIGATORIOS. Tablet sem apps CAA nao serve."),
                Especificacao("Case protetor", "Case resistente a queda (military-grade)",
                              True, "Crianca com autismo pode ter comportamento de quebrar. Tablet = R$ 1000+."),
                Especificacao("Suporte", "Compativel com suportes de mesa/mobiliario",
                              True, "Cadeirante precisa do tablet montado na cadeira."),
            ],
            produtos=[
        ...
        SpecDispositivo(
            ...
            specs=[
                Especificacao("Tela", "E-ink (nao LCD/LED)",
                              True, "E-ink nao emite luz azul. Nao cansa. Dislexico e TDAH sao sensivel a luz."),
                Especificacao("Fonte ajustavel", "Sim (tamanho + familia + espacamento)",
                              True, "OpenDyslexic, Atkinson Hyperlegible, espacamento entre linhas."),
                Especificacao("Tamanho de fonte", "Ate minimo 36pt",
                              True, "Baixa visao precisa de fonte grande. E-reader permite sem limite fisico."),
                Especificacao("TTS", "Sim (text-to-speech integrado)",
                              True, "Dislexico que cansa de ler pode OUVIR. Cego pode ouvir o livro todo."),
                Especificacao("Contraste", "Ajustavel (escuro sobre claro, claro sobre escuro)",
                              True, "Modo escuro (fundo preto, texto branco) para baixa visao e sensibilidade a luz."),
                Especificacao("Suporte a formatos", "EPUB, PDF, TXT, HTML",
                              True, "Nao pode prender o usuario num formato proprietario."),
                Especificacao("Bateria", "Minimo 4 semanas",
                              True, "E-ink consome quase nada. Meses de uso sem recarregar."),
            ],
            produtos=[
        ...
        SpecDispositivo(
            ...
            specs=[
                Especificacao("GPS", "Sim (com A-GPS)",
                              True, "Localizacao precisa (< 10m em externo)."),
                Especificacao("Bateria", "Minimo 5 dias (ideal 15+)",
                              True, "Crianca/idoso NAO vai lembrar de recarregar todo dia. Bateria longa e CRITICA."),
                Especificacao("Botao SOS", "Sim (1 toque = alerta)",
                              True, "Crianca aperta = pais recebem localizacao + alerta. Idoso aperta = socorro."),
                Especificacao("Cerca virtual", "Sim (geofencing)",
                              True, "Pais definem area segura. Crianca sai = alerta automatico."),
                Especificacao("Resistencia", "IPX7+ (agua e impacto)",
                              True, "Crianca derruba, molha, perde. Tracker precisa sobreviver."),
                Especificacao("Conectividade", "4G LTE (chip SIM integrado)",
                              True, "Nao depende de WiFi. Funciona em qualquer lugar com sinal celular."),
                Especificacao("Tamanho", "Pequeno e discreto (< 50g)",
                              True, "Crianca nao quer usar algo grande/visivel. Discreto = uso continuo."),
                Especificacao("Audio bidirecional", "Sim (microfone + alto-falante)",
                              False, "Pais podem FALAR com a crianca pelo tracker. Crianca pode chamar."),
            ],
            produtos=[
...
class AccessibilityHardwareSpecEngine:
    ...
    def listar_categorias(self) -> List[CategoriaDispositivo]:
        return [s.categoria for s in self.specs]

    def spec_por_categoria(self, cat: CategoriaDispositivo) -> Optional[SpecDispositivo]:
        for s in self.specs:
            if s.categoria == cat:
                return s
        return None

    def produtos_por_categoria(self, cat: CategoriaDispositivo) -> List[ProdutoSuportado]:
        s = self.spec_por_categoria(cat)
        return s.produtos if s else []

    def produtos_por_deficiencia(self, defic: DeficienciaAlvo) -> List[Tuple[ProdutoSuportado, SpecDispositivo]]:
        """Retorna produtos que atendem uma deficiencia especifica."""
        resultado: List[Tuple[ProdutoSuportado, SpecDispositivo]] = []
        for s in self.specs:
            for p in s.produtos:
                if defic in p.deficiencias_atendidas:
                    resultado.append((p, s))
        return resultado

    def produtos_por_status(self, status: StatusSpec) -> List[ProdutoSuportado]:
        resultado: List[ProdutoSuportado] = []
        for s in self.specs:
            for p in s.produtos:
                if p.status == status:
                    resultado.append(p)
    ...
    def produtos_recomendados(self) -> List[ProdutoSuportado]:
        return self.produtos_por_status(StatusSpec.RECOMENDADO)
    ...
    def verificar_produto(
        ...
        todos_atendidos = all(specs_atendidas.get(spec.parametro, False) for spec in obrigatorios)
        if todos_atendidos:
            # verificar se tambem atende os opcionais
            opcionais = [spec for spec in s.specs if not spec.obrigatorio]
            opc_atendidos = sum(1 for spec in opcionais if specs_atendidas.get(spec.parametro, False))
            if opc_atendidos == len(opcionais) and len(opcionais) > 0:
                return StatusSpec.RECOMENDADO
            return StatusSpec.CONFORME
        # contar quantos obrigatorios falham
        falhas = sum(1 for spec in obrigatorios if not specs_atendidas.get(spec.parametro, False))
        if falhas <= 2:
            return StatusSpec.PARCIAL
        return StatusSpec.NAO_CONFORME

    # -- catalogo por custo -----------------------------------------------

    def catalogo_por_faixa_custo(self) -> Dict[str, List[ProdutoSuportado]]:
        """Agrupa produtos por faixa de custo."""
        resultado: Dict[str, List[ProdutoSuportado]] = defaultdict(list)
        for s in self.specs:
            for p in s.produtos:
    ...
    def _classificar_custo(self, preco: float) -> NivelCusto:
        for n in NivelCusto:
            if n.min_real <= preco <= n.max_real:
                return n
