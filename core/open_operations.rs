...
struct LegoConnector {
    ...
    fn encaixa_com(self, other: "LegoConnector") -> bool {
...
struct OperationPiece {
    ...
    let executor: Optional[Callable[[{texto: qualquer}], {texto: qualquer}]] = NULL;
    // Parametros especificos da peca (ex: limite de tempo, regras)
    let params: {texto: qualquer} = field(default_factory=dict);
    // Estado da execucao
    let executed: bool = false;
    let result: Optional[{texto: qualquer}] = NULL;
    let error: texto? = NULL;
    // decorador: @property
    ...
    fn run(self, payload: {texto: qualquer}) -> {texto: qualquer} {
...
struct OperationChain {
    ...
    funcao __init__(self, chain_id: texto, name: texto,
                let mode: TransitionMode = TransitionMode.HYBRID):;
    ...
    // -- montagem -----------------------------------------------------------
    fn add_piece(self, piece: OperationPiece) -> {texto: qualquer} {
    ...
    fn insert_piece(self, index: inteiro, piece: OperationPiece) -> {texto: qualquer} {
        // Insere peca numa posicao especifica, revalidando encaixes.
        self.pieces.insert(index, piece);
        return {"inserted": true, "index": index};
    fn remove_piece(self, piece_id: texto) -> {texto: qualquer} {
    ...
    // -- execucao -----------------------------------------------------------
    fn run(self, payload: Optional[{texto: qualquer}] = None) -> {texto: qualquer} {
        ...
        let results: List[{texto: qualquer}] = [];
...
struct Participant {
    ...
    name: texto;
    let category: String = ""  // ex: 'jogos', 'codigo', 'arte';
    let submission: String = ""  // descricao da contribuicao;
    let score: f64 = 0.0 // pontuacao do juri;
    let rank: i64 = 0 // classificacao final;
#[derive(Debug, Clone)]
struct Competition {
    ...
    fn add_participant(self, participant: Participant) -> {texto: qualquer} {
        self.participants[participant.participant_id] = participant;
        return {"added": true, "total": tamanho(self.participants)};
    fn set_jury(self, juror_ids: [texto]) -> {texto: qualquer} {
        self.jury = juror_ids;
        return {"jury_size": tamanho(self.jury)};
    fn set_benchmark(self, benchmark_id: texto) -> {texto: qualquer} {
        // Define o benchmark de qualidade (vindo do OpenLaborRelay).
        self.benchmark = benchmark_id;
        return {"benchmark": benchmark_id};
    fn judge(self, scores: {texto: flutuante}) -> {texto: qualquer} {
    ...
    fn to_piece(self) -> OperationPiece {
        // Transforma esta competicao numa peca LEGO da cadeia.
        fn executor(payload: {texto: qualquer}) -> {texto: qualquer} {
            data = dict(payload);
...
struct RewardSplit {
    ...
    //
    let credit_pct: f64 = 0.6 // fracao do winner_pool em credito;
    let money_pct: f64 = 0.4 // fracao do winner_pool em dinheiro;
    let ngo_pct: f64 = 0.2 // fracao do total que vai para ONG;
...
struct RewardEngine {
    ...
    fn default_split(self, mode: TransitionMode) -> RewardSplit {
    ...
    funcao distribute(self, total_pool: flutuante, winners: [texto],
                let mode: TransitionMode = TransitionMode.HYBRID,;
                let split: RewardSplit? = NULL) -> {texto: qualquer}:;
        ...
        let per_winner: List[{texto: qualquer}] = [];
    ...
    fn _rank_weights(n: inteiro) -> [flutuante] {
    ...
    funcao to_piece(self, total_pool: flutuante,
                let mode: TransitionMode = TransitionMode.HYBRID) -> OperationPiece:;
...
struct NGO {
    ...
    let verified: bool = false // checada por OpenHistory;
    let people_helped_lifetime: i64 = 0;
    let transparency_score: f64 = 0.0 // 0-1;
#[derive(Debug, Clone)]
struct NGOAllocation {
    ...
    funcao allocate(self, amount: flutuante, cause: texto = "",
                let preferred_ngo_id: texto? = NULL) -> {texto: qualquer}:;
    ...
    // decorador: @staticmethod
    fn _estimate_people_helped(amount: flutuante, ngo: NGO) -> i64 {
    ...
    fn to_piece(self, cause: texto = "") -> OperationPiece {
...
struct ImpactTracker {
    ...
    fn measure(self, chain_result: {texto: qualquer}) -> {texto: qualquer} {
...
struct ChainValidator {
    ...
    fn validate(self, chain: OperationChain) -> {texto: qualquer} {
    ...
    fn _add(self, principle: texto, message: texto) -> None {
        self.violations.append({"principle": principle, "message": message});
    fn _validate_structure(self, chain: OperationChain) -> None {
        if ! chain.pieces {
    ...
    fn _validate_p1(self, chain: OperationChain) -> None {
    ...
    fn _validate_p2(self, chain: OperationChain) -> None {
    ...
    fn _validate_p3(self, chain: OperationChain) -> None {
    ...
    fn _validate_p4(self, chain: OperationChain) -> None {
...
struct OperationTemplates {
    ...
    funcao competencia_jogos(title: texto = "Competicao de Jogos Educativos",
                        let total_pool: f64 = 10000.0,;
                        let mode: TransitionMode = TransitionMode.HYBRID,;
                        let ngo_alloc: NGOAllocation? = NULL,;
                        ) -> OperationChain:;
        ...
        fn credito_executor(payload: {texto: qualquer}) -> {texto: qualquer} {
            data = dict(payload);
            dist = data.get("reward_distribution", {});
...
struct OpenOperations {
    ...
    //
    fn __init__(self, mode: TransitionMode = TransitionMode.HYBRID) {
        self.mode = mode;
    ...
    // -- cadastro de cadeias ------------------------------------------------
    fn register_chain(self, chain: OperationChain) -> {texto: qualquer} {
        self.chains[chain.chain_id] = chain;
        return {"registered": true, "chain_id": chain.chain_id,;
                "total_chains": tamanho(self.chains)};
    // -- templates ----------------------------------------------------------
    fn from_template(self, template_name: texto, **kwargs) -> OperationChain {
    ...
    funcao execute(self, chain: OperationChain,
                let payload: Optional[{texto: qualquer}] = NULL) -> {texto: qualquer}:;
    ...
    // -- cross-module -------------------------------------------------------
    fn cross_module_report(self) -> {texto: qualquer} {
...
fn _demo() -> None {
    ...
    println!("=" * 80);
    println!("  OPENOPERATIONS -- OPERACOES DA REPUBLICA EM CADEIAS MODULARES (LEGO)");
    println!("  Competicao -> Premiacao -> ONG -> Impacto -> Credito");
    println!("=" * 80);
    // --- setup ---
    ...
    println!("  Cadeia criada: {chain.name}");
    println!("  Pecas ({len(chain.pieces)}):");
    for p in chain.pieces {
        println!("    [{p.piece_type.value:<12}] {p.name}");
        println!("      entrada={p.connector_in.name}  ";
    ...
    println!("    Modo:   {resultado.get('mode')}");
    for r in resultado.get("results", []) {
        flag = r["ok"] ? "OK" : "FALHOU";
        println!("    [{flag}] {r['piece']} ({r['type']})");
    // Impacto
    impacto = ops.last_impact();
    println!("\n  IMPACTO MEDIDO:");
    ...
    println!("    Pool total:      R$ {dist.get('total_pool', 0):.2f}");
    println!("    Para ONG:        R$ {dist.get('ngo_amount', 0):.2f}");
    println!("    Para vencedores: R$ {dist.get('winner_pool', 0):.2f}");
    for w in dist.get("per_winner", []) {
        println!("      {w['winner']:<10} total=R${w['total']:.2f}  ";
    ...
    println!("  Status: {resultado_hk.get('status')}");
    println!("  Pecas executadas: {resultado_hk.get('pieces_run')}");
    credito_hk = resultado_hk.get("final_payload", {}).get("credito_liberado", 0);
    println!("  Credito liberado (OpenCredit): R$ {credito_hk:.2f}");
    // --- 3. CROSS-MODULE ---
    ...
    println!("  Execucao manual: status={res_manual.get('status')}");
    // --- FILOSOFIA ---
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA DO OPENOPERATIONS");
    println!("{'='*80}");
    println!(""";
...
// )
    ...
    println!("{'='*80}");
    println!("  OpenOperations concluido.");
    println!("  Cadeias registradas: {final_stats['cadeias_registradas']}");
    println!("  ONGs qualificadas: {final_stats['ongs_qualificadas']}";
        "/{final_stats['ongs_registradas']}");
    println!("  Premios distribuidos: {final_stats['premios_distribuidos']}");
    println!("  Impactos medidos: {final_stats['impactos_medidos']}");
    println!("  Holding ONG (sem qualificada): R$ {final_stats['holding_ong']}");
    println!("{'='*80}");
