...
class HardwareAvailability(Enum):
...
class HardwareDevice:
    ...
    platforms: List[str] = field(default_factory=list)  # Android, iOS, Linux, Windows, etc
    disabilities_served: List[str] = field(default_factory=list)  # categories served
    input_capabilities: List[str] = field(default_factory=list)  # what it can provide
    output_capabilities: List[str] = field(default_factory=list)  # what it can display
    battery_hours: float = 0.0  # 0 = plugged in
    offline_capable: bool = True  # works without internet
    languages_supported: List[str] = field(default_factory=lambda: ["pt-BR"])
    description: str = ""
...
class HardwareCompatibilityEngine:
    ...
    def find_by_disability(self, disability_category: str) -> List[HardwareDevice]:
        """Encontra dispositivos para uma categoria de deficiencia."""
        return [d for d in HARDWARE_CATALOG if disability_category in d.disabilities_served]

    def find_by_cost(self, max_cost: HardwareCost) -> List[HardwareDevice]:
        ...
        return [d for d in HARDWARE_CATALOG if cost_order.index(d.cost) <= max_idx]

    def find_by_platform(self, platform: str) -> List[HardwareDevice]:
        """Encontra dispositivos para uma plataforma."""
        return [d for d in HARDWARE_CATALOG if platform in d.platforms]

    def find_by_input_capability(self, capability: str) -> List[HardwareDevice]:
        """Encontra dispositivos que oferecem uma capacidade de entrada."""
        return [d for d in HARDWARE_CATALOG if capability in d.input_capabilities]

    def find_by_output_capability(self, capability: str) -> List[HardwareDevice]:
        """Encontra dispositivos que oferecem uma capacidade de saida."""
        return [d for d in HARDWARE_CATALOG if capability in d.output_capabilities]

    def find_offline_capable(self) -> List[HardwareDevice]:
        """Dispositivos que funcionam sem internet."""
        return [d for d in HARDWARE_CATALOG if d.offline_capable]
...
class HardwareBridge:
    ...
    def connect_device(self, device: HardwareDevice) -> bool:
    ...
    def disconnect_device(self, device: HardwareDevice) -> bool:
    ...
    def _update_capabilities(self) -> None:
        ...
        for d in self.connected_devices:
            for cap in d.input_capabilities:
                if cap not in self.active_inputs:
                    self.active_inputs.append(cap)
            for cap in d.output_capabilities:
                if cap not in self.active_outputs:
                    self.active_outputs.append(cap)

    def available_input_modes(self) -> List[str]:
        """Lista modos de entrada disponiveis com hardware atual."""
        modes = set()
        for d in self.connected_devices:
            if "voice" in d.input_capabilities or "microphone" in d.input_capabilities:
                modes.add("voz")
                modes.add("voz_codigo")
            if "touch" in d.input_capabilities or "touch_small" in d.input_capabilities:
                modes.add("toque")
            if "keyboard" in d.input_capabilities:
                modes.add("teclado_completo")
            if "braille_keys" in d.input_capabilities or "braille_input_6_keys" in d.input_capabilities:
                modes.add("teclado_braille")
            if "eye_gaze" in d.input_capabilities:
                modes.add("rastreio_olhos")
            if "single_switch" in d.input_capabilities or "dual_switch" in d.input_capabilities:
                modes.add("chave")
                modes.add("chave_dupla")
            if "trackball" in d.input_capabilities:
                modes.add("trackball")
            if "foot_press_left" in d.input_capabilities:
                modes.add("pedal_pe")
            if "head_stick" in d.input_capabilities:
                modes.add("teclado_cabeca")
            if "sip_puff" in d.input_capabilities:
                modes.add("teclado_boca")
            if "eeg_waves" in d.input_capabilities:
                modes.add("interface_cerebral")
            if "neural_spikes" in d.input_capabilities:
                modes.add("interface_cerebral")
            if "emg_signal" in d.input_capabilities:
                modes.add("eletromiografo")
            if "hand_tracking" in d.input_capabilities:
                modes.add("gesto")
            if "heart_rate" in d.input_capabilities:
    ...
    def available_output_modes(self) -> List[str]:
        ...
        modes = set()
        for d in self.connected_devices:
            if "screen" in d.output_capabilities or "screen_large" in d.output_capabilities:
                modes.add("texto_visual")
            if "screen_tiny" in d.output_capabilities:
                modes.add("texto_tela_pequena")
            if "tts" in d.output_capabilities or "tts_basic" in d.output_capabilities or "speaker" in d.output_capabilities:
                modes.add("texto_para_voz")
            if "braille_cells_40" in d.output_capabilities or "braille_cells_14" in d.output_capabilities:
                modes.add("display_braille")
            if "vibration" in d.output_capabilities or "haptic" in d.output_capabilities:
                modes.add("haptico")
            if "color_light" in d.output_capabilities:
                modes.add("luz_cor")
            if "audio_amplified" in d.output_capabilities:
                modes.add("audio_amplificado")
            if "audio_anc" in d.output_capabilities:
                modes.add("audio_cancelamento_ruido")
            if "hud_overlay" in d.output_capabilities:
                modes.add("hud_oculos")
            if "taptic_engine" in d.output_capabilities:
                modes.add("taptic_preciso")
        return sorted(modes)

    def supports_input_mode(self, mode: str) -> bool:
        return mode in self.available_input_modes()

    def supports_output_mode(self, mode: str) -> bool:
        return mode in self.available_output_modes()
...
def create_setup_budget() -> List[HardwareDevice]:
...
def create_setup_blind() -> List[HardwareDevice]:
...
def create_setup_deaf() -> List[HardwareDevice]:
...
def create_setup_motor_severe() -> List[HardwareDevice]:
...
def create_setup_autism() -> List[HardwareDevice]:
...
def create_setup_adhd() -> List[HardwareDevice]:
    """Setup para TDAH."""
    engine = HardwareCompatibilityEngine()
    return [
        engine.catalog["HW-001"],   # Smartphone
...
def create_setup_epilepsy() -> List[HardwareDevice]:
...
def create_setup_public_terminal() -> List[HardwareDevice]:
...
def create_setup_zero_cost() -> List[HardwareDevice]:
    """Setup ZERO custo."""
    engine = HardwareCompatibilityEngine()
    return [
...
class HardwareEscalationLadder:
    ...
    @classmethod
    def recommend_rung(cls, budget: HardwareCost) -> Tuple[int, str]:
        """Recomenda degrau baseado em orcamento."""
        if budget == HardwareCost.FREE:
            return 0, cls.RUNGS[0][2]
        elif budget in (HardwareCost.VERY_LOW, HardwareCost.LOW):
            return 1, cls.RUNGS[1][2]
        elif budget == HardwareCost.MEDIUM:
            return 2, cls.RUNGS[2][2]
        elif budget == HardwareCost.HIGH:
            return 3, cls.RUNGS[3][2]
        elif budget == HardwareCost.VERY_HIGH:
            return 5, cls.RUNGS[5][2]
        return 0, cls.RUNGS[0][2]

    @classmethod
    def show_ladder(cls) -> None:
...
def demo():
    ...
    for cost, count in sorted(costs.items()):
    ...
    for a, count in sorted(avail.items()):
    ...
    for disab in sorted(all_disabilities):
