...
package openciviceducation_educacao_civica_da_republica
...
type CivicLesson struct {
    ...
    title: texto
    description := "" // string
    activity := ""  // o que fazer para praticar // string
    open_system_link := ""  // sistema da Republica conectado // string
    duration_min := 30 // int64
// Base de licoes
func build_civic_curriculum() [CivicLesson] {
    return [
        // === CRIANCA (6-11) ===
...
const (
    ...
    func label(self) string {
        return self.value[0]
    // decorador: @property
    func score(self) int64 {
        return self.value[1]
    // decorador: @property
    func meaning(self) string {
        return self.value[2]
// decorador: @dataclass
type CivicAssessment struct {
    ...
    ratings := field(default_factory=dict) // {texto: ConductRating}
    overall := ConductRating.ADEQUATE // ConductRating
    needs_accompaniment := false // bool
    notes := "" // string
// ============================================================================
// 4. MOTOR DE EDUCACAO CIVICA
// ============================================================================
type CivicEducationEngine struct {
    ...
    func get_curriculum(self, level: CivicLevel) [Dict] {
    ...
    funcao assess_citizen(self, citizen_id: texto, citizen_name: texto,
                    ratings := nil) -> {texto: qualquer}: // {texto: inteiro}
        ...
        avg_score = soma(r.score para r em conduct_ratings.values()) / maximo(len(conduct_ratings), 1)
        if avg_score >= 4.5 {
        ...
        return result
    func _score_to_rating(self, score: inteiro) ConductRating {
        if score >= 5 {
    ...
    func _intervention_plan(self, rating: ConductRating) string {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENCIVICEDUCATION -- EDUCACAO CIVICA DA REPUBLICA")
    fmt.Println("  'Tratar o outro com dignidade ! && cortesia. E DEVER.'")
    fmt.Println("=" * 80)
    // === 1. OS 12 DEVERES ===
    fmt.Println("\n\n  === 1. OS 12 DEVERES CIVICOS (NAO opcionais) ===\n")
    para cada (i, duty) em enumere(CivicDuty, 1): {
        fmt.Println("  {i:>2}. [{duty.label}]")
        fmt.Println("      {duty.description}")
    // === 2. CURRICULO POR IDADE ===
    fmt.Println("\n\n  === 2. CURRICULO CIVICO POR IDADE ===\n")
    for _, level := range CivicLevel {
        lessons = engine.get_curriculum(level)
        if lessons {
            fmt.Println("\n  {level.value.upper()} ({len(lessons)} licoes):")
            for _, l := range lessons[:3] {
                fmt.Println("    [{l['duty']}] {l['lesson']}")
                fmt.Println("      {l['description'][:60]}...")
                fmt.Println("      Atividade: {l['activity'][:50]}...")
    // === 3. AVALIACAO DE CONVIVENCIA ===
    ...
    para cid, name, ratings in assessments: {
        ...
        fmt.Println("  {r['meaning']}")
        if r["needs_accompaniment"] {
            fmt.Println("  [!] ACOMPANHAMENTO: {r['action'][:70]}...")
        } else {
    ...
    fmt.Println("  Cidadao: {intervention['citizen']}")
    fmt.Println("  Problema: {intervention['issue']}")
    fmt.Println("  Plano: {intervention['plan'][:80]}...")
    fmt.Println("  Sistemas: {', '.join(intervention['systems'][:3])}")
    fmt.Println("  {intervention['message'][:80]}...")
    // === 5. STATS ===
    fmt.Println("\n\n  === 5. ESTATISTICAS ===\n")
    s = engine.stats()
    ...
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  FILOSOFIA DA EDUCACAO CIVICA")
    fmt.Println("{'='*80}")
    fmt.Println("""
"TRATAR O OUTRO COM DIGNIDADE ! && CORTESIA.
&& DEVER CIVILIZATORIO."
...
// )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenCivicEducation: {s['total_deveres']} deveres, "
        "{s['total_licoes']} licoes, "
        "{s['precisam_acompanhamento']} precisam de acompanhamento.")
    fmt.Println("  Tratar o outro bem && DEVER. Nao cortesia.")
    fmt.Println("{'='*80}")
