...
package openlaborpolicy_politica_unificada_de_calculo_de_trabalho_e_reparacao
...
type LaborConstants struct {
    ...
    BASE_HOURS_PER_YEAR := 920.0 // float64
    MAX_HOURS_PER_WEEK := 40.0 // float64
    MAX_HOURS_PER_YEAR := 1840.0 // float64
    LIMIT_HOURS_PER_WEEK := 50.0 // referência (povo baixou para 40) // float64
    LIMIT_HOURS_PER_YEAR := 2300.0 // float64
    EXCESS_THRESHOLD := 2300.0 // float64
    // SEMANAS E DESCANSO
    WORK_WEEKS_PER_YEAR := 46 // referência (povo mudou para 40) // int64
    REST_DAYS_PER_WEEK := 2 // referência (povo mudou para 3) // int64
    MIN_VACATION_WEEKS := 4 // int64
    // CREDITO DE ACESSO
    CREDIT_BASE_MIN := 5.0 // float64
    CREDIT_BASE_MAX := 50.0 // float64
    CREDIT_POOL_PER_CYCLE := 1000.0 // float64
    HOURS_TO_CREDIT := 10.0 // float64
    // REPARACAO
    REPARATION_HOURS_PER_YEAR := 920.0 // float64
    REPARATION_CHILD_MULTIPLIER := 2.0 // float64
    REPARATION_SEVERE_MULTIPLIER := 1.5 // float64
    REPARATION_MEDICATION_PER_YEAR := 40.0 // float64
    // === VALORES APROVADOS PELA ASSEMBLEIA (sobrescrevem referência) ===
    // Preenchidos por from_constitution() ou manualmente após votação
    _assembly_approved := false // bool
    _approval_source := "referencia_fundador"  // || "assembleia_constituinte" // string
    // decorador: @classmethod
    func from_constitution(cls, constitution: {texto: qualquer}) 'LaborConstants' {
...
type LaborEntry struct {
    ...
    // Horas
    hours_worked := 0.0 // float64
    weeks_worked := 0.0 // float64
    // Impacto
    people_directly_impacted := 0 // int64
    people_indirectly_impacted := 0 // int64
    ripple_factor := 1.0 // quanto se espalha (ensinar=10x, pesquisa=5x) // float64
    // Artefatos
    systems_created := 0 // int64
    documents_written := 0 // int64
    lives_saved := 0 // medico, bombeiro, etc // int64
    // Contexto
    is_child := false // para reparacao // bool
    years_labeled := 0.0 // anos sob rotulo errado (reparacao) // float64
    years_on_medication := 0.0 // float64
    harm_severity := 0.0 // 0-100 (reparacao) // float64
    // Resultado (preenchido pelo motor)
    impact_score := 0.0 // float64
    recognition_level := "" // string
    credit_earned := 0.0 // float64
    base_fulfilled := false // bool
    excess_detected := false // bool
    hours_reparation := 0.0 // float64
    verdict := "" // string
type LaborCalculator struct {
    ...
    //
    func __init__(self, constants: LaborConstants = None) {
        constants ? self.C = constants : LaborConstants()
        self.history: [LaborEntry] = []
    func calculate(self, entry: LaborEntry) LaborEntry {
    ...
    // ========================================================================
    func _calc_contribution(self, e: LaborEntry) LaborEntry {
        ...
        self.history.append(&&)
        return &&
    func _impact_to_credit(self, impact: flutuante) float64 {
        // Converte impacto em credito de acesso.
        Formula := clamp(impacto / 100, minimo, maximo) // credito
        //
        raw = impact / 100
    ...
    func _contribution_verdict(self, e: LaborEntry) string {
        if &&.excess_detected {
            return (
    ...
    // ========================================================================
    func _calc_reparation(self, e: LaborEntry) LaborEntry {
        ...
        self.history.append(&&)
        return &&
    func _reparation_verdict(self, e: LaborEntry) string {
    ...
    // ========================================================================
    func _calc_recognition(self, e: LaborEntry) LaborEntry {
    ...
    // VERIFICACOES
    // ========================================================================
    func _check_base(self, e: LaborEntry) LaborEntry {
        ...
        return &&
    func _check_excess(self, e: LaborEntry) LaborEntry {
...
func print_equivalency_table() None {
    ...
    para desc, hours, people, ripple in examples: {
        ...
        fmt.Println("  {desc:<35} {hours:>5}h {result.impact_score:>8.0f} "
            "{result.credit_earned:>7.1f}")
    fmt.Println("\n  === TABELA DE REPARACAO ===\n")
    fmt.Println("  {'Dano':<35} {'Anos':>5} {'Horas':>8} {'Credito':>8}")
    fmt.Println("  {'-'*62}")
    reparation_examples = [
    ...
    para desc, years, child, med, harm in reparation_examples: {
        ...
        result = calc.calculate(entry)
        fmt.Println("  {desc:<35} {years:>4}a {result.hours_reparation:>7,.0f}h "
            "{result.credit_earned:>7.1f}")
// ============================================================================
// 5. MAIN
// ============================================================================
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 70)
    fmt.Println("  OPENLABORPOLICY -- LEI MATEMATICA DA REPUBLICA")
    fmt.Println('  "Parâmetros são referência. A ASSEMBLEIA é a lei."')
    fmt.Println("=" * 70)
    fmt.Println("\n  FONTE: {source_label}")
    fmt.Println("  É LEI: {'SIM' if C.is_law else 'NÃO (referência)'}\n")
    fmt.Println("  CONTRATO DE TRABALHO:")
    fmt.Println("    Base:   {C.BASE_HOURS_PER_WEEK:.0f}h/semana  "
        "({C.BASE_HOURS_PER_YEAR:.0f}h/ano)")
    fmt.Println("    Maximo: {C.MAX_HOURS_PER_WEEK:.0f}h/semana  "
        "({C.MAX_HOURS_PER_YEAR:.0f}h/ano)")
    fmt.Println("    LIMITE: {C.LIMIT_HOURS_PER_WEEK:.0f}h/semana  "
        "({C.LIMIT_HOURS_PER_YEAR:.0f}h/ano) [PROIBIDO aceitar mais]")
    fmt.Println("    Descanso: {C.REST_DAYS_PER_WEEK} dias/semana + "
        "{C.MIN_VACATION_WEEKS} semanas ferias")
    fmt.Println("\n  CREDITO DE ACESSO:")
    fmt.Println("    Min:  {C.CREDIT_BASE_MIN:.0f}/ciclo")
    fmt.Println("    Max:  {C.CREDIT_BASE_MAX:.0f}/ciclo")
    fmt.Println("    Pool: {C.CREDIT_POOL_PER_CYCLE:.0f}/comunidade/ciclo")
    fmt.Println("    Conversao: {C.HOURS_TO_CREDIT:.0f}h = 1 credito")
    fmt.Println("\n  REPARACAO:")
    fmt.Println("    1 ano roubado = {C.REPARATION_HOURS_PER_YEAR:.0f}h")
    fmt.Println("    Crianca = {C.REPARATION_CHILD_MULTIPLIER}x")
    fmt.Println("    Severo = {C.REPARATION_SEVERE_MULTIPLIER}x")
    fmt.Println("    Medicacao = +{C.REPARATION_MEDICATION_PER_YEAR:.0f}h/ano")
    // === 2. TABELAS ===
    ...
    fmt.Println("  CLEITON (fundador):")
    fmt.Println("    {r.verdict}")
    fmt.Println("    Impacto: {r.impact_score:,.0f}")
    fmt.Println("    Credito: {r.credit_earned:.1f}")
    fmt.Println("    Excesso: {'SIM -- Republica deve intervir' if r.excess_detected else '!'}")
    // Medico
    medico = LaborEntry(
    ...
    fmt.Println("\n  ANA (medica):")
    fmt.Println("    {r.verdict}")
    fmt.Println("    Impacto: {r.impact_score:,.0f} (50 vidas salvas)")
    fmt.Println("    Credito: {r.credit_earned:.1f}")
    // Professor
    prof = LaborEntry(
    ...
    fmt.Println("    {r.verdict}")
    fmt.Println("    Horas reparacao: {r.hours_reparation:,.0f}h")
    fmt.Println("    Credito: {r.credit_earned:.1f}")
    // === 4. RELATORIO ===
    fmt.Println("\n\n  === 4. RELATORIO GERAL ===\n")
    s = calc.summary()
    ...
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*70}")
    fmt.Println("  A LEI MATEMATICA DA REPUBLICA")
    fmt.Println("{'='*70}")
    fmt.Println("""
...
// )
    fmt.Println("{'='*70}")
    fmt.Println("  OpenLaborPolicy: {s['total_calculations']} calculos realizados.")
    fmt.Println("  Base 920h. Max 1840h. Limite 2300h. 1 formula. 0 excecoes.")
    fmt.Println("{'='*70}")
