...
public class OpenxEstrategiaParaXTwitter {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENX -- ESTRATEGIA PARA X/TWITTER");
        System.out.println("  'Um canal. Opensoftware para estender. Sem prisao.'");
        System.out.println("=" * 70);
        /* TODO: for-each Java para strat em XStrategy */
            System.out.println("\n  === {strat.value.upper()} ===\n");
            items = [x para x em X_STRATEGY_PLAN if x.strategy == strat];
            items.sort(key=(x) -> x.priority);
            /* TODO: for-each Java para x em items */
                System.out.println("  [{x.id}] P{x.priority} {x.title}");
                System.out.println("    {x.description}");
                if (x.estimated_effort) {
                    System.out.println("    Esforco: {x.estimated_effort}");
                System.out.println();
        System.out.println("\n{'='*70}");
        System.out.println("  {len(X_STRATEGY_PLAN)} acoes. 3 estrategias. 1 canal.");
        System.out.println("  X: usar, estender, libertar. Nunca depender.");
        System.out.println("{'='*70}");
}
