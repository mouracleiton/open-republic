...
struct LegoInput {
    ...
    let default: dyn std::fmt::Debug = NULL;
...
struct LegoBlock {
    ...
    let description: String = "";
    // Conectores
    let inputs: [LegoInput] = field(default_factory=list);
    let outputs: [LegoOutput] = field(default_factory=list);
    // Execucao (funcao real ou descricao para IA gerar)
    let logic: String = ""  // descricao do que faz (IA implementa);
    let code_rust: String = ""  // codigo Rust gerado (producao);
    // Metadata
    let author: String = "";
    let version: String = "1.0.0";
    let tested: bool = false;
    let uses: i64 = 0 // quantas cadeias usam este bloco;
    let color: String = "blue"  // cor visual do bloco (categoria);
    funcao can_connect(self, other: "LegoBlock",
                    let my_output_idx: i64 = 0,;
                    let their_input_idx: i64 = 0) -> logico:;
...
struct LegoProgram {
    ...
    name: texto;
    let description: String = "";
    let blocks: {texto: LegoBlock} = field(default_factory=dict);
    let connections: [LegoConnection] = field(default_factory=list);
    ...
    // decorador: @property
    fn all_inputs_connected(self) -> bool {
...
// ============================================================================
fn build_block_library() -> {texto: LegoBlock} {
...
struct LegoCodeEngine {
    ...
    fn list_blocks(self, category: texto = None) -> [Dict] {
    ...
    funcao add_block(self, program_id: texto, block_id: texto,
                let instance_name: String = "") -> {texto: qualquer}:;
    ...
    fn validate_program(self, program_id: texto) -> {texto: qualquer} {
    ...
    fn _has_cycle(self, prog: LegoProgram) -> bool {
    ...
    funcao execute(self, program_id: texto,
                let initial_values: {texto: qualquer} = NULL) -> {texto: qualquer}:;
    ...
    fn _topo_sort(self, prog: LegoProgram) -> [texto] {
        ...
        return order;
    fn generate_rust(self, program_id: texto) -> {texto: qualquer} {
    ...
    fn ai_generate_block(self, description: texto) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENLEGOCODE -- PROGRAMACAO MODULAR EM FORMATO LEGO");
    println!("  Escrever codigo && coisa do passado. Programa-se ENCAIXANDO.");
    println!("=" * 80);
    ...
    para cada (cat, count) em ordene(cats.items(), key=(x) -> -x[1]): {
        ...
        for b in blocks_in_cat {
            ...
            println!("    [{b.block_id}] {b.name}");
            println!("      Entradas: ({in_types or 'nenhuma'})");
            println!("      Saidas: ({out_types or 'nenhuma'})");
    ...
    println!("  Blocos adicionados: {engine.programs[pid].block_count}");
    ...
    para from_inst, from_out, to_inst, to_in in connections: {
        ...
        println!("  [{status}] {r.get('from', '?')} -> {r.get('to', '?')}");
    // Validar
    println!("\n  Validacao:");
    val = engine.validate_program(pid);
    println!("  Valido: {'SIM' if val['valid'] else 'NAO'}");
    println!("  Blocos: {val['blocks']}, Conexoes: {val['connections']}");
    if val["issues"] {
        println!("  Issues: {val['issues']}");
    ...
    println!("  Valido: {'SIM' if val2['valid'] else 'NAO'}");
    println!("  Blocos: {val2['blocks']}, Conexoes: {val2['connections']}");
    ...
    for desc in ai_blocks {
        ...
        println!("  [{r['block_id']}] {r['name']}");
        println!("    Logica: {r['logic'][:60]}...");
        println!("    Saida: {r['output_type']}");
    ...
    println!("  Tentativa: texto -> soma");
    println!("  Resultado: {r.get('error', 'conectado')}");
    println!("  Mensagem: {r.get('message', '')}");
    println!("\n  LEGO: pino texto NAO encaixa em buraco numero.");
    println!("  Sistema IMPEDE erro de tipo antes de executar.");
    // === 6. GERAR CODIGO RUST ===
    println!("\n\n  === 6. GERAR CODIGO RUST DO PROGRAMA ===\n");
    rust = engine.generate_rust(pid);
    println!("  Programa: {rust['program']}");
    println!("  Linguagem: {rust['language']}");
    println!("  Linhas: {rust['lines']}");
    ...
    // === FILOSOFIA ===
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA: PROGRAMACAO EM LEGO");
    println!("{'='*80}");
    println!(""";
ESCREVER CODIGO && COISA DO PASSADO:;
...
// )
    println!("{'='*80}");
    println!("  OpenLegoCode: {s['total_blocos_biblioteca']} blocos na biblioteca, ";
        "{s['total_programas']} programas montados.");
    println!("  Escrever codigo && coisa do passado. Encaixa pecas.");
    println!("{'='*80}");
