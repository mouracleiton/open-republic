...
#ifndef OPENNIGHTLIFE_OPENESTABLISHMENTUPGRADE_H
#define OPENNIGHTLIFE_OPENESTABLISHMENTUPGRADE_H
...
typedef struct NightEvent {
    ...
    char* organizer = ""  // quem organiza (rotativo);
    int max_attendees = 200;
    int current_attendees = 0;
    char* description = "";
    bool has_food = true;
    bool has_music = true;
    bool has_drinks = true // agua/cha/suco (! alcool);
    [NightSafety] safety = field(default_factory=list);
    bool family_friendly = true;
    double cost = 0.0 // SEMPRE ZERO;
    bool alcohol_allowed = false;
    bool broadcast_on_tv = false // transmitido na OpenTV;
    // decorador: @property
    bool is_late_night(self) {
        return self.end_hour > 1 || self.end_hour < 6;
// decorador: @dataclass
typedef struct NightZone {
    ...
    bool has_food = true;
    bool has_transport = true // OpenMobility carona a noite;
typedef struct NightLifeEngine {
    ...
    funcao create_event(self, name: texto, etype: NightEventType,
                    ...
                    int max_att = 200,;
                    bool family = true,;
                    bool broadcast = false) -> {texto: qualquer}:;
        // Cria evento noturno.
    ...
    [Dict] nightly_schedule(self, day: texto = "sexta") {
        // Grade de eventos noturnos para um dia.
        schedule = [];
        /* TODO: iterador C manual para event em self.events.values() */
            schedule.append({
                "name": event.name,;
    ...
    {texto: qualquer} night_safety_report(self) {
        return {;
            "total_events": sizeof(self.events),;
...
// ============================================================================
typedef struct EstablishmentStatus {
...
typedef struct Establishment {
    ...
    char* upgrade_status = "pendente"  // pendente, em_andamento, concluido;
typedef struct EstablishmentUpgrader {
    ...
    {texto: qualquer} assess(self, est: Establishment) {
    ...
    char* _benefit_description(self, est: Establishment) {
        if (est.type == "consultorio_dental") {
    ...
    {texto: qualquer} execute_upgrade(self, est_id: texto) {
    ...
    {texto: qualquer} batch_upgrade_report(self) {
        total = sizeof(self.establishments);
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENNIGHTLIFE + OPENESTABLISHMENTUPGRADE");
    printf("  Noite comunitaria + modernizacao com custo minimo");
    printf("=" * 80);
    // === PARTE 1: NIGHTLIFE ===
    printf("\n\n  {'='*40}");
    printf("  PARTE 1: OPENNIGHTLIFE");
    printf("  {'='*40}\n");
    // Criar eventos
    events_data = [;
    ...
    /* para name, etype, venue, start, end, org, max_att, family in events_data: */
        ...
        printf("  [{result['time']}] {result['event']:<40} ";
            "{result['venue']:<20} {result['cost']}");
    // Simular comparecimento
    /* TODO: iterador C manual para event em nightlife.events.values() */
    ...
    // Grade noturna
    printf("\n\n  === GRADE NOTURNA ===\n");
    printf("  {'Horario':<14} {'Evento':<40} {'Local':<22} {'Publico':<10} {'TV'}");
    printf("  {'-'*90}");
    /* TODO: iterador C manual para ev em nightlife.nightly_schedule() */
        tv = ev["on_tv"] ? "SIM" : "";
        printf("  {ev['time']:<14} {ev['name']:<40} {ev['venue']:<22} ";
            "{ev['attendees']:<10} {tv}");
    // Seguranca
    printf("\n\n  === SEGURANCA NOTURNA ===\n");
    ...
    printf("  {'Aspecto':<25} {'Balada atual':<25} {'Republica':<25}");
    printf("  {'-'*65}");
    comparisons = [;
    ...
    /* para aspect, balada, republica in comparisons: */
        printf("  {aspect:<25} {balada:<25} {republica:<25}");
    // === PARTE 2: ESTABLISHMENT UPGRADE ===
    printf("\n\n  {'='*40}");
    printf("  PARTE 2: MODERNIZACAO DE ESTABELECIMENTOS");
    printf("  {'='*40}\n");
    ...
    /* TODO: iterador C manual para est em establishments */
        upgrader.establishments[est.est_id] = est;
    // Avaliar necessidades
    printf("\n  === AVALIACAO DE NECESSIDADES ===\n");
    printf("  {'Estabelecimento':<30} {'Status':<15} {'Horas':>6} {'Custo':<10} {'Beneficio'}");
    printf("  {'-'*95}");
    /* TODO: iterador C manual para est em establishments */
        assessment = upgrader.assess(est);
        printf("  {est.name:<30} {est.status.value:<15} ";
    ...
    /* TODO: iterador C manual para est em establishments */
        ...
        printf("    Status: {result['new_status']}");
        printf("    Capacidades novas: {', '.join(result['new_capabilities'][:4])}");
        printf("    Trabalho investido: {result['work_invested']:.0f}h");
        printf("    Custo dinheiro: {result['money_cost']}");
        printf("    {result['message'][:70]}");
    // Relatorio
...
// )
    ...
    printf("  NightLife: {nl_stats['total_events']} eventos, ";
        "{nl_stats['events_family_friendly']} familiares, ";
        "{nl_stats['events_no_alcohol']} sem alcool.");
    printf("  Upgrade: {up_stats['modernized']}/{up_stats['total_establishments']} ";
        "modernizados. Custo: ZERO.");
    printf("{'='*80}");

#endif // OPENNIGHTLIFE_OPENESTABLISHMENTUPGRADE_H
