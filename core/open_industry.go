...
package openindustry_copiar_melhorar_e_criar_marcas_nacionais_superiores
...
type NationalProduct struct {
    ...
    // Comparação
    foreign_price := "R$ caro"  // preço estrangeiro // string
    national_price := "ZERO"  // preço na República // string
    foreign_quality := 8 // 1-10 // int64
    national_quality := 9 // 1-10 // int64
    // Vantagens
    advantages := field(default_factory=list) // [AdvantageType]
    why_better := "" // string
    // Produção
    status := CopyStatus.IDENTIFIED // CopyStatus
    fablab_capable := true // FabLab nacional produz? // bool
    materials_national := 0.8 // % de material nacional // float64
    production_capacity := ""  // escala // string
    // Especificações
    specs := field(default_factory=dict) // {texto: texto}
// ============================================================================
...
type IndustryEngine struct {
    ...
    func list_by_category(self, category: IndustryCategory = None) [Dict] {
    ...
    func compare(self, product_id: texto) {texto: qualquer} {
    ...
    func advance_status(self, product_id: texto) {texto: qualquer} {
    ...
    func dependency_report(self) {texto: qualquer} {
        // Relatório de dependência externa (o que ainda importamos).
        by_country = Counter()
        by_category = Counter()
        for _, p := range self.products.values() {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENINDUSTRY -- COPIAR, MELHORAR, SUPERAR")
    fmt.Println("  Brasil copia tudo. Cria marcas nacionais. Qualidade SUPERIOR.")
    fmt.Println("=" * 80)
    // === 1. PRODUTOS NACIONAIS ===
    fmt.Println("\n\n  === 1. PRODUTOS NACIONAIS ({len(engine.products)}) ===\n")
    fmt.Println("  {'Nacional':<15} {'Estrangeiro':<25} {'Preço Ext':<18} {'Preço Nac':<10} {'Status'}")
    fmt.Println("  {'-'*85}")
    for _, p := range engine.list_by_category() {
        fmt.Println("  {p['nacional']:<15} {p['estrangeiro'][:24]:<25} "
    ...
    fmt.Println("  NACIONAL: {comp['nacional']} (qualidade {comp['qualidade_nacional']})")
    fmt.Println("  ESTRANGEIRO: {comp['estrangeiro']} ({comp['empresa_estrangeira']}, {comp['pais']})")
    fmt.Println("  (qualidade {comp['qualidade_estrangeiro']})")
    fmt.Println("\n  PREÇO:")
    fmt.Println("    iPhone: {comp['preco_estrangeiro']}")
    fmt.Println("    OpenPhone: {comp['preco_nacional']}")
    fmt.Println("\n  POR QUE O NACIONAL É SUPERIOR:")
    fmt.Println("    {comp['por_que_melhor']}")
    ...
    fmt.Println("  NACIONAL: {comp2['nacional']}")
    fmt.Println("  ESTRANGEIRO: {comp2['estrangeiro']} ({comp2['pais']})")
    fmt.Println("  PREÇO: {comp2['preco_estrangeiro']} vs {comp2['preco_nacional']}")
    fmt.Println("  POR QUE MELHOR: {comp2['por_que_melhor']}")
    // === 4. COMPARAÇÃO: OpenTrator vs John Deere ===
    fmt.Println("\n\n  === 4. COMPARAÇÃO: OpenTrator vs John Deere ===\n")
    comp3 = engine.compare("IND-TR")
    fmt.Println("  NACIONAL: {comp3['nacional']}")
    fmt.Println("  ESTRANGEIRO: {comp3['estrangeiro']} ({comp3['pais']})")
    fmt.Println("  POR QUE MELHOR: {comp3['por_que_melhor']}")
...
// )
    ...
    fmt.Println("  Total de produtos: {dep['total_produtos']}")
    fmt.Println("  Já superiores: {dep['ja_superiores']}")
    fmt.Println("  Em produção: {dep['em_producao']}")
    fmt.Println("\n  Dependência por país:")
    para cada (country, count) em dep["dependencia_por_pais"].items(): {
        fmt.Println("    {country:<15} {count} produtos")
    fmt.Println("\n  Dependência por setor:")
    para cada (cat, count) em dep["dependencia_por_setor"].items(): {
        fmt.Println("    {cat:<20} {count} produtos")
    // === 7. STATUS ===
    fmt.Println("\n\n  === 7. DISTRIBUIÇÃO DE STATUS ===\n")
    s = engine.stats()
    para cada (status, count) em s["status_distribution"].items(): {
        bar = "#" * count
        fmt.Println("  {status:<25} {count:>2} {bar}")
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  FILOSOFIA DO OPENINDUSTRY")
    fmt.Println("{'='*80}")
    fmt.Println("""
...
PRINCÍPIOS:
    ...
    P3 := trabalho de alto impacto. // Fabricar nacional
    P4: Quais produtos priorizar? Assembleia vota.
// )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenIndustry: {s['total_produtos']} produtos em {s['categorias_cobertas']} setores. "
        "Custo: {s['custo_ao_cidadao']}.")
    fmt.Println("  Copiar. Melhorar. Superar. Tudo nacional. Tudo CC0.")
    fmt.Println("{'='*80}")
