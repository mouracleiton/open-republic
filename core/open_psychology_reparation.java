...
public class OpenpsychologyreparationReavaliacaoEReparacaoDeDiagnosticos {
    ...
    sofrimento por erro P3 = TRABALHO ROUBADO (anos de vida perdidos);
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class DiagnosisCase {
        // Um caso de diagnostico a ser reavaliado.
        case_id: texto;
        citizen_id: texto;
        citizen_name: texto;
        original_diagnosis: texto // diagnostico original;
        String diagnosing_doctor = "";
        String diagnosing_institution = "";
        String diagnosis_date = ""  // quando foi diagnosticado;
        int diagnosis_age = 0 // idade na epoca;
        int current_age = 0;
        String medication_prescribed = "";
        double years_on_medication = 0.0;
        double years_labeled = 0.0 // anos vivendo com o rotulo;
        // Reavaliacao
        ErrorType? error_type = null;
        [HarmCategory] harm_categories = field(default_factory=list);
        double harm_severity_score = 0.0 // 0-100;
        Liable liable = Liable.INDIVIDUAL_DOCTOR;
        // Reparacao
        double reparation_work_hours = 0.0 // horas de trabalho reconhecidas como reparacao;
        double reparation_credit = 0.0 // credito de acesso;
        boolean apology_required = true;
        boolean public_record_correction = true // apagar rotulo do prontuario;
        // Status
        String status = "pendente"  // pendente, reavaliado, reparado;
        [texto] reavaliated_by = field(default_factory=list);
        String notes = "";
        // decorador: @property
        public boolean was_child(self) {
            return self.diagnosis_age > 0 && self.diagnosis_age < 18;
        // decorador: @property
        public String harm_description(self) {
            descs = {
    ...
    // ============================================================================
    public static class DiagnosisReassessment {
        ...
        public {texto: qualquer} submit_case(self, case: DiagnosisCase) {
        ...
        public double _calculate_harm(self, case: DiagnosisCase) {
            ...
            /* TODO: for-each Java para h em case.harm_categories */
        ...
        public void _calculate_reparation(self, case: DiagnosisCase) retorna (flutuante, flutuante) {
            ...
            return hours, credit;
        public String _verdict(self, case: DiagnosisCase) {
            // Veredicto textual.
    ...
    // ============================================================================
    public static class SufferingCompensation {
        ...
        public {texto: qualquer} process_compensation(self, case_id: texto) {
            ...
            return compensation;
        public String _who_apologizes(self, case: DiagnosisCase) {
        ...
        public {texto: texto} _support_plan(self, case: DiagnosisCase) {
    ...
    // ============================================================================
    public DiagnosisReassessment _seed_cases() {
        ...
        /* TODO: for-each Java para case em cases */
            r.submit_case(case);
        return r;
    // ============================================================================
    // 6. MAIN
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENPSYCHOLOGYREPARATION");
        System.out.println("  Reavaliacao de Diagnosticos + Pagamento por Sofrimento");
        System.out.println("=" * 80);
        // === 1. CASOS SUBMETIDOS ===
        System.out.println("\n\n  === 1. CASOS PARA REAVALIACAO: {len(r.cases)} ===\n");
        System.out.println("  {'ID':<10} {'Cidadao':<14} {'Diagnostico':<20} ";
            "{'Idade':>6} {'Anos rotulo':>11} {'Medicado'}");
        System.out.println("  {'-'*80}");
        /* TODO: for-each Java para c em r.cases.values() */
            System.out.println("  {c.case_id:<10} {c.citizen_name:<14} ";
        ...
        r.reassess("CASE-006", ErrorType.CHILD_LABEL,;
                ...
                "Infancia roubada.");
        /* TODO: for-each Java para cid em r.cases */
            c = r.cases[cid];
            System.out.println("\n  [{c.case_id}] {c.citizen_name}");
            System.out.println("  Diagnostico original: {c.original_diagnosis}");
            System.out.println("  Erro: {c.error_type.value}");
            System.out.println("  Dano: {c.harm_severity_score:.0f}/100");
            System.out.println("  Responsavel: {c.liable.value}");
            System.out.println("  Danos: {c.harm_description}");
            System.out.println("  VEREDICTO: {r._verdict(c)}");
        // === 3. COMPENSACAO ===
        ...
        /* TODO: for-each Java para cid em r.cases */
            ...
            System.out.println("  Diagnostico: {result['diagnosis']}");
            comp_components = result["components"];
            System.out.println("  Trabalho reconhecido: ";
                "{comp_components['trabalho_reconhecido_horas']:.0f}h ";
                "({comp_components['trabalho_reconhecido_anos']:.1f} anos)");
            System.out.println("  Credito de acesso: {comp_components['credito_acesso']}");
            System.out.println("  Retificacao: {comp_components['retificacao_prontuario'][:60]}");
            System.out.println("  Desculpas: {comp_components['pedido_desculpas'][:80]}");
            plan = comp_components['apoio_continuo'];
            System.out.println("  Apoio: {', '.join(plan.keys())}");
        // === 4. RELATORIO GERAL ===
        System.out.println("\n\n  === 4. RELATORIO GERAL ===\n");
        rep = r.report();
        System.out.println("  Casos reavaliados:     {rep['reassessed']}");
        System.out.println("  Diagnosticos revogados:{rep['overturned']}");
        System.out.println("  Dano medio:            {rep['avg_harm_score']:.0f}/100");
        System.out.println("  Reparacao total:       {rep['total_reparation_hours']:,.0f}h ";
            "({rep['total_reparation_hours']/920:.0f} anos de trabalho)");
        System.out.println("\n  Por tipo de erro:");
        /* para err, count in ordene(rep["by_error_type"].items(), */
                                key = (x) -> -x[1]):;
            System.out.println("    {err:<25} {count}");
        System.out.println("\n  Por responsavel:");
        /* para liable, count in ordene(rep["by_liable"].items(), */
                                    key = (x) -> -x[1]):;
            System.out.println("    {liable:<25} {count}");
        System.out.println("\n  Por tipo de dano:");
        /* para harm, count in ordene(rep["by_harm"].items(), */
                                key = (x) -> -x[1]):;
            System.out.println("    {harm:<25} {count}");
        // === FILOSOFIA ===
        System.out.println("\n\n{'='*80}");
        System.out.println("  POLITICA DE REPARACAO PSICOLOGICA");
        System.out.println("{'='*80}");
        System.out.println(""";
    ...
    // )
        System.out.println("{'='*80}");
        System.out.println("  Reavaliados: {rep['reassessed']} | ";
