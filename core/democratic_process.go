...
package openrepublic_principio_democracia_nao_elitismo
...
type Proposal struct {
    ...
    ptype: ProposalType
    // Impacto
    affects_sectors := field(default_factory=list) // [texto]
    affects_nations := field(default_factory=list) // [texto]
    affects_population := 0 // int64
    // Processo democratico
    status := ProposalStatus.DRAFT // ProposalStatus
    submitted_at := 0.0 // float64
    debate_duration_days := 7 // minimo 7 dias de debate // int64
    voting_duration_days := 3 // minimo 3 dias de votacao // int64
    // Votacao
    votes_for := 0 // int64
    votes_against := 0 // int64
    votes_abstain := 0 // int64
    total_eligible := 0 // int64
    // Debate
    debate_comments := field(default_factory=list) // [Dict]
    concerns_raised := field(default_factory=list) // [texto]
    amendments := field(default_factory=list) // [texto]
    // Verificacao
    anti_elitism_check := false // passou verificacao anti-elitismo // bool
    co_created := false // mais de 1 pessoa participou da criacao // bool
type DemocraticProcess struct {
    ...
    funcao submit_proposal(self, title: texto, description: texto,
                        ...
                        ptype: ProposalType,
                        affects_sectors := nil, // [texto]
                        affects_population := 1000) -> Proposal: // int64
    ...
    func start_voting(self, proposal_id: texto) {texto: qualquer} {
    ...
    func cast_vote(self, proposal_id: texto, vote: texto) {
    ...
    funcao tally(self, proposal_id: texto,
            constitutional := false) -> {texto: qualquer}: // bool
        // Apurar votacao.
        p = self.proposals.get(proposal_id)
        if ! p {
...
if __name__ == "__main__" {
    fmt.Println("=" * 80)
    fmt.Println("  OPENREPUBLIC -- DEMOCRACIA, NAO ELITISMO")
    fmt.Println("  'Ninguem decide por todos. Todos decidem por todos.'")
    fmt.Println("=" * 80)
    ...
    fmt.Println("  Proposta: {prop.proposal_id}")
    fmt.Println("  Titulo: {prop.title}")
    fmt.Println("  Proponente: {prop.proposer_name}")
    fmt.Println("  Status: {prop.status.value}")
    fmt.Println("  Verificacao anti-elitismo: {'PASSOU' if prop.anti_elitism_check else 'FALHOU'}")
    fmt.Println("  Setores afetados: {prop.affects_sectors}")
    // === Debate ===
    ...
    process.add_debate_comment(prop.proposal_id, "C-005", "Kofi",
        ...
        is_concern = false)
    fmt.Println("  Comentarios de debate: {len(prop.debate_comments)}")
    for _, c := range prop.debate_comments {
        flag = c["is_concern"] ? "[CONCERNO]" : "[APOIO]"
        fmt.Println("    {flag} {c['citizen']}: {c['comment'][:70]}...")
    ...
    fmt.Println("  Proposta: {result['proposal']}")
    ...
    fmt.Println("  Se o proponente diz 'eu decidi construir X':")
    fmt.Println("    -> Proposta REJEITADA por anti-democratica.")
    fmt.Println("    -> Ninguem decide. Propoe.")
    fmt.Println("    -> O coletivo decide.")
    fmt.Println("  Se o proponente diz 'eu proponho construir X':")
    fmt.Println("    -> Proposta ACEITA para debate.")
    fmt.Println("    -> O coletivo debate.")
    fmt.Println("    -> O coletivo vota.")
    fmt.Println("    -> O coletivo decide.")
    // === Case 3: Emergency ===
    ...
    fmt.Println("  Proposta: {emergency.proposal_id}")
    fmt.Println("  Tipo: {emergency.ptype.value}")
    fmt.Println("  Origem: {emergency.origin.value}")
    fmt.Println("  Processo: ACELERADO (votacao em 24h)")
    fmt.Println("  Auto-expira em 30 dias se ! revalidada")
    // === Constitution ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  PRINCIPIO CONSTITUCIONAL: DEMOCRACIA > IDEIA")
    fmt.Println("{'='*80}")
    fmt.Println("""
PRINCIPIO CONSTITUCIONAL NUMERO 1:
    "Nenhuma ideia, projeto, sistema || mudanca entra na Republica
