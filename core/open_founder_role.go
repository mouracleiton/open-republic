...
package openfounderrole_papel_constitucional_do_fundador
...
type Deviation struct {
    ...
    correction := "" // string
    correction_speed := CorrectionSpeed.INSTANT // CorrectionSpeed
    resolved := false // bool
    timestamp := field(default_factory=() -> datetime.now().isoformat()) // string
type FounderCorrectionSystem struct {
    ...
    funcao check_proposal(self, proposal_text: texto,
                    proposal_type := "politica") -> {texto: qualquer}: // string
    ...
    func _severity(self, dev_type: DeviationType) int64 {
        return {
    ...
    func _principle(self, dev_type: DeviationType) string {
        return {
    ...
    func _correction(self, dev_type: DeviationType) string {
        return {
            DeviationType.ELITISM: (
    ...
    func appeal(self, deviation_id: texto) {texto: qualquer} {
        // Fundador apela de correção -> assembleia decide.
        dev = next((d para d em self.deviations if d.deviation_id == deviation_id), nil)
        if ! dev {
...
type AssemblyVote struct {
    ...
    votes := field(default_factory=dict) // {texto: inteiro}
    winner := "" // string
    approved := false // bool
func run_founder_role_assembly(n_voters: inteiro = 10000) {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENFOUNDERROLE -- PAPEL CONSTITUCIONAL DO FUNDADOR")
    fmt.Println("  Assembleia Constituinte Especial")
    fmt.Println("=" * 80)
    // === 1. PERGUNTA ===
    fmt.Println("\n\n  === PERGUNTA SUBMETIDA A ASSEMBLEIA ===\n")
    fmt.Println('  "A posicao do fundador pode ser definitiva && sempre')
    fmt.Println('   auxiliada pelo sistema, com correcao automatica de desvios?"')
    fmt.Println("\n  POSICAO DO FUNDADOR:")
    fmt.Println('  "Sim, quero voz permanente + auxilio + correcao quando errar."')
    // === 2. VOTACAO ===
    fmt.Println("\n\n  === VOTACAO (10.000 CIDADAOs) ===\n")
    result = run_founder_role_assembly(10000)
    fmt.Println("  SIM (voz + correcao):    {result['votes_for']:>6} ({result['votes_for']/100:.0f}%)")
    fmt.Println("  PARCIAL (voz, sem prio): {result['votes_partial']:>6} ({result['votes_partial']/100:.0f}%)")
    fmt.Println("  NAO (cidadao comum):     {result['votes_against']:>6} ({result['votes_against']/100:.0f}%)")
    ...
    para cada (key, val) em d.items(): {
        ...
        fmt.Println("  {key:<25} {status}")
    fmt.Println("\n  RESTRICOES INALTERAVEIS:")
    for _, r := range result["restrictions_inalteraveis"] {
        fmt.Println("    {r}")
    ...
    para cada (proposal, expected) em test_proposals: {
        ...
        if check["status"] != "approved" {
            fmt.Println("       Tipo: {check.get('type', '?')}")
            fmt.Println("       Principio: {check.get('principle', '?')}")
            fmt.Println("       Correcao: {check['correction'][:80]}")
        } else {
            fmt.Println("       Conforme. Pode prosseguir para votacao.")
    // === 5. APELO ===
    fmt.Println("\n\n  === DIREITO DE APELO ===\n")
    for _, dev := range corrector.deviations {
        if ! dev.resolved {
            appeal = corrector.appeal(dev.deviation_id)
            fmt.Println("  Fundador apela correcao: {dev.deviation_type.value}")
            fmt.Println("  Resultado: {appeal['appeal_result']}")
            fmt.Println("  {appeal['votes']}")
    ...
    // === RESPOSTA FINAL ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  RESPOSTA A PERGUNTA DO FUNDADOR")
    fmt.Println("{'='*80}")
    fmt.Println("""
PERGUNTA:
...
// )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenFounderRole: voz permanente + auxilio + correcao automatica.")
    fmt.Println("  Fundador propoe. Sistema corrige. Povo decide.")
    fmt.Println("  {'='*80}")
