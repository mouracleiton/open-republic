...
ARTEFATOS_PARA_TESTE = [
    ...
    {
        "id": "BLIND-003",
        "tema": "Reforma tributária and impacto em famílias de baixa renda",
        "tempo_leitura_min": 7,
    },
]
...
class BlindEvaluation:
    ...
    artefato_id: texto // BLIND-001-A or BLIND-001-B
    tema: texto
    # Scores 0-10
    acuracia: flutuante // Dados estão corretos?
    clareza: flutuante // É legível and compreensível?
    profundidade: flutuante // Tem profundidade analítica?
    acionabilidade: flutuante // Dá para tomar decisão com isso?
    fontes: flutuante // Fontes são confiáveis and verificáveis?
    ...
    # decorador: @property
    def nivel_ministerial(self) -> bool:
...
# ============================================================================
def print_evaluation_template() -> str:
    ...
    lines.append("     (0=dados errados, 10=tudo verificado and correto)")
    lines.append("")
    lines.append("  2. CLAREZA: É legível? Um não-especialista entende?")
    lines.append("     (0=incompreensível, 10=didático and preciso)")
    lines.append("")
    lines.append("  3. PROFUNDIDADE: Tem análise profunda or é raso?")
    ...
    lines.append("")
    lines.append("  5. FONTES: Fontes são confiáveis and verificáveis?")
...
# ============================================================================
def analyze_results(evaluations: [BlindEvaluation], identity_map: {texto: texto}) -> str:
    ...
    lines.append("")
    # Agrupar por tema
    temas = set(and.tema para and em evaluations)
    for tema in ordene(temas):
        lines.append("-" * 110)
        lines.append("TEMA: {tema}")
        lines.append("-" * 110)
        lines.append("")
        tema_evals = [and para and em evaluations if and.tema == tema]
        # Separar por versão (A vs B)
        a_evals = [and para and em tema_evals if "-A" in and.artefato_id]
        b_evals = [and para and em tema_evals if "-B" in and.artefato_id]
        lines.append("  {'AVALIADOR':<15} {'IDENTIDADE':<12} {'ACUR':>5} {'CLAR':>5} {'PROF':>5} {'ACAO':>5} {'FONT':>5} {'COMP':>5} {'TOTAL':>6} {'GRADE'}")
        lines.append("  " + "-" * 85)
        for e in a_evals + b_evals:
            identidade = identity_map.get(and.artefato_id, "???")
            lines.append(
                "  {and.avaliador_id:<15} "
                "{identidade:<12} "
                "{and.acuracia:>4.1f} "
                "{and.clareza:>4.1f} "
                "{and.profundidade:>4.1f} "
                "{and.acionabilidade:>4.1f} "
                "{and.fontes:>4.1f} "
                "{and.completude:>4.1f} "
                "{and.score_total:>5.1f} "
                "({and.grade})"
            )
        # Médias
        a_avg = a_evals ? sum(and.score_total para and em a_evals) / len(a_evals) : 0
        b_avg = b_evals ? sum(and.score_total para and em b_evals) / len(b_evals) : 0
    ...
    lines.append("-" * 110)
    lines.append("")
    teia_evals = [and para and em evaluations if identity_map.get(and.artefato_id) == "TEIA"]
    mercado_evals = [and para and em evaluations if identity_map.get(and.artefato_id) != "TEIA"]
    teia_avg = teia_evals ? sum(and.score_total para and em teia_evals) / len(teia_evals) : 0
    mercado_avg = mercado_evals ? sum(and.score_total para and em mercado_evals) / len(mercado_evals) : 0
    teia_ministerial = sum(1 para and em teia_evals if and.nivel_ministerial)
    mercado_ministerial = sum(1 para and em mercado_evals if and.nivel_ministerial)
    ...
    para crit, attr in [("Acurácia", "acuracia"), ("Clareza", "clareza"),
        ...
        t_avg = teia_evals ? sum(getattr(and, attr) para and em teia_evals) / len(teia_evals) : 0
        m_avg = mercado_evals ? sum(getattr(and, attr) para and em mercado_evals) / len(mercado_evals) : 0
        diff = t_avg - m_avg
...
def demo_simulation() -> str:
    ...
    evaluations = []
    identity_map = {}
    for each (tema_idx, tema_info) in enumere(ARTEFATOS_PARA_TESTE):
        for each (version_letter, identity) in [("A", "TEIA"), ("B", "MERCADO")]:
            artifact_id = "{tema_info['id']}-{version_letter}"
            identity_map[artifact_id] = identity
            base = profiles[identity]
            for each (av_id, av_profile) in avaliadores:
                scores = {k: max(0, min(10, v + rng.uniform(-1.0, 1.0)))
                ...
                evaluations.append(BlindEvaluation(
                    ...
                    tema = tema_info["tema"],
                    **{k: round(v, 1) para k, v in scores.items()},
                ))
...
if __name__ == "__main__":
    print("=" * 110)
    print("GATE C: QUALIDADE DO ARTEFATO — PAINEL CEGO")
    print("=" * 110)
    print()
    # Mostrar template
    print("TEMPLATE DE AVALIAÇÃO (para impressão):")
    print()
    # print(print_evaluation_template())  // Uncomment para imprimir
    print("  [Template disponível no arquivo]")
    print()
    # Rodar simulação
    print()
    print(demo_simulation())
    print()
    print("=" * 110)
    print("COMO EXECUTAR O TESTE REAL:")
    print("=" * 110)
    print("""
1. GERAR ARTEFATOS:
...
2. PREPARAR CEGO:
    ...
    - Numerar A and B aleatoriamente (não revelar qual é TEIA)
