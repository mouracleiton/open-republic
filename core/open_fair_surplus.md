...
**Arquivo original:** `core/open_fair_surplus.py`

**Descricao:** ====================================================
DESCOBERTA DO FUNDADOR:
  "Se for definido 5% de excedente em cima da mercadoria,
   voce vai receber 5% de cada uma.
   O que incentiva voce ter MAIS.
   Ou seja: mais gente trabalhando e se sustentando."
O QUE ISTO MUDA:
...
A MATEMATICA:
  ...
  5% de excedente = R$ 200 volta pra ele (ou pra pool de trabalhadores)
  ...
  Ninguem acumula (nao e lucro do dono).
  Todo mundo se sustenta (5% volta em forma de credito/servico).
  DONO NAO EXISTE. O 5% e COLETIVO.
...
O QUE ISTO MUDA:
  CAPITALISMO: excedente de 60-90% vai pro DONO (1 pessoa lucra)
  REPUBLICA EXECUTAVEL: excedente de 5% volta pro TRABALHADOR
  seja INCENTIVO: mais trabalhadores = mais 5% circulando = mais se sustentando
...
classe SurplusFlow herda de Enum:
    // Para onde vai o 5%.
...
classe ScaleIncentive herda de Enum:
...
classe FairSurplusCalc:
    ...
    seja output_value_month: flutuante = 0.0 // valor total produzido/mes
    seja worker_pay_month: flutuante = 0.0 // quanto cada trabalhador recebe

    // Os 5%
    seja surplus_5pct: flutuante = 0.0 // 5% do valor produzido
    seja surplus_per_worker: flutuante = 0.0 // quanto cada trabalhador recebe do 5%
    seja surplus_flow: SurplusFlow = SurplusFlow.COLLECTIVE_POOL

    // Comparacao com predatorio
    seja predatory_90pct: flutuante = 0.0 // o que o dono levaria no capitalismo
    seja worker_gain_vs_predatory: flutuante = 0.0 // quanto o trabalhador GANHA trocando

    // Escala
    seja surplus_at_10_workers: flutuante = 0.0
    seja surplus_at_100_workers: flutuante = 0.0
    seja surplus_at_1000_workers: flutuante = 0.0

    // decorador: @property
    funcao worker_total_month(self) -> flutuante:
        // Quanto o trabalhador recebe no total (salario + 5%).
        retorne self.worker_pay_month + self.surplus_per_worker
...
classe FairSurplusEngine:
    ...
    funcao calculate(self, establishment: texto, est_type: texto,
                  ...
                  seja flow: SurplusFlow = SurplusFlow.COLLECTIVE_POOL
                  ) -> {texto: qualquer}:
        ...
        retorne {
            ...
            "COMPARACAO_PREDATORIA": {
                ...
                "republica_dono_leva": "R$ 0,00 (dono nao existe)",
            ...
            "message": (
                ...
                "5% = R$ {surplus:,.0f} VOLTA (nao vai pra dono). "
    ...
    funcao compare_capitalism_vs_republic(self, output_per_worker: flutuante = 4000.0
        ...
        retorne {
            ...
            "REPUBLICA_EXECUTAVEL": {
                ...
                "dono_levara": "R$ 0,00 (dono nao existe)",
...
se __name__ == "__main__" entao:
    ...
    imprima("=" * 80)
    imprima("  OPENFAIRSURPLUS -- 5% QUE VOLTA PRA QUEM TRABALHA")
    imprima("  Mais gente trabalhando = mais se sustentando")
    imprima("=" * 80)
...
// )
    ...
    imprima("  Producao por trabalhador: {scale['output_por_trabalhador']}")
    imprima("  Surplus: {scale['surplus_pct']}\n")
    imprima("  {'Trabalhadores':>15} {'Producao Total':>20} {'5% Circulando':>20} {'Por Trabalhador':>20}")
    imprima("  {'-'*80}")
    para cada r em scale["escala"]:
        imprima("  {r['trabalhadores']:>15} {r['producao_total']:>20} "
    ...
    imprima("\n  {r1['message']}")
    imprima("  Trabalhador total: {r1['trabalhador_recebe_total']}")
    imprima("  No capitalismo, dono levaria: {r1['COMPARACAO_PREDATORIA']['capitalismo_dono_leva']}")
    ...
    imprima("\n  {r2['message']}")
    imprima("  100 trabalhadores: {r2['ESCALA']['100_trabalhadores']}")
    ...
    imprima("\n{'='*80}")
    imprima("  OpenFairSurplus: {s['surplus_definido']} que volta. {s['principio']}")
    imprima("{'='*80}")
