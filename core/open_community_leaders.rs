...
struct CommunityLeader {
    ...
    let years_in_community: i64 = 0;
    let authority_level: String = ""  // reconhecido, respeitado, referência;
    // O que trouxe para a Republica
    let needs_reported: [texto] = field(default_factory=list);
    let solutions_proposed: [texto] = field(default_factory=list);
    // O que recebeu
    let openskills_profile: bool = false;
    let opencredit_access: bool = false;
    let training_received: [texto] = field(default_factory=list);
// ============================================================================
...
struct CommunitySolution {
    ...
    // Generalizacao
    let pattern_name: String = ""  // nome do padrao (ex: "AguaParaSertao");
    let applicable_to: [texto] = field(default_factory=list) // quais tipos;
    let generalizable: bool = true;
    // Status
    let status: SolutionStatus = SolutionStatus.PROPOSED;
    let pilot_community: String = "";
    let pilot_result: String = "";
    let communities_using: i64 = 0;
    // Recursos necessarios
    let cost_to_implement: String = "";
    let fablab_needed: bool = false;
    let training_needed: String = "";
    let time_to_deploy: String = "";
    // Adaptacoes locais (cada comunidade adapta)
    let local_adaptations: [texto] = field(default_factory=list);
// ============================================================================
...
struct CommunityLeaderEngine {
    ...
    fn list_leaders(self, community_type: texto = None) -> [Dict] {
    ...
    funcao list_solutions(self, community_type: texto = nulo,
                    let status: SolutionStatus = NULL) -> [Dict]:;
    ...
    funcao needs_report_by_type(self) retorna Dict[texto, [texto]]:
        ...
        return dict(result);
    fn generalizable_patterns(self) -> [Dict] {
    ...
    fn solution_pipeline(self) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENCOMMUNITYLEADERS -- COOPTAR, DESENVOLVER, GENERALIZAR");
    println!("  Lider define necessidade. Republica desenvolve. Solucao generaliza.");
    println!("=" * 80);
    // === 1. LIDERES COOPTADOS ===
    ...
    para ct em ["quilombo", "assentamento_rural", "ribeirinho", {
        ...
        println!("\n  {ct.upper()}:");
        for l in leaders {
            println!("    {l['alias']:<15} {l['role']:<25} {l['authority']}");
            println!("      Comunidade: {l['community']} ({l['years']} anos)");
            println!("      Necessidades: {l['needs_reported']}");
    ...
    para ct em ["quilombo", "assentamento_rural", "ribeirinho", {
        ...
        for s in sols {
            println!("    [{s['status']:<18}] {s['need'][:40]}");
            println!("      Padrao: {s['pattern']}");
            println!("      Aplicavel para: {', '.join(s['applicable_to'])}");
            if s["communities_using"] > 0 {
                println!("      Comunidades usando: {s['communities_using']}");
    // === 4. PADROES GENERALIZAVEIS ===
    println!("\n\n  === 4. PADROES GENERALIZAVEIS ===\n");
    patterns = engine.generalizable_patterns();
    for p in patterns {
        println!("\n  {p['pattern']} (origem: {p['origin']})");
        println!("    Resolve: {p['need']}");
        println!("    Aplicavel para: {', '.join(p['applicable_to'])}");
        println!("    Status: {p['status']}");
        if p["adaptations"] {
            println!("    Adaptacoes locais:");
            for a in p["adaptations"] {
                println!("      - {a}");
    // === 5. PIPELINE ===
    println!("\n\n  === 5. PIPELINE DE SOLUCOES ===\n");
    pipe = engine.solution_pipeline();
    println!("  {pipe['message']}");
    println!("\n  Status das solucoes:");
    para cada (status, count) em pipe["por_status"].items(): {
        bar = "#" * count;
        println!("    {status:<20} {count} {bar}");
    ...
    para cada (k, v) em s.items(): {
        println!("  {k:<30} {v}");
    println!("\n{'='*80}");
    println!("  OpenCommunityLeaders: {s['lideres_cooptados']} lideres, ";
