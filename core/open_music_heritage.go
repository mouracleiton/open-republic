...
package openmusicheritage_patrimonio_musical_da_republica
...
// decorador: @dataclass
type MusicalGenre struct {
    // Um genero musical patrimonio da Republica.
    genre_id: texto
    name: texto
    origin_region: texto // onde nasceu
    cultural_origin: CulturalOrigin
    birth_decade := ""  // quando surgiu // string
    description := "" // string
    bpm_range := ""  // BPM tipico // string
    instruments := field(default_factory=list) // [texto]
    subgenres := field(default_factory=list) // [texto]
    // Significado cultural
    cultural_meaning := "" // string
    social_function := ""  // o que faz pela comunidade // string
    dance_style := ""  // como se dança // string
    // Protecao
    is_heritage := true // PATRIMONIO da Republica // bool
    copyrightable := false // NINGUEM pode copyrightar // bool
    commercial_monopoly := false // sem monopolio corporativo // bool
    // Estatistica
    registered_musicians := 0 // int64
    total_songs := 0 // int64
// ============================================================================
...
// ============================================================================
type MusicHeritageEngine struct {
    ...
    func check_permission(self, action: texto, genre: texto) {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENMUSICHERITAGE -- PATRIMONIO MUSICAL DA REPUBLICA")
    fmt.Println("  Generos musicais sao BEM COMUM. Ninguem && dono. Todos herdeiros.")
    fmt.Println("=" * 80)
    // === 1. GENEROS PATRIMONIO ===
    fmt.Println("\n\n  === 1. GENEROS PATRIMONIO ({len(engine.genres)}) ===\n")
    fmt.Println("  {'ID':<12} {'Genero':<20} {'Origem':<30} {'Decada':<10} {'BPM'}")
    fmt.Println("  {'-'*85}")
    for _, g := range engine.genres.values() {
        fmt.Println("  {g.genre_id:<12} {g.name:<20} {g.origin_region:<30} "
            "{g.birth_decade:<10} {g.bpm_range}")
    // === 2. DETALHE: FUNK ===
    fmt.Println("\n\n  === 2. FUNK: PATRIMONIO DA PERIFERIA ===\n")
    funk = engine.genres["GEN-FUNK"]
    fmt.Println("  Nome: {funk.name}")
    fmt.Println("  Origem: {funk.origin_region}")
    fmt.Println("  Raiz: {funk.cultural_origin.value}")
    fmt.Println("  Instrumentos: {', '.join(funk.instruments)}")
    fmt.Println("  Subgeneros: {', '.join(funk.subgenres)}")
    fmt.Println("  Significado: {funk.cultural_meaning[:120]}...")
    fmt.Println("  Funcao social: {funk.social_function[:120]}...")
    // === 3. DETALHE: RAP ===
    fmt.Println("\n\n  === 3. RAP: JORNALISMO DA PERIFERIA ===\n")
    rap = engine.genres["GEN-RAP"]
    fmt.Println("  Nome: {rap.name}")
    fmt.Println("  Origem: {rap.origin_region}")
    fmt.Println("  Significado: {rap.cultural_meaning[:120]}...")
    fmt.Println("  Funcao social: {rap.social_function[:120]}...")
    // === 4. DETALHE: SAMBA ===
    fmt.Println("\n\n  === 4. SAMBA: ALMA DO BRASIL ===\n")
    samba = engine.genres["GEN-SAMBA"]
    fmt.Println("  Nome: {samba.name}")
    fmt.Println("  Origem: {samba.origin_region}")
    fmt.Println("  Instrumentos: {', '.join(samba.instruments)}")
    fmt.Println("  Subgeneros: {', '.join(samba.subgenres)}")
    fmt.Println("  Significado: {samba.cultural_meaning[:120]}...")
    ...
    para cada (genre, claimant) em claims: {
        ...
        fmt.Println("  {claimant} tentou copyrightar {genre} -> {result['status']}")
        fmt.Println("    {result['reason'][:80]}")
    // === 6. PERMISSAO ===
    fmt.Println("\n\n  === 6. PERMISSAO PARA CRIAR ===\n")
    for _, action := range ["criar", "tocar", "dançar", "remixar", "ensinar"] {
    ...
    P3 := trabalho base 1.0. Sem royalty adicional. // Criar musica
    P4: Quem decide o que && cultura && o POVO, ! corporacao.
    // )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenMusicHeritage: {s['total_genres_heritage']} generos patrimonio. "
        "{s['copyright_claims_rejected']} copyrights rejeitados.")
    fmt.Println("  Musica ! && produto. E CULTURA.")
    fmt.Println("{'='*80}")
