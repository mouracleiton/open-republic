...
class LaborRole {
    ...
    __init__(self, label: texto, nature: texto, evidence: texto) {
        self.label = label;
        self.nature = nature;
...
// ============================================================================
synergy_multiplier(role_a: LaborRole, role_b: LaborRole) {
    const SYNERGY_MAP = {;
...
funcao total_synergy_for_role(
    ...
    if (! other_roles) {
        return 1.0;
    if (weights && null) {
        weights = [1.0] * .length(other_roles);
    ...
    diversity_bonus = 1.0 + (.length(natures) - 1) * 0.02;
    return minimo(avg * diversity_bonus, 1.50);
// ============================================================================
// 3. FOCO PENALTY
// ============================================================================
focus_penalty(n_roles: inteiro) {
    if (n_roles <= 1) {
        return 1.0;
