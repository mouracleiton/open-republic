...
#ifndef OPENCOMMUNITYLEADERS_COOPTAR_LIDERANCAS_GENERALIZAR_SOLUCOES_H
#define OPENCOMMUNITYLEADERS_COOPTAR_LIDERANCAS_GENERALIZAR_SOLUCOES_H
...
// decorador: @dataclass
typedef struct CommunityLeader {
    ...
    char* authority_level = ""  // reconhecido, respeitado, referência;
...
// decorador: @dataclass
typedef struct CommunitySolution {
    ...
    // Generalizacao
    char* pattern_name = ""  // nome do padrao (ex: "AguaParaSertao");
    ...
    char* pilot_community = "";
    char* pilot_result = "";
    int communities_using = 0;
    // Recursos necessarios
    char* cost_to_implement = "";
    bool fablab_needed = false;
    char* training_needed = "";
    char* time_to_deploy = "";
...
// ============================================================================
typedef struct CommunityLeaderEngine {
    ...
    [Dict] list_leaders(self, community_type: texto = None) {
        ...
        return [;
            {
                ...
                "needs_reported": sizeof(l.needs_reported),;
            };
            /* para l em leaders */
        ];
    ...
    funcao needs_report_by_type(self) retorna Dict[texto, [texto]]:
        ...
        /* TODO: iterador C manual para leader em self.leaders.values() */
            /* TODO: iterador C manual para need em leader.needs_reported */
                if (need ! in result[leader.community_type]) {
                    result[leader.community_type].append(need);
        return dict(result);
    [Dict] generalizable_patterns(self) {
    ...
    {texto: qualquer} solution_pipeline(self) {
        // Pipeline de solucoes: proposta -> padrao.
        by_status = defaultdict(inteiro);
        /* TODO: iterador C manual para s em self.solutions.values() */
            by_status[s.status.value] += 1;
        return {;
            "total_solucoes": sizeof(self.solutions),;
    ...
    {texto: qualquer} stats(self) {
        all_needs = soma(sizeof(l.needs_reported) para l em self.leaders.values());
        return {;
            "lideres_cooptados": sizeof(self.leaders),;
            "tipos_representados": sizeof(set(l.community_type para l em self.leaders.values())),;
            "necessidades_reportadas": all_needs,;
            "solucoes_desenvolvidas": sizeof(self.solutions),;
            "padroes_generalizaveis": soma(;
                1 para s em self.solutions.values();
                if s.generalizable && sizeof(s.applicable_to) > 1),;
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENCOMMUNITYLEADERS -- COOPTAR, DESENVOLVER, GENERALIZAR");
    printf("  Lider define necessidade. Republica desenvolve. Solucao generaliza.");
    printf("=" * 80);
    // === 1. LIDERES COOPTADOS ===
    ...
    /* para ct em ["quilombo", "assentamento_rural", "ribeirinho", */
        ...
        printf("\n  {ct.upper()}:");
        /* TODO: iterador C manual para l em leaders */
            printf("    {l['alias']:<15} {l['role']:<25} {l['authority']}");
            printf("      Comunidade: {l['community']} ({l['years']} anos)");
            printf("      Necessidades: {l['needs_reported']}");
    ...
    /* para cada (ct, ct_needs) em needs.items(): */
        ...
        /* TODO: iterador C manual para n em ct_needs */
            printf("    - {n}");
    ...
    /* para ct em ["quilombo", "assentamento_rural", "ribeirinho", */
        ...
        /* TODO: iterador C manual para s em sols */
            printf("    [{s['status']:<18}] {s['need'][:40]}");
            printf("      Padrao: {s['pattern']}");
            printf("      Aplicavel para: {', '.join(s['applicable_to'])}");
            if (s["communities_using"] > 0) {
                printf("      Comunidades usando: {s['communities_using']}");
    // === 4. PADROES GENERALIZAVEIS ===
    printf("\n\n  === 4. PADROES GENERALIZAVEIS ===\n");
    patterns = engine.generalizable_patterns();
    /* TODO: iterador C manual para p em patterns */
        printf("\n  {p['pattern']} (origem: {p['origin']})");
        printf("    Resolve: {p['need']}");
        printf("    Aplicavel para: {', '.join(p['applicable_to'])}");
        printf("    Status: {p['status']}");
        if (p["adaptations"]) {
            printf("    Adaptacoes locais:");
            /* TODO: iterador C manual para a em p["adaptations"] */
                printf("      - {a}");
    // === 5. PIPELINE ===
    ...
    /* para cada (k, v) em s.items(): */
        printf("  {k:<30} {v}");
    printf("\n{'='*80}");
    printf("  OpenCommunityLeaders: {s['lideres_cooptados']} lideres, ";
...
#endif // OPENCOMMUNITYLEADERS_COOPTAR_LIDERANCAS_GENERALIZAR_SOLUCOES_H
