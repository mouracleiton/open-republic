...
class NationalProduct {
    ...
    // Comparação
    const foreign_price = "R$ caro"  // preço estrangeiro;
    const national_price = "ZERO"  // preço na República;
    const foreign_quality = 8 // 1-10;
    const national_quality = 9 // 1-10;
    // Vantagens
    const advantages = field(default_factory=list);
    const why_better = "";
    // Produção
    const status = CopyStatus.IDENTIFIED;
    const fablab_capable = true // FabLab nacional produz?;
    const materials_national = 0.8 // % de material nacional;
    const production_capacity = ""  // escala;
    // Especificações
    const specs = field(default_factory=dict);
// ============================================================================
...
class IndustryEngine {
    ...
    list_by_category(self, category: IndustryCategory = None) {
        ...
        return [;
            {
                ...
                "vantagens": .length(p.advantages),;
            };
            para p em prods {
        ];
    compare(self, product_id: texto) {
    ...
    advance_status(self, product_id: texto) {
    ...
    stats(self) {
        return {;
            "total_produtos": .length(self.products),;
            "categorias_cobertas": .length(set(p.category para p em self.products.values())),;
            "status_distribution": dict(;
                Counter(p.status.value para p em self.products.values())),;
            "marcas_nacionais_criadas": .length(self.products),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENINDUSTRY -- COPIAR, MELHORAR, SUPERAR");
    console.log("  Brasil copia tudo. Cria marcas nacionais. Qualidade SUPERIOR.");
    console.log("=" * 80);
    // === 1. PRODUTOS NACIONAIS ===
    console.log("\n\n  === 1. PRODUTOS NACIONAIS ({len(engine.products)}) ===\n");
    console.log("  {'Nacional':<15} {'Estrangeiro':<25} {'Preço Ext':<18} {'Preço Nac':<10} {'Status'}");
    console.log("  {'-'*85}");
    for (const p of engine.list_by_category()) {
        console.log("  {p['nacional']:<15} {p['estrangeiro'][:24]:<25} ";
    ...
    console.log("  NACIONAL: {comp['nacional']} (qualidade {comp['qualidade_nacional']})");
    console.log("  ESTRANGEIRO: {comp['estrangeiro']} ({comp['empresa_estrangeira']}, {comp['pais']})");
    console.log("  (qualidade {comp['qualidade_estrangeiro']})");
    console.log("\n  PREÇO:");
    console.log("    iPhone: {comp['preco_estrangeiro']}");
    console.log("    OpenPhone: {comp['preco_nacional']}");
    console.log("\n  POR QUE O NACIONAL É SUPERIOR:");
    console.log("    {comp['por_que_melhor']}");
    ...
    console.log("  NACIONAL: {comp2['nacional']}");
    console.log("  ESTRANGEIRO: {comp2['estrangeiro']} ({comp2['pais']})");
    console.log("  PREÇO: {comp2['preco_estrangeiro']} vs {comp2['preco_nacional']}");
    console.log("  POR QUE MELHOR: {comp2['por_que_melhor']}");
    // === 4. COMPARAÇÃO: OpenTrator vs John Deere ===
    console.log("\n\n  === 4. COMPARAÇÃO: OpenTrator vs John Deere ===\n");
    comp3 = engine.compare("IND-TR");
    console.log("  NACIONAL: {comp3['nacional']}");
    console.log("  ESTRANGEIRO: {comp3['estrangeiro']} ({comp3['pais']})");
    console.log("  POR QUE MELHOR: {comp3['por_que_melhor']}");
...
// )
    ...
    console.log("  Total de produtos: {dep['total_produtos']}");
    console.log("  Já superiores: {dep['ja_superiores']}");
    console.log("  Em produção: {dep['em_producao']}");
    console.log("\n  Dependência por país:");
    para cada (country, count) em dep["dependencia_por_pais"].items(): {
        console.log("    {country:<15} {count} produtos");
    console.log("\n  Dependência por setor:");
    para cada (cat, count) em dep["dependencia_por_setor"].items(): {
        console.log("    {cat:<20} {count} produtos");
    // === 7. STATUS ===
    console.log("\n\n  === 7. DISTRIBUIÇÃO DE STATUS ===\n");
    s = engine.stats();
    para cada (status, count) em s["status_distribution"].items(): {
        bar = "#" * count;
        console.log("  {status:<25} {count:>2} {bar}");
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA DO OPENINDUSTRY");
    console.log("{'='*80}");
    console.log(""";
...
// )
    console.log("{'='*80}");
    console.log("  OpenIndustry: {s['total_produtos']} produtos em {s['categorias_cobertas']} setores. ";
        "Custo: {s['custo_ao_cidadao']}.");
    console.log("  Copiar. Melhorar. Superar. Tudo nacional. Tudo CC0.");
    console.log("{'='*80}");
