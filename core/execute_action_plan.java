...
public class OpenrepublicExecucaoDoPlanoDeAcaoImediato {
    ...
    // importa json
    public void aplicar_plano(republic) {
        ...
        /* para cada (eid, count) em redistribuicao.items(): */
            /* TODO: for-each Java para nation em republic.nations.values() */
    ...
    public void main() {
        ...
        System.out.println("  OPENREPUBLIC -- EXECUCAO DO PLANO DE ACAO IMEDIATO");
        System.out.println("=" * 75);
        // 1. Estado ANTES
        ...
        System.out.println("  Saude media: {health_before['avg_health']:.0%}");
        System.out.println("  Entidades criticas: {health_before['critical_count']}");
        System.out.println("  Entidades urgentes: {health_before['urgent_count']}");
        System.out.println("  Pessoas em risco: {health_before['people_at_risk']:,}");
        // 2. Aplicar plano
        ...
        System.out.println("  Saude media: {health_after['avg_health']:.0%}");
        System.out.println("  Entidades criticas: {health_after['critical_count']}");
        System.out.println("  Entidades urgentes: {health_after['urgent_count']}");
        System.out.println("  Pessoas em risco: {health_after['people_at_risk']:,}");
        // 4. Comparativo
        System.out.println("\n\n  +-------------------------------------------------+");
        System.out.println("  |  METRICA              ANTES     DEPOIS    DELTA  |");
        System.out.println("  +-------------------------------------------------+");
        metrics = [;
        ...
        /* para name, before, after in metrics: */
            System.out.println("  |  {name:<22} {before:>8} {after:>10}{'':>7} |");
        System.out.println("  +-------------------------------------------------+");
        ...
        if (red) {
            /* TODO: for-each Java para item em red */
                ...
                System.out.println("      Risk: {item['risk_score']} | {', '.join(item['issues'][:2])}");
        } else {
            System.out.println("    NENHUMA -- todas as entidades criticas estabilizadas!");
        System.out.println("\n  ZONA AMARELA RESTANTE: {len(yellow)} entidades");
        /* TODO: for-each Java para item em yellow */
            System.out.println("    [{item['urgency_name']}] {item['name']} ({item['nation']})");
            System.out.println("      Risk: {item['risk_score']} | {item['issues'][0]}");
        // 6. Saude por nacao depois
        System.out.println("\n  SAUDE POR NACAO (DEPOIS):");
        System.out.println("  {'Nacao':<15} {'Saude':>7} {'Criticas':>9} {'Em risco':>10}");
        System.out.println("  {'-'*45}");
        /* para cada (nid, nation) em republic.nations.items(): */
            crit = tamanho(nation.critical_entities());
            risk = nation.people_at_risk;
            marker = crit == 0 ? " <-- OK" : "";
            System.out.println("  {nation.name:<15} {nation.avg_health:>6.0%} {crit:>9} {risk:>10,}{marker}");
        System.out.println("\n{'='*75}");
        if (health_after['critical_count'] == 0) {
            System.out.println("  MISSAO CUMPRIDA: 0 entidades criticas restantes.");
            System.out.println("  {health_before['people_at_risk']:,} pessoas sairam de risco.");
        } else {
            System.out.println("  PROGRESSO: {health_before['critical_count']} -> {health_after['critical_count']} criticas.");
            System.out.println("  {health_before['people_at_risk'] - health_after['people_at_risk']:,} pessoas sairam de risco.");
        System.out.println("{'='*75}");
