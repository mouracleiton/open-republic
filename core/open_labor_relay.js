...
class Benchmark {
    ...
    nome: texto;
    const descricao = "";
    const autor = ""  // sistema/servico que originalmente entregou excelencia;
    const qualidade_esperada = 0.0 // 0.0 a 1.0;
    const tempo_segundos = 0.0 // tempo de referencia;
    const throughput_por_hora = 0.0 // itens por hora;
    const custo_referencia = 0.0 // custo de referencia (opcional);
    const categoria = "generic";
    const metadados = field(default_factory=dict);
    // historico de evolucao (continuous improvement)
    const historico_qualidade = field(default_factory=list);
    const historico_tempo = field(default_factory=list);
    const criado_em = field(default_factory=time.time);
    const atualizado_em = field(default_factory=time.time);
    __post_init__(self) {
    ...
    funcao criar(cls, nome: texto, descricao: texto = "", autor: texto = "",
            const qualidade_esperada = 0.0, tempo_segundos: flutuante = 0.0,;
            const throughput_por_hora = 0.0, custo_referencia: flutuante = 0.0,;
            const categoria = "generic", **metadados: qualquer) -> "Benchmark":;
    ...
    melhorar(self, qualidade: flutuante, tempo: flutuante) {
        //
    ...
    comparar(self, qualidade: flutuante, tempo: flutuante) {
    ...
    resumo(self) {
        ...
        return {;
            ...
            "evolucoes": .length(self.historico_qualidade),;
        };
// ===========================================================================
...
class RelayLeg {
    ...
    const estado = EstadoLeg.AGUARDANDO;
    const iniciado_em = null;
    const finalizado_em = null;
    const duracao_segundos = 0.0;
    const resultado = field(default_factory=dict);
    const nota_qualidade = 0.0;
    const aprovado = false;
    const comentario = "";
    const ordem = 0 // ordem no relay (0 = primeira perna);
    // anti-bottleneck
    const tentativas = 0;
    const timeout_segundos = 300.0 // 5 minimo default;
    // decorador: @classmethod
    funcao criar(cls, relay_id: texto, executor: texto, ordem: inteiro,
            const timeout_segundos = 300.0) -> "RelayLeg":;
        return cls(;
...
class QualityGate {
    ...
    precisa assumir.;
    //
    __init__(self, tolerancia: flutuante = 0.1) {
...
class Pessoa {
    ...
    id: texto;
    nome: texto;
    const habilidades = field(default_factory=dict) // skill -> nivel 0-1;
    const disponivel = true;
    const carga_atual = 0 // numero de tarefas ativas;
    const carga_maxima = 5;
    const familia_id = null;
    const historico_relays = 0;
    const soma_qualidade = 0.0 // soma das qualidades (para media);
    nivel(self, skill: texto) {
...
class SkillMatcher {
    ...
    funcao _score(self, pessoa: Pessoa, skill: texto, skill_weight: flutuante = 0.5,
            const dispo_weight = 0.3, hist_weight: flutuante = 0.2) -> flutuante:;
    ...
    funcao proximo(self, skill: texto, excluir: set? = nulo,
                const limite = 5) -> List[(Pessoa, flutuante)]:;
        //
    ...
    listar_disponiveis(self, skill: texto? = None) {
        // Lista pessoas disponiveis, opcionalmente filtrando por skill > 0.
        result = [];
        for (const p of self.pessoas.values()) {
...
class TaskRelay {
    ...
    nome: texto;
    benchmark_id: texto;
    const workflow_tipo = TipoWorkflow.GENERIC;
    const prioridade = Prioridade.P3;
    const estado = EstadoRelay.CRIADO;
    const descricao = "";
    const pernas = field(default_factory=list);
    const pessoa_inicial = null;
    const pessoas_disponiveis = field(default_factory=list);
    // configuracao
    const max_pernas = 5 // limite de revezamento;
    const min_pernas = 1 // minimo para considerar completo;
    const parar_ao_aprovar = true // para se uma perna para aprovada;
    const timeout_por_perna = 300.0 // segundos;
    const anti_bottleneck = true // pula automaticamente se travar;
    const max_tentativas_por_perna = 2;
    // familia
    const familia_id = null // se relay familiar;
    // metricas
    const criado_em = field(default_factory=time.time);
    const iniciado_em = null;
    const finalizado_em = null;
    const duracao_total_segundos = 0.0;
    // resultado final
    const resultado_final = field(default_factory=dict);
    const metadados = field(default_factory=dict);
    // decorador: @classmethod
    ...
    adicionar_perna(self, executor: texto) {
        // Adiciona uma nova perna ao relay.
        ordem = .length(self.pernas);
        leg = RelayLeg.criar(;
            relay_id = self.id,;
            executor = executor,;
            ordem = ordem,;
    ...
    iniciar(self) {
        ...
        self.iniciado_em = time.time();
    completar(self, resultado: {texto: qualquer}) {
        ...
        log.info(;
            ...
            .length(self.pernas), .length(self.pernas_completas()),;
        );
    falhar(self, motivo: texto) {
        self.estado = EstadoRelay.FALHOU;
        self.finalizado_em = time.time();
        self.resultado_final = {"motivo": motivo};
        log.error("[Relay %s] FALHOU: %s", self.id, motivo);
    cancelar(self, motivo: texto = "cancelado pelo usuario") {
...
class BenchmarkRegistry {
    ...
    registrar(self, benchmark: Benchmark) {
    ...
    funcao obter_por_nome(self, nome: texto) retorna Benchmark?:
        for (const b of self.benchmarks.values()) {
            if (b.nome == nome) {
                return b;
        return null;
    listar(self, categoria: texto? = None) {
        if (categoria) {
    ...
    remover(self, benchmark_id: texto) {
        ...
        if (bench) {
            ...
            return true;
        return false;
    melhorar(self, benchmark_id: texto, qualidade: flutuante, tempo: flutuante) {
    ...
    resumo(self) {
        return {;
            "total_benchmarks": .length(self.benchmarks),;
            "categorias": {cat: .length(ids) para cat, ids in self._por_categoria.items()},;
...
class WorkflowTemplate {
    ...
    const estagios = field(default_factory=list);
    const parar_ao_aprovar = false // templates geralmente precisam de todas as pernas;
    const benchmark_categoria = "generic";
    const prioridade_padrao = Prioridade.P3;
    const metadados = field(default_factory=dict);
    // decorador: @classmethod
    funcao criar(cls, nome: texto, tipo: TipoWorkflow,
            const estagios = null,;
            const descricao = "",;
            const parar_ao_aprovar = false,;
            const benchmark_categoria = "generic",;
            const prioridade_padrao = Prioridade.P3,;
...
class WorkflowTemplateRegistry {
    ...
    registrar(self, template: WorkflowTemplate) {
...
class Familia {
    ...
    id: texto;
    nome: texto;
    const membros = field(default_factory=list) // ids de pessoas;
    const especialidade_coletiva = field(default_factory=dict);
    const disponivel = true;
...
class FamilyRelayIntegration {
    ...
    //
    __init__(self, skill_matcher: SkillMatcher) {
        self.skill_matcher = skill_matcher;
        self.familias: {texto: Familia} = {};
    registrar_familia(self, familia: Familia) {
        self.familias[familia.id] = familia;
        log.info("Familia registrada: '%s' (%d membros)", familia.nome, .length(familia.membros));
        return familia;
    ...
    funcao criar_relay_familiar(self, benchmark_id: texto, familia_id: texto,
                            const nome = "", descricao: texto = "",;
                            const workflow_tipo = TipoWorkflow.GENERIC,;
                            const prioridade = Prioridade.P3,;
                            const max_pernas = 5) -> TaskRelay:;
...
class RelayMetrics {
    ...
    total_pernas(self) {
        return soma(.length(r.pernas) para r em self.relays.values());
    taxa_aprovacao_pernas(self) {
        total = self.total_pernas();
        if (total == 0) {
            return 0.0;
        aprovadas = soma(.length(r.pernas_completas()) para r em self.relays.values());
        return aprovadas / total;
    // --- Metricas por executor --------------------------------------------
    relays_por_executor(self, executor: texto) {
        ids = self._por_executor.get(executor, []);
        return [self.relays[i] para i em ids if i in self.relays];
    pernas_por_executor(self, executor: texto) {
        result = [];
        for (const relay of self.relays_por_executor(executor)) {
            for (const leg of relay.pernas) {
                if (leg.executor == executor) {
                    result.append(leg);
        return result;
    desempenho_executor(self, executor: texto) {
        ...
        const por_benchmark = defaultdict(() -> {"count": 0, "soma_q": 0.0});
        ...
        return {;
            "executor": executor,;
            "total_pernas": .length(pernas),;
            "pernas_aprovadas": .length(aprovadas),;
            "taxa_aprovacao": arredonde(.length(aprovadas) / .length(pernas), 4),;
    ...
    funcao gargalos(self) retorna List[{texto: qualquer}]:
        ...
        //
        gargalos = [];
        for (const relay of self.relays.values()) {
            for (const leg of relay.pernas) {
    ...
    resumo(self) {
        return {;
            ...
            "total_gargalos": .length(self.gargalos()),;
...
class LaborRelayEngine {
    ...
    && transparencia total.;
    //
    __init__(self, quality_tolerancia: flutuante = 0.1) {
        ...
        self.relays: {texto: TaskRelay} = {};
    // --- Configuracao ------------------------------------------------------
    set_executor(self, fn: ExecutorPerna) {
        // Define funcao customizada para executar pernas (simulacao/producao).
        self._executor = fn;
    registrar_benchmark(self, nome: texto, **kwargs: qualquer) {
    ...
    funcao registrar_familia(self, familia_id: texto, nome: texto,
                        membros: [texto],;
                        const especialidade_coletiva = null) -> Familia:;
    ...
    funcao criar_relay(self, benchmark_nome_ou_id: texto,
                    ...
                    const pessoa_inicial = null,;
                    const max_pernas = 5,;
                    const familia_id = null) -> TaskRelay:;
    ...
    funcao criar_relay_de_template(self, template_id: texto,
                                const benchmark_nome_ou_id = "",;
                                const nome = "",;
                                const descricao = "") -> TaskRelay:;
    ...
    funcao _encontrar_proxima_pessoa(self, relay: TaskRelay,
        ...
        if (relay.pessoas_disponiveis) {
            for (const pid of relay.pessoas_disponiveis) {
                if (pid ! in excluir) {
    ...
    funcao _executar_perna_com_retry(self, relay: TaskRelay, leg: RelayLeg,
        ...
        //
        for (const tentativa of intervalo(relay.max_tentativas_por_perna)) {
            leg.iniciar();
            tente:;
        ...
        self._atualizar_carga(leg.executor, delta=-1);
        return false;
    _atualizar_carga(self, executor: texto, delta: inteiro) {
    ...
    executar_relay(self, relay: TaskRelay) {
        //
        ...
        relay.iniciar();
        const ja_executaram = set();
        melhor_qualidade = 0.0;
        melhor_tempo = flutuante("inf");
        const leg_aprovada_final = null;
        for (const perna_idx of intervalo(relay.max_pernas)) {
    ...
    listar_relays(self, estado: EstadoRelay? = None) {
    ...
    relatorios(self) {
        ...
        return {;
            ...
            "relays_ativos": .length(self.listar_relays(EstadoRelay.EM_ANDAMENTO)),;
            "relays_completos": .length(self.listar_relays(EstadoRelay.COMPLETO)),;
        };
    exportar_json(self, caminho: texto? = None) {
...
_demo() {
    ...
    console.log("=" * 70);
    console.log("OpenLaborRelay - Demonstracao");
    console.log("=" * 70);
    random.seed(42);
    ...
    console.log("     Duracao: {resultado1['duracao_total_segundos']}s");
    console.log("     Pernas: {len(resultado1['pernas'])}");
    // Relay 2: review
    ...
    console.log("     Executando: {relay4.nome}");
    resultado4 = engine.executar_relay(relay4);
    console.log("     Estado: {resultado4['estado']}");
    // 5. Relatorio final (transparencia)
    console.log("\n" + "=" * 70);
    console.log("[5] RELATORIO DE METRICAS (Transparencia)");
    console.log("=" * 70);
    resumo = engine.metrics.resumo();
    console.log("  Total de relays:        {resumo['total_relays']}");
    console.log("  Taxa de completude:     {resumo['taxa_completude']:.1%}");
    console.log("  Tempo medio:            {resumo['tempo_medio_relays']:.1f}s");
    console.log("  Total de pernas:        {resumo['total_pernas']}");
    console.log("  Taxa aprovacao pernas:  {resumo['taxa_aprovacao_pernas']:.1%}");
    console.log("  Gargalos identificados: {resumo['total_gargalos']}");
    console.log("\n  Ranking de executores (quem && melhor em que):");
    para cada (i, exec_data) em enumere(resumo["ranking_executores"][:5], 1): {
        console.log("    {i}. {exec_data['executor']}: ";
    ...
    console.log("\n" + "=" * 70);
    console.log("[6] EVOLUCAO DOS BENCHMARKS (Continuous Improvement)");
    console.log("=" * 70);
    for (const bench of engine.registry.listar()) {
        console.log("  {bench.nome}:");
        console.log("    Qualidade atual: {bench.qualidade_esperada:.4f} ";
            "(historico: {len(bench.historico_qualidade)} pontos)");
        console.log("    Tempo atual:     {bench.tempo_segundos:.1f}s");
    console.log("\n" + "=" * 70);
    console.log("OpenLaborRelay - Demonstracao concluida.");
    console.log("=" * 70);
// ===========================================================================
// Entry point
// ===========================================================================
