...
#ifndef OPENMUSICHERITAGE_PATRIMONIO_MUSICAL_DA_REPUBLICA_H
#define OPENMUSICHERITAGE_PATRIMONIO_MUSICAL_DA_REPUBLICA_H
...
// decorador: @dataclass
typedef struct MusicalGenre {
    // Um genero musical patrimonio da Republica.
    genre_id: texto;
    name: texto;
    origin_region: texto // onde nasceu;
    cultural_origin: CulturalOrigin;
    char* birth_decade = ""  // quando surgiu;
    char* description = "";
    char* bpm_range = ""  // BPM tipico;
    [texto] instruments = field(default_factory=list);
    [texto] subgenres = field(default_factory=list);
    // Significado cultural
    char* cultural_meaning = "";
    char* social_function = ""  // o que faz pela comunidade;
    char* dance_style = ""  // como se dança;
    // Protecao
    bool is_heritage = true // PATRIMONIO da Republica;
    bool copyrightable = false // NINGUEM pode copyrightar;
    bool commercial_monopoly = false // sem monopolio corporativo;
    // Estatistica
...
// ============================================================================
typedef struct MusicHeritageEngine {
    ...
    {texto: qualquer} check_permission(self, action: texto, genre: texto) {
    ...
    {texto: qualquer} stats(self) {
        return {;
            "total_genres_heritage": sizeof(self.genres),;
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENMUSICHERITAGE -- PATRIMONIO MUSICAL DA REPUBLICA");
    printf("  Generos musicais sao BEM COMUM. Ninguem && dono. Todos herdeiros.");
    printf("=" * 80);
    // === 1. GENEROS PATRIMONIO ===
    printf("\n\n  === 1. GENEROS PATRIMONIO ({len(engine.genres)}) ===\n");
    printf("  {'ID':<12} {'Genero':<20} {'Origem':<30} {'Decada':<10} {'BPM'}");
    printf("  {'-'*85}");
    /* TODO: iterador C manual para g em engine.genres.values() */
        printf("  {g.genre_id:<12} {g.name:<20} {g.origin_region:<30} ";
            "{g.birth_decade:<10} {g.bpm_range}");
    // === 2. DETALHE: FUNK ===
    printf("\n\n  === 2. FUNK: PATRIMONIO DA PERIFERIA ===\n");
    funk = engine.genres["GEN-FUNK"];
    printf("  Nome: {funk.name}");
    printf("  Origem: {funk.origin_region}");
    printf("  Raiz: {funk.cultural_origin.value}");
    printf("  Instrumentos: {', '.join(funk.instruments)}");
    printf("  Subgeneros: {', '.join(funk.subgenres)}");
    printf("  Significado: {funk.cultural_meaning[:120]}...");
    printf("  Funcao social: {funk.social_function[:120]}...");
    // === 3. DETALHE: RAP ===
    printf("\n\n  === 3. RAP: JORNALISMO DA PERIFERIA ===\n");
    rap = engine.genres["GEN-RAP"];
    printf("  Nome: {rap.name}");
    printf("  Origem: {rap.origin_region}");
    printf("  Significado: {rap.cultural_meaning[:120]}...");
    printf("  Funcao social: {rap.social_function[:120]}...");
    // === 4. DETALHE: SAMBA ===
    printf("\n\n  === 4. SAMBA: ALMA DO BRASIL ===\n");
    samba = engine.genres["GEN-SAMBA"];
    printf("  Nome: {samba.name}");
    printf("  Origem: {samba.origin_region}");
    printf("  Instrumentos: {', '.join(samba.instruments)}");
    printf("  Subgeneros: {', '.join(samba.subgenres)}");
    printf("  Significado: {samba.cultural_meaning[:120]}...");
    ...
    /* para cada (genre, claimant) em claims: */
        ...
        printf("  {claimant} tentou copyrightar {genre} -> {result['status']}");
        printf("    {result['reason'][:80]}");
    // === 6. PERMISSAO ===
    printf("\n\n  === 6. PERMISSAO PARA CRIAR ===\n");
    /* TODO: iterador C manual para action em ["criar", "tocar", "dançar", "remixar", "ensinar"] */
    ...
    // )
    printf("{'='*80}");
    printf("  OpenMusicHeritage: {s['total_genres_heritage']} generos patrimonio. ";
        "{s['copyright_claims_rejected']} copyrights rejeitados.");
    printf("  Musica ! && produto. E CULTURA.");
    printf("{'='*80}");

#endif // OPENMUSICHERITAGE_PATRIMONIO_MUSICAL_DA_REPUBLICA_H
