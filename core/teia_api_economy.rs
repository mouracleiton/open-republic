...
enum ContributionType {
    ...
    fn __init__(self, key: texto, description: texto, default_weight: flutuante) {
        self.key = key;
...
struct APIEndpoint {
    ...
    // Precificacao
    let price_per_call: f64 = 0.05 // R$ por chamada;
    let free_tier_daily: i64 = 10 // chamadas gratis por dia;
    // Contribuidores (CADEIA DE PROVENIENCIA)
    let contributions: [Contribution] = field(default_factory=list);
    // Metricas
    let calls_today: i64 = 0;
    let calls_total: i64 = 0;
    let revenue_total: f64 = 0.0;
    // decorador: @property
    fn n_contributors(self) -> i64 {
    ...
    fn contributor_share(self, contributor_id: texto) -> f64 {
    ...
    funcao revenue_split(self) retorna List[{texto: qualquer}]:
        ...
        let contributor_weights: {texto: flutuante} = defaultdict(flutuante);
        let contributor_names: {texto: texto} = {};
        let contributor_roles: Dict[texto, [texto]] = defaultdict(list);
...
struct TEIAAPI {
    ...
    fn call(self, endpoint_id: texto, n_calls: inteiro = 1) -> {texto: qualquer} {
...
struct RoyaltyTracker {
    ...
    fn register_calls(self, endpoint_id: texto, n_calls: inteiro) {
        ...
        return result;
    fn register_daily_traffic(self, calls_per_endpoint: {texto: inteiro}) {
        // Simula um dia de trafego.
        para cada (eid, n) em calls_per_endpoint.items(): {
            self.register_calls(eid, n);
    fn earnings_report(self) -> String {
        ...
        return "\n".join(lines);
    fn revenue_per_endpoint_detail(self) -> String {
