...
**Arquivo original:** `open-republic/core/open_sign_language_universal.py`
...
classe SignLanguageFamily herda de Enum:
...
classe SignLanguage:
    ...
    declare one_handed: bool  <- FALSO  // uma mao ou duas maos
    declare fingerspelling: bool  <- VERDADEIRO  // tem alfabeto datilologico
    declare facial_grammar: bool  <- VERDADEIRO  // usa expressao facial como gramatica
    declare iso_standard: str  <- ""  // ISO 639-3
    declare mutual_intelligibility: List[str]  <- field(default_factory=list)  // outras LS que entende
...
declare SIGN_LANGUAGES: List[SignLanguage]  <- [
    // === AMERICA DO SUL ===
...
classe SpokenLanguage:
    ...
    declare countries: List[str]  <- field(default_factory=list)  // paises onde e oficial
    declare speakers_millions: float  <- 0.0
    declare rtl: bool  <- FALSO  // right-to-left (arabe, hebraico)


declare SPOKEN_LANGUAGES: List[SpokenLanguage]  <- [
    ...
    SpokenLanguage("ar", "Arabe", "Al-Arabiya",
                   ["SA", "EG", "AE", "MA", "DZ", "IQ", "JO", "LB"], 420, rtl=VERDADEIRO),
...
classe GlosaConcept herda de Enum:
...
classe GlossEntry:
    ...
    declare spoken_translations: Dict[str, str]  <- field(default_factory=dict)  // {codigo_idioma: palavra}

    // Sinal equivalente em cada lingua de sinais
    declare sign_descriptions: Dict[str, Dict[str, str]]  <- field(default_factory=dict)
...
declare GLOSSARY: List[GlossEntry]  <- [
    GlossEntry(
...
classe TranslationPath herda de Enum:
...
classe TranslationRequest:
    ...
    declare path: TranslationPath  <- TranslationPath.SIGN_TO_SPOKEN


// decorador: @dataclass
classe UniversalTranslation:
    // Resultado de uma traducao universal.
    request: TranslationRequest
    source_language_name: str
    target_language_name: str
    declare translated_text: str  <- ""
    declare sign_description: Dict[str, str]  <- field(default_factory=dict)
    declare avatar_animation: str  <- ""
    declare confidence: float  <- 0.85
    declare processing_time_ms: int  <- 100
    declare alternative_translations: List[str]  <- field(default_factory=list)


classe UniversalSignTranslator:
    // 
    ...
    funcao translate(self, request: TranslationRequest) retorna UniversalTranslation:
    ...
    funcao _identify_concept(self, content: str, source_language: str) retorna Optional[GlosaConcept]:
        // Identifica o conceito universal a partir do conteudo.
        content_lower <- content.lower().strip()

        para cada (concept, entry) em self.glossary.items():
            // Verificar na lingua falada
            se source_language in entry.spoken_translations entao:
                se entry.spoken_translations[source_language] == content_lower entao:
                    retorne concept

            // Verificar em todas as linguas faladas
            para cada (lang, word) em entry.spoken_translations.items():
                se word == content_lower entao:
                    retorne concept

        retorne nulo

    funcao _get_lang_name(self, code: str, lang_type: str) retorna str:
        // Pega nome amigavel da lingua.
        se lang_type == "sign" entao:
            sl <- self.sign_languages.get(code)
            retorne sl.name if sl else code
        senao:
            pl <- self.spoken_languages.get(code)
            retorne pl.name if pl else code

    funcao list_sign_languages(self) retorna List[SignLanguage]:
        retorne SIGN_LANGUAGES

    funcao list_spoken_languages(self) retorna List[SpokenLanguage]:
        retorne SPOKEN_LANGUAGES

    funcao languages_by_country(self, country_code: str) retorna Dict[str, List[str]]:
        // Retorna linguas faladas e de sinais de um pais.
        result <- {"spoken": [], "sign": []}

        para cada sl em SIGN_LANGUAGES:
            se sl.country_code == country_code entao:
                result["sign"].append(f"{sl.name} ({sl.code})")

        para cada pl em SPOKEN_LANGUAGES:
            se country_code in pl.countries entao:
...
classe UniversalAvatar:
    ...
    funcao __init__(self):
        ...
        self.show_facial: bool = VERDADEIRO

    def sign_in_language(self, text: str, sign_language_code: str,
                         declare spoken_source: str  <- "pt") -> Dict[str, Any]:
        ...
        retorne {
            ...
            "speed": self.speed,
            "facial_expressions": self.show_facial  E  sl.facial_grammar if sl else VERDADEIRO,
            "one_handed": sl.one_handed if sl else FALSO,
            "skin_tone": self.current_skin,
            "style": self.style,
        }

    def sign_multilingual(self, text: str, sign_codes: List[str],
                           declare spoken_source: str  <- "pt") -> List[Dict[str, Any]]:
        // Gera animacoes em multiplas linguas de sinais.
        retorne [self.sign_in_language(text, code, spoken_source) for code in sign_codes]
...
classe SignLanguageUniversal:
    ...
    def translate_sign_to_sign(self, content: str, source_code: str,
        ...
        retorne self.translator.translate(req)
    ...
    def translate_to_many(self, content: str, source_code: str,
        ...
        results <- []
        para cada target em target_codes:
            // Detectar se e sign ou spoken pelo codigo
            se target in self.translator.sign_languages entao:
                se source_code in self.translator.sign_languages entao:
    ...
    def sign_in_all_languages(self, text: str, spoken_source: str = "pt",
                              declare limit: int  <- 10) -> List[Dict[str, Any]]:
        // Gera avatar signando em N linguas de sinais.
        codes <- [sl.code for sl in SIGN_LANGUAGES[:limit]]
        retorne self.avatar.sign_multilingual(text, codes, spoken_source)

    funcao find_language(self, country_code: str) retorna Dict[str, Any]:
        // Encontra linguas faladas e de sinais de um pais.
        retorne self.translator.languages_by_country(country_code)

    funcao stats(self) retorna Dict[str, Any]:
...
funcao scenario_brazilian_meets_american():
...
funcao scenario_ouvinte_fala_com_surdo_estrangeiro():
...
funcao scenario_conferencia_internacional():
...
funcao scenario_emergencia_multilingual():
...
funcao scenario_country_lookup():
    ...
    para cada country em ["BR", "US", "JP", "FR", "DE", "CN", "IN"]:
...
funcao demo():
    ...
    para cada (family, langs) em sorted(by_family.items(), key=lambda x: -len(x[1])):
        print(f"\n  {family.value.upper()} ({len(langs)} linguas):")
        para cada sl em langs:
