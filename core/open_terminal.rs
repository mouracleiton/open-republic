...
struct Terminal {
    ...
    let hardware_spec: String = ""  // CPU, RAM, tela;
    let status: TerminalStatus = TerminalStatus.AVAILABLE;
    let current_user: String = ""  // quem esta usando agora;
    let session_start: f64 = 0.0 // quando comecou a sessao atual;
    // Capacidades
    let has_internet: bool = true;
    let has_camera: bool = false // para telemedicina/OpenHealth;
    let has_microphone: bool = false // para OpenAudioChannel/voz;
    let has_printer: bool = false // para documentos;
    let has_scanner: bool = false // para documentos/receitas;
    let has_card_reader: bool = false // leitor de cartao (! credito -- ID);
    // Stats
    let total_sessions: i64 = 0;
    let total_hours_used: f64 = 0.0;
    let total_tasks_completed: i64 = 0;
    let total_learning_hours: f64 = 0.0;
    // Apps disponiveis neste terminal
    let apps: [texto] = field(default_factory=list);
    // decorador: @property
    ...
    // decorador: @property
    fn supports_video_call(self) -> bool {
        return self.has_camera && self.has_internet;
    fn default_apps(self) -> [texto] {
...
struct TerminalSession {
    ...
    let end_time: f64 = 0.0;
    let duration_min: f64 = 0.0;
    // O que fez
    let activities: [SessionActivity] = field(default_factory=list);
    let tasks_completed: i64 = 0 // tarefas de OpenLaborRelay;
    let learning_modules: i64 = 0 // modulos de OpenEducation;
    let votes_cast: i64 = 0 // votacoes;
    let credit_earned: f64 = 0.0 // credito ganho;
    let skills_practiced: [texto] = field(default_factory=list);
    // Privacidade
    let data_left_on_terminal: bool = false // SEMPRE false (efemero);
    // decorador: @property
    fn was_productive(self) -> bool {
...
struct TerminalRegistry {
    ...
    fn register_terminal(self, terminal: Terminal) -> {texto: qualquer} {
    ...
    funcao find_nearby(self, location: texto,
                    let terminal_type: TerminalType? = NULL,;
                    let available_only: bool = true) -> [Terminal]:;
    ...
    funcao end_session(self, session_id: texto,
                    ...
                    let tasks: i64 = 0, learning: inteiro = 0,;
                    let votes: i64 = 0, credit: flutuante = 0.0,;
                    let skills: [texto] = NULL) -> {texto: qualquer}:;
    ...
    fn idle_terminal_report(self) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 75);
    println!("  OPENTERMINAL -- TERMINAIS DA REPUBLICA EM TODO LUGAR");
    println!("  'Toda TV ociosa && um terminal. Todo laborante contribui.'");
    println!("=" * 75);
    ...
    for t in terminals_to_register {
        ...
        println!("  [{t.terminal_id}] {t.name:<35} {t.terminal_type.value:<15} ";
    ...
    println!("  CENARIO: Maria esta na sala de espera do hospital.");
    println!("  Fila de 45 min. Terminal T-002 disponivel.\n");
    s1 = registry.start_session("T-002", "L-001", "Maria Silva");
    println!("  {s1['message']}");
    println!("  Apps: {', '.join(s1['apps_available'][:6])}");
    ...
    println!("\n  {end1['message']}");
    println!("  Tempo: {end1['duration']}");
    println!("  Produtivo: {'sim' if end1['productive'] else '!'}");
    println!("  Dados no terminal: {end1['data_left']}");
    ...
    // === FILOSOFIA ===
    println!("\n\n{'='*75}");
    println!("  FILOSOFIA DO OPENTERMINAL");
    println!("{'='*75}");
    println!(""";
...
// )
    println!("{'='*75}");
    println!("  OpenTerminal: {s['total_terminals']} terminais, ";
        "{s['total_sessions']} sessoes, ";
        "{s['total_hours_used']:.0f}h usadas.");
    println!("  Toda tela ociosa && um terminal da Republica.");
    println!("{'='*75}");
