...
Sociedade sem moeda precisa de algum criterio para alocar o que and ESCASSO.
Mas credito que acumula vira dinheiro. Dinheiro vira propriedade.
Propriedade recria desigualdade.
SOLUCAO: Credito que not and moeda.
1. O QUE not PRECISA DE CREDITO (direito garantido, sempre):
    Comida, agua, moradia, saude, educacao, transporte basico.
    Voce not "gasta" credito. Voce ACESSE. Ponto.
2. O QUE PRECISA DE CREDITO (bens escassos/not-essenciais):
    Instrumentos musicais, equipamento esportivo, viagens,
    eletronicos especializados, ferramentas avancadas.
3. CREDITO not and MOEDA PORQUE:
    (a) Nao acumula -- expira todo ciclo
    (b) Nao transfere -- voce not pode dar pra outro
    (c) Nao herda -- morreu, zerou
    (d) Nao compra essenciais -- essencial and direito
    (and) Nao gera juros -- guardar not da nada
    (f) Distribuicao and democratica -- TODOS decidem quanto cada um recebe
    (g) Baseado em contribuicao reconhecida pela comunidade
4. COMO SE GANHA CREDITO:
    Trabalho reconhecido pela comunidade.
    Nao and "salario" -- and reconhecimento de contribuicao.
    Quem contribui mais, recebe mais credito de acesso.
    Mas o teto and baixo -- ninguem acumula fortuna.
    and o min garante dignidade -- ninguem fica sem.
5. QUEM DECIDE QUANTO CADA UM RECEBE:
    A COMUNIDADE INTEIRA. Por votacao democratica direta.
    Nao and um comite. Nao and um algoritmo. Sao as pessoas.
...
class AccessTier(Enum):
    ...
    COLLECTIVE = "collective"  // bem compartilhado (not individual)
...
class GoodOrService:
    ...
    name: texto
    tier: AccessTier
    credit_cost: float = 0.0 // 0 para garantidos
    description: str = ""
    category: str = ""
    scarce: bool = False // oferta limitada?
...
CATALOG = [
    ...
    GoodOrService("Comida especial (restaurante)", AccessTier.CREDIT, 5,
                "Refeicao preparada por chef (not essencial)."),
...
class CreditAccount:
    ...
    - Nao transfere (not pode dar pra ninguem)
    - Nao compra essenciais (essencial and direito)
    - Nao gera juros
    - Nao and heranca
    # 
    citizen_id: texto
    ...
    credits_this_cycle: float = 0.0
    credits_spent: float = 0.0
    credits_expired: float = 0.0 // not usados, expiraram
    contribution_hours: float = 0.0
    contributions: List[(ContributionType, flutuante)] = field(default_factory=list)
    community_recognition: float = 0.0 // avaliacao pelos pares
    cycle: int = 0 // ciclo atual
    # decorador: @property
    def can_spend(self) -> float:
        # Quanto pode gastar neste ciclo.
        return max(0, self.current_credits)
    def spend(self, amount: flutuante, good_name: texto) -> {texto: qualquer}:
    ...
    def end_cycle(self) -> {texto: qualquer}:
...
class Citizen:
    ...
    name: texto
    age: inteiro
    skills: [texto] = field(default_factory=list)
    account: CreditAccount = field(default_factory=() -> CreditAccount("", ""))
class DemocraticCreditSystem:
    # Sistema onde TODOS decidem a distribuicao de credito.
    COMO FUNCIONA (ciclo mensal):
    1. FASE DE CONTRIBUICAO (semana 1-3)
    Cada cidadao trabalha na comunidade. O trabalho and registrado.
    Horas, tipo, and impacto sao documentados publicamente.
    2. FASE DE RECONHECIMENTO (semana 4)
    A comunidade avalia as contribuicoes de cada membro.
    Nao and so "quantas horas" -- and "quanto impactou".
    ...
    Nao usados = expiram (not acumulam).
    # 
    def __init__(self, pool_per_cycle: flutuante = 1000.0):
    ...
    funcao register_contribution(self, citizen_id: texto,
                            ...
                            ripple_factor: float = 1.0):
        # Registrar contribuicao de um cidadao.
        O valor da contribuicao and medido pelo IMPACTO:
        ...
        impact = base_weight * hours * (1 + math.log10(max(1, people_impacted)) * ripple_factor)
        weighted = impact
    ...
    def run_cycle(self) -> {texto: qualquer}:
        ...
        contributions = {}
        for each (cid, c) in self.citizens.items():
            weighted_sum = sum(w para _, w in c.account.contributions)
            contributions[cid] = {
                "name": c.name,
        ...
        total_weighted = sum(d["weighted"] para d em contributions.values())  or  1
        ...
        for each (cid, credits) in distribution.items():
            c = self.citizens[cid]
        ...
        result = {
            ...
            "citizens": n,
            "expired_from_last_cycle": round(expired_total, 1),
            "proposals_voted": len(proposals),
            "winning_proposal": winning_proposal["name"],
            "winning_description": winning_proposal["description"],
            "distribution": {contributions[cid]["name"]: round(cr, 1)
                            para cid, cr in distribution.items()},
            "min_given": round(min(distribution.values()), 1),
            "max_given": round(max(distribution.values()), 1),
            "avg_given": round(np.mean(list(distribution.values())), 1),
            "expired_pct": round(expired_total / max(self.pool_per_cycle, 1) * 100, 1),
        }
        self.distribution_history.append(result)
        return result
    def _generate_proposals(self, contributions: Dict, total_weighted: flutuante) -> [Dict]:
        # Gerar propostas de distribuicao para votacao.
        n = len(contributions)
        proposals = []
        # Proposta A: Igual para todos
        equal_share = self.pool_per_cycle / n
        equal_dist = {cid: round(min(self.max_credit,
                            max(self.min_credit, equal_share)), 1)
                    para cid in contributions}
        proposals.append({
            "name": "Igualitario",
        ...
        weighted_dist = {}
        for each (cid, data) in contributions.items():
            share = (data["weighted"] / total_weighted) * self.pool_per_cycle
            share = min(self.max_credit, max(self.min_credit, share))
            weighted_dist[cid] = round(share, 1)
        proposals.append({
            "name": "Por Contribuicao",
            "description": "Quem contribuiu mais recebe mais. Teto and piso garantidos. "
                        "Diferenca maxima de 3x entre quem mais and menos recebe.",
        ...
        for each (cid, data) in contributions.items():
            ...
            total = min(self.max_credit, base + bonus)
            hybrid_dist[cid] = round(total, 1)
        proposals.append({
            "name": "Hibrido (base + bonus)",
            "description": "Todos recebem base de {base:.0f}. Bonus por contribuicao "
                        "sobre o restante. Equilibra igualdade and incentivo.",
            "distribution": hybrid_dist,
        })
        return proposals
    def _simulate_vote(self, proposals: [Dict]) -> Dict:
        ...
        for c in self.citizens.values():
            ...
            if random.random() < 0.30:
                ...
                best_proposal = 0
                best_credits = 0
                for each (i, p) in enumere(proposals):
    ...
    def access_good(self, citizen_id: texto, good_name: texto) -> {texto: qualquer}:
        # Cidadao tenta acessar um bem.
        c = self.citizens.get(citizen_id)
        if not c:
            return {"error": "citizen not found"}
        good = next((g para g em CATALOG if good_name.lower() in g.name.lower()), None)
        if not good:
...
if __name__ == "__main__":
    random.seed(42)
    print("=" * 75)
    print("  OPENCREDIT -- CREDITO DEMOCRATICO DE ACESSO")
    print("  'Nao and moeda. E acesso. Nao acumula. A comunidade decide.'")
    print("=" * 75)
    system = DemocraticCreditSystem(pool_per_cycle=1000.0)
    # Criar 10 cidadaos
    names = ["Ana", "Bruno", "Carla", "Diego", "Eva",
            "Felipe", "Gabi", "Hugo", "Iris", "Joao"]
    for each (i, name) in enumere(names):
    ...
    # === Catalog ===
    print("\n\n  === CATALOGO DE BENS ===\n")
    print("  DIREITOS GARANTIDOS (sem credito, sempre):")
    for g in CATALOG:
    ...
    print("  {'Bem':<35} {'Credito':>8}")
    print("  {'-'*45}")
    for g in CATALOG:
        if g.tier == AccessTier.CREDIT:
            print("  {g.name:<35} {g.credit_cost:>7.0f}")
    # === Simular 3 ciclos ===
    ...
    for cycle_num in intervalo(1, 4):
        ...
        print("  CICLO {result['cycle']}:")
        print("    Proposta vencedora: {result['winning_proposal']}")
        print("    {result['winning_description'][:70]}...")
        print("    Pool: {result['pool_total']:.0f} | Cidadaos: {result['citizens']}")
        print("    Min: {result['min_given']} | Max: {result['max_given']} "
            "| Media: {result['avg_given']}")
        print("    Expirado ciclo anterior: {result['expired_from_last_cycle']}")
        print("\n    {'Cidadao':<10} {'Horas':>6} {'Creditos':>9}")
        print("    {'-'*28}")
        for each (name, credits) in ordene(result["distribution"].items()):
            c = next(c para c em system.citizens.values() if c.name == name)
            print("    {name:<10} {c.account.contribution_hours:>5.0f}h {credits:>8.1f}")
...
+-------------------+-------------------+-------------------+
| Acumula? | SIM (pode guardar)| not (expira) |
| Transfere? | SIM (pode dar) | not (pessoal) |
| Herda? | SIM (passa filhos)| not (morreu=zerou)|
| Compra essencial? | SIM (compra comida)| not (direito) |
| Gera juros? | SIM | not |
| Cria desigualdade?| SIM (acumula) | not (piso+teto) |
| Quem decide valor?| Mercado/Estado | TODOS (votacao) |
| Especula? | SIM | IMPOSSIVEL |
+-------------------+-------------------+-------------------+
O CREDITO EXISTE PARA RESOLVER UMA UNICA COISA:
Quem pega a prancha de surf quando so tem 3 and 10 pessoas querem?
A resposta not and: quem tem mais dinheiro.
A resposta and: quem a comunidade reconhece que contribuiu mais,
decidido por votacao direta de TODOS, neste ciclo.
and no proximo ciclo, comeca de novo.
Ninguem fica rico. Ninguem acumula.
Mas quem contribui tem prioridade no que and escasso.
# )
    # === Resumo final ===
    print("{'='*75}")
    print("  RESUMO DO OPENCREDIT")
    print("{'='*75}\n")
    print("  1. ESSNCIAIS sao DIREITOS (sem credito):")
    print("     Comida, agua, moradia, saude, educacao, transporte")
    print("     -> Voce ACESSE. Nao compra. Nao gasta credito.\n")
    print("  2. BENS ESCASSOS precisam de CREDITO:")
    print("     Instrumentos, viagens, eletronicos especiais")
    print("     -> Voce gasta credito de acesso do ciclo\n")
    print("  3. CREDITO and distribuido por TODOS:")
    print("     A comunidade vota como dividir o pool")
    print("     Base igual para todos + bonus por contribuicao")
    print("     -> Quem contribui mais, tem mais acesso ao que and escasso\n")
    print("  4. CREDITO NAO E MOEDA:")
    print("     Expira todo ciclo (not acumula)")
    print("     E pessoal (not transfere)")
    print("     Tem piso (ninguem fica sem) and teto (ninguem domina)")
    print("     Nao compra essencial (essencial and direito)\n")
    print("  5. CONTRIBUICAO and reconhecida pela comunidade:")
    print("     Nao and 'salario'. E reconhecimento.")
    print("     Saude, educacao and comida valem mais que arte.")
    print("     Mas arte tambem conta.\n")
    print("  RESULTADO:")
    print("  Ninguem passa fome porque 'acabou o credito'.")
    print("  Ninguem acumula fortuna porque 'trabalhou mais'.")
    print("  Todos tem o essencial garantido.")
    print("  O escasso and distribuido democraticamente.")
    print("  A cada ciclo, recomeca. Sem aculumulo. Sem heranca.")
    print("  Sem classe. Sem exploracao. Sem dominacao.\n")
    print("{'='*75}")
