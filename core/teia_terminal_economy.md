...
**Arquivo original:** `core/teia_terminal_economy.py`
...
O MODELO:
  ...
  4. TEIA DIRETO (nao revendedor) executa sem pagar 30%
...
classe ResellerProfile herda de Enum:
    ...
    STARTUP_GOVTECH = (
        "Startup Govtech",
        "Empresa que vende para governo. TEIA e insumo.",
        0.45,
        6,
        25_000,
    )
...
funcao simulate_demand(
    seja terminal_monthly_brl: flutuante = 1_500,
    seja training_brl: flutuante = 2_000,
    seja teia_pct: flutuante = 0.30,
    seja months: inteiro = 12,
    seja seed: inteiro = 42,
) -> [SimulationResult]:
    ...
    para cada profile em ResellerProfile:
        ...
        avg_active = inteiro(base_interested * soma(adoption_curve) / tamanho(adoption_curve))
...
classe PoolDistribution:
    // Como o POOL DA REPUBLICA e distribuido internamente.

    ANTES: 5% fixo para "pool da Republica" (caixa unico).
    PROBLEMA: 5% nao cobre as necessidades da organizacao.

    AGORA: o pool e 25% dos 30% (7,5% do produto final).
    e distribuido em SUB-FATIAS para garantir que cada area recebe:
    ...
    // decorador: @classmethod
    funcao validate(cls) -> logico:
        retorne abs(soma(cls.POOL_SPLITS.values()) - 1.0) < 0.001
    ...
    funcao breakdown(cls, pool_amount_brl: flutuante) retorna List[{texto: qualquer}]:
        ...
        para cada (name, pct) em cls.POOL_SPLITS.items():
            result.append({
                "area": name,
                "pct": pct,
...
classe Certification herda de Enum:
    // Niveis de certificacao para revendedores.

    Sem certificacao: nao pode revender.
...
classe ModuleAccess:
    ...
    seja sensitive: logico = falso // se verdadeiro, NINGUEM fora da equipe TEIA acessa
    seja teia_only: logico = falso // se verdadeiro, so TEIA direto (nao revendedor)
    seja description: texto = ""


seja MODULES: [ModuleAccess] = [
    ...
    ModuleAccess("juros_spread", "Analise de Juros e Spread Bancario", Certification.TEIA_ESPECIALISTA,
    ...
    ModuleAccess("comunidades_reais", "Dados de Comunidades Reais", Certification.TEIA_ESPECIALISTA,
                 sensitive = verdadeiro, teia_only=verdadeiro,
...
funcao check_access(reseller_id: texto, cert: Certification, module_id: texto,
                 seja is_teia_direct: logico = falso) -> {texto: qualquer}:
    // Verifica se o revendedor pode acessar o modulo.

    Retorna: {allowed: logico, reason: texto}
    // 

    mod = next((m para m em MODULES if m.module_id == module_id), nulo)
    se nao mod entao:
        retorne {"allowed": falso, "reason": "Modulo '{module_id}' nao existe."}

    // TEIA direto: acesso a tudo exceto dados sensiveis de parceiros
    se is_teia_direct entao:
        se mod.teia_only e mod.sensitive entao:
            // TEIA direto pode acessar dados sensiveis da propria organizacao
            retorne {"allowed": verdadeiro, "reason": "TEIA direto: acesso autorizado."}
        retorne {"allowed": verdadeiro, "reason": "TEIA direto: sem restricao."}
    ...
    se mod.sensitive entao:
        retorne {
            "allowed": falso,
            "reason": (
                "DADO SENSIVEL. Modulo '{mod.name}' e restrito a equipe TEIA. "
                "LGPD/sigilo/parceiro. Revendedor nao tem acesso."
            ),
        }
    ...
    se mod.teia_only entao:
        retorne {
            "allowed": falso,
            "reason": (
                "Modulo '{mod.name}' e operacao interna TEIA. "
                "Revendedor nao tem acesso. Para este servico, "
    ...
    se reseller_level < mod_level entao:
        retorne {
            "allowed": falso,
            "reason": (
    ...
    retorne {"allowed": verdadeiro, "reason": "Acesso autorizado ({cert.label})."}
...
classe CryptoPaymentAnalysis:
    ...
    1. LEI 14.478/2022 (Marco Legal das Cripto)
       ...
       - nao e moeda legal (nao substitui R$)
    ...
    4. RECEITA FEDERAL (IN RFB 1888/2019)
       - Declaracao de cripto obrigatorio para PJ e PF
    ...
    STATUS PARA TEIA:
       ...
       - Deve emitir NF-e em R$ (valor convertido na data da transacao)
    // 

    seja viable: logico = verdadeiro
    seja stablecoin_recommended: texto = "USDC"
    seja reason: texto = (
        "USDC e audited, lastreado em dolar, aceito em exchanges brasileiras. "
        "Mercado Bitcoin, Foxbit e Binance Brasil convertem USDC->BRL via PIX."
    )
    ...
    requirements = [
        ...
        "Emitir NF-e em R$ (mesmo se pago em cripto)",
        "Manter registro da transacao em cripto (hash, carteira, data)",
        "Contract clause: 'Pagamento em cripto nao isenta obriga(S) fiscais'",
    ]
...
funcao print_simulation() -> texto:
    ...
    total_interested = soma(r.interested para r em results)
    total_terminal_rev = soma(r.terminal_revenue para r em results)
    total_training_rev = soma(r.training_revenue para r em results)
    total_artifact_rev = soma(r.artifact_revenue para r em results)
    total_all = soma(r.total_revenue para r em results)
    ...
    lines.append("    - NAO paga os 30% (nao ha split de revenda)")
    ...
    test_cases = [
        ("revendedor_base", Certification.TEIA_BASE, "fome", falso),
        ("revendedor_base", Certification.TEIA_BASE, "impacto_fiscal", falso),
        ("revendedor_base", Certification.TEIA_BASE, "comunidades_reais", falso),
        ("revendedor_analista", Certification.TEIA_ANALISTA, "impacto_fiscal", falso),
        ("revendedor_analista", Certification.TEIA_ANALISTA, "comunidades_reais", falso),
        ("revendedor_espec", Certification.TEIA_ESPECIALISTA, "due_diligence", falso),
        ("revendedor_espec", Certification.TEIA_ESPECIALISTA, "comunidades_reais", falso),
        ("teia_direto", Certification.TEIA_ESPECIALISTA, "comunidades_reais", verdadeiro),
        ("teia_direto", Certification.TEIA_ESPECIALISTA, "open_credit", verdadeiro),
    ]
    ...
    para cada r em analysis.requirements:
        lines.append("    -> {r}")

    lines.append("")
    lines.append("  BENEFICIOS:")
    para cada b em analysis.benefits:
        lines.append("    + {b}")

    lines.append("")
    lines.append("  RISCOS:")
    para cada r em analysis.risks:
        lines.append("    ! {r}")
    ...
    lines.append("  ID = bloqueia dados sensiveis (LGPD) e modulos TEIA-only")
    lines.append("  CRIPTO = viavel (USDC), declarar Receita, NF em R$")
    lines.append("  MODULO FALTANTE = sistema bloqueia e diz qual certificacao precisa")
    lines.append("")
...
se __name__ == "__main__" entao:
    // Validar splits
    afirme RevenueSplit.validate(), "RevenueSplit nao soma 100%!"
    afirme PoolDistribution.validate(), "PoolDistribution nao soma 100%!"

    imprima(print_simulation())
