...
class HandState {
    ...
    const gesture = Gesture.OPEN_HAND;
    const landmarks = field(default_factory=list);
    const center = (0, 0);
    const wrist_angle = 0.0;
    const fingers_up = field(default_factory=() -> [false]*5);
    const pinch_distance = 0.0;
    const prev_center = (0, 0);
...
class HandTracker {
    ...
    funcao process(self, frame: np.ndarray) retorna Tuple[np.ndarray, HandState]:
        ...
        if (results.multi_hand_landmarks) {
            for (const hand_lms of results.multi_hand_landmarks) {
                self.state.detected = true;
                // Converter landmarks para pixels
                landmarks = [];
                for (const lm of hand_lms.landmark) {
                    px = inteiro(lm.x * w);
                    py = inteiro(lm.y * h);
    ...
    _classify_gesture(self) {
        ...
        if (f[0] && f[1] && ! f[2] && ! f[3] && ! f[4]) {
            return Gesture.GUN;
        return Gesture.FLAT;
    _buffer_gesture(self, gesture: Gesture) {
        // Buffer para estabilizar gesto (evita oscilacao).
        self.gesture_buffer.append(gesture);
        if (.length(self.gesture_buffer) > self.buffer_size) {
            self.gesture_buffer.pop(0);
    _stable_gesture(self) {
        // Retorna gesto mais comum no buffer (estabilidade).
        if (! self.gesture_buffer) {
            return Gesture.OPEN_HAND;
        const counts = {};
        for (const g of self.gesture_buffer) {
...
class VirtualUI {
    ...
    class GameObject {
        ...
        const vx = 0.0;
        const vy = 0.0;
    __init__(self, width: inteiro = 640, height: inteiro = 480) {
        self.width = width;
        self.height = height;
    ...
    update(self, hand: HandState) {
        ...
        if (hand.detected) {
            ...
            for (const obj of self.objects) {
                obj.grabbed = false;
    _update_menu(self, hand: HandState) {
        ...
        for (const btn of self.buttons) {
            ...
            if (btn.pressed && btn.on_click && ! None) {
                btn.on_click();
    _update_catch(self, hand: HandState) {
        ...
        while (.length(self.objects) < 5) {
            self._spawn_fruit();
    _update_paint(self, hand: HandState) {
        ...
        if (hand.gesture == Gesture.POINT) {
            ...
            if (.length(self.paint_points) > 1000) {
                self.paint_points.pop(0);
    _update_free(self, hand: HandState) {
...
class HandTrackingApp {
    ...
    //
    __init__(self, camera_index: inteiro = 0) {
    ...
    run(self) {
        ...
        if (! cap.isOpened()) {
            console.log("ERRO: webcam ! disponivel");
            return null;
        // Configurar camera
        ...
        self.running = true;
        console.log("=" * 50);
        console.log("  OpenHandTracking");
        console.log("  Mostre a mao para a camera.");
        console.log("  PINCA/PUNHO = clicar. ESC = sair. M = menu.");
        console.log("=" * 50);
        prev_time = time.time();
        while (self.running) {
