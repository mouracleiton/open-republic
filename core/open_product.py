...
# !/usr/bin/env python3
# 
OpenProduct -- Produtos Otimizados para Meio Ambiente, Pessoas and Animais
=========================================================================
"Todo produto da Republica and projetado para:
1. not prejudicar PESSOAS (toxico, perigoso, explorador)
2. not prejudicar ANIMAIS (teste, crueldade, extincao)
3. not prejudicar MEIO AMBIENTE (poluicao, desperdicio, extracao)
4. SER REPARAVEL (OpenRepair, not descartavel)
5. SER MODULAR (LEGO, troca peca, not produto inteiro)
...
Produto que and descartavel? SUBSTITUIDO."
...
class HarmLevel(Enum):
    ...
    MINIMAL = ("min", 1)  // dano min aceitavel
...
class RepublicProduct:
    ...
    human_health_harm: HarmLevel = HarmLevel.NONE
    animal_harm: HarmLevel = HarmLevel.NONE
    environment_harm: HarmLevel = HarmLevel.NONE
    social_harm: HarmLevel = HarmLevel.NONE
    # Materiais
    materials: [MaterialType] = field(default_factory=list)
    materials_detail: str = ""
    # Ciclo de vida
    biodegradable: bool = False
    recyclable: bool = True
    repairable: bool = True // OpenRepair
    modular: bool = True // LEGO
    lifespan_years: float = 10.0
    # Energia
    energy_efficient: bool = True
    energy_source: str = "solar/eolica"
    # Producao
    fablab_producible: bool = True // FabLab fabrica?
    tested_on_animals: bool = False // PROIBIDO
    fair_labor: bool = True // trabalho base 1.0
    # Status
    status: ProductStatus = ProductStatus.APPROVED
    replacement_for: str = ""  // substitui qual produto nocivo?
    ...
    def overall_score(self) -> int:
        ...
        penalty = sum(h.value[1] * 5 para h em harms)
        bonus = 0
...
PRODUCTS: [RepublicProduct] = [
    ...
    RepublicProduct(
        ...
        "Garrafa de aco inoxidavel or vidro. Reutilizavel eternamente.",
        materials = [MaterialType.MINERAL],
        materials_detail = "Aco inoxidavel reciclavel or vidro. Sem plastico.",
    ...
    RepublicProduct(
        ...
        "PROIBIDO. Teste em animais and CRUELDADE desnecessaria.",
...
class ProductEngine:
    ...
    1. not prejudica saude humana (sem toxico, sem cancerigeno)
    2. not testa em animais (PROIBIDO)
    3. not prejudica meio ambiente (biodegradavel/reciclavel)
    4. REPARAVEL (OpenRepair, not descartavel)
    5. MODULAR (LEGO, troca peca)
    6. FABLAB PRODUCIVEL (producao nacional)
    7. TRABALHO JUSTO (base 1.0, sem exploracao)
    SE FALHA EM UM CRITERIO: reformular or proibir.
    ...
    def evaluate(self, product_id: texto) -> {texto: qualquer}:
        # Avalia produto contra todos os criterios.
        p = self.products.get(product_id)
        if not p:
            return {"error": "Produto not encontrado"}
        checks = {
            "saude_humana": p.human_health_harm.value[1] <= 1,
            "sem_teste_animal": not  p.tested_on_animals,
            ...
            "sem_toxico": MaterialType.TOXIC not  in p.materials,
        }
        failed = [k para k, v in checks.items() if not v]
        any_critical = any([
    ...
    def list_approved(self) -> [Dict]:
        return [
            ...
            para p in self.products.values()
            if p.status == ProductStatus.APPROVED
        ]
    def list_prohibited(self) -> [Dict]:
        return [
            {"id": p.product_id, "name": p.name,
    ...
    def waste_report(self) -> {texto: qualquer}:
        ...
        return {
            ...
            "message": (
                ...
                "Descartavel not existe. Lixo not existe."
            ),
        }
    def stats(self) -> {texto: qualquer}:
        approved = sum(1 para p em self.products.values()
                    if p.status == ProductStatus.APPROVED)
        prohibited = sum(1 para p em self.products.values()
                        if p.status == ProductStatus.PROHIBITED)
        avg_score = sum(p.overall_score para p em self.products.values()
                        if p.status == ProductStatus.APPROVED) / max(approved, 1)
        return {
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("  OPENPRODUCT -- PRODUTOS QUE NAO PREJUDICAM")
    print("  Pessoas. Animais. Meio Ambiente. Tudo protegido.")
    print("=" * 80)
    # === 1. AVALIAR TODOS OS PRODUTOS ===
    print("\n\n  === 1. AVALIACAO DE PRODUTOS ===\n")
    for pid in engine.products:
        r = engine.evaluate(pid)
    # === 2. PRODUTOS APROVADOS ===
    print("\n\n  === 2. PRODUTOS APROVADOS ===\n")
    approved = engine.list_approved()
    print("  {'Produto':<25} {'Score':>5} {'Vida':>5} {'Bio':>4} {'Rep':>4} {'Mod':>4} {'Substitui'}")
    print("  {'-'*85}")
    for p in approved:
        print("  {p['name'][:24]:<25} {p['score']:>5} {p['lifespan']:>4.0f}a "
    ...
    for p in prohibited:
        print("  {p['name']:<30} DANO: {p['why']}")
        print("    Alternativa: {p['alternative']}")
    # === 4. RELATORIO DE RESIDUOS ===
    print("\n\n  === 4. RESIDUOS EVITADOS ===\n")
    waste = engine.waste_report()
    for each (key, data) in waste.items():
        if key != "message"  and  isinstance(data, dict):
            print("\n  {key.upper()}:")
            print("    Atual: {data['atual']}")
            print("    Republica: {data['republica']}")
    print("\n  {waste['message']}")
    # === 5. STATS ===
    print("\n\n  === 5. ESTATISTICAS ===\n")
    s = engine.stats()
    for each (k, v) in s.items():
        print("  {k:<30} {v}")
    print("\n{'='*80}")
    print("  OpenProduct: {s['aprovados']} aprovados, {s['proibidos']} proibidos. "
        "Score medio: {s['score_medio_aprovados']}.")
    print("  Nao prejudica pessoas. Nao prejudica animais. Nao prejudica planeta.")
    print("{'='*80}")
