...
public class OpenrepublicPoliticaDeCuidadosFisicos {
    ...
    // ============================================================================
    public static class CareDimension {
        ...
        ERGONOMIC = "ergonomia";
    public static class CareStatus {
        OPTIMAL = "otimo";
        ADEQUATE = "adequado";
        BELOW = "abaixo";
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class CarePolicy {
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class CitizenCareProfile {
    ...
    public static class PhysicalCareSystem {
        ...
        public void __init__(self) {
            ...
            self._init_profiles();
        public void _init_profiles(self) {
            ...
            /* TODO: for-each Java para d em data */
        ...
        public {texto: qualquer} assess(self, citizen_id: texto) {
        ...
        public {texto: qualquer} republic_report(self) {
        ...
        public CareStatus _status_from_score(score: flutuante) {
            if (score >= 80) {
        ...
        public String _intervention(self, dim: CareDimension) {
        ...
        funcao _common_barriers(self) retorna List[(texto, inteiro)]:
            ...
            /* TODO: for-each Java para p em self.profiles.values() */
                /* TODO: for-each Java para b em p.barriers */
                    barrier_count[b] += 1;
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENREPUBLIC -- POLITICA DE CUIDADOS FISICOS");
        System.out.println("  'O corpo ! && maquina. E templo da existencia.'");
        System.out.println("=" * 80);
        ...
        /* para cada (dim, policy) em system.policies.items(): */
            System.out.println("\n  {'='*70}");
            System.out.println("  {dim.value.upper()} -- {policy.right_type.value}");
            ...
            System.out.println("  Meta: {policy.target}");
        // === 2. Citizen Assessment ===
        System.out.println("\n\n  === AVALIACAO INDIVIDUAL ===\n");
        /* TODO: for-each Java para cid em ["C-001", "C-002", "C-005", "C-007"] */
            result = system.assess(cid);
            System.out.println("\n  {result['citizen']} (overall: {result['overall_score']} -- {result['status']})");
            System.out.println("    Mais forte: {result['strongest']}");
            System.out.println("    Mais fragil: {result['weakest']}");
            if (result["barriers"]) {
                System.out.println("    Barreiras: {', '.join(result['barriers'])}");
            if (result.get("intervention_needed")) {
                System.out.println("    INTERVENCAO: {result['intervention_needed'].value}");
        // === 3. Republic Report ===
        System.out.println("\n\n  === RELATORIO DA REPUBLICA ===\n");
        report = system.republic_report();
        System.out.println("  Cidadaos avaliados: {report['citizens_assessed']}");
        System.out.println("  Score geral da Republica: {report['overall_republic_score']}");
        System.out.println("\n  Por dimensao:");
        /* para cada (dim, score) em ordene(report["by_dimension"].items(), key=(x) -> x[1]): */
            bar = "#" * inteiro(score / 5);
            flag = score < 60 ? " !!!" : "";
            System.out.println("    {dim:<15} {score:>5.1f}  {bar}{flag}");
        System.out.println("\n  Dimensoes abaixo da meta:");
        /* TODO: for-each Java para item em report["below_target"] */
            System.out.println("    {item['dimension']}: {item['average']} -> {item['action'][:80]}");
        System.out.println("\n  Barreiras mais comuns:");
        /* para cada (barrier, count) em report["barriers_common"]: */
            System.out.println("    {barrier} ({count} cidadaos)");
        // === Philosophy ===
        System.out.println("\n\n{'='*80}");
        System.out.println("  FILOSOFIA: CUIDADO FISICO");
        System.out.println("{'='*80}");
        System.out.println(""";
