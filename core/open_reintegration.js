...
class ReintegrationPlan {
    ...
    citizen_name: texto;
    age: inteiro;
    const original_crime = "";
    const years_served = 0.0;
    const skills_learned = field(default_factory=list);
    const treatment_needs = field(default_factory=list);
    // Fase atual
    const phase = ReintegrationPhase.PRE_RELEASE;
    const release_date = "";
    const days_since_release = 0;
    // Suporte
    const needs = field(default_factory=dict);
    const housing_location = "";
    ...
    const milestones_achieved = field(default_factory=list);
    const flags = field(default_factory=list) // alertas (! infracoes);
    // Reincidencia
    const recidivated = false;
    const recidivism_reason = ""  // se voltou, POR QUE? (culpa do sistema);
    init_needs(self) {
        // Inicializa todas as necessidades como pendentes.
        for (const need of SupportNeed) {
...
class InfrastructureProvider {
    ...
    provide_openkit(self, citizen_id: texto) {
    ...
    funcao assign_community(self, citizen_id: texto,
                        const group_name = "") -> {texto: qualquer}:;
        ...
        for (const g of self.community_groups) {
            if (g["name"] == group) {
    ...
    funcao provide_healthcare(self, citizen_id: texto,
                        const needs_treatment = null) -> {texto: qualquer}:;
...
class RecordExpungement {
    ...
    funcao check_eligibility(self, citizen_id: texto,
                        const was_hediondous = false) -> {texto: qualquer}:;
    ...
    verify_clean_record(self, citizen_id: texto) {
...
class ReintegrationEngine {
    ...
    funcao create_plan(self, citizen_id: texto, citizen_name: texto, age: inteiro,
                    ...
                    const treatment_needs = null) -> ReintegrationPlan:;
    ...
    funcao prepare_release(self, plan_id: texto,
                        const mentor_name = "Mentor Voluntario",;
                        const mentor_id = "M-001") -> {texto: qualquer}:;
    ...
    report_recidivism(self, plan_id: texto, reason: texto) {
    ...
    stats(self) {
        ...
        in_progress = soma(1 para p em self.plans.values();
                        ...
                        && ! p.recidivated);
        total_outcome = .length(self.plans);
        rate = total_outcome > 0 ? (recidivated / total_outcome * 100) : 0;
        return {;
            "total_plans": .length(self.plans),;
            ...
            "expunged_records": .length(self.expungement.expunged),;
            "housing_provided": .length(self.infra.housing_units),;
            "mentors_active": .length(self.infra.mentors),;
            "community_groups": .length(self.infra.community_groups),;
        };
// ============================================================================
// 6. MAIN
// ============================================================================
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENREINTEGRATION");
    console.log("  Infraestrutura Completa para Ex-Penitenciarios");
    console.log("  'Para que ! precisem cometer erros.'");
    console.log("=" * 80);
    ...
    para cid, name, age, crime, served, skills, *treatments in cases: {
        ...
        console.log("  [{plan.plan_id}] {name} ({age} anos)");
        console.log("    Crime original: {crime}");
        console.log("    Tempo cumprido: {served:.1f} anos");
        console.log("    Skills: {', '.join(skills[:3])}");
        if (treatment) {
            console.log("    Tratamento: {', '.join(treatment)}");
        console.log("    Readiness inicial: {plan.readiness_score:.0f}%");
    ...
    para cid, name, *_ in cases: {
        ...
        if (plans) {
            ...
            console.log("    Moradia: {prep['details']['housing']['location']}");
            console.log("    Trabalho: {prep['details']['employment']['job']}");
            console.log("    Mentor: {prep['details']['mentor']['mentor']}");
            console.log("    Comunidade: {prep['details']['community']['group']}");
            console.log("    Saude: {prep['details']['health']['level']}");
            console.log("    OpenKit: {prep['details']['kit']['cost']}");
            console.log("    Pode liberar: {'SIM' if prep['can_release'] else 'NAO'}");
    // === 3. LIBERACAO ===
    ...
    para cid, name, *_ in cases: {
        ...
        if (plans) {
            ...
            if (release.get("released")) {
                console.log("    LIBERADO com infraestrutura completa");
                console.log("    Na rua: NAO -- {release['housing']}");
                console.log("    Abandonado: NAO -- mentor + comunidade");
                console.log("    Trabalho: {release['work']}");
                ex = release.get("prontuario", {});
                if (ex.get("expunged")) {
                    console.log("    Prontuario: {ex['after']}");
                    console.log("    Visivel a: {ex['visible']}");
            } else {
                console.log("    {release.get('error', 'ERRO')}");
    ...
    para cid, name, *_ in cases: {
        ...
        if (plans) {
            ...
            for (const _ of intervalo(4)) {
                ci = engine.check_in(plans[0].plan_id, "bom", "");
            console.log("  {name}: {ci['days_since_release']} dias -> {ci['phase']}");
    ...
    para cid, name, *_ in cases: {
        ...
        console.log("  {name:<20} -> {check['record']} ({check['status']})");
    ...
    if (plans_list) {
        ...
        console.log("  {rec['citizen']} reincidiu.");
        console.log("  Motivo: {rec['reason']}");
        console.log("  {rec['system_failure']}");
        console.log("  Suportes que falharam: {rec['failed_supports']}");
        console.log("  Acao: {rec['action']}");
    // === 7. COMPARACAO ===
    console.log("\n\n  === 7. COMPARACAO: SISTEMA ATUAL vs REPUBLICA ===\n");
    s = engine.stats();
    console.log("  {'Metrica':<30} {'Sistema Atual':>15} {'Republica':>15}");
    console.log("  {'-'*62}");
    console.log("  {'Reincidencia':<30} {'70%':>14} {s['recidivism_rate']:>14}");
    console.log("  {'Moradia ao sair':<30} {'Rua/favela':>15} {'Garantida':>15}");
    console.log("  {'Trabalho ao sair':<30} {'Nenhum':>15} {'Garantido':>15}");
    console.log("  {'Mentor':<30} {'Nenhum':>15} {'1 ano':>15}");
    console.log("  {'Saude':<30} {'SUS fila':>15} {'Sirio-Lib.':>15}");
    console.log("  {'Prontuario':<30} {'Marcado':>15} {'LIMPO':>15}");
    console.log("  {'Comunidade':<30} {'Isolado':>15} {'Apoio mutuo':>15}");
    console.log("  {'Custo':<30} {'R$ 42k/ano':>15} {'ZERO':>15}");
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA DA REINTEGRACAO");
    console.log("{'='*80}");
    console.log(""";
...
// )
    console.log("{'='*80}");
    console.log("  OpenReintegration: {s['total_plans']} planos, ";
        "{s['fully_reintegrated']} integrados, ";
        "{s['expunged_records']} prontuarios limpos.");
    console.log("  Reincidencia: {s['recidivism_rate']} (vs 70% atual).");
    console.log("  Prevenir > Punir. Sempre.");
    console.log("{'='*80}");
