...
class Relationship {
    ...
    const status = RelationshipStatus.ACTIVE;
    const declared_date = "";
    const ended_date = "";
    const public = true // TODOS sao publicos (regra);
    const children = 0;
    const notes = ""  // optional (P2 -- sem detalhar intimidade);
    // Anti-inveja
    const public_recognition = "relacao_normal"  // ! && "melhor" nem "pior";
// ============================================================================
// 2. ANTI-INVEJA
// ============================================================================
class EnvyType {
...
// ============================================================================
run_relationship_assembly(n_voters: inteiro = 10000) {
...
class RelationshipEngine {
    ...
    funcao end_relationship(self, rel_id: texto,
                        const mutual = true) -> {texto: qualquer}:;
    ...
    check_status(self, person: texto) {
        // Verifica status relacional de uma pessoa (publico).
        active = [];
        ended = [];
        for (const rel of self.relationships.values()) {
        ...
        return {;
            ...
            "past_relationships": .length(ended),;
            "available": .length(active) == 0,;
            "message": (;
    ...
    funcao report_envy(self, person: texto, target: texto,
                    const envy_type = EnvyType.RELATIONAL,;
    ...
    stats(self) {
        return {;
            "total_relationships": .length(self.relationships),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENRELATIONSHIPS -- TRANSPARENCIA RELACIONAL + ANTI-INVEJA");
    console.log("  Assembleia Constituinte sobre relacionamentos publicos");
    console.log("=" * 80);
    // === 1. PERGUNTA A ASSEMBLEIA ===
    console.log("\n\n  === 1. PERGUNTA A ASSEMBLEIA ===\n");
    console.log('  "Todo relacionamento deve ser publico para evitar');
    console.log('   invasao de espaco && combater inveja?"');
    // === 2. VOTACAO ===
    console.log("\n\n  === 2. VOTACAO (10.000 cidadaos) ===\n");
    result = run_relationship_assembly(10000);
    console.log("  PUBLICO TOTAL (fato + detalhe): {result['votes_public_total']:>6} ";
        "({result['votes_public_total']/100:.0f}%)");
    console.log("  PARCIAL (fato sim, detalhe !): {result['votes_partial_p2']:>6} ";
        "({result['votes_partial_p2']/100:.0f}%)");
    console.log("  PRIVADO (cada um decide):       {result['votes_private']:>6} ";
    ...
    para a, b, rtype in rels: {
        ...
        console.log("  {a:<12} + {b:<12} -> {rtype.value:<15} ";
            "[{result_decl['status']}]");
    // === 5. VERIFICAR STATUS ===
    console.log("\n\n  === 5. VERIFICACAO DE STATUS (anti-invasao) ===\n");
    for (const person of ["Cleiton", "Carlos", "Joao", "Ana"]) {
        status = engine.check_status(person);
        avail = status["available"] ? "DISPONIVEL" : "EM RELACIONAMENTO";
        console.log("  {person:<12} -> {avail}", end="");
        if (status["current_relationships"]) {
            for (const rel of status["current_relationships"]) {
                console.log(" ({rel['partner']}, {rel['type']})", end="");
        console.log();
    ...
    if (rels_list) {
        ...
        console.log("  {end_result['public_announcement']}");
        console.log("  {end_result['message']}");
    // === 7. ANTI-INVEJA ===
    console.log("\n\n  === 7. ANTI-INVEJA ===\n");
    envies = [;
    ...
    para person, target, etype, desc in envies: {
        ...
        console.log("  {result_envy['treatment'][:80]}");
    // === 8. STATS ===
    console.log("\n\n  === 8. ESTATISTICAS ===\n");
    s = engine.stats();
    ...
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA DO OPENRELATIONSHIPS");
    console.log("{'='*80}");
    console.log(""";
...
// )
    console.log("{'='*80}");
    console.log("  OpenRelationships: {s['total_relationships']} declarados, ";
        "{s['active']} ativos, ";
        "{s['envy_cases_treated']} casos de inveja tratados.");
    console.log("  Publico no fato. Privado na intimidade. Sem inveja.");
    console.log("{'='*80}");
