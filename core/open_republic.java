...
public class OpenrepublicFederacaoDeNacoesOpennation {
    ...
    public static class FederalProposalCategory {
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class NationMetrics {
        ...
        // decorador: @property
        public int carrying_capacity(self) {
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class FederalInfrastructure {
    ...
    public static class InfrastructureManager {
        ...
        funcao propose(self, name: texto, itype: InfrastructureType,
            ...
            return iid;
        public void contribute(self, infra_id: texto, nation_id: texto, labor_hours: flutuante) {
            proj = self.projects.get(infra_id);
            if (proj) {
                proj.contributing_nations.add(nation_id);
                proj.labor_invested += labor_hours;
        public {texto: qualquer} advance_build(self, infra_id: texto) {
        ...
        public {texto: qualquer} summary(self) {
            ...
            /* TODO: for-each Java para proj em self.projects.values() */
    ...
    public static class MigrationSystem {
        ...
        public {texto: qualquer} flow_stats(self) {
            // Estatisticas de fluxo migratorio.
            by_reason = defaultdict(inteiro);
            /* TODO: for-each Java para e em self.events */
                by_reason[&&.reason] += 1;
    ...
    public static class Treaty {
        ...
        String status = "proposed"  // proposed, active, violated, dissolved;
        int created_cycle = 0;
    public static class TreatySystem {
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class FederalProposal {
        ...
        String status = "open"  // open, passed, rejected;
        int cycle_submitted = 0;
        String affects_infra = ""  // infra_id if applicable;
    public static class FederalGovernance {
        ...
        public {texto: qualquer} close(self, proposal_id: texto, total_nations: inteiro) {
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class GrandProject {
        ...
        {texto} contributing_nations = field(default_factory=set);
        String status = "recruiting"  // recruiting, active, completed;
        String benefit = ""  // quem se beneficia;
    public static class GrandProjectManager {
        ...
        funcao propose(self, name: texto, description: texto, required_nations: inteiro,
            ...
            self.projects[pid] = GrandProject(;
                ...
                benefit = benefit);
            return pid;
        public void contribute(self, project_id: texto, nation_id: texto, labor_hours: flutuante) {
            proj = self.projects.get(project_id);
            if (! proj) {
                return null;
            proj.contributing_nations.add(nation_id);
            proj.current_labor += labor_hours;
            if (tamanho(proj.contributing_nations) >= proj.required_nations  &&  proj.status == "recruiting") {
                proj.status = "active";
            if (proj.current_labor >= proj.required_labor_hours  &&  proj.status == "active") {
                proj.status = "completed";
        public {texto: qualquer} progress(self, project_id: texto) {
            proj = self.projects.get(project_id);
            if (! proj) {
                return {};
            return {;
    ...
    // Republic
    // ============================================================================
    public static class OpenRepublic {
        ...
        //
        public void __init__(self, name: texto = "OpenRepublic") {
            self.name = name;
            ...
            self.history: [Dict] = [];
        public void admit_nation(self, nation: Nation) {
            self.nations[nation.nation_id] = nation;
            nation.founded_cycle = self.current_cycle;
        public int total_population(self) {
        ...
        public {texto: qualquer} run_cycle(self) {
            ...
            if (tamanho(nation_list) >= 2) {
                /* TODO: for-each Java para _ em intervalo(random.randint(5, 20)) */
            ...
            infra_completed = 0;
            /* TODO: for-each Java para iid em list(self.infrastructure.projects.keys()) */
                // Random nations contribute labor
                /* TODO: for-each Java para n em random.sample(nation_list, minimo(3, tamanho(nation_list))) */
            ...
            // 3. Treaty signing
            if (random.random() < 0.3 && tamanho(nation_list) >= 2) {
                types = [;
                ...
                if (random.random() < 0.5 && tamanho(nation_list) > 2) {
            ...
            /* TODO: for-each Java para _ em intervalo(proposals) */
                cats = [;
                ...
                /* TODO: for-each Java para n em nation_list */
            ...
            /* TODO: for-each Java para pid em list(self.grand_projects.projects.keys()) */
                /* TODO: for-each Java para n em random.sample(nation_list, minimo(2, tamanho(nation_list))) */
            ...
            return cycle_data;
        public String full_report(self) {
            ...
            /* TODO: for-each Java para n em nations_sorted */
    ...
    // ============================================================================
    public OpenRepublic create_sample_republic() {
    ...
    if (__name__ == "__main__") {
        random.seed(42);
        np.random.seed(42);
        System.out.println("Criando Republica dos Bens Comuns com 6 nacoes...\n");
        republic = create_sample_republic();
        System.out.println("Nacoes fundadoras: {len(republic.nations)}");
        /* para cada (nid, n) em republic.nations.items(): */
            System.out.println("  {n.name:<15} ({n.region:<15}) pop={n.metrics.population:>6}  ";
                "QoL={n.metrics.qol_index:.0f}  cap={n.metrics.carrying_capacity:>6}  ";
                "especialidades: {', '.join(n.specialties[:2])}");
        // Run 12 cycles
        System.out.println("\nSimulando 12 ciclos...\n");
        /* TODO: for-each Java para c em intervalo(12) */
            result = republic.run_cycle();
            if (c < 3 || c >= 9) {
                System.out.println("  Ciclo {result['cycle']:2d}: migracoes={result.get('migrations',0):2d}  ";
                    "infra_completas={result.get('infra_completed',0)}  ";
                    "federal={len(result.get('federal_results',[]))}");
                /* TODO: for-each Java para fr em result.get("federal_results", []) */
                    System.out.println("    -> {fr['proposal'][:45]:<45} {fr['result']}");
        ...
        /* para cada (pid, proj) em republic.grand_projects.projects.items(): */
            ...
            System.out.println("  {proj.name[:30]:<30} [{bar}] {p['labor_progress_pct']:>5.1f}%  ";
