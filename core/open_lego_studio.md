...
**Arquivo original:** `core/open_lego_studio.py`
...
classe BlockColor herda de Enum:
...
classe BlockShape herda de Enum:
...
classe CanvasSlot herda de Enum:
    // Estado de um slot no canvas.
    EMPTY = "vazio"
    OCCUPIED = "ocupado"
    VALID_TARGET = "alvo_valido"  // verde (pode soltar aqui)
    INVALID_TARGET = "alvo_invalido"  // vermelho (nao pode)
    HOVER = "hover"
...
classe VisualBlock:
    ...
    Tem PINOS de entrada (esquerda) e saida (direita).
    // 
    instance_id: texto
    ...
    color: BlockColor
    shape: BlockShape
    seja x: inteiro = 0 // posicao no canvas
    seja y: inteiro = 0
    seja width: inteiro = 20 // largura visual
    seja height: inteiro = 5 // altura visual

    // Pinos
    seja input_pins: [Dict] = field(default_factory=list)  // [{"name", "type", "connected"}]
    seja output_pins: [Dict] = field(default_factory=list)

    // Estado
    seja executing: logico = falso
    seja has_error: logico = falso
    seja error_msg: texto = ""
    seja result_value: qualquer = nulo

    // decorador: @property
    funcao center_x(self) -> inteiro:
        retorne self.x + self.width // 2

    // decorador: @property
    funcao center_y(self) -> inteiro:
        retorne self.y + self.height // 2
...
classe VisualConnection:
    ...
    seja color: texto = "verde"  // verde=valido, vermelho=invalido

    // Renderizacao
    seja wire_path: List[(inteiro, inteiro)] = field(default_factory=list)
...
classe Canvas:
    ...
    Cada celula pode conter parte de um bloco ou fio de conexao.
    // 
    seja width: inteiro = 80
    seja height: inteiro = 40
    seja grid: List[[texto]] = field(default_factory=list)
    seja blocks: {texto: VisualBlock} = field(default_factory=dict)
    seja connections: [VisualConnection] = field(default_factory=list)
    seja cursor_x: inteiro = 0
    seja cursor_y: inteiro = 0
    seja selected_block: texto? = nulo
    seja dragging_block: texto? = nulo
    seja snap_target: CanvasSlot? = CanvasSlot.EMPTY

    funcao __post_init__(self):
        self.grid = [[" " para _ em intervalo(self.width)] para _ em intervalo(self.height)]

    funcao place_block(self, block: VisualBlock) -> None:
        self.blocks[block.instance_id] = block

    funcao render(self) -> texto:
        ...
        para cada block em self.blocks.values():
        ...
        para cada row em self.grid:
            lines.append("|" + "".join(row) + "|")
        lines.append("+" + "-" * self.width + "+")
        retorne "\n".join(lines)

    funcao _render_block(self, block: VisualBlock) -> None:
        ...
        b = block
        // Verificar limites
        se b.y >= self.height ou b.x >= self.width entao:
            retorne nulo

        // Moldura superior
        para cada i em intervalo(minimo(b.width, self.width - b.x)):
            se b.y < self.height e b.x + i < self.width entao:
                self.grid[b.y][b.x + i] = "-"

        // Linha do titulo
        title = " {icon} {b.name[:b.width-4]} "
        para cada (i, ch) em enumere(title):
            se b.y + 1 < self.height e b.x + i < self.width entao:
                i < b.width ? self.grid[b.y + 1][b.x + i] = ch : " "

        // Pinos de entrada (esquerda)
        para cada (idx, pin) em enumere(b.input_pins):
            se b.y + 2 + idx < self.height entao:
                ...
                para cada (i, ch) em enumere(pin_label[:b.width-2]):
                    se b.x + 1 + i < self.width entao:
                        self.grid[b.y + 2 + idx][b.x + 1 + i] = ch

        // Pinos de saida (direita)
        para cada (idx, pin) em enumere(b.output_pins):
            y_pos = b.y + 2 + idx
            se y_pos < self.height entao:
                pin_label = "{pin['name']}:{pin['type'][:3]}>"
                conn_marker = pin.get("connected") ? "*" : "o"
                start_x = b.x + b.width - tamanho(pin_label) - 1
                se start_x > 0 entao:
                    para cada (i, ch) em enumere(pin_label):
                        se start_x + i < self.width entao:
                            self.grid[y_pos][start_x + i] = ch
                se b.x + b.width < self.width entao:
                    self.grid[y_pos][b.x + b.width] = conn_marker

        // Moldura inferior
        bottom_y = b.y + b.height - 1
        para cada i em intervalo(minimo(b.width, self.width - b.x)):
            se bottom_y < self.height e b.x + i < self.width entao:
                self.grid[bottom_y][b.x + i] = "-"

        // Marcador de erro
        se block.has_error entao:
            se b.y + 1 < self.height e b.x + b.width - 2 < self.width entao:
                self.grid[b.y + 1][b.x + b.width - 2] = "X"

        // Marcador de execucao
        se block.executing entao:
            se b.y + 1 < self.height e b.x < self.width entao:
                self.grid[b.y + 1][b.x] = ">"

    funcao _render_wire(self, conn: VisualConnection) -> None:
        ...
        para cada x em intervalo(x1, minimo(mid_x, self.width)):
            se y1 < self.height  e  self.grid[y1][x] == " " entao:
                self.grid[y1][x] = wire_char

        // Vertical de y1 ate y2
        step = y2 > y1 ? 1 : -1
        para cada y em intervalo(y1, y2, step):
            se 0 <= y < self.height e 0 <= mid_x < self.width entao:
                se self.grid[y][mid_x] == " " entao:
                    self.grid[y][mid_x] = wire_char

        // Horizontal de mid_x ate entrada
        para cada x em intervalo(maximo(mid_x, 0), minimo(x2, self.width)):
            se 0 <= y2 < self.height  e  self.grid[y2][x] == " " entao:
...
classe BlockPalette:
    ...
    Cidadao clica (ou seleciona) um bloco da paleta.
    Depois clica no canvas onde quer colocar.
    Bloco aparece no canvas.
    // 
    seja categories: Dict[texto, [Dict]] = field(default_factory=dict)
    ...
    funcao render(self) -> texto:
        ...
        para cada (cat, blocks) em self.categories.items():
            lines.append("\n  [{cat.upper()}]")
            para cada b em blocks:
                lines.append("    {b['id']:<12} {b['name']}")
        retorne "\n".join(lines)


funcao build_palette() -> BlockPalette:
...
classe LegoAssistant:
    ...
    Cidadao diz: "Quero verificar se frase e preconceito"
    ...
    funcao understand(self, description: texto) -> {texto: qualquer}:
        // Entende descricao e sugere cadeia de blocos.
        desc = description.lower()

        // Matching simples
        selected_template = nulo
        matched_keywords = []
        ...
        // Busca adicional
        se nao selected_template entao:
            se any(w in desc para w em ["credito", "pagar", "trabalho"]) entao:
                selected_template = self.templates["credito"]
            senao se any(w in desc para w em ["preconceito", "racismo", "fact"]) entao:
                selected_template = self.templates["fact_check"]
            senao se any(w in desc para w em ["traduzir", "traducao"]) entao:
                selected_template = self.templates["traduzir"]
            senao se any(w in desc para w em ["resumir", "resumo"]) entao:
                selected_template = self.templates["resumir"]
            senao se any(w in desc para w em ["diagnostic", "saude", "doente"]) entao:
    ...
    funcao auto_connect(self, template: [texto]) -> [Dict]:
        // Gera conexoes automaticas baseado no template.
        connections = []
        para cada i em intervalo(tamanho(template) - 1):
            connections.append({
                "from": template[i],
                "from_pin": 0,
                "to": template[i + 1],
...
// decorador: @dataclass
classe GalleryProgram:
    // Programa compartilhado na galeria.
    program_id: texto
    name: texto
    author: texto
    seja description: texto = ""
    seja blocks: [texto] = field(default_factory=list)
    seja downloads: inteiro = 0
    seja rating: flutuante = 0.0


funcao build_gallery() -> [GalleryProgram]:
    retorne [
        ...
        GalleryProgram("G-002", "Fact-Check Preconceito",
            "Maria", "Verifica frase preconceituosa e corrige",
...
classe LegoStudio:
    ...
    funcao new_program(self, name: texto) -> {texto: qualquer}:
        // Cria novo programa LEGO.
        self.current_program = name
        self.canvas = Canvas(width=80, height=36)
        retorne {"created": verdadeiro, "program": name, "message": "Novo programa: {name}"}

    funcao drag_drop_block(self, block_id: texto, x: inteiro, y: inteiro) -> {texto: qualquer}:
        ...
        para cada (prefix, c) em color_map.items():
            se block_id.startswith(prefix) entao:
                color = c
                interrompa
        ...
        input_pins = []
        output_pins = []
        se block_id.startswith("IN-") entao:
            output_pins = [{"name": "valor", "type": "num/str", "connected": falso}]
        senao se block_id.startswith("MATH-")  ou  block_id.startswith("LOGIC-") entao:
            input_pins = [{"name": "a", "type": "num", "connected": falso},
                          {"name": "b", "type": "num", "connected": falso}]
            output_pins = [{"name": "result", "type": "num/bool", "connected": falso}]
        senao se block_id.startswith("REP-") entao:
            input_pins = [{"name": "dados", "type": "any", "connected": falso}]
            output_pins = [{"name": "result", "type": "any", "connected": falso}]
        senao se block_id == "OUT" entao:
            input_pins = [{"name": "resultado", "type": "any", "connected": falso}]
        senao se block_id.startswith("IA-") entao:
            input_pins = [{"name": "texto", "type": "str", "connected": falso}]
            output_pins = [{"name": "result", "type": "str", "connected": falso}]
    ...
    funcao draw_wire(self, from_inst: texto, from_pin: inteiro,
                  ...
                  seja valid: logico = verdadeiro) -> {texto: qualquer}:
        ...
        se from_inst in self.canvas.blocks entao:
            para cada p em self.canvas.blocks[from_inst].output_pins:
                p["connected"] = verdadeiro
        se to_inst in self.canvas.blocks entao:
            para cada p em self.canvas.blocks[to_inst].input_pins:
                p["connected"] = verdadeiro
    ...
    funcao validate(self) -> {texto: qualquer}:
        // Valida programa atual.
        blocks = tamanho(self.canvas.blocks)
        connections = tamanho(self.canvas.connections)
        invalid = soma(1 para c em self.canvas.connections if nao c.valid)

        retorne {
            "blocks": blocks,
            "connections": connections,
            "invalid_connections": invalid,
            "valid": invalid == 0  e  blocks > 0,
            "ready_to_run": invalid == 0  e  blocks > 0  e  connections > 0,
        }

    funcao execute(self) -> {texto: qualquer}:
        // Executa programa (simula fluxo visual).
        val = self.validate()
        se nao  val["ready_to_run"] entao:
            retorne {"error": "Programa invalido ou incompleto"}

        // Marcar todos como executando em sequencia
        self.execution_log = []
        para cada (inst_id, block) em self.canvas.blocks.items():
            block.executing = verdadeiro
    ...
    funcao ai_assemble(self, description: texto) -> {texto: qualquer}:
        ...
        x = 2
        para cada block_id em template:
        ...
        para cada (i, conn) em enumere(auto_conns):
            se i + 1 < tamanho(block_instances) entao:
                self.draw_wire(block_instances[i], 0, block_instances[i + 1], 0)

        retorne {
            "assembled": verdadeiro,
            "description": description,
...
se __name__ == "__main__" entao:
    ...
    imprima("=" * 80)
    imprima("  OPENLEGOSTUDIO -- INTERFACE VISUAL DE PROGRAMACAO LEGO")
    imprima("  Arrastar. Soltar. Encaixar. Sem texto. Sem sintaxe.")
    imprima("=" * 80)

    // === 1. IA MONTA PROGRAMA POR DESCRICAO ===
    imprima("\n\n  === 1. IA MONTA PROGRAMA ===\n")
    imprima("  Cidadao diz: 'Quero calcular credito de trabalho'\n")
    r = studio.ai_assemble("calcular credito de trabalho baseado em horas")
    imprima("  IA: {r['message']}")
    imprima("  Blocos colocados: {r['blocks_placed']}")
    imprima("  Conexoes feitas: {r['connections_made']}")
    ...
    studio.draw_wire(blocks[0], 0, blocks[1], 0, valid=falso)
    imprima(studio.render())
    imprima("\n  [!] Fio VERMELHO: pino 'texto' NAO encaixa em 'numero'.")
    imprima("  Visual: cidadao ve o erro SEM precisar ler codigo.")

    // === 7. GALERIA COMUNITARIA ===
    imprima("\n\n  === 7. GALERIA COMUNITARIA ===\n")
    para cada prog em studio.gallery:
        stars = "*" * inteiro(prog.rating)
        imprima("  [{prog.program_id}] {prog.name:<30} por {prog.author:<12} "
              "{prog.downloads:>4} downloads  {stars} ({prog.rating})")
        imprima("    {prog.description}")
        imprima("    Blocos: {' -> '.join(prog.blocks)}")

    // === 8. IA ASSISTENTE EM ACAO ===
    imprima("\n\n  === 8. IA ASSISTENTE (entende linguagem natural) ===\n")
    queries = [
        "quero verificar se uma frase e racista",
    ...
    para cada q em queries:
        result = studio.assistant.understand(q)
        imprima("  '{q}'")
        imprima("    -> IA entendeu. {result['block_count']} blocos.")
        imprima("    -> Cadeia: {' -> '.join(result['template'])}")
        imprima()

    // === FILOSOFIA ===
    imprima("\n{'='*80}")
    imprima("  FILOSOFIA DO OPENLEGOSTUDIO")
    imprima("{'='*80}")
    imprima("""
  ...
  IA ASSISTENTE (monta por voz/texto):
    ...
    Cidadao nao precisa saber quais blocos existem. IA sabe.
  ...
  PRINCIPIOS:
    ...
    P2: Bloco e autonomo. Encaixa ou nao encaixa. Sem ambiguidade.
    seja P3: Criar bloco reusavel = trabalho de alto impacto.
    P4: Galeria e bem comum. Todos usam. Todos contribuem.
// )
    imprima("{'='*80}")
    imprima("  OpenLegoStudio: interface visual para programar encaixando pecas.")
    imprima("  Sem texto. Sem sintaxe. Sem erro. So LEGO.")
    imprima("{'='*80}")
