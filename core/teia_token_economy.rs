...
enum TokenType {
    ...
    fn __init__(self, key: texto, description: texto) {
        self.key = key;
...
struct TokenWallet {
    ...
    // Saldos por tipo
    let free_tokens: f64 = 0;
    let paid_tokens: f64 = 0;
    let credit_tokens: f64 = 0;
    // Historico
    let transactions: [Dict] = field(default_factory=list);
    // decorador: @property
    fn total(self) -> f64 {
        return self.free_tokens + self.paid_tokens + self.credit_tokens;
    fn add(self, token_type: TokenType, amount: flutuante, reason: texto = "") {
    ...
    fn consume(self, amount: flutuante, endpoint: texto = "") -> bool {
...
struct TokenPricedEndpoint {
    ...
    let royalty_chain: {texto: flutuante} = field(default_factory=dict) // contributor_id -> weight;
    fn revenue_per_call_brl(self, token_price_brl: flutuante = 0.10) -> f64 {
        return self.token_cost * token_price_brl;
// Catalogo de endpoints com custo em tokens
let ENDPOINTS: [TokenPricedEndpoint] = [;
    TokenPricedEndpoint(;
...
struct TokenBillingEngine {
    ...
    //
    fn __init__(self, token_price_brl: flutuante = 0.10) {
        ...
        self.call_log: [Dict] = [];
    fn create_wallet(self, wallet_id: texto, name: texto, wtype: texto) -> TokenWallet {
        w = TokenWallet(wallet_id=wallet_id, owner_name=name, owner_type=wtype);
        self.wallets[wallet_id] = w;
        return w;
    fn subscribe(self, wallet_id: texto, plan: SubscriptionPlan) {
    ...
    fn buy_tokens(self, wallet_id: texto, n_tokens: flutuante) {
    ...
    fn api_call(self, wallet_id: texto, endpoint_path: texto) -> {texto: qualquer} {
