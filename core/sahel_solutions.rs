...
if __name__ == "__main__" {
    println!("=" * 75);
    println!("  SOLUCOES PARA OS 3 GARGALOS DO SAHEL");
    println!("  Sem dinheiro. Sem empreiteira. Sem 'contratar'.");
    println!("  Com automatizacao + comunidade + conhecimento aberto.");
    println!("=" * 75);
    // === SOLUCAO 1: CUIDADO INFANTIL ===
    println!("\n\n" + "=" * 75);
    println!("  GARGALO 1: CUIDADO INFANTIL (300% sobrecarga)");
    println!("  1 pessoa tentando cuidar de 300 criancas");
    println!("=" * 75);
    ...
    println!("  Demanda ajustada (IA reduz 25%): {childcare['demanda_ajustada_por_ia']:,}");
    println!("\n  FONTES DE CUIDADO:");
    println!("  {'Fonte':<35} {'Pessoas':>8} {'Equiv FT':>10}");
    println!("  {'-'*55}");
    para cada (fonte, dados) em childcare['fontes_de_cuidado'].items(): {
        if 'pessoas' in dados {
            println!("  {fonte.replace('_', ' ').title():<35} {dados['pessoas']:>8} ";
                "{dados['equiv_fulltime']:>10}");
        } else if 'estacoes' in dados {
            println!("  {fonte.replace('_', ' ').title():<35} {dados['estacoes']:>8} estacoes");
    println!("  {'-'*55}");
    println!("  {'CAPACIDADE TOTAL':<35} {'':>8} {childcare['capacidade_total_equiv']:>10}");
    println!("  {'DEMANDA TOTAL':<35} {'':>8} {childcare['demanda_total_equiv']:>10}");
    println!("\n  COBERTURA: {childcare['cobertura']}");
    println!("  VIÁVEL: {'SIM' if childcare['viavel'] else 'NAO'}");
    println!("\n  PRINCIPIO: Nao se trata de contratar 75 profissionais.");
    println!("  Trata-se de ativar a COMUNIDADE INTEIRA:");
    println!("    - Cada pai/mae: 4h/semana");
    println!("    - Cada idoso ativo: 10h/semana (continuar contribuindo)");
    println!("    - Cada adolescente: 6h/semana (mentoria + responsabilidade)");
    println!("    - Estacoes de IA: 25% do aprendizado && autonomo");
    println!("  Resultado: a comunidade SE cuida. Nao precisa de Estado.");
    // === SOLUCAO 2: AGRICULTURA ===
    println!("\n\n" + "=" * 75);
    println!("  GARGALO 2: AGRICULTURA (167% sobrecarga)");
    println!("  Faltam 2.000 kg/mes de comida para 40.000 pessoas");
    println!("=" * 75);
    ...
    println!("\n  ESTRATEGIAS:");
    println!("  {'Estrategia':<30} {'Escala':>15} {'Producao':>12} {'Pessoas':>8}");
    println!("  {'-'*68}");
    para cada (nome, dados) em agriculture['estrategias'].items(): {
        if isinstance(dados, dict) {
            ...
            println!("  {nome.replace('_', ' ').title():<30} {escala:>15} {prod:>12} {pessoas:>8}");
    println!("\n  {'PRODUCAO NOVA TOTAL':<30} {'':>15} {agriculture['producao_total_nova_kg_mes']:>9} kg/mes");
    println!("  {'TRABALHADORES':<30} {'':>15} {'':>12} {agriculture['trabalhadores_necessarios']:>8}");
    println!("  COBRE O GAP: {'SIM' if agriculture['cobre_gap'] else 'NAO'}");
    println!("  AUTO-SUSTENTAVEL APOS: {agriculture['auto_sustentavel_apos']}");
    println!("\n  PRINCIPIO: Hidroponia (10x mais por m2) + permacultura");
    println!("  (auto-sustentavel apos 3 anos) + aquaponia (proteina + vegetais)");
    println!("  + automacao IoT (40% menos trabalho) + conhecimento da Amazonia");
    // === SOLUCAO 3: HABITACAO ===
    println!("\n\n" + "=" * 75);
    println!("  GARGALO 3: HABITACAO (167% sobrecarga)");
    println!("  Faltam ~2.000 unidades para 40.000 pessoas");
    println!("=" * 75);
    ...
    println!("\n  ESTRATEGIAS:");
    println!("  {'Estrategia':<25} {'Unid/mes':>10} {'Material':<25} {'Pessoas':>8}");
    println!("  {'-'*70}");
    para cada (nome, dados) em housing['estrategias'].items(): {
        if isinstance(dados, dict)  &&  ('casas_por_mes' in dados  ||  'kits_por_mes' in dados) {
            ...
            println!("  {nome.replace('_', ' ').title():<25} {unid:>10} {mat:<25} {pessoas:>8}");
        } else if isinstance(dados, dict)  &&  'massa_termica' in dados {
            println!("  {'Design Passivo':<25} {'N/A':>10} {'Sem ar condicionado':<25}");
    println!("\n  {'TOTAL':<25} {housing['casas_por_mes_total']:>10}");
    println!("  MESES PARA COBRIR GAP: {housing['meses_para_cobrir_gap']}");
    println!("  TRABALHADORES: {housing['trabalhadores_necessarios']}");
    println!("  CUSTO: {housing['custo_monetario']}");
    println!("  MATERIAL: {housing['material_principal']}");
    println!("\n  PRINCIPIO: Terra local && gratuita. Prensa manual faz bloco.");
    println!("  Impressora 3D faz casa em 30h. Mutirao constroi 10 casas/semana.");
    println!("  Design passivo: 40cm de parede de terra = sem ar condicionado.");
    // === RESUMO INTEGRADO ===
    println!("\n\n" + "=" * 75);
    println!("  RESUMO: COMO RESOLVER OS 3 GARGALOS SEM DINHEIRO");
    println!("=" * 75);
    println!(""";
+-----------+----------------+------------------+----------+--------+;
...
| | automacao | | drones | |;
+-----------+----------------+------------------+----------+--------+;
| Habitacao | 3D println! + | Terra local | ~40 + | 6-12 |;
