...
struct CommunityProfile {
    ...
    let cultural_pillars: [CulturalPillar] = field(default_factory=list);
    let existing_practices: {texto: texto} = field(default_factory=dict);
    // existing_practices = {"nome_ja_existente": "equivalente_republica"}
    // Necessidades especificas (o que FALTA)
    let needs: [texto] = field(default_factory=list);
    // Adaptações do modelo Republica
    let adapted_systems: {texto: texto} = field(default_factory=dict);
    // adapted_systems = {"sistema_republica": "como_adapta_para_esta_comunidade"}
    // O que NÃO se importa (respeito cultural)
    let do_not_import: [texto] = field(default_factory=list);
    // Governanca adaptada
    let governance_model: String = "";
    let decision_making: String = ""  // como decidem (consenso, voto, conselho);
    // Lingua
    let primary_language: String = "pt-BR";
    let secondary_languages: [texto] = field(default_factory=list);
    // Isolamento geografico
    let isolation_level: String = ""  // urbano, semi-isolado, isolado, muito isolado;
    let access_challenge: String = ""  // barreira de acesso (rio, estrada, etc);
// ============================================================================
// 3. OS 6 PERFIS
// ============================================================================
...
struct CommunityEngine {
    ...
    fn get_profile(self, community_type: CommunityType) -> {texto: qualquer} {
    ...
    fn adaptation_matrix(self) -> [Dict] {
        ...
        for system in ordene(systems) {
            ...
            matrix.append(row);
        return matrix;
    fn what_already_exists(self) -> {texto: Dict} {
    ...
    fn compare_communities(self) -> [Dict] {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENCOMMUNITIES -- 6 ADAPTACOES COMUNITARIAS");
    println!("  O modelo se ADAPTA. Nao se impoe.");
    println!("=" * 80);
    // === 1. COMPARACAO GERAL ===
    println!("\n\n  === 1. COMPARACAO GERAL ===\n");
    println!("  {'Comunidade':<25} {'Necessidades':>13} {'Sistemas':>10} {'Respeitos':>10} {'Praticas':>10}");
    println!("  {'-'*70}");
    for c in engine.compare_communities() {
        println!("  {c['comunidade']:<25} {c['necessidades']:>13} ";
    ...
    for ct in CommunityType {
        ...
        println!("  {'='*70}");
        println!("  {p['comunidade'].upper()}");
        println!("  {p['descricao'][:80]}...");
        println!("  {'='*70}\n");
        println!("  PILARES CULTURAIS:");
        ...
        para cada (practice, equivalent) em p["praticas_existentes"].items(): {
            println!("    {practice:<25} -> {equivalent}");
        ...
        para cada (system, adaptation) em p["sistemas_adaptados"].items(): {
            println!("\n    {system}:");
            println!("      {adaptation[:100]}...");
        println!("\n  O QUE NUNCA IMPORTAR ({len(p['o_que_nao_importar'])}):");
        for rule in p["o_que_nao_importar"] {
            println!("    X {rule}");
        println!("\n  GOVERNANCA: {p['governanca']}");
        println!("  DECISAO: {p['tomada_decisao']}");
        println!("  LINGUA: {p['lingua_principal']} {p['linguas_secundarias']}");
        println!("  ISOLAMENTO: {p['isolamento']}");
        println!("  ACESSO: {p['acesso']}");
    // === 3. STATS ===
    println!("\n\n  {'='*70}");
    ...
    para cada (k, v) em s.items(): {
        println!("  {k:<30} {v}");
    println!("\n{'='*80}");
    println!("  OpenCommunities: {s['comunidades']} comunidades adaptadas.");
    println!("  {s['total_necessidades']} necessidades mapeadas.");
    println!("  {s['total_sistemas_adaptados']} sistemas adaptados.");
    println!("  {s['total_respeitos_culturais']} respeitos culturais.");
    println!("  {s['principio']}");
    println!("{'='*80}");
