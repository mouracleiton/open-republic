...
class TokenType {
    ...
    __init__(self, key: texto, description: texto) {
        self.key = key;
...
class TokenWallet {
    ...
    const free_tokens = 0;
    const paid_tokens = 0;
    const credit_tokens = 0;
    // Historico
    const transactions = field(default_factory=list);
    // decorador: @property
    total(self) {
        return self.free_tokens + self.paid_tokens + self.credit_tokens;
    add(self, token_type: TokenType, amount: flutuante, reason: texto = "") {
    ...
    consume(self, amount: flutuante, endpoint: texto = "") {
...
class TokenPricedEndpoint {
    ...
    const royalty_chain = field(default_factory=dict) // contributor_id -> weight;
    revenue_per_call_brl(self, token_price_brl: flutuante = 0.10) {
...
class TokenBillingEngine {
    ...
    //
    __init__(self, token_price_brl: flutuante = 0.10) {
        ...
        self.call_log: [Dict] = [];
    create_wallet(self, wallet_id: texto, name: texto, wtype: texto) {
        w = TokenWallet(wallet_id=wallet_id, owner_name=name, owner_type=wtype);
        self.wallets[wallet_id] = w;
        return w;
    subscribe(self, wallet_id: texto, plan: SubscriptionPlan) {
    ...
    buy_tokens(self, wallet_id: texto, n_tokens: flutuante) {
    ...
    api_call(self, wallet_id: texto, endpoint_path: texto) {
...
print_token_system() {
    ...
    for (const ep of ENDPOINTS) {
        n_contribs = .length(ep.royalty_chain);
        lines.append(;
            "{ep.path:<38} ";
    ...
    para cada (name, cost) em examples: {
        ...
        for (const pname of ["FREE", "STARTER", "PRO", "GOV", "ENTR"]) {
    ...
    for (const cid of ["cleiton", "subagent_1", "subagent_3", "hermes"]) {
...
if (__name__ == "__main__") {
    console.log(print_token_system());
