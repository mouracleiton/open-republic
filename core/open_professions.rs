...
struct ProfessionSkill {
    ...
    let min_level: CareerLevel = CareerLevel.TECNICO // nivel minimo para fazer;
// decorador: @dataclass
#[derive(Debug, Clone)]
struct Profession {
    ...
    let description: String = "";
    let skills: [ProfessionSkill] = field(default_factory=list);
    // Criterios de promocao por nivel
    let promotion_criteria: {texto: Dict} = field(default_factory=dict);
    // Distribuicao ideal de carga
    let ideal_load: {texto: texto} = field(default_factory=dict);
// ============================================================================
// 3. CATALOGO DE PROFISSOES (30+)
// ============================================================================
fn build_profession_catalog() -> [Profession] {
...
struct ProfessionEngine {
    ...
    funcao recognize_self_taught(self, name: texto, profession: texto,
                            level: CareerLevel,;
                            let proof: String = "",;
                            let cases: i64 = 0) -> {texto: qualquer}:;
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENPROFESSIONS -- CARREIRA MODULAR PARA TODAS AS PROFISSOES");
    println!("  Competencia > Titulo. Cada um faz o que PODE.");
    println!("=" * 80);
    ...
    for area in ordene(by_area.keys()) {
        ...
        for p in by_area[area] {
            println!("    [{p.prof_id}] {p.name:<30} {len(p.skills)} skills");
    ...
    for prof_id in ["PR-MED", "PR-DEV", "PR-CHEF", "PR-BUILD"] {
        ...
        para cada (level_label, data) em path["career_path"].items(): {
            skills = data["skills"];
            if skills {
                println!("    [{level_label}] {data['meaning'][:40]}");
                println!("      Skills: {', '.join(skills[:4])}");
        if path.get("ideal_load") {
            println!("    Distribuicao ideal:");
    ...
    para name, prof, level, proof in self_taught: {
        ...
        println!("  {r['name']:<20} -> {r['profession']:<15} ({r['level']})");
    ...
    println!("  {'Profissao':<25} {'Valor':<18} {'Por que'}");
    println!("  {'-'*70}");
    for c in comparisons {
        println!("  {c['profissao']:<25} {c['valor']:<18} {c['por_que']}");
    println!("\n  SEM 'superior' vs 'inferior'.");
    println!("  Medico NAO vale mais que lixeiro.");
    println!("  Advogado NAO vale mais que cozinheiro.");
    println!("  Todos base 1.0 + impacto.");
    // === 6. STATS ===
    println!("\n\n  === 6. ESTATISTICAS ===\n");
    s = engine.stats();
    ...
    // === FILOSOFIA ===
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA: CARREIRA MODULAR PARA TODOS");
    println!("{'='*80}");
    println!(""";
COMPETENCIA > TITULO (P1 anti-elitismo):;
...
// )
    println!("{'='*80}");
    println!("  OpenProfessions: {s['total_profissoes']} profissoes, ";
