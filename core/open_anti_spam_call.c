...
#ifndef OPENANTISPAMCALL_LIGACAO_TELEFONICA_PREDATORIA_PROIBIDA_H
#define OPENANTISPAMCALL_LIGACAO_TELEFONICA_PREDATORIA_PROIBIDA_H
...
// 3. MOTOR ANTI-SPAM
// ============================================================================
typedef struct AntiSpamCallEngine {
    ...
    {texto: qualquer} consent_list(self) {
    ...
    {texto: qualquer} stats(self) {
        return {;
            "numeros_bloqueados": sizeof(self.blocked_numbers),;
...
// ============================================================================
{texto: qualquer} run_spam_assembly(n_voters: inteiro = 10000) {
    votes_ban = 0;
    votes_regulate = 0;
    votes_keep = 0;
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENANTISPAMCALL -- LIGACAO PREDATORIA PROIBIDA");
    ...
    /* para phone, caller, is_robot, authorized in test_calls: */
        ...
        printf("    Motivo: {r['reason']}");
    // === 5. CONSENTIMENTO ===
    printf("\n\n  === 5. SISTEMA DE CONSENTIMENTO ===\n");
    consent = engine.consent_list();
    printf("  Regra: {consent['regra']}");
    printf("  Autorizacao: {consent['autorizacao']}");
    printf("  Revogavel: {consent['revogavel']}");
    printf("  Onde: {consent['onde']}");
    printf("\n  AUTORIZADOS:");
    /* TODO: iterador C manual para a em consent["exemplos_autorizados"] */
        printf("    + {a}");
    printf("\n  PROIBIDOS:");
    /* TODO: iterador C manual para p em consent["exemplos_proibidos"] */
        printf("    X {p}");
    // === 6. PENALIDADE ===
    ...
    /* para cada (k, v) em s.items(): */
        printf("  {k:<30} {v}");
    printf("\n{'='*80}");
    printf("  OpenAntiSpamCall: {s['numeros_bloqueados']} bloqueados. ";
        "{s['principio']}");
    printf("{'='*80}");

#endif // OPENANTISPAMCALL_LIGACAO_TELEFONICA_PREDATORIA_PROIBIDA_H
