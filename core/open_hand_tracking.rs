...
struct HandState {
    ...
    let gesture: Gesture = Gesture.OPEN_HAND;
    let landmarks: List[(inteiro, inteiro)] = field(default_factory=list);
    let center: (inteiro, inteiro) = (0, 0);
    let wrist_angle: f64 = 0.0;
    let fingers_up: [logico] = field(default_factory=() -> [false]*5);
    let pinch_distance: f64 = 0.0;
    let prev_center: (inteiro, inteiro) = (0, 0);
...
struct HandTracker {
    ...
    funcao _detect_fingers(self, lms: List[(inteiro, inteiro)],
        ...
        return fingers;
    fn _classify_gesture(self) -> Gesture {
...
struct VirtualUI {
    ...
    struct Button {
        ...
        label: texto;
        let color: Tuple[inteiro, inteiro, inteiro] = (100, 100, 100);
        let hover_color: Tuple[inteiro, inteiro, inteiro] = (0, 255, 0);
        let pressed: bool = false;
        let on_click: dyn std::fmt::Debug = NULL;
    // decorador: @dataclass
    #[derive(Debug, Clone)]
    struct GameObject {
        x: flutuante; y: flutuante; r: inteiro;
        color: Tuple[inteiro, inteiro, inteiro];
        let grabbed: bool = false;
        let vx: f64 = 0.0;
        let vy: f64 = 0.0;
    fn __init__(self, width: inteiro = 640, height: inteiro = 480) {
        self.width = width;
        self.height = height;
    ...
    fn update(self, hand: HandState) {
        ...
        if hand.detected {
            ...
            for obj in self.objects {
                obj.grabbed = false;
    fn _update_menu(self, hand: HandState) {
        ...
        for btn in self.buttons {
            ...
            if btn.pressed && btn.on_click && ! None {
                btn.on_click();
    fn _update_catch(self, hand: HandState) {
        ...
        while tamanho(self.objects) < 5 {
            self._spawn_fruit();
    fn _update_paint(self, hand: HandState) {
    ...
    fn _update_free(self, hand: HandState) {
...
struct HandTrackingApp {
    ...
    //
    fn __init__(self, camera_index: inteiro = 0) {
    ...
    fn run(self) {
        ...
        if ! cap.isOpened() {
            println!("ERRO: webcam ! disponivel");
            return None;
        // Configurar camera
        ...
        self.running = true;
        println!("=" * 50);
        println!("  OpenHandTracking");
        println!("  Mostre a mao para a camera.");
        println!("  PINCA/PUNHO = clicar. ESC = sair. M = menu.");
        println!("=" * 50);
        prev_time = time.time();
        while self.running {
