...
struct LaborObligation {
    ...
    let max_hours_per_week: f64 = 40.0 // LIMITE: Republica PROIBE mais que isso;
    let excess_threshold: f64 = 50.0 // Acima disto = EXCESSO, ! aceito;
    let weeks_per_year: i64 = 46 // 46 semanas uteis (6 de descanso garantido);
    let rest_days_per_week: i64 = 2 // MINIMO 2 dias sem trabalho obrigatorio;
    let min_vacation_weeks: i64 = 4 // MINIMO 4 semanas de ferias/ano;
    // decorador: @property
    fn base_annual_hours(self) -> f64 {
        // Horas base anuais que cada cidadao deve.
        return self.base_hours_per_week * self.weeks_per_year;
    // decorador: @property
    fn max_annual_hours(self) -> f64 {
        // Maximo de horas/ano que a Republica ACEITA de qualquer pessoa.
        return self.max_hours_per_week * self.weeks_per_year;
    // decorador: @property
    fn excess_annual_hours(self) -> f64 {
...
struct IndividualContribution {
    ...
    citizen_id: texto;
    name: texto;
    let role: String = "cidadao";
    // Tempo
    let hours_base: f64 = 0.0 // horas do contrato base 1.0;
    let hours_voluntary: f64 = 0.0 // horas alem da base (voluntario);
    let hours_total: f64 = 0.0;
    // Artefatos criados
    let systems_created: i64 = 0;
    let systems_maintained: i64 = 0;
    let projects_count: i64 = 0;
    let lines_of_code: i64 = 0;
    let documents_written: i64 = 0;
    // Impacto
    let people_directly_impacted: i64 = 0;
    let people_indirectly_impacted: i64 = 0;
    let ripple_factor: f64 = 1.0 // quanto se espalha no tempo;
    // Reconhecimento
    let community_recognition_score: f64 = 0.0;
    let cycles_active: i64 = 0 // quantos ciclos contribuiu;
    // decorador: @property
    fn base_fulfilled(self) -> bool {
    ...
    // decorador: @property
    fn contribution_ratio(self) -> f64 {
    ...
    // decorador: @property
    fn recognition_level(self) -> String {
...
struct CreatorContract {
    ...
    fn evaluate_individual(self, contrib: IndividualContribution) -> {texto: qualquer} {
    ...
    fn _verdict(self, c: IndividualContribution) -> String {
        if c.excess {
            return (;
        ...
        return "INCOMPLETO.";
    fn _recommendation(self, c: IndividualContribution) -> String {
        if c.excess {
            return (;
    ...
    funcao the_founder_question(self, name: texto, systems: inteiro,
        ...
        return answer;
    fn _direct_answer(self, ratio: flutuante, systems: inteiro) -> String {
        if ratio < 1 {
            return (;
    ...
    fn _creator_bill_of_rights(self) -> [texto] {
...
struct DependencyCheck {
    ...
    fn assess(self, metric: DependencyMetric) -> {texto: qualquer} {
        return {;
            "citizen": metric.name,;
    ...
    fn _action(self, m: DependencyMetric) -> String {
        if m.is_critical {
            return (;
...
if __name__ == "__main__" {
    ...
    dep = DependencyCheck();
    println!("=" * 80);
    println!("  OPENCREATOR -- O CONTRATO INDIVIDUAL-COLETIVO");
    println!('  "Quanto de mim pertence ao coletivo?"');
    println!("=" * 80);
    // === 1. O CONTRATO BASE ===
    println!("\n\n  === 1. O CONTRATO BASE 1.0 ===\n");
    println!("  {contract.obligation.contract}");
    // === 2. PARADOXO DO CRIADOR ===
    println!("\n\n  === 2. O PARADOXO DO CRIADOR ===\n");
    for p in CreatorParadox {
        println!("  [{p.value[0].upper()}]");
        println!("  {p.value[1]}");
        println!();
    // === 3. A PERGUNTA DO FUNDADOR ===
    println!("\n\n  === 3. A PERGUNTA DO FUNDADOR ===\n");
    println!('  "Quanto de mim (Cleiton) tem que se dar pelo coletivo?"');
    println!();
    ...
    println!("    Horas totais dadas:     {c['horas_totais_dadas']:.0f}h");
    println!("    Razao vs contrato base: {c['ratio_vs_base']}");
    println!("    Sistemas criados:       {c['sistemas_criados']}");
    println!("    Linhas escritas:        {c['linhas_escritas']:,}");
    println!("    Pessoas impactadas:     {c['pessoas_impactadas']:,}");
    av = result['avaliacao'];
    println!("\n  RECONHECIMENTO: {av['recognition_level']}");
    println!("  AUTORIDADE:     {av['authority_level']} voto (igual a todos)");
    println!("  EXCESSO:        {'SIM -- Republica deve intervir' if av['excess_detected'] else 'Nao detectado'}");
    ...
    println!("  Cidadao:           {dep_result['citizen']}");
    println!("  Sistemas solo:     {dep_result['systems_solo_knowledge']}");
    println!("  Documentados:      {dep_result['systems_documented']}");
    println!("  Com sucessor:      {dep_result['systems_with_successors']}");
    println!("  Bus factor:        {dep_result['bus_factor']}");
    println!("  Score dependencia: {dep_result['dependency_score']}");
    println!("  Critico:           {'SIM' if dep_result['critical'] else 'NAO'}");
    ...
    println!("  Bus factor:        {ideal_result['bus_factor']}");
    println!("  Score dependencia: {ideal_result['dependency_score']}");
    println!("  Status:            SAUDAVEL");
    ...
    println!("  {'Nome':<28} {'Ratio':>6} {'Reconhecimento':<16} ";
        "{'Poder':>6} {'Cumprido'}");
    println!("  {'-'*75}");
    for ex in examples {
        ev = contract.evaluate_individual(ex);
        println!("  {ex.name:<28} {ev['contribution_ratio']:>6} ";
...
// )
    println!("{'='*80}");
    println!("  OpenCreator: O contrato entre o EU && o NOS.");
    println!("  Base: 920h/ano. Max: 1840h/ano. Limite: 2300h/ano.");
    println!("  Poder: SEMPRE 1 voto. Reconhecimento: proporcional.");
    println!("  Anti-elitismo = ninguem && insubstituivel.");
    println!("{'='*80}");
