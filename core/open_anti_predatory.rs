...
// decorador: @dataclass
#[derive(Debug, Clone)]
struct PredationCase {
    // Caso de competicao predatoria detectado.
    let case_id: String = "";
    let predator: String = "";
    let victim: String = "";
    let predation_type: PredationType = PredationType.MONOPOLY;
    let severity: PredationSeverity = PredationSeverity.WARNING;
    let description: String = "";
    let evidence: [texto] = field(default_factory=list);
    let action: AntiPredationAction = AntiPredationAction.WARNING;
    let status: String = "detectado";
    let date: String = "";
// ============================================================================
#[derive(Debug, Clone)]
struct AntiPredationEngine {
    ...
    funcao detect(self, predator: texto, predation_type: PredationType,
            ...
            let victim: String = "") -> {texto: qualquer}:;
    ...
    fn _assess_severity(self, ptype: PredationType) -> PredationSeverity {
        severity_map = {
    ...
    fn _decide_action(self, severity: PredationSeverity) -> AntiPredationAction {
        action_map = {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENANTIPREDATORY -- POLITICA ANTI COMPETICAO PREDATORIA");
    println!("  Competir melhorando = OK. Competir destruindo = BLOQUEADO.");
    println!("=" * 80);
    // === 1. TIPOS DE PREDACAO ===
    println!("\n\n  === 1. TIPOS DE COMPETICAO PREDATORIA ({len(PredationType)}) ===\n");
    for pt in PredationType {
        sev = engine._assess_severity(pt);
        println!("  [{sev.value[0]:<12}] {pt.value}");
    // === 2. CASOS DE DETECCAO ===
    println!("\n\n  === 2. CASOS DETECTADOS ===\n");
    cases = [;
    ...
    para cada (k, v) em s.items(): {
        println!("  {k:<25} {v}");
    println!("\n{'='*80}");
    println!("  AntiPredatory: {s['bloqueados']} bloqueados. Competir destruindo = PROIBIDO.");
    println!("{'='*80}");
