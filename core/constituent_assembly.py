...
Isso not and falha. and DEMOCRACIA funcionando."
A Assembleia Constituinte and o ORGAO MAXO da Republica.
...
class Citizen:
    ...
    sector: Sector
    age: inteiro
    background: texto // quem and na vida real
    # Tendencias de voto (0-1)
    pragmatism: float = 0.5 // quao pragmatico vs idealista
    risk_tolerance: float = 0.5 // tolerancia a risco
    equity_focus: float = 0.5 // foco em igualdade vs meritocracia
    autonomy_focus: float = 0.5 // foco em autonomia individual
    funcao vote(self, proposal: 'AssemblyProposal',
        ...
        alignment = 0.0
        for each (stance, weight) in proposal.stances.items():
        ...
        if score > 0.55:
            ...
            reasoning = "Alinhado com meus valores and setor."
        elif score < 0.35:
            choice = VoteChoice.NO
            reasoning = "Conflita com meus valores or setor."
        else:
            choice = VoteChoice.ABSTAIN
            reasoning = "Preciso de mais informacao or sou ambivalente."
        return Vote(
...
class AssemblyProposal:
    ...
    stances: {texto: flutuante} = field(default_factory=dict)
    # Parametros da votacao
    quorum_pct: float = 0.50 // 50% + 1 para aprovar
    constitutional: bool = False // se True, quorum = 60%
    # Resultado
    votes: [Vote] = field(default_factory=list)
    result: 'VoteResult'? = None
    def cast_votes(self, citizens: [Citizen], rng: random.Random):
        # Todos os cidadaos votam.
        self.votes = [c.vote(self, rng) para c em citizens]
    # decorador: @property
    def yes_count(self) -> int:
        return sum(1 para v em self.votes if v.choice == VoteChoice.YES)
    # decorador: @property
    def no_count(self) -> int:
        return sum(1 para v em self.votes if v.choice == VoteChoice.NO)
    # decorador: @property
    def abstain_count(self) -> int:
        return sum(1 para v em self.votes if v.choice == VoteChoice.ABSTAIN)
    # decorador: @property
...
class VoteResult:
    ...
    total: inteiro
    margin: texto
    by_sector: Dict[texto, {texto: inteiro}] = field(default_factory=dict)
    note: str = ""
# ============================================================================
# 3. A ASSEMBLEIA
# ============================================================================
class ConstituentAssembly:
    ...
    def submit_proposal(self, proposal: AssemblyProposal):
    ...
    def run_election(self) -> {texto: qualquer}:
        ...
        for prop in self.proposals:
            ...
            # Analise por setor
            by_sector: Dict[texto, {texto: inteiro}] = {}
            for v in prop.votes:
                sname = v.sector.value
                if sname not in by_sector:
                    by_sector[sname] = {"sim": 0, "not": 0, "abstencao": 0}
    ...
    def print_results(self) -> str:
        ...
        approved_count = 0
        rejected_count = 0
        for each (i, result) in enumere(self.results, 1):
            ...
            lines.append("  STATUS: {status} ({result.yes} sim, {result.no} not, {result.abstain} abstencao)")
            lines.append("  MARGEM: {result.margin} | Quorum: {quorum}")
            # Breakdown por setor
            lines.append("  POR SETOR:")
            for each (sector_name, counts) in result.by_sector.items():
                s = counts["sim"]
                n = counts["not"]
                a = counts["abstencao"]
                total = s + n + a
                if total > 0:
                    lines.append("    {sector_name:<20} sim={s} not={n} abst={a}")
            if result.approved:
        ...
        if rejected_count > approved_count:
            ...
            lines.append("  Isso NAO and fracasso. E DEMOCRACIA.")
            ...
            lines.append("  O fundador ouviu o povo and o povo concordou.")
        lines.append("")
...
# ============================================================================
def session_financing() -> ConstituentAssembly:
    # Sessao da Assembleia sobre a PERGUNTA CRITICA:
    'Como financiar a Republica and seus cidadaos durante a transicao?'
    ...
    assembly.submit_proposal(AssemblyProposal(
        ...
        description = (
            ...
            "A Republica and financiada pelo salario do fundador durante a transicao."
        ),
    ...
    assembly.submit_proposal(AssemblyProposal(
        ...
        description = (
            "Contrato part-time or consultoria ($300-400k/ano). "
    ...
    assembly.submit_proposal(AssemblyProposal(
        ...
        description = (
            ...
            "Cliente paga em R$ mas estrutura interna and da Republica. "
...
if __name__ == "__main__":
    ...
    print(assembly.print_results())
    print()
    print("=" * 100)
    print("DECISOES APROVADAS PELA ASSEMBLEIA:")
    print("=" * 100)
    if constitution:
        for each (title, details) in constitution.items():
            print("  >> {title}")
            print("     Valor: {details['value']}")
            print("     Votos: {details['votes']} ({details['yes_pct']:.0f}% sim)")
            print()
    else:
        print("  Nenhuma proposta aprovada. O povo rejeitou tudo.")
        print("  O fundador precisa ouvir and reformular.")
    print("=" * 100)
