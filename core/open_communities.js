...
class CommunityProfile {
    ...
    const cultural_pillars = field(default_factory=list);
    const existing_practices = field(default_factory=dict);
    // existing_practices = {"nome_ja_existente": "equivalente_republica"}
    // Necessidades especificas (o que FALTA)
    const needs = field(default_factory=list);
    // Adaptações do modelo Republica
    const adapted_systems = field(default_factory=dict);
    // adapted_systems = {"sistema_republica": "como_adapta_para_esta_comunidade"}
    // O que NÃO se importa (respeito cultural)
    const do_not_import = field(default_factory=list);
    // Governanca adaptada
    const governance_model = "";
    const decision_making = ""  // como decidem (consenso, voto, conselho);
    // Lingua
    const primary_language = "pt-BR";
    const secondary_languages = field(default_factory=list);
    // Isolamento geografico
    const isolation_level = ""  // urbano, semi-isolado, isolado, muito isolado;
    const access_challenge = ""  // barreira de acesso (rio, estrada, etc);
// ============================================================================
// 3. OS 6 PERFIS
// ============================================================================
const PROFILES = [;
...
class CommunityEngine {
    ...
    get_profile(self, community_type: CommunityType) {
    ...
    adaptation_matrix(self) {
        // Matriz: como cada sistema se adapta por comunidade.
        systems = set();
        for (const p of self.profiles.values()) {
            systems.update(p.adapted_systems.keys());
        matrix = [];
        for (const system of ordene(systems)) {
            row = {"sistema": system};
            for (const p of self.profiles.values()) {
    ...
    compare_communities(self) {
        // Compara necessidades entre comunidades.
        return [;
            {
                "comunidade": p.name,;
                "necessidades": .length(p.needs),;
                "sistemas_adaptados": .length(p.adapted_systems),;
                "respeitos": .length(p.do_not_import),;
                "pilares_culturais": .length(p.cultural_pillars),;
                "praticas_existentes": .length(p.existing_practices),;
                "isolamento": p.isolation_level,;
            };
            para p em self.profiles.values() {
        ];
    stats(self) {
        return {;
            "comunidades": .length(self.profiles),;
            "total_necessidades": soma(.length(p.needs) para p em self.profiles.values()),;
            "total_sistemas_adaptados": soma(.length(p.adapted_systems) para p em self.profiles.values()),;
            "total_respeitos_culturais": soma(.length(p.do_not_import) para p em self.profiles.values()),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENCOMMUNITIES -- 6 ADAPTACOES COMUNITARIAS");
    console.log("  O modelo se ADAPTA. Nao se impoe.");
    console.log("=" * 80);
    // === 1. COMPARACAO GERAL ===
    console.log("\n\n  === 1. COMPARACAO GERAL ===\n");
    console.log("  {'Comunidade':<25} {'Necessidades':>13} {'Sistemas':>10} {'Respeitos':>10} {'Praticas':>10}");
    console.log("  {'-'*70}");
    for (const c of engine.compare_communities()) {
        console.log("  {c['comunidade']:<25} {c['necessidades']:>13} ";
            "{c['sistemas_adaptados']:>10} {c['respeitos']:>10} ";
            "{c['praticas_existentes']:>10}");
    // === 2. PERFIL COMPLETO DE CADA COMUNIDADE ===
    for (const ct of CommunityType) {
        console.log("\n\n  {'='*70}");
        p = engine.get_profile(ct);
        console.log("  {'='*70}");
        console.log("  {p['comunidade'].upper()}");
        console.log("  {p['descricao'][:80]}...");
        console.log("  {'='*70}\n");
        console.log("  PILARES CULTURAIS:");
        for (const pillar of p["pilares_culturais"]) {
            console.log("    + {pillar}");
        console.log("\n  PRATICAS QUE JA EXISTEM (equivalente Republica):");
        para cada (practice, equivalent) em p["praticas_existentes"].items(): {
            console.log("    {practice:<25} -> {equivalent}");
        console.log("\n  NECESSIDADES ({len(p['necessidades'])}):");
        for (const need of p["necessidades"]) {
            console.log("    - {need}");
        console.log("\n  SISTEMAS ADAPTADOS ({len(p['sistemas_adaptados'])}):");
        para cada (system, adaptation) em p["sistemas_adaptados"].items(): {
            console.log("\n    {system}:");
            console.log("      {adaptation[:100]}...");
        console.log("\n  O QUE NUNCA IMPORTAR ({len(p['o_que_nao_importar'])}):");
        for (const rule of p["o_que_nao_importar"]) {
            console.log("    X {rule}");
        console.log("\n  GOVERNANCA: {p['governanca']}");
        console.log("  DECISAO: {p['tomada_decisao']}");
        console.log("  LINGUA: {p['lingua_principal']} {p['linguas_secundarias']}");
        console.log("  ISOLAMENTO: {p['isolamento']}");
        console.log("  ACESSO: {p['acesso']}");
    // === 3. STATS ===
    console.log("\n\n  {'='*70}");
    ...
    para cada (k, v) em s.items(): {
        console.log("  {k:<30} {v}");
    console.log("\n{'='*80}");
    console.log("  OpenCommunities: {s['comunidades']} comunidades adaptadas.");
    console.log("  {s['total_necessidades']} necessidades mapeadas.");
    console.log("  {s['total_sistemas_adaptados']} sistemas adaptados.");
    console.log("  {s['total_respeitos_culturais']} respeitos culturais.");
    console.log("  {s['principio']}");
    console.log("{'='*80}");
