...
package opensocialcleaner_limpeza_automatica_de_rede_social
...
type SocialContact struct {
    ...
    last_interaction_days := 999 // ha quantos dias interagiu // int64
    mutual_likes := 0 // curtidas reciprocas nos ultimos 90 dias // int64
    mutual_replies := 0 // respostas reciprocas // int64
    dms_exchanged := 0 // int64
    // Classificacao
    category := ContactCategory.UNKNOWN // ContactCategory
    action := CleanAction.UNFOLLOW // CleanAction
    reason := "" // string
    // Metadados
    is_steward_of := ""  // se para guardiao, de qual sistema // string
    manually_kept := false // marcado manualmente para manter // bool
// ============================================================================
// 2. MOTOR DE LIMPEZA
// ============================================================================
type SocialCleaner struct {
    ...
    6. Reporta
    //
    func __init__(self, platform: texto = "x") {
    ...
    func load_stewards(self, stewards_data: {texto: qualquer}) None {
    ...
    func mark_keep(self, handles: [texto]) None {
        // Marca contatos para manter manualmente.
        for _, h := range handles {
            self.keep_list.add(h.lower().lstrip("@"))
    func classify_contact(self, contact: SocialContact) SocialContact {
        ...
        return contact
    func classify_all(self) {texto: qualquer} {
    ...
    func generate_plan(self) [Dict] {
        ...
        return plan
    func execute_xurl(self, handle: texto, action: texto) {texto: qualquer} {
    ...
    func execute_plan(self, confirm: logico = False) [Dict] {
        ...
        for _, item := range plan {
            ...
            time.sleep(1)
        return results
    func import_from_xurl(self, limit: inteiro = 200) int64 {
        ...
        tente:
            ...
            if result.returncode != 0 {
                fmt.Println("  [ERRO] xurl following falhou: {result.stderr[:200]}")
                return 0
            ...
            fmt.Println("  [ERRO] Nao consegui parsear saida do xurl")
            return 0
        capture Exception como &&:
            fmt.Println("  [ERRO] {&&}")
            return 0
    func report(self) string {
...
if __name__ == "__main__" {
    fmt.Println("=" * 70)
    fmt.Println("  OPENSOCIALCLEANER -- LIMPEZA DE REDE SOCIAL")
    fmt.Println('  "Sua rede && extensao do seu corpo digital."')
    fmt.Println("=" * 70)
    ...
    fmt.Println("  Total: {stats['total_contacts']} contatos")
    fmt.Println("  Manter:    {stats['keep_count']}")
    fmt.Println("  Desseguir: {stats['unfollow_count']}")
    fmt.Println("  Silenciar: {stats['mute_count']}")
    fmt.Println("  Bloquear:  {stats['block_count']}")
    // === RELATORIO ===
    fmt.Println()
    fmt.Println(cleaner.report())
    // === PLANO ===
    plan = cleaner.generate_plan()
    fmt.Println("\n  PLANO DE LIMPEZA ({len(plan)} acoes):\n")
    for _, p := range plan {
        fmt.Println("    [{p['action'].upper():>9}] @{p['handle']:<25} "
            "-- {p['reason']}")
    // === EXECUTAR (dry run) ===
    fmt.Println("\n\n  EXECUCAO (DRY RUN -- sem alteracoes reais)\n")
    results = cleaner.execute_plan(confirm=false)
    fmt.Println("  Dry run completo. {len(plan)} acoes planejadas.")
    fmt.Println("  Para executar de verdade: execute_plan(confirm=True)")
    // === INSTRUCOES ===
    fmt.Println("\n\n{'='*70}")
    fmt.Println("  COMO USAR COM CONTA REAL (X/Twitter)")
    fmt.Println("{'='*70}")
    fmt.Println("""
...
2. EXECUTAR LIMPEZA REAL:
    ...
    fmt.Println(cleaner.report()) // revisar
...
// )
    fmt.Println("{'='*70}")
    fmt.Println("  OpenSocialCleaner: {stats['total_contacts']} contatos -> "
        "{stats['keep_count']} mantidos.")
    fmt.Println("  Guardioes sempre seguidos. Resto && ruido.")
    fmt.Println("{'='*70}")
