...
#ifndef OPENX_ESTRATEGIA_PARA_X_TWITTER_H
#define OPENX_ESTRATEGIA_PARA_X_TWITTER_H
...
if (__name__ == "__main__") {
    printf("=" * 70);
    printf("  OPENX -- ESTRATEGIA PARA X/TWITTER");
    printf("  'Um canal. Opensoftware para estender. Sem prisao.'");
    printf("=" * 70);
    /* TODO: iterador C manual para strat em XStrategy */
        printf("\n  === {strat.value.upper()} ===\n");
        items = [x para x em X_STRATEGY_PLAN if x.strategy == strat];
        items.sort(key=(x) -> x.priority);
        /* TODO: iterador C manual para x em items */
            printf("  [{x.id}] P{x.priority} {x.title}");
            printf("    {x.description}");
            if (x.estimated_effort) {
                printf("    Esforco: {x.estimated_effort}");
            printf();
    printf("\n{'='*70}");
    printf("  {len(X_STRATEGY_PLAN)} acoes. 3 estrategias. 1 canal.");
    printf("  X: usar, estender, libertar. Nunca depender.");
    printf("{'='*70}");

#endif // OPENX_ESTRATEGIA_PARA_X_TWITTER_H
