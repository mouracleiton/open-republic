...
#ifndef OPENFAIRSURPLUSASSEMBLY_ASSEMBLEIA_VOTA_5_E_O_SUFICIENTE_H
#define OPENFAIRSURPLUSASSEMBLY_ASSEMBLEIA_VOTA_5_E_O_SUFICIENTE_H
...
{texto: qualquer} run_assembly(n_voters: inteiro = 10000) {
...
if (__name__ == "__main__") {
    printf("=" * 80);
    printf("  ASSEMBLEIA CONSTITUINTE -- VOTACAO: EXCEDENTE DE 5%");
    printf("=" * 80);
    result = run_assembly(10000);
    printf("\n  PERGUNTA:\n    {result['question']}\n");
    printf("  VOTACAO ({result['total']:,} cidadaos):\n");
    /* para option, count in ordene(result["votes"].items(), */
                                key = (x) -> -x[1]):;
        pct = count / result["total"] * 100;
        bar = "#" * inteiro(pct / 2);
        winner_mark = option == result["winner"] ? " <--- VENCEDOR" : "";
        printf("    {option:<25} {count:>6} ({pct:.0f}%) {bar}{winner_mark}");
    printf("\n  RESULTADO: {result['decision']}");
    printf("  Vencedor: {result['winner']} ({result['winner_percent']})");
    printf("  Runner-up: {result['runner_up']} ({result['runner_up_votes']/result['total']*100:.0f}%)");
    printf("\n  LEI RATIFICADA:");
    /* para cada (key, val) em result["law"].items(): */
        if (isinstance(val, flutuante)) {
            ...
            printf("    {key:<30} {val}");
    printf("\n{'='*80}");
    printf("  ASSEMBLEIA DECIDIU: 5% de excedente. LEI.");
    printf("  62% dos cidadaos concordaram.");
    printf("  Acima de 5% = PREDATORIO = BLOQUEADO.");
    printf("  O 5% volta pra quem trabalha. Sempre.");
    printf("{'='*80}");

#endif // OPENFAIRSURPLUSASSEMBLY_ASSEMBLEIA_VOTA_5_E_O_SUFICIENTE_H
