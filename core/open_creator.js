...
class LaborObligation {
    ...
    const max_hours_per_week = 40.0 // LIMITE: Republica PROIBE mais que isso;
    const excess_threshold = 50.0 // Acima disto = EXCESSO, ! aceito;
    const weeks_per_year = 46 // 46 semanas uteis (6 de descanso garantido);
    const rest_days_per_week = 2 // MINIMO 2 dias sem trabalho obrigatorio;
    const min_vacation_weeks = 4 // MINIMO 4 semanas de ferias/ano;
    // decorador: @property
    base_annual_hours(self) {
        // Horas base anuais que cada cidadao deve.
        return self.base_hours_per_week * self.weeks_per_year;
    // decorador: @property
    max_annual_hours(self) {
        // Maximo de horas/ano que a Republica ACEITA de qualquer pessoa.
        return self.max_hours_per_week * self.weeks_per_year;
    // decorador: @property
    excess_annual_hours(self) {
...
class IndividualContribution {
    ...
    const role = "cidadao";
    // Tempo
    const hours_base = 0.0 // horas do contrato base 1.0;
    const hours_voluntary = 0.0 // horas alem da base (voluntario);
    const hours_total = 0.0;
    // Artefatos criados
    const systems_created = 0;
    const systems_maintained = 0;
    ...
    const ripple_factor = 1.0 // quanto se espalha no tempo;
    // Reconhecimento
    const community_recognition_score = 0.0;
    const cycles_active = 0 // quantos ciclos contribuiu;
    // decorador: @property
    base_fulfilled(self) {
...
class CreatorContract {
    ...
    evaluate_individual(self, contrib: IndividualContribution) {
    ...
    _verdict(self, c: IndividualContribution) {
        if (c.excess) {
            return (;
        ...
        return "INCOMPLETO.";
    _recommendation(self, c: IndividualContribution) {
        if (c.excess) {
            return (;
    ...
    funcao the_founder_question(self, name: texto, systems: inteiro,
        ...
        return answer;
    _direct_answer(self, ratio: flutuante, systems: inteiro) {
        if (ratio < 1) {
            return (;
...
class DependencyCheck {
    ...
    assess(self, metric: DependencyMetric) {
        return {;
            "citizen": metric.name,;
    ...
    _action(self, m: DependencyMetric) {
        if (m.is_critical) {
            return (;
...
if (__name__ == "__main__") {
    ...
    dep = DependencyCheck();
    console.log("=" * 80);
    console.log("  OPENCREATOR -- O CONTRATO INDIVIDUAL-COLETIVO");
    console.log('  "Quanto de mim pertence ao coletivo?"');
    console.log("=" * 80);
    // === 1. O CONTRATO BASE ===
    console.log("\n\n  === 1. O CONTRATO BASE 1.0 ===\n");
    console.log("  {contract.obligation.contract}");
    // === 2. PARADOXO DO CRIADOR ===
    console.log("\n\n  === 2. O PARADOXO DO CRIADOR ===\n");
    for (const p of CreatorParadox) {
        console.log("  [{p.value[0].upper()}]");
        console.log("  {p.value[1]}");
        console.log();
    // === 3. A PERGUNTA DO FUNDADOR ===
    console.log("\n\n  === 3. A PERGUNTA DO FUNDADOR ===\n");
    console.log('  "Quanto de mim (Cleiton) tem que se dar pelo coletivo?"');
    console.log();
    ...
    console.log("    Horas totais dadas:     {c['horas_totais_dadas']:.0f}h");
    console.log("    Razao vs contrato base: {c['ratio_vs_base']}");
    console.log("    Sistemas criados:       {c['sistemas_criados']}");
    console.log("    Linhas escritas:        {c['linhas_escritas']:,}");
    console.log("    Pessoas impactadas:     {c['pessoas_impactadas']:,}");
    av = result['avaliacao'];
    console.log("\n  RECONHECIMENTO: {av['recognition_level']}");
    console.log("  AUTORIDADE:     {av['authority_level']} voto (igual a todos)");
    console.log("  EXCESSO:        {'SIM -- Republica deve intervir' if av['excess_detected'] else 'Nao detectado'}");
    ...
    for (const right of result['declaracao_de_direitos_do_criador']) {
        console.log("  {right}");
    ...
    console.log("  Cidadao:           {dep_result['citizen']}");
    console.log("  Sistemas solo:     {dep_result['systems_solo_knowledge']}");
    console.log("  Documentados:      {dep_result['systems_documented']}");
    console.log("  Com sucessor:      {dep_result['systems_with_successors']}");
    console.log("  Bus factor:        {dep_result['bus_factor']}");
    console.log("  Score dependencia: {dep_result['dependency_score']}");
    console.log("  Critico:           {'SIM' if dep_result['critical'] else 'NAO'}");
    ...
    console.log("  Bus factor:        {ideal_result['bus_factor']}");
    console.log("  Score dependencia: {ideal_result['dependency_score']}");
    console.log("  Status:            SAUDAVEL");
    ...
    console.log("  {'Nome':<28} {'Ratio':>6} {'Reconhecimento':<16} ";
        "{'Poder':>6} {'Cumprido'}");
    console.log("  {'-'*75}");
    for (const ex of examples) {
        ev = contract.evaluate_individual(ex);
        console.log("  {ex.name:<28} {ev['contribution_ratio']:>6} ";
...
// )
    console.log("{'='*80}");
    console.log("  OpenCreator: O contrato entre o EU && o NOS.");
    console.log("  Base: 920h/ano. Max: 1840h/ano. Limite: 2300h/ano.");
    console.log("  Poder: SEMPRE 1 voto. Reconhecimento: proporcional.");
    console.log("  Anti-elitismo = ninguem && insubstituivel.");
    console.log("{'='*80}");
