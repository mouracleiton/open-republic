...
class Deviation {
    ...
    const correction = "";
    const correction_speed = CorrectionSpeed.INSTANT;
...
class FounderCorrectionSystem {
    ...
    funcao check_proposal(self, proposal_text: texto,
                    const proposal_type = "politica") -> {texto: qualquer}:;
    ...
    _severity(self, dev_type: DeviationType) {
        return {;
    ...
    _principle(self, dev_type: DeviationType) {
        return {;
    ...
    _correction(self, dev_type: DeviationType) {
        return {;
            DeviationType.ELITISM: (;
    ...
    appeal(self, deviation_id: texto) {
    ...
    stats(self) {
        return {;
            "total_checked": .length(self.deviations),;
...
run_founder_role_assembly(n_voters: inteiro = 10000) {
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENFOUNDERROLE -- PAPEL CONSTITUCIONAL DO FUNDADOR");
    console.log("  Assembleia Constituinte Especial");
    console.log("=" * 80);
    // === 1. PERGUNTA ===
    console.log("\n\n  === PERGUNTA SUBMETIDA A ASSEMBLEIA ===\n");
    console.log('  "A posicao do fundador pode ser definitiva && sempre');
    console.log('   auxiliada pelo sistema, com correcao automatica de desvios?"');
    console.log("\n  POSICAO DO FUNDADOR:");
    console.log('  "Sim, quero voz permanente + auxilio + correcao quando errar."');
    // === 2. VOTACAO ===
    console.log("\n\n  === VOTACAO (10.000 CIDADAOs) ===\n");
    result = run_founder_role_assembly(10000);
    console.log("  SIM (voz + correcao):    {result['votes_for']:>6} ({result['votes_for']/100:.0f}%)");
    console.log("  PARCIAL (voz, sem prio): {result['votes_partial']:>6} ({result['votes_partial']/100:.0f}%)");
    console.log("  NAO (cidadao comum):     {result['votes_against']:>6} ({result['votes_against']/100:.0f}%)");
    ...
    para cada (key, val) em d.items(): {
        ...
        console.log("  {key:<25} {status}");
    console.log("\n  RESTRICOES INALTERAVEIS:");
    for (const r of result["restrictions_inalteraveis"]) {
        console.log("    {r}");
    ...
    para cada (proposal, expected) em test_proposals: {
        ...
        if (check["status"] != "approved") {
            console.log("       Tipo: {check.get('type', '?')}");
            console.log("       Principio: {check.get('principle', '?')}");
            console.log("       Correcao: {check['correction'][:80]}");
        } else {
            console.log("       Conforme. Pode prosseguir para votacao.");
    // === 5. APELO ===
    console.log("\n\n  === DIREITO DE APELO ===\n");
    for (const dev of corrector.deviations) {
        if (! dev.resolved) {
            appeal = corrector.appeal(dev.deviation_id);
            console.log("  Fundador apela correcao: {dev.deviation_type.value}");
            console.log("  Resultado: {appeal['appeal_result']}");
            console.log("  {appeal['votes']}");
    ...
    // === RESPOSTA FINAL ===
    console.log("\n\n{'='*80}");
    console.log("  RESPOSTA A PERGUNTA DO FUNDADOR");
    console.log("{'='*80}");
    console.log(""";
PERGUNTA:;
...
// )
    console.log("{'='*80}");
    console.log("  OpenFounderRole: voz permanente + auxilio + correcao automatica.");
    console.log("  Fundador propoe. Sistema corrige. Povo decide.");
    console.log("  {'='*80}");
