...
class StatusDisponibilidade(Enum):
...
class FluxoEnergetico:
    ...
    inputs: List[TipoInput] = field(default_factory=list)
    conversao: str = ""        # como se converte input em output
    output: str = ""           # o que o sistema PRODUZ
    residuos: List[TipoResiduo] = field(default_factory=list)
    eficiencia_pct: float = 100.0  # % do input que vira output util
...
class SistemaEnergetico:
    ...
    fluxo: FluxoEnergetico = field(default_factory=FluxoEnergetico)
    consumo_corpo_pct: float = 0.0  # % do orcamanto energetico do corpo (se aplicavel)
    observacao: str = ""
...
class CoberturaRepublica:
    ...
    modulos: List[str] = field(default_factory=list)    # modulos que cobrem
    lacunas: List[str] = field(default_factory=list)    # o que falta
...
class DiagnosticoEnergetico:
    ...
    status_por_tipo: Dict[str, StatusDisponibilidade] = field(default_factory=dict)
    tipos_em_deficit: List[TipoEnergia] = field(default_factory=list)
    tipos_abundantes: List[TipoEnergia] = field(default_factory=list)
    veredito: str = ""
...
def _init_sistemas() -> List[SistemaEnergetico]:
...
def _init_coberturas() -> List[CoberturaRepublica]:
...
class EnergyTaxonomyEngine:
    ...
    def listar_sistemas(self) -> List[SistemaEnergetico]:
        return sorted(self.sistemas, key=lambda s: s.tipo.numero)

    def sistema_por_tipo(self, tipo: TipoEnergia) -> Optional[SistemaEnergetico]:
        for s in self.sistemas:
            if s.tipo == tipo:
                return s
        return None

    def coberturas_por_nivel(self, nivel: NivelCobertura) -> List[CoberturaRepublica]:
        return [c for c in self.coberturas if c.nivel == nivel]

    def lacunas_identificadas(self) -> List[Tuple[TipoEnergia, List[str]]]:
...
def _demo() -> None:
    ...
    for tid, st in sorted(diag_maria.status_por_tipo.items()):
        print(f"    {tid:.<20} {st}")
