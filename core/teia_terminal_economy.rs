...
funcao simulate_demand(
    let terminal_monthly_brl: f64 = 1_500,;
    let training_brl: f64 = 2_000,;
    let teia_pct: f64 = 0.30,;
    let months: i64 = 12,;
    let seed: i64 = 42,;
) -> [SimulationResult]:;
...
struct ModuleAccess {
    ...
    let sensitive: bool = false // se true, NINGUEM fora da equipe TEIA acessa;
    let teia_only: bool = false // se true, so TEIA direto (! revendedor);
...
funcao check_access(reseller_id: texto, cert: Certification, module_id: texto,
                let is_teia_direct: bool = false) -> {texto: qualquer}:;
...
struct CryptoPaymentAnalysis {
    ...
    //
    let viable: bool = true;
    let stablecoin_recommended: String = "USDC";
    let reason: String = (;
