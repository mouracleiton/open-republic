...
**Arquivo original:** `open-republic/core/open_drone.py`
...
classe TipoMissao herda de Enum:
...
classe StatusMissao herda de Enum:
...
classe TipoProibicao herda de Enum:
...
classe VereditoGate herda de Enum:
...
classe PrioridadeCorredor herda de Enum:
...
classe ZonaVoo:
    ...
    declare sobrevoa_privado: bool  <- FALSO
    declare consentimento_privado: bool  <- FALSO
...
classe Drone:
    ...
    carga_max_kg: float
    declare tem_camera_navegacao: bool  <- VERDADEIRO  // permitida (feed local, nao gravado)
    declare tem_camera_vigilancia: bool  <- FALSO  // PROIBIDA
    declare tem_armamento: bool  <- FALSO  // PROIBIDO
    declare coleta_dados_pessoais: bool  <- FALSO  // PROIBIDO
    declare ativo: bool  <- VERDADEIRO
    declare missoes_concluidas: int  <- 0
...
classe MissaoDrone:
    ...
    descricao: str
    zona: ZonaVoo
    declare destino: Optional[Coordenada]  <- nulo
    declare carga_descricao: str  <- ""
    declare urgencia: bool  <- FALSO
    declare status: StatusMissao  <- StatusMissao.PLANEJADA
    declare veredito_gate: Optional[VereditoGate]  <- nulo
    declare razao_rejeicao: str  <- ""
    declare proibicoes_violadas: List[TipoProibicao]  <- field(default_factory=list)
    declare criada_em: str  <- ""
    declare concluida_em: str  <- ""
    declare log_trajeto: List[Coordenada]  <- field(default_factory=list)
...
classe LogVoo:
    ...
    pouso: str
    declare destino_lat: Optional[float]  <- nulo
    declare destino_lon: Optional[float]  <- nulo
    declare sucesso: bool  <- VERDADEIRO
    declare observacoes: str  <- ""
...
declare DESCRICOES_PROIBICOES: Dict[str, str]  <- {
    "vigilancia": (
...
declare PRIORIDADE_POR_TIPO: Dict[str, int]  <- {
...
classe DroneCivicoEngine:
    ...
    funcao _drone_id_novo(self) retorna str:
        self._drone_id += 1
        retorne f"DRONE-{self._drone_id:04d}"

    funcao _missao_id_novo(self) retorna str:
        self._missao_id += 1
        retorne f"MISSAO-{self._missao_id:04d}"

    funcao _zona_id_novo(self) retorna str:
        self._zona_id += 1
        retorne f"ZONA-{self._zona_id:04d}"
    ...
    def registrar_drone(
        ...
        autonomia_minutos: int,
        carga_max_kg: float,
        declare tem_camera_navegacao: bool  <- VERDADEIRO,
        declare tem_camera_vigilancia: bool  <- FALSO,
        declare tem_armamento: bool  <- FALSO,
        declare coleta_dados_pessoais: bool  <- FALSO,
    ) -> Drone:
        d <- Drone(
        ...
        se tem_camera_vigilancia  OU  tem_armamento  OU  coleta_dados_pessoais entao:
            d.ativo = FALSO
        self.drones[d.id] = d
        retorne d
    ...
    funcao _verificar_uso_comercial(self, missao: MissaoDrone) retorna bool:
        ...
        para cada p em palavras_nao_civicas:
            se p in texto entao:
                retorne VERDADEIRO
        retorne FALSO

    funcao aprovar_missao(self, missao_id: str) retorna Tuple[VereditoGate, str]:
        // Executa o gate P10 e atualiza o status da missao.
        missao <- self.missoes.get(missao_id)
        se missao e nulo entao:
            retorne VereditoGate.REJEITADA, "Missao nao encontrada"
        ...
        se gravidade_max >= 5 entao:
            ...
            retorne missao.veredito_gate, missao.razao_rejeicao
        ...
        se drone entao:
            ...
            se autonomia_necessaria > drone.autonomia_minutos entao:
        ...
        missao.status = StatusMissao.APROVADA
        retorne missao.veredito_gate, "Missao aprovada pelo gate P10"

    funcao _estimar_distancia(self, missao: MissaoDrone) retorna float:
        // Estima distancia de voo em km (zona raio x 2 ida+volta).
        retorne (missao.zona.raio_metros / 1000.0) * 2.0

    // -- execucao de missao ------------------------------------------------

    funcao decolar(self, missao_id: str) retorna bool:
        // Coloca uma missao aprovada em voo.
        missao <- self.missoes.get(missao_id)
        se missao e nulo  OU  missao.status != StatusMissao.APROVADA entao:
            retorne FALSO
    ...
    def concluir_missao(
        ...
        log <- LogVoo(
            ...
            destino_lat <- missao.destino.lat if missao.destino else nulo,
            destino_lon <- missao.destino.lon if missao.destino else nulo,
            sucesso <- sucesso,
    ...
    def resolver_conflito_corredor(
        ...
        se ma e nulo  OU  mb e nulo entao:
            retorne nulo
        pri_a <- PRIORIDADE_POR_TIPO.get(ma.tipo.id, 4)
        pri_b <- PRIORIDADE_POR_TIPO.get(mb.tipo.id, 4)
        // Urgencia (resgate de vida) sobrescreve
        se ma.urgencia  E  NAO  mb.urgencia entao:
            retorne ma.id
        se mb.urgencia  E  NAO  ma.urgencia entao:
            retorne mb.id
        se pri_a < pri_b entao:
            retorne ma.id
        se pri_b < pri_a entao:
            retorne mb.id
        retorne nulo  // empate: negociar na assembleia

    // -- metricas ----------------------------------------------------------

    funcao medir_frota(self, regiao_id: str = "default") retorna MetricaFrota:
        ...
        entregas <- sum(
            ...
            if m.status == StatusMissao.CONCLUIDA  E  m.tipo == TipoMissao.ENTREGA_SUPRIMENTOS
        )
        resgates <- sum(
            1 for m in self.missoes.values()
            if m.status == StatusMissao.CONCLUIDA  E  m.tipo == TipoMissao.BUSCA_RESGATE
        )
...
funcao _demo() retorna None:
    ...
    d_vigia <- e.registrar_drone(
        ...
        carga_max_kg <- 3.0,
        tem_camera_vigilancia <- VERDADEIRO,
    )
    ...
    z_privada <- e.registrar_zona(
        ...
        sobrevoa_privado <- VERDADEIRO,
        consentimento_privado <- FALSO,
    )
    ...
    desempacote v1, r1 <- e.aprovar_missao(m1.id)
    ...
    desempacote v2, r2 <- e.aprovar_missao(m2.id)
    ...
    desempacote v3, r3 <- e.aprovar_missao(m3.id)
    ...
    desempacote v4, r4 <- e.aprovar_missao(m4.id)
    ...
    desempacote v5, r5 <- e.aprovar_missao(m5.id)
