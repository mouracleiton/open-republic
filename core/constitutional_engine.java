...
public class OpenrepublicSistemaConstitucionalRepresentantesPorSistema {
    ...
    // decorador: @dataclass
    public static class ConstitutionalViolation {
    ...
    // ============================================================================
    public static class ConstitutionalEngine {
        ...
        funcao register_system(self, system_id: texto, name: texto,
                            ...
                            String created_by = "comunidade",;
                            boolean approved_by_vote = false,;
                            int vote_count = 0,;
                            boolean affects_bodies = false,;
                            boolean collects_data = false,;
                            boolean opaque_decisions = false,;
                            boolean single_point_of_failure = false,;
                            boolean requires_bodily_consent = false,;
                            boolean labor_base_respected = true,;
                            boolean has_successor = false,;
                            boolean public_docs = false,;
        ...
        public {texto: qualquer} validate_system(self, system_id: texto) {
            ...
            return result;
        public {texto: qualquer} validate_all(self) {
            // Valida TODOS os sistemas registrados.
            results = {};
            /* TODO: for-each Java para sid em self.systems_registry */
        ...
        public ComplianceCheck _check_anti_elitism(self, sys_data: Dict) {
        ...
        public ComplianceCheck _check_bodily_autonomy(self, sys_data: Dict) {
        ...
        public ComplianceCheck _check_equal_labor(self, sys_data: Dict) {
        ...
        public ComplianceCheck _check_democratic_process(self, sys_data: Dict) {
        ...
        public String _status_from_severity(severity: ViolationSeverity) {
    ...
    // ============================================================================
    public static class StewardRole {
    ...
    // decorador: @dataclass
    public static class SystemSteward {
        ...
        funcao receive_demand(self, source: texto, demand: texto,
            ...
            return entry;
        public {texto: qualquer} execute_demand(self, demand_id: texto) {
            // Marca demanda como executada.
            /* TODO: for-each Java para d em self.pending_demands */
        ...
        public {texto: qualquer} report_status(self) {
    ...
    public static class StewardAssignment {
        ...
        public void _bootstrap_systems(self) {
        ...
        funcao founder_batch_demands(self, demands: List[(texto, texto)]
            ...
            /* para cada (sys_id, demand_text) em demands: */
                ...
                results.append(r);
            return results;
        public {texto: qualquer} system_status_report(self) {
        ...
        public [Dict] list_systems_for_demand(self) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENREPUBLIC -- MOTOR CONSTITUCIONAL + GUARDIOES");
        System.out.println('  "Os principios ! sao adornos. Sao LEIS."');
        System.out.println("=" * 80);
        mgr = StewardAssignment();
        // === 1. OS 4 PRINCIPIOS CODIFICADOS ===
        System.out.println("\n\n  === 1. PRINCIPIOS CONSTITUCIONAIS CODIFICADOS ===\n");
        /* TODO: for-each Java para p em ConstitutionalPrinciple */
            System.out.println("  P{p.value[0]}: {p.value[1].upper()}");
            System.out.println("  {p.value[2]}");
            System.out.println();
        // === 2. REGISTRO DE SISTEMAS ===
        System.out.println("\n  === 2. SISTEMAS REGISTRADOS: ";
            "{len(mgr.engine.systems_registry)} ===\n");
        System.out.println("  {'ID':<12} {'Sistema':<24} {'Dominio':<16} {'Guardiao'}");
        ...
        /* TODO: for-each Java para s em mgr.list_systems_for_demand() */
            System.out.println("  {s['system_id']:<12} {s['name']:<24} ";
                "{s['domain']:<16} {s['steward']}");
        // === 3. VALIDACAO CONSTITUCIONAL ===
        System.out.println("\n\n  === 3. AUDITORIA CONSTITUCIONAL COMPLETA ===\n");
        audit = mgr.engine.validate_all();
        System.out.println("  Total de sistemas:     {audit['total_systems']}");
        System.out.println("  Conformes:             {audit['fully_compliant']}");
        System.out.println("  Suspensos:             {audit['suspended']}");
        System.out.println("  Banidos:               {audit['banned']}");
        System.out.println("  Taxa de conformidade:  {audit['compliance_rate']}");
        System.out.println("  Precisam revisao:      {audit['needs_review']}");
        ...
        /* para cada (sid, demand) em demands: */
            ...
            if ("error" in r) {
                ...
                System.out.println("  [{r['system']}] -> {r['steward']}");
                System.out.println("    Demanda: {demand}");
                System.out.println("    Status constitucional: {r['constitutional_status']}");
                System.out.println("    Alertas: {r['constitutional_alerts']}");
                System.out.println();
        // === 5. SISTEMAS SEM GUARDIAO ===
        report = mgr.system_status_report();
        System.out.println("\n  === 5. SISTEMAS SEM GUARDIAO ===\n");
        System.out.println("  Designados: {report['stewards_assigned']}/{report['total_systems']}");
        System.out.println("  Sem guardiao: {report['unassigned_systems']}");
        if (report['unassigned_list']) {
            System.out.println("  IDs: {', '.join(report['unassigned_list'][:10])}...");
        // === 6. DETALHE: AUDITORIA DE UM SISTEMA ===
        System.out.println("\n\n  === 6. AUDITORIA DETALHADA: OpenHealth ===\n");
        detail = mgr.engine.validate_system("R-HEA-01");
        System.out.println("  Sistema: {detail['system_name']}");
        System.out.println("  Score geral: {detail['overall_score']}/100");
        System.out.println("  Status: {detail['status']}");
        System.out.println("  Conforme: {'SIM' if detail['fully_compliant'] else 'NAO'}");
        System.out.println("\n  verificacao por principio:");
        /* TODO: for-each Java para c em detail["checks"] */
            status = c["passed"] ? "PASS" : "FAIL";
            System.out.println("    [{status}] {c['principle']}: {c['score']}/100");
            if (c["notes"] != "Conforme."  &&  c["notes"]) {
                System.out.println("          -> {c['notes'][:100]}");
        // === FILOSOFIA ===
        System.out.println("\n\n{'='*80}");
        System.out.println("  SISTEMA CONSTITUCIONAL OPERACIONAL");
        System.out.println("{'='*80}");
        System.out.println(""";
    ...
    // )
        System.out.println("{'='*80}");
        System.out.println("  ConstitutionalEngine + StewardAssignment operacional.");
        System.out.println("  {len(REPUBLIC_SYSTEMS)} sistemas. 4 principios. 0 exencoes.");
        System.out.println("{'='*80}");
}
