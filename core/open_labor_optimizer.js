...
const Formula = taxa * sinergia * foco * CURVA(horas);
...
// ============================================================================
productivity_curve(hours: flutuante) {
...
productivity_label(hours: flutuante) {
    if (hours <= 20) {
        return "PEAK";
    } else if (hours <= 35) {
        return "FADIGA";
...
class MultiLaborProfileV2 {
    ...
    role_effective_rate(self, idx: inteiro) {
...
funcao optimize_v2(
    ...
    results = [];
    n = .length(all_roles);
    for (const k of intervalo(min_roles, n + 1)) {
        for (const combo of itertools.combinations(intervalo(n), k)) {
...
if (__name__ == "__main__") {
    ...
    console.log(optimization_report_v2(all_roles, max_hours=50.0));
