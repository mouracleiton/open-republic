...
if (__name__ == "__main__") {
    console.log("=" * 75);
    console.log("  SOLUCOES PARA OS 3 GARGALOS DO SAHEL");
    console.log("  Sem dinheiro. Sem empreiteira. Sem 'contratar'.");
    console.log("  Com automatizacao + comunidade + conhecimento aberto.");
    console.log("=" * 75);
    // === SOLUCAO 1: CUIDADO INFANTIL ===
    console.log("\n\n" + "=" * 75);
    console.log("  GARGALO 1: CUIDADO INFANTIL (300% sobrecarga)");
    console.log("  1 pessoa tentando cuidar de 300 criancas");
    console.log("=" * 75);
    ...
    console.log("  Demanda ajustada (IA reduz 25%): {childcare['demanda_ajustada_por_ia']:,}");
    console.log("\n  FONTES DE CUIDADO:");
    console.log("  {'Fonte':<35} {'Pessoas':>8} {'Equiv FT':>10}");
    console.log("  {'-'*55}");
    para cada (fonte, dados) em childcare['fontes_de_cuidado'].items(): {
        if ('pessoas' in dados) {
            console.log("  {fonte.replace('_', ' ').title():<35} {dados['pessoas']:>8} ";
                "{dados['equiv_fulltime']:>10}");
        } else if ('estacoes' in dados) {
            console.log("  {fonte.replace('_', ' ').title():<35} {dados['estacoes']:>8} estacoes");
    console.log("  {'-'*55}");
    console.log("  {'CAPACIDADE TOTAL':<35} {'':>8} {childcare['capacidade_total_equiv']:>10}");
    console.log("  {'DEMANDA TOTAL':<35} {'':>8} {childcare['demanda_total_equiv']:>10}");
    console.log("\n  COBERTURA: {childcare['cobertura']}");
    console.log("  VIÁVEL: {'SIM' if childcare['viavel'] else 'NAO'}");
    console.log("\n  PRINCIPIO: Nao se trata de contratar 75 profissionais.");
    console.log("  Trata-se de ativar a COMUNIDADE INTEIRA:");
    console.log("    - Cada pai/mae: 4h/semana");
    console.log("    - Cada idoso ativo: 10h/semana (continuar contribuindo)");
    console.log("    - Cada adolescente: 6h/semana (mentoria + responsabilidade)");
    console.log("    - Estacoes de IA: 25% do aprendizado && autonomo");
    console.log("  Resultado: a comunidade SE cuida. Nao precisa de Estado.");
    // === SOLUCAO 2: AGRICULTURA ===
    console.log("\n\n" + "=" * 75);
    console.log("  GARGALO 2: AGRICULTURA (167% sobrecarga)");
    console.log("  Faltam 2.000 kg/mes de comida para 40.000 pessoas");
    console.log("=" * 75);
    ...
    console.log("\n  ESTRATEGIAS:");
    console.log("  {'Estrategia':<30} {'Escala':>15} {'Producao':>12} {'Pessoas':>8}");
    console.log("  {'-'*68}");
    para cada (nome, dados) em agriculture['estrategias'].items(): {
        if (isinstance(dados, dict)) {
            ...
            console.log("  {nome.replace('_', ' ').title():<30} {escala:>15} {prod:>12} {pessoas:>8}");
    console.log("\n  {'PRODUCAO NOVA TOTAL':<30} {'':>15} {agriculture['producao_total_nova_kg_mes']:>9} kg/mes");
    console.log("  {'TRABALHADORES':<30} {'':>15} {'':>12} {agriculture['trabalhadores_necessarios']:>8}");
    console.log("  COBRE O GAP: {'SIM' if agriculture['cobre_gap'] else 'NAO'}");
    console.log("  AUTO-SUSTENTAVEL APOS: {agriculture['auto_sustentavel_apos']}");
    console.log("\n  PRINCIPIO: Hidroponia (10x mais por m2) + permacultura");
    console.log("  (auto-sustentavel apos 3 anos) + aquaponia (proteina + vegetais)");
    console.log("  + automacao IoT (40% menos trabalho) + conhecimento da Amazonia");
    // === SOLUCAO 3: HABITACAO ===
    console.log("\n\n" + "=" * 75);
    console.log("  GARGALO 3: HABITACAO (167% sobrecarga)");
    console.log("  Faltam ~2.000 unidades para 40.000 pessoas");
    console.log("=" * 75);
    ...
    console.log("\n  ESTRATEGIAS:");
    console.log("  {'Estrategia':<25} {'Unid/mes':>10} {'Material':<25} {'Pessoas':>8}");
    console.log("  {'-'*70}");
    para cada (nome, dados) em housing['estrategias'].items(): {
        if (isinstance(dados, dict)  &&  ('casas_por_mes' in dados  ||  'kits_por_mes' in dados)) {
            ...
            console.log("  {nome.replace('_', ' ').title():<25} {unid:>10} {mat:<25} {pessoas:>8}");
        } else if (isinstance(dados, dict)  &&  'massa_termica' in dados) {
            console.log("  {'Design Passivo':<25} {'N/A':>10} {'Sem ar condicionado':<25}");
    console.log("\n  {'TOTAL':<25} {housing['casas_por_mes_total']:>10}");
    console.log("  MESES PARA COBRIR GAP: {housing['meses_para_cobrir_gap']}");
    console.log("  TRABALHADORES: {housing['trabalhadores_necessarios']}");
    console.log("  CUSTO: {housing['custo_monetario']}");
    console.log("  MATERIAL: {housing['material_principal']}");
    console.log("\n  PRINCIPIO: Terra local && gratuita. Prensa manual faz bloco.");
    console.log("  Impressora 3D faz casa em 30h. Mutirao constroi 10 casas/semana.");
    console.log("  Design passivo: 40cm de parede de terra = sem ar condicionado.");
    // === RESUMO INTEGRADO ===
    console.log("\n\n" + "=" * 75);
    console.log("  RESUMO: COMO RESOLVER OS 3 GARGALOS SEM DINHEIRO");
    console.log("=" * 75);
    console.log(""";
+-----------+----------------+------------------+----------+--------+;
...
| | automacao | | drones | |;
+-----------+----------------+------------------+----------+--------+;
| Habitacao | 3D console.log + | Terra local | ~40 + | 6-12 |;
