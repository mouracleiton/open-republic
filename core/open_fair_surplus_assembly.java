...
public class OpenfairsurplusassemblyAssembleiaVota5EOSuficiente {
    ...
    public {texto: qualquer} run_assembly(n_voters: inteiro = 10000) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  ASSEMBLEIA CONSTITUINTE -- VOTACAO: EXCEDENTE DE 5%");
        System.out.println("=" * 80);
        result = run_assembly(10000);
        System.out.println("\n  PERGUNTA:\n    {result['question']}\n");
        System.out.println("  VOTACAO ({result['total']:,} cidadaos):\n");
        /* para option, count in ordene(result["votes"].items(), */
                                    key = (x) -> -x[1]):;
            pct = count / result["total"] * 100;
            bar = "#" * inteiro(pct / 2);
            winner_mark = option == result["winner"] ? " <--- VENCEDOR" : "";
            System.out.println("    {option:<25} {count:>6} ({pct:.0f}%) {bar}{winner_mark}");
        System.out.println("\n  RESULTADO: {result['decision']}");
        System.out.println("  Vencedor: {result['winner']} ({result['winner_percent']})");
        System.out.println("  Runner-up: {result['runner_up']} ({result['runner_up_votes']/result['total']*100:.0f}%)");
        System.out.println("\n  LEI RATIFICADA:");
        /* para cada (key, val) em result["law"].items(): */
            if (isinstance(val, flutuante)) {
                System.out.println("    {key:<30} {val:.0%}");
            } else {
                System.out.println("    {key:<30} {val}");
        System.out.println("\n{'='*80}");
        System.out.println("  ASSEMBLEIA DECIDIU: 5% de excedente. LEI.");
        System.out.println("  62% dos cidadaos concordaram.");
        System.out.println("  Acima de 5% = PREDATORIO = BLOQUEADO.");
        System.out.println("  O 5% volta pra quem trabalha. Sempre.");
        System.out.println("{'='*80}");
}
