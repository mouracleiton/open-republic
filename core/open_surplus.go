...
package opensurplus_metrica_de_necessidade_de_excedente_por_estabelecimento
...
type SurplusMetric struct {
    ...
    name: texto
    // No modo EXECUTAVEL
    custo_operacional_pct := 0.0 // % da producao gasto em operacao // float64
    investimento_justo_pct := 0.0 // % para melhorar // float64
    reserva_pct := 0.0 // % para emergencia // float64
    comunitario_pct := 0.0 // % para comunidade // float64
    // No modo IDEAL
    ideal_equivalent := ""  // como funciona sem excedente // string
    // Custo operacional detalhado
    custo_aluguel_pct := 0.0 // float64
    custo_equipamento_pct := 0.0 // float64
    custo_insumos_pct := 0.0 // float64
    custo_energia_pct := 0.0 // float64
    custo_manutencao_pct := 0.0 // float64
    // Limite predatorio
    max_excedente_justo_pct := 0.0 // acima disso = predatorio // float64
    excedente_predatorio_exemplo := "" // string
    // decorador: @property
    func excedente_necessario_total(self) float64 {
        return (self.custo_operacional_pct + self.investimento_justo_pct
                + self.reserva_pct + self.comunitario_pct)
    // decorador: @property
    func max_predatorio_pct(self) float64 {
        return maximo(0.0, 1.0 - self.excedente_necessario_total)
    // decorador: @property
    func trabalhador_recebe_pct(self) float64 {
...
type SurplusCalculation struct {
    ...
    // Dados reais
    workers := 1 // int64
    hours_per_worker_month := 160.0 // 40h/sem * 4 // float64
    value_per_hour := 0.0 // valor que cada hora gera // float64
    worker_salary_month := 0.0 // quanto o trabalhador recebe // float64
    establishment_revenue_month := 0.0 // receita total // float64
    operational_cost_month := 0.0 // custo operacional real // float64
    // Resultados
    production_total := 0.0 // valor total produzido // float64
    worker_receives_total := 0.0 // total pago a trabalhadores // float64
    surplus_extracted := 0.0 // excedente extraido // float64
    surplus_pct := 0.0 // % de excedente // float64
    surplus_type := SurplusType.NECESSARIO // SurplusType
    // decorador: @property
    func is_predatory(self) bool {
        return self.surplus_type in (SurplusType.EXCESSIVO, SurplusType.PREDATORIO)
// ============================================================================
// 5. MOTOR DE EXCEDENTE
// ============================================================================
type SurplusEngine struct {
    ...
    func get_metric(self, est_type: EstablishmentType) {texto: qualquer} {
    ...
    funcao calculate(self, est_type: EstablishmentType,
                ...
                operational_cost_month := 0.0 // float64
                ) -> {texto: qualquer}:
    ...
    func ideal_mode_equivalent(self, est_type: EstablishmentType) {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENSURPLUS -- METRICA DE EXCEDENTE POR ESTABELECIMENTO")
    fmt.Println("  Quanto excedente && NECESSARIO vs PREDATORIO?")
    fmt.Println("=" * 80)
    // === 1. METRICAS POR TIPO ===
    fmt.Println("\n\n  === 1. LIMITES DE EXCEDENTE POR TIPO ===\n")
    fmt.Println("  {'Tipo':<25} {'Necessario':>12} {'Max Justo':>12} {'Trab. recebe':>14} {'Acima &&':>10}")
    fmt.Println("  {'-'*75}")
    for _, c := range engine.compare_all() {
        fmt.Println("  {c['tipo']:<25} {c['necessario']:>12} {c['max_justo']:>12} "
            "{c['trabalhador_recebe']:>14} {c['acima_e_predatorio']:>10}")
    // === 2. DETALHE: HOSPITAL ===
    fmt.Println("\n\n  === 2. DETALHE: HOSPITAL/CLINICA ===\n")
    hosp = engine.get_metric(EstablishmentType.SAUDE)
    fmt.Println("  Excedente necessario: {hosp['excedente_necessario']}")
    fmt.Println("  Trabalhador recebe: {hosp['trabalhador_recebe']}")
    fmt.Println("  Limite predatorio: {hosp['limite_predatorio']}")
    fmt.Println("\n  Custo operacional:")
    ...
    fmt.Println("  Cada trabalhador: produz {r1['cada_trabalhador']['produz']}, "
    ...
    fmt.Println("  Razao exploracao: {r2['cada_trabalhador']['razao_exploracao']}")
    ...
    para et em [EstablishmentType.SAUDE, EstablishmentType.SOFTWARE, {
        ...
        fmt.Println("\n  {ideal['tipo']}:")
        fmt.Println("    Excedente: {ideal['excedente']}")
        fmt.Println("    Trabalhador: {ideal['trabalhador_recebe']}")
        fmt.Println("    {ideal['ideal'][:70]}...")
    // === 5. STATS ===
    fmt.Println("\n\n  === 5. ESTATISTICAS ===\n")
    s = engine.stats()
    para cada (k, v) em s.items(): {
        fmt.Println("  {k:<30} {v}")
    fmt.Println("\n{'='*80}")
    fmt.Println("  OpenSurplus: {s['tipos_cadastrados']} tipos, "
