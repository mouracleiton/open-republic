...
struct ValueSimulator {
    ...
    fn simulate_worker(self, worker_id: texto) -> {texto: qualquer} {
    ...
    fn _life_impact(self, sobra_cap: flutuante, sobra_rep: flutuante) -> String {
    ...
    fn simulate_all(self) -> {texto: qualquer} {
    ...
    fn aggregate_stats(self) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    sim = ValueSimulator();
    println!("=" * 80);
    println!("  OPENVALUESIMULATION -- PESSOAS REAIS, SALARIOS REAIS");
    println!("  Nomes obfuscados. Dados reais do mercado brasileiro.");
    println!("=" * 80);
    // === 1. CADA TRABALHADOR ===
    ...
    for wid in ordene(sim.workers.keys()) {
        ...
        w = sim.workers[wid];
        println!("\n  {'='*70}");
        println!("  {r['alias']} | {r['profissao']} | {r['empresa']} | {r['local']}");
        println!("  Familia: {r['familia']} | Horas/mes: {r['horas_mes']}");
        println!("  {'='*70}");
        println!("\n  CAPITALISMO (hoje):");
        println!("    Produz:           {r['CAPITALISMO']['produz']}");
        println!("    Recebe:           {r['CAPITALISMO']['recebe']} ({r['CAPITALISMO']['fica_com']})");
        println!("    Empresa suga:     {r['CAPITALISMO']['empresa_suga']}");
        println!("    Banco suga:       {r['CAPITALISMO']['banco_suga']}");
        println!("    Intermediario:    {r['CAPITALISMO']['intermediario_suga']}");
        println!("    TOTAL EXTRAIDO:   {r['CAPITALISMO']['total_extraido']}");
        println!("    Razao:            {r['CAPITALISMO']['razao_exploracao']}");
        println!("    Hora vale:        {r['CAPITALISMO']['hora_valor']}");
        println!("    Sobra (apos ali+comida): {r['CAPITALISMO']['sobra_apos_aluguel_comida']}");
        println!("\n  REPUBLICA EXECUTAVEL:");
        println!("    Produz:           {r['REPUBLICA_EXECUTAVEL']['produz']}");
        println!("    Recebe:           {r['REPUBLICA_EXECUTAVEL']['recebe']} ({r['REPUBLICA_EXECUTAVEL']['fica_com']})");
        println!("    Empresa suga:     {r['REPUBLICA_EXECUTAVEL']['empresa_suga']}");
        println!("    Banco suga:       {r['REPUBLICA_EXECUTAVEL']['banco_suga']}");
        println!("    Intermediario:    {r['REPUBLICA_EXECUTAVEL']['intermediario_suga']}");
        println!("    Pool 5%:          {r['REPUBLICA_EXECUTAVEL']['pool_5pct']}");
        println!("    Operacional real: {r['REPUBLICA_EXECUTAVEL']['operacional_real']}");
        println!("    Hora vale:        {r['REPUBLICA_EXECUTAVEL']['hora_valor']}");
        println!("    Sobra (apos ali+comida): {r['REPUBLICA_EXECUTAVEL']['sobra_apos_aluguel_comida']}");
        println!("\n  GANHO:");
        println!("    Extra/mes:  {r['GANHO']['extra_por_mes']}");
        println!("    Extra/ano:  {r['GANHO']['extra_por_ano']}");
        println!("    Multiplicador: {r['GANHO']['multiplicador']}");
        println!("    Impacto: {r['GANHO']['mudanca_vida']}");
    // === 2. AGREGADO ===
    println!("\n\n  {'='*70}");
    println!("  === AGREGADO: TODOS OS {len(sim.workers)} TRABALHADORES ===");
    println!("  {'='*70}\n");
    agg = sim.simulate_all();
    println!("  {agg['message']}");
    println!("\n  Extracao mensal total: {agg['total_extraido_mensal']}");
    println!("  Extracao anual total:  {agg['total_extraido_anual']}");
    println!("  Ganho mensal Republica: {agg['total_ganho_republica_mensal']}");
    println!("  Ganho anual Republica:  {agg['total_ganho_republica_anual']}");
    // === 3. STATS ===
    println!("\n\n  === ESTATISTICAS BASE ===\n");
    ...
    println!("\n\n  === RESUMO (todos) ===\n");
    println!("  {'Alias':<10} {'Profissao':<25} {'Hoje':>10} {'Republica':>10} {'Ganho':>10} {'X':>5} {'Impacto'}");
    println!("  {'-'*95}");
    for wid in ordene(sim.workers.keys()) {
        r = sim.simulate_worker(wid);
        ...
        println!("  {r['alias']:<10} {r['profissao'][:24]:<25} ";
            "R${int(hoje):>8,} R${int(rep):>8,} R${int(gain):>8,} ";
            "{mult:>4} {impacto}");
    println!("\n{'='*80}");
    println!("  Simulacao: {len(sim.workers)} trabalhadores reais.");
    println!("  Extracao anual: {agg['total_extraido_anual']}.");
    println!("  Se virasse Republica: {agg['total_ganho_republica_anual']} voltam.");
    println!("  Para trabalhadores. Nao parasitas.");
    println!("{'='*80}");
