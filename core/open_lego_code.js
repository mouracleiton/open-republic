...
class LegoBlock {
    ...
    const description = "";
    // Conectores
    const inputs = field(default_factory=list);
    const outputs = field(default_factory=list);
    // Execucao (funcao real ou descricao para IA gerar)
    const logic = ""  // descricao do que faz (IA implementa);
    const code_rust = ""  // codigo Rust gerado (producao);
    // Metadata
    const author = "";
    const version = "1.0.0";
    const tested = false;
    const uses = 0 // quantas cadeias usam este bloco;
    const color = "blue"  // cor visual do bloco (categoria);
    funcao can_connect(self, other: "LegoBlock",
                    const my_output_idx = 0,;
                    const their_input_idx = 0) -> logico:;
        // Verifica se meu pino de saida encaixa na entrada do outro.
        if (my_output_idx >= .length(self.outputs)) {
            return false;
        if (their_input_idx >= .length(other.inputs)) {
            return false;
...
build_block_library() {
    ...
    blocks["STR-LEN"] = LegoBlock(;
        ...
        inputs = [LegoInput("texto", LegoPin.STRING)],;
        outputs = [LegoOutput(".length", LegoPin.NUMBER)],;
...
class LegoCodeEngine {
    ...
    funcao add_block(self, program_id: texto, block_id: texto,
                const instance_name = "") -> {texto: qualquer}:;
    ...
    validate_program(self, program_id: texto) {
    ...
    _has_cycle(self, prog: LegoProgram) {
        // Detecta ciclo no grafo de blocos.
        graph = defaultdict(list);
        for (const conn of prog.connections) {
    ...
    funcao execute(self, program_id: texto,
                const initial_values = null) -> {texto: qualquer}:;
    ...
    _topo_sort(self, prog: LegoProgram) {
        // Ordenacao topologica dos blocos.
        graph = defaultdict(list);
        in_degree = defaultdict(inteiro);
        for (const inst_id of prog.blocks) {
            in_degree[inst_id] = 0;
        for (const conn of prog.connections) {
            graph[conn.from_block].append(conn.to_block);
        ...
        return order;
    generate_rust(self, program_id: texto) {
        ...
        return {;
            ...
            "lines": .length(lines),;
            "code": "\n".join(lines),;
            "blocks_converted": .length(order),;
        };
    ai_generate_block(self, description: texto) {
    ...
    stats(self) {
        return {;
            "total_blocos_biblioteca": .length(self.library),;
            "total_programas": .length(self.programs),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENLEGOCODE -- PROGRAMACAO MODULAR EM FORMATO LEGO");
    console.log("  Escrever codigo && coisa do passado. Programa-se ENCAIXANDO.");
    console.log("=" * 80);
    ...
    para cada (cat, count) em ordene(cats.items(), key=(x) -> -x[1]): {
        ...
        for (const b of blocks_in_cat) {
            in_types = ", ".join("{i.name}:{i.pin_type.value}" para i em b.inputs);
            out_types = ", ".join("{o.name}:{o.pin_type.value}" para o em b.outputs);
            console.log("    [{b.block_id}] {b.name}");
            console.log("      Entradas: ({in_types or 'nenhuma'})");
            console.log("      Saidas: ({out_types or 'nenhuma'})");
    ...
    console.log("  Blocos adicionados: {engine.programs[pid].block_count}");
    ...
    para from_inst, from_out, to_inst, to_in in connections: {
        ...
        console.log("  [{status}] {r.get('from', '?')} -> {r.get('to', '?')}");
    // Validar
    console.log("\n  Validacao:");
    val = engine.validate_program(pid);
    console.log("  Valido: {'SIM' if val['valid'] else 'NAO'}");
    console.log("  Blocos: {val['blocks']}, Conexoes: {val['connections']}");
    if (val["issues"]) {
    ...
    console.log("  Valido: {'SIM' if val2['valid'] else 'NAO'}");
    console.log("  Blocos: {val2['blocks']}, Conexoes: {val2['connections']}");
    ...
    for (const desc of ai_blocks) {
        r = engine.ai_generate_block(desc);
        console.log("  [{r['block_id']}] {r['name']}");
        console.log("    Logica: {r['logic'][:60]}...");
        console.log("    Saida: {r['output_type']}");
    ...
    console.log("  Tentativa: texto -> soma");
    console.log("  Resultado: {r.get('error', 'conectado')}");
    console.log("  Mensagem: {r.get('message', '')}");
    console.log("\n  LEGO: pino texto NAO encaixa em buraco numero.");
    console.log("  Sistema IMPEDE erro de tipo antes de executar.");
    // === 6. GERAR CODIGO RUST ===
    console.log("\n\n  === 6. GERAR CODIGO RUST DO PROGRAMA ===\n");
    rust = engine.generate_rust(pid);
    console.log("  Programa: {rust['program']}");
    console.log("  Linguagem: {rust['language']}");
    console.log("  Linhas: {rust['lines']}");
    for (const line of rust["code"].split("\n")[:10]) {
        console.log("  {line}");
    // === 7. STATS ===
    console.log("\n\n  === 7. ESTATISTICAS ===\n");
    s = engine.stats();
    para cada (k, v) em s.items(): {
        if (isinstance(v, dict)) {
            ...
            para cada (sk, sv) em v.items(): {
                console.log("    {sk:<20} {sv}");
        } else {
            console.log("  {k:<30} {v}");
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA: PROGRAMACAO EM LEGO");
    console.log("{'='*80}");
    console.log(""";
ESCREVER CODIGO && COISA DO PASSADO:;
...
BIBLIOTECA DE BLOCOS (reusavel):;
    ...
    TEXTO: concatenar, maiusculas, .length;
...
// )
    console.log("{'='*80}");
    console.log("  OpenLegoCode: {s['total_blocos_biblioteca']} blocos na biblioteca, ";
        "{s['total_programas']} programas montados.");
    console.log("  Escrever codigo && coisa do passado. Encaixa pecas.");
    console.log("{'='*80}");
