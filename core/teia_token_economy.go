...
package teia_token_economy_assinaturas_e_creditos_tokenizados
...
const (
    ...
    func __init__(self, key: texto, description: texto) {
        self.key = key
...
type TokenWallet struct {
    ...
    // Saldos por tipo
    free_tokens := 0 // float64
    paid_tokens := 0 // float64
    credit_tokens := 0 // float64
    // Historico
    transactions := field(default_factory=list) // [Dict]
    // decorador: @property
    func total(self) float64 {
        return self.free_tokens + self.paid_tokens + self.credit_tokens
    func add(self, token_type: TokenType, amount: flutuante, reason: texto = "") {
    ...
    func consume(self, amount: flutuante, endpoint: texto = "") bool {
...
// ============================================================================
// decorador: @dataclass
type TokenPricedEndpoint struct {
    ...
    royalty_chain := field(default_factory=dict) // contributor_id -> weight // {texto: flutuante}
    func revenue_per_call_brl(self, token_price_brl: flutuante = 0.10) float64 {
...
type SubscriptionPlan struct {
    ...
    // decorador: @property
    func discount_pct(self) float64 {
...
// 5. MOTOR DE COBRANCA
// ============================================================================
type TokenBillingEngine struct {
    ...
    //
    func __init__(self, token_price_brl: flutuante = 0.10) {
        ...
        self.call_log: [Dict] = []
    func create_wallet(self, wallet_id: texto, name: texto, wtype: texto) TokenWallet {
        w = TokenWallet(wallet_id=wallet_id, owner_name=name, owner_type=wtype)
        self.wallets[wallet_id] = w
        return w
    func subscribe(self, wallet_id: texto, plan: SubscriptionPlan) {
    ...
    func buy_tokens(self, wallet_id: texto, n_tokens: flutuante) {
    ...
    func api_call(self, wallet_id: texto, endpoint_path: texto) {texto: qualquer} {
...
// ============================================================================
func print_token_system() string {
    lines = []
    lines.append("=" * 110)
    ...
    para cada (name, cost) em examples: {
        ...
        for _, pname := range ["FREE", "STARTER", "PRO", "GOV", "ENTR"] {
    ...
    for _, cid := range ["cleiton", "subagent_1", "subagent_3", "hermes"] {
    ...
    for _, day := range intervalo(30) {
        for _, cw := range client_wallets {
            w = engine.wallets[cw]
            // Cada cliente faz 20-100 chamadas/dia
            n_calls = rng.randint(20, 100)
            for _, _ := range intervalo(n_calls) {
...
if __name__ == "__main__" {
    fmt.Println(print_token_system())
