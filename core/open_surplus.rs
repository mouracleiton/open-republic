...
struct SurplusMetric {
    ...
    name: texto;
    // No modo EXECUTAVEL
    let custo_operacional_pct: f64 = 0.0 // % da producao gasto em operacao;
    let investimento_justo_pct: f64 = 0.0 // % para melhorar;
    let reserva_pct: f64 = 0.0 // % para emergencia;
    let comunitario_pct: f64 = 0.0 // % para comunidade;
    // No modo IDEAL
    let ideal_equivalent: String = ""  // como funciona sem excedente;
    // Custo operacional detalhado
    let custo_aluguel_pct: f64 = 0.0;
    let custo_equipamento_pct: f64 = 0.0;
    let custo_insumos_pct: f64 = 0.0;
    let custo_energia_pct: f64 = 0.0;
    let custo_manutencao_pct: f64 = 0.0;
    // Limite predatorio
    let max_excedente_justo_pct: f64 = 0.0 // acima disso = predatorio;
    let excedente_predatorio_exemplo: String = "";
    // decorador: @property
    fn excedente_necessario_total(self) -> f64 {
        return (self.custo_operacional_pct + self.investimento_justo_pct;
                + self.reserva_pct + self.comunitario_pct);
    // decorador: @property
    fn max_predatorio_pct(self) -> f64 {
        return maximo(0.0, 1.0 - self.excedente_necessario_total);
    // decorador: @property
    fn trabalhador_recebe_pct(self) -> f64 {
...
struct SurplusCalculation {
    ...
    // Dados reais
    let workers: i64 = 1;
    let hours_per_worker_month: f64 = 160.0 // 40h/sem * 4;
    let value_per_hour: f64 = 0.0 // valor que cada hora gera;
    let worker_salary_month: f64 = 0.0 // quanto o trabalhador recebe;
    let establishment_revenue_month: f64 = 0.0 // receita total;
    let operational_cost_month: f64 = 0.0 // custo operacional real;
    // Resultados
    let production_total: f64 = 0.0 // valor total produzido;
    let worker_receives_total: f64 = 0.0 // total pago a trabalhadores;
    let surplus_extracted: f64 = 0.0 // excedente extraido;
    let surplus_pct: f64 = 0.0 // % de excedente;
    let surplus_type: SurplusType = SurplusType.NECESSARIO;
    // decorador: @property
    fn is_predatory(self) -> bool {
...
struct SurplusEngine {
    ...
    fn get_metric(self, est_type: EstablishmentType) -> {texto: qualquer} {
    ...
    funcao calculate(self, est_type: EstablishmentType,
                ...
                let operational_cost_month: f64 = 0.0;
                ) -> {texto: qualquer}:;
    ...
    fn ideal_mode_equivalent(self, est_type: EstablishmentType) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENSURPLUS -- METRICA DE EXCEDENTE POR ESTABELECIMENTO");
    println!("  Quanto excedente && NECESSARIO vs PREDATORIO?");
    println!("=" * 80);
    // === 1. METRICAS POR TIPO ===
    println!("\n\n  === 1. LIMITES DE EXCEDENTE POR TIPO ===\n");
    println!("  {'Tipo':<25} {'Necessario':>12} {'Max Justo':>12} {'Trab. recebe':>14} {'Acima &&':>10}");
    println!("  {'-'*75}");
    for c in engine.compare_all() {
        println!("  {c['tipo']:<25} {c['necessario']:>12} {c['max_justo']:>12} ";
            "{c['trabalhador_recebe']:>14} {c['acima_e_predatorio']:>10}");
    // === 2. DETALHE: HOSPITAL ===
    println!("\n\n  === 2. DETALHE: HOSPITAL/CLINICA ===\n");
    hosp = engine.get_metric(EstablishmentType.SAUDE);
    println!("  Excedente necessario: {hosp['excedente_necessario']}");
    println!("  Trabalhador recebe: {hosp['trabalhador_recebe']}");
    println!("  Limite predatorio: {hosp['limite_predatorio']}");
    println!("\n  Custo operacional:");
    ...
    println!("  {r1['message']}");
    println!("  Cada trabalhador: produz {r1['cada_trabalhador']['produz']}, ";
    ...
    println!("  {r2['message']}");
    println!("  Razao exploracao: {r2['cada_trabalhador']['razao_exploracao']}");
    ...
    para et em [EstablishmentType.SAUDE, EstablishmentType.SOFTWARE, {
        ...
        println!("\n  {ideal['tipo']}:");
        println!("    Excedente: {ideal['excedente']}");
        println!("    Trabalhador: {ideal['trabalhador_recebe']}");
        println!("    {ideal['ideal'][:70]}...");
    // === 5. STATS ===
    println!("\n\n  === 5. ESTATISTICAS ===\n");
    s = engine.stats();
    para cada (k, v) em s.items(): {
        println!("  {k:<30} {v}");
    println!("\n{'='*80}");
    println!("  OpenSurplus: {s['tipos_cadastrados']} tipos, ";
