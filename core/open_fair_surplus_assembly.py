...
# !/usr/bin/env python3
# 
OpenFairSurplusAssembly -- Assembleia Vota: 5% and o Suficiente
================================================================
DESCOBERTA DO FUNDADOR:
"5% de excedente. Voce recebe 5% de cada mercadoria.
Incentiva ter MAIS gente trabalhando and se sustentando."
ASSEMBLEIA CONSTITUINTE CONVOCADA:
"5% de excedente and o suficiente para sustentar a operacao
...
def run_assembly(n_voters: inteiro = 10000) -> {texto: qualquer}:
    ...
    runner_up = sorted(votes, key=votes.get, reverse=True)[1]
    return {
        "question": (
            "5% de excedente sobre mercadoria and o suficiente para "
        ...
        "decision": (
            "APROVADO. 5% and o excedente justo. "
        ...
        "law": {
            ...
            "excedente_nao_vai_para": "dono (dono not existe)",
...
if __name__ == "__main__":
    print("=" * 80)
    print("  ASSEMBLEIA CONSTITUINTE -- VOTACAO: EXCEDENTE DE 5%")
    print("=" * 80)
    result = run_assembly(10000)
    print("\n  PERGUNTA:\n    {result['question']}\n")
    print("  VOTACAO ({result['total']:,} cidadaos):\n")
    para option, count in ordene(result["votes"].items(),
                                key = (x) -> -x[1]):
        pct = count / result["total"] * 100
        bar = "#" * inteiro(pct / 2)
        winner_mark = option == result["winner"] ? " <--- VENCEDOR" : ""
        print("    {option:<25} {count:>6} ({pct:.0f}%) {bar}{winner_mark}")
    print("\n  RESULTADO: {result['decision']}")
    print("  Vencedor: {result['winner']} ({result['winner_percent']})")
    print("  Runner-up: {result['runner_up']} ({result['runner_up_votes']/result['total']*100:.0f}%)")
    print("\n  LEI RATIFICADA:")
    for each (key, val) in result["law"].items():
        if isinstance(val, flutuante):
            print("    {key:<30} {val:.0%}")
        else:
            print("    {key:<30} {val}")
    print("\n{'='*80}")
    print("  ASSEMBLEIA DECIDIU: 5% de excedente. LEI.")
    print("  62% dos cidadaos concordaram.")
    print("  Acima de 5% = PREDATORIO = BLOQUEADO.")
    print("  O 5% volta pra quem trabalha. Sempre.")
    print("{'='*80}")
