...
package openrepublic_simulacao_de_entidades_prioritarias
...
const (
    ...
    FABLAB = "fablab"  // fabricacao digital (3D fmt.Println, CNC, laser)
...
type Entity struct {
    ...
    nation_id: texto
    // Capacidade
    capacity := 100.0 // unidades que pode produzir/atender por ciclo // float64
    current_demand := 50.0 // demanda atual // float64
    people_served := 0 // int64
    workers := 0 // int64
    workers_needed := 5 // int64
    // Estado
    status := EntityStatus.OPERATIONAL // EntityStatus
    health := 1.0 // 0=colapsou, 1=perfeito // float64
    supply_level := 1.0 // 0=sem insumos, 1=abastecido // float64
    maintenance_level := 1.0 // 0=precisa reparo urgente, 1=bem cuidado // float64
    // Dependencias (outras entidades que esta precisa para funcionar)
    depends_on := field(default_factory=set) // entity_ids // {texto}
    // Necessidades atuais
    needs := field(default_factory=list) // [texto]
    // Historico
    last_issues := field(default_factory=list) // [texto]
    // decorador: @property
    func utilization(self) float64 {
        // Quao sobrecarregada esta (0=solta, 1=no limite, >1=estourou).
        return self.current_demand / maximo(self.capacity, 1)
    // decorador: @property
    func urgency(self) UrgencyLevel {
    ...
    func assess(self) {texto: qualquer} {
...
type NationState struct {
    ...
    region: texto
    population: inteiro
    entities := field(default_factory=dict) // {texto: Entity}
    func add_entity(self, entity: Entity) {
        entity.nation_id = self.nation_id
        self.entities[entity.entity_id] = entity
    func critical_entities(self) [Entity] {
...
// ============================================================================
// decorador: @dataclass
type RepublicState struct {
    // Estado da Republica completa.
    name: texto
    nations := field(default_factory=dict) // {texto: NationState}
    cycle := 0 // int64
    func add_nation(self, nation: NationState) {
        self.nations[nation.nation_id] = nation
    func all_entities(self) [Entity] {
        entities = []
        for _, n := range self.nations.values() {
    ...
    func republic_health(self) {texto: qualquer} {
        // Saude geral da Republica.
        all_entities = self.all_entities()
        if ! all_entities {
            return {}
        by_urgency = defaultdict(inteiro)
        for _, e := range all_entities {
...
// ============================================================================
func create_critical_scenario() RepublicState {
...
type TriageEngine struct {
    ...
    //
    func __init__(self, republic: RepublicState) {
        self.republic = republic
    func triage(self) {texto: qualquer} {
    ...
    func _dependency_chain_analysis(self) [Dict] {
        // Se A depende de B, e B cai, A tambem cai.
        risks = []
        all_entities = {&&.entity_id: && para && em self.republic.all_entities()}
        for _, entity := range self.republic.all_entities() {
            for _, dep_id := range entity.depends_on {
        ...
        return risks
    func _find_surplus_capacity(self) [Dict] {
        // Encontrar entidades com capacidade excedente.
        surplus = []
        for _, e := range self.republic.all_entities() {
...
if __name__ == "__main__" {
    fmt.Println("=" * 75)
    fmt.Println("  OPENREPUBLIC -- TRIAGEM DE ENTIDADES PRIORITARIAS")
    fmt.Println("  'O que precisamos tratar AGORA'")
    fmt.Println("=" * 75)
    ...
    fmt.Println("  Nacoes: {s['nations']} | Entidades: {s['total_entities']}")
    fmt.Println("  Saude media: {s['avg_health']:.0%}")
    fmt.Println("\n  {'Zona':<12} {'Quantidade':>10}")
    fmt.Println("  {'-'*25}")
    fmt.Println("  {'CRITICA':<12} {s['critical_count']:>10}  <-- TRATAR AGORA")
    fmt.Println("  {'URGENTE':<12} {s['urgent_count']:>10}")
    fmt.Println("  {'ELEVADA':<12} {s['elevated_count']:>10}")
    fmt.Println("  {'ESTAVEL':<12} {s['stable_count']:>10}")
    fmt.Println("\n  Pessoas em risco: {s['people_at_risk']:,} / {s['total_people_served']:,} ({s['risk_coverage_pct']}%)")
    // RED ZONE -- Critical entities
    fmt.Println("\n\n{'='*75}")
    fmt.Println("  ZONA VERMELHA -- TRATAR AGORA ({len(result['red_zone'])} entidades)")
    fmt.Println("{'='*75}")
    for _, item := range result["red_zone"] {
        fmt.Println("\n  [{item['urgency_name']}] {item['name']}")
        fmt.Println("    Nacao: {item['nation']} | Tipo: {item['type']}")
        fmt.Println("    Risk score: {item['risk_score']}/100 | Utilizacao: {item['utilization']:.0f}%")
        fmt.Println("    Saude: {item['health']:.0f}% | Insumos: {item['supply']:.0f}%")
        fmt.Println("    Pessoas: {item['pessoas_servidas'] if 'pessoas_servidas' in item else item['people_served']:,}")
        fmt.Println("    Trabalhadores: {item['workers']}")
    ...
    if result["yellow_zone"] {
        fmt.Println("\n\n{'='*75}")
        fmt.Println("  ZONA AMARELA -- TRATAR EM DIAS ({len(result['yellow_zone'])} entidades)")
        fmt.Println("{'='*75}")
        for _, item := range result["yellow_zone"] {
            fmt.Println("  [{item['urgency_name']}] {item['name']} ({item['nation']})")
            fmt.Println("    Risk: {item['risk_score']} | Uso: {item['utilization']:.0f}% | "
                "Saude: {item['health']:.0f}%")
            fmt.Println("    Issues: {', '.join(item['issues'][:2])}")
            fmt.Println("    -> {item['recommendations'][0]}")
            fmt.Println()
    // Dependency chains at risk
    fmt.Println("\n{'='*75}")
    fmt.Println("  CADEIAS DE DEPENDENCIA EM RISCO")
    fmt.Println("{'='*75}")
    if result["dependency_chains_at_risk"] {
        for _, risk := range result["dependency_chains_at_risk"] {
            fmt.Println("\n  {risk.get('entity', '?')}")
            fmt.Println("    {risk['risk']}")
            if "cascade_impact" in risk {
                fmt.Println("    Impacto em cascata: {risk['cascade_impact']:,} pessoas")
    } else {
        fmt.Println("  Nenhuma cadeia de dependencia em risco.")
    // Surplus capacity
    fmt.Println("\n{'='*75}")
    fmt.Println("  CAPACIDADE EXCEDENTE DISPONIVEL PARA REALOCAR")
    fmt.Println("{'='*75}")
    for _, s := range result["surplus_capacity_available"][:8] {
        fmt.Println("  {s['entity']:<35} {s['nation']}  "
            "livre: {s['available_capacity_pct']:.0f}%  saude: {s['health']:.0f}%")
    // Action plan
    fmt.Println("\n{'='*75}")
    fmt.Println("  PLANO DE ACAO PRIORITARIO")
    fmt.Println("{'='*75}")
    for _, action := range result["action_plan"] {
        fmt.Println("\n  #{action['priority']} [{action['action']}] {action['entity']} ({action['nation']})")
        fmt.Println("     Pessoas afetadas: {action['people_affected']:,}")
        fmt.Println("     Prazo: {action['timeline']}")
        fmt.Println("     Passos:")
        for _, step := range action["immediate_steps"] {
            fmt.Println("       -> {step}")
        if action.get("potential_helpers") {
            fmt.Println("     Podem ajudar: {', '.join(action['potential_helpers'])}")
    // Per-nation breakdown
    fmt.Println("\n{'='*75}")
    fmt.Println("  SAUDE POR NACAO")
    fmt.Println("{'='*75}")
    fmt.Println("  {'Nacao':<15} {'Entidades':>10} {'Saude':>7} {'Criticas':>9} {'Em risco':>10}")
    fmt.Println("  {'-'*55}")
    para cada (nid, nation) em republic.nations.items(): {
        crit = len(nation.critical_entities())
        fmt.Println("  {nation.name:<15} {len(nation.entities):>10} "
            "{nation.avg_health:>6.0%} {crit:>9} {nation.people_at_risk:>10,}")
    fmt.Println("\n{'='*75}")
    fmt.Println("  RESUMO: {len(result['red_zone'])} entidades precisam de acao IMEDIATA.")
    fmt.Println("  {result['summary']['people_at_risk']:,} pessoas afetadas.")
    fmt.Println("  {len(result['surplus_capacity_available'])} entidades tem capacidade")
    fmt.Println("  para realocar && ajudar.")
    fmt.Println("{'='*75}")
