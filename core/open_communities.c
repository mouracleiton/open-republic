...
#ifndef OPENCOMMUNITIES_6_ADAPTACOES_COMUNITARIAS_DA_REPUBLICA_H
#define OPENCOMMUNITIES_6_ADAPTACOES_COMUNITARIAS_DA_REPUBLICA_H
...
// ============================================================================
// decorador: @dataclass
typedef struct CommunityProfile {
    ...
    char* decision_making = ""  // como decidem (consenso, voto, conselho);
    // Lingua
    char* primary_language = "pt-BR";
    [texto] secondary_languages = field(default_factory=list);
    // Isolamento geografico
    char* isolation_level = ""  // urbano, semi-isolado, isolado, muito isolado;
    char* access_challenge = ""  // barreira de acesso (rio, estrada, etc);
// ============================================================================
// 3. OS 6 PERFIS
// ============================================================================
...
// ============================================================================
typedef struct CommunityEngine {
    ...
    {texto: qualquer} get_profile(self, community_type: CommunityType) {
    ...
    [Dict] adaptation_matrix(self) {
        ...
        matrix = [];
        /* TODO: iterador C manual para system em ordene(systems) */
            row = {"sistema": system};
            /* TODO: iterador C manual para p em self.profiles.values() */
            ...
            matrix.append(row);
        return matrix;
    {texto: Dict} what_already_exists(self) {
    ...
    [Dict] compare_communities(self) {
        // Compara necessidades entre comunidades.
        return [;
            {
                "comunidade": p.name,;
                "necessidades": sizeof(p.needs),;
                "sistemas_adaptados": sizeof(p.adapted_systems),;
                "respeitos": sizeof(p.do_not_import),;
                "pilares_culturais": sizeof(p.cultural_pillars),;
                "praticas_existentes": sizeof(p.existing_practices),;
                "isolamento": p.isolation_level,;
            };
            /* para p em self.profiles.values() */
        ];
    {texto: qualquer} stats(self) {
        return {;
            "comunidades": sizeof(self.profiles),;
            "total_necessidades": soma(sizeof(p.needs) para p em self.profiles.values()),;
            "total_sistemas_adaptados": soma(sizeof(p.adapted_systems) para p em self.profiles.values()),;
            "total_respeitos_culturais": soma(sizeof(p.do_not_import) para p em self.profiles.values()),;
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENCOMMUNITIES -- 6 ADAPTACOES COMUNITARIAS");
    printf("  O modelo se ADAPTA. Nao se impoe.");
    printf("=" * 80);
    // === 1. COMPARACAO GERAL ===
    printf("\n\n  === 1. COMPARACAO GERAL ===\n");
    printf("  {'Comunidade':<25} {'Necessidades':>13} {'Sistemas':>10} {'Respeitos':>10} {'Praticas':>10}");
    printf("  {'-'*70}");
    /* TODO: iterador C manual para c em engine.compare_communities() */
        printf("  {c['comunidade']:<25} {c['necessidades']:>13} ";
            "{c['sistemas_adaptados']:>10} {c['respeitos']:>10} ";
            "{c['praticas_existentes']:>10}");
    // === 2. PERFIL COMPLETO DE CADA COMUNIDADE ===
    /* TODO: iterador C manual para ct em CommunityType */
        printf("\n\n  {'='*70}");
        p = engine.get_profile(ct);
        printf("  {'='*70}");
        printf("  {p['comunidade'].upper()}");
        printf("  {p['descricao'][:80]}...");
        printf("  {'='*70}\n");
        printf("  PILARES CULTURAIS:");
        /* TODO: iterador C manual para pillar em p["pilares_culturais"] */
            printf("    + {pillar}");
        printf("\n  PRATICAS QUE JA EXISTEM (equivalente Republica):");
        /* para cada (practice, equivalent) em p["praticas_existentes"].items(): */
            printf("    {practice:<25} -> {equivalent}");
        printf("\n  NECESSIDADES ({len(p['necessidades'])}):");
        /* TODO: iterador C manual para need em p["necessidades"] */
            printf("    - {need}");
        printf("\n  SISTEMAS ADAPTADOS ({len(p['sistemas_adaptados'])}):");
        /* para cada (system, adaptation) em p["sistemas_adaptados"].items(): */
            printf("\n    {system}:");
            printf("      {adaptation[:100]}...");
        printf("\n  O QUE NUNCA IMPORTAR ({len(p['o_que_nao_importar'])}):");
        /* TODO: iterador C manual para rule em p["o_que_nao_importar"] */
            printf("    X {rule}");
        printf("\n  GOVERNANCA: {p['governanca']}");
        printf("  DECISAO: {p['tomada_decisao']}");
        printf("  LINGUA: {p['lingua_principal']} {p['linguas_secundarias']}");
        printf("  ISOLAMENTO: {p['isolamento']}");
        printf("  ACESSO: {p['acesso']}");
    // === 3. STATS ===
    printf("\n\n  {'='*70}");
    printf("  === ESTATISTICAS ===\n");
    s = engine.stats();
    /* para cada (k, v) em s.items(): */
        printf("  {k:<30} {v}");
    printf("\n{'='*80}");
    printf("  OpenCommunities: {s['comunidades']} comunidades adaptadas.");
    printf("  {s['total_necessidades']} necessidades mapeadas.");
    printf("  {s['total_sistemas_adaptados']} sistemas adaptados.");
    printf("  {s['total_respeitos_culturais']} respeitos culturais.");
    printf("  {s['principio']}");
    printf("{'='*80}");

#endif // OPENCOMMUNITIES_6_ADAPTACOES_COMUNITARIAS_DA_REPUBLICA_H
