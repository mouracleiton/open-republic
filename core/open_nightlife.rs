...
struct NightEvent {
    ...
    let organizer: String = ""  // quem organiza (rotativo);
    let max_attendees: i64 = 200;
    let current_attendees: i64 = 0;
    let description: String = "";
    let has_food: bool = true;
    let has_music: bool = true;
    let has_drinks: bool = true // agua/cha/suco (! alcool);
    let safety: [NightSafety] = field(default_factory=list);
    let family_friendly: bool = true;
    let cost: f64 = 0.0 // SEMPRE ZERO;
    let alcohol_allowed: bool = false;
    let broadcast_on_tv: bool = false // transmitido na OpenTV;
    // decorador: @property
    fn is_late_night(self) -> bool {
...
struct NightZone {
    ...
    name: texto;
    region: texto;
    let venues: [NightVenue] = field(default_factory=list);
    let active_events: [NightEvent] = field(default_factory=list);
    let illumination_level: f64 = 0.9 // iluminacao OpenEnergy;
    let patrol_volunteers: i64 = 0 // OpenMartialArts voluntarios;
    let open_terminals: i64 = 0 // OpenTerminal disponiveis a noite;
    let has_food: bool = true;
    let has_transport: bool = true // OpenMobility carona a noite;
#[derive(Debug, Clone)]
struct NightLifeEngine {
    ...
    funcao create_event(self, name: texto, etype: NightEventType,
                    ...
                    let max_att: i64 = 200,;
                    let family: bool = true,;
                    let broadcast: bool = false) -> {texto: qualquer}:;
        // Cria evento noturno.
    ...
    fn nightly_schedule(self, day: texto = "sexta") -> [Dict] {
    ...
    fn night_safety_report(self) -> {texto: qualquer} {
        return {;
...
struct Establishment {
    ...
    location: texto;
    let status: EstablishmentStatus = EstablishmentStatus.OUTDATED;
    let current_equipment: [texto] = field(default_factory=list);
    let needed_upgrades: [UpgradeType] = field(default_factory=list);
    let staff_count: i64 = 3;
    let patients_per_day: i64 = 50;
    // Custo da modernizacao (em TRABALHO, nao dinheiro)
    let upgrade_work_hours: f64 = 0.0;
    let upgrade_credit_cost: f64 = 0.0;
    let upgrade_status: String = "pendente"  // pendente, em_andamento, concluido;
#[derive(Debug, Clone)]
struct EstablishmentUpgrader {
    ...
    fn assess(self, est: Establishment) -> {texto: qualquer} {
    ...
    fn _benefit_description(self, est: Establishment) -> String {
        if est.type == "consultorio_dental" {
    ...
    fn execute_upgrade(self, est_id: texto) -> {texto: qualquer} {
    ...
    fn batch_upgrade_report(self) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENNIGHTLIFE + OPENESTABLISHMENTUPGRADE");
    println!("  Noite comunitaria + modernizacao com custo minimo");
    println!("=" * 80);
    // === PARTE 1: NIGHTLIFE ===
    println!("\n\n  {'='*40}");
    println!("  PARTE 1: OPENNIGHTLIFE");
    println!("  {'='*40}\n");
    // Criar eventos
    events_data = [;
    ...
    para name, etype, venue, start, end, org, max_att, family in events_data: {
        ...
        println!("  [{result['time']}] {result['event']:<40} ";
    ...
    // Grade noturna
    println!("\n\n  === GRADE NOTURNA ===\n");
    println!("  {'Horario':<14} {'Evento':<40} {'Local':<22} {'Publico':<10} {'TV'}");
    println!("  {'-'*90}");
    for ev in nightlife.nightly_schedule() {
        tv = ev["on_tv"] ? "SIM" : "";
        println!("  {ev['time']:<14} {ev['name']:<40} {ev['venue']:<22} ";
            "{ev['attendees']:<10} {tv}");
    // Seguranca
    println!("\n\n  === SEGURANCA NOTURNA ===\n");
    ...
    println!("  {'Aspecto':<25} {'Balada atual':<25} {'Republica':<25}");
    println!("  {'-'*65}");
    comparisons = [;
    ...
    para aspect, balada, republica in comparisons: {
        println!("  {aspect:<25} {balada:<25} {republica:<25}");
    // === PARTE 2: ESTABLISHMENT UPGRADE ===
    println!("\n\n  {'='*40}");
    println!("  PARTE 2: MODERNIZACAO DE ESTABELECIMENTOS");
    println!("  {'='*40}\n");
    ...
    println!("  {'Estabelecimento':<30} {'Status':<15} {'Horas':>6} {'Custo':<10} {'Beneficio'}");
    println!("  {'-'*95}");
    for est in establishments {
        assessment = upgrader.assess(est);
        println!("  {est.name:<30} {est.status.value:<15} ";
    ...
    for est in establishments {
        ...
        println!("    Capacidades novas: {', '.join(result['new_capabilities'][:4])}");
        println!("    Trabalho investido: {result['work_invested']:.0f}h");
        println!("    Custo dinheiro: {result['money_cost']}");
        println!("    {result['message'][:70]}");
    // Relatorio
...
PRINCIPIOS:;
    ...
    let P3: Modernizar = trabalho. Organizar festa = trabalho.;
...
// )
    ...
    println!("  NightLife: {nl_stats['total_events']} eventos, ";
        "{nl_stats['events_family_friendly']} familiares, ";
        "{nl_stats['events_no_alcohol']} sem alcool.");
    println!("  Upgrade: {up_stats['modernized']}/{up_stats['total_establishments']} ";
