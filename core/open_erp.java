...
public class OpenerpSistemaDeGestaoEmpresarialUniversalDaRepublica {
    ...
    public static class CountryCode {
        ...
        public String code(self) {
            return self.value[0];
        // decorador: @property
        public String name_pt(self) {
            return self.value[1];
        // decorador: @property
        public String currency_name(self) {
            return self.value[2];
        // decorador: @property
        public String currency_code(self) {
            return self.value[3];
        // decorador: @property
        public String locale(self) {
            return self.value[4];
    // decorador: @dataclass
    public static class TaxRule {
        ...
        String calculation_base = "valor_adicionado"  // || "valor_total";
        String who_pays = "consumidor";
        String who_collects = "empresa";
        String filing_frequency = "mensal"  // mensal, trimestral, anual;
    // decorador: @dataclass
    public static class CountryLegislation {
        // Legislacao fiscal/trabalhista de um pais.
        country: CountryCode;
        // Fiscal
        [TaxRule] tax_rules = field(default_factory=list);
        String invoice_format = ""  // NF-&& (BR), Invoice (US), etc;
        boolean invoice_digitization = true // nota fiscal eletronica?;
        boolean einvoicing_mandatory = true;
        String tax_id_name = "CNPJ"  // CNPJ (BR), EIN (US), etc;
        String personal_tax_id = "CPF"  // CPF (BR), SSN (US), etc;
        // Trabalhista
        ...
        // Contabil
        String accounting_standard = ""  // BR GAAP, IFRS, US GAAP;
        String fiscal_year_start = "01/01";
        int currency_decimal_places = 2;
        // Comercio
        double import_tariff_avg = 0.0;
        boolean export_incentive = false;
    ...
    public static class ERPModule {
        ...
        ECOMMERCE = "ecommerce";
    // decorador: @dataclass
    public static class ERPBlock {
        // Bloco LEGO do ERP.
        module: ERPModule;
        name: texto;
        String description = "";
        boolean country_specific = false // adaptado para pais?;
        boolean auto_configured = false // IA configurou automaticamente?;
    // ============================================================================
    // 3. EMPRESA
    // ============================================================================
    // decorador: @dataclass
    public static class Company {
        ...
        country: CountryCode;
        String industry = "";
        String size = "pequena"  // micro, pequena, media, grande;
        String tax_id = "";
        String currency = "";
    ...
    public static class OpenERP {
        ...
        public {ERPModule: ERPBlock} _build_module_catalog(self) {
            return {;
        ...
        funcao register_company(self, name: texto, country: CountryCode,
                            String industry = "", size: texto = "pequena",;
                            String tax_id = "";
                            ) -> {texto: qualquer}:;
        ...
        public [ERPModule] _auto_select_modules(self, industry: texto, size: texto) {
            // IA seleciona modulos baseado em industria + porte.
            base_modules = [;
        ...
        funcao calculate_taxes(self, company_id: texto, amount: flutuante,
            ...
            taxes = [];
            total_tax = 0.0;
            /* TODO: for-each Java para rule em company.legislation.tax_rules */
        ...
        public {texto: qualquer} transition_to_credit(self, company_id: texto) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENERP -- GESTAO EMPRESARIAL UNIVERSAL");
        System.out.println("  Todos os paises. Todas as legislacoes. Automacao maxima.");
        System.out.println("=" * 80);
        // === 1. PAISES SUPORTADOS ===
        System.out.println("\n\n  === 1. PAISES SUPORTADOS ({len(erp.legislations)}) ===\n");
        /* para cada (code, leg) em erp.legislations.items(): */
            System.out.println("  [{code}] {leg.country.name_pt:<20} Moeda: {leg.country.currency_code:<5} ";
        ...
        /* para name, country, industry, size, tax_id in companies: */
            ...
            System.out.println("\n  {r['name']} ({r['country']})");
            System.out.println("    Faturamento: {r['invoice_format']}");
            System.out.println("    Contabilidade: {r['accounting_standard']}");
            System.out.println("    Modulos: {r['modules_auto_configured']}");
        // === 3. CALCULO DE IMPOSTOS POR PAIS ===
        System.out.println("\n\n  === 3. CALCULO DE IMPOSTOS (mesmo valor, paises diferentes) ===\n");
        amount = 1000.0;
        /* TODO: for-each Java para cid em list(erp.companies.keys())[:4] */
            taxes = erp.calculate_taxes(cid, amount);
            System.out.println("\n  {taxes['company']} ({taxes['country']}):");
            System.out.println("    Valor: {taxes['amount']}");
            /* TODO: for-each Java para t em taxes["taxes"][:3] */
                System.out.println("    {t['tax']}: {t['rate']} = {t['amount']}");
            System.out.println("    Total imposto: {taxes['total_tax']}");
            System.out.println("    Total com imposto: {taxes['total_with_tax']}");
        ...
        System.out.println("  Tipo: {invoice['invoice_type']}");
        System.out.println("  Empresa: {invoice['company']}");
        System.out.println("  Cliente: {invoice['client']}");
        System.out.println("  Pais: {invoice['country']}");
        System.out.println("  Total: {invoice['total']}");
        System.out.println("  Eletronica: {invoice['e_invoicing']}");
        // === 5. FOLHA DE PAGAMENTO ===
        System.out.println("\n\n  === 5. FOLHA DE PAGAMENTO (paises diferentes) ===\n");
        /* TODO: for-each Java para cid em list(erp.companies.keys())[:3] */
            payroll = erp.payroll(cid, "Joao", 5000.0);
            System.out.println("  {payroll['country']}: bruto {payroll['gross']} -> liquido {payroll['net']}");
        ...
        /* para cada (module, block) em erp.modules_catalog.items(): */
            System.out.println("  [{module.value}] {block.name}");
            System.out.println("    {block.description[:60]}");
        // === 8. STATS ===
        System.out.println("\n\n  === 8. ESTATISTICAS ===\n");
        s = erp.stats();
        ...
        System.out.println("\n{'='*80}");
        System.out.println("  OpenERP: {s['total_empresas']} empresas, {s['paises_suportados']} paises, ";
