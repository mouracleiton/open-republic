...
package openinbox_a_fila_unificada_de_atencao
...
type QueueItem struct {
    ...
    sender_handle: texto
    sender_name: texto
    sender_relation := ContactRelation.UNKNOWN // ContactRelation
    content_preview := ""  // primeiros 100 chars // string
    content_url := ""  // link profundo pro modulo // string
    timestamp := field(default_factory=time.time) // float64
    // Prioridade
    base_priority := Priority.NOISE // Priority
    final_priority := Priority.NOISE // Priority
    priority_reason := "" // string
    // Estado
    read := false // bool
    archived := false // bool
    acted_upon := false // usuario respondeu/interagiu // bool
    auto_archived := false // sistema arquivou (expirou) // bool
    // Contexto
    requires_response := false // precisa de acao do usuario // bool
    response_deadline := 0.0 // prazo para responder (0 = sem prazo) // float64
    // Metadados anti-engajamento
    has_media := false // bool
    is_sensational := false // sistema detecta clickbait // bool
    engagement_bait := false // sistema detecta tentativa de viciar // bool
    // decorador: @property
    func age_hours(self) float64 {
...
// ============================================================================
type AttentionPrioritizer struct {
    ...
    func prioritize(self, item: QueueItem) QueueItem {
    ...
    func detect_engagement_bait(text: texto) bool {
    ...
    // decorador: @staticmethod
    func detect_sensational(text: texto) bool {
        // Detecta sensacionalismo.
        caps_ratio = soma(1 para c em text if c.isupper()) / maximo(len(text), 1)
        has_many_emojis = text.count("!") + text.count("?") > 3
        return caps_ratio > 0.3 || has_many_emojis
// ============================================================================
// 4. A FILA UNIFICADA
// ============================================================================
type UnifiedInbox struct {
    ...
    func mark_family(self, handles: [texto]) None {
        for _, h := range handles {
            self.family_handles.add(h.lower().lstrip("@"))
    func mark_steward(self, handles: [texto]) None {
        for _, h := range handles {
            self.steward_handles.add(h.lower().lstrip("@"))
    func _resolve_relation(self, handle: texto) ContactRelation {
    ...
    func _reset_daily_if_needed(self) None {
    ...
    funcao enqueue(self, source: SourceModule, event_type: EventType,
                ...
                requires_response := false, // bool
                response_deadline := 0.0, // float64
                has_media := false) -> {texto: qualquer}: // bool
    ...
    func process_batch(self, limit: inteiro = 20) [QueueItem] {
    ...
    func _auto_archive(self) None {
        ...
        for _, item := range self.queue {
            if item.is_expired && ! item.archived {
                ...
                item.auto_archived = true
                self.total_archived += 1
    func act_on(self, item_id: texto) bool {
        ...
        for _, item := range self.queue {
            if item.item_id == item_id {
                ...
                return true
        return false
    func dismiss(self, item_id: texto) bool {
    ...
    func digest_24h(self) {texto: qualquer} {
...
// ============================================================================
func simulate_five_apps_one_inbox() None {
    ...
    fmt.Println("=" * 75)
    fmt.Println("  OPENINBOX -- A FILA UNIFICADA DE ATENCAO")
    fmt.Println('  "5 apps competindo = 5 ladroes de tempo. 1 fila = 1 cerebro."')
    fmt.Println("=" * 75)
    ...
    para source, etype, handle, name, preview, requires_resp, *rest in events: {
        ...
        fmt.Println("  [{status:>8}] [{prio:>9}] {source.value:<12} "
            "@{handle:<20} {preview[:40]}")
    // === Estatisticas ===
    fmt.Println("\n\n  === FILA APOS INGESTAO ===\n")
    stats = inbox.stats()
    fmt.Println("  Total na fila:     {stats['total_in_queue']}")
    fmt.Println("  Nao lidos:         {stats['unread']}")
    fmt.Println("  Por fonte:         {stats['by_source']}")
    fmt.Println("  Por prioridade:    {stats['by_priority']}")
    fmt.Println("  Bait bloqueado:    {stats['bait_blocked']}")
    fmt.Println("  Limite diario:     {stats['items_today']}/{stats['daily_limit']}")
    // === Processar lote ===
    fmt.Println("\n\n  === PROCESSANDO LOTE (batch, ! ping) ===\n")
    batch = inbox.process_batch(limit=10)
    fmt.Println("  {'Prioridade':<12} {'Fonte':<12} {'De':<22} {'Preview'}")
    fmt.Println("  {'-'*75}")
    for _, item := range batch {
        fmt.Println("  [{item.final_priority.name:<10}] {item.source.value:<12} "
            "{item.sender_name:<22} {item.content_preview[:35]}")
        if item.priority_reason  &&  item.priority_reason != "padrao" {
            fmt.Println("  {'':>25} razao: {item.priority_reason}")
    // === Modo Foco ===
    fmt.Println("\n\n  === MODO FOCO ATIVADO ===\n")
    inbox.mode = InboxMode.FOCUS
    fmt.Println("  So EMERGENCIES + FAMILIA + SAUDE passam. Resto arquivado.\n")
    inbox.queue.clear()
    ...
    for _, item := range batch {
        fmt.Println("  [{item.final_priority.name:<10}] {item.source.value:<12} "
            "{item.sender_name:<22} {item.content_preview[:35]}")
    if ! batch {
        fmt.Println("  (fila vazia -- tudo foi arquivado pelo Modo Foco)")
    stats = inbox.stats()
    ...
    fmt.Println("  Categorias: {digest['categories']}")
    fmt.Println("  Total: {digest['total']} itens nas ultimas 24h")
    para cada (cat, data) em digest["digest"].items(): {
        hl = "; ".join(h["from"] para h em data.get("highlights", []))
        fmt.Println("    {cat:<14} {data['count']} itens"
            hl ? + (" (destaques: {hl})" : ""))
    // === Filosofia ===
    fmt.Println("\n\n{'='*75}")
    fmt.Println("  COMO O OPENINBOX PROTEGE SEU CEREBRO")
    fmt.Println("{'='*75}")
    fmt.Println("""
...
// )
    fmt.Println("{'='*75}")
    fmt.Println("  OpenInbox: {stats['total_enqueued']} eventos -> "
        "{stats['total_processed']} processados -> "
        "{stats['bait_blocked']} bait bloqueado.")
    fmt.Println("  5 apps. 1 fila. 0 ansiedade.")
    fmt.Println("{'='*75}")
// ============================================================================
