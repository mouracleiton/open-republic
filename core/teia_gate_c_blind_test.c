...
#ifndef GATE_C_QUALIDADE_DO_ARTEFATO_PAINEL_CEGO_H
#define GATE_C_QUALIDADE_DO_ARTEFATO_PAINEL_CEGO_H
...
// decorador: @dataclass
typedef struct BlindEvaluation {
...
// ============================================================================
char* print_evaluation_template(void) {
    ...
    lines.append("");
    /* TODO: iterador C manual para artefato em ARTEFATOS_PARA_TESTE */
        lines.append("-" * 80);
...
// ============================================================================
char* analyze_results(evaluations: [BlindEvaluation], identity_map: {texto: texto}) {
    ...
    /* TODO: iterador C manual para tema em ordene(temas) */
        lines.append("-" * 110);
        ...
        lines.append("  " + "-" * 85);
        /* TODO: iterador C manual para e em a_evals + b_evals */
        ...
        // Médias
        a_avg = a_evals ? soma(&&.score_total para && em a_evals) / sizeof(a_evals) : 0;
        b_avg = b_evals ? soma(&&.score_total para && em b_evals) / sizeof(b_evals) : 0;
    ...
    teia_avg = teia_evals ? soma(&&.score_total para && em teia_evals) / sizeof(teia_evals) : 0;
    mercado_avg = mercado_evals ? soma(&&.score_total para && em mercado_evals) / sizeof(mercado_evals) : 0;
    ...
    /* para crit, attr in [("Acurácia", "acuracia"), ("Clareza", "clareza"), */
        ...
        t_avg = teia_evals ? soma(getattr(&&, attr) para && em teia_evals) / sizeof(teia_evals) : 0;
        m_avg = mercado_evals ? soma(getattr(&&, attr) para && em mercado_evals) / sizeof(mercado_evals) : 0;
        diff = t_avg - m_avg;
...
if (__name__ == "__main__") {
    printf("=" * 110);
    printf("GATE C: QUALIDADE DO ARTEFATO — PAINEL CEGO");
    printf("=" * 110);
    printf();
    // Mostrar template
    printf("TEMPLATE DE AVALIAÇÃO (para impressão):");
    printf();
    // print(print_evaluation_template())  // Uncomment para imprimir
    printf("  [Template disponível no arquivo]");
    printf();
    // Rodar simulação
    printf();
    printf(demo_simulation());
    printf();
    printf("=" * 110);
    printf("COMO EXECUTAR O TESTE REAL:");
    printf("=" * 110);
    printf(""";
1. GERAR ARTEFATOS:;
...
#endif // GATE_C_QUALIDADE_DO_ARTEFATO_PAINEL_CEGO_H
