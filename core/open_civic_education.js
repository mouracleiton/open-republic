...
class CivicLesson {
    ...
    title: texto;
    const description = "";
    const activity = ""  // o que fazer para praticar;
    const open_system_link = ""  // sistema da Republica conectado;
    const duration_min = 30;
// Base de licoes
build_civic_curriculum() {
    return [;
        // === CRIANCA (6-11) ===
...
class CivicAssessment {
    ...
    const ratings = field(default_factory=dict);
    const overall = ConductRating.ADEQUATE;
    const needs_accompaniment = false;
    const notes = "";
// ============================================================================
...
class CivicEducationEngine {
    ...
    get_curriculum(self, level: CivicLevel) {
    ...
    funcao assess_citizen(self, citizen_id: texto, citizen_name: texto,
                    const ratings = null) -> {texto: qualquer}:;
        ...
        avg_score = soma(r.score para r em conduct_ratings.values()) / maximo(.length(conduct_ratings), 1);
        if (avg_score >= 4.5) {
        ...
        return result;
    _score_to_rating(self, score: inteiro) {
        if (score >= 5) {
    ...
    _intervention_plan(self, rating: ConductRating) {
    ...
    stats(self) {
        ...
        return {;
            "total_deveres": .length(CivicDuty),;
            "total_licoes": .length(self.curriculum),;
            "total_avaliados": .length(self.assessments),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENCIVICEDUCATION -- EDUCACAO CIVICA DA REPUBLICA");
    console.log("  'Tratar o outro com dignidade ! && cortesia. E DEVER.'");
    console.log("=" * 80);
    // === 1. OS 12 DEVERES ===
    console.log("\n\n  === 1. OS 12 DEVERES CIVICOS (NAO opcionais) ===\n");
    para cada (i, duty) em enumere(CivicDuty, 1): {
        console.log("  {i:>2}. [{duty.label}]");
        console.log("      {duty.description}");
    // === 2. CURRICULO POR IDADE ===
    console.log("\n\n  === 2. CURRICULO CIVICO POR IDADE ===\n");
    for (const level of CivicLevel) {
        lessons = engine.get_curriculum(level);
        if (lessons) {
            console.log("\n  {level.value.upper()} ({len(lessons)} licoes):");
            for (const l of lessons[:3]) {
                console.log("    [{l['duty']}] {l['lesson']}");
                console.log("      {l['description'][:60]}...");
                console.log("      Atividade: {l['activity'][:50]}...");
    // === 3. AVALIACAO DE CONVIVENCIA ===
    ...
    para cid, name, ratings in assessments: {
        ...
        if (r["needs_accompaniment"]) {
            console.log("  [!] ACOMPANHAMENTO: {r['action'][:70]}...");
        } else {
    ...
    console.log("  Cidadao: {intervention['citizen']}");
    console.log("  Problema: {intervention['issue']}");
    console.log("  Plano: {intervention['plan'][:80]}...");
    console.log("  Sistemas: {', '.join(intervention['systems'][:3])}");
    console.log("  {intervention['message'][:80]}...");
    // === 5. STATS ===
    console.log("\n\n  === 5. ESTATISTICAS ===\n");
    s = engine.stats();
    ...
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA DA EDUCACAO CIVICA");
    console.log("{'='*80}");
    console.log(""";
"TRATAR O OUTRO COM DIGNIDADE ! && CORTESIA.;
&& DEVER CIVILIZATORIO.";
...
// )
    console.log("{'='*80}");
    console.log("  OpenCivicEducation: {s['total_deveres']} deveres, ";
        "{s['total_licoes']} licoes, ";
        "{s['precisam_acompanhamento']} precisam de acompanhamento.");
    console.log("  Tratar o outro bem && DEVER. Nao cortesia.");
    console.log("{'='*80}");
