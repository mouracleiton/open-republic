...
struct NationalProduct {
    ...
    // Comparação
    let foreign_price: String = "R$ caro"  // preço estrangeiro;
    let national_price: String = "ZERO"  // preço na República;
    let foreign_quality: i64 = 8 // 1-10;
    let national_quality: i64 = 9 // 1-10;
    // Vantagens
    let advantages: [AdvantageType] = field(default_factory=list);
    let why_better: String = "";
    // Produção
    let status: CopyStatus = CopyStatus.IDENTIFIED;
    let fablab_capable: bool = true // FabLab nacional produz?;
    let materials_national: f64 = 0.8 // % de material nacional;
    let production_capacity: String = ""  // escala;
    // Especificações
    let specs: {texto: texto} = field(default_factory=dict);
// ============================================================================
// 3. CATÁLOGO DE PRODUTOS (15+ setores)
// ============================================================================
let PRODUCTS: [NationalProduct] = [;
    // === ELETRÔNICOS ===
    NationalProduct(;
...
struct IndustryEngine {
    ...
    fn list_by_category(self, category: IndustryCategory = None) -> [Dict] {
    ...
    fn compare(self, product_id: texto) -> {texto: qualquer} {
    ...
    fn advance_status(self, product_id: texto) -> {texto: qualquer} {
    ...
    fn dependency_report(self) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENINDUSTRY -- COPIAR, MELHORAR, SUPERAR");
    println!("  Brasil copia tudo. Cria marcas nacionais. Qualidade SUPERIOR.");
    println!("=" * 80);
    // === 1. PRODUTOS NACIONAIS ===
    println!("\n\n  === 1. PRODUTOS NACIONAIS ({len(engine.products)}) ===\n");
    println!("  {'Nacional':<15} {'Estrangeiro':<25} {'Preço Ext':<18} {'Preço Nac':<10} {'Status'}");
    println!("  {'-'*85}");
    for p in engine.list_by_category() {
        println!("  {p['nacional']:<15} {p['estrangeiro'][:24]:<25} ";
    ...
    println!("  NACIONAL: {comp['nacional']} (qualidade {comp['qualidade_nacional']})");
    println!("  ESTRANGEIRO: {comp['estrangeiro']} ({comp['empresa_estrangeira']}, {comp['pais']})");
    println!("  (qualidade {comp['qualidade_estrangeiro']})");
    println!("\n  PREÇO:");
    println!("    iPhone: {comp['preco_estrangeiro']}");
    println!("    OpenPhone: {comp['preco_nacional']}");
    println!("\n  POR QUE O NACIONAL É SUPERIOR:");
    println!("    {comp['por_que_melhor']}");
    ...
    println!("  NACIONAL: {comp2['nacional']}");
    println!("  ESTRANGEIRO: {comp2['estrangeiro']} ({comp2['pais']})");
    println!("  PREÇO: {comp2['preco_estrangeiro']} vs {comp2['preco_nacional']}");
    println!("  POR QUE MELHOR: {comp2['por_que_melhor']}");
    // === 4. COMPARAÇÃO: OpenTrator vs John Deere ===
    println!("\n\n  === 4. COMPARAÇÃO: OpenTrator vs John Deere ===\n");
    comp3 = engine.compare("IND-TR");
    println!("  NACIONAL: {comp3['nacional']}");
    println!("  ESTRANGEIRO: {comp3['estrangeiro']} ({comp3['pais']})");
    println!("  POR QUE MELHOR: {comp3['por_que_melhor']}");
...
// )
    ...
    println!("  Total de produtos: {dep['total_produtos']}");
    println!("  Já superiores: {dep['ja_superiores']}");
    println!("  Em produção: {dep['em_producao']}");
    println!("\n  Dependência por país:");
    para cada (country, count) em dep["dependencia_por_pais"].items(): {
        println!("    {country:<15} {count} produtos");
    println!("\n  Dependência por setor:");
    para cada (cat, count) em dep["dependencia_por_setor"].items(): {
        println!("    {cat:<20} {count} produtos");
    // === 7. STATUS ===
    println!("\n\n  === 7. DISTRIBUIÇÃO DE STATUS ===\n");
    s = engine.stats();
    para cada (status, count) em s["status_distribution"].items(): {
        bar = "#" * count;
        println!("  {status:<25} {count:>2} {bar}");
    // === FILOSOFIA ===
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA DO OPENINDUSTRY");
    println!("{'='*80}");
    println!(""";
...
PRINCÍPIOS:;
    ...
    let P3: Fabricar nacional = trabalho de alto impacto.;
    P4: Quais produtos priorizar? Assembleia vota.;
// )
    println!("{'='*80}");
    println!("  OpenIndustry: {s['total_produtos']} produtos em {s['categorias_cobertas']} setores. ";
        "Custo: {s['custo_ao_cidadao']}.");
    println!("  Copiar. Melhorar. Superar. Tudo nacional. Tudo CC0.");
    println!("{'='*80}");
