...
class MergeProposal {
    ...
    const description = "";
    const files_changed = field(default_factory=list);
    const lines_added = 0;
    const lines_removed = 0;
    const status = MergeStatus.DRAFT;
    const votes_for = 0;
    const votes_against = 0;
    const total_voters = 10000;
    const quorum_needed = 0.51 // 51% para aprovar;
    // Validacao automatica
    const passes_constitution = true;
    const fails_reason = "";
    const tested = false // rodou sem erro?;
    const test_result = "";
    const created_date = "";
    const merged_date = "";
class RepoEngine {
    ...
    funcao propose(self, author: texto, title: texto,
                const description = "",;
                const files = null,;
                const added = 0, removed: inteiro = 0,;
                const passes_constitution = true,;
                const fails_reason = "",;
                const tested = true) -> {texto: qualquer}:;
    ...
    _merge(self, proposal: MergeProposal) {
    ...
    reject_fork(self, citizen: texto, reason: texto) {
    ...
    repo_status(self) {
        return {;
            ...
            **self.repo_stats,;
            "propostas_totais": .length(self.proposals),;
...
class VerifiedSkill {
    ...
    const level = SkillLevel.NONE;
    const proofs = field(default_factory=list);
    // Comprovacoes
    const system_tests_passed = 0;
    const tasks_completed = 0;
    const courses_certified = 0;
    const peer_verifications = 0;
    const contributions = 0;
    const mentor_endorsements = 0;
    const demonstrations = 0;
    // Score de confianca
    const confidence = 0.0 // 0-1 (quao confiavel && a skill);
    // decorador: @property
    is_verified(self) {
        return .length(self.proofs) > 0 && self.confidence > 0.3;
// decorador: @dataclass
class SkillProfile {
    ...
    const age = 0;
    const profession = "";
    const profession_level = ""  // OpenProfessions (aprendiz -> mestre);
    const skills = field(default_factory=dict);
    const created_date = "";
    // Stats
    const total_skills_verified = 0;
    const total_tasks_completed = 0;
    const total_contributions = 0;
    const impact_score = 0.0 // OpenCredit/OpenCreator;
    // decorador: @property
    skill_count(self) {
        return .length(self.skills);
    // decorador: @property
    verified_count(self) {
...
class SkillsEngine {
    ...
    funcao add_proof(self, citizen_id: texto, skill_name: texto,
                ...
                const proof_detail = "",;
                const level = SkillLevel.BASIC) -> {texto: qualquer}:;
        ...
        total_proofs = .length(skill.proofs);
        ...
        skill.level = levels[minimo(total_proofs, .length(levels) - 1)];
        // Confianca
        weights = {
    ...
    get_profile(self, citizen_id: texto) {
    ...
    funcao search_by_skill(self, skill_name: texto,
                        const min_level = 2) -> [Dict]:;
        // Busca cidadaos com skill (para OpenLaborRelay).
        results = [];
        for (const profile of self.profiles.values()) {
    ...
    stats(self) {
        ...
        return {;
            "total_perfis": .length(self.profiles),;
            "total_skills_registradas": .length(all_skills),;
            "skills_unicas": .length(set(all_skills)),;
...
if (__name__ == "__main__") {
    ...
    skills = SkillsEngine();
    console.log("=" * 80);
    console.log("  OPENREPOPOLICY + OPENSKILLS");
    console.log("  Repositorio unico + Skills comprovadas pelo sistema");
    console.log("=" * 80);
    // === PARTE 1: REPOSITORIO ===
    console.log("\n\n  {'='*40}");
    console.log("  PARTE 1: OPENREPOPOLICY");
    console.log("  {'='*40}\n");
    ...
    // Pull
    console.log("  === PULL (pegar do main) ===\n");
    p = repo.pull("cleiton");
    ...
    console.log("  {fork['citizen']}: {fork['action']}");
    console.log("  Politica: {fork['policy']}");
    console.log("  Alternativa: {fork['alternative']}");
    // Status repositorio
    console.log("\n\n  === STATUS DO REPOSITORIO ===\n");
    ...
    // === PARTE 2: OPENSKILLS ===
    console.log("\n\n  {'='*40}");
    console.log("  PARTE 2: OPENSKILLS");
    console.log("  {'='*40}\n");
    // Criar perfis
    ...
    para skill, cat, proof, detail, level in proofs_cleiton: {
        ...
        console.log("  {r['citizen']}: {r['skill']} -> {r['level']} ";
    ...
    for (const cid of ["C-001", "C-002", "C-003", "C-004"]) {
        profile = skills.get_profile(cid);
        console.log("\n  {profile['citizen']} ({profile['profession']})");
        console.log("  Skills verificadas: {profile['verified_skills']}/{profile['total_skills']}");
        for (const s of profile["skills"][:3]) {
            console.log("    [{s['level']:<13}] {s['skill']:<25} ";
                "conf: {s['confidence']} provas: {s['proofs']}");
            for (const d of s["proofs_detail"][:1]) {
                console.log("      -> {d[:60]}");
    // === BUSCAR POR SKILL ===
    console.log("\n\n  === BUSCAR: quem sabe Rust? ===\n");
    rust_devs = skills.search_by_skill("programacao_rust", min_level=2);
    for (const dev of rust_devs) {
        console.log("  {dev['citizen']:<15} {dev['level']:<13} ";
            "conf: {dev['confidence']} provas: {dev['proofs']}");
    console.log("\n  === BUSCAR: quem sabe alvenaria? ===\n");
    builders = skills.search_by_skill("alvenaria", min_level=2);
    for (const b of builders) {
        console.log("  {b['citizen']:<15} {b['level']:<13} ";
            "conf: {b['confidence']} provas: {b['proofs']}");
    // === COMPARACAO CV vs OPENSKILLS ===
    console.log("\n\n  === CV TRADICIONAL vs OPENSKILLS ===\n");
    comp = skills.compare_to_cv();
    console.log("  {'Aspecto':<15} {'CV Tradicional':<30} {'OpenSkills'}");
    console.log("  {'-'*75}");
    for (const c of comp) {
        console.log("  {c['aspecto']:<15} {c['cv_tradicional'][:29]:<30} {c['openskills'][:35]}");
    // === STATS ===
    console.log("\n\n  === ESTATISTICAS ===\n");
    s = skills.stats();
    ...
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA: REPOSITORIO UNICO + SKILLS COMPROVADAS");
    console.log("{'='*80}");
    console.log(""";
...
// )
    console.log("{'='*80}");
    rs = repo.repo_status();
    console.log("  Repo: {rs['repositorio']} | {rs['propostas_integradas']} integradas | ";
        "{rs['forks_rejeitados']} forks rejeitados.");
    console.log("  Skills: {s['total_perfis']} perfis, {s['skills_unicas']} skills unicas.");
    console.log("  Sem fork. Sem CV. So Republica.");
    console.log("{'='*80}");
