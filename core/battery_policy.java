...
public class OpenrepublicPoliticaFimDasBateriasDescartaveis {
    ...
    public static class BatterySpec {
        ...
        int capacity_mah = 0;
        int cycles = 0 // ciclos de carga ate 80% capacidade;
        double lifespan_years = 0;
        boolean repairable = false // pode trocar celula individual?;
        double recyclable_pct = 0 // % recuperavel;
        [texto] toxic_materials = field(default_factory=list);
        double co2_manufacture_kg = 0;
        int cost_recovery_credits = 0 // creditos para recuperar na reciclagem;
        String notes = "";
    {BatteryType: BatterySpec} BATTERY_DATABASE = {;
        // === PROIBIDAS ===
    ...
    public static class Product {
        ...
        int battery_count = 1;
        boolean battery_replaceable = false;
        boolean glued = false;
        double expected_life_years = 0;
    public static class BatteryPolicy {
        ...
        public {texto: qualquer} check_product(self, product: Product) {
        ...
        public {texto: qualquer} recommend_replacement(self, prohibited_type: BatteryType) {
        ...
        public {texto: qualquer} impact_report(self, population: inteiro = 1_000_000) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENREPUBLIC -- FIM DAS BATERIAS DESCARTAVEIS");
        System.out.println("  'Pilha no lixo = 100 anos de veneno.'");
        System.out.println("  'Bateria reparavel = 15 anos de uso.'");
        System.out.println("=" * 75);
        policy = BatteryPolicy();
        // === 1. Battery Database ===
        System.out.println("\n\n  === BANCO DE BATERIAS ===\n");
        System.out.println("  {'Tipo':<20} {'Status':<14} {'Vida':>5} {'Ciclos':>7} {'Recicl.':>8} {'Toxico'}");
        System.out.println("  {'-'*75}");
        /* para cada (bt, spec) em BATTERY_DATABASE.items(): */
            toxic = spec.toxic_materials ? "SIM" : "!";
            System.out.println("  {bt.value:<20} {spec.status.value:<14} ";
        ...
        /* TODO: for-each Java para p em products */
            result = policy.check_product(p);
            verdict = result["verdict"];
            ok = result["allowed"] ? "OK" : "BLOQUEADO";
            System.out.println("\n  {p.name}:");
            System.out.println("    Bateria: {p.battery_type.value} x{p.battery_count}");
            System.out.println("    Substituivel: {'sim' if p.battery_replaceable else 'NAO'} | ";
                "Colada: {'sim' if p.glued else '!'}");
            System.out.println("    Veredito: {ok} ({verdict})");
            System.out.println("    Razao: {result['reason'][:80]}");
        ...
        /* TODO: for-each Java para pt em prohibited_types */
            rec = policy.recommend_replacement(pt);
            System.out.println("\n  {rec['prohibited']:<20} -> {rec['replacement']}");
            System.out.println("    Como: {rec['how']}");
            System.out.println("    Ganho: {rec['lifespan_gain']}");
            System.out.println("    Reciclavel: {rec['recycle_gain']}");
        // === 4. Impact Report ===
        System.out.println("\n\n  === IMPACTO AMBIENTAL ===\n");
        /* TODO: for-each Java para pop em [10_000, 100_000, 1_000_000] */
            impact = policy.impact_report(pop);
            System.out.println("\n  Populacao: {pop:,}");
            System.out.println("    {impact['comparacao']}");
            System.out.println("    CO2 evitado: {impact['co2_evitado_ton']:,} toneladas/ano");
            System.out.println("    Material toxico evitado: {impact['material_toxico_evitado_ton']} ton/ano");
        // === Philosophy ===
        System.out.println("\n\n{'='*75}");
