...
#ifndef OPENDESCRIMINALIZE_DESCRIMINALIZACAO_POPULAR_VIA_ASSEMBLEIA_H
#define OPENDESCRIMINALIZE_DESCRIMINALIZACAO_POPULAR_VIA_ASSEMBLEIA_H
...
// ============================================================================
typedef struct SubstanceCategory {
    ...
    ALCOHOL = "alcool";
    TOBACCO = "tabaco";
typedef struct DecriminalizationStatus {
...
typedef struct Substance {
    ...
    // Regras
    int min_age = 18;
    bool public_use = false // pode usar em publico?;
    bool driving_after = false // pode dirigir apos uso?;
    bool production_allowed = false // pode cultivar/produzir?;
    bool commerce_allowed = false // pode vender?;
    // Razao da politica
    char* rationale = "";
...
// ============================================================================
{texto: qualquer} run_decrim_assembly(n_voters: inteiro = 10000) {
    ...
    substance_votes = {};
    /* TODO: iterador C manual para sub em SUBSTANCES */
...
typedef struct DecrimEngine {
    ...
    funcao check_arrest_type(self, arrest_reason: texto,
                        ...
                        bool is_trafficking = false) -> {texto: qualquer}:;
    ...
    {texto: qualquer} prison_impact_report(self) {
    ...
    {texto: qualquer} substance_policy(self, substance_id: texto) {
    ...
    {texto: qualquer} stats(self) {
        return {;
            "total_substancias": sizeof(self.substances),;
...
if (__name__ == "__main__") {
    engine = DecrimEngine();
    printf("=" * 80);
    printf("  OPENDESCRIMINALIZE -- ASSEMBLEIA CONSTITUINTE");
    printf("  'Usuario && PACIENTE. Nao criminoso. Nunca mais.'");
    printf("=" * 80);
    // === 1. PERGUNTA A ASSEMBLEIA ===
    printf("\n\n  === 1. PERGUNTA A ASSEMBLEIA ===\n");
    result = run_decrim_assembly(10000);
    printf("  {result['question']}");
    printf("\n  PACIENTE (tratamento):    {result['votes_patient']:>6} ";
        "({result['pct_patient']})");
    printf("  REGULAR (taxar):          {result['votes_regulate']:>6} ";
        "({result['votes_regulate']/100:.0f}%)");
    printf("  CRIMINOSO (prisao):       {result['votes_criminal']:>6} ";
    ...
    /* para reason, qty, trafficking in cases: */
        ...
        action = check["action"];
        printf("  [{check['type']:<11}] {reason:<45} -> {action}");
    ...
    /* para cada (person, sub_id) em treatments: */
        ...
        printf("  {t['person']:<12} {t['substance']:<20} -> {t['treatment_type']}");
        printf("    Voluntario: {t['voluntary']}. P2: {t['p2']}");
    // === 7. IMPACTO PRISIONAL ===
    ...
    // === FILOSOFIA ===
    printf("\n\n{'='*80}");
    printf("  FILOSOFIA DO OPENDESCRIMINALIZE");
    printf("{'='*80}");
    printf(""";
...
// )
    printf("{'='*80}");
    printf("  OpenDescriminalize: {s['presos_liberados']} presos liberados, ";
        "{s['usuarios_tratados']} usuarios em tratamento.");
    printf("  Usuario && paciente. Nao criminoso. Nunca mais.");
    printf("{'='*80}");

#endif // OPENDESCRIMINALIZE_DESCRIMINALIZACAO_POPULAR_VIA_ASSEMBLEIA_H
