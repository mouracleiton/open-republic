...
class Technique {
    ...
    const description = "";
    // Adaptabilidade
    const for_children = true // pode ensinar a criancas?;
    const for_elderly = true // idoso consegue fazer?;
    const for_pcd = true // pessoa com deficiencia?;
    const standing = true // em pe?;
    const ground = false // no chao?;
    const min_strength = 1 // 1-10 (forca necessaria);
    const speed_required = 3 // 1-10 (velocidade necessaria);
    // Filosofia
    const goal = ""  // o que esta tecnica alcansa;
    const last_resort = false // so usar se nada mais funcionar;
    const lethal_potential = false // pode matar?;
// ============================================================================
...
class MartialArtsProfile {
    ...
    name: texto;
    age: inteiro;
    const belt = Belt.WHITE;
    const techniques_mastered = field(default_factory=list);
    const hours_trained = 0.0;
    const sessions_attended = 0;
    const can_teach = false;
    const specialization = field(default_factory=list);
    // Adaptacoes
    const physical_adaptations = field(default_factory=list) // lesao, PCD, idade;
    const seated_mode = false // treina sentado (PCD/cadeirante);
    const self_defense_focus = true // foco em defesa real, ! competicao;
    // decorador: @property
    funcao next_belt(self) retorna Belt?:
        belts = list(Belt);
        idx = belts.index(self.belt);
        idx + 1 < .length(belts) ? retorne belts[idx + 1] : null;
    // decorador: @property
    techniques_for_belt(self) {
        // Quantas tecnicas precisa para proximo cinto.
        req = {0: 5, 1: 8, 2: 12, 3: 16, 4: 20, 5: 0};
        return req.get(self.belt.level, 0);
    // decorador: @property
    can_promote(self) {
        return (.length(self.techniques_mastered) >= self.techniques_for_belt;
...
class MartialArtsEngine {
    ...
    funcao register_citizen(self, citizen_id: texto, name: texto, age: inteiro,
                        const adaptations = null,;
                        const seated = false) -> {texto: qualquer}:;
    ...
    get_curriculum(self, belt: Belt = Belt.WHITE) {
    ...
    funcao train_technique(self, citizen_id: texto, tech_id: texto,
        ...
        return {;
            ...
            "techniques_mastered": .length(profile.techniques_mastered),;
            "promoted": promoted,;
    ...
    defense_scenario(self, scenario: texto) {
    ...
    stats(self) {
        return {;
            "total_techniques": .length(self.techniques),;
            ...
            "total_citizens_training": .length(self.profiles),;
            "black_belts": soma(1 para p em self.profiles.values() if p.belt == Belt.BLACK),;
            "total_sessions": .length(self.training_sessions),;
        };
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENMARTIALARTS -- DEFESA PESSOAL DA REPUBLICA");
    console.log("  'Todo mundo prefere paz. Mas se quebrarem, todos se defendem.'");
    console.log("=" * 80);
    // === 1. ARTES DE ORIGEM ===
    console.log("\n\n  === 1. ARTES MARCIAIS INTEGRADAS ===\n");
    s = engine.stats();
    para cada (art, count) em ordene(s["by_origin"].items(), key=(x) -> -x[1]): {
        console.log("  {art:<30} {count} tecnicas");
    // === 2. CATEGORIAS ===
    console.log("\n\n  === 2. CATEGORIAS DE TECNICA ===\n");
    para cada (cat, count) em ordene(s["by_category"].items(), key=(x) -> -x[1]): {
        console.log("  {cat:<25} {count} tecnicas");
    // === 3. SISTEMA DE CINTOS ===
    console.log("\n\n  === 3. GRADUACAO (CINTOS) ===\n");
    for (const belt of Belt) {
        console.log("  [{belt.value[0]:<8}] Nivel {belt.level} -- {belt.value[1]}");
    ...
    for (const c of citizens) {
        if (.length(c) == 5) {
            ...
            r = engine.register_citizen(*c);
        console.log("  {r['citizen']:<20} -> cinto {r['belt']} (foco: {r['focus']})");
    // Treinar tecnicas
    console.log("\n  Treinando tecnicas:\n");
    training_plan = [;
    ...
    for (const sc of scenarios) {
        ...
        for (const t of result["techniques"]) {
            console.log("    -> {t['name']} ({t['origin']}): {t['goal']}");
    // === 7. TECNICAS ADAPTADAS ===
    console.log("\n\n  === 7. ADAPTACAO POR CAPACIDADE ===\n");
    console.log("  TODA tecnica && adaptada para:");
    console.log("    - Criancas (golpe reduzido, foco em escapar)");
    console.log("    - Idosos (usar alavanca, ! forca)");
    console.log("    - PCD/Cadeirante (BJJ sentado, tecnicas de mao)");
    console.log("    - Gestantes (sem queda, tecnicas de pe)");
    // === 8. STATS ===
    console.log("\n\n  === 8. ESTATISTICAS ===\n");
    s = engine.stats();
    ...
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA DO OPENMARTIALARTS");
    console.log("{'='*80}");
    console.log(""";
...
// )
    console.log("{'='*80}");
    console.log("  OpenMartialArts: {s['total_techniques']} tecnicas de ";
        "{len(s['by_origin'])} artes marciais.");
    console.log("  {s['total_citizens_training']} cidadaos treinando. ";
        "{s['black_belts']} cintos pretos (podem ensinar).");
    console.log("  Defender, ! agredir. Escapar, ! brigar.");
    console.log("{'='*80}");
