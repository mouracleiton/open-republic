...
**Arquivo original:** `open-republic/core/open_big_linux.py`
...
classe CamadaDistro herda de Enum:
...
classe TipoFerramentaSeguranca herda de Enum:
...
classe TipoFerramentaAcessibilidade herda de Enum:
...
classe NivelHardening herda de Enum:
...
classe StatusPacote herda de Enum:
...
classe PacoteDistro:
    ...
    camada: CamadaDistro
    descricao: str
    declare status: StatusPacote  <- StatusPacote.DISPONIVEL
    declare comando_instalacao: str  <- ""  // "apt install orca"
    declare dependencias: List[str]  <- field(default_factory=list)
    declare tamanho_mb: float  <- 0.0


// decorador: @dataclass
classe FerramentaSeguranca:
    // Uma ferramenta de seguranca do Kali.
    nome: str
    tipo: TipoFerramentaSeguranca
    descricao: str
    declare uso_defensivo: str  <- ""  // como a Republica USA para defender
    declare uso_educativo: str  <- ""  // o que o cidadao APRENDE com ela
    declare comando: str  <- ""  // comando base
...
classe PerfilDistro:
    ...
    declare pacotes_incluidos: List[str]  <- field(default_factory=list)
    declare acessibilidade_ativa: bool  <- VERDADEIRO
    declare ia_local_ativa: bool  <- VERDADEIRO
    declare seguranca_ativa: bool  <- VERDADEIRO
    declare publico_alvo: str  <- ""
...
funcao _init_ferramentas_seguranca() retorna List[FerramentaSeguranca]:
    retorne [
...
funcao _init_pacotes_acessibilidade() retorna List[PacoteDistro]:
    retorne [
...
funcao _init_pacotes_republica() retorna List[PacoteDistro]:
    retorne [
...
funcao _init_perfis() retorna List[PerfilDistro]:
    retorne [
        PerfilDistro(
            ...
            acessibilidade_ativa <- VERDADEIRO, ia_local_ativa=VERDADEIRO, seguranca_ativa=VERDADEIRO,
        ...
        PerfilDistro(
            ...
            acessibilidade_ativa <- VERDADEIRO, ia_local_ativa=VERDADEIRO, seguranca_ativa=FALSO,
        ...
        PerfilDistro(
            ...
            acessibilidade_ativa <- VERDADEIRO, ia_local_ativa=FALSO, seguranca_ativa=VERDADEIRO,
...
classe BigLinuxEngine:
    ...
    funcao todas_camadas(self) retorna List[CamadaDistro]:
        retorne list(CamadaDistro)

    funcao ferramentas_por_tipo(self, tipo: TipoFerramentaSeguranca) retorna List[FerramentaSeguranca]:
        retorne [f for f in self.ferramentas_seg if f.tipo == tipo]

    funcao pacotes_por_camada(self, camada: CamadaDistro) retorna List[PacoteDistro]:
        todos <- self.pacotes_a11y + self.pacotes_ia + self.pacotes_rep
        retorne [p for p in todos if p.camada == camada]

    funcao perfil_por_nome(self, nome: str) retorna Optional[PerfilDistro]:
        para cada p em self.perfis:
            se p.nome == nome entao:
                retorne p
        retorne nulo

    // -- meta-pacote --------------------------------------------------------

    funcao gerar_metapacote(self, perfil: PerfilDistro) retorna Dict[str, Any]:
        // Gera a definicao do meta-pacote .deb para o perfil.
        declare depends: List[str]  <- []
        para cada pkg_name em perfil.pacotes_incluidos:
    ...
    funcao checklist_hardening(self, nivel: NivelHardening) retorna List[str]:
        ...
        retorne checklists.get(nivel, [])

    // -- manifesto ----------------------------------------------------------

    funcao manifesto_kali(self) retorna str:
        retorne (
...
funcao _demo() retorna None:
    ...
    para cada item em e.checklist_hardening(NivelHardening.REPUBLICA):
        print(f"  {item}")
