...
struct ReplicationCost {
    ...
    let data_collection_hours: f64 = 0 // baixar/coletar;
    let data_cleaning_hours: f64 = 0 // limpar/normalizar;
    let cross_referencing_hours: f64 = 0 // cruzar com outros;
    let model_building_hours: f64 = 0 // construir modelo;
    let documentation_hours: f64 = 0 // documentar;
    let fact_checking_hours: f64 = 0 // verificar;
    let engineering_hours: f64 = 0 // construir API/endpoint;
    // Custo/hora de um profissional senior no Brasil (R$)
    let senior_rate_brl: f64 = 200.0 // R$200/h (Data Engineer senior);
    // decorador: @property
    ...
    // decorador: @property
    fn total_cost_brl(self) -> f64 {
        return self.total_hours * self.senior_rate_brl;
    // decorador: @property
    fn floor_price_per_call(self) -> f64 {
...
let REPLICATION_COSTS: {texto: ReplicationCost} = {;
...
struct MarketComparable {
    ...
    note: texto;
let MARKET_COMPARABLES: [MarketComparable] = [;
...
struct MarketFeedback {
    ...
    //
    endpoint: texto;
    let calls_last_30_days: i64 = 0;
    let unique_callers: i64 = 0;
    let revenue_30_days: f64 = 0.0;
    // Para calcular elasticidade (apos mudanca de preco)
    let calls_before_price_change: i64 = 0;
    let calls_after_price_change: i64 = 0;
    let price_change_pct: f64 = 0;
    // Conversao
    let free_tier_users: i64 = 0;
    let free_to_paid_conversion: f64 = 0 // %;
#[derive(Debug, Clone)]
struct DynamicPricer {
...
struct CalibratedPrice {
    ...
    let feedback_adjustment: flutuante? = NULL;
    let feedback_reason: String = "SEM DADOS AINDA";
    // Camada 4: governance
    let assembly_override: flutuante? = NULL;
    let assembly_reason: String = "";
    // decorador: @property
    fn calibrated_token_cost(self) -> f64 {
    ...
    // decorador: @property
    fn calibrated_price_brl(self) -> f64 {
        return self.calibrated_token_cost * 0.10;
fn calibrate_all() -> [CalibratedPrice] {
