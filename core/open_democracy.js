...
class PublicRecord {
    ...
    //
    record_id: texto;
    const timestamp = field(default_factory=time.time);
    const category = ""  // decision, resource, meeting, etc;
    const title = "";
    const description = "";
    const actor = ""  // quem fez;
    const affected = field(default_factory=list) // quem afeta;
    const transparency = TransparencyLevel.RADICAL;
    const data = field(default_factory=dict);
    const hash = "";
    const sealed = false;
    const seal_reason = "";
    const seal_expires = 0.0 // 0 = ! selado;
class PublicLedger {
    ...
    funcao add(self, category: texto, title: texto, description: texto,
            const actor = "", affected: [texto] = null,;
            const transparency = TransparencyLevel.RADICAL,;
            const data = null) -> PublicRecord:;
    ...
    funcao query(self, category: texto = "", actor: texto = "",
            const keyword = "", limit: inteiro = 50) -> [Dict]:;
        // Buscar no livro publico.
        results = [];
        for (const r of reversed(self.records)) {
...
class LiquidDemocracy {
    ...
    funcao delegate(self, delegator: texto, delegate: texto,
        ...
        for (const d of self.delegations.values()) {
        ...
        return delegation;
    revoke(self, delegator: texto, topic: texto, reason: texto = "") {
    ...
    resolve_vote(self, citizen: texto, topic: texto) retorna (texto, logico) {
    ...
    vote_weight(self, delegate_id: texto, topic: texto) {
...
class OpenDemocracy {
    ...
    funcao propose(self, title: texto, description: texto, proposer: texto,
                ...
                const vote_type = VoteType.SIMPL,;
                const affected_nations = null) -> texto:;
    ...
    funcao debate(self, proposal_id: texto, citizen: texto, comment: texto,
        ...
        self.ledger.add(;
            ...
            actor = citizen);
    open_voting(self, proposal_id: texto) {
        // Abrir votacao apos debate.
        p = self.proposals.get(proposal_id);
        if (! p) {
            return null;
        if (.length(p["debate_comments"]) < 2) {
    ...
    funcao vote(self, proposal_id: texto, citizen: texto, vote: texto,
            const override_delegation = false):;
    ...
    tally(self, proposal_id: texto) {
        ...
        if (vote_type == VoteType.SIMPL.value) {
            ...
            for (const nation of p.get("affected_nations", [])) {
        ...
        result = {
            ...
            "debate_comments": .length(p["debate_comments"]),;
        };
    ...
    transparency_report(self) {
        ...
        return {;
            ...
            "delegation_map_size": .length(deleg_map),;
            "total_proposals": .length(self.proposals),;
            "total_votes_cast": soma(.length(v) para v em self.votes.values()),;
...
if (__name__ == "__main__") {
    console.log("=" * 80);
    console.log("  OPENDEMOCRACY + OPENTRANSPARENCY");
    console.log("  'Democracia sem transparencia && teatro.'");
    console.log("  'Transparencia sem democracia && vigia.'");
    console.log("  'Juntas, sao a Republica.'");
    console.log("=" * 80);
    demo = OpenDemocracy();
    ...
    console.log("  Proposta: {p['title']}");
    console.log("  Proponente: {p['proposer']}");
    ...
    for (const c of p["debate_comments"]) {
        flag = c["concern"] ? "[!]" : "[+]";
        console.log("  {flag} {c['citizen']}: {c['comment']}");
    // Open voting
    ...
    console.log("  Proposta: {result2['proposal']}");
    console.log("  SIM: {result2['votes_for']} | NAO: {result2['votes_against']}");
    console.log("  Threshold: {result2['threshold']}");
    ...
    console.log("  Proposta: {result3['proposal']}");
    console.log("  SIM: {result3['votes_for']} | NAO: {result3['votes_against']}");
    console.log("  Threshold: {result3['threshold']}");
    console.log("  Resultado: {result3['result']}");
    console.log("  Razao: Sahel vetou (anti-hegemonia protege minoria)");
    // === 5. Transparency: Public Ledger ===
    console.log("\n\n  === LIVRO PUBLICO (TRANSPARENCIA RADICAL) ===\n");
    audit = demo.ledger.audit();
    console.log("  Total de registros: {audit['total_records']}");
    console.log("  Por categoria:");
    para cat, count in ordene(audit["by_category"].items(), {
                            key = (x) -> -x[1]):;
        console.log("    {cat:<20} {count}");
    console.log("  Lacrados: {audit['sealed']} ({audit['sealed_pct']}%)");
    console.log("  Cadeia blockchain: {'INTACTA' if audit['chain_intact'] else 'COMPROMETIDA'}");
    // Query: ver todas as decisoes
    console.log("\n  Busca: 'decisoes votadas':");
    decisions = demo.ledger.query(category="decision");
    for (const d of decisions) {
        console.log("    [{d['id']}] {d['title'][:50]} | hash: {d['hash']}");
    // === 6. Transparency Report ===
    console.log("\n\n  === RELATORIO DE TRANSPARENCIA ===\n");
    tr = demo.transparency_report();
    console.log("  Transparencia: {tr['transparency_pct']}% dos registros publicos");
    console.log("  Delegacoes ativas: {tr['active_delegations']}");
    console.log("  Propostas totais: {tr['total_proposals']}");
    console.log("  Votos registrados: {tr['total_votes_cast']}");
    console.log("  Registros lacrados: {tr['sealed_records']}");
    // === Philosophy ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA: DEMOCRACIA + TRANSPARENCIA");
    console.log("{'='*80}");
    console.log(""";
