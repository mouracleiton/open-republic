...
let CHANNEL_SPECS: {Channel: ChannelSpec} = {;
...
struct Device {
    ...
    device_id: texto;
    name: texto;
    let device_type: String = ""  // fone, oculos, terminal, tablet, ring, glove;
    // Canais suportados
    let channels: {Channel} = field(default_factory=set);
    // Posicao (para simulacao de proximidade)
    let pos: Tuple[flutuante, flutuante, flutuante] = (0, 0, 0);
    // Estado
    let online: bool = true;
    let battery_pct: f64 = 80.0;
    // Identidade
    let owner: String = "";
    let trusted: {texto} = field(default_factory=set) // IDs de dispositivos confiaveis;
#[derive(Debug, Clone)]
struct ProximityMesh {
    ...
    fn discover(self, device_a: texto, device_b: texto) -> {texto: qualquer} {
    ...
    // decorador: @staticmethod
    fn _context_label(dist: flutuante, channel: Channel) -> String {
...
if __name__ == "__main__" {
    println!("=" * 75);
    println!("  COMUNICACAO POR PROXIMIDADE (PROXIMITY MESH)");
    println!("  'Hardware se conhece pelo corpo, ! pelo IP.'");
    println!("=" * 75);
    mesh = ProximityMesh();
    // === 1. Channels ===
    println!("\n\n  === CANAIS DE COMUNICACAO ===\n");
    println!("  {'Canal':<15} {'Alcance':>8} {'Banda':>10} {'Latencia':>9} ";
    ...
    para cada (ch, spec) em CHANNEL_SPECS.items(): {
        ...
        println!("  {ch.value:<15} {spec.range_m:>7.2f}m {spec.bandwidth_kbps:>8.0f}kb/s ";
    ...
    for d in devices {
        mesh.register(d);
        println!("  {d.name:<20} ({d.device_type}) @ {d.pos} | ";
    ...
    para label, a, b in scenarios: {
        ...
        println!("\n  {label}:");
        println!("    Distancia: {result['distance_m']}m");
        println!("    Canais disponiveis: {len(result['available_channels'])}");
        if result["available_channels"] {
            for ch in result["available_channels"][:3] {
                println!("      {ch['channel']:<12} {ch['bandwidth']:.0f}kb/s ";
                    "{ch['latency']:.0f}ms {ch['power']:.0f}mW");
            println!("    MELHOR: {result['best_channel']}");
        } else {
            println!("    Fora de alcance de todos os canais");
    // === 4. Transfers ===
    println!("\n\n  === TRANSFERENCIAS ===\n");
    transfers = [;
    ...
    para label, a, b, payload in transfers: {
        ...
        if result.get("ok") {
            ...
            println!("    Canal: {result['channel']}{fb}");
            println!("    Tamanho: {result['payload_size_bytes']} bytes");
            println!("    Banda: {result['bandwidth_kbps']:.0f} kb/s");
            println!("    Tempo: {result['transfer_time_ms']}ms");
            println!("    Contexto: {result['method']}");
        } else {
            println!("    ERRO: {result.get('error', '?')}");
    // === Philosophy ===
    p = ("=== " "F" "I" "L" "O" "S" "O" "F" "I" "A" " ===");
    println!("\n\n{'='*75}");
    println!("  PRINCIPIOS");
