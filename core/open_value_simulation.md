...
**Arquivo original:** `core/open_value_simulation.py`
...
classe ValueSimulator:
    ...
    4. GANHO: diferenca (R$ e multiplicador)
    ...
    funcao simulate_worker(self, worker_id: texto) -> {texto: qualquer}:
    ...
    funcao _life_impact(self, sobra_cap: flutuante, sobra_rep: flutuante) -> texto:
        // O que muda na vida da pessoa.
        se sobra_cap < 0 e sobra_rep > 0 entao:
            retorne "SAI DA POBREZA. Hoje passa fome. Republica: sobra dinheiro."
        se sobra_cap < 500 e sobra_rep > 2000 entao:
            retorne "MUDANCA DRAMATICA. Hoje apertado. Republica: qualidade de vida."
        se sobra_cap < 2000 e sobra_rep > 5000 entao:
            retorne "QUALIDADE DE VIDA. Hoje sobrevive. Republica: prospera."
        se sobra_cap < 5000 e sobra_rep > 10000 entao:
            retorne "PROSPERIDADE. Hoje confortavel. Republica: investe."
        se sobra_rep > sobra_cap * 2 entao:
            retorne "DOBRAR PADRAO DE VIDA."
        retorne "Ganho de R$ {sobra_rep - sobra_cap:,.0f}/mes de sobra."

    funcao simulate_all(self) -> {texto: qualquer}:
        // Simula todos os trabalhadores.
        results = []
        total_gain_monthly = 0
        total_extracted_monthly = 0
        total_workers = tamanho(self.workers)

        para cada wid em self.workers:
    ...
    funcao aggregate_stats(self) -> {texto: qualquer}:
        ...
        retorne {
            "trabalhadores": tamanho(self.workers),
...
se __name__ == "__main__" entao:
    ...
    imprima("=" * 80)
    imprima("  OPENVALUESIMULATION -- PESSOAS REAIS, SALARIOS REAIS")
    imprima("  Nomes obfuscados. Dados reais do mercado brasileiro.")
    imprima("=" * 80)

    // === 1. CADA TRABALHADOR ===
    imprima("\n\n  === SIMULACAO POR TRABALHADOR ===\n")
    para cada wid em ordene(sim.workers.keys()):
        r = sim.simulate_worker(wid)
        w = sim.workers[wid]

        imprima("\n  {'='*70}")
        imprima("  {r['alias']} | {r['profissao']} | {r['empresa']} | {r['local']}")
        imprima("  Familia: {r['familia']} | Horas/mes: {r['horas_mes']}")
        imprima("  {'='*70}")

        imprima("\n  CAPITALISMO (hoje):")
        imprima("    Produz:           {r['CAPITALISMO']['produz']}")
        imprima("    Recebe:           {r['CAPITALISMO']['recebe']} ({r['CAPITALISMO']['fica_com']})")
        imprima("    Empresa suga:     {r['CAPITALISMO']['empresa_suga']}")
        imprima("    Banco suga:       {r['CAPITALISMO']['banco_suga']}")
        imprima("    Intermediario:    {r['CAPITALISMO']['intermediario_suga']}")
        imprima("    TOTAL EXTRAIDO:   {r['CAPITALISMO']['total_extraido']}")
        imprima("    Razao:            {r['CAPITALISMO']['razao_exploracao']}")
        imprima("    Hora vale:        {r['CAPITALISMO']['hora_valor']}")
        imprima("    Sobra (apos ali+comida): {r['CAPITALISMO']['sobra_apos_aluguel_comida']}")

        imprima("\n  REPUBLICA EXECUTAVEL:")
        imprima("    Produz:           {r['REPUBLICA_EXECUTAVEL']['produz']}")
        imprima("    Recebe:           {r['REPUBLICA_EXECUTAVEL']['recebe']} ({r['REPUBLICA_EXECUTAVEL']['fica_com']})")
        imprima("    Empresa suga:     {r['REPUBLICA_EXECUTAVEL']['empresa_suga']}")
        imprima("    Banco suga:       {r['REPUBLICA_EXECUTAVEL']['banco_suga']}")
        imprima("    Intermediario:    {r['REPUBLICA_EXECUTAVEL']['intermediario_suga']}")
        imprima("    Pool 5%:          {r['REPUBLICA_EXECUTAVEL']['pool_5pct']}")
        imprima("    Operacional real: {r['REPUBLICA_EXECUTAVEL']['operacional_real']}")
        imprima("    Hora vale:        {r['REPUBLICA_EXECUTAVEL']['hora_valor']}")
        imprima("    Sobra (apos ali+comida): {r['REPUBLICA_EXECUTAVEL']['sobra_apos_aluguel_comida']}")

        imprima("\n  GANHO:")
        imprima("    Extra/mes:  {r['GANHO']['extra_por_mes']}")
        imprima("    Extra/ano:  {r['GANHO']['extra_por_ano']}")
        imprima("    Multiplicador: {r['GANHO']['multiplicador']}")
        imprima("    Impacto: {r['GANHO']['mudanca_vida']}")

    // === 2. AGREGADO ===
    imprima("\n\n  {'='*70}")
    imprima("  === AGREGADO: TODOS OS {len(sim.workers)} TRABALHADORES ===")
    imprima("  {'='*70}\n")
    agg = sim.simulate_all()
    imprima("  {agg['message']}")
    imprima("\n  Extracao mensal total: {agg['total_extraido_mensal']}")
    imprima("  Extracao anual total:  {agg['total_extraido_anual']}")
    imprima("  Ganho mensal Republica: {agg['total_ganho_republica_mensal']}")
    imprima("  Ganho anual Republica:  {agg['total_ganho_republica_anual']}")
    ...
    imprima("\n\n  === RESUMO (todos) ===\n")
    imprima("  {'Alias':<10} {'Profissao':<25} {'Hoje':>10} {'Republica':>10} {'Ganho':>10} {'X':>5} {'Impacto'}")
    imprima("  {'-'*95}")
    para cada wid em ordene(sim.workers.keys()):
        r = sim.simulate_worker(wid)
        ...
        imprima("  {r['alias']:<10} {r['profissao'][:24]:<25} "
              "R${int(hoje):>8,} R${int(rep):>8,} R${int(gain):>8,} "
              "{mult:>4} {impacto}")

    imprima("\n{'='*80}")
    imprima("  Simulacao: {len(sim.workers)} trabalhadores reais.")
    imprima("  Extracao anual: {agg['total_extraido_anual']}.")
    imprima("  Se virasse Republica: {agg['total_ganho_republica_anual']} voltam.")
    imprima("  Para trabalhadores. Nao parasitas.")
    imprima("{'='*80}")
