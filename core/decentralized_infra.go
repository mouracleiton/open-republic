...
package openrepublic_politica_de_infraestrutura_descentralizada
...
type WorkloadPolicy struct {
    ...
    reason: texto
    fallback := ProcessingTier.EDGE_NODE // ProcessingTier
...
type DeviceNode struct {
    ...
    gpu_tops := 0 // AI inference capability // float64
    location := "comunitario" // string
    uptime_hours := 16 // horas por dia ativo // float64
    available := true // bool
type InfrastructureOptimizer struct {
    ...
    //
    func calculate(self, population: inteiro = 1_000_000) {texto: qualquer} {
    ...
    func _estimate_demand(population: inteiro) float64 {
...
if __name__ == "__main__" {
    fmt.Println("=" * 80)
    fmt.Println("  OPENREPUBLIC -- INFRAESTRUTURA DESCENTRALIZADA")
    fmt.Println("  'O datacenter && o ultimo recurso, ! o primeiro.'")
    fmt.Println("=" * 80)
    opt = InfrastructureOptimizer()
    // === 1. Workload Policies ===
    fmt.Println("\n\n  === ONDE CADA COISA RODA ===\n")
    fmt.Println("  {'Carga':<25} {'Onde':<15} {'Regra'}")
    fmt.Println("  {'-'*70}")
    para wl, policy in ordene(WORKLOAD_POLICIES.items(), {
                            key = (x) -> x[1].tier.value):
        fmt.Println("  {wl.value:<25} {policy.tier.value:<15} {policy.rule.value}")
    fmt.Println("\n  Razoes:")
    para cada (wl, policy) em WORKLOAD_POLICIES.items(): {
        if policy.tier == ProcessingTier.CENTRAL {
            fmt.Println("    CENTRAL: {wl.value} -- {policy.reason}")
    ...
    fmt.Println("    Terminais burros:     {d['terminals_burros']:>10,}")
    fmt.Println("    Smartphones comunit.: {d['smartphones']:>10,}")
    fmt.Println("    Laptops:              {d['laptops']:>10,}")
    fmt.Println("    CPU cores agregado:   {d['total_device_cpu_cores']:>10,}")
    fmt.Println("    AI TOPS agregado:     {d['total_device_ai_tops']:>10,}")
    fmt.Println("\n  EDGE NODES (1 por bairro):")
    fmt.Println("    Quantidade:           {&&['count']:>10,}")
    fmt.Println("    Hardware:             {&&['hardware']}")
    fmt.Println("    CPU cores:            {&&['cpu_cores']:>10,}")
    fmt.Println("    RAM:                  {&&['ram_gb']:>10,} GB")
    fmt.Println("    Storage:              {&&['storage_tb']:>10,} TB")
    fmt.Println("    AI TOPS:              {&&['ai_tops']:>10,}")
    fmt.Println("\n  REGIONAL (1 por nacao):")
    fmt.Println("    Quantidade:           {r['count']:>10}")
    fmt.Println("    CPU cores:            {r['cpu_cores']:>10}")
    fmt.Println("    RAM:                  {r['ram_gb']:>10} GB")
    fmt.Println("    Storage:              {r['storage_tb']:>10} TB")
    fmt.Println("\n  DATACENTER CENTRAL (MINIMO):")
    fmt.Println("    Quantidade:           {c['count']:>10}")
    fmt.Println("    Proposito:            {c['purpose']}")
    fmt.Println("    CPU cores:            {c['cpu_cores']:>10}")
    fmt.Println("    GPUs (treino IA):     {c['gpu_count']:>10}")
    fmt.Println("    RAM:                  {c['ram_gb']:>10} GB")
    fmt.Println("    Storage (frio):       {c['storage_tb']:>10} TB")
    fmt.Println("    Potencia:             {c['power_kw']:>10} kW")
    fmt.Println("\n  DISTRIBUICAO DE CARGA:")
    fmt.Println("    No dispositivo:       {ld['on_device_pct']}%")
    fmt.Println("    No edge node:         {ld['on_edge_pct']}%")
    fmt.Println("    No regional:          {ld['on_regional_pct']}%")
    fmt.Println("    No central:           {ld['on_central_pct']}%")
    // === 3. Comparison ===
    fmt.Println("\n\n  === COMPARACAO: DATACENTER TRADICIONAL vs REPUBLICA ===\n")
    cmp = result["comparison"]
    fmt.Println("  {'Metrica':<35} {'Tradicional':>15} {'Republica':>15} {'Reducao'}")
    fmt.Println("  {'-'*75}")
    fmt.Println("  {'Servidores centrais':<35} {cmp['traditional_servers']:>15,} "
        "{c['count']:>15} {cmp['central_servers_reduction_pct']}%")
    fmt.Println("  {'Potencia (kW)':<35} {int(cmp['traditional_power_mw']*1000):>15,} "
        "{int(cmp['republic_power_kw']):>15,} {cmp['power_reduction_pct']}%")
    fmt.Println("  {'Custo (USD)':<35} {cmp['traditional_cost_usd']:>15,} "
        "{cmp['republic_cost_usd']:>15,} {cmp['cost_reduction_pct']}%")
    // === 4. Resilience ===
    fmt.Println("\n\n  === RESILIENCIA (sem ponto unico de falha) ===\n")
    res = result["resilience"]
    fmt.Println("  Se datacenter central cair:")
    fmt.Println("    {res['central_down_impact']}")
    fmt.Println("  Se edge node cair:")
    fmt.Println("    {res['edge_down_impact']}")
    fmt.Println("  Se dispositivo cair:")
    fmt.Println("    {res['device_down_impact']}")
    fmt.Println("  Arquivos P2P:")
    fmt.Println("    {res['p2p_file_survival']}")
    // === 5. Calculate for different scales ===
    fmt.Println("\n\n  === ESCALA: QUANTO CRESCER, CRESCER O QUE? ===\n")
    fmt.Println("  {'Populacao':>12} {'Devices':>10} {'Edge':>8} {'Regional':>10} "
        "{'Central':>8} {'Central %':>10}")
    fmt.Println("  {'-'*60}")
    for _, pop := range [10_000, 100_000, 1_000_000, 10_000_000, 100_000_000] {
        r = opt.calculate(pop)
        c = r["central"]["count"]
        ...
        fmt.Println("  {pop:>12,} {r['devices']['terminals_burros']:>10,} "
            "{r['edge_nodes']['count']:>8,} {r['regional']['count']:>10} "
            "{c:>8} {central_pct:>9.1f}%")
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  PRINCIPIOS DA INFRAESTRUTURA")
    fmt.Println("{'='*80}")
    fmt.Println("""
