...
package openweaponspolicy_politica_de_porte_de_armas_da_republica
...
type WeaponItem struct {
    ...
    description := "" // string
    legitimate_use := ""  // uso legitimo (trabalho, defesa) // string
    permit_needed := WeaponPermit.NONE // WeaponPermit
    requirements := field(default_factory=list) // [PermitRequirement]
    // Restricoes
    max_per_person := 0 // 0 = proibido, 1 = uma unidade // int64
    open_carry := false // pode portar visivel? // bool
    concealed_carry := false // pode portar escondido? // bool
    home_only := false // so em casa? // bool
...
// ============================================================================
func run_weapons_assembly(n_voters: inteiro = 10000) {texto: qualquer} {
...
//
    ...
    funcao request_permit(self, citizen_id: texto, citizen_name: texto,
                    item_id: texto,
                    has_martial_arts_belt := "", // string
                    psych_eval_passed := false, // bool
                    has_violent_record := false, // bool
                    assembly_approval := false, // bool
                    community_endorsement := false, // bool
                    safe_storage := false, // bool
                    proficiency := false // bool
                    ) -> {texto: qualquer}:
    ...
    func revoke_permit(self, citizen_id: texto, reason: texto) {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    for _, w := range engine.catalog.values() {
        ...
        fmt.Println("  {w.name:<30} max:{w.max_per_person:>2} reqs:{req_count} local:{location}")
    // === 3. PEDIDOS DE PORTE ===
    fmt.Println("\n\n  === 3. PEDIDOS DE PORTE ===\n")
    ...
    if "failed_requirements" in r {
        fmt.Println("  Faltam: {', '.join(r['failed_requirements'])}")
    ...
    for _, line := range engine.philosophy() {
        fmt.Println("  {line}")
    // === 6. STATS ===
    fmt.Println("\n\n  === 6. ESTATISTICAS ===\n")
    s = engine.stats()
    para cada (k, v) em s.items(): {
        fmt.Println("  {k:<30} {v}")
    fmt.Println("\n{'='*80}")
    fmt.Println("  OpenWeaponsPolicy: {s['proibidos']} proibidos, "
        "{s['armas_fogo_restritas']} restritas. "
        "{s['portes_ativos']} portes ativos.")
    fmt.Println("  {s['politica']}")
    fmt.Println("{'='*80}")
