...
#ifndef OPENREPUBLIC_MANIFEST_CENTRAL_H
#define OPENREPUBLIC_MANIFEST_CENTRAL_H
...
{texto: qualquer} count_asset(asset: RepublicAsset, base_path: Path) {
    ...
    total_lines = soma(sizeof(f.read_text(errors="ignore").splitlines());
                    /* para f em code_files) */
    langs = ordene(set(f.suffix para f em code_files));
    asset.files = sizeof(code_files);
    asset.lines = total_lines;
    asset.languages = [l.lstrip(".") para l em langs];
    return {"files": sizeof(code_files), "lines": total_lines, "exists": true};
{texto: qualquer} full_inventory(base_path: Path = None) {
    ...
    total_files = 0;
    total_lines = 0;
    /* TODO: iterador C manual para asset em REPUBLIC_REGISTRY */
    ...
    return {;
        "total_assets": sizeof(REPUBLIC_REGISTRY),;
        "total_files": total_files,;
        "total_lines": total_lines,;
        "domains": sizeof(by_domain),;
        "by_domain": by_domain,;
    };
if (__name__ == "__main__") {
    printf("=" * 75);
    printf("  OPENREPUBLIC -- REGISTRO CENTRAL DE BENS COMUNS");
    printf("  'Tudo que existe na Republica && bem publico && cadastrado.'");
    printf("=" * 75);
    inv = full_inventory();
    printf("\n  TOTAL: {inv['total_assets']} sistemas | ";
        "{inv['total_files']} arquivos | ";
        "{inv['total_lines']:,} linhas\n");
    printf("  {'Dominio':<16} {'Sistemas':>8} {'Arquivos':>8} {'Linhas':>8}");
    printf("  {'-'*44}");
    /* para cada (domain, data) em ordene(inv["by_domain"].items()): */
        printf("  {domain:<16} {len(data['assets']):>8} ";
            "{data['total_files']:>8} {data['total_lines']:>8,}");
    printf("\n  {'SISTEMA':<28} {'DOMINIO':<14} {'LINHAS':>7}");
    printf("  {'-'*52}");
    /* para cada (domain, data) em ordene(inv["by_domain"].items()): */
        /* TODO: iterador C manual para asset em ordene(data["assets"], key=(x) -> -x["lines"]) */
            marker = asset["lines"] == 0 ? " <== VAZIO" : "";
            printf("  {asset['name']:<28} {domain:<14} ";
                "{asset['lines']:>7,}{marker}");
    printf("\n{'='*75}");
    printf("  8 dominios. {inv['total_assets']} sistemas integrados.");
    printf("  {inv['total_lines']:,} linhas de codigo aberto.");
    printf("  Tudo bem comum. Tudo da Republica.");
    printf("{'='*75}");

#endif // OPENREPUBLIC_MANIFEST_CENTRAL_H
