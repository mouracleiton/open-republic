...
#ifndef OPENREPUBLIC_SOLUCOES_PARA_OS_3_GARGALOS_RESTANTES_DO_SAHEL_H
#define OPENREPUBLIC_SOLUCOES_PARA_OS_3_GARGALOS_RESTANTES_DO_SAHEL_H
...
// ============================================================================
typedef struct ChildcareSolution {
...
// ============================================================================
typedef struct AgricultureSolution {
...
// ============================================================================
typedef struct HousingSolution {
...
if (__name__ == "__main__") {
    printf("=" * 75);
    printf("  SOLUCOES PARA OS 3 GARGALOS DO SAHEL");
    printf("  Sem dinheiro. Sem empreiteira. Sem 'contratar'.");
    printf("  Com automatizacao + comunidade + conhecimento aberto.");
    printf("=" * 75);
    // === SOLUCAO 1: CUIDADO INFANTIL ===
    printf("\n\n" + "=" * 75);
    printf("  GARGALO 1: CUIDADO INFANTIL (300% sobrecarga)");
    printf("  1 pessoa tentando cuidar de 300 criancas");
    printf("=" * 75);
    ...
    printf("  Demanda ajustada (IA reduz 25%): {childcare['demanda_ajustada_por_ia']:,}");
    printf("\n  FONTES DE CUIDADO:");
    printf("  {'Fonte':<35} {'Pessoas':>8} {'Equiv FT':>10}");
    printf("  {'-'*55}");
    /* para cada (fonte, dados) em childcare['fontes_de_cuidado'].items(): */
        if ('pessoas' in dados) {
            printf("  {fonte.replace('_', ' ').title():<35} {dados['pessoas']:>8} ";
                "{dados['equiv_fulltime']:>10}");
        } else if ('estacoes' in dados) {
            printf("  {fonte.replace('_', ' ').title():<35} {dados['estacoes']:>8} estacoes");
    printf("  {'-'*55}");
    printf("  {'CAPACIDADE TOTAL':<35} {'':>8} {childcare['capacidade_total_equiv']:>10}");
    printf("  {'DEMANDA TOTAL':<35} {'':>8} {childcare['demanda_total_equiv']:>10}");
    printf("\n  COBERTURA: {childcare['cobertura']}");
    printf("  VIÁVEL: {'SIM' if childcare['viavel'] else 'NAO'}");
    printf("\n  PRINCIPIO: Nao se trata de contratar 75 profissionais.");
    printf("  Trata-se de ativar a COMUNIDADE INTEIRA:");
    printf("    - Cada pai/mae: 4h/semana");
    printf("    - Cada idoso ativo: 10h/semana (continuar contribuindo)");
    printf("    - Cada adolescente: 6h/semana (mentoria + responsabilidade)");
    printf("    - Estacoes de IA: 25% do aprendizado && autonomo");
    printf("  Resultado: a comunidade SE cuida. Nao precisa de Estado.");
    // === SOLUCAO 2: AGRICULTURA ===
    printf("\n\n" + "=" * 75);
    printf("  GARGALO 2: AGRICULTURA (167% sobrecarga)");
    printf("  Faltam 2.000 kg/mes de comida para 40.000 pessoas");
    printf("=" * 75);
    ...
    printf("\n  ESTRATEGIAS:");
    printf("  {'Estrategia':<30} {'Escala':>15} {'Producao':>12} {'Pessoas':>8}");
    printf("  {'-'*68}");
    /* para cada (nome, dados) em agriculture['estrategias'].items(): */
        if (isinstance(dados, dict)) {
            ...
            printf("  {nome.replace('_', ' ').title():<30} {escala:>15} {prod:>12} {pessoas:>8}");
    printf("\n  {'PRODUCAO NOVA TOTAL':<30} {'':>15} {agriculture['producao_total_nova_kg_mes']:>9} kg/mes");
    printf("  {'TRABALHADORES':<30} {'':>15} {'':>12} {agriculture['trabalhadores_necessarios']:>8}");
    printf("  COBRE O GAP: {'SIM' if agriculture['cobre_gap'] else 'NAO'}");
    printf("  AUTO-SUSTENTAVEL APOS: {agriculture['auto_sustentavel_apos']}");
    printf("\n  PRINCIPIO: Hidroponia (10x mais por m2) + permacultura");
    printf("  (auto-sustentavel apos 3 anos) + aquaponia (proteina + vegetais)");
    printf("  + automacao IoT (40% menos trabalho) + conhecimento da Amazonia");
    // === SOLUCAO 3: HABITACAO ===
    printf("\n\n" + "=" * 75);
    printf("  GARGALO 3: HABITACAO (167% sobrecarga)");
    printf("  Faltam ~2.000 unidades para 40.000 pessoas");
    printf("=" * 75);
    ...
    printf("\n  ESTRATEGIAS:");
    printf("  {'Estrategia':<25} {'Unid/mes':>10} {'Material':<25} {'Pessoas':>8}");
    printf("  {'-'*70}");
    /* para cada (nome, dados) em housing['estrategias'].items(): */
        if (isinstance(dados, dict)  &&  ('casas_por_mes' in dados  ||  'kits_por_mes' in dados)) {
            ...
            printf("  {nome.replace('_', ' ').title():<25} {unid:>10} {mat:<25} {pessoas:>8}");
        } else if (isinstance(dados, dict)  &&  'massa_termica' in dados) {
            printf("  {'Design Passivo':<25} {'N/A':>10} {'Sem ar condicionado':<25}");
    printf("\n  {'TOTAL':<25} {housing['casas_por_mes_total']:>10}");
    printf("  MESES PARA COBRIR GAP: {housing['meses_para_cobrir_gap']}");
    printf("  TRABALHADORES: {housing['trabalhadores_necessarios']}");
    printf("  CUSTO: {housing['custo_monetario']}");
    printf("  MATERIAL: {housing['material_principal']}");
    printf("\n  PRINCIPIO: Terra local && gratuita. Prensa manual faz bloco.");
    printf("  Impressora 3D faz casa em 30h. Mutirao constroi 10 casas/semana.");
    printf("  Design passivo: 40cm de parede de terra = sem ar condicionado.");
    // === RESUMO INTEGRADO ===
    printf("\n\n" + "=" * 75);
    printf("  RESUMO: COMO RESOLVER OS 3 GARGALOS SEM DINHEIRO");
    printf("=" * 75);
    printf(""";
+-----------+----------------+------------------+----------+--------+;
...
| | automacao | | drones | |;
+-----------+----------------+------------------+----------+--------+;
| Habitacao | 3D printf + | Terra local | ~40 + | 6-12 |;
...
#endif // OPENREPUBLIC_SOLUCOES_PARA_OS_3_GARGALOS_RESTANTES_DO_SAHEL_H
