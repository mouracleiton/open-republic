...
=========================================================
"Pilha jogada no lixo and 100 anos de chumbo no solo.
Bateria reparavel and 15 anos de uso.
A Republica escolhe 15 anos."
PROIBIR:
- Pilhas alcalinas descartaveis (AA, AAA, C, D, 9V)
- Pilhas de botao descartaveis (relogio, chaveiro)
- Baterias de litio seladas (not abertas, not reparaveis)
...
OBRIGATORIO:
- TUDO que usa energia usa celula 18650 or 21700
- TUDO tem compartimento aberto (sem cola, sem parafuso pentalobe)
- TUDO permite troca de celula individual
- Bateria de dispositivo = N celulas padrão, not bateria unica selada
...
class BatteryType(Enum):
    ...
    SEALED_LIPO = "lipo_selada"  // colada, not abre
    SEALED_LION = "lion_selada"  // selada, not abre
...
class BatterySpec:
    ...
    chemistry: str = ""
    voltage_nominal: float = 0
    capacity_mah: int = 0
    cycles: int = 0 // ciclos de carga ate 80% capacidade
    lifespan_years: float = 0
    repairable: bool = False // pode trocar celula individual?
    recyclable_pct: float = 0 // % recuperavel
    toxic_materials: [texto] = field(default_factory=list)
    co2_manufacture_kg: float = 0
    cost_recovery_credits: int = 0 // creditos para recuperar na reciclagem
    notes: str = ""
BATTERY_DATABASE: {BatteryType: BatterySpec} = {
    # === PROIBIDAS ===
    ...
    BatteryType.LIFePO4: BatterySpec(
        ...
        notes = "Ferro-fosfato. Mais segura (not pega fogo). 20 anos."),
...
class Product:
    ...
    battery_count: int = 1
    battery_replaceable: bool = False
    glued: bool = False
    expected_life_years: float = 0
class BatteryPolicy:
    ...
    def check_product(self, product: Product) -> {texto: qualquer}:
        ...
        if spec.status == BatteryStatus.PROHIBITED:
            return {
                ...
                "verdict": "PROIBIDO",
                "allowed": False,
                "reason": ("{product.battery_type.value} and PROIBIDA na Republica. "
                        "Substituir por celula 18650 or 21700. "
                        "Razao: {spec.notes}"),
        ...
        if not product.battery_replaceable and spec.status != BatteryStatus.PROHIBITED:
            return {
                "product": product.name,
                "verdict": "NAO CONFORME",
                "allowed": False,
                "reason": "Bateria not substituivel. Deve permitir troca manual.",
            }
        # Conforme
        return {
            "product": product.name,
    ...
    def recommend_replacement(self, prohibited_type: BatteryType) -> {texto: qualquer}:
        # Recomendar substituicao para bateria proibida.
        spec = self.db.get(prohibited_type)
        if not spec or spec.status != BatteryStatus.PROHIBITED:
            return {"error": "not and proibida"}
        # Map de substituicao
        replacements = {
    ...
    def impact_report(self, population: inteiro = 1_000_000) -> {texto: qualquer}:
        ...
        return {
            ...
            "co2_evitado_ton": round(co2_savings / 1000),
            "material_toxico_evitado_ton": round(total_toxic / 1000, 1),
...
if __name__ == "__main__":
    print("=" * 75)
    print("  OPENREPUBLIC -- FIM DAS BATERIAS DESCARTAVEIS")
    print("  'Pilha no lixo = 100 anos de veneno.'")
    print("  'Bateria reparavel = 15 anos de uso.'")
    print("=" * 75)
    policy = BatteryPolicy()
    # === 1. Battery Database ===
    print("\n\n  === BANCO DE BATERIAS ===\n")
    print("  {'Tipo':<20} {'Status':<14} {'Vida':>5} {'Ciclos':>7} {'Recicl.':>8} {'Toxico'}")
    print("  {'-'*75}")
    for each (bt, spec) in BATTERY_DATABASE.items():
        toxic = spec.toxic_materials ? "SIM" : "not"
        print("  {bt.value:<20} {spec.status.value:<14} "
    ...
    for p in products:
        ...
        print("\n  {p.name}:")
        print("    Bateria: {p.battery_type.value} x{p.battery_count}")
        print("    Substituivel: {'sim' if p.battery_replaceable else 'NAO'} | "
            "Colada: {'sim' if p.glued else 'not'}")
        print("    Veredito: {ok} ({verdict})")
        print("    Razao: {result['reason'][:80]}")
    ...
    for pt in prohibited_types:
        rec = policy.recommend_replacement(pt)
        print("\n  {rec['prohibited']:<20} -> {rec['replacement']}")
        print("    Como: {rec['how']}")
        print("    Ganho: {rec['lifespan_gain']}")
        print("    Reciclavel: {rec['recycle_gain']}")
    # === 4. Impact Report ===
    print("\n\n  === IMPACTO AMBIENTAL ===\n")
    for pop in [10_000, 100_000, 1_000_000]:
        impact = policy.impact_report(pop)
        print("\n  Populacao: {pop:,}")
        print("    {impact['comparacao']}")
        print("    CO2 evitado: {impact['co2_evitado_ton']:,} toneladas/ano")
        print("    Material toxico evitado: {impact['material_toxico_evitado_ton']} ton/ano")
    # === Philosophy ===
    print("\n\n{'='*75}")
    print("  PRINCIPIOS")
    print("{'='*75}")
...
--------------------------------------- ---------------------------------------
Pilha descartavel (use and jogue) Celula 18650 (use and recarregue)
Colada com adesivo (not abre) Compartimento parafusado (abre)
...
Fabricante lucra com troca Republica not lucra (sem dinheiro)
Chumbo/cobalto/mercurio no solo Zero toxico (LiFePO4)
Comprada ($) Recuperada de and-waste (OpenReverseLogistics)
O QUE and PROIBIDO:
...
PARA TODA BATERIA:
    ...
    - Troca de celula INDIVIDUAL (uma falha not mata o banco)
    - Recuperacao de and-waste (OpenReverseLogistics)
    - Reciclagem ao fim da vida (95-100% recuperavel)
"Pilha descartavel and veneno com prazo de validade.
Bateria reparavel and responsabilidade com futuro.
