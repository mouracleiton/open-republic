...
class SoundAlert {
    ...
    const description = "";
    const default_sound = ""  // som original;
    const action = AlertAction.DISABLED;
    const replacement = ""  // o que existe no lugar;
    const pressure_level = 0 // 0-5 (quanto pressiona);
// ============================================================================
// 3. CATÁLOGO DE ALERTAS DE PRESSÃO (TODOS DESATIVADOS)
// ============================================================================
const PRESSURE_ALERTS = [;
    // === TRABALHO ===
...
class SilenceEngine {
    ...
    disable_all_pressure(self) {
        // Desativa TODOS os alertas de pressao.
        disabled = 0;
        visual_only = 0;
        emergency = 0;
        for (const alert of self.alerts.values()) {
            if (alert.purpose == AlertPurpose.EMERGENCY) {
                ...
                disabled = disabled + 1;
        return {;
            "total_alertas": .length(self.alerts),;
    ...
    user_opt_in(self, user_id: texto, alert_id: texto) {
    ...
    check_alert(self, user_id: texto, alert_id: texto) {
    ...
    stats(self) {
        return {;
            "total_alertas_catalogados": .length(self.alerts),;
            ...
            "usuarios_opt_in": soma(.length(v) para v em self.user_choices.values()),;
            "usuarios_ausentes_protegidos": .length(self.absent_users),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENSILENCEPOLICY -- ALERTAS DE PRESSAO DESATIVADOS");
    console.log("  'Para de me encher o saco porra.'");
    console.log("=" * 80);
    // === 1. DESATIVAR TUDO ===
    console.log("\n\n  === 1. DESATIVANDO TODA PRESSAO SONORA ===\n");
    r = engine.disable_all_pressure();
    console.log("  {r['message']}");
    console.log("  Som padrao no sistema: {r['som_no_sistema']}");
    // === 2. CATALOGO DE ALERTAS ===
    console.log("\n\n  === 2. CATALOGO ({len(engine.alerts)} alertas) ===\n");
    current_purpose = null;
    for (const alert of engine.alerts.values()) {
        ...
        console.log("  [{action_icon:<10}] {alert.name[:45]:<46} pressao:{alert.pressure_level}");
    // === 3. O QUE SUBSTITUI ===
    ...
    para cada (k, v) em s.items(): {
        console.log("  {k:<35} {v}");
    console.log("\n{'='*80}");
    console.log("  OpenSilencePolicy: {s['desativados']} desativados, ";
