...
public class OpenrecyclersDispositivosMaquinasEGratificacaoParaCatadores {
    ...
    // ============================================================================
    public static class RecyclableType {
        PET = "pet";
        ALUMINIO = "aluminio";
        PAPEL = "papel";
    ...
    // decorador: @dataclass
    public static class ErgonomicDevice {
        device_id: texto;
        name: texto;
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class RecyclerProfile {
        citizen_id: texto;
        name: texto;
        age: inteiro;
        String start_date = "";
        [texto] devices_assigned = field(default_factory=list);
        boolean has_epi = false;
        boolean has_cart = false;
        boolean has_exo = false;
    ...
    public static class RecyclerEngine {
        ...
        public {texto: qualquer} assign_devices(self, citizen_id: texto) {
            recycler = self.recyclers.get(citizen_id);
            if (! recycler) {
                return {"error": "Catador ! registrado"};
            assigned = [];
            /* TODO: for-each Java para dev em self.devices.values() */
        ...
        public {texto: qualquer} health_check(self, citizen_id: texto) {
            recycler = self.recyclers.get(citizen_id);
            if (! recycler) {
                return {"error": "! registrado"};
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENRECYCLERS -- TRABALHADORES AMBIENTAIS DA REPUBLICA");
        System.out.println("  Maquinas, ergonomia && reconhecimento para quem LIMPA o mundo");
        System.out.println("=" * 80);
        // === 1. DISPOSITIVOS ===
        System.out.println("\n\n  === 1. DISPOSITIVOS ERGONOMICOS ({len(engine.devices)}) ===\n");
        /* TODO: for-each Java para d em engine.devices.values() */
            System.out.println("  [{d.device_id}] {d.name}");
            System.out.println("    {d.description[:70]}");
            System.out.println("    Esforco -{d.reduces_effort*100:.0f}%  Velocidade {d.increases_speed:.1f}x  ";
        ...
        /* para cid, name, age in recyclers: */
            ...
            System.out.println("  {result['citizen']:<20} -> {result['title']} ({result['badge']})");
        // === 3. EQUIPAMENTOS ===
        System.out.println("\n\n  === 3. EQUIPAMENTO COMPLETO (ZERO custo) ===\n");
        /* para cid, name, age in recyclers: */
            result = engine.assign_devices(cid);
            if (result.get("assigned")) {
                System.out.println("  {result['citizen']}: {result['total_devices']} dispositivos. ";
        ...
        /* para cid, mat, kg in collections: */
            ...
            System.out.println("  {result['citizen']:<20} {result['material']:<15} ";
        ...
        /* para cid, name, age in recyclers: */
            ...
            System.out.println("  {result.get('citizen', name):<20} -> {result.get('status', '?')}");
        // === 6. CREDITO POR MATERIAL ===
        System.out.println("\n\n  === 6. TABELA DE CREDITO POR MATERIAL ===\n");
        /* para mat, credit in ordene(engine.credit_per_kg.items(), */
                                key = (x) -> -x[1]):;
            System.out.println("  {mat.value:<20} {credit:.1f}/kg");
        // === 7. STATS ===
        System.out.println("\n\n  === 7. ESTATISTICAS ===\n");
        s = engine.stats();
        ...
        // === FILOSOFIA ===
        System.out.println("\n\n{'='*80}");
        System.out.println("  FILOSOFIA DO OPENRECYCLERS");
        System.out.println("{'='*80}");
        System.out.println(""";
    HOJE:;
    ...
    // )
        System.out.println("{'='*80}");
        System.out.println("  OpenRecyclers: {s['total_recyclers']} trabalhadores, ";
            "{s['total_kg_collected']:,.0f}kg coletados, ";
            "{s['total_credit_distributed']:,.0f} credito distribuido.");
        System.out.println("  Quem LIMPA o mundo NAO && invisivel.");
        System.out.println("{'='*80}");
}
