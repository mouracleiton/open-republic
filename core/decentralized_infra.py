...
and um no da rede. Junto, eles sao mais poderosos que qualquer
datacenter corporativo.
O datacenter central existe so para o QUE not PODE ser descentralizado.
...
class DecentralizationRule(Enum):
...
class WorkloadPolicy:
    ...
    reason: texto
    fallback: ProcessingTier = ProcessingTier.EDGE_NODE
WORKLOAD_POLICIES: {WorkloadType: WorkloadPolicy} = {
    ...
    WorkloadType.AI_TRAINING: WorkloadPolicy(
        ...
        "Mas modelo treinado and distribuido para edge nodes."),
    ...
    WorkloadType.REALTIME_COMM: WorkloadPolicy(
        ...
        DecentralizationRule.MUST_DECENTRALIZE,
        "Videochamada and P2P direto entre os dois dispositivos. "
    ...
    WorkloadType.GOVERNANCE_VOTE: WorkloadPolicy(
        ...
        DecentralizationRule.MUST_DECENTRALIZE,
        "Votacao and blockchain P2P. Cada terminal and um no. "
    ...
    WorkloadType.SIMULATION: WorkloadPolicy(
        ...
        "Justifica datacenter. Mas resultado and distribuido."),
}
# ============================================================================
...
class DeviceNode:
    ...
    gpu_tops: float = 0 // AI inference capability
    location: str = "comunitario"
    uptime_hours: float = 16 // horas por dia ativo
    available: bool = True
class InfrastructureOptimizer:
    ...
    # 
    def calculate(self, population: inteiro = 1_000_000) -> {texto: qualquer}:
        ...
        n_nations = max(1, population // 100_000)
        ...
        n_central = max(1, population // 200_000) // 1 para cada 200k
        ...
        return {
            ...
            "edge_nodes": {
                ...
                "ai_tops": edge_ai,
                "hardware": "desktops recuperados de and-waste",
            },
            "regional": {
                "count": n_regional,
            ...
            "load_distribution": {
                "on_device_pct": round(served_by_device / total_compute_demand * 100),
                "on_edge_pct": round(served_by_edge / total_compute_demand * 100),
                "on_regional_pct": round(served_by_regional / total_compute_demand * 100),
                "on_central_pct": round(served_by_central / total_compute_demand * 100),
            },
            "comparison": {
                "traditional_servers": traditional_servers,
                "traditional_power_mw": round(traditional_power_mw, 1),
                "traditional_cost_usd": traditional_cost,
                "republic_cost_usd": republic_cost,
                "republic_power_kw": round(republic_power_kw, 1),
                "cost_reduction_pct": round((1 - republic_cost / traditional_cost) * 100),
                "power_reduction_pct": round((1 - republic_power_kw / (traditional_power_mw * 1000)) * 100),
                "central_servers_reduction_pct": round((1 - n_central / traditional_servers) * 100),
            },
            "resilience": {
                ...
                "p2p_file_survival": "3 copias em nos diferentes. Arquivo not se perde.",
            },
        }
    # decorador: @staticmethod
    def _estimate_demand(population: inteiro) -> float:
...
if __name__ == "__main__":
    print("=" * 80)
    print("  OPENREPUBLIC -- INFRAESTRUTURA DESCENTRALIZADA")
    print("  'O datacenter and o ultimo recurso, not o primeiro.'")
    print("=" * 80)
    opt = InfrastructureOptimizer()
    # === 1. Workload Policies ===
    print("\n\n  === ONDE CADA COISA RODA ===\n")
    print("  {'Carga':<25} {'Onde':<15} {'Regra'}")
    print("  {'-'*70}")
    para wl, policy in ordene(WORKLOAD_POLICIES.items(),
                            key = (x) -> x[1].tier.value):
        print("  {wl.value:<25} {policy.tier.value:<15} {policy.rule.value}")
    print("\n  Razoes:")
    for each (wl, policy) in WORKLOAD_POLICIES.items():
        if policy.tier == ProcessingTier.CENTRAL:
            print("    CENTRAL: {wl.value} -- {policy.reason}")
    # === 2. Calculate for 1M population ===
    print("\n\n  === CALCULO PARA 1.000.000 DE CIDADOS ===\n")
    result = opt.calculate(1_000_000)
    d = result["devices"]
    and = result["edge_nodes"]
    r = result["regional"]
    c = result["central"]
    ld = result["load_distribution"]
    print("\n  DISPOSITIVOS (ja existem, custam zero):")
    print("    Terminais burros:     {d['terminals_burros']:>10,}")
    print("    Smartphones comunit.: {d['smartphones']:>10,}")
    print("    Laptops:              {d['laptops']:>10,}")
    print("    CPU cores agregado:   {d['total_device_cpu_cores']:>10,}")
    print("    AI TOPS agregado:     {d['total_device_ai_tops']:>10,}")
    print("\n  EDGE NODES (1 por bairro):")
    print("    Quantidade:           {and['count']:>10,}")
    print("    Hardware:             {and['hardware']}")
    print("    CPU cores:            {and['cpu_cores']:>10,}")
    print("    RAM:                  {and['ram_gb']:>10,} GB")
    print("    Storage:              {and['storage_tb']:>10,} TB")
    print("    AI TOPS:              {and['ai_tops']:>10,}")
    print("\n  REGIONAL (1 por nacao):")
    print("    Quantidade:           {r['count']:>10}")
    print("    CPU cores:            {r['cpu_cores']:>10}")
    print("    RAM:                  {r['ram_gb']:>10} GB")
    print("    Storage:              {r['storage_tb']:>10} TB")
    print("\n  DATACENTER CENTRAL (MINIMO):")
    print("    Quantidade:           {c['count']:>10}")
    print("    Proposito:            {c['purpose']}")
    print("    CPU cores:            {c['cpu_cores']:>10}")
    print("    GPUs (treino IA):     {c['gpu_count']:>10}")
    print("    RAM:                  {c['ram_gb']:>10} GB")
    print("    Storage (frio):       {c['storage_tb']:>10} TB")
    print("    Potencia:             {c['power_kw']:>10} kW")
    print("\n  DISTRIBUICAO DE CARGA:")
    print("    No dispositivo:       {ld['on_device_pct']}%")
    print("    No edge node:         {ld['on_edge_pct']}%")
    print("    No regional:          {ld['on_regional_pct']}%")
    print("    No central:           {ld['on_central_pct']}%")
    # === 3. Comparison ===
    print("\n\n  === COMPARACAO: DATACENTER TRADICIONAL vs REPUBLICA ===\n")
    cmp = result["comparison"]
    print("  {'Metrica':<35} {'Tradicional':>15} {'Republica':>15} {'Reducao'}")
    print("  {'-'*75}")
    print("  {'Servidores centrais':<35} {cmp['traditional_servers']:>15,} "
        "{c['count']:>15} {cmp['central_servers_reduction_pct']}%")
    print("  {'Potencia (kW)':<35} {int(cmp['traditional_power_mw']*1000):>15,} "
        "{int(cmp['republic_power_kw']):>15,} {cmp['power_reduction_pct']}%")
    print("  {'Custo (USD)':<35} {cmp['traditional_cost_usd']:>15,} "
        "{cmp['republic_cost_usd']:>15,} {cmp['cost_reduction_pct']}%")
    # === 4. Resilience ===
    print("\n\n  === RESILIENCIA (sem ponto unico de falha) ===\n")
    res = result["resilience"]
    print("  Se datacenter central cair:")
    print("    {res['central_down_impact']}")
    print("  Se edge node cair:")
    print("    {res['edge_down_impact']}")
    print("  Se dispositivo cair:")
    print("    {res['device_down_impact']}")
    print("  Arquivos P2P:")
    print("    {res['p2p_file_survival']}")
    # === 5. Calculate for different scales ===
    print("\n\n  === ESCALA: QUANTO CRESCER, CRESCER O QUE? ===\n")
    print("  {'Populacao':>12} {'Devices':>10} {'Edge':>8} {'Regional':>10} "
        "{'Central':>8} {'Central %':>10}")
    print("  {'-'*60}")
    for pop in [10_000, 100_000, 1_000_000, 10_000_000, 100_000_000]:
        r = opt.calculate(pop)
        c = r["central"]["count"]
        ...
        print("  {pop:>12,} {r['devices']['terminals_burros']:>10,} "
            "{r['edge_nodes']['count']:>8,} {r['regional']['count']:>10} "
            "{c:>8} {central_pct:>9.1f}%")
    print("\n\n{'='*80}")
    print("  PRINCIPIOS DA INFRAESTRUTURA")
    print("{'='*80}")
    print("""
...
$500M construcao $0.5M (desktops recuperados de and-waste)
Ponto unico de falha Sem ponto unico de falha
Se cai, TODOS sem servico Se cai, 8% afetado. 92% continua.
Servidor proprietario ($$$) Desktop recuperado (and-waste -> edge)
...
A REGRA DE OURO:
    ...
    TUDO que not pode, roda no edge node do bairro.
    TUDO que not pode, roda no servidor da nacao.
...
CUSTO REAL:
    Edge nodes = desktops RECUPERADOS de and-waste.
...
"Na Republica, processar and um ato comunitario.
