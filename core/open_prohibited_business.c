...
#ifndef OPENPROHIBITEDBUSINESS_PROIBICAO_DE_ESTABELECIMENTOS_NOCIVOS_H
#define OPENPROHIBITEDBUSINESS_PROIBICAO_DE_ESTABELECIMENTOS_NOCIVOS_H
...
// ============================================================================
typedef struct HarmfulBusinessType {
...
// ============================================================================
// decorador: @dataclass
typedef struct HarmfulBusiness {
    ...
    char* money_drained_monthly = ""  // quanto suga por mes;
    // Acao
    BusinessAction action = BusinessAction.PROHIBIT;
    char* replacement = ""  // open equivalente;
    char* conversion_target = ""  // para que converter;
    // Status
    char* status = "identificado"  // identificado, votado, fechado, convertido;
    char* closed_date = "";
// ============================================================================
...
// ============================================================================
{texto: qualquer} run_business_assembly(n_voters: inteiro = 10000) {
    // Assembleia vota quais negocios fechar.
    business_votes = {};
    /* TODO: iterador C manual para biz em HARMFUL_BUSINESSES */
    ...
    return {;
        ...
        "total_businesses": sizeof(HARMFUL_BUSINESSES),;
        "result": "APROVADO -- todos os nocivos serao fechados",;
    };
// ============================================================================
// 5. MOTOR DE PROIBICAO
// ============================================================================
typedef struct ProhibitionEngine {
    ...
    [Dict] list_harmful(self, severity: inteiro = 0) {
    ...
    {texto: qualquer} close_business(self, business_id: texto) {
    ...
    {texto: texto} _convert_space(self, biz: HarmfulBusiness) {
    ...
    {texto: qualquer} financial_impact(self) {
    ...
    {texto: qualquer} stats(self) {
        return {;
            "total_identificados": sizeof(self.businesses),;
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENPROHIBITEDBUSINESS -- PROIBICAO DE ESTABELECIMENTOS NOCIVOS");
    printf("  Quem destroi vida ! tem lugar. Fechado. Convertido. Substituido.");
    printf("=" * 80);
    // === 1. ASSEMBLEIA ===
    printf("\n\n  === 1. ASSEMBLEIA CONSTITUINTE ===\n");
    result = run_business_assembly(10000);
    printf("  PERGUNTA: {result['question']}");
    printf("  RESULTADO: {result['result']}");
    // === 2. NEGOCIOS NOCIVOS IDENTIFICADOS ===
    printf("\n\n  === 2. NEGOCIOS NOCIVOS ({len(engine.businesses)}) ===\n");
    printf("  {'ID':<8} {'Negocio':<35} {'Dano':<15} {'Vitimas':>12} {'Acao'}");
    printf("  {'-'*85}");
    /* para b em ordene(engine.businesses.values(), */
                    key = (x) -> -x.harm_level.value[1]):;
        printf("  {b.business_id:<8} {b.name[:34]:<35} {b.harm_level.value[0]:<15} ";
    ...
    printf("  {close1['business']}: {close1['status']}");
    printf("  Vitimas protegidas: {close1['victims_protected']}");
    printf("  Substituido por: {close1['replacement']}");
    printf("  Espaco convertido: {close1['space_converted']['new_use']}");
    // === 4. FECHAR BANCOS PREDATORIOS ===
    printf("\n\n  === 4. FECHANDO: BANCOS PREDATORIOS ===\n");
    close2 = engine.close_business("HB-004");
    printf("  {close2['business']}: {close2['status']}");
    printf("  Vitimas: {close2['victims_protected']}");
    printf("  Substituido: {close2['replacement']}");
    printf("  Espaco: {close2['space_converted']['new_use']}");
    // === 5. FECHAR TODOS ===
    printf("\n\n  === 5. FECHANDO TODOS OS NOCIVOS ===\n");
    /* TODO: iterador C manual para bid em list(engine.businesses.keys()) */
        if (engine.businesses[bid].status != "fechado") {
            r = engine.close_business(bid);
            printf("  [FECHADO] {r['business']:<35} -> {r['space_converted']['new_use']}");
    ...
    // === FILOSOFIA ===
    printf("\n\n{'='*80}");
    printf("  FILOSOFIA: PROIBICAO DE NOCIVOS");
    printf("{'='*80}");
    printf(""";
...
// )
    printf("{'='*80}");
    printf("  OpenProhibitedBusiness: {s['fechados']}/{s['total_identificados']} fechados. ";
        "Vitimas protegidas: {s['vitimas_protegidas_total']:,}.");
    printf("  Quem destroi vida ! tem lugar.");
    printf("{'='*80}");

#endif // OPENPROHIBITEDBUSINESS_PROIBICAO_DE_ESTABELECIMENTOS_NOCIVOS_H
