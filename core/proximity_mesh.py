...
================================================================
"Hardware se conhece pelo corpo, not pelo IP."
Dois dispositivos da Republica, quando se aproximam, se comunicam
INDEPENDENTEMENTE de WiFi, internet, or qualquer infraestrutura.
O PROBLEMA:
...
O conteudo and o mesmo. O canal and indiferente.
...
class ChannelSpec:
    ...
    directional: logico // and direcional?
    needs_pairing: logico // precisa pareamento manual?
    penetrate_walls: logico // atravessa parede?
    secure: logico // and criptografado nativamente?
CHANNEL_SPECS: {Channel: ChannelSpec} = {
...
class Device:
    ...
    device_id: texto
    name: texto
    device_type: str = ""  // fone, oculos, terminal, tablet, ring, glove
    # Canais suportados
    channels: {Channel} = field(default_factory=set)
    # Posicao (para simulacao de proximidade)
    pos: Tuple[flutuante, flutuante, flutuante] = (0, 0, 0)
    # Estado
    online: bool = True
    battery_pct: float = 80.0
    # Identidade
    owner: str = ""
    trusted: {texto} = field(default_factory=set) // IDs de dispositivos confiaveis
class ProximityMesh:
    ...
    # decorador: @staticmethod
    def _context_label(dist: flutuante, channel: Channel) -> str:
        if dist <= 0.05 and channel == Channel.NFC:
            return "TOQUE (intencional, confirmar)"
        if dist <= 0.1 and channel == Channel.BODY:
            return "CORPO (mesma pessoa tocando)"
        if dist <= 2 and channel == Channel.MAGNETIC:
...
if __name__ == "__main__":
    print("=" * 75)
    print("  COMUNICACAO POR PROXIMIDADE (PROXIMITY MESH)")
    print("  'Hardware se conhece pelo corpo, not pelo IP.'")
    print("=" * 75)
    mesh = ProximityMesh()
    # === 1. Channels ===
    print("\n\n  === CANAIS DE COMUNICACAO ===\n")
    print("  {'Canal':<15} {'Alcance':>8} {'Banda':>10} {'Latencia':>9} "
        "{'Energia':>8} {'Parede':>6} {'Setup'}")
    print("  {'-'*75}")
    for each (ch, spec) in CHANNEL_SPECS.items():
        wall = spec.penetrate_walls ? "sim" : "not"
        setup = not  spec.needs_pairing ? "auto" : "manual"
        print("  {ch.value:<15} {spec.range_m:>7.2f}m {spec.bandwidth_kbps:>8.0f}kb/s "
    ...
    for d in devices:
        mesh.register(d)
        print("  {d.name:<20} ({d.device_type}) @ {d.pos} | "
    ...
    para label, a, b in scenarios:
        ...
        print("\n  {label}:")
        print("    Distancia: {result['distance_m']}m")
        print("    Canais disponiveis: {len(result['available_channels'])}")
        if result["available_channels"]:
            for ch in result["available_channels"][:3]:
                print("      {ch['channel']:<12} {ch['bandwidth']:.0f}kb/s "
                    "{ch['latency']:.0f}ms {ch['power']:.0f}mW")
            print("    MELHOR: {result['best_channel']}")
        else:
            print("    Fora de alcance de todos os canais")
    # === 4. Transfers ===
    print("\n\n  === TRANSFERENCIAS ===\n")
    transfers = [
    ...
    para label, a, b, payload in transfers:
        ...
        if result.get("ok"):
            ...
            print("    Canal: {result['channel']}{fb}")
            print("    Tamanho: {result['payload_size_bytes']} bytes")
            print("    Banda: {result['bandwidth_kbps']:.0f} kb/s")
            print("    Tempo: {result['transfer_time_ms']}ms")
            print("    Contexto: {result['method']}")
        else:
...
1 canal por vez (WiFi or BT) TODOS os canais simultaneamente
...
Internet para vizinho Vizinho and direto, sem internet
8 CANAIS QUE O HARDWARE USA:
    ...
    -> quando BLE and bloqueado (agua, metal)
...
A rede and fisica. A rede and real.
