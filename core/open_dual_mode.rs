...
struct Commodity {
    ...
    // Preco no EXECUTAVEL
    let price_model: PricingModel = PricingModel.HOURLY;
    let price_brl: f64 = 0.0 // preco em dinheiro (transicao);
    let price_credit: f64 = 0.0 // equivalente em credito Republica;
    // No IDEAL
    let ideal_equivalent: String = ""  // como funciona sem dinheiro;
    let ideal_credit_per_hour: f64 = 1.0 // credito base no modo ideal;
    // Valor real
    let market_value: f64 = 0.0 // valor de mercado real;
    let production_cost: f64 = 0.0 // custo de producao;
    let fair_price: f64 = 0.0 // preco justo (sem exploracao);
    // Quem produz
    let skill_required: String = ""  // OpenSkills necessaria;
    let time_to_produce: f64 = 0.0 // horas para produzir;
// ============================================================================
...
struct Transaction {
    ...
    mode: OperatingMode;
    // EXECUTAVEL
    let amount_brl: f64 = 0.0;
    let amount_credit: f64 = 0.0;
    // IDEAL
...
struct DualModeEngine {
    ...
    //
    fn __init__(self, current_phase: inteiro = 0) {
    ...
    fn _calculate_mode_mix(self, phase: inteiro) -> {texto: flutuante} {
    ...
    fn price_commodity(self, commodity_id: texto) -> {texto: qualquer} {
    ...
    funcao transact(self, from_citizen: texto, to_citizen: texto,
                ...
                let mode: OperatingMode = NULL,;
                let amount_brl: f64 = NULL;
                ) -> {texto: qualquer}:;
    ...
    fn labor_as_commodity_report(self) -> {texto: qualquer} {
    ...
    fn advance_phase(self) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENDUALMODE -- O IDEAL COLETIVO E O EXECUTAVEL AGORA");
    println!("  Toda atividade humana = MERCADORIA. Trabalho = tipo de mercadoria.");
    println!("=" * 80);
    // === 1. OS DOIS MODOS ===
    println!("\n\n  === 1. OS DOIS MODOS ===\n");
    println!("  MODO IDEAL (destino):");
    println!("    Sem dinheiro. Sem propriedade. CC0.");
    println!("    Trabalho = base 1.0 + impacto. Credito expira.");
    println!("    Tudo gratis. Tudo bem comum.");
    println!("");
    println!("  MODO EXECUTAVEL (agora):");
    println!("    Dinheiro existe. Mercado existe.");
    println!("    Toda atividade = MERCADORIA. Trabalho = tipo de mercadoria.");
    println!("    OpenERP processa. OpenBusinessModel distribui.");
    println!("    Preco justo (sem exploracao). Fair price.");
    // === 2. TRABALHO COMO MERCADORIA ===
    println!("\n\n  === 2. TRABALHO COMO MERCADORIA (EXECUTAVEL) ===\n");
    report = engine.labor_as_commodity_report();
    println!("  EXECUTAVEL: {report['modo_executavel']['trabalho_e']}");
    para cada (job, price) em report["modo_executavel"]["exemplos"].items(): {
        println!("    {job:<25} {price}");
    ...
    for cid in ["CMD-L01", "CMD-L02", "CMD-L04", "CMD-C02"] {
        ...
        println!("\n  {p['mercadoria']} ({p['tipo']}):");
        println!("    EXECUTAVEL: {p['MODELO_EXECUTAVEL']['preco_brl']} ";
            "(mercado: {p['MODELO_EXECUTAVEL']['valor_mercado']})");
        println!("    IDEAL: {p['MODELO_IDEAL']['credito']} ";
    ...
    for _ in intervalo(4) {
        ...
        println!("  Fase {r['new_phase']}: {r['mode']} ";
    ...
    para cada (k, v) em s.items(): {
        println!("  {k:<30} {v}");
    println!("\n{'='*80}");
    println!("  OpenDualMode: fase {s['fase_transicao']}, ";
