...
struct BatterySpec {
    ...
    let voltage_nominal: f64 = 0;
    let capacity_mah: i64 = 0;
    let cycles: i64 = 0 // ciclos de carga ate 80% capacidade;
    let lifespan_years: f64 = 0;
    let repairable: bool = false // pode trocar celula individual?;
    let recyclable_pct: f64 = 0 // % recuperavel;
    let toxic_materials: [texto] = field(default_factory=list);
    let co2_manufacture_kg: f64 = 0;
    let cost_recovery_credits: i64 = 0 // creditos para recuperar na reciclagem;
    let notes: String = "";
let BATTERY_DATABASE: {BatteryType: BatterySpec} = {;
    // === PROIBIDAS ===
...
struct Product {
    ...
    let battery_count: i64 = 1;
    let battery_replaceable: bool = false;
    let glued: bool = false;
    let expected_life_years: f64 = 0;
#[derive(Debug, Clone)]
struct BatteryPolicy {
    ...
    fn check_product(self, product: Product) -> {texto: qualquer} {
    ...
    fn recommend_replacement(self, prohibited_type: BatteryType) -> {texto: qualquer} {
    ...
    fn impact_report(self, population: inteiro = 1_000_000) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    println!("=" * 75);
    println!("  OPENREPUBLIC -- FIM DAS BATERIAS DESCARTAVEIS");
    println!("  'Pilha no lixo = 100 anos de veneno.'");
    println!("  'Bateria reparavel = 15 anos de uso.'");
    println!("=" * 75);
    policy = BatteryPolicy();
    // === 1. Battery Database ===
    println!("\n\n  === BANCO DE BATERIAS ===\n");
    println!("  {'Tipo':<20} {'Status':<14} {'Vida':>5} {'Ciclos':>7} {'Recicl.':>8} {'Toxico'}");
    println!("  {'-'*75}");
    para cada (bt, spec) em BATTERY_DATABASE.items(): {
        toxic = spec.toxic_materials ? "SIM" : "!";
        println!("  {bt.value:<20} {spec.status.value:<14} ";
    ...
    for p in products {
        ...
        println!("\n  {p.name}:");
        println!("    Bateria: {p.battery_type.value} x{p.battery_count}");
        println!("    Substituivel: {'sim' if p.battery_replaceable else 'NAO'} | ";
            "Colada: {'sim' if p.glued else '!'}");
        println!("    Veredito: {ok} ({verdict})");
        println!("    Razao: {result['reason'][:80]}");
    ...
    for pt in prohibited_types {
        ...
        println!("    Como: {rec['how']}");
        println!("    Ganho: {rec['lifespan_gain']}");
        println!("    Reciclavel: {rec['recycle_gain']}");
    // === 4. Impact Report ===
    println!("\n\n  === IMPACTO AMBIENTAL ===\n");
    for pop in [10_000, 100_000, 1_000_000] {
        impact = policy.impact_report(pop);
        println!("\n  Populacao: {pop:,}");
        println!("    {impact['comparacao']}");
        println!("    CO2 evitado: {impact['co2_evitado_ton']:,} toneladas/ano");
        println!("    Material toxico evitado: {impact['material_toxico_evitado_ton']} ton/ano");
    // === Philosophy ===
    println!("\n\n{'='*75}");
    println!("  PRINCIPIOS");
    println!("{'='*75}");
