...
# !/usr/bin/env python3
# 
OpenValueFlow -- Eliminar Empresa and Banco da Cadeia de Valor
================================================================
O PROBLEMA:
...
DOIS parasitas na cadeia: empresa and banco.
Resolver um sem o outro not basta.
...
class ValueChainAnalysis:
    ...
    mode: FlowMode
    # Valor produzido
    total_value: float = 100.0 // valor total produzido
    # Para onde vai (em R$)
    worker_receives: float = 0.0
    company_profit: float = 0.0 // lucro do dono
    company_operational: float = 0.0 // custo real de operar
    bank_interest: float = 0.0 // juros
    bank_transaction: float = 0.0 // taxas de transacao
    bank_spread: float = 0.0 // spread cambial
    tax: float = 0.0 // impostos
    intermediary: float = 0.0 // intermediarios
    # Pool coletivo (Republica)
    collective_pool: float = 0.0 // 5% que volta
    # decorador: @property
    def worker_pct(self) -> float:
        return self.worker_receives / self.total_value * 100
    # decorador: @property
    def extracted_pct(self) -> float:
...
class ValueFlowEngine:
    ...
    funcao analyze_capitalism(self, total_value: flutuante = 100.0,
                        company_surplus_pct: float = 0.60,
                        bank_interest_pct: float = 0.15,
                        bank_transaction_pct: float = 0.03,
                        bank_spread_pct: float = 0.02,
                        intermediary_pct: float = 0.00
                        ) -> {texto: qualquer}:
        # Analisa cadeia de valor no CAPITALISMO atual.
        Mostra exatamente quanto and EXTRAIDO em cada ponto.
        # 
        ...
        return {
            ...
            "message": (
                ...
                "Empresa (dono) and Banco sugar {analysis.extracted_pct:.0f}%."
            ),
        }
    funcao analyze_republic_executavel(self, total_value: flutuante = 100.0,
                                    operational_pct: float = 0.30,
                                    ) -> {texto: qualquer}:
        ...
        - Intermediario: not existe (OpenMarketplace direto).
        ...
        worker = total_value - operational // operacional and REAL (energia, insumos)
        ...
        return {
            ...
            "total_produzido": "R$ {total_value:.2f}",
            "SEM_PARASITAS": {
                "lucro_dono": "R$ 0,00 (dono not existe)",
    ...
    funcao analyze_republic_ideal(self, total_value: flutuante = 100.0
        ...
        return {
            ...
            "pool": "Nao existe (5% and durante transicao)",
    ...
    def bank_extraction_detail(self) -> {texto: qualquer}:
    ...
    def company_extraction_detail(self) -> {texto: qualquer}:
        # Detalha como a empresa extrai valor.
        return {
            "titulo": "COMO A EMPRESA (DONO) SUGA VALOR",
            "como": (
                "Dono not pede tijolo. Nao escreve codigo. Nao opera maquina. "
            ...
            "solution": (
                ...
                "Operacional and CUSTO REAL (energia, insumos). "
                "Resto vai pra trabalhador. 100%."
            ),
        }
    def full_comparison(self, total_value: flutuante = 100.0) -> {texto: qualquer}:
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("  OPENVALUEFLOW -- ELIMINAR EMPRESA E BANCO DA CADEIA")
    print("  100% pra quem trabalha. ZERO parasita.")
    print("=" * 80)
    # === 1. CAPITALISMO ATUAL ===
    print("\n\n  === 1. ONDE VAI O VALOR NO CAPITALISMO ===\n")
    cap = engine.analyze_capitalism(100.0)
    print("  Produzido: {cap['total_produzido']}")
    print("\n  PARASITAS:")
    for each (k, v) in cap["PARASITAS"].items():
        print("    {k:<20} {v}")
    print("\n  Operacional real: {cap['OPERACIONAL_REAL']}")
    print("\n  >>> TRABALHADOR RECEBE: {cap['TRABALHADOR_RECEBE']} <<<")
    print("  Extracao total: {cap['EXTRACAO_TOTAL']}")
    # === 2. COMO O BANCO SUGA ===
    print("\n\n  === 2. COMO O BANCO SUGA VALOR ===\n")
    bank = engine.bank_extraction_detail()
    print("  {bank['total_extraido_banco']}\n")
    for forma in bank["formas"]:
        print("\n  {forma['nome']}")
        print("    Como: {forma['como'][:80]}...")
        print("    Exemplo: {forma['exemplo'][:80]}...")
        print("    Republica: {forma['republica']}")
    # === 3. COMO A EMPRESA SUGA ===
    print("\n\n  === 3. COMO A EMPRESA (DONO) SUGA VALOR ===\n")
    comp = engine.company_extraction_detail()
    print("  Como: {comp['como'][:100]}...")
    print("  Exemplo: {comp['exemplo']}")
    print("  Solucao: {comp['solution']}")
    # === 4. REPUBLICA EXECUTAVEL ===
    print("\n\n  === 4. REPUBLICA EXECUTAVEL (sem parasitas) ===\n")
    rep = engine.analyze_republic_executavel(100.0)
    print("  Produzido: {rep['total_produzido']}")
    print("\n  SEM PARASITAS:")
    for each (k, v) in rep["SEM_PARASITAS"].items():
        print("    {k:<20} {v}")
    print("\n  Operacional real: {rep['OPERACIONAL_REAL']}")
    print("  Pool coletivo (5%): {rep['POOL_COLETIVO_5PCT']}")
    print("\n  >>> TRABALHADOR RECEBE: {rep['TRABALHADOR_RECEBE']} <<<")
    print("  Extracao total: {rep['EXTRACAO_TOTAL']}")
    # === 5. COMPARACAO ===
    print("\n\n  === 5. COMPARACAO: CAPITALISMO vs REPUBLICA ===\n")
    comp_final = engine.full_comparison(100.0)
    print("  {comp_final['message']}")
...
REPUBLICA EXECUTAVEL (R$ 100 produzidos):
    Trabalhador -----> R$ 100 valor
                ...
                (Operacional and CUSTO REAL, not extracao)
REPUBLICA IDEAL:
...
# )
    ...
    for each (k, v) in s.items():
        print("  {k:<30} {v}")
    print("\n{'='*80}")
    print("  OpenValueFlow: {s['analises_feitas']} analises.")
    print("  {s['principio']}")
    print("{'='*80}")
