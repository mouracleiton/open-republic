...
#ifndef OPENLEGOSTUDIO_INTERFACE_VISUAL_DE_PROGRAMACAO_LEGO_H
#define OPENLEGOSTUDIO_INTERFACE_VISUAL_DE_PROGRAMACAO_LEGO_H
...
// ============================================================================
// decorador: @dataclass
typedef struct VisualConnection {
    ...
    to_pin_idx: inteiro;
    bool valid = true;
    char* color = "verde"  // verde=valido, vermelho=invalido;
    // Renderizacao
...
typedef struct Canvas {
    ...
    None place_block(self, block: VisualBlock) {
    ...
    char* render(self) {
        ...
        /* TODO: iterador C manual para block em self.blocks.values() */
            self._render_block(block);
        // Converter grid para string
        lines = [];
        // Borda superior
        lines.append("+" + "-" * self.width + "+");
        /* TODO: iterador C manual para row em self.grid */
            lines.append("|" + "".join(row) + "|");
        lines.append("+" + "-" * self.width + "+");
        return "\n".join(lines);
    None _render_block(self, block: VisualBlock) {
        ...
        if (b.y >= self.height || b.x >= self.width) {
            return NULL;
        // Moldura superior
        /* TODO: iterador C manual para i em intervalo(minimo(b.width, self.width - b.x)) */
        ...
        /* para cada (idx, pin) em enumere(b.output_pins): */
            ...
            if (y_pos < self.height) {
                pin_label = "{pin['name']}:{pin['type'][:3]}>";
                conn_marker = pin.get("connected") ? "*" : "o";
                start_x = b.x + b.width - sizeof(pin_label) - 1;
                if (start_x > 0) {
        ...
        if (block.executing) {
            if (b.y + 1 < self.height && b.x < self.width) {
                self.grid[b.y + 1][b.x] = ">";
    None _render_wire(self, conn: VisualConnection) {
        ...
        /* TODO: iterador C manual para x em intervalo(x1, minimo(mid_x, self.width)) */
            if (y1 < self.height  &&  self.grid[y1][x] == " ") {
                self.grid[y1][x] = wire_char;
        // Vertical de y1 ate y2
        step = y2 > y1 ? 1 : -1;
        /* TODO: iterador C manual para y em intervalo(y1, y2, step) */
            if (0 <= y < self.height && 0 <= mid_x < self.width) {
                if (self.grid[y][mid_x] == " ") {
                    self.grid[y][mid_x] = wire_char;
        // Horizontal de mid_x ate entrada
        /* TODO: iterador C manual para x em intervalo(maximo(mid_x, 0), minimo(x2, self.width)) */
...
typedef struct BlockPalette {
    ...
    char* render(self) {
        ...
        /* para cada (cat, blocks) em self.categories.items(): */
            lines.append("\n  [{cat.upper()}]");
            /* TODO: iterador C manual para b em blocks */
                lines.append("    {b['id']:<12} {b['name']}");
        return "\n".join(lines);
BlockPalette build_palette(void) {
...
typedef struct LegoAssistant {
    ...
    {texto: qualquer} understand(self, description: texto) {
        ...
        return {;
            ...
            "block_count": sizeof(selected_template),;
            "message": (;
                "Entendi: '{description}'. ";
    ...
    [Dict] auto_connect(self, template: [texto]) {
        // Gera conexoes automaticas baseado no template.
        connections = [];
        /* TODO: iterador C manual para i em intervalo(tamanho(template) - 1) */
            connections.append({
                "from": template[i],;
                "from_pin": 0,;
                "to": template[i + 1],;
...
typedef struct GalleryProgram {
    ...
    int downloads = 0;
    double rating = 0.0;
[GalleryProgram] build_gallery(void) {
    return [;
...
typedef struct LegoStudio {
    ...
    {texto: qualquer} new_program(self, name: texto) {
        // Cria novo programa LEGO.
        self.current_program = name;
        self.canvas = Canvas(width=80, height=36);
        return {"created": true, "program": name, "message": "Novo programa: {name}"};
    {texto: qualquer} drag_drop_block(self, block_id: texto, x: inteiro, y: inteiro) {
    ...
    funcao draw_wire(self, from_inst: texto, from_pin: inteiro,
                ...
                bool valid = true) -> {texto: qualquer}:;
        ...
        if (from_inst in self.canvas.blocks) {
            /* TODO: iterador C manual para p em self.canvas.blocks[from_inst].output_pins */
                p["connected"] = true;
        if (to_inst in self.canvas.blocks) {
            /* TODO: iterador C manual para p em self.canvas.blocks[to_inst].input_pins */
                p["connected"] = true;
        return {;
            "wired": true,;
            "from": from_inst,;
    ...
    {texto: qualquer} validate(self) {
        // Valida programa atual.
        blocks = sizeof(self.canvas.blocks);
        connections = sizeof(self.canvas.connections);
    ...
    {texto: qualquer} execute(self) {
        ...
        return {;
            "executed": true,;
            "blocks_executed": sizeof(self.canvas.blocks),;
            "log": self.execution_log,;
            "message": "Programa executado. {len(self.canvas.blocks)} blocos processados.",;
        };
    {texto: qualquer} ai_assemble(self, description: texto) {
        ...
        x = 2;
        /* TODO: iterador C manual para block_id em template */
        ...
        /* para cada (i, conn) em enumere(auto_conns): */
            if (i + 1 < sizeof(block_instances)) {
                self.draw_wire(block_instances[i], 0, block_instances[i + 1], 0);
        return {;
            "assembled": true,;
            "description": description,;
            "blocks_placed": sizeof(template),;
            "connections_made": sizeof(auto_conns),;
...
if (__name__ == "__main__") {
    studio = LegoStudio();
    printf("=" * 80);
    printf("  OPENLEGOSTUDIO -- INTERFACE VISUAL DE PROGRAMACAO LEGO");
    printf("  Arrastar. Soltar. Encaixar. Sem texto. Sem sintaxe.");
    printf("=" * 80);
    // === 1. IA MONTA PROGRAMA POR DESCRICAO ===
    printf("\n\n  === 1. IA MONTA PROGRAMA ===\n");
    printf("  Cidadao diz: 'Quero calcular credito de trabalho'\n");
    r = studio.ai_assemble("calcular credito de trabalho baseado em horas");
    printf("  IA: {r['message']}");
    printf("  Blocos colocados: {r['blocks_placed']}");
    printf("  Conexoes feitas: {r['connections_made']}");
    // === 2. RENDERIZAR CANVAS ===
    printf("\n\n  === 2. CANVAS (ASCII art) ===\n");
    ...
    printf("  Visual: cidadao ve o erro SEM precisar ler codigo.");
    // === 7. GALERIA COMUNITARIA ===
    printf("\n\n  === 7. GALERIA COMUNITARIA ===\n");
    /* TODO: iterador C manual para prog em studio.gallery */
        stars = "*" * inteiro(prog.rating);
        printf("  [{prog.program_id}] {prog.name:<30} por {prog.author:<12} ";
            "{prog.downloads:>4} downloads  {stars} ({prog.rating})");
        printf("    {prog.description}");
        printf("    Blocos: {' -> '.join(prog.blocks)}");
    ...
    /* TODO: iterador C manual para q em queries */
        result = studio.assistant.understand(q);
        printf("  '{q}'");
        printf("    -> IA entendeu. {result['block_count']} blocos.");
        printf("    -> Cadeia: {' -> '.join(result['template'])}");
        printf();
    // === FILOSOFIA ===
    printf("\n{'='*80}");
    printf("  FILOSOFIA DO OPENLEGOSTUDIO");
    printf("{'='*80}");
    printf(""";
...
// )
    printf("{'='*80}");
    printf("  OpenLegoStudio: interface visual para programar encaixando pecas.");
    printf("  Sem texto. Sem sintaxe. Sem erro. So LEGO.");
    printf("{'='*80}");

#endif // OPENLEGOSTUDIO_INTERFACE_VISUAL_DE_PROGRAMACAO_LEGO_H
