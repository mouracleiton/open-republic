...
package openhandtracking_interface_de_games_com_webcam_hand_tracking
...
type HandState struct {
    ...
    detected := false // bool
    gesture := Gesture.OPEN_HAND // Gesture
    landmarks := field(default_factory=list) // List[(inteiro, inteiro)]
    center := (0, 0) // (inteiro, inteiro)
    wrist_angle := 0.0 // float64
    fingers_up := field(default_factory=() -> [false]*5) // [logico]
    pinch_distance := 0.0 // float64
    prev_center := (0, 0) // (inteiro, inteiro)
...
type HandTracker struct {
    ...
    funcao process(self, frame: np.ndarray) retorna Tuple[np.ndarray, HandState]:
        ...
        if results.multi_hand_landmarks {
            for _, hand_lms := range results.multi_hand_landmarks {
                self.state.detected = true
                // Converter landmarks para pixels
                landmarks = []
                for _, lm := range hand_lms.landmark {
                    px = inteiro(lm.x * w)
                    py = inteiro(lm.y * h)
    ...
    funcao _detect_fingers(self, lms: List[(inteiro, inteiro)],
        ...
        return fingers
    func _classify_gesture(self) Gesture {
...
type VirtualUI struct {
    ...
    type Button struct {
        ...
        label: texto
        color := (100, 100, 100) // Tuple[inteiro, inteiro, inteiro]
        hover_color := (0, 255, 0) // Tuple[inteiro, inteiro, inteiro]
        pressed := false // bool
        on_click := nil // interface{}
    // decorador: @dataclass
    type GameObject struct {
        ...
        vx := 0.0 // float64
        vy := 0.0 // float64
    func __init__(self, width: inteiro = 640, height: inteiro = 480) {
        self.width = width
        self.height = height
    ...
    func update(self, hand: HandState) {
        ...
        if hand.detected {
            ...
            for _, obj := range self.objects {
                obj.grabbed = false
    func _update_menu(self, hand: HandState) {
        ...
        for _, btn := range self.buttons {
            ...
            if btn.pressed && btn.on_click && ! None {
                btn.on_click()
    func _update_catch(self, hand: HandState) {
        ...
        for len(self.objects) < 5 {
            self._spawn_fruit()
    func _update_paint(self, hand: HandState) {
    ...
    func _update_free(self, hand: HandState) {
...
type HandTrackingApp struct {
    ...
    //
    func __init__(self, camera_index: inteiro = 0) {
    ...
    func run(self) {
        ...
        if ! cap.isOpened() {
            fmt.Println("ERRO: webcam ! disponivel")
            return nil
        // Configurar camera
        ...
        self.running = true
        fmt.Println("=" * 50)
        fmt.Println("  OpenHandTracking")
        fmt.Println("  Mostre a mao para a camera.")
        fmt.Println("  PINCA/PUNHO = clicar. ESC = sair. M = menu.")
        fmt.Println("=" * 50)
        prev_time = time.time()
        for self.running {
