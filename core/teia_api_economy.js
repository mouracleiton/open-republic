...
class ContributionType {
    ...
    __init__(self, key: texto, description: texto, default_weight: flutuante) {
        self.key = key;
...
class APIEndpoint {
    ...
    // Precificacao
    const price_per_call = 0.05 // R$ por chamada;
    const free_tier_daily = 10 // chamadas gratis por dia;
    // Contribuidores (CADEIA DE PROVENIENCIA)
    const contributions = field(default_factory=list);
    // Metricas
    const calls_today = 0;
    const calls_total = 0;
    const revenue_total = 0.0;
    // decorador: @property
    n_contributors(self) {
        return .length(set(c.contributor_id para c em self.contributions));
    // decorador: @property
    total_weight(self) {
        return soma(c.effective_weight para c em self.contributions);
    contributor_share(self, contributor_id: texto) {
    ...
    funcao revenue_split(self) retorna List[{texto: qualquer}]:
        ...
        const contributor_weights = defaultdict(flutuante);
        const contributor_names = {};
        const contributor_roles = defaultdict(list);
        for (const c of self.contributions) {
...
class TEIAAPI {
    ...
    call(self, endpoint_id: texto, n_calls: inteiro = 1) {
...
class RoyaltyTracker {
    ...
    register_calls(self, endpoint_id: texto, n_calls: inteiro) {
        ...
        self.call_log.append({
            ...
            "timestamp": .length(self.call_log),;
        });
        return result;
    register_daily_traffic(self, calls_per_endpoint: {texto: inteiro}) {
        // Simula um dia de trafego.
        para cada (eid, n) em calls_per_endpoint.items(): {
            self.register_calls(eid, n);
    earnings_report(self) {
        ...
        return "\n".join(lines);
    revenue_per_endpoint_detail(self) {
