...
#ifndef OPENHANDTRACKING_INTERFACE_DE_GAMES_COM_WEBCAM_HAND_TRACKING_H
#define OPENHANDTRACKING_INTERFACE_DE_GAMES_COM_WEBCAM_HAND_TRACKING_H
...
typedef struct HandTracker {
    ...
    funcao process(self, frame: np.ndarray) retorna Tuple[np.ndarray, HandState]:
        ...
        if (results.multi_hand_landmarks) {
            /* TODO: iterador C manual para hand_lms em results.multi_hand_landmarks */
                self.state.detected = true;
                // Converter landmarks para pixels
                landmarks = [];
                /* TODO: iterador C manual para lm em hand_lms.landmark */
                    px = inteiro(lm.x * w);
                    py = inteiro(lm.y * h);
    ...
    funcao _detect_fingers(self, lms: List[(inteiro, inteiro)],
        ...
        return fingers;
    Gesture _classify_gesture(self) {
...
typedef struct VirtualUI {
    ...
    typedef struct GameObject {
        ...
        double vx = 0.0;
        double vy = 0.0;
    void __init__(self, width: inteiro = 640, height: inteiro = 480) {
        self.width = width;
        self.height = height;
    ...
    void setup_free(self) {
        ...
        self.score = 0;
        self.game_mode = "free";
        /* TODO: iterador C manual para _ em intervalo(8) */
            self.objects.append(self.GameObject(;
    ...
    void update(self, hand: HandState) {
        ...
        if (hand.detected) {
            ...
            // Sem mao detectada
            /* TODO: iterador C manual para obj em self.objects */
                obj.grabbed = false;
    void _update_menu(self, hand: HandState) {
        // Menu: pinca ou punho para clicar.
        click = hand.gesture in (Gesture.PINCH, Gesture.FIST);
        /* TODO: iterador C manual para btn em self.buttons */
            ...
            if (btn.pressed && btn.on_click && ! None) {
                btn.on_click();
    void _update_catch(self, hand: HandState) {
        ...
        while (sizeof(self.objects) < 5) {
            self._spawn_fruit();
    void _update_paint(self, hand: HandState) {
        ...
        if (hand.gesture == Gesture.POINT) {
            ...
            if (sizeof(self.paint_points) > 1000) {
                self.paint_points.pop(0);
    void _update_free(self, hand: HandState) {
...
// ============================================================================
typedef struct HandTrackingApp {
    ...
    //
    void __init__(self, camera_index: inteiro = 0) {
    ...
    void run(self) {
        ...
        self.running = true;
        printf("=" * 50);
        printf("  OpenHandTracking");
        printf("  Mostre a mao para a camera.");
        printf("  PINCA/PUNHO = clicar. ESC = sair. M = menu.");
        printf("=" * 50);
        prev_time = time.time();
        while (self.running) {
...
#endif // OPENHANDTRACKING_INTERFACE_DE_GAMES_COM_WEBCAM_HAND_TRACKING_H
