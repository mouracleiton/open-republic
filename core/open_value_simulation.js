...
class ValueSimulator {
    ...
    simulate_worker(self, worker_id: texto) {
    ...
    _life_impact(self, sobra_cap: flutuante, sobra_rep: flutuante) {
    ...
    simulate_all(self) {
        ...
        total_workers = .length(self.workers);
        for (const wid of self.workers) {
    ...
    aggregate_stats(self) {
        ...
        return {;
            "trabalhadores": .length(self.workers),;
...
if (__name__ == "__main__") {
    sim = ValueSimulator();
    console.log("=" * 80);
    console.log("  OPENVALUESIMULATION -- PESSOAS REAIS, SALARIOS REAIS");
    console.log("  Nomes obfuscados. Dados reais do mercado brasileiro.");
    console.log("=" * 80);
    // === 1. CADA TRABALHADOR ===
    console.log("\n\n  === SIMULACAO POR TRABALHADOR ===\n");
    for (const wid of ordene(sim.workers.keys())) {
        r = sim.simulate_worker(wid);
        w = sim.workers[wid];
        console.log("\n  {'='*70}");
        console.log("  {r['alias']} | {r['profissao']} | {r['empresa']} | {r['local']}");
        console.log("  Familia: {r['familia']} | Horas/mes: {r['horas_mes']}");
        console.log("  {'='*70}");
        console.log("\n  CAPITALISMO (hoje):");
        console.log("    Produz:           {r['CAPITALISMO']['produz']}");
        console.log("    Recebe:           {r['CAPITALISMO']['recebe']} ({r['CAPITALISMO']['fica_com']})");
        console.log("    Empresa suga:     {r['CAPITALISMO']['empresa_suga']}");
        console.log("    Banco suga:       {r['CAPITALISMO']['banco_suga']}");
        console.log("    Intermediario:    {r['CAPITALISMO']['intermediario_suga']}");
        console.log("    TOTAL EXTRAIDO:   {r['CAPITALISMO']['total_extraido']}");
        console.log("    Razao:            {r['CAPITALISMO']['razao_exploracao']}");
        console.log("    Hora vale:        {r['CAPITALISMO']['hora_valor']}");
        console.log("    Sobra (apos ali+comida): {r['CAPITALISMO']['sobra_apos_aluguel_comida']}");
        console.log("\n  REPUBLICA EXECUTAVEL:");
        console.log("    Produz:           {r['REPUBLICA_EXECUTAVEL']['produz']}");
        console.log("    Recebe:           {r['REPUBLICA_EXECUTAVEL']['recebe']} ({r['REPUBLICA_EXECUTAVEL']['fica_com']})");
        console.log("    Empresa suga:     {r['REPUBLICA_EXECUTAVEL']['empresa_suga']}");
        console.log("    Banco suga:       {r['REPUBLICA_EXECUTAVEL']['banco_suga']}");
        console.log("    Intermediario:    {r['REPUBLICA_EXECUTAVEL']['intermediario_suga']}");
        console.log("    Pool 5%:          {r['REPUBLICA_EXECUTAVEL']['pool_5pct']}");
        console.log("    Operacional real: {r['REPUBLICA_EXECUTAVEL']['operacional_real']}");
        console.log("    Hora vale:        {r['REPUBLICA_EXECUTAVEL']['hora_valor']}");
        console.log("    Sobra (apos ali+comida): {r['REPUBLICA_EXECUTAVEL']['sobra_apos_aluguel_comida']}");
        console.log("\n  GANHO:");
        console.log("    Extra/mes:  {r['GANHO']['extra_por_mes']}");
        console.log("    Extra/ano:  {r['GANHO']['extra_por_ano']}");
        console.log("    Multiplicador: {r['GANHO']['multiplicador']}");
        console.log("    Impacto: {r['GANHO']['mudanca_vida']}");
    // === 2. AGREGADO ===
    console.log("\n\n  {'='*70}");
    console.log("  === AGREGADO: TODOS OS {len(sim.workers)} TRABALHADORES ===");
    console.log("  {'='*70}\n");
    agg = sim.simulate_all();
    console.log("  {agg['message']}");
    console.log("\n  Extracao mensal total: {agg['total_extraido_mensal']}");
    console.log("  Extracao anual total:  {agg['total_extraido_anual']}");
    console.log("  Ganho mensal Republica: {agg['total_ganho_republica_mensal']}");
    console.log("  Ganho anual Republica:  {agg['total_ganho_republica_anual']}");
    // === 3. STATS ===
    console.log("\n\n  === ESTATISTICAS BASE ===\n");
    ...
    console.log("\n\n  === RESUMO (todos) ===\n");
    console.log("  {'Alias':<10} {'Profissao':<25} {'Hoje':>10} {'Republica':>10} {'Ganho':>10} {'X':>5} {'Impacto'}");
    console.log("  {'-'*95}");
    for (const wid of ordene(sim.workers.keys())) {
        r = sim.simulate_worker(wid);
        ...
        console.log("  {r['alias']:<10} {r['profissao'][:24]:<25} ";
            "R${int(hoje):>8,} R${int(rep):>8,} R${int(gain):>8,} ";
            "{mult:>4} {impacto}");
    console.log("\n{'='*80}");
    console.log("  Simulacao: {len(sim.workers)} trabalhadores reais.");
    console.log("  Extracao anual: {agg['total_extraido_anual']}.");
    console.log("  Se virasse Republica: {agg['total_ganho_republica_anual']} voltam.");
    console.log("  Para trabalhadores. Nao parasitas.");
    console.log("{'='*80}");
