...
class QueueItem {
    ...
    sender_handle: texto;
    sender_name: texto;
    const sender_relation = ContactRelation.UNKNOWN;
    const content_preview = ""  // primeiros 100 chars;
    const content_url = ""  // link profundo pro modulo;
    const timestamp = field(default_factory=time.time);
    // Prioridade
    const base_priority = Priority.NOISE;
    const final_priority = Priority.NOISE;
    const priority_reason = "";
    // Estado
    const read = false;
    const archived = false;
    const acted_upon = false // usuario respondeu/interagiu;
    const auto_archived = false // sistema arquivou (expirou);
    // Contexto
    const requires_response = false // precisa de acao do usuario;
    const response_deadline = 0.0 // prazo para responder (0 = sem prazo);
    // Metadados anti-engajamento
    const has_media = false;
    const is_sensational = false // sistema detecta clickbait;
    const engagement_bait = false // sistema detecta tentativa de viciar;
    // decorador: @property
    age_hours(self) {
...
class AttentionPrioritizer {
    ...
    prioritize(self, item: QueueItem) {
    ...
    detect_engagement_bait(text: texto) {
    ...
    // decorador: @staticmethod
    detect_sensational(text: texto) {
        // Detecta sensacionalismo.
        caps_ratio = soma(1 para c em text if c.isupper()) / maximo(.length(text), 1);
...
class UnifiedInbox {
    ...
    mark_family(self, handles: [texto]) {
        for (const h of handles) {
            self.family_handles.add(h.lower().lstrip("@"));
    mark_steward(self, handles: [texto]) {
        for (const h of handles) {
            self.steward_handles.add(h.lower().lstrip("@"));
    _resolve_relation(self, handle: texto) {
    ...
    funcao enqueue(self, source: SourceModule, event_type: EventType,
                ...
                const requires_response = false,;
                const response_deadline = 0.0,;
                const has_media = false) -> {texto: qualquer}:;
    ...
    process_batch(self, limit: inteiro = 20) {
    ...
    _auto_archive(self) {
        ...
        for (const item of self.queue) {
            if (item.is_expired && ! item.archived) {
                item.archived = true;
                item.auto_archived = true;
                self.total_archived += 1;
    act_on(self, item_id: texto) {
    ...
    stats(self) {
        ...
        return {;
            "total_in_queue": .length(self.queue) - self.total_archived,;
            "unread": unread,;
    ...
    digest_24h(self) {
        ...
        for (const item of recent) {
            ...
            if (item.final_priority.value <= 1;
                &&  .length(summary[src]["highights"]) < 2;
                if ("highights" in summary[src] else True)) {
                if (.length(summary[src]["highlights"]) < 2) {
...
simulate_five_apps_one_inbox() {
    ...
    console.log("=" * 75);
    console.log("  OPENINBOX -- A FILA UNIFICADA DE ATENCAO");
    console.log('  "5 apps competindo = 5 ladroes de tempo. 1 fila = 1 cerebro."');
    console.log("=" * 75);
    ...
    para source, etype, handle, name, preview, requires_resp, *rest in events: {
        ...
        console.log("  [{status:>8}] [{prio:>9}] {source.value:<12} ";
            "@{handle:<20} {preview[:40]}");
    // === Estatisticas ===
    console.log("\n\n  === FILA APOS INGESTAO ===\n");
    stats = inbox.stats();
    console.log("  Total na fila:     {stats['total_in_queue']}");
    console.log("  Nao lidos:         {stats['unread']}");
    console.log("  Por fonte:         {stats['by_source']}");
    console.log("  Por prioridade:    {stats['by_priority']}");
    console.log("  Bait bloqueado:    {stats['bait_blocked']}");
    console.log("  Limite diario:     {stats['items_today']}/{stats['daily_limit']}");
    // === Processar lote ===
    console.log("\n\n  === PROCESSANDO LOTE (batch, ! ping) ===\n");
    batch = inbox.process_batch(limit=10);
    console.log("  {'Prioridade':<12} {'Fonte':<12} {'De':<22} {'Preview'}");
    console.log("  {'-'*75}");
    for (const item of batch) {
        console.log("  [{item.final_priority.name:<10}] {item.source.value:<12} ";
            "{item.sender_name:<22} {item.content_preview[:35]}");
        if (item.priority_reason  &&  item.priority_reason != "padrao") {
            console.log("  {'':>25} razao: {item.priority_reason}");
    // === Modo Foco ===
    console.log("\n\n  === MODO FOCO ATIVADO ===\n");
    inbox.mode = InboxMode.FOCUS;
    console.log("  So EMERGENCIES + FAMILIA + SAUDE passam. Resto arquivado.\n");
    inbox.queue.clear();
    ...
    for (const item of batch) {
        console.log("  [{item.final_priority.name:<10}] {item.source.value:<12} ";
            "{item.sender_name:<22} {item.content_preview[:35]}");
    if (! batch) {
        console.log("  (fila vazia -- tudo foi arquivado pelo Modo Foco)");
    stats = inbox.stats();
    ...
    console.log("  Categorias: {digest['categories']}");
    console.log("  Total: {digest['total']} itens nas ultimas 24h");
    para cada (cat, data) em digest["digest"].items(): {
        hl = "; ".join(h["from"] para h em data.get("highlights", []));
        console.log("    {cat:<14} {data['count']} itens";
            hl ? + (" (destaques: {hl})" : ""));
    // === Filosofia ===
    console.log("\n\n{'='*75}");
    console.log("  COMO O OPENINBOX PROTEGE SEU CEREBRO");
    console.log("{'='*75}");
    console.log(""";
...
// )
    console.log("{'='*75}");
    console.log("  OpenInbox: {stats['total_enqueued']} eventos -> ";
        "{stats['total_processed']} processados -> ";
        "{stats['bait_blocked']} bait bloqueado.");
    console.log("  5 apps. 1 fila. 0 ansiedade.");
    console.log("{'='*75}");
// ============================================================================
