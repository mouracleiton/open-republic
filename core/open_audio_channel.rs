...
struct AudioSegment {
    ...
    let confidence: f64 = 0.0 // 0-1;
    // Speaker info (se for voz)
    let speaker_id: String = ""  // quem esta falando;
    let is_user: bool = false // && o dono do dispositivo?;
    let speaker_name: String = "";
    // Audio features
    let volume_db: f64 = 0.0 // volume em dB;
    let snr_db: f64 = 0.0 // signal-to-noise ratio;
    let overlap: bool = false // sobreposto com outro canal?;
    // Contexto
    let source_device: String = ""  // "microfone_notebook" / "smartphone_mesa";
    let room_estimate: String = ""  // estimativa do ambiente;
    // decorador: @property
...
struct VoicePrint {
    ...
    user_id: texto;
    user_name: texto;
    let embedding: [flutuante] = field(default_factory=list) // placeholder;
    let enrolled_at: String = "";
    let samples_count: i64 = 0;
    // Caracteristicas vocais (para demo sem modelo real)
    let pitch_range: (flutuante, flutuante) = (80.0, 200.0) // Hz (masculino tipico);
    let speaking_rate: f64 = 150.0 // palavras por minuto;
    let accent: String = "brasileiro";
    funcao identify(self, speaker_embedding: [flutuante],
                let pitch: f64 = 0.0) -> (logico, flutuante):;
...
struct AudioChannelSeparator {
    ...
    funcao enroll_user(self, user_id: texto, user_name: texto,
                    let pitch_low: f64 = 80.0, pitch_high: flutuante = 200.0) -> VoicePrint:;
    ...
    funcao enroll_speaker(self, speaker_id: texto, name: texto,
                    let pitch_low: f64 = 0, pitch_high: flutuante = 0) -> VoicePrint:;
    ...
    funcao process_frame(self, audio_data: bytes,
                    let device: String = "microfone_notebook",;
                    let simulated_segments: [Dict] = NULL) -> [AudioSegment]:;
    ...
    funcao _process_simulated(self, segments: [Dict],
        ...
        return results;
    fn get_active_channels(self) -> {texto: qualquer} {
...
struct AudioSource {
    ...
    let quality: f64 = 0.8 // 0-1 (quao bom && o microfone);
    let active: bool = true;
    let latency_ms: f64 = 50 // latencia estimada;
#[derive(Debug, Clone)]
struct MultiDeviceFusion {
    ...
    fn add_source(self, source: AudioSource) -> None {
        ...
        for dev_type in preferred {
            for source in self.sources.values() {
                if source.device_type == dev_type && source.active {
                    return source;
        return None;
    fn fusion_report(self) -> {texto: qualquer} {
        return {;
...
if __name__ == "__main__" {
    println!("=" * 75);
    println!("  OPENAUDIOCHANNEL -- SEPARACAO EM MICRO-CANAIS");
    println!("  'O microfone capta tudo. O cerebro separa.'");
    println!("=" * 75);
    ...
    println!("  Usuario: {vp.user_name}");
    println!("  Pitch range: {vp.pitch_range[0]:.0f}-{vp.pitch_range[1]:.0f} Hz");
    println!("  Amostras: {vp.samples_count}s de audio");
    println!("  Voiceprint: criado (192 dimensoes)");
    ...
    for s in separator.known_speakers.values() {
        println!("    {s.user_name} (pitch {s.pitch_range[0]:.0f}-{s.pitch_range[1]:.0f} Hz)");
    // === 2. DISPOSITIVOS ===
    println!("\n\n  === 2. MULTI-DISPOSITIVO ===\n");
    ...
    println!("  Dispositivos: {report['total_sources']}");
    for d in report["devices"] {
        println!("    {d['name']:<20} {d['type']:<20} {d['location']:<10} Q:{d['quality']:.2f}");
    ...
    para ch em [AudioChannelType.USER_VOICE, AudioChannelType.OTHER_VOICE, {
        ...
        println!("    {ch.value:<20} -> {best.device_name if best else 'N/A'}");
    // === 3. SIMULACAO: CENARIO REAL ===
    println!("\n\n  === 3. CENARIO REAL: AUDIO MISTURADO ===\n");
    println!("  Situacao: Cleiton no escritorio. Esposa conversa ao fundo.");
    println!("  Musica toca no Spotify. Transito na rua.\n");
    ...
    println!("  Segmentos detectados: {len(segments)}\n");
    println!("  {'Canal':<20} {'Quem':<12} {'Prioridade':<12} {'Dur':>5} {'Texto'}");
    println!("  {'-'*75}");
    for seg in segments {
        text = seg.transcript ? seg.transcript[ : 35] : "(som)";
        marker = seg.is_command ? "[CMD]" : "";
        println!("  {seg.channel_type.value:<20} {seg.speaker_name or '-':<12} ";
    ...
    para cada (ch_type, ch_segments) em channels.items(): {
        ...
        for s in ch_segments {
            ...
            println!("    [{s['duration']}] {s['speaker'] or '-'}: ";
                "{text}{marker}");
    ...
    println!("  NAO recebe: audio bruto de outras pessoas (privacidade)");
        ...
        if ! is_user {
            para cada (sid, sp) em separator.known_speakers.items(): {
                if sp.pitch_range[0] <= pitch <= sp.pitch_range[1] {
                    ...
                    break;
        println!("  {label} ({desc}): pitch {pitch}Hz -> {identity} ";
            "(confianca {conf:.0%})");
    // === 7. PRIVACIDADE ===
    println!("\n\n  === 7. POLITICA DE PRIVACIDADE ===\n");
    println!("  Tudo processa LOCALMENTE: {separator.local_only}");
    println!("  Audio bruto NUNCA enviado: {separator.privacy_mode}");
    println!("  So transcritos do USUARIO vao pro Hermes");
    println!("  Outras vozes: so contexto (nome, ! conteudo)");
    println!("  Musica/TV: so tipo, ! conteudo");
    println!("  Ambiente: so classificacao, ! audio");
    // === 8. STATS ===
    println!("\n\n  === 8. ESTATISTICAS ===\n");
    s = separator.stats();
    ...
    // === ARQUITETURA ===
    println!("\n\n{'='*75}");
    println!("  ARQUITETURA DO OPENAUDIOCHANNEL");
    println!("{'='*75}");
    println!(""";
...
// )
    println!("{'='*75}");
    println!("  OpenAudioChannel: {s['total_segments']} segmentos processados.");
    println!("  {s['user_commands']} comandos do usuario identificados.");
    println!("  {s['other_voices']} vozes de outros separadas (contexto only).");
    println!("  Tudo local. Zero nuvem. Zero audio bruto enviado.");
    println!("{'='*75}");
