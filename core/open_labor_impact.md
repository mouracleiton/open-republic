# OpenLaborImpact -- Otimizacao por IMPACTO (nao dinheiro)

**Linguagem:** Portugol++ (PPM)

**Arquivo original:** `core/open_labor_impact.py`

**Descricao:** ==========================================================
"Na Republica, o nivel mede ALCANCE de impacto, nao valor em reais.
...
4 DIMENSOES DE IMPACTO:
  ...
  2. DEPTH (profundidade)
     Quao transformacional e o impacto por pessoa.
...
classe ImpactMetrics:
    ...
    seja evidence: texto = ""  // justificativa

    // decorador: @property
    funcao vte(self) -> flutuante:
        ...
        // 
        retorne self.reach * self.depth * self.duration * self.multiplier / 100.0

    // decorador: @property
    funcao vte_formatted(self) -> texto:
        v = self.vte
        se v >= 1_000_000 entao:
            retorne "{v/1_000_000:.1f}M"
        senao se v >= 1_000 entao:
            retorne "{v/1_000:.1f}k"
        senao:
            retorne "{v:.0f}"


// Impacto REAL de cada papel (estimado a partir de evidencia do historico)
seja IMPACT_DATA: {LaborRole: ImpactMetrics} = {
    ...
    LaborRole.ARCHITECT: ImpactMetrics(
        ...
        depth = 7, // arquitetura determina o que e possivel construir
    ...
    LaborRole.PROGRAMMER: ImpactMetrics(
        ...
        depth = 5, // ferramentas uteis mas nao transformacionais por si
    ...
    LaborRole.CONTENT_CREATOR: ImpactMetrics(
        ...
        evidence = "@clouramlearning no X. 5 frentes de propagacao. Mas conteudo social e efemero."
    ),
    ...
    LaborRole.PRODUCT_OWNER: ImpactMetrics(
        ...
        depth = 5, // produtos uteis mas nao transformacionais
        duration = 5, // produtos tem ciclo de vida
        multiplier = 1.5, // habilita uso, mas nao cascata forte
...
// decorador: @dataclass
classe ImpactRoleAssignment:
    role: LaborRole
    hours_per_week: flutuante
    impact: ImpactMetrics

    // decorador: @property
    funcao impact_per_hour(self) -> flutuante:
        // VTE por hora de trabalho.
        se self.hours_per_week == 0 entao:
            retorne 0.0
        retorne self.impact.vte / self.hours_per_week

    // decorador: @property
    funcao weekly_impact(self) -> flutuante:
        ...
        prod = 1.0
        se self.hours_per_week > 20 entao:
            prod = 1.0 - 0.10 * minimo((self.hours_per_week - 20) / 15, 1.0)
        senao se self.hours_per_week < 5 entao:
            // Muito poucas horas = nao ganha tracao
            prod = self.hours_per_week / 5.0

        retorne self.impact.vte * (self.hours_per_week / 20.0) * prod / 52.0 // semanal


// decorador: @dataclass
classe ImpactProfile:
    // Perfil otimizado por impacto, nao dinheiro.

    seja citizen_id: texto = ""
    seja roles: [ImpactRoleAssignment] = field(default_factory=list)
    ...
    // decorador: @property
    funcao total_weekly_impact(self) -> flutuante:
        // Total VTE semanal COM foco penalty.
        raw = soma(r.weekly_impact para r em self.roles)
        retorne raw * self.focus_factor

    // decorador: @property
    funcao total_annual_impact(self) -> flutuante:
        retorne self.total_weekly_impact * 52

    // decorador: @property
    funcao impact_per_hour(self) -> flutuante:
        se self.total_hours == 0 entao:
            retorne 0.0
        retorne self.total_weekly_impact / self.total_hours
    ...
    funcao summary(self) -> texto:
        ...
        ranked = ordene(self.roles, key=(r) -> r.weekly_impact * 52, reverse=verdadeiro)
    ...
    funcao _fmt_vte(v: flutuante) -> texto:
        se v >= 1_000_000 entao:
            retorne "{v/1_000_000:.2f}M"
        senao se v >= 1_000 entao:
            retorne "{v/1_000:.1f}k"
        senao:
            retorne "{v:.0f}"
...
funcao optimize_impact(
    all_roles: [LaborRole],
    seja max_hours: flutuante = 50.0,
    seja min_roles: inteiro = 1,
    ...
    impact_per_hour = {}
    para cada role em all_roles:
        im = IMPACT_DATA[role]
        impact_per_hour[role] = im.vte / 20.0 // VTE/hora a produtividade maxima

    para cada k em intervalo(min_roles, n + 1):
        para cada combo em itertools.combinations(intervalo(n), k):
            ...
            assignments = []
            para cada role em selected_roles:
    ...
    results.sort(key=(x) -> x[0], reverse=verdadeiro)
    retorne results
...
funcao impact_optimization_report(
    ...
    se fin_roles == imp_roles entao:
        lines.append("  >>> MESMA combinacao! Financeiro e impacto concordam.")
    senao se overlap entao:
    ...
    senao:
        ...
        lines.append("  >>> O papel mais lucrativo (Engenheiro N6) nao e necessariamente o de maior impacto.")
...
se __name__ == "__main__" entao:
    ...
    imprima(impact_optimization_report(all_roles, max_hours=50.0))
