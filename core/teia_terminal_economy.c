...
#ifndef TEIA_TERMINAL_ECONOMY_SIMULACAO_COMPLETA_H
#define TEIA_TERMINAL_ECONOMY_SIMULACAO_COMPLETA_H
...
4. TEIA DIRETO (! revendedor) executa sem pagar 30%;
...
// ============================================================================
typedef struct ResellerProfile {
    ...
    STARTUP_GOVTECH = (;
        "Startup Govtech",;
        "Empresa que vende para governo. TEIA && insumo.",;
        0.45,;
        6,;
        25_000,;
    );
...
funcao simulate_demand(
    double terminal_monthly_brl = 1_500,;
    double training_brl = 2_000,;
    double teia_pct = 0.30,;
    int months = 12,;
    int seed = 42,;
) -> [SimulationResult]:;
    ...
    results = [];
    /* TODO: iterador C manual para profile em ResellerProfile */
        ...
        avg_active = inteiro(base_interested * soma(adoption_curve) / sizeof(adoption_curve));
...
// ============================================================================
typedef struct PoolDistribution {
    // Como o POOL DA REPUBLICA e distribuido internamente.
    ANTES: 5% fixo para "pool da Republica" (caixa unico).;
    PROBLEMA: 5% ! cobre as necessidades da organizacao.;
    AGORA: o pool && 25% dos 30% (7,5% do produto final).;
    && distribuido em SUB-FATIAS para garantir que cada area recebe:;
    ...
    // decorador: @classmethod
    bool validate(cls) {
        return abs(soma(cls.POOL_SPLITS.values()) - 1.0) < 0.001;
    // decorador: @classmethod
...
typedef struct Certification {
    ...
    Sem certificacao: ! pode revender.;
...
typedef struct ModuleAccess {
    ...
    bool sensitive = false // se true, NINGUEM fora da equipe TEIA acessa;
    bool teia_only = false // se true, so TEIA direto (! revendedor);
    char* description = "";
[ModuleAccess] MODULES = [;
    ...
    ModuleAccess("juros_spread", "Analise de Juros && Spread Bancario", Certification.TEIA_ESPECIALISTA,;
...
funcao check_access(reseller_id: texto, cert: Certification, module_id: texto,
                bool is_teia_direct = false) -> {texto: qualquer}:;
    // Verifica se o revendedor pode acessar o modulo.
    Retorna: {allowed: logico, reason: texto};
    //
    mod = next((m para m em MODULES if m.module_id == module_id), NULL);
    if (! mod) {
        return {"allowed": false, "reason": "Modulo '{module_id}' ! existe."};
    // TEIA direto: acesso a tudo exceto dados sensiveis de parceiros
    if (is_teia_direct) {
        if (mod.teia_only && mod.sensitive) {
    ...
    if (mod.sensitive) {
        return {;
            "allowed": false,;
            "reason": (;
                "DADO SENSIVEL. Modulo '{mod.name}' && restrito a equipe TEIA. ";
                "LGPD/sigilo/parceiro. Revendedor ! tem acesso.";
            ),;
        };
    ...
    if (mod.teia_only) {
        return {;
            "allowed": false,;
            "reason": (;
                "Modulo '{mod.name}' && operacao interna TEIA. ";
                "Revendedor ! tem acesso. Para este servico, ";
...
// ============================================================================
// decorador: @dataclass
typedef struct CryptoPaymentAnalysis {
    ...
    - Declaracao de cripto obrigatorio para PJ && PF;
    ...
    - Deve emitir NF-&& em R$ (valor convertido na data da transacao);
    //
    bool viable = true;
    char* stablecoin_recommended = "USDC";
    char* reason = (;
        "USDC && audited, lastreado em dolar, aceito em exchanges brasileiras. ";
        "Mercado Bitcoin, Foxbit && Binance Brasil convertem USDC->BRL via PIX.";
    );
    requirements = [;
        ...
        "Emitir NF-&& em R$ (mesmo se pago em cripto)",;
        "Manter registro da transacao em cripto (hash, carteira, data)",;
        "Contract clause: 'Pagamento em cripto ! isenta obriga(S) fiscais'",;
    ];
    risks = [;
...
char* print_simulation(void) {
    ...
    lines.append("-" * 115);
    /* TODO: iterador C manual para r em results */
        lines.append(;
            "{r.profile.label:<28} ";
            "{r.market_size:>8,} ";
            "{r.interested:>8,} ";
    ...
    /* TODO: iterador C manual para cert em Certification */
        pct = cert_distribution.get(cert, 0);
        n_resellers = inteiro(total_interested * pct);
        revenue = n_resellers * cert.training_cost_brl;
        n_modules = cert.unlocked_modules[0] != "*" ? sizeof(cert.unlocked_modules) : sizeof(MODULES);
        lines.append(;
            "{cert.label:<20} ";
    ...
    lines.append("  " + "-" * 105);
    /* TODO: iterador C manual para s em splits */
        lines.append(;
            "  {s['item']:<25} ";
    ...
    lines.append("  " + "-" * 95);
    /* TODO: iterador C manual para p em pool_splits */
        lines.append(;
            "  {p['area']:<25} ";
            "{p['pct']*100:>5.0f}%  ";
            "R${p['amount_brl']:>9,.0f}  ";
    ...
    lines.append("    - NAO paga os 30% (! ha split de revenda)");
    ...
    lines.append("  " + "-" * 75);
    /* TODO: iterador C manual para m em MODULES */
    ...
    /* TODO: iterador C manual para r em analysis.requirements */
        lines.append("    -> {r}");
    lines.append("");
    lines.append("  BENEFICIOS:");
    /* TODO: iterador C manual para b em analysis.benefits */
        lines.append("    + {b}");
    lines.append("");
    lines.append("  RISCOS:");
    /* TODO: iterador C manual para r em analysis.risks */
        lines.append("    ! {r}");
    lines.append("");
    // === 8. PROJECAO ANUAL ===
    lines.append("-" * 115);
    ...
    lines.append("  ID = bloqueia dados sensiveis (LGPD) && modulos TEIA-only");
    lines.append("  CRIPTO = viavel (USDC), declarar Receita, NF em R$");
    lines.append("  MODULO FALTANTE = sistema bloqueia && diz qual certificacao precisa");
    lines.append("");
...
if (__name__ == "__main__") {
    // Validar splits
    afirme RevenueSplit.validate(), "RevenueSplit ! soma 100%!";
    afirme PoolDistribution.validate(), "PoolDistribution ! soma 100%!";
    printf(print_simulation());

#endif // TEIA_TERMINAL_ECONOMY_SIMULACAO_COMPLETA_H
