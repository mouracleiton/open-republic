...
package openresponsibility_divisao_e_estabelecimento_de_responsabilidades
...
// ============================================================================
type ResponsibilityContext int
const (
...
type ResponsibilityPriority int
const (
...
type Responsibility struct {
    ...
    citizen_name := "" // string
    // Carga
    hours_per_week := 0.0 // quantas horas por semana // float64
    flexibility := 0.8 // 0-1 (1 = totalmente flexivel) // float64
    // Rotatividade
    is_rotative := false // reveza entre pessoas? // bool
    rotation_cycle := ""  // semanal, mensal // string
    // Status
    status := ResponsibilityStatus.ASSIGNED // ResponsibilityStatus
    // Consequencias
    if_not_done := ""  // o que acontece se ! fizer // string
    who_affected := ""  // quem && afetado // string
// ============================================================================
...
// ============================================================================
// decorador: @dataclass
type CitizenResponsibilities struct {
    // Perfil completo de responsabilidades de um cidadao.
    citizen_id: texto
    citizen_name: texto
    age := 0 // int64
    family_role := ""  // pai, mae, filho, avo, sozinho // string
    profession := "" // string
    profession_level := "" // string
    // Responsabilidades atribuidas
    responsibilities := field(default_factory=list) // [Responsibility]
    // decorador: @property
    func total_hours_week(self) float64 {
        return soma(r.hours_per_week para r em self.responsibilities)
    // decorador: @property
    func hours_by_context(self) {texto: flutuante} {
        hours = defaultdict(flutuante)
        for _, r := range self.responsibilities {
            hours[r.context.value] += r.hours_per_week
        return dict(hours)
    // decorador: @property
    func is_overloaded(self) bool {
    ...
    // decorador: @property
    func has_family_time(self) bool {
...
type ResponsibilityEngine struct {
    ...
    funcao create_profile(self, citizen_id: texto, name: texto, age: inteiro,
        ...
        return profile
    func assign_default(self, citizen_id: texto) {texto: qualquer} {
    ...
    funcao distribute_family(self, family_members: [texto]
        ...
        for _, mid := range family_members {
    ...
    func check_balance(self, citizen_id: texto) {texto: qualquer} {
    ...
    func _recommend(self, profile: CitizenResponsibilities) string {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENRESPONSIBILITY -- DIVISAO DE RESPONSABILIDADES")
    fmt.Println("  Cada um no que pode. Sem sobrecarga. Justo.")
    fmt.Println("=" * 80)
    ...
    para cada (context, items) em engine.catalog.items(): {
        ...
        for _, item := range items {
            fmt.Println("    [{item['priority'].value[0]:<10}] {item['name']:<40} "
                "{item['hours']}h/sem")
            if item.get("rotative") {
                fmt.Println("      (ROTATIVO)")
    ...
    para cid, name, *_ in citizens: {
        ...
        fmt.Println("    Sobrecarregado: {balance['overloaded']}")
        fmt.Println("    Tempo familia: {'SIM' if balance['has_family_time'] else 'NAO'}")
        fmt.Println("    Recomendacao: {balance['recommendation'][:60]}")
    // === 5. ROTATIVIDADE ===
    fmt.Println("\n\n  === 5. ROTATIVIDADE (limpeza familiar) ===\n")
    rot = engine.rotate(ResponsibilityContext.FAMILY, ["C-001", "C-002", "C-003", "C-004"])
    fmt.Println("  {rot['message']}")
    para cada (member, task) em rot["schedule"].items(): {
        fmt.Println("    {member}: {task}")
    // === 6. STATS ===
    fmt.Println("\n\n  === 6. ESTATISTICAS ===\n")
    s = engine.stats()
    para cada (k, v) em s.items(): {
        fmt.Println("  {k:<30} {v}")
    fmt.Println("\n{'='*80}")
    fmt.Println("  OpenResponsibility: {s['total_responsabilidades']} responsabilidades, "
