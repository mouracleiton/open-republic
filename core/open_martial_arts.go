...
package openmartialarts_defesa_pessoal_e_artes_marciais_da_republica
...
type Technique struct {
    ...
    description := "" // string
    // Adaptabilidade
    for_children := true // pode ensinar a criancas? // bool
    for_elderly := true // idoso consegue fazer? // bool
    for_pcd := true // pessoa com deficiencia? // bool
    standing := true // em pe? // bool
    ground := false // no chao? // bool
    min_strength := 1 // 1-10 (forca necessaria) // int64
    speed_required := 3 // 1-10 (velocidade necessaria) // int64
    // Filosofia
    goal := ""  // o que esta tecnica alcansa // string
    last_resort := false // so usar se nada mais funcionar // bool
    lethal_potential := false // pode matar? // bool
// ============================================================================
...
const (
    ...
    // decorador: @property
    func meaning(self) string {
        return self.value[1]
// decorador: @dataclass
type MartialArtsProfile struct {
    // Perfil de cada cidadao no OpenMartialArts.
    citizen_id: texto
    name: texto
    age: inteiro
    belt := Belt.WHITE // Belt
    techniques_mastered := field(default_factory=list) // [texto]
    hours_trained := 0.0 // float64
    sessions_attended := 0 // int64
    can_teach := false // bool
    specialization := field(default_factory=list) // [MartialArt]
    // Adaptacoes
    physical_adaptations := field(default_factory=list) // lesao, PCD, idade // [texto]
    seated_mode := false // treina sentado (PCD/cadeirante) // bool
    self_defense_focus := true // foco em defesa real, ! competicao // bool
    // decorador: @property
    funcao next_belt(self) retorna Belt?:
        belts = list(Belt)
        idx = belts.index(self.belt)
        idx + 1 < len(belts) ? retorne belts[idx + 1] : nil
    // decorador: @property
    func techniques_for_belt(self) int64 {
        // Quantas tecnicas precisa para proximo cinto.
        req = {0: 5, 1: 8, 2: 12, 3: 16, 4: 20, 5: 0}
        return req.get(self.belt.level, 0)
    // decorador: @property
    func can_promote(self) bool {
        return (len(self.techniques_mastered) >= self.techniques_for_belt
                && self.hours_trained >= self.belt.level * 50)
// ============================================================================
// 5. MOTOR DE TREINAMENTO
// ============================================================================
type MartialArtsEngine struct {
    ...
    funcao register_citizen(self, citizen_id: texto, name: texto, age: inteiro,
                        adaptations := nil, // [texto]
                        seated := false) -> {texto: qualquer}: // bool
    ...
    func get_curriculum(self, belt: Belt = Belt.WHITE) {texto: qualquer} {
    ...
    funcao train_technique(self, citizen_id: texto, tech_id: texto,
                        hours := 1.0) -> {texto: qualquer}: // float64
        ...
        return {
            ...
            "promoted": promoted,
            promoted ? "new_belt": profile.belt.value[0] : nil,
            "message": (
    ...
    func defense_scenario(self, scenario: texto) {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENMARTIALARTS -- DEFESA PESSOAL DA REPUBLICA")
    fmt.Println("  'Todo mundo prefere paz. Mas se quebrarem, todos se defendem.'")
    fmt.Println("=" * 80)
    // === 1. ARTES DE ORIGEM ===
    fmt.Println("\n\n  === 1. ARTES MARCIAIS INTEGRADAS ===\n")
    s = engine.stats()
    para cada (art, count) em ordene(s["by_origin"].items(), key=(x) -> -x[1]): {
        fmt.Println("  {art:<30} {count} tecnicas")
    // === 2. CATEGORIAS ===
    fmt.Println("\n\n  === 2. CATEGORIAS DE TECNICA ===\n")
    para cada (cat, count) em ordene(s["by_category"].items(), key=(x) -> -x[1]): {
        fmt.Println("  {cat:<25} {count} tecnicas")
    // === 3. SISTEMA DE CINTOS ===
    fmt.Println("\n\n  === 3. GRADUACAO (CINTOS) ===\n")
    for _, belt := range Belt {
        fmt.Println("  [{belt.value[0]:<8}] Nivel {belt.level} -- {belt.value[1]}")
    ...
    for _, c := range citizens {
        if len(c) == 5 {
            ...
            r = engine.register_citizen(*c)
        fmt.Println("  {r['citizen']:<20} -> cinto {r['belt']} (foco: {r['focus']})")
    // Treinar tecnicas
    fmt.Println("\n  Treinando tecnicas:\n")
    training_plan = [
    ...
    for _, sc := range scenarios {
        ...
        fmt.Println("\n  CENARIO: {sc}")
        fmt.Println("  RESPOSTA: {result['response']}")
        if result.get("warning") {
            fmt.Println("  [!] {result['warning']}")
        for _, t := range result["techniques"] {
            fmt.Println("    -> {t['name']} ({t['origin']}): {t['goal']}")
    // === 7. TECNICAS ADAPTADAS ===
    fmt.Println("\n\n  === 7. ADAPTACAO POR CAPACIDADE ===\n")
    fmt.Println("  TODA tecnica && adaptada para:")
    fmt.Println("    - Criancas (golpe reduzido, foco em escapar)")
    fmt.Println("    - Idosos (usar alavanca, ! forca)")
    fmt.Println("    - PCD/Cadeirante (BJJ sentado, tecnicas de mao)")
    fmt.Println("    - Gestantes (sem queda, tecnicas de pe)")
    // === 8. STATS ===
    fmt.Println("\n\n  === 8. ESTATISTICAS ===\n")
    s = engine.stats()
    ...
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  FILOSOFIA DO OPENMARTIALARTS")
    fmt.Println("{'='*80}")
    fmt.Println("""
...
PRINCIPIOS:
    ...
    P3 := trabalho de alto impacto. // Ensinar artes marciais
    P4: Cinturao preto ensina. Nao domina.
// )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenMartialArts: {s['total_techniques']} tecnicas de "
        "{len(s['by_origin'])} artes marciais.")
    fmt.Println("  {s['total_citizens_training']} cidadaos treinando. "
        "{s['black_belts']} cintos pretos (podem ensinar).")
    fmt.Println("  Defender, ! agredir. Escapar, ! brigar.")
    fmt.Println("{'='*80}")
