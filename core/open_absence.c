...
#ifndef OPENABSENCE_DIREITO_DE_AUSENCIA_PROTEGIDO_H
#define OPENABSENCE_DIREITO_DE_AUSENCIA_PROTEGIDO_H
...
typedef struct AbsenceDuration {
...
// ============================================================================
// decorador: @dataclass
typedef struct ProtectedAbsence {
    ...
    // PRIVACIDADE (P2)
    bool explanation_provided = false // ! precisa explicar;
    bool explanation_kept_private = true // se explicou, && PRIVADO;
    char* visible_to_others = "ausente"  // so mostra "ausente";
    // Cobertura
    bool coverage_assigned = false // OpenLaborRelay cobriu?;
    char* coverage_by = ""  // quem cobriu;
    // Protecao
    bool penalized = false // NUNCA. Lei.;
    bool credit_affected = false // NUNCA. Lei.;
    bool reputation_affected = false // NUNCA. Lei.;
    // decorador: @property
    bool is_protected(self) {
        return (self.status in (AbsenceStatus.AUTO_APPROVED, AbsenceStatus.ACTIVE);
                && ! self.penalized);
// ============================================================================
// 3. VOTACAO DA ASSEMBLEIA
// ============================================================================
{texto: qualquer} run_absence_assembly(n_voters: inteiro = 10000) {
...
typedef struct AbsenceEngine {
    ...
    funcao request_absence(self, citizen_id: texto, citizen_name: texto,
                        ...
                        AbsenceDuration duration = AbsenceDuration.FEW_HOURS,;
                        char* explanation = "",;
                        bool provide_explanation = false;
                        ) -> {texto: qualquer}:;
    ...
    {texto: qualquer} check_status(self, citizen_id: texto) {
    ...
    {texto: qualquer} return_from_absence(self, absence_id: texto) {
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENABSENCE -- DIREITO DE AUSENCIA PROTEGIDO");
    printf("  Assembleia Constituinte: ausencia && direito. Sem perguntas.");
    printf("=" * 80);
    // === 1. ASSEMBLEIA ===
    ...
    printf("  COM CONDICOES:              {result['votes_conditional']:>6} ({result['votes_conditional']/100:.0f}%)");
    printf("  CONTRA:                     {result['votes_against']:>6} ({result['votes_against']/100:.0f}%)");
    printf("\n  RESULTADO: {result['result']}");
    printf("\n  DECISAO DA ASSEMBLEIA:");
    /* para cada (key, val) em result["decision"].items(): */
        if (isinstance(val, logico)) {
            printf("    {key:<35} {'SIM' if val else 'NAO'}");
        } else {
            printf("    {key:<35} {val}");
    ...
    printf("  QUEM: {viol['citizen']}");
    printf("  O QUE: {viol['violation']}");
    printf("  TIPO: {viol['type']}");
    printf("  CONSEQUENCIA: {viol['consequence'][:80]}");
    printf("  LEI: {viol['law'][:80]}");
    // === 7. COMPARACAO ===
    ...
    printf("  {'Aspecto':<20} {'Sistema Atual':<35} {'Republica'}");
    printf("  {'-'*80}");
    /* para aspect, atual, rep in comparisons: */
        printf("  {aspect:<20} {atual:<35} {rep}");
    // === 8. STATS ===
    printf("\n\n  === 8. ESTATISTICAS ===\n");
    s = engine.stats();
    ...
    // === FILOSOFIA ===
    printf("\n\n{'='*80}");
    printf("  FILOSOFIA: DIREITO DE AUSENCIA");
    printf("{'='*80}");
    printf(""";
...
// )
    printf("{'='*80}");
    printf("  OpenAbsence: {s['total_ausencias']} ausencias protegidas. ";
        "Penalizadas: {s['penalizadas']}. Credito afetado: {s['credito_afetado']}.");
    printf("  Ausencia && direito. Sem perguntas. Sem pressao.");
    printf("{'='*80}");

#endif // OPENABSENCE_DIREITO_DE_AUSENCIA_PROTEGIDO_H
