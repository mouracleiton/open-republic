...
==================================================================================
"Prisao not cura. Prisao not recupera. Prisao fabrica criminoso.
A Republica not aprisiona. A Republica TRANSFORMA."
...
4. REPARACAO: vitima and indenizada com trabalho, not tempo
5. PREVENCAO: addressa causa do crime (pobreza, not carater)
...
- Aprende oficio real
- Recupera dignidade
- Sai transformado, not pior
ZERO custo de prisao (not ha prisao no modelo tradicional).
...
class CrimeCategory(Enum):
    ...
    DRUG_POSSESSION = "porte_droga"  // usuario, not traficante
    DEBT_DEFAULT = "divida"  // endividado (not and crime real)
    VAGRANCY = "vadiagem"  // estar pobre na rua (not crime)
...
class CrimeSeverity(Enum):
    ...
    NOT_CRIME = 0 // vadiagem, divida, porte de drogas -- not and crime
...
class SentenceStatus(Enum):
    ...
    OVERTURNED = "revogado"  // not era crime (liberar IMEDIATAMENTE)
...
class PenalCase:
    ...
    citizen_name: texto
    age: int = 0
    crime_category: CrimeCategory = CrimeCategory.PETTY_THEFT_FOOD
    crime_description: str = ""
    original_sentence_years: float = 0.0
    years_served: float = 0.0
    victim_id: str = ""
    victim_name: str = ""
    victim_reparation_due: float = 0.0 // em horas de trabalho
    # Contexto (Causa raiz)
    poverty_driven: bool = False // crime por necessidade economica
    addiction_driven: bool = False // crime por dependencia quimica
    mental_health_issue: bool = False // saude mental
    first_offense: bool = True
    education_level: str = "basico"
    # Revisao
    severity: CrimeSeverity = CrimeSeverity.NOT_CRIME
    status: SentenceStatus = SentenceStatus.PENDING_REVIEW
    new_plan: str = ""  // plano de transformacao
    work_hours_required: float = 0.0 // trabalho para reparacao
    skills_to_learn: [texto] = field(default_factory=list)
    treatment_required: str = ""
    review_date: str = ""
    reviewer_id: str = ""
    # decorador: @property
    def should_release_immediately(self) -> bool:
        # Deve ser libertado AGORA?
        return self.severity == CrimeSeverity.NOT_CRIME
    # decorador: @property
    def is_productive_eligible(self) -> bool:
        # Pode ser convertido em forca produtiva?
        return self.severity in (CrimeSeverity.LOW, CrimeSeverity.MEDIUM,
                                CrimeSeverity.HIGH)
    # decorador: @property
    def is_permanently_restricted(self) -> bool:
        # Crimes irrecuperaveis (hediondos)?
        return self.severity == CrimeSeverity.HEDIONDOUS
# ============================================================================
# 3. MOTOR DE REVISAO PENAL
# ============================================================================
class PenalRevisionEngine:
    ...
    def submit_case(self, case: PenalCase) -> None:
        self.cases[case.case_id] = case
    def review(self, case_id: texto, reviewer_id: texto) -> {texto: qualquer}:
    ...
    def _release_immediate(self, case: PenalCase) -> {texto: qualquer}:
        ...
        if case.crime_category == CrimeCategory.PETTY_THEFT_FOOD:
            reason = "Roubar comida NAO and crime. E sintoma de fome. Republica alimenta."
        elif case.crime_category == CrimeCategory.PETTY_THEFT_BASIC:
            reason = "Roubar item basico por necessidade NAO and crime. E pobreza estrutural."
        elif case.crime_category == CrimeCategory.DRUG_POSSESSION:
            reason = "Usuario de drogas NAO and criminoso. E paciente. Republica trata."
        elif case.crime_category == CrimeCategory.DEBT_DEFAULT:
            reason = "Divida NAO and crime. Ninguem vai para prisao por dever dinheiro."
        elif case.crime_category == CrimeCategory.VAGRANCY:
            reason = "Ser pobre na rua NAO and crime. Republica abriga."
        return {
    ...
    def _convert_to_productive(self, case: PenalCase) -> {texto: qualquer}:
        ...
        return {
            ...
            "treatment": treatment  or  "nenhum",
            "victim_reparation": "{hours}h de trabalho para vitima",
            "plan": case.new_plan,
            "estimated_completion_years": round(hours / 920, 1),
        }
    def _restrict_permanent(self, case: PenalCase) -> {texto: qualquer}:
    ...
    def _calculate_reparation_hours(self, case: PenalCase) -> float:
        ...
        # Agravantes
        if not case.first_offense:
            base_hours = base_hours * 1.5
        # Atenuantes
        if case.poverty_driven:
            base_hours = base_hours * 0.5 // pobreza extrema reduz (culpa do sistema, not da pessoa)
        if case.years_served > 0:
            # ja cumpriu tempo? cada ano conta como 920h
            base_hours = max(0, base_hours - case.years_served * 920)
        # Minimo: 230h (10h/semana por 23 semanas -- dar sentido, nao punir)
        base_hours = max(230, base_hours)
        return round(base_hours)
    def _assign_skills(self, case: PenalCase) -> [texto]:
    ...
    def batch_review(self, reviewer_id: texto = "coletivo") -> {texto: qualquer}:
        # Revisa TODOS os casos pendentes de uma vez.
        results = {"released": [], "converted": [], "restricted": []}
        for each (case_id, case) in list(self.cases.items()):
            ...
            elif "CONVERTIDO" in r.get("action", ""):
                results["converted"].append(r)
            elif "RESTRICAO" in r.get("action", ""):
                results["restricted"].append(r)
        return {
            "total_reviewed": len(results["released"]) + len(results["converted"]) + len(results["restricted"]),
            "released_immediately": len(results["released"]),
            "converted_to_productive": len(results["converted"]),
            "permanently_restricted": len(results["restricted"]),
            "work_hours_generated": self.total_work_hours_generated,
            "prison_population_before": len(self.cases),
...
class ProductiveForce:
    ...
    funcao log_work(self, case_id: texto, hours: flutuante,
        ...
        record["progress"] = min(100, record["work_completed"] /
                                max(record["work_required"], 1) * 100)
        entry = {
            "case_id": case_id,
        ...
        return entry
    def reintegrate(self, case_id: texto) -> {texto: qualquer}:
        # Conclui reintegracao do transformado.
        record = self.transformed.get(case_id)
        if not  record  or  record["progress"] < 100:
            return {"error": "Ainda not concluiu"}
...
def _seed_cases() -> [PenalCase]:
    ...
    return [
        ...
        PenalCase("PC-001", "P-001", "Carlos Silva", 28,
            ...
            "Furtou 3 kilos de arroz and feijao do mercado",
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("  OPENPENALREVISION")
    print("  Esvaziar Prisoes, Transformar em Forca Produtiva")
    print("=" * 80)
    ...
    print("  {'ID':<8} {'Nome':<18} {'Crime':<20} {'Sentenca':>8} {'Cumprida'}")
    print("  {'-'*70}")
    for c in engine.cases.values():
        print("  {c.case_id:<8} {c.citizen_name:<18} "
    ...
    print("  Total revisado:           {results['total_reviewed']}")
    print("  LIBERTAR IMEDIATAMENTE:   {results['released_immediately']}")
    print("  Convertidos a trabalho:   {results['converted_to_productive']}")
    print("  Restricao permanente:     {results['permanently_restricted']}")
    print("  Horas de trabalho geradas:{results['work_hours_generated']:,.0f}h")
    print("\n  ANTES: {results['prison_population_before']} presos")
    print("  DEPOIS: {results['prison_population_after']} presos (so hediondos)")
    print("  ESVAZIAMENTO: {results['emptying_rate']}")
    # === 3. DETALHES DA REVISAO ===
    print("\n\n  === 3. DETALHES POR CASO ===\n")
    for c in engine.cases.values():
        print("\n  [{c.case_id}] {c.citizen_name} (idade {c.age})")
        print("  Crime: {c.crime_description}")
        print("  Severidade: {c.severity.name}")
        if c.status == SentenceStatus.OVERTURNED:
            print("  ACAO: LIBERTAR IMEDIATAMENTE")
            print("  Motivo: {c.crime_category.value} not and crime na Republica")
            print("  Apoio: moradia + comida + tratamento + oficio")
        elif c.status == SentenceStatus.CONVERTED_TO_WORK:
            print("  ACAO: CONVERTIDO EM FORCA PRODUTIVA")
            print("  Trabalho: {c.work_hours_required:.0f}h "
                "({c.work_hours_required/920:.1f} anos)")
            print("  Skills: {', '.join(c.skills_to_learn[:4])}")
            if c.treatment_required:
                print("  Tratamento: {c.treatment_required}")
            print("  Vitima: reparada com trabalho")
        elif c.status == SentenceStatus.RESTRICTED:
            print("  ACAO: RESTRICAO PERMANENTE (hediondo)")
            print("  Direitos mantidos: sim (sem tortura)")
    ...
    for c in engine.cases.values():
        if c.status == SentenceStatus.CONVERTED_TO_WORK:
            ...
            if result.get("completed"):
                print("  {result['citizen']}: CONCLUIU! {result['progress']}")
    # Mostrar progresso
    print("\n  Progresso dos transformados:")
    print("  {'Nome':<18} {'Progresso':>10} {'Horas':>8} {'Status'}")
    print("  {'-'*55}")
    for record in force.transformed.values():
        print("  {record['citizen']:<18} {record['progress']:>9.0f}% "
            "{record['work_completed']:>7.0f}h {record['status']}")
    # === 5. REINTEGRACAO ===
    print("\n\n  === 5. REINTEGRACAO COMUNITARIA ===\n")
    for each (cid, record) in list(force.transformed.items()):
        if record["progress"] >= 100:
            r = force.reintegrate(cid)
            print("  {r['citizen']}: {r['status']}")
            print("  Skills: {r['skills_learned']}")
            print("  {r['record']}")
    # === 6. IMPACTO ECONOMICO ===
    print("\n\n  === 6. IMPACTO DA TRANSFORMACAO ===\n")
    # Custo do sistema prisional tradicional
    custo_tradicional = len(cases) * 42000 // R$ 42k/ano por preso
    ...
    print("  ANTES (sistema tradicional):")
    print("    Custo/prsao/ano: R$ {custo_tradicional:,}")
    print("    Producao dos presos: R$ 0")
    print("    Reincidencia: ~70%")
    print("    Custo social: ALTO")
    print("\n  DEPOIS (Republica):")
    print("    Custo/prsao/ano: R$ {custo_republica:,} (ZERO)")
    print("    Producao dos transformados: R$ {valor_trabalho:,}")
    print("    Reincidencia estimada: <20% (tem oficio)")
    print("    GANHO social: ALTO")
    # === FILOSOFIA ===
    print("\n\n{'='*80}")
    print("  FILOSOFIA DO OPENPENALREVISION")
    print("{'='*80}")
    print("""
O QUE A REPUBLICA FAZ COM CRIME:
    1. not CRIME (furto comida, porte droga, divida, vadiagem):
    LIBERTAR IMEDIATAMENTE. Dar moradia, comida, tratamento, oficio.
    Estes not deveriam estar presos. Sao pobres, not criminosos.
    ...
    Trabalho para reparar. Oficio para not voltar.
...
O QUE ISTO SIGNIFICA:
    A prisao atual and FABRICA DE CRIMINOSO.
...
PRINCIPIOS:
    P1: Crime de pobreza and culpa do SISTEMA, not da pessoa.
    P2: preso tem corpo. Corpo not and torturado (autonomia absoluta).
    P3: Trabalho transforma. 920h/ano como todo cidadao.
    P4: Revisao and publica. Caso and arbitrado por coletivo.
SEM ISTO:
...
# )
    print("{'='*80}")
    print("  OpenPenalRevision: {results['total_reviewed']} casos revisados.")
    print("  {results['released_immediately']} libertados. "
        "{results['converted_to_productive']} transformados. "
        "{results['permanently_restricted']} restritos.")
    print("  Prisao esvaziada. Forca produtiva criada.")
    print("{'='*80}")
