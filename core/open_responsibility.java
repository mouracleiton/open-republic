...
public class OpenresponsibilityDivisaoEEstabelecimentoDeResponsabilidades {
    ...
    public static class ResponsibilityPriority {
    ...
    public static class ResponsibilityStatus {
    ...
    public static class Responsibility {
        ...
        // Rotatividade
        boolean is_rotative = false // reveza entre pessoas?;
        String rotation_cycle = ""  // semanal, mensal;
        // Status
        ResponsibilityStatus status = ResponsibilityStatus.ASSIGNED;
        // Consequencias
        String if_not_done = ""  // o que acontece se ! fizer;
        String who_affected = ""  // quem && afetado;
    // ============================================================================
    ...
    // ============================================================================
    // decorador: @dataclass
    public static class CitizenResponsibilities {
        // Perfil completo de responsabilidades de um cidadao.
        citizen_id: texto;
        citizen_name: texto;
        int age = 0;
        String family_role = ""  // pai, mae, filho, avo, sozinho;
        String profession = "";
        String profession_level = "";
        // Responsabilidades atribuidas
        [Responsibility] responsibilities = field(default_factory=list);
        // decorador: @property
        public double total_hours_week(self) {
            return soma(r.hours_per_week para r em self.responsibilities);
        // decorador: @property
        public {texto: flutuante} hours_by_context(self) {
            hours = defaultdict(flutuante);
            /* TODO: for-each Java para r em self.responsibilities */
                hours[r.context.value] += r.hours_per_week;
            return dict(hours);
        // decorador: @property
        public boolean is_overloaded(self) {
        ...
        // decorador: @property
        public boolean has_family_time(self) {
    ...
    // ============================================================================
    public static class ResponsibilityEngine {
        ...
        funcao create_profile(self, citizen_id: texto, name: texto, age: inteiro,
            ...
            return profile;
        public {texto: qualquer} assign_default(self, citizen_id: texto) {
            ...
            /* para cada (context, items) em self.catalog.items(): */
                /* TODO: for-each Java para item em items */
        ...
        funcao distribute_family(self, family_members: [texto]
            ...
            /* TODO: for-each Java para mid em family_members */
                profile = self.citizens.get(mid);
                if (! profile) {
                    continue;
                /* TODO: for-each Java para r em profile.responsibilities */
                    if (r.context == ResponsibilityContext.FAMILY) {
        ...
        public {texto: qualquer} check_balance(self, citizen_id: texto) {
        ...
        public String _recommend(self, profile: CitizenResponsibilities) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENRESPONSIBILITY -- DIVISAO DE RESPONSABILIDADES");
        System.out.println("  Cada um no que pode. Sem sobrecarga. Justo.");
        System.out.println("=" * 80);
        ...
        /* para cada (context, items) em engine.catalog.items(): */
            ...
            /* TODO: for-each Java para item em items */
                System.out.println("    [{item['priority'].value[0]:<10}] {item['name']:<40} ";
                    "{item['hours']}h/sem");
        ...
        /* para cid, name, age, role, prof, level in citizens: */
            ...
            System.out.println("  Total: {r['total_hours']:.0f}h/sem. ";
                "{'SOBRECARREGADO!' if r['overloaded'] else 'OK'}");
            /* TODO: for-each Java para a em r["assigned"][:4] */
                System.out.println("    [{a['context']:<12}] {a['name'][:35]:<36} {a['hours']:.0f}h");
        ...
        /* para cada (member, tasks) em family["by_member"].items(): */
            System.out.println("\n  {member}:");
            /* TODO: for-each Java para t em tasks[:3] */
                System.out.println("    {t['task'][:35]:<36} {t['hours']:.0f}h");
        // === 4. EQUILIBRIO ===
        ...
        /* para cid, name, *_ in citizens: */
            ...
            System.out.println("    Sobrecarregado: {balance['overloaded']}");
            System.out.println("    Tempo familia: {'SIM' if balance['has_family_time'] else 'NAO'}");
            System.out.println("    Recomendacao: {balance['recommendation'][:60]}");
        // === 5. ROTATIVIDADE ===
        System.out.println("\n\n  === 5. ROTATIVIDADE (limpeza familiar) ===\n");
        rot = engine.rotate(ResponsibilityContext.FAMILY, ["C-001", "C-002", "C-003", "C-004"]);
        System.out.println("  {rot['message']}");
        /* para cada (member, task) em rot["schedule"].items(): */
            System.out.println("    {member}: {task}");
        // === 6. STATS ===
        System.out.println("\n\n  === 6. ESTATISTICAS ===\n");
        s = engine.stats();
        ...
        System.out.println("\n{'='*80}");
        System.out.println("  OpenResponsibility: {s['total_responsabilidades']} responsabilidades, ";
