...
#ifndef OPENREPUBLIC_EXECUCAO_DO_PLANO_DE_ACAO_IMEDIATO_H
#define OPENREPUBLIC_EXECUCAO_DO_PLANO_DE_ACAO_IMEDIATO_H
...
void aplicar_plano(republic) {
    ...
    /* para cada (eid, count) em redistribuicao.items(): */
        /* TODO: iterador C manual para nation em republic.nations.values() */
...
void main(void) {
    printf("=" * 75);
    printf("  OPENREPUBLIC -- EXECUCAO DO PLANO DE ACAO IMEDIATO");
    printf("=" * 75);
    // 1. Estado ANTES
    ...
    printf("  Saude media: {health_before['avg_health']:.0%}");
    printf("  Entidades criticas: {health_before['critical_count']}");
    printf("  Entidades urgentes: {health_before['urgent_count']}");
    printf("  Pessoas em risco: {health_before['people_at_risk']:,}");
    // 2. Aplicar plano
    ...
    printf("  Saude media: {health_after['avg_health']:.0%}");
    printf("  Entidades criticas: {health_after['critical_count']}");
    printf("  Entidades urgentes: {health_after['urgent_count']}");
    printf("  Pessoas em risco: {health_after['people_at_risk']:,}");
    // 4. Comparativo
    printf("\n\n  +-------------------------------------------------+");
    printf("  |  METRICA              ANTES     DEPOIS    DELTA  |");
    printf("  +-------------------------------------------------+");
    metrics = [;
    ...
    /* para name, before, after in metrics: */
        printf("  |  {name:<22} {before:>8} {after:>10}{'':>7} |");
    printf("  +-------------------------------------------------+");
    ...
    if (red) {
        /* TODO: iterador C manual para item em red */
            ...
            printf("      Risk: {item['risk_score']} | {', '.join(item['issues'][:2])}");
    } else {
        printf("    NENHUMA -- todas as entidades criticas estabilizadas!");
    printf("\n  ZONA AMARELA RESTANTE: {len(yellow)} entidades");
    /* TODO: iterador C manual para item em yellow */
        printf("    [{item['urgency_name']}] {item['name']} ({item['nation']})");
        printf("      Risk: {item['risk_score']} | {item['issues'][0]}");
    // 6. Saude por nacao depois
    printf("\n  SAUDE POR NACAO (DEPOIS):");
    printf("  {'Nacao':<15} {'Saude':>7} {'Criticas':>9} {'Em risco':>10}");
    printf("  {'-'*45}");
    /* para cada (nid, nation) em republic.nations.items(): */
        crit = sizeof(nation.critical_entities());
        risk = nation.people_at_risk;
        marker = crit == 0 ? " <-- OK" : "";
        printf("  {nation.name:<15} {nation.avg_health:>6.0%} {crit:>9} {risk:>10,}{marker}");
    printf("\n{'='*75}");
    if (health_after['critical_count'] == 0) {
        printf("  MISSAO CUMPRIDA: 0 entidades criticas restantes.");
        printf("  {health_before['people_at_risk']:,} pessoas sairam de risco.");
    } else {
        printf("  PROGRESSO: {health_before['critical_count']} -> {health_after['critical_count']} criticas.");
        printf("  {health_before['people_at_risk'] - health_after['people_at_risk']:,} pessoas sairam de risco.");
    printf("{'='*75}");
if (__name__ == "__main__") {
    main();

#endif // OPENREPUBLIC_EXECUCAO_DO_PLANO_DE_ACAO_IMEDIATO_H
