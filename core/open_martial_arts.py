...
# !/usr/bin/env python3
# 
OpenMartialArts -- Defesa Pessoal and Artes Marciais da Republica
==================================================================
"Todo mundo prefere fazer seus negocios em paz.
Mas se a paz para quebrada, TODO MUNDO tem o direito
and a CAPACIDADE de se defender.
Autonomia corporal (P2) not sofrer. Tambem se PROTEGER."
CONCEITO:
A Republica not and militarista. not glorifica violencia.
MAS reconhece que o mundo tem violencia.
Quem not sabe se defender and PRESA.
...
- Krav Maga (defesa real, not militar): agressao, objetos, multipla
- Capoeira: mobilidade, esquiva, cultura brasileira
- Karate: base, tempo, distancia de golpe
and CRIA uma arte marcial unificada da Republica:
- Defesa PESSOAL real (not esporte, not competicao)
...
NIVEL 1 (AZUL): ESCAPAR -- a melhor luta and a que voce foge
NIVEL 2 (VERDE): NEUTRALIZAR -- imobilizar sem ferir gravemente
NIVEL 3 (PRETO): PROTEGER -- defender outros que not podem se defender
...
class Technique:
    ...
    description: str = ""
    # Adaptabilidade
    for_children: bool = True // pode ensinar a criancas?
    for_elderly: bool = True // idoso consegue fazer?
    for_pcd: bool = True // pessoa com deficiencia?
    standing: bool = True // em pe?
    ground: bool = False // no chao?
    min_strength: int = 1 // 1-10 (forca necessaria)
    speed_required: int = 3 // 1-10 (velocidade necessaria)
    # Filosofia
    goal: str = ""  // o que esta tecnica alcansa
    last_resort: bool = False // so usar se nada mais funcionar
    lethal_potential: bool = False // pode matar?
# ============================================================================
...
TECHNIQUES: [Technique] = [
    ...
    Technique("TEC-002", "Desescalada Verbal",
        ...
        "Ofecer saida honrosa. A melhor luta and a que not acontece.",
    ...
    Technique("TEC-003", "Postura de Alerta",
        ...
        "Maos abertas na frente do rosto (not punho -- mostra paz). "
    ...
    Technique("TEC-010", "Quebra de Pulso",
        ...
        "Todo agarrao tem um lado fraco. Polegar not segura.",
    ...
    Technique("TEC-021", "Joelhada Reto",
        ...
        "subir joelho. Dano max em distancia curta.",
    ...
    Technique("TEC-022", "Cotovelada Horizontal",
        ...
        "Cortar. Usar quando espaco and insuficiente para soco.",
    ...
    Technique("TEC-023", "Chute Frontal (Teep)",
        ...
        "Parede invisivel entre voce and o agressor.",
        goal = "Manter distancia"),
    Technique("TEC-024", "Chute Baixo (Caneada)",
        TechniqueCategory.STRIKING, MartialArt.MUAY_THAI, DifficultyLevel.BLUE,
        "Chute na coxa or canela do agressor. Difícil de ver. "
    ...
    Technique("TEC-032", "Double Leg Takedown (Wrestling)",
        ...
        "Baixar nivel, abraçar ambas as pernas, levantar and derrubar. "
        "Clássico de wrestling. Rapido and eficaz.",
    ...
    Technique("TEC-040", "Triangulo Fechado",
        ...
        "estrangulamento sanguineo (not de ar). Desmaia em 8s sem dano.",
    ...
    Technique("TEC-041", "Chave de Braco (Armbar)",
        ...
        "Se not soltar: quebra. Ameaca suficiente para render.",
    ...
    Technique("TEC-042", "Defesa de Estupro (Guarda Fechada)",
        ...
        "Controlar punhos. Triangulo, chave de braco, or varrer para cima. "
    ...
    Technique("TEC-043", "Montada - Controle",
        ...
        "Permite finalizar or bater sem levar dano.",
    ...
    Technique("TEC-052", "Bastao/Escrima (Defesa and Objeto)",
        ...
        "Tecnicas de Kali/Escrima: bater na mao, not na lamina.",
    ...
    Technique("TEC-060", "Posicionamento em Linha",
        ...
        "numa linha (um bloqueia o outro). Bater and fugir.",
        goal = "Nao ser cercado por multiplos"),
    Technique("TEC-061", "Empurra and Gira",
...
class Belt(Enum):
    ...
    - not and hierarquia. not and elitismo (P1).
    - and marcador de CONHECIMENTO.
    - Cinto preto not quer dizer "melhor". Quer dizer "pode ensinar".
    - TODO cidadao tem direito de aprender ate onde conseguir.
    # 
    WHITE = ("branco", "Iniciante -- aprendendo a cair and escapar")
    GREY = ("cinza", "Consciencia + defesa basica")
    BLUE = ("azul", "Escapou de agarrões and estrangulamentos")
    GREEN = ("verde", "Pode neutralizar ameaca sem ferir gravemente")
    BROWN = ("marrom", "Avancado -- defesa contra armas")
    BLACK = ("preto", "Pode PROTEGER outros and ENSINAR")
    # decorador: @property
    def level(self) -> int:
    ...
    def meaning(self) -> str:
        return self.value[1]
# decorador: @dataclass
class MartialArtsProfile:
    # Perfil de cada cidadao no OpenMartialArts.
    citizen_id: texto
    name: texto
    age: inteiro
    belt: Belt = Belt.WHITE
    techniques_mastered: [texto] = field(default_factory=list)
    hours_trained: float = 0.0
    sessions_attended: int = 0
    can_teach: bool = False
    specialization: [MartialArt] = field(default_factory=list)
    # Adaptacoes
    physical_adaptations: [texto] = field(default_factory=list) // lesao, PCD, idade
    seated_mode: bool = False // treina sentado (PCD/cadeirante)
    self_defense_focus: bool = True // foco em defesa real, not competicao
    # decorador: @property
    funcao next_belt(self) retorna Belt?:
        belts = list(Belt)
        idx = belts.index(self.belt)
        idx + 1 < len(belts) ? retorne belts[idx + 1] : None
    # decorador: @property
    def techniques_for_belt(self) -> int:
        # Quantas tecnicas precisa para proximo cinto.
        req = {0: 5, 1: 8, 2: 12, 3: 16, 4: 20, 5: 0}
        return req.get(self.belt.level, 0)
    # decorador: @property
    def can_promote(self) -> bool:
        return (len(self.techniques_mastered) >= self.techniques_for_belt
                and self.hours_trained >= self.belt.level * 50)
# ============================================================================
...
class MartialArtsEngine:
    ...
    PRINCIPIOS:
    1. DEFESA, not AGRESSAO -- not se inicia violenca
    ...
    5. SEM COMPETICAO OBRIGATORIA -- luta and para defesa, not esporte
    6. CULTURA BRASILEIRA -- capoeira and base cultural
    7. ANTI-BULLYING -- criancas aprendem a not usar contra colegas
    # 
    def __init__(self):
    ...
    funcao register_citizen(self, citizen_id: texto, name: texto, age: inteiro,
        ...
        return {
            ...
            profile.next_belt ? "next": profile.next_belt.value[0] : "max",
            "focus": "DEFESA PESSOAL -- not competicao",
            "adaptations": adaptations  or  "nenhuma",
            "message": (
    ...
    def get_curriculum(self, belt: Belt = Belt.WHITE) -> {texto: qualquer}:
    ...
    funcao train_technique(self, citizen_id: texto, tech_id: texto,
                        hours: float = 1.0) -> {texto: qualquer}:
        # Cidadao treina uma tecnica.
        profile = self.profiles.get(citizen_id)
        if not profile:
            return {"error": "Cidadao not registrado"}
        tech = self.techniques.get(tech_id)
        if not tech:
            return {"error": "Tecnica not encontrada"}
        ...
        # Registrar treino
        if tech_id not in profile.techniques_mastered:
        ...
        # Verificar promocao
        promoted = False
        if profile.can_promote and profile.next_belt:
        ...
        return {
            ...
            "techniques_mastered": len(profile.techniques_mastered),
            "promoted": promoted,
            promoted ? "new_belt": profile.belt.value[0] : None,
            "message": (
    ...
    def defense_scenario(self, scenario: texto) -> {texto: qualquer}:
        ...
        scenarios = {
            ...
            "tentativa_de_estupro": {
                "response": "BJJ defesa de solo, guarda fechada, triangulo or chave",
            ...
            "bullying_escolar": {
                ...
                "note": "Criancas: priorizar desescalada. Lutar and ultimo recurso.",
            },
        }
        result = scenarios.get(scenario)
        if not result:
            return {"error": "Cenario not encontrado. Disponiveis: {list(scenarios.keys())}"}
        return {
            "scenario": scenario,
    ...
    def stats(self) -> {texto: qualquer}:
        return {
            ...
            "total_citizens_training": len(self.profiles),
            "black_belts": sum(1 para p em self.profiles.values() if p.belt == Belt.BLACK),
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("  OPENMARTIALARTS -- DEFESA PESSOAL DA REPUBLICA")
    print("  'Todo mundo prefere paz. Mas se quebrarem, todos se defendem.'")
    print("=" * 80)
    # === 1. ARTES DE ORIGEM ===
    print("\n\n  === 1. ARTES MARCIAIS INTEGRADAS ===\n")
    s = engine.stats()
    for each (art, count) in ordene(s["by_origin"].items(), key=(x) -> -x[1]):
        print("  {art:<30} {count} tecnicas")
    # === 2. CATEGORIAS ===
    print("\n\n  === 2. CATEGORIAS DE TECNICA ===\n")
    for each (cat, count) in ordene(s["by_category"].items(), key=(x) -> -x[1]):
        print("  {cat:<25} {count} tecnicas")
    # === 3. SISTEMA DE CINTOS ===
    print("\n\n  === 3. GRADUACAO (CINTOS) ===\n")
    for belt in Belt:
        print("  [{belt.value[0]:<8}] Nivel {belt.level} -- {belt.value[1]}")
    ...
    for c in citizens:
        if len(c) == 5:
            ...
            r = engine.register_citizen(*c)
        print("  {r['citizen']:<20} -> cinto {r['belt']} (foco: {r['focus']})")
    # Treinar tecnicas
    print("\n  Treinando tecnicas:\n")
    training_plan = [
    ...
    for sc in scenarios:
        ...
        for t in result["techniques"]:
            print("    -> {t['name']} ({t['origin']}): {t['goal']}")
    # === 7. TECNICAS ADAPTADAS ===
    print("\n\n  === 7. ADAPTACAO POR CAPACIDADE ===\n")
    print("  TODA tecnica and adaptada para:")
    print("    - Criancas (golpe reduzido, foco em escapar)")
    print("    - Idosos (usar alavanca, not forca)")
    print("    - PCD/Cadeirante (BJJ sentado, tecnicas de mao)")
    print("    - Gestantes (sem queda, tecnicas de pe)")
    # === 8. STATS ===
    print("\n\n  === 8. ESTATISTICAS ===\n")
    s = engine.stats()
    ...
    # === FILOSOFIA ===
    print("\n\n{'='*80}")
    print("  FILOSOFIA DO OPENMARTIALARTS")
    print("{'='*80}")
    print("""
A REPUBLICA not and MILITARISTA:
...
A ARTE UNIFICADA:
    ...
    - Judo: derrubar com alavanca (not forca)
    - Boxe: footwork and combinacao
    - Krav Maga: defesa real (armas, multiplos)
    - Capoeira: cultura brasileira + mobilidade
    and criamos uma arte da Republica que serve para:
    - Crianca de 8 anos
    - Mulher de 50
    - Idoso de 70
    - Cadeirante
...
3 NIVEIS DE RESPOSTA (sempre nesta ordem):
    1. ESCAPAR -- a melhor luta and a que voce foge
    2. NEUTRALIZAR -- imobilizar sem ferir gravemente
    3. PROTEGER -- defender outros que not podem
O QUE not EXISTE:
    - Competicao obrigatoria (luta and defesa, not esporte)
    - Elitismo de cinto (cinto not and hierarquia, and conhecimento)
...
O QUE EXISTE:
    ...
    - Foco em defesa real (not medalha)
    - Anti-bullying (criancas aprendem a not usar)
    - Cultura brasileira (capoeira)
PRINCIPIOS:
    P1: Todo mundo treina. Mesmo cinto. Mesma chance.
    P2: Autonomia corporal inclui se defender.
    P3: Ensinar artes marciais = trabalho de alto impacto.
    P4: Cinturao preto ensina. Nao domina.
# )
    print("{'='*80}")
    print("  OpenMartialArts: {s['total_techniques']} tecnicas de "
        "{len(s['by_origin'])} artes marciais.")
    print("  {s['total_citizens_training']} cidadaos treinando. "
        "{s['black_belts']} cintos pretos (podem ensinar).")
    print("  Defender, not agredir. Escapar, not brigar.")
    print("{'='*80}")
