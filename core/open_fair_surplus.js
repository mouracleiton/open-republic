...
const INCENTIVO = mais 5% circulando = mais se sustentando;
A MATEMATICA:;
...
class FairSurplusCalc {
    ...
    // Producao
    const workers = 0;
    const output_value_month = 0.0 // valor total produzido/mes;
    const worker_pay_month = 0.0 // quanto cada trabalhador recebe;
    // Os 5%
    const surplus_5pct = 0.0 // 5% do valor produzido;
    const surplus_per_worker = 0.0 // quanto cada trabalhador recebe do 5%;
    const surplus_flow = SurplusFlow.COLLECTIVE_POOL;
    // Comparacao com predatorio
    const predatory_90pct = 0.0 // o que o dono levaria no capitalismo;
    const worker_gain_vs_predatory = 0.0 // quanto o trabalhador GANHA trocando;
    // Escala
    const surplus_at_10_workers = 0.0;
    const surplus_at_100_workers = 0.0;
    const surplus_at_1000_workers = 0.0;
    // decorador: @property
    worker_total_month(self) {
...
class FairSurplusEngine {
    ...
    funcao calculate(self, establishment: texto, est_type: texto,
                ...
                const flow = SurplusFlow.COLLECTIVE_POOL;
                ) -> {texto: qualquer}:;
    ...
    stats(self) {
        return {;
            ...
            "calculos_feitos": .length(self.calcs),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENFAIRSURPLUS -- 5% QUE VOLTA PRA QUEM TRABALHA");
    console.log("  Mais gente trabalhando = mais se sustentando");
    console.log("=" * 80);
    // === 1. A DESCOBERTA ===
    console.log(""";
A DESCOBERTA:;
...
// )
    ...
    console.log("  Producao por trabalhador: {scale['output_por_trabalhador']}");
    console.log("  Surplus: {scale['surplus_pct']}\n");
    console.log("  {'Trabalhadores':>15} {'Producao Total':>20} {'5% Circulando':>20} {'Por Trabalhador':>20}");
    console.log("  {'-'*80}");
    for (const r of scale["escala"]) {
        console.log("  {r['trabalhadores']:>15} {r['producao_total']:>20} ";
    ...
    console.log("\n  {r1['message']}");
    console.log("  Trabalhador total: {r1['trabalhador_recebe_total']}");
    console.log("  No capitalismo, dono levaria: {r1['COMPARACAO_PREDATORIA']['capitalismo_dono_leva']}");
    // FabLab
    r2 = engine.calculate("FabLab Central", "construcao",;
                        workers = 20, output_per_worker_month=6000,;
                        worker_pay_month = 3500);
    console.log("\n  {r2['message']}");
    console.log("  100 trabalhadores: {r2['ESCALA']['100_trabalhadores']}");
    // Software
    ...
    for (const f of flow) {
        console.log("  {f['destino']:<25} {f['pct']:<15} {f['descricao']}");
    // === 5. COMPARACAO CAPITALISMO vs REPUBLICA ===
    console.log("\n\n  === 5. CAPITALISMO vs REPUBLICA ===\n");
    comp = engine.compare_capitalism_vs_republic(4000.0);
    console.log("  Producao: {comp['producao_por_trabalhador']}\n");
    console.log("  CAPITALISMO:");
    ...
    para cada (k, v) em s.items(): {
        console.log("  {k:<25} {v}");
    console.log("\n{'='*80}");
    console.log("  OpenFairSurplus: {s['surplus_definido']} que volta. {s['principio']}");
    console.log("{'='*80}");
