...
struct FamilyMember {
    ...
    let current_hours: f64 = 0.0 // horas que ja trabalha;
    let specialties: [texto] = field(default_factory=list) // areas de competencia;
    // decorador: @property
    ...
    // decorador: @property
    fn max_hours_per_year(self) -> f64 {
    ...
    // decorador: @property
    fn base_hours_per_year(self) -> f64 {
...
struct FamilyUnit {
    ...
    family_id: texto;
    family_name: texto;
    let members: [FamilyMember] = field(default_factory=list);
    let shared_projects: [texto] = field(default_factory=list) // projetos da Republica;
    // decorador: @property
    fn working_members(self) -> [FamilyMember] {
        return [m para m em self.members if m.can_work];
    // decorador: @property
    fn total_capacity_factor(self) -> f64 {
        // Soma das capacidades de todos os membros.
        return soma(m.capacity_factor para m em self.members);
    // decorador: @property
    fn total_base_expected(self) -> f64 {
        // Total de horas base que a familia deve coletivamente.
        return soma(m.base_hours_per_year para m em self.working_members);
    // decorador: @property
    fn total_max_capacity(self) -> f64 {
...
struct FamilyLaborDistributor {
    ...
    fn register_family(self, family: FamilyUnit) -> None {
    ...
    fn family_report(self, family_id: texto) -> {texto: qualquer} {
        family = self.families.get(family_id);
        if ! family {
...
if __name__ == "__main__" {
    println!("=" * 75);
    println!("  OPENFAMILYLABOR -- DISTRIBUICAO DE CARGA FAMILIAR");
    println!("  'O avo divide com o pai que divide com o filho.'");
    println!("=" * 75);
    ...
    println!("  Familia: {report['family_name']}");
    println!("  {'Nome':<20} {'Papel':<12} {'Idade':>5} {'Capacidade':<12} ";
        "{'Horas':>6} {'Max':>6}");
    println!("  {'-'*65}");
    for m in report["members"] {
        excess = m["current_hours"] > m["max"] ? " [EXCESSO!]" : "";
        println!("  {m['name']:<20} {m['role']:<12} {m['age']:>4} ";
            "{m['capacity']:<12} {m['current_hours']:>5.0f} ";
            "{m['max']:>5.0f}{excess}");
    println!("\n  TOTAL atual: {report['total_current_hours']:.0f}h");
    println!("  Pessoas em excesso: 1 (Cleiton: 4000h, limite 1840h)");
    println!("  Excesso: {4000 - 1840:.0f}h ACIMA do limite constitucional");
    // === DISTRIBUICAO ===
    println!("\n\n  === DEPOIS: DISTRIBUINDO ENTRE A FAMILIA ===\n");
    result = dist.distribute("F-LOURA", total_work_hours=4000);
    println!("  Trabalho total a distribuir: {result['total_work_to_distribute']}h");
    println!("  Capacidade total da familia: {result['total_capacity_factor']}");
    ...
    for d in result["distribution"] {
        ...
        println!("  {d['name']:<20} {d['role']:<14} {d['capacity_factor']:>4.1f} ";
    ...
    println!("  Familia agora tem {len(family.members)} membros.");
    println!("  Capacidade total: {result2['total_capacity_factor']}");
    println!("\n  {'Nome':<20} {'Papel':<14} {'Horas':>6} {'h/sem':>6} {'OK?'}");
    println!("  {'-'*55}");
    for d in result2["distribution"] {
        ok = d["within_limit"] ? "OK" : "EXCESSO";
        println!("  {d['name']:<20} {d['role']:<14} ";
    ...
    println!("  Carga minima: ";
        "{result2['after_distribution']['min_individual_hours']:.0f}h");
    // === COMPARACAO ===
    println!("\n\n  === COMPARACAO: ANTES VS DEPOIS ===\n");
    println!("  {'Cenario':<35} {'Max individual':>15} {'Excesso?':>10}");
    println!("  {'-'*62}");
    println!("  {'Cleiton sozinho':<35} {'4000h':>14} {'2160h':>10}");
    println!("  {'Familia nuclear (4 pessoas)':<35} ";
        "{result['after_distribution']['max_individual_hours']:>13.0f}h ";
        "{'ZERO':>10}");
    println!("  {'Familia extensa (7 pessoas)':<35} ";
...
// )
    println!("{'='*75}");
    println!("  OpenFamilyLabor: distribuicao matematica de carga.");
    println!("  1 pessoa faz 4000h -> 4+ pessoas fazem 1000h cada.");
    println!("  Mesma producao. Zero excesso. Zero burnout.");
    println!("{'='*75}");
