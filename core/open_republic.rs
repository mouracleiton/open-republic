...
// decorador: @dataclass
#[derive(Debug, Clone)]
struct NationMetrics {
    // Metricas de uma nacao membro.
    let population: i64 = 0;
    let housing_units: i64 = 0;
    let food_kg_month: f64 = 0.0;
    let energy_kwh_month: f64 = 0.0;
    let water_liters_day: f64 = 0.0;
    let healthcare_capacity: i64 = 0;
    let education_centers: i64 = 0;
    let innovation_count: i64 = 0;
    let ecological_health: f64 = 0.8;
    let commons_count: i64 = 0;
    let conflicts_open: i64 = 0;
    // decorador: @property
    fn qol_index(self) -> f64 {
...
struct Nation {
    ...
    languages: [texto];
    let founded_cycle: i64 = 0;
    let metrics: NationMetrics = field(default_factory=NationMetrics);
    let migration_status: MigrationStatus = MigrationStatus.FREE;
    let specialties: [texto] = field(default_factory=list) // areas de excelencia;
    let citizens_ids: {texto} = field(default_factory=set);
    let federal_proposals_voted: i64 = 0;
    let federal_projects_contributed: i64 = 0;
// ============================================================================
// Shared Infrastructure (Federal)
// ============================================================================
// decorador: @dataclass
#[derive(Debug, Clone)]
struct FederalInfrastructure {
    ...
    description: texto;
    let serving_nations: {texto} = field(default_factory=set) // nation_ids;
    let capacity: f64 = 0.0;
    let unit: String = "";
    let condition: f64 = 1.0;
    let build_cycles: i64 = 0;
    let contributing_nations: {texto} = field(default_factory=set);
    let labor_invested: f64 = 0.0 // horas totais investidas;
#[derive(Debug, Clone)]
struct InfrastructureManager {
    ...
    funcao propose(self, name: texto, itype: InfrastructureType,
        ...
        return iid;
    fn contribute(self, infra_id: texto, nation_id: texto, labor_hours: flutuante) {
        proj = self.projects.get(infra_id);
        if proj {
            proj.contributing_nations.add(nation_id);
            proj.labor_invested += labor_hours;
    fn advance_build(self, infra_id: texto) -> {texto: qualquer} {
...
struct MigrationSystem {
    ...
    fn flow_stats(self) -> {texto: qualquer} {
...
struct Treaty {
    ...
    ttype: TreatyType;
    description: texto;
    let signatories: {texto} = field(default_factory=set);
    let terms: {texto: texto} = field(default_factory=dict);
    let status: String = "proposed"  // proposed, active, violated, dissolved;
    let created_cycle: i64 = 0;
#[derive(Debug, Clone)]
struct TreatySystem {
    ...
    funcao propose(self, name: texto, ttype: TreatyType, description: texto,
        ...
        return tid;
    fn sign(self, treaty_id: texto, nation_id: texto) {
...
struct FederalProposal {
    ...
    let votes_by_nation: Dict[texto, (flutuante, flutuante)] = field(default_factory=dict);
    // nation_id -> (votes_for, votes_against) weighted by population
    let status: String = "open"  // open, passed, rejected;
    let cycle_submitted: i64 = 0;
    let affects_infra: String = ""  // infra_id if applicable;
#[derive(Debug, Clone)]
struct FederalGovernance {
    ...
    fn close(self, proposal_id: texto, total_nations: inteiro) -> {texto: qualquer} {
...
struct GrandProject {
    ...
    let current_labor: f64 = 0.0;
    let contributing_nations: {texto} = field(default_factory=set);
    let status: String = "recruiting"  // recruiting, active, completed;
    let benefit: String = ""  // quem se beneficia;
#[derive(Debug, Clone)]
struct GrandProjectManager {
    ...
    funcao propose(self, name: texto, description: texto, required_nations: inteiro,
        ...
        self.projects[pid] = GrandProject(;
            ...
            benefit = benefit);
        return pid;
    fn contribute(self, project_id: texto, nation_id: texto, labor_hours: flutuante) {
    ...
    fn progress(self, project_id: texto) -> {texto: qualquer} {
        proj = self.projects.get(project_id);
        if ! proj {
            return {};
        return {;
...
struct OpenRepublic {
    ...
    //
    fn __init__(self, name: texto = "OpenRepublic") {
        self.name = name;
...
// ============================================================================
fn create_sample_republic() -> OpenRepublic {
...
if __name__ == "__main__" {
    random.seed(42);
    np.random.seed(42);
    println!("Criando Republica dos Bens Comuns com 6 nacoes...\n");
    republic = create_sample_republic();
    println!("Nacoes fundadoras: {len(republic.nations)}");
    para cada (nid, n) em republic.nations.items(): {
        println!("  {n.name:<15} ({n.region:<15}) pop={n.metrics.population:>6}  ";
    ...
    para cada (pid, proj) em republic.grand_projects.projects.items(): {
        ...
        println!("  {proj.name[:30]:<30} [{bar}] {p['labor_progress_pct']:>5.1f}%  ";
