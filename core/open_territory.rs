...
struct PopulationCenter {
    ...
    population: inteiro;
    area_km2: flutuante;
    let current_density: f64 = 0.0;
    let target_density: f64 = 0.0 // meta da Republica;
    let status: String = "desconhecido"  // esvaziar, manter, crescer, criar;
    // decorador: @property
    ...
    // decorador: @property
    fn overpopulated(self) -> bool {
        return self.current_density > 2000;
    // decorador: @property
    fn underpopulated(self) -> bool {
        return self.current_density < 50;
    // decorador: @property
    fn surplus_population(self) -> i64 {
...
struct NewMetropolis {
    ...
    let current_population: i64 = 0 // geralmente <50k (interior);
    let area_km2: f64 = 500.0;
    // Infraestrutura planejada
    let has_healthcare: bool = false // nivel Sirio-Libanes;
    let has_education: bool = false // universidade + escolas;
    let has_transport: bool = false // OpenMobility (metro, VLT, aeroporto);
    let has_energy: bool = false // OpenEnergy (solar/eolica);
    let has_network: bool = false // OpenNetwork/OpenProtocol;
    let has_fablab: bool = false // FabLab industrial;
    let has_terminal: bool = false // OpenTerminal em todo estabelecimento;
    let has_housing: bool = false // OpenCivilConstruction;
    let has_food_production: bool = false // agricultura urbana periurbana;
    let has_water: bool = false // saneamento + reservatorio;
    // Status de construcao
    let construction_phase: String = "planejamento"  // planejamento, obras, habitavel, ativa;
    let construction_progress: f64 = 0.0 // 0-1;
    let est_completion_months: i64 = 36;
    // Atrativos (por que morar aqui?)
    let incentives: [texto] = field(default_factory=list);
    // decorador: @property
...
struct TerritoryEngine {
    ...
    fn _init_current_population(self) {
    ...
    fn diagnosis(self) -> {texto: qualquer} {
    ...
    fn new_metroplan_summary(self) -> {texto: qualquer} {
    ...
    fn build_infrastructure(self, metro_id: texto) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENTERRITORY -- REDISTRIBUICAO POPULACIONAL E NOVAS METROPOLES");
    println!("  'SP ta pequeno demais. O interior ! existe mais.'");
    println!("=" * 80);
    // === 1. DIAGNOSTICO ===
    println!("\n\n  === 1. DIAGNOSTICO: DESEQUILIBRIO TERRITORIAL ===\n");
    println!("  {'Cidade':<25} {'Regiao':<15} {'Populacao':>12} {'Densidade':>12} {'Status'}");
    println!("  {'-'*80}");
    para c em ordene(engine.current_centers.values(), {
                    key = (x) -> -x.current_density):;
        println!("  {c.name:<25} {c.region:<15} {c.population:>12,} ";
            "{c.current_density:>10.0f}/km2  {c.status.upper()}");
    // === 2. DESEQUILIBRIO ===
    println!("\n\n  === 2. O PROBLEMA ===\n");
    diag = engine.diagnosis();
    println!("  Hiper-saturadas: {diag['hiper_saturadas']}");
    println!("  Saturadas: {diag['saturadas']}");
    println!("  Vazios demograficos: {diag['vazios_demograficos']}");
    println!("  Populacao excedente (precisa sair): {diag['populacao_excedente']:,}");
    println!("  Pior caso: {diag['pior_caso']['nome']} ";
        "({diag['pior_caso']['densidade']}, ";
        "excedente: {diag['pior_caso']['excedente']})");
    println!("  Desequilibrio: {diag['desequilibrio']}");
    ...
    para cada (region, count) em ordene(plan["por_regiao"].items()): {
        println!("    {region:<15} {count} metropoles");
    println!("\n  Por tamanho:");
    println!("    ALPHA (1M-2M):    {plan['alfa_1m_2m']}");
    println!("    BETA (500k-1M):   {plan['beta_500k_1m']}");
    println!("    GAMMA (200k-500k):{plan['gamma_200k_500k']}");
    ...
    for region in ordene(by_region.keys()) {
        ...
        for m in by_region[region] {
            println!("    [{m.metro_id}] {m.name:<25} {m.state}  ";
    ...
    println!("  {len(engine.new_metropolises)} metropoles construidas.");
    println!("  Cada uma tem:");
    println!("    [OK] Hospital nivel Sirio-Libanes");
    println!("    [OK] Universidade + escolas");
    println!("    [OK] Metro/VLT + OpenMobility");
    println!("    [OK] Energia solar/eolica");
    println!("    [OK] OpenNetwork/OpenProtocol");
    println!("    [OK] FabLab industrial");
    println!("    [OK] OpenTerminal em todo lugar");
    println!("    [OK] Moradia (OpenCivilConstruction)");
    println!("    [OK] Agricultura periurbana");
    println!("    [OK] Saneamento + agua");
    ...
    para cid, name, from_city, to_metro in migrations: {
        ...
        println!("    Status saida: {result['from_status']}");
        println!("    Bonus: +{result['bonus_credit']} credito");
        println!("    Voluntario: SIM (P2)");
    // === 7. SP SENDO ESVAZIADA PARA OBRAS ===
    println!("\n\n  === 7. SAO PAULO: ESVAZIAR PARA REFORMAR ===\n");
    sp = engine.current_centers.get("Sao Paulo");
    if sp {
        println!("  ANTES:");
        println!("    Populacao: {sp.population:,}");
        println!("    Densidade: {sp.current_density:.0f} hab/km2");
        println!("    Excedente: {sp.surplus_population:,} pessoas precisam sair");
        println!("\n  DEPOIS (meta):");
        println!("    Populacao meta: {int(2000 * sp.area_km2):,}");
        println!("    Densidade meta: 2.000 hab/km2 (ainda urbano, mas respiravel)");
        println!("    Reducao: {sp.surplus_population:,} pessoas ({sp.surplus_population/sp.population*100:.0f}%)");
        println!("\n  O QUE FAZER COM ESPACO LIBERADO:");
        println!("    - Reformar infraestrutura antiga");
        println!("    - Criar espacos verdes");
        println!("    - Reconstruir moradias dignas");
        println!("    - Reduzir transito");
        println!("    - Limpar rios (Tiete, Pinheiros)");
        println!("    - Descongestionar hospitais");
    // === 8. STATS ===
    println!("\n\n  === 8. ESTATISTICAS ===\n");
    s = engine.stats();
    println!("  Novas metropoles: {s['total_metropoles']}");
    println!("  Construidas: {s['construidas']}");
    println!("  Migracoes oferecidas: {s['migracoes_oferecidas']}");
    println!("  Capacidade total: {s['plano']['capacidade_total']:,}");
    println!("  Populacao excedente: {s['diagnostico']['populacao_excedente']:,}");
    // === FILOSOFIA ===
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA DO OPENTERRITORY");
    println!("{'='*80}");
    println!(""";
O PROBLEMA:;
...
PRINCIPIOS:;
    ...
    let P3: Construir novas metropoles = trabalho massivo (OpenLaborRelay).;
    P4: Plano votado pela assembleia. Regioes decidem suas prioridades.;
// )
    println!("{'='*80}");
    println!("  OpenTerritory: {s['total_metropoles']} novas metropoles, ";
        "{s['plano']['capacidade_total']:,} de capacidade.");
    println!("  'Interior' ! existe mais. Todo ponto && centro.");
    println!("{'='*80}");
