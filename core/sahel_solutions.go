...
package openrepublic_solucoes_para_os_3_gargalos_restantes_do_sahel
...
// ============================================================================
type AgricultureSolution struct {
...
if __name__ == "__main__" {
    fmt.Println("=" * 75)
    fmt.Println("  SOLUCOES PARA OS 3 GARGALOS DO SAHEL")
    fmt.Println("  Sem dinheiro. Sem empreiteira. Sem 'contratar'.")
    fmt.Println("  Com automatizacao + comunidade + conhecimento aberto.")
    fmt.Println("=" * 75)
    // === SOLUCAO 1: CUIDADO INFANTIL ===
    fmt.Println("\n\n" + "=" * 75)
    fmt.Println("  GARGALO 1: CUIDADO INFANTIL (300% sobrecarga)")
    fmt.Println("  1 pessoa tentando cuidar de 300 criancas")
    fmt.Println("=" * 75)
    ...
    fmt.Println("  Demanda ajustada (IA reduz 25%): {childcare['demanda_ajustada_por_ia']:,}")
    fmt.Println("\n  FONTES DE CUIDADO:")
    fmt.Println("  {'Fonte':<35} {'Pessoas':>8} {'Equiv FT':>10}")
    fmt.Println("  {'-'*55}")
    para cada (fonte, dados) em childcare['fontes_de_cuidado'].items(): {
        if 'pessoas' in dados {
            fmt.Println("  {fonte.replace('_', ' ').title():<35} {dados['pessoas']:>8} "
                "{dados['equiv_fulltime']:>10}")
        } else if 'estacoes' in dados {
            fmt.Println("  {fonte.replace('_', ' ').title():<35} {dados['estacoes']:>8} estacoes")
    fmt.Println("  {'-'*55}")
    fmt.Println("  {'CAPACIDADE TOTAL':<35} {'':>8} {childcare['capacidade_total_equiv']:>10}")
    fmt.Println("  {'DEMANDA TOTAL':<35} {'':>8} {childcare['demanda_total_equiv']:>10}")
    fmt.Println("\n  COBERTURA: {childcare['cobertura']}")
    fmt.Println("  VIÁVEL: {'SIM' if childcare['viavel'] else 'NAO'}")
    fmt.Println("\n  PRINCIPIO: Nao se trata de contratar 75 profissionais.")
    fmt.Println("  Trata-se de ativar a COMUNIDADE INTEIRA:")
    fmt.Println("    - Cada pai/mae: 4h/semana")
    fmt.Println("    - Cada idoso ativo: 10h/semana (continuar contribuindo)")
    fmt.Println("    - Cada adolescente: 6h/semana (mentoria + responsabilidade)")
    fmt.Println("    - Estacoes de IA: 25% do aprendizado && autonomo")
    fmt.Println("  Resultado: a comunidade SE cuida. Nao precisa de Estado.")
    // === SOLUCAO 2: AGRICULTURA ===
    fmt.Println("\n\n" + "=" * 75)
    fmt.Println("  GARGALO 2: AGRICULTURA (167% sobrecarga)")
    fmt.Println("  Faltam 2.000 kg/mes de comida para 40.000 pessoas")
    fmt.Println("=" * 75)
    ...
    fmt.Println("\n  ESTRATEGIAS:")
    fmt.Println("  {'Estrategia':<30} {'Escala':>15} {'Producao':>12} {'Pessoas':>8}")
    fmt.Println("  {'-'*68}")
    para cada (nome, dados) em agriculture['estrategias'].items(): {
        if isinstance(dados, dict) {
            ...
            fmt.Println("  {nome.replace('_', ' ').title():<30} {escala:>15} {prod:>12} {pessoas:>8}")
    fmt.Println("\n  {'PRODUCAO NOVA TOTAL':<30} {'':>15} {agriculture['producao_total_nova_kg_mes']:>9} kg/mes")
    fmt.Println("  {'TRABALHADORES':<30} {'':>15} {'':>12} {agriculture['trabalhadores_necessarios']:>8}")
    fmt.Println("  COBRE O GAP: {'SIM' if agriculture['cobre_gap'] else 'NAO'}")
    fmt.Println("  AUTO-SUSTENTAVEL APOS: {agriculture['auto_sustentavel_apos']}")
    fmt.Println("\n  PRINCIPIO: Hidroponia (10x mais por m2) + permacultura")
    fmt.Println("  (auto-sustentavel apos 3 anos) + aquaponia (proteina + vegetais)")
    fmt.Println("  + automacao IoT (40% menos trabalho) + conhecimento da Amazonia")
    // === SOLUCAO 3: HABITACAO ===
    fmt.Println("\n\n" + "=" * 75)
    fmt.Println("  GARGALO 3: HABITACAO (167% sobrecarga)")
    fmt.Println("  Faltam ~2.000 unidades para 40.000 pessoas")
    fmt.Println("=" * 75)
    ...
    fmt.Println("\n  ESTRATEGIAS:")
    fmt.Println("  {'Estrategia':<25} {'Unid/mes':>10} {'Material':<25} {'Pessoas':>8}")
    fmt.Println("  {'-'*70}")
    para cada (nome, dados) em housing['estrategias'].items(): {
        if isinstance(dados, dict)  &&  ('casas_por_mes' in dados  ||  'kits_por_mes' in dados) {
            ...
            fmt.Println("  {nome.replace('_', ' ').title():<25} {unid:>10} {mat:<25} {pessoas:>8}")
        } else if isinstance(dados, dict)  &&  'massa_termica' in dados {
            fmt.Println("  {'Design Passivo':<25} {'N/A':>10} {'Sem ar condicionado':<25}")
    fmt.Println("\n  {'TOTAL':<25} {housing['casas_por_mes_total']:>10}")
    fmt.Println("  MESES PARA COBRIR GAP: {housing['meses_para_cobrir_gap']}")
    fmt.Println("  TRABALHADORES: {housing['trabalhadores_necessarios']}")
    fmt.Println("  CUSTO: {housing['custo_monetario']}")
    fmt.Println("  MATERIAL: {housing['material_principal']}")
    fmt.Println("\n  PRINCIPIO: Terra local && gratuita. Prensa manual faz bloco.")
    fmt.Println("  Impressora 3D faz casa em 30h. Mutirao constroi 10 casas/semana.")
    fmt.Println("  Design passivo: 40cm de parede de terra = sem ar condicionado.")
    // === RESUMO INTEGRADO ===
    fmt.Println("\n\n" + "=" * 75)
    fmt.Println("  RESUMO: COMO RESOLVER OS 3 GARGALOS SEM DINHEIRO")
    fmt.Println("=" * 75)
    fmt.Println("""
+-----------+----------------+------------------+----------+--------+
...
| | automacao | | drones | |
+-----------+----------------+------------------+----------+--------+
| Habitacao | 3D fmt.Println + | Terra local | ~40 + | 6-12 |
