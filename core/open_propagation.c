...
#ifndef OPENPROPAGATION_ESTRATEGIA_DE_DIVULGACAO_MASSIVA_H
#define OPENPROPAGATION_ESTRATEGIA_DE_DIVULGACAO_MASSIVA_H
...
// ============================================================================
typedef struct PropagationFront {
...
// ============================================================================
// decorador: @dataclass
typedef struct CommunityPropagation {
...
// ============================================================================
typedef struct PropagationEngine {
    ...
    {texto: qualquer} stats(self) {
        return {;
            "pilotos_planejados": sizeof(self.pilots),;
            "pecas_conteudo": sizeof(self.content),;
            "estrategias_comunitarias": sizeof(self.communities),;
            "aliancas_potenciais": sizeof(self.alliances),;
            "produtos_publicos": sizeof(self.products),;
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENPROPAGATION -- ESTRATEGIA DE DIVULGACAO MASSIVA");
    printf("  Nao grite. Demonstre. 1 prova > 1000 teorias.");
    printf("=" * 80);
    // === 1. AS 5 FRENTES ===
    printf("\n\n  === AS 5 FRENTES DE PROPAGACAO ===\n");
    printf("  1. DEMONSTRACAO REAL (piloto funcionando) -- A QUE MATRIA");
    printf("  2. CONTEUDO VIRAL (X/Twitter, video, infografico)");
    printf("  3. COMUNIDADES REAIS (CONAQ, MST, G10, ASA)");
    printf("  4. ALIANCAS (artistas, ONGs, jornalistas)");
    printf("  5. PRODUTO PUBLICO (simulador, app, landing page)");
    // === 2. PILOTOS ===
    printf("\n\n  === PILOTOS REAIS ({len(engine.pilots)}) ===\n");
    /* TODO: iterador C manual para p em engine.pilots */
        printf("\n  {p.name} ({p.location})");
        printf("    Demonstra: {p.what_demonstrates}");
        printf("    Impacto: {p.expected_impact}");
        printf("    Custo: {p.cost}");
        printf("    Prazo: {p.timeline}");
        printf("    Viral: {p.viral_potential}");
        printf("    Doc: {p.documentation}");
    // === 3. CONTEUDO ===
    printf("\n\n  === CONTEUDO VIRAL ({len(engine.content)}) ===\n");
    /* TODO: iterador C manual para c em engine.content */
        printf("\n  [{c.format}] {c.topic}");
        printf("    Hook: '{c.hook}'");
        printf("    Plataforma: {c.platform}");
        printf("    CTA: {c.call_to_action}");
        printf("    Esperado: {c.expected_reach}");
    // === 4. COMUNIDADES ===
    printf("\n\n  === EXPANSAO COMUNITARIA ({len(engine.communities)}) ===\n");
    /* TODO: iterador C manual para c em engine.communities */
        printf("  {c.strategy}");
        printf("    Metodo: {c.method}");
        printf("    Fator: {c.expansion_factor}");
    // === 5. ALIANCAS ===
    printf("\n\n  === ALIANCAS ({len(engine.alliances)}) ===\n");
    /* TODO: iterador C manual para a em engine.alliances */
        printf("  [{a.priority}] {a.ally_type}: {a.who}");
        printf("    Eles ganham: {a.what_they_gain[:60]}");
        printf("    Republica ganha: {a.what_republica_gains[:60]}");
    // === 6. PRODUTOS PUBLICOS ===
    printf("\n\n  === PRODUTOS PUBLICOS USAVEIS ({len(engine.products)}) ===\n");
    /* TODO: iterador C manual para p em engine.products */
        printf("  [{p.status}] {p.name}");
        printf("    O que: {p.what}");
        printf("    Viraliza porque: {p.why_viral}");
    // === 7. PLANO DE EXECUCAO ===
    printf("\n\n  === PLANO DE EXECUCAO ===\n");
    plan = engine.execution_plan();
    /* TODO: iterador C manual para fase em plan */
        printf("\n  {fase['fase']}:");
        /* TODO: iterador C manual para acao em fase["acoes"] */
            printf("    -> {acao}");
        printf("    Custo: {fase['custo']}");
        printf("    Resultado: {fase['resultado']}");
    // === 8. CALENDARIO ===
    printf("\n\n  === CALENDARIO DE CONTEUDO (X/Twitter) ===\n");
    cal = engine.content_calendar();
    /* TODO: iterador C manual para day em cal */
        printf("  {day['dia']:<12} {day['tipo']}: {day['exemplo']}");
    // === 9. METRICAS ===
    printf("\n\n  === METRICAS ===\n");
    /* TODO: iterador C manual para m em engine.key_metrics() */
        printf("  {m['metrica']}");
        printf("    3 meses: {m['meta_3meses']}");
        printf("    6 meses: {m['meta_6meses']}");
        printf("    12 meses: {m['meta_12meses']}");
    // === 10. O QUE NAO FAZER ===
    printf("\n\n  === O QUE NAO FAZER (ERROS QUE MATAM) ===\n");
    /* TODO: iterador C manual para dont em engine.what_NOT_to_do() */
        printf("  X {dont}");
    // === STATS ===
    printf("\n\n  === ESTATISTICAS ===\n");
    s = engine.stats();
    /* para cada (k, v) em s.items(): */
        printf("  {k:<30} {v}");
    printf("\n{'='*80}");
    printf("  OpenPropagation: {s['pilotos_planejados']} pilotos, ";
        "{s['pecas_conteudo']} pecas de conteudo, ";
        "{s['aliancas_potenciais']} aliancas.");
    printf("  {s['principio']}");
    printf("{'='*80}");

#endif // OPENPROPAGATION_ESTRATEGIA_DE_DIVULGACAO_MASSIVA_H
