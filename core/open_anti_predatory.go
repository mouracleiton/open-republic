...
package openantipredatory_politica_anti_competicao_predatoria
...
// ============================================================================
// decorador: @dataclass
type PredationCase struct {
    // Caso de competicao predatoria detectado.
    case_id := "" // string
    predator := "" // string
    victim := "" // string
    predation_type := PredationType.MONOPOLY // PredationType
    severity := PredationSeverity.WARNING // PredationSeverity
    description := "" // string
    evidence := field(default_factory=list) // [texto]
    action := AntiPredationAction.WARNING // AntiPredationAction
    status := "detectado" // string
    date := "" // string
// ============================================================================
type AntiPredationEngine struct {
    ...
    funcao detect(self, predator: texto, predation_type: PredationType,
            description: texto, evidence: [texto] = nil,
    ...
    func _assess_severity(self, ptype: PredationType) PredationSeverity {
        severity_map = {
    ...
    func _decide_action(self, severity: PredationSeverity) AntiPredationAction {
        action_map = {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENANTIPREDATORY -- POLITICA ANTI COMPETICAO PREDATORIA")
    fmt.Println("  Competir melhorando = OK. Competir destruindo = BLOQUEADO.")
    fmt.Println("=" * 80)
    // === 1. TIPOS DE PREDACAO ===
    fmt.Println("\n\n  === 1. TIPOS DE COMPETICAO PREDATORIA ({len(PredationType)}) ===\n")
    for _, pt := range PredationType {
        sev = engine._assess_severity(pt)
        fmt.Println("  [{sev.value[0]:<12}] {pt.value}")
    // === 2. CASOS DE DETECCAO ===
    fmt.Println("\n\n  === 2. CASOS DETECTADOS ===\n")
    cases = [
    ...
    fmt.Println("  Market dominance check: {r.get('status', r.get('action', '?'))}")
    // Resource hoarding
    r = engine.check_resource_hoarding("Acumulador", 0.35)
    fmt.Println("  Resource hoarding check: {r.get('status', r.get('action', '?'))}")
    // Walled garden
    r = engine.check_walled_garden("RedeFechada", false, false)
    fmt.Println("  Walled garden check: {r.get('status', r.get('action', '?'))}")
    // Planned obsolescence
    r = engine.check_planned_obsolescence("FoneDescartavel", 1.5, false)
    fmt.Println("  Obsolescence check: {r.get('status', r.get('action', '?'))}")
    // === 4. STATS ===
    fmt.Println("\n\n  === 4. ESTATISTICAS ===\n")
    s = engine.stats()
    para cada (k, v) em s.items(): {
        fmt.Println("  {k:<25} {v}")
    fmt.Println("\n{'='*80}")
    fmt.Println("  AntiPredatory: {s['bloqueados']} bloqueados. Competir destruindo = PROIBIDO.")
    fmt.Println("{'='*80}")
