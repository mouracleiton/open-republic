...
package openrepublic_emenda_constitucional_autonomia_corporal
...
// ============================================================================
type HistoricalViolation struct {
...
if __name__ == "__main__" {
    fmt.Println("=" * 80)
    fmt.Println("  EMENDA CONSTITUCIONAL: AUTONOMIA CORPORAL")
    fmt.Println("  'O corpo && a fronteira ultima da liberdade.'")
    fmt.Println("=" * 80)
    // === 1. The Amendment ===
    fmt.Println("""
PRINCIPIO CONSTITUCIONAL NUMERO 2:
...
// )
    ...
    for _, r := range RIGHTS {
        fmt.Println("\n  {'='*70}")
        fmt.Println("  DIREITO: {r.right.value.upper()}")
        fmt.Println("  {'='*70}")
        fmt.Println("\n  O QUE SIGNIFICA:\n  {r.what_it_means}")
    ...
    for _, v := range HistoricalViolation.VIOLATIONS {
        fmt.Println("\n  {v['perpetrator']} ({v['country']}, {v['era']})")
        fmt.Println("    Vitimas: {v['victims']}")
        fmt.Println("    Violacao: {v['violation']}")
        fmt.Println("    Licao: {v['lesson']}")
    // === Philosophy ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  POR QUE ISSO É CONSTITUCIONAL")
    fmt.Println("{'='*80}")
    fmt.Println("""
