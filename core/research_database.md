...
**Arquivo original:** `core/research_database.py`

**Descricao:** ==========================================================
Compilacao das pesquisas mais importantes em:
1. Materiais biodegradaveis e bioplasticos
2. Reciclagem e economia circular
3. Energia renovavel e armazenamento
4. Agricultura sustentavel e seguranca alimentar
5. Captura de carbono e remediacao ambiental
6. Purificacao de agua
7. Bioengenharia e bioremidiacao
Cada entrada tem: paper/research, descoberta, aplicabilidade na Republica,
TRL (Technology Readiness Level), e como integrar.
Baseado em pesquisas reais publicadas em Nature, Science, Cell Press,
PNAS, Environmental Science & Technology, e outras.
...
classe ResearchArea herda de Enum:
...
classe ResearchEntry:
    ...
    trl: TRL
    seja materials_needed: [texto] = field(default_factory=list)
    seja co2_impact: texto = ""
    seja scalability: texto = ""
    seja open_source: logico = falso
    seja patent_free: logico = verdadeiro
    seja integration: texto = ""
...
seja RESEARCH_DB: [ResearchEntry] = [
    ...
    ResearchEntry("R-003", "Micelio estrutural (Ecovative design)",
        ...
        patent_free = falso, // Ecovative tem patente mas expirou em varios paises
    ...
    ResearchEntry("R-005", "Recina epoxi bio de oleo de soja",
        ...
        "Para impressao SLA e compoaitos.",
    ...
    ResearchEntry("R-007", "Reciclagem de bateria litio-ion (LiFePO4)",
        ...
        "Processo hidrometalurgico recupera 99% de litio, cobalto, e ferro "
        ...
        scalability = "Acido citrico e abundante (suco de laranja)",
        integration = "OpenReverseLogistics: cadeia de bateria fechada"),

    ResearchEntry("R-008", "Bio-lixiviacao de ouro de e-waste",
        ...
        co2_impact = "Elimina cianeto quimico e mercurio da mineracao",
    ...
    ResearchEntry("R-009", "Despolimerizacao catalitica de PET",
        ...
        "Reciclagem infinita de PET (nao downcycling)",
        TRL.PROTOTYPE,
    ...
    ResearchEntry("R-012", "Celula solar de perovskita",
        ...
        TRL.DEMO,
        materials_needed = ["chumbo ou estanho", "halogenios", "solvente"],
    ...
    ResearchEntry("R-013", "Bateria de areia (sand battery)",
        ...
        co2_impact = "Zero. Areia e abundante e nao toxica.",
    ...
    ResearchEntry("R-014", "Bateria de gravidade (Energy Vault)",
        ...
        TRL.PROTOTYPE,
        materials_needed = ["concreto (ou terra comprimida)", "motor/generator", "cabos"],
    ...
    ResearchEntry("R-015", "Celula de combustivel microbiana (MFC)",
        ...
        "Environmental Science & Technology",
        "Bacterias comem materia organica e geram eletricidade. Trata "
    ...
    ResearchEntry("R-018", "Biochar como fertilizante + sequestrador",
        ...
        "50%, retem agua, e sequestra carbono por milenios (Terra Preta).",
    ...
    ResearchEntry("R-020", "Insetos como proteina (BSF larvae)",
        ...
        "e produzem 43% proteina. 80x mais eficiente que boi.",
    ...
    ResearchEntry("R-021", "Agrofloresta sucessional (Ernst Gotsch)",
        ...
        "Apos 5 anos, e auto-sustentavel. Nao precisa irrigar.",
    ...
    ResearchEntry("R-023", "Captura direta de ar (DAC) com minerais",
        ...
        "Nature",
        "Peridotito (rocha abundante) reage com CO2 do ar e o transforma "
    ...
    ResearchEntry("R-025", "Fitorremediacao com girassol",
        ...
        "Plantar -> crescer -> colher -> queimar e recuperar metal.",
    ...
    ResearchEntry("R-028", "Captura de agua da neblina (fog catching)",
        ...
        TRL.WIDESPREAD,
        materials_needed = ["rede de polipropileno (ou fibra de coco)", "estrutura"],
    ...
    ResearchEntry("R-031", "Concreto de cinza volante + bambu",
        ...
        scalability = "Bambu cresce em todo Brasil. Cinza e residuo industrial.",
    ...
    ResearchEntry("R-032", "Transitor de grafeno (ignicao de plastico)",
        ...
        TRL.PROTOTYPE,
        materials_needed = ["grafeno (ou grafite reciclado)", "solvente"],
    ...
    ResearchEntry("R-033", "Madeira transparente",
        ...
        "Janelas que nao quebram e isolam",
        TRL.PROOF,
    ...
    ResearchEntry("R-035", "Fermentacao precisa (Impossible-style)",
        ...
        "Sabor e nutricao de carne sem animal",
        TRL.COMMERCIAL,
...
classe ResearchIntegrator:
    ...
    funcao by_area(self, area: ResearchArea) -> [ResearchEntry]:
        retorne [r para r em self.db if r.area == area]

    funcao ready_to_deploy(self) -> [ResearchEntry]:
        // Pesquisas prontas para implementar (TRL >= 7).
        retorne ordene([r para r em self.db if r.trl.value >= 7],
                      key = (r) -> -r.trl.value)

    funcao needs_research(self) -> [ResearchEntry]:
        // Pesquisas que precisam mais desenvolvimento (TRL < 5).
        retorne [r para r em self.db if r.trl.value < 5]

    funcao integration_map(self) retorna Dict[texto, [texto]]:
        // Mapeia cada sistema Republica -> pesquisas que o melhoram.
        mapping = defaultdict(list)
        para cada r em self.db:
            se r.integration entao:
                system = r.integration.split(":")[0].strip()
                mapping[system].append("{r.title} (TRL {r.trl.value})")
        retorne dict(mapping)

    funcao material_requirements(self) retorna Dict[texto, [texto]]:
        // Todos os materiais necessarios para todas as pesquisas.
        materials = defaultdict(list)
        para cada r em self.db:
            para cada mat em r.materials_needed:
                materials[mat].append(r.entry_id)
        retorne dict(materials)

    funcao co2_impact_summary(self) -> {texto: qualquer}:
        // Resumo do impacto CO2 de todas as tecnologias.
        negative = [r para r em self.db if "negativo" in r.co2_impact.lower()  ou 
                    "-" in r.co2_impact[:5]]
        ...
        retorne {
            ...
            "biggest_impact": ordene(negative, key=(r) -> tamanho(r.co2_impact), reverse=verdadeiro)[:3],
        }
...
se __name__ == "__main__" entao:
    imprima("=" * 80)
    imprima("  OPENRESEARCH -- BASE DE CONHECIMENTO AMBIENTAL")
    imprima("  60+ pesquisas cientificas integradas na Republica")
    imprima("=" * 80)
    ...
    imprima("  Prontas para deploy: {stats['ready_to_deploy']}")
    imprima("  Precisam mais pesquisa: {stats['needs_more_research']}")
    imprima("  Sem patente (bem comum): {stats['patent_free']}")

    imprima("\n  POR AREA:")
    para cada (area, count) em ordene(stats["by_area"].items(), key=(x) -> -x[1]):
        imprima("    {area:<25} {count}")

    imprima("\n  POR TRL (prontidao):")
    para cada (trl, count) em ordene(stats["by_trl"].items()):
        imprima("    {trl:<15} {count}")

    // === Ready to deploy ===
    imprima("\n\n  === PRONTAS PARA IMPLEMENTAR (TRL >= 7) ===\n")
    para cada r em engine.ready_to_deploy():
        imprima("  [{r.trl.name}] {r.title}")
        imprima("    {r.key_finding[:90]}")
        imprima("    Integracao: {r.integration}")
        imprima("    CO2: {r.co2_impact}")
        imprima()
    ...
    para cada (system, techs) em ordene(imap.items()):
        imprima("\n  {system}:")
        para cada tech em techs:
            imprima("    -> {tech}")

    // === CO2 impact ===
    imprima("\n\n  === IMPACTO CO2 DAS TECNOLOGIAS ===\n")
    co2 = engine.co2_impact_summary()
    imprima("  Carbono NEGATIVO (sequestram): {co2['carbon_negative']}")
    imprima("  Carbono ZERO: {co2['carbon_zero']}")
    imprima("  Carbono REDUZEM: {co2['carbon_reducing']}")

    imprima("\n  TECNOLOGIAS CARBONO-NEGATIVAS:")
    para cada r em co2["biggest_impact"]:
        imprima("    {r.title}: {r.co2_impact}")

    // === Material requirements ===
    imprima("\n\n  === MATERIAIS NECESSARIOS ===\n")
    mats = engine.material_requirements()
    para cada mat em ordene(mats.keys()):
        count = tamanho(mats[mat])
        imprima("  {mat:<40} usado em {count} tecnologias")

    // === The full picture ===
    imprima("\n\n{'='*80}")
    imprima("  A BASE CIENTIFICA DA NOVA ORDEM")
    imprima("{'='*80}")
    imprima("""
  A Republica nao se baseia em ideologia. Baseia-se em CIENCIA.
  ...
  MATERIAIS:
    ...
    - PLA + canhamo: biodegradavel e mais resistente
  ...
  RECICLAGEM:
    ...
    - Bio-lixiviacao: bacterias mineram ouro de e-waste
