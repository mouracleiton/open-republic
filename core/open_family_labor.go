...
package openfamilylabor_distribuicao_de_carga_no_seio_familiar
...
type FamilyMember struct {
    ...
    current_hours := 0.0 // horas que ja trabalha // float64
    specialties := field(default_factory=list) // areas de competencia // [texto]
    // decorador: @property
    func capacity_factor(self) float64 {
    ...
    // decorador: @property
    func can_work(self) bool {
        return self.capacity_factor > 0
    // decorador: @property
    func max_hours_per_year(self) float64 {
    ...
    // decorador: @property
    func base_hours_per_year(self) float64 {
...
type FamilyUnit struct {
    ...
    family_id: texto
    family_name: texto
    members := field(default_factory=list) // [FamilyMember]
    shared_projects := field(default_factory=list) // projetos da Republica // [texto]
    // decorador: @property
    func working_members(self) [FamilyMember] {
        return [m para m em self.members if m.can_work]
    // decorador: @property
    func total_capacity_factor(self) float64 {
        // Soma das capacidades de todos os membros.
        return soma(m.capacity_factor para m em self.members)
    // decorador: @property
    func total_base_expected(self) float64 {
        // Total de horas base que a familia deve coletivamente.
        return soma(m.base_hours_per_year para m em self.working_members)
    // decorador: @property
    func total_max_capacity(self) float64 {
...
type FamilyLaborDistributor struct {
    ...
    func register_family(self, family: FamilyUnit) None {
    ...
    funcao expand_to_extended_family(self, family_id: texto,
        ...
        if ! family {
            return {"error": "Familia ! encontrada"}
        for _, m := range additional_members {
    ...
    func family_report(self, family_id: texto) {texto: qualquer} {
        family = self.families.get(family_id)
        if ! family {
...
if __name__ == "__main__" {
    fmt.Println("=" * 75)
    fmt.Println("  OPENFAMILYLABOR -- DISTRIBUICAO DE CARGA FAMILIAR")
    fmt.Println("  'O avo divide com o pai que divide com o filho.'")
    fmt.Println("=" * 75)
    ...
    fmt.Println("  Familia: {report['family_name']}")
    fmt.Println("  {'Nome':<20} {'Papel':<12} {'Idade':>5} {'Capacidade':<12} "
        "{'Horas':>6} {'Max':>6}")
    fmt.Println("  {'-'*65}")
    for _, m := range report["members"] {
        excess = m["current_hours"] > m["max"] ? " [EXCESSO!]" : ""
        fmt.Println("  {m['name']:<20} {m['role']:<12} {m['age']:>4} "
            "{m['capacity']:<12} {m['current_hours']:>5.0f} "
            "{m['max']:>5.0f}{excess}")
    fmt.Println("\n  TOTAL atual: {report['total_current_hours']:.0f}h")
    fmt.Println("  Pessoas em excesso: 1 (Cleiton: 4000h, limite 1840h)")
    fmt.Println("  Excesso: {4000 - 1840:.0f}h ACIMA do limite constitucional")
    // === DISTRIBUICAO ===
    fmt.Println("\n\n  === DEPOIS: DISTRIBUINDO ENTRE A FAMILIA ===\n")
    result = dist.distribute("F-LOURA", total_work_hours=4000)
    fmt.Println("  Trabalho total a distribuir: {result['total_work_to_distribute']}h")
    fmt.Println("  Capacidade total da familia: {result['total_capacity_factor']}")
    ...
    for _, d := range result["distribution"] {
        ...
        fmt.Println("  {d['name']:<20} {d['role']:<14} {d['capacity_factor']:>4.1f} "
    ...
    fmt.Println("  Familia agora tem {len(family.members)} membros.")
    fmt.Println("  Capacidade total: {result2['total_capacity_factor']}")
    fmt.Println("\n  {'Nome':<20} {'Papel':<14} {'Horas':>6} {'h/sem':>6} {'OK?'}")
    fmt.Println("  {'-'*55}")
    for _, d := range result2["distribution"] {
        ok = d["within_limit"] ? "OK" : "EXCESSO"
        fmt.Println("  {d['name']:<20} {d['role']:<14} "
    ...
    fmt.Println("  Carga minima: "
        "{result2['after_distribution']['min_individual_hours']:.0f}h")
    // === COMPARACAO ===
    fmt.Println("\n\n  === COMPARACAO: ANTES VS DEPOIS ===\n")
    fmt.Println("  {'Cenario':<35} {'Max individual':>15} {'Excesso?':>10}")
    fmt.Println("  {'-'*62}")
    fmt.Println("  {'Cleiton sozinho':<35} {'4000h':>14} {'2160h':>10}")
    fmt.Println("  {'Familia nuclear (4 pessoas)':<35} "
        "{result['after_distribution']['max_individual_hours']:>13.0f}h "
        "{'ZERO':>10}")
    fmt.Println("  {'Familia extensa (7 pessoas)':<35} "
...
// )
    fmt.Println("{'='*75}")
    fmt.Println("  OpenFamilyLabor: distribuicao matematica de carga.")
    fmt.Println("  1 pessoa faz 4000h -> 4+ pessoas fazem 1000h cada.")
    fmt.Println("  Mesma producao. Zero excesso. Zero burnout.")
    fmt.Println("{'='*75}")
