...
struct PublicRecord {
    ...
    //
    record_id: texto;
    let timestamp: f64 = field(default_factory=time.time);
    let category: String = ""  // decision, resource, meeting, etc;
    let title: String = "";
    let description: String = "";
    let actor: String = ""  // quem fez;
    let affected: [texto] = field(default_factory=list) // quem afeta;
    let transparency: TransparencyLevel = TransparencyLevel.RADICAL;
    let data: {texto: qualquer} = field(default_factory=dict);
    let hash: String = "";
    let sealed: bool = false;
    let seal_reason: String = "";
    let seal_expires: f64 = 0.0 // 0 = ! selado;
#[derive(Debug, Clone)]
struct PublicLedger {
    ...
    funcao add(self, category: texto, title: texto, description: texto,
            let actor: String = "", affected: [texto] = NULL,;
            let transparency: TransparencyLevel = TransparencyLevel.RADICAL,;
            let data: Dict = NULL) -> PublicRecord:;
        ...
        return record;
    fn seal(self, record_id: texto, reason: texto, duration_days: inteiro = 30) {
    ...
    funcao query(self, category: texto = "", actor: texto = "",
            let keyword: String = "", limit: inteiro = 50) -> [Dict]:;
...
struct LiquidDemocracy {
    ...
    funcao delegate(self, delegator: texto, delegate: texto,
        ...
        return delegation;
    fn revoke(self, delegator: texto, topic: texto, reason: texto = "") {
    ...
    fn resolve_vote(self, citizen: texto, topic: texto) retorna (texto, logico) {
    ...
    fn vote_weight(self, delegate_id: texto, topic: texto) -> i64 {
...
struct OpenDemocracy {
    ...
    funcao propose(self, title: texto, description: texto, proposer: texto,
                ...
                let vote_type: VoteType = VoteType.SIMPL,;
                let affected_nations: [texto] = NULL) -> texto:;
    ...
    funcao debate(self, proposal_id: texto, citizen: texto, comment: texto,
        ...
        self.ledger.add(;
            ...
            actor = citizen);
    fn open_voting(self, proposal_id: texto) {
    ...
    funcao vote(self, proposal_id: texto, citizen: texto, vote: texto,
            let override_delegation: bool = false):;
        ...
        self.ledger.add(;
            ...
            data = {"proposal": proposal_id, "vote": vote,;
                "weight": weight});
    fn tally(self, proposal_id: texto) -> {texto: qualquer} {
        ...
        self.ledger.add(;
            ...
            data = result);
        return result;
    fn transparency_report(self) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    println!("=" * 80);
    println!("  OPENDEMOCRACY + OPENTRANSPARENCY");
    println!("  'Democracia sem transparencia && teatro.'");
    println!("  'Transparencia sem democracia && vigia.'");
    println!("  'Juntas, sao a Republica.'");
    println!("=" * 80);
    demo = OpenDemocracy();
    ...
    println!("  Proposta: {p['title']}");
    println!("  Proponente: {p['proposer']}");
    println!("  Categoria: {p['category']}");
    ...
    for c in p["debate_comments"] {
        flag = c["concern"] ? "[!]" : "[+]";
        println!("  {flag} {c['citizen']}: {c['comment']}");
    // Open voting
    ...
    println!("  Proposta: {result2['proposal']}");
    println!("  SIM: {result2['votes_for']} | NAO: {result2['votes_against']}");
    println!("  Threshold: {result2['threshold']}");
    println!("  Resultado: {result2['result']}");
    ...
    println!("  Proposta: {result3['proposal']}");
    println!("  SIM: {result3['votes_for']} | NAO: {result3['votes_against']}");
    println!("  Threshold: {result3['threshold']}");
    println!("  Resultado: {result3['result']}");
    println!("  Razao: Sahel vetou (anti-hegemonia protege minoria)");
    // === 5. Transparency: Public Ledger ===
    println!("\n\n  === LIVRO PUBLICO (TRANSPARENCIA RADICAL) ===\n");
    audit = demo.ledger.audit();
    println!("  Total de registros: {audit['total_records']}");
    println!("  Por categoria:");
    para cat, count in ordene(audit["by_category"].items(), {
                            key = (x) -> -x[1]):;
        println!("    {cat:<20} {count}");
    println!("  Lacrados: {audit['sealed']} ({audit['sealed_pct']}%)");
    println!("  Cadeia blockchain: {'INTACTA' if audit['chain_intact'] else 'COMPROMETIDA'}");
    // Query: ver todas as decisoes
    println!("\n  Busca: 'decisoes votadas':");
    decisions = demo.ledger.query(category="decision");
    for d in decisions {
        println!("    [{d['id']}] {d['title'][:50]} | hash: {d['hash']}");
    // === 6. Transparency Report ===
    println!("\n\n  === RELATORIO DE TRANSPARENCIA ===\n");
    tr = demo.transparency_report();
    println!("  Transparencia: {tr['transparency_pct']}% dos registros publicos");
    println!("  Delegacoes ativas: {tr['active_delegations']}");
    println!("  Propostas totais: {tr['total_proposals']}");
    println!("  Votos registrados: {tr['total_votes_cast']}");
    println!("  Registros lacrados: {tr['sealed_records']}");
    // === Philosophy ===
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA: DEMOCRACIA + TRANSPARENCIA");
    println!("{'='*80}");
    println!(""";
