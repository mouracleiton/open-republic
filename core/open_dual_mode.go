...
package opendualmode_o_ideal_coletivo_e_o_executavel_agora
...
type Commodity struct {
    ...
    description := "" // string
    // Preco no EXECUTAVEL
    price_model := PricingModel.HOURLY // PricingModel
    price_brl := 0.0 // preco em dinheiro (transicao) // float64
    price_credit := 0.0 // equivalente em credito Republica // float64
    // No IDEAL
    ideal_equivalent := ""  // como funciona sem dinheiro // string
    ideal_credit_per_hour := 1.0 // credito base no modo ideal // float64
    // Valor real
    market_value := 0.0 // valor de mercado real // float64
    production_cost := 0.0 // custo de producao // float64
    fair_price := 0.0 // preco justo (sem exploracao) // float64
    // Quem produz
    skill_required := ""  // OpenSkills necessaria // string
    time_to_produce := 0.0 // horas para produzir // float64
// ============================================================================
...
type Transaction struct {
    ...
    mode: OperatingMode
    // EXECUTAVEL
    amount_brl := 0.0 // float64
    amount_credit := 0.0 // float64
    // IDEAL
...
type DualModeEngine struct {
    ...
    //
    func __init__(self, current_phase: inteiro = 0) {
    ...
    func _calculate_mode_mix(self, phase: inteiro) {texto: flutuante} {
    ...
    funcao transact(self, from_citizen: texto, to_citizen: texto,
                ...
                mode := nil, // OperatingMode
                amount_brl := nil // float64
                ) -> {texto: qualquer}:
        ...
        if mode == OperatingMode.EXECUTAVEL {
            brl = amount_brl is ! nil ? amount_brl : c.price_brl * hours
    ...
    func labor_as_commodity_report(self) {texto: qualquer} {
    ...
    func advance_phase(self) {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENDUALMODE -- O IDEAL COLETIVO E O EXECUTAVEL AGORA")
    fmt.Println("  Toda atividade humana = MERCADORIA. Trabalho = tipo de mercadoria.")
    fmt.Println("=" * 80)
    // === 1. OS DOIS MODOS ===
    fmt.Println("\n\n  === 1. OS DOIS MODOS ===\n")
    fmt.Println("  MODO IDEAL (destino):")
    fmt.Println("    Sem dinheiro. Sem propriedade. CC0.")
    fmt.Println("    Trabalho = base 1.0 + impacto. Credito expira.")
    fmt.Println("    Tudo gratis. Tudo bem comum.")
    fmt.Println("")
    fmt.Println("  MODO EXECUTAVEL (agora):")
    fmt.Println("    Dinheiro existe. Mercado existe.")
    fmt.Println("    Toda atividade = MERCADORIA. Trabalho = tipo de mercadoria.")
    fmt.Println("    OpenERP processa. OpenBusinessModel distribui.")
    fmt.Println("    Preco justo (sem exploracao). Fair price.")
    // === 2. TRABALHO COMO MERCADORIA ===
    fmt.Println("\n\n  === 2. TRABALHO COMO MERCADORIA (EXECUTAVEL) ===\n")
    report = engine.labor_as_commodity_report()
    fmt.Println("  EXECUTAVEL: {report['modo_executavel']['trabalho_e']}")
    para cada (job, price) em report["modo_executavel"]["exemplos"].items(): {
        fmt.Println("    {job:<25} {price}")
    ...
    for _, cid := range ["CMD-L01", "CMD-L02", "CMD-L04", "CMD-C02"] {
        p = engine.price_commodity(cid)
        fmt.Println("\n  {p['mercadoria']} ({p['tipo']}):")
        fmt.Println("    EXECUTAVEL: {p['MODELO_EXECUTAVEL']['preco_brl']} "
            "(mercado: {p['MODELO_EXECUTAVEL']['valor_mercado']})")
        fmt.Println("    IDEAL: {p['MODELO_IDEAL']['credito']} "
    ...
    para cada (k, v) em s.items(): {
        fmt.Println("  {k:<30} {v}")
    fmt.Println("\n{'='*80}")
    fmt.Println("  OpenDualMode: fase {s['fase_transicao']}, "
