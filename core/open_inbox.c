...
#ifndef OPENINBOX_A_FILA_UNIFICADA_DE_ATENCAO_H
#define OPENINBOX_A_FILA_UNIFICADA_DE_ATENCAO_H
...
typedef struct ContactRelation {
...
typedef struct QueueItem {
    ...
    char* content_preview = ""  // primeiros 100 chars;
    char* content_url = ""  // link profundo pro modulo;
    ...
    char* priority_reason = "";
    // Estado
    bool read = false;
    bool archived = false;
    bool acted_upon = false // usuario respondeu/interagiu;
    bool auto_archived = false // sistema arquivou (expirou);
    // Contexto
    bool requires_response = false // precisa de acao do usuario;
    double response_deadline = 0.0 // prazo para responder (0 = sem prazo);
    // Metadados anti-engajamento
    bool has_media = false;
    bool is_sensational = false // sistema detecta clickbait;
    bool engagement_bait = false // sistema detecta tentativa de viciar;
    // decorador: @property
    double age_hours(self) {
...
// ============================================================================
typedef struct AttentionPrioritizer {
    ...
    QueueItem prioritize(self, item: QueueItem) {
    ...
    bool detect_engagement_bait(text: texto) {
    ...
    // decorador: @staticmethod
    bool detect_sensational(text: texto) {
        // Detecta sensacionalismo.
        caps_ratio = soma(1 para c em text if c.isupper()) / maximo(sizeof(text), 1);
        has_many_emojis = text.count("!") + text.count("?") > 3;
        return caps_ratio > 0.3 || has_many_emojis;
// ============================================================================
// 4. A FILA UNIFICADA
// ============================================================================
typedef struct UnifiedInbox {
    ...
    None mark_family(self, handles: [texto]) {
        /* TODO: iterador C manual para h em handles */
            self.family_handles.add(h.lower().lstrip("@"));
    None mark_steward(self, handles: [texto]) {
        /* TODO: iterador C manual para h em handles */
            self.steward_handles.add(h.lower().lstrip("@"));
    ContactRelation _resolve_relation(self, handle: texto) {
    ...
    None _reset_daily_if_needed(self) {
    ...
    funcao enqueue(self, source: SourceModule, event_type: EventType,
                ...
                bool has_media = false) -> {texto: qualquer}:;
    ...
    [QueueItem] process_batch(self, limit: inteiro = 20) {
        ...
        batch = items[:limit];
        // Marcar como lido
        /* TODO: iterador C manual para item em batch */
            item.read = true;
            self.total_processed += 1;
        return batch;
    None _auto_archive(self) {
        // Arquiva itens nao lidos com mais de 24h.
        /* TODO: iterador C manual para item em self.queue */
            if (item.is_expired && ! item.archived) {
                item.archived = true;
                item.auto_archived = true;
                self.total_archived += 1;
    bool act_on(self, item_id: texto) {
        ...
        /* TODO: iterador C manual para item em self.queue */
            if (item.item_id == item_id) {
                ...
                return true;
        return false;
    bool dismiss(self, item_id: texto) {
    ...
    {texto: qualquer} stats(self) {
        ...
        return {;
            "total_in_queue": sizeof(self.queue) - self.total_archived,;
            "unread": unread,;
    ...
    {texto: qualquer} digest_24h(self) {
        ...
        /* TODO: iterador C manual para item em recent */
            src = item.source.value;
            summary[src]["count"] += 1;
            if (item.final_priority.value <= 1;
                &&  sizeof(summary[src]["highights"]) < 2;
                if ("highights" in summary[src] else True)) {
                if (sizeof(summary[src]["highlights"]) < 2) {
        ...
        return {;
            "period": "24h",;
            "categories": sizeof(summary),;
            "total": sizeof(recent),;
            "digest": dict(summary),;
        };
typedef struct InboxMode {
...
// ============================================================================
None simulate_five_apps_one_inbox(void) {
    ...
    printf("=" * 75);
    printf("  OPENINBOX -- A FILA UNIFICADA DE ATENCAO");
    printf('  "5 apps competindo = 5 ladroes de tempo. 1 fila = 1 cerebro."');
    printf("=" * 75);
    ...
    /* para source, etype, handle, name, preview, requires_resp, *rest in events: */
        ...
        printf("  [{status:>8}] [{prio:>9}] {source.value:<12} ";
            "@{handle:<20} {preview[:40]}");
    // === Estatisticas ===
    printf("\n\n  === FILA APOS INGESTAO ===\n");
    stats = inbox.stats();
    printf("  Total na fila:     {stats['total_in_queue']}");
    printf("  Nao lidos:         {stats['unread']}");
    printf("  Por fonte:         {stats['by_source']}");
    printf("  Por prioridade:    {stats['by_priority']}");
    printf("  Bait bloqueado:    {stats['bait_blocked']}");
    printf("  Limite diario:     {stats['items_today']}/{stats['daily_limit']}");
    // === Processar lote ===
    printf("\n\n  === PROCESSANDO LOTE (batch, ! ping) ===\n");
    batch = inbox.process_batch(limit=10);
    printf("  {'Prioridade':<12} {'Fonte':<12} {'De':<22} {'Preview'}");
    printf("  {'-'*75}");
    /* TODO: iterador C manual para item em batch */
        printf("  [{item.final_priority.name:<10}] {item.source.value:<12} ";
            "{item.sender_name:<22} {item.content_preview[:35]}");
        if (item.priority_reason  &&  item.priority_reason != "padrao") {
            printf("  {'':>25} razao: {item.priority_reason}");
    // === Modo Foco ===
    printf("\n\n  === MODO FOCO ATIVADO ===\n");
    inbox.mode = InboxMode.FOCUS;
    printf("  So EMERGENCIES + FAMILIA + SAUDE passam. Resto arquivado.\n");
    inbox.queue.clear();
    ...
    /* TODO: iterador C manual para item em batch */
        printf("  [{item.final_priority.name:<10}] {item.source.value:<12} ";
            "{item.sender_name:<22} {item.content_preview[:35]}");
    if (! batch) {
        printf("  (fila vazia -- tudo foi arquivado pelo Modo Foco)");
    stats = inbox.stats();
    ...
    printf("  Categorias: {digest['categories']}");
    printf("  Total: {digest['total']} itens nas ultimas 24h");
    /* para cada (cat, data) em digest["digest"].items(): */
        hl = "; ".join(h["from"] para h em data.get("highlights", []));
        printf("    {cat:<14} {data['count']} itens";
            hl ? + (" (destaques: {hl})" : ""));
    // === Filosofia ===
    printf("\n\n{'='*75}");
    printf("  COMO O OPENINBOX PROTEGE SEU CEREBRO");
    printf("{'='*75}");
    printf(""";
...
// )
    printf("{'='*75}");
    printf("  OpenInbox: {stats['total_enqueued']} eventos -> ";
        "{stats['total_processed']} processados -> ";
        "{stats['bait_blocked']} bait bloqueado.");
    printf("  5 apps. 1 fila. 0 ansiedade.");
    printf("{'='*75}");
// ============================================================================
if (__name__ == "__main__") {
    simulate_five_apps_one_inbox();

#endif // OPENINBOX_A_FILA_UNIFICADA_DE_ATENCAO_H
