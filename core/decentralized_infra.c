...
#ifndef OPENREPUBLIC_POLITICA_DE_INFRAESTRUTURA_DESCENTRALIZADA_H
#define OPENREPUBLIC_POLITICA_DE_INFRAESTRUTURA_DESCENTRALIZADA_H
...
typedef struct DecentralizationRule {
...
typedef struct DeviceNode {
    ...
    char* location = "comunitario";
    double uptime_hours = 16 // horas por dia ativo;
    bool available = true;
typedef struct InfrastructureOptimizer {
    ...
    //
    {texto: qualquer} calculate(self, population: inteiro = 1_000_000) {
    ...
    double _estimate_demand(population: inteiro) {
...
if (__name__ == "__main__") {
    printf("=" * 80);
    printf("  OPENREPUBLIC -- INFRAESTRUTURA DESCENTRALIZADA");
    printf("  'O datacenter && o ultimo recurso, ! o primeiro.'");
    printf("=" * 80);
    opt = InfrastructureOptimizer();
    // === 1. Workload Policies ===
    printf("\n\n  === ONDE CADA COISA RODA ===\n");
    printf("  {'Carga':<25} {'Onde':<15} {'Regra'}");
    printf("  {'-'*70}");
    /* para wl, policy in ordene(WORKLOAD_POLICIES.items(), */
                            key = (x) -> x[1].tier.value):;
        printf("  {wl.value:<25} {policy.tier.value:<15} {policy.rule.value}");
    printf("\n  Razoes:");
    /* para cada (wl, policy) em WORKLOAD_POLICIES.items(): */
        if (policy.tier == ProcessingTier.CENTRAL) {
            printf("    CENTRAL: {wl.value} -- {policy.reason}");
    ...
    printf("    Terminais burros:     {d['terminals_burros']:>10,}");
    printf("    Smartphones comunit.: {d['smartphones']:>10,}");
    printf("    Laptops:              {d['laptops']:>10,}");
    printf("    CPU cores agregado:   {d['total_device_cpu_cores']:>10,}");
    printf("    AI TOPS agregado:     {d['total_device_ai_tops']:>10,}");
    printf("\n  EDGE NODES (1 por bairro):");
    printf("    Quantidade:           {&&['count']:>10,}");
    printf("    Hardware:             {&&['hardware']}");
    printf("    CPU cores:            {&&['cpu_cores']:>10,}");
    printf("    RAM:                  {&&['ram_gb']:>10,} GB");
    printf("    Storage:              {&&['storage_tb']:>10,} TB");
    printf("    AI TOPS:              {&&['ai_tops']:>10,}");
    printf("\n  REGIONAL (1 por nacao):");
    printf("    Quantidade:           {r['count']:>10}");
    printf("    CPU cores:            {r['cpu_cores']:>10}");
    printf("    RAM:                  {r['ram_gb']:>10} GB");
    printf("    Storage:              {r['storage_tb']:>10} TB");
    printf("\n  DATACENTER CENTRAL (MINIMO):");
    printf("    Quantidade:           {c['count']:>10}");
    printf("    Proposito:            {c['purpose']}");
    printf("    CPU cores:            {c['cpu_cores']:>10}");
    printf("    GPUs (treino IA):     {c['gpu_count']:>10}");
    printf("    RAM:                  {c['ram_gb']:>10} GB");
    printf("    Storage (frio):       {c['storage_tb']:>10} TB");
    printf("    Potencia:             {c['power_kw']:>10} kW");
    printf("\n  DISTRIBUICAO DE CARGA:");
    printf("    No dispositivo:       {ld['on_device_pct']}%");
    printf("    No edge node:         {ld['on_edge_pct']}%");
    printf("    No regional:          {ld['on_regional_pct']}%");
    printf("    No central:           {ld['on_central_pct']}%");
    // === 3. Comparison ===
    printf("\n\n  === COMPARACAO: DATACENTER TRADICIONAL vs REPUBLICA ===\n");
    cmp = result["comparison"];
    printf("  {'Metrica':<35} {'Tradicional':>15} {'Republica':>15} {'Reducao'}");
    printf("  {'-'*75}");
    printf("  {'Servidores centrais':<35} {cmp['traditional_servers']:>15,} ";
        "{c['count']:>15} {cmp['central_servers_reduction_pct']}%");
    printf("  {'Potencia (kW)':<35} {int(cmp['traditional_power_mw']*1000):>15,} ";
        "{int(cmp['republic_power_kw']):>15,} {cmp['power_reduction_pct']}%");
    printf("  {'Custo (USD)':<35} {cmp['traditional_cost_usd']:>15,} ";
        "{cmp['republic_cost_usd']:>15,} {cmp['cost_reduction_pct']}%");
    // === 4. Resilience ===
    printf("\n\n  === RESILIENCIA (sem ponto unico de falha) ===\n");
    res = result["resilience"];
    printf("  Se datacenter central cair:");
    printf("    {res['central_down_impact']}");
    printf("  Se edge node cair:");
    printf("    {res['edge_down_impact']}");
    printf("  Se dispositivo cair:");
    printf("    {res['device_down_impact']}");
    printf("  Arquivos P2P:");
    printf("    {res['p2p_file_survival']}");
    // === 5. Calculate for different scales ===
    printf("\n\n  === ESCALA: QUANTO CRESCER, CRESCER O QUE? ===\n");
    printf("  {'Populacao':>12} {'Devices':>10} {'Edge':>8} {'Regional':>10} ";
        "{'Central':>8} {'Central %':>10}");
    printf("  {'-'*60}");
    /* TODO: iterador C manual para pop em [10_000, 100_000, 1_000_000, 10_000_000, 100_000_000] */
        r = opt.calculate(pop);
        c = r["central"]["count"];
        ...
        printf("  {pop:>12,} {r['devices']['terminals_burros']:>10,} ";
            "{r['edge_nodes']['count']:>8,} {r['regional']['count']:>10} ";
            "{c:>8} {central_pct:>9.1f}%");
    printf("\n\n{'='*80}");
    printf("  PRINCIPIOS DA INFRAESTRUTURA");
    printf("{'='*80}");
    printf(""";
...
#endif // OPENREPUBLIC_POLITICA_DE_INFRAESTRUTURA_DESCENTRALIZADA_H
