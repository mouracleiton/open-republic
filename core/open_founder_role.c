...
#ifndef OPENFOUNDERROLE_PAPEL_CONSTITUCIONAL_DO_FUNDADOR_H
#define OPENFOUNDERROLE_PAPEL_CONSTITUCIONAL_DO_FUNDADOR_H
...
// ============================================================================
typedef struct FounderPrivilege {
...
typedef struct FounderRestriction {
...
typedef struct CorrectionSpeed {
...
typedef struct FounderCorrectionSystem {
    ...
    funcao check_proposal(self, proposal_text: texto,
                    char* proposal_type = "politica") -> {texto: qualquer}:;
    ...
    int _severity(self, dev_type: DeviationType) {
        return {;
    ...
    char* _principle(self, dev_type: DeviationType) {
        return {;
    ...
    char* _correction(self, dev_type: DeviationType) {
        return {;
            DeviationType.ELITISM: (;
    ...
    {texto: qualquer} appeal(self, deviation_id: texto) {
    ...
    {texto: qualquer} stats(self) {
        return {;
            "total_checked": sizeof(self.deviations),;
...
typedef struct AssemblyVote {
    ...
    char* winner = "";
    bool approved = false;
{texto: qualquer} run_founder_role_assembly(n_voters: inteiro = 10000) {
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENFOUNDERROLE -- PAPEL CONSTITUCIONAL DO FUNDADOR");
    printf("  Assembleia Constituinte Especial");
    printf("=" * 80);
    // === 1. PERGUNTA ===
    printf("\n\n  === PERGUNTA SUBMETIDA A ASSEMBLEIA ===\n");
    printf('  "A posicao do fundador pode ser definitiva && sempre');
    printf('   auxiliada pelo sistema, com correcao automatica de desvios?"');
    printf("\n  POSICAO DO FUNDADOR:");
    printf('  "Sim, quero voz permanente + auxilio + correcao quando errar."');
    // === 2. VOTACAO ===
    printf("\n\n  === VOTACAO (10.000 CIDADAOs) ===\n");
    result = run_founder_role_assembly(10000);
    printf("  SIM (voz + correcao):    {result['votes_for']:>6} ({result['votes_for']/100:.0f}%)");
    printf("  PARCIAL (voz, sem prio): {result['votes_partial']:>6} ({result['votes_partial']/100:.0f}%)");
    printf("  NAO (cidadao comum):     {result['votes_against']:>6} ({result['votes_against']/100:.0f}%)");
    ...
    /* para cada (key, val) em d.items(): */
        ...
        printf("  {key:<25} {status}");
    printf("\n  RESTRICOES INALTERAVEIS:");
    /* TODO: iterador C manual para r em result["restrictions_inalteraveis"] */
        printf("    {r}");
    ...
    /* para cada (proposal, expected) em test_proposals: */
        ...
        if (check["status"] != "approved") {
            printf("       Tipo: {check.get('type', '?')}");
            printf("       Principio: {check.get('principle', '?')}");
            printf("       Correcao: {check['correction'][:80]}");
        } else {
            printf("       Conforme. Pode prosseguir para votacao.");
    // === 5. APELO ===
    printf("\n\n  === DIREITO DE APELO ===\n");
    /* TODO: iterador C manual para dev em corrector.deviations */
        if (! dev.resolved) {
            appeal = corrector.appeal(dev.deviation_id);
            printf("  Fundador apela correcao: {dev.deviation_type.value}");
            printf("  Resultado: {appeal['appeal_result']}");
            printf("  {appeal['votes']}");
    ...
    // === RESPOSTA FINAL ===
    printf("\n\n{'='*80}");
    printf("  RESPOSTA A PERGUNTA DO FUNDADOR");
    printf("{'='*80}");
    printf(""";
PERGUNTA:;
...
// )
    printf("{'='*80}");
    printf("  OpenFounderRole: voz permanente + auxilio + correcao automatica.");
    printf("  Fundador propoe. Sistema corrige. Povo decide.");
    printf("  {'='*80}");

#endif // OPENFOUNDERROLE_PAPEL_CONSTITUCIONAL_DO_FUNDADOR_H
