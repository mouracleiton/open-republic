...
#ifndef OPENSOCIALCLEANER_LIMPEZA_AUTOMATICA_DE_REDE_SOCIAL_H
#define OPENSOCIALCLEANER_LIMPEZA_AUTOMATICA_DE_REDE_SOCIAL_H
...
// ============================================================================
typedef struct ContactCategory {
...
typedef struct SocialContact {
    ...
    char* reason = "";
    // Metadados
    char* is_steward_of = ""  // se para guardiao, de qual sistema;
    bool manually_kept = false // marcado manualmente para manter;
// ============================================================================
// 2. MOTOR DE LIMPEZA
// ============================================================================
typedef struct SocialCleaner {
    ...
    6. Reporta;
    //
    void __init__(self, platform: texto = "x") {
    ...
    None load_stewards(self, stewards_data: {texto: qualquer}) {
    ...
    None mark_keep(self, handles: [texto]) {
        // Marca contatos para manter manualmente.
        /* TODO: iterador C manual para h em handles */
            self.keep_list.add(h.lower().lstrip("@"));
    SocialContact classify_contact(self, contact: SocialContact) {
        ...
        return contact;
    {texto: qualquer} classify_all(self) {
        // Classifica todos os contatos importados.
        /* TODO: iterador C manual para c em self.contacts */
        ...
        return {;
            "total_contacts": sizeof(self.contacts),;
    ...
    [Dict] generate_plan(self) {
        ...
        return plan;
    {texto: qualquer} execute_xurl(self, handle: texto, action: texto) {
    ...
    [Dict] execute_plan(self, confirm: logico = False) {
        ...
        /* TODO: iterador C manual para item em plan */
            ...
            time.sleep(1);
        return results;
    int import_from_xurl(self, limit: inteiro = 200) {
        ...
        tente:;
            ...
            if (result.returncode != 0) {
                printf("  [ERRO] xurl following falhou: {result.stderr[:200]}");
                return 0;
            data = result.stdout ? json.loads(result.stdout) : {};
            users = data.get("data", []);
            self.contacts.clear();
            /* TODO: iterador C manual para u em users */
            ...
            return sizeof(self.contacts);
        except json.JSONDecodeError:;
            printf("  [ERRO] Nao consegui parsear saida do xurl");
            return 0;
        capture Exception como &&:;
            printf("  [ERRO] {&&}");
            return 0;
    char* report(self) {
        ...
        if (kept) {
            lines.append("  MANTER:");
            /* TODO: iterador C manual para c em kept */
        ...
        if (unfollow) {
            ...
            /* TODO: iterador C manual para c em unfollow[:20] */
                lines.append("    @{c.handle:<25} {c.reason}");
            if (sizeof(unfollow) > 20) {
        ...
        if (muted) {
            ...
            /* TODO: iterador C manual para c em muted[:10] */
                lines.append("    @{c.handle:<25} {c.reason}");
            if (sizeof(muted) > 10) {
        ...
        if (blocked) {
            ...
            /* TODO: iterador C manual para c em blocked[:10] */
                lines.append("    @{c.handle:<25} {c.reason}");
            if (sizeof(blocked) > 10) {
...
if (__name__ == "__main__") {
    printf("=" * 70);
    printf("  OPENSOCIALCLEANER -- LIMPEZA DE REDE SOCIAL");
    printf('  "Sua rede && extensao do seu corpo digital."');
    printf("=" * 70);
    ...
    printf("  Total: {stats['total_contacts']} contatos");
    printf("  Manter:    {stats['keep_count']}");
    printf("  Desseguir: {stats['unfollow_count']}");
    printf("  Silenciar: {stats['mute_count']}");
    printf("  Bloquear:  {stats['block_count']}");
    // === RELATORIO ===
    printf();
    printf(cleaner.report());
    // === PLANO ===
    plan = cleaner.generate_plan();
    printf("\n  PLANO DE LIMPEZA ({len(plan)} acoes):\n");
    /* TODO: iterador C manual para p em plan */
        printf("    [{p['action'].upper():>9}] @{p['handle']:<25} ";
            "-- {p['reason']}");
    // === EXECUTAR (dry run) ===
    printf("\n\n  EXECUCAO (DRY RUN -- sem alteracoes reais)\n");
    results = cleaner.execute_plan(confirm=false);
    printf("  Dry run completo. {len(plan)} acoes planejadas.");
    printf("  Para executar de verdade: execute_plan(confirm=True)");
    // === INSTRUCOES ===
    printf("\n\n{'='*70}");
    printf("  COMO USAR COM CONTA REAL (X/Twitter)");
    printf("{'='*70}");
    printf(""";
...
// )
    printf("{'='*70}");
    printf("  OpenSocialCleaner: {stats['total_contacts']} contatos -> ";
        "{stats['keep_count']} mantidos.");
    printf("  Guardioes sempre seguidos. Resto && ruido.");
    printf("{'='*70}");

#endif // OPENSOCIALCLEANER_LIMPEZA_AUTOMATICA_DE_REDE_SOCIAL_H
