...
**Arquivo original:** `core/open_cryptography.py`
...
FILOSOFIA:
  1. PRIVACIDADE E DIREITO (P2 autonomia corporal inclui dados)
  2. NENHUM backdoor (governo nao tem chave. Ninguem tem.)
  3. E2E (extremo-a-extremo. So emissor e receptor leem.)
  4. ZERO conhecimento (servidor nao sabe o que armazena)
  5. FORWARD SECRECY (se chave vaza, passado nao e' lido)
...
classe CryptoPurpose herda de Enum:
...
classe CryptoAlgorithm herda de Enum:
...
classe KeyStrength herda de Enum:
...
classe CryptoKey:
    ...
    seja strength: KeyStrength = KeyStrength.STANDARD

    // Material (em producao: HSM ou secure enclave)
    seja private_key_b64: texto = ""  // NUNCA em texto plano. So referencia.
    seja public_key_b64: texto = ""
    seja derived_key_b64: texto = ""  // chave derivada (HKDF)

    // Metadata
    seja created: texto = ""
    seja expires: texto = ""  // chaves expiram (forward secrecy)
    seja rotated_from: texto = ""  // se rotacionou, de onde veio
    seja rotated_to: texto = ""

    // Protecao
    seja encrypted_at_rest: logico = verdadeiro // chave tambem e' criptografada
    seja hsm_backed: logico = falso // Hardware Security Module?
    seja user_controlled: logico = verdadeiro // so usuario tem acesso

    // Audit
    seja times_used: inteiro = 0
    seja last_used: texto = ""
...
classe CryptoOps:
    ...
    Se nao: usa SHA-256 + AES simulado (para prototipo).

    EM PRODUCAO (Rust):
    - ring ou RustCrypto crates
    ...
    funcao generate_key(purpose: CryptoPurpose,
                     seja algorithm: CryptoAlgorithm = CryptoAlgorithm.AES_256_GCM
                     ) -> CryptoKey:
        ...
        retorne CryptoKey(
            ...
            encrypted_at_rest = verdadeiro, user_controlled=verdadeiro,
        )

    // decorador: @staticmethod
    funcao encrypt(plaintext: texto, key: CryptoKey) -> {texto: texto}:
        ...
        senao:
            ...
            stream = stream * (tamanho(data) // tamanho(stream) + 1)
            ct = bytes(a ^ b para a, b in intercale(data, stream[:tamanho(data)]))
            retorne {
                "ciphertext": base64.b64encode(ct).decode(),
                "nonce": base64.b64encode(nonce).decode(),
    ...
    // decorador: @staticmethod
    funcao decrypt(encrypted: {texto: texto}, key: CryptoKey) -> texto:
        // Descriptografa.
        se HAS_CRYPTO entao:
            key_bytes = base64.b64decode(key.private_key_b64)
        ...
        senao:
            ...
            ct = base64.b64decode(encrypted["ciphertext"])
            stream = hashlib.sha256(key_bytes + nonce).digest()
            stream = stream * (tamanho(ct) // tamanho(stream) + 1)
            pt = bytes(a ^ b para a, b in intercale(ct, stream[:tamanho(ct)]))
            retorne pt.decode()

    // decorador: @staticmethod
    funcao hash(data: texto, salt: texto = "") -> texto:
        // Hash SHA3-512.
        retorne hashlib.sha3_512((data + salt).encode()).hexdigest()

    // decorador: @staticmethod
    funcao password_hash(password: texto, salt: texto = "") -> {texto: texto}:
        ...
        para cada _ em intervalo(10000):
            h = hashlib.sha3_512(h.encode()).hexdigest()
        retorne {"hash": h, "salt": salt, "algorithm": "Argon2id (simulado)"}

    // decorador: @staticmethod
    funcao verify_password(password: texto, stored: {texto: texto}) -> logico:
        // Verifica senha contra hash armazenado.
        h = hashlib.sha3_512((password + stored["salt"]).encode()).hexdigest()
        para cada _ em intervalo(10000):
            h = hashlib.sha3_512(h.encode()).hexdigest()
        retorne h == stored["hash"]

    // decorador: @staticmethod
    funcao sign(data: texto, key: CryptoKey) -> texto:
        // Assina dados (Ed25519 em producao).
        key_bytes = base64.b64decode(key.private_key_b64)
        signature = hashlib.sha3_512(data.encode() + key_bytes).hexdigest()
        retorne base64.b64encode(signature.encode()).decode()

    // decorador: @staticmethod
    funcao verify_signature(data: texto, signature: texto, key: CryptoKey) -> logico:
        // Verifica assinatura.
        key_bytes = base64.b64decode(key.private_key_b64)
        expected = base64.b64encode(
            hashlib.sha3_512(data.encode() + key_bytes).hexdigest().encode()
        ).decode()
        retorne signature == expected
...
classe CryptoEngine:
    ...
    VOTACAO (assembleia):
    - Voto e SECRETO. Criptograficamente.
    ...
    O QUE nao e CRIPTOGRAFADO (transparencia):
    - Parametros da assembleia (publicos)
    - Contagem de votos (publica, mas voto individual e secreto)
    ...
    - Se servidor e' comprometido: dados estao criptografados. Inuteis.
    - Se chave e' comprometida: forward secrecy protege passado.
    - Republica nao pode descriptografar dados do cidadao. NUNCA.
    - Se cidadao perde chave: dados PERDIDOS (trade-off de privacidade).

    POST-QUANTUM:
    - Computador quantico quebrara RSA e ECC.
    - Republica ja prepara: Kyber (KEM) e Dilithium (assinatura).
    ...
    funcao setup_citizen_keys(self, citizen_id: texto) -> {texto: qualquer}:
    ...
    funcao encrypt_message(self, sender_id: texto, recipient_id: texto,
        ...
        retorne {
            ...
            "e2e": verdadeiro,
            "server_can_read": falso,
            "republic_can_read": falso,
            "message": (
    ...
    funcao decrypt_message(self, encrypted: {texto: texto},
        ...
        retorne CryptoOps.decrypt(encrypted, key)
    ...
    funcao encrypt_health_record(self, citizen_id: texto,
        ...
        record_str = json.dumps(record, ensure_ascii=falso)
        ...
        retorne {
            ...
            "message": (
                ...
                "Servidor nao le. Governo nao le."
            ),
        }
    ...
    funcao encrypt_brain_data(self, citizen_id: texto,
        ...
        data_str = json.dumps(data, ensure_ascii=falso)
    ...
    funcao secure_vote(self, citizen_id: texto, vote: texto,
        ...
        retorne {
            ...
            "vote_revealed": falso,   // NUNCA revela voto individual
            "counted": verdadeiro,
            "anonymous": verdadeiro,
            "message": (
                "Voto de {citizen_id} em {proposal_id}: "
                "CRIPTOGRAFADO e ANONIMIZADO. "
                "Prova que votou (ZKP). "
    ...
    funcao sign_transaction(self, citizen_id: texto, amount: flutuante,
        ...
        retorne {
            ...
            "forgeable": falso,
            "verified": verdadeiro,
            "transparent": verdadeiro,   // todos veem QUEM transacionou
            "private": verdadeiro,       // ninguem ve O QUE comprou
            "message": (
    ...
    funcao anti_backdoor_policy(self) retorna List[{texto: texto}]:
        ...
        retorne [
            ...
            {"regra": "2. NENHUM acesso governamental",
             "detalhe": "Governo, polícia, agencia: NENHUM acesso. Criptografado e inutil para eles."},
            {"regra": "3. ZERO conhecimento do servidor",
             "detalhe": "Servidor armazena ciphertext. Nao sabe o que e'. Nao pode ler."},
            ...
            {"regra": "7. Open-source auditavel",
             "detalhe": "Todo codigo cripto e CC0. Auditavel. Sem implementacao secreta."},
            {"regra": "8. Nao logs de metadados",
             "detalhe": "Servidor nao registra quem falou com quem, quando, quanto."},
        ]
    ...
    funcao post_quantum_readiness(self) -> {texto: texto}:
        // Preparacao pos-quantica.
        retorne {
            "ameaca": (
                "Computador quantico quebrara RSA-2048 e ECC-256. "
            ...
            "harvest_now_decrypt_later": (
                "Atacante pode gravar ciphertext hoje e quebrar amanha. "
    ...
    funcao key_rotation_policy(self) -> {texto: qualquer}:
        ...
        retorne {
            ...
            "automatica": verdadeiro,
            "transparente": verdadeiro,
            "sem_downtime": verdadeiro,
        }

    funcao stats(self) -> {texto: qualquer}:
        retorne {
            "biblioteca": self.crypto_lib,
            "algoritmos_suportados": tamanho(CryptoAlgorithm),
            "propositos": tamanho(CryptoPurpose),
            "chaves_ativas": tamanho(self.keys),
            "operacoes": tamanho(self.operations_log),
            "backdoor": "ZERO (nenhuma)",
            "e2e": verdadeiro,
            "forward_secrecy": verdadeiro,
            "post_quantum": "preparando (Kyber/Dilithium)",
            "principio": "Privacidade e direito. Sem backdoor. Nunca.",
        }
...
se __name__ == "__main__" entao:
    ...
    imprima("=" * 80)
    imprima("  OPENCRYPTOGRAPHY -- CRIPTOGRAFIA DA REPUBLICA")
    imprima("  Tudo criptografado. E2E. Sem backdoor. Nunca.")
    imprima("=" * 80)
    ...
    para cada alg em CryptoAlgorithm:
        imprima("  {alg.value[0]:<25} tipo: {alg.value[1]:<20} {alg.value[2]} bits")

    // === 2. O QUE E CRIPTOGRAFADO ===
    imprima("\n\n  === 2. O QUE E CRIPTOGRAFADO ===\n")
    para cada item em engine.what_is_encrypted():
        imprima("  {item['dado']:<40} [{item['nivel']}]")
        imprima("    Quem le: {item['quem_le']}")

    // === 3. O QUE E PUBLICO ===
    imprima("\n\n  === 3. O QUE E PUBLICO (transparencia) ===\n")
    para cada item em engine.what_is_NOT_encrypted():
        imprima("  {item['dado']:<40} {item['motivo']}")
    ...
    imprima("  Chave gerada: {key.key_id}")
    imprima("  Algoritmo: {key.algorithm.value[0]}")

    // Criptografar
    mensagem = "Isso e mensagem secreta da Republica. Ninguem le exceno nos."
    encrypted = CryptoOps.encrypt(mensagem, key)
    imprima("\n  Original: {mensagem}")
    imprima("  Ciphertext: {encrypted['ciphertext'][:50]}...")
    imprima("  Nonce: {encrypted['nonce'][:30]}...")
    imprima("  Tag: {encrypted['tag'][:30]}...")

    // Descriptografar
    decrypted = CryptoOps.decrypt(encrypted, key)
    imprima("\n  Descriptografado: {decrypted}")
    imprima("  Confere: {'SIM' if decrypted == mensagem else 'NAO'}")

    // === 5. CIDADAO: CHAVES ===
    imprima("\n\n  === 5. CHAVES DE UM CIDADAO ===\n")
    setup = engine.setup_citizen_keys("cleiton")
    imprima("  {setup['message']}")
    para cada p em setup["purposes"]:
        imprima("    + {p}")
    ...
    para cada rule em engine.anti_backdoor_policy():
        imprima("  {rule['regra']}")
        imprima("    {rule['detalhe']}")
    ...
    imprima("\n{'='*80}")
    imprima("  OpenCryptography: {s['algoritmos_suportados']} algoritmos. "
