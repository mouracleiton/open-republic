...
class DebtParameters:
    ...
    @property
    def gdp_brl(self) -> float:
        return self.initial_gdp_brl

    def real_interest_rate(self) -> float:
        """Juros real = nominal - inflacao."""
        return self.annual_interest_rate - self.annual_inflation

    def growth_gap(self) -> float:
        """Diferenca entre juros e crescimento. Se positiva = INSUSTENTAVEL."""
        return self.annual_interest_rate - self.annual_gdp_growth
...
class YearProjection:
    ...
    point_of_no_return: bool = False         # passou do ponto de nao retorno?
...
class DebtProjectionEngine:
    ...
    def __init__(self, params: DebtParameters):
        self.params = params
    ...
    def project(self) -> List[YearProjection]:
        ...
        point_of_no_return_found = False

        for i in range(self.params.years_to_project + 1):
            ...
            ponr = interest_pct_revenue > 50 and not point_of_no_return_found
            if ponr:
    ...
    def find_point_of_no_return(self) -> Optional[YearProjection]:
        """Encontra o ano em que os juros superam 50% da receita."""
        for p in self.projections:
            if p.point_of_no_return:
                return p
        return None

    def total_interest_paid(self) -> float:
        """Total de juros pagos em todos os anos projetados."""
        return sum(p.interest_paid_brl for p in self.projections)

    def final_debt(self) -> float:
        return self.projections[-1].debt_brl if self.projections else 0

    def debt_multiplier(self) -> float:
        """Quantas vezes a divida cresceu."""
        if not self.projections:
            return 1.0
        return self.projections[-1].debt_brl / self.params.initial_debt_brl
    ...
    def proof_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "point_of_no_return_year": ponr.year_label if ponr else None,
...
class ASCIIBarChart:
    ...
    @staticmethod
    def render(projections: List[YearProjection], metric: str = "debt_to_gdp") -> str:
        """Renderiza grafico de barras ASCII."""
        labels_map = {
            "debt_to_gdp": ("Divida/PIB (%)", lambda p: p.debt_to_gdp),
            "interest_pct_gdp": ("Juros/PIB (%)", lambda p: p.interest_as_pct_gdp),
            "interest_pct_revenue": ("Juros/Receita (%)", lambda p: p.interest_as_pct_revenue),
            "per_capita_debt": ("Divida per capita (R$ mil)", lambda p: p.per_capita_debt_brl / 1e3),
        }

        title, getter = labels_map.get(metric, labels_map["debt_to_gdp"])
...
class MarkdownTable:
    ...
    @staticmethod
    def render(projections: List[YearProjection]) -> str:
        lines = []
...
class HTMLPage:
    ...
    @staticmethod
    def render(projections: List[YearProjection], proof: Dict[str, Any]) -> str:
...
"""
        ...
        for i, year in enumerate(years):
            ...
            html += f'        <div class="bar-container"><span class="bar-year">{year}</span>'
            html += f'<div class="bar" style="width:{debt_pct:.1f}%"></div>'
            html += f'<span class="bar-label">R$ {debts[i]:.1f}T</span></div>\n'
            html += f'        <div class="bar-container"><span class="bar-year"></span>'
            html += f'<div class="bar gdp" style="width:{gdp_pct:.1f}%"></div>'
            html += f'<span class="bar-label">PIB R$ {gdps[i]:.1f}T</span></div>\n'
...
"""
        ...
        for i, year in enumerate(years):
            ...
            html += f'        <div class="bar-container"><span class="bar-year">{year}</span>'
            html += f'<div class="bar interest" style="width:{int_pct:.1f}%"></div>'
            html += f'<span class="bar-label{ponr_class}">R$ {interests[i]:.1f}T'
...
"""
        ...
        for i in range(0, n, max(1, n // 10)):
            svg += f'    <text x="{to_x(i):.0f}" y="{height-margin+20}" text-anchor="middle" fill="#888" font-size="11" font-family="monospace">{years[i]}</text>\n'

        # Labels Y
        for j in range(5):
            val = max_val * j / 4
            y = height - margin - (val / max_val) * chart_h
            svg += f'    <text x="{margin-10}" y="{y+4:.0f}" text-anchor="end" fill="#888" font-size="10" font-family="monospace">{val:.0f}T</text>\n'
            svg += f'    <line x1="{margin}" y1="{y:.0f}" x2="{width-margin}" y2="{y:.0f}" stroke="#222" stroke-width="0.5" stroke-dasharray="3,3"/>\n'
