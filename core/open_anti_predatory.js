...
class PredationCase {
    ...
    const predator = "";
    const victim = "";
    const predation_type = PredationType.MONOPOLY;
    const severity = PredationSeverity.WARNING;
    const description = "";
    const evidence = field(default_factory=list);
    const action = AntiPredationAction.WARNING;
    const status = "detectado";
    const date = "";
// ============================================================================
class AntiPredationEngine {
    ...
    _assess_severity(self, ptype: PredationType) {
        severity_map = {
    ...
    _decide_action(self, severity: PredationSeverity) {
        action_map = {
    ...
    stats(self) {
        return {;
            "total_casos": .length(self.cases),;
            "bloqueados": self.blocked_count,;
            "corrigidos": self.corrected_count,;
            "tipos_detectados": .length(set(c.predation_type para c em self.cases.values())),;
        };
// ============================================================================
if (__name__ == "__main__") {
    engine = AntiPredationEngine();
    console.log("=" * 80);
    console.log("  OPENANTIPREDATORY -- POLITICA ANTI COMPETICAO PREDATORIA");
    console.log("  Competir melhorando = OK. Competir destruindo = BLOQUEADO.");
    console.log("=" * 80);
    // === 1. TIPOS DE PREDACAO ===
    console.log("\n\n  === 1. TIPOS DE COMPETICAO PREDATORIA ({len(PredationType)}) ===\n");
    for (const pt of PredationType) {
        sev = engine._assess_severity(pt);
        console.log("  [{sev.value[0]:<12}] {pt.value}");
    // === 2. CASOS DE DETECCAO ===
    console.log("\n\n  === 2. CASOS DETECTADOS ===\n");
    cases = [;
    ...
    para predator, ptype, desc, evidence in cases: {
        ...
        console.log("  Acao: {r['action']}");
        console.log("  {r['message'][:70]}");
    // === 3. CHECKS AUTOMATICOS ===
    console.log("\n\n  === 3. CHECKS AUTOMATICOS ===\n");
    // Market dominance
    r = engine.check_market_dominance("TechGiant", 0.45);
    console.log("  Market dominance check: {r.get('status', r.get('action', '?'))}");
    // Resource hoarding
    r = engine.check_resource_hoarding("Acumulador", 0.35);
    console.log("  Resource hoarding check: {r.get('status', r.get('action', '?'))}");
    // Walled garden
    r = engine.check_walled_garden("RedeFechada", false, false);
    console.log("  Walled garden check: {r.get('status', r.get('action', '?'))}");
    // Planned obsolescence
    r = engine.check_planned_obsolescence("FoneDescartavel", 1.5, false);
    console.log("  Obsolescence check: {r.get('status', r.get('action', '?'))}");
    // === 4. STATS ===
    console.log("\n\n  === 4. ESTATISTICAS ===\n");
    s = engine.stats();
    para cada (k, v) em s.items(): {
        console.log("  {k:<25} {v}");
    console.log("\n{'='*80}");
    console.log("  AntiPredatory: {s['bloqueados']} bloqueados. Competir destruindo = PROIBIDO.");
    console.log("{'='*80}");
