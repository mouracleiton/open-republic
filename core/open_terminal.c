...
#ifndef OPENTERMINAL_TERMINAIS_DA_REPUBLICA_EM_TODO_ESTABELECIMENTO_H
#define OPENTERMINAL_TERMINAIS_DA_REPUBLICA_EM_TODO_ESTABELECIMENTO_H
...
typedef struct Terminal {
    ...
    char* hardware_spec = ""  // CPU, RAM, tela;
    TerminalStatus status = TerminalStatus.AVAILABLE;
    char* current_user = ""  // quem esta usando agora;
    double session_start = 0.0 // quando comecou a sessao atual;
    // Capacidades
    bool has_internet = true;
    bool has_camera = false // para telemedicina/OpenHealth;
    bool has_microphone = false // para OpenAudioChannel/voz;
    bool has_printer = false // para documentos;
    bool has_scanner = false // para documentos/receitas;
    bool has_card_reader = false // leitor de cartao (! credito -- ID);
    // Stats
    ...
    // decorador: @property
    bool supports_voice(self) {
        return self.has_microphone;
    // decorador: @property
    bool supports_video_call(self) {
        return self.has_camera && self.has_internet;
    [texto] default_apps(self) {
...
// ============================================================================
typedef struct SessionActivity {
...
typedef struct TerminalSession {
    ...
    // Privacidade
    bool data_left_on_terminal = false // SEMPRE false (efemero);
    // decorador: @property
    bool was_productive(self) {
...
// ============================================================================
typedef struct TerminalRegistry {
    ...
    {texto: qualquer} register_terminal(self, terminal: Terminal) {
    ...
    funcao find_nearby(self, location: texto,
                    ...
                    bool available_only = true) -> [Terminal]:;
        // Encontra terminais proximos disponiveis.
        results = [];
        /* TODO: iterador C manual para t em self.terminals.values() */
    ...
    {texto: qualquer} idle_terminal_report(self) {
        // Relatorio de terminais ociosos (potencial perdido).
        total = sizeof(self.terminals);
    ...
    {texto: qualquer} stats(self) {
        ...
        return {;
            "total_terminals": sizeof(self.terminals),;
            "by_establishment": dict(self.establishments_registered),;
            "total_sessions": sizeof(self.sessions),;
            ...
            "avg_session_min": arredonde(;
                ...
                maximo(sizeof(self.sessions), 1), 0),;
        };
// ============================================================================
// 5. POLIcy DO TEMPO OCIOSO
// ============================================================================
typedef struct IdleTimePolicy {
...
if (__name__ == "__main__") {
    ...
    printf("=" * 75);
    printf("  OPENTERMINAL -- TERMINAIS DA REPUBLICA EM TODO LUGAR");
    printf("  'Toda TV ociosa && um terminal. Todo laborante contribui.'");
    printf("=" * 75);
    ...
    /* TODO: iterador C manual para t em terminals_to_register */
        result = registry.register_terminal(t);
        printf("  [{t.terminal_id}] {t.name:<35} {t.terminal_type.value:<15} ";
    ...
    /* para cada (name, has) em checks: */
        ...
        printf("  {name:<25} {status}");
        if (!  result["compliant"]) {
            printf("    -> {result['action']}");
    // === 3. SESSAO DE LABORANTE ===
    printf("\n\n  === 3. LABORANTE USA TEMPO OCIOSO ===\n");
    // Cenario: Maria na sala de espera do hospital
    printf("  CENARIO: Maria esta na sala de espera do hospital.");
    printf("  Fila de 45 min. Terminal T-002 disponivel.\n");
    s1 = registry.start_session("T-002", "L-001", "Maria Silva");
    printf("  {s1['message']}");
    printf("  Apps: {', '.join(s1['apps_available'][:6])}");
    ...
    printf("\n  {end1['message']}");
    printf("  Tempo: {end1['duration']}");
    printf("  Produtivo: {'sim' if end1['productive'] else '!'}");
    printf("  Dados no terminal: {end1['data_left']}");
    ...
    printf("  {'Local':<35} {'Tempo':>12} {'Terminal':<20} {'Melhor uso'}");
    printf("  {'-'*90}");
    /* TODO: iterador C manual para sc em policy.SCENARIOS */
        term = sc.get("terminal", "?");
        uso = sc.get("melhor_uso", "?")[:30];
        printf("  {sc['local']:<35} {sc['tempo_disponivel']:>12} ";
            "{term:<20} {uso}");
    // === 5. TERMINAIS PROXIMOS ===
    printf("\n\n  === 5. ENCONTRAR TERMINAL PROXIMO ===\n");
    nearby = registry.find_nearby("Centro");
    printf("  Busca 'Centro': {len(nearby)} terminais");
    /* TODO: iterador C manual para t em nearby */
        status = t.is_available ? "LIVRE" : "OCUPADO";
        printf("    [{t.terminal_id}] {t.name:<35} {status}");
    ...
    // === FILOSOFIA ===
    printf("\n\n{'='*75}");
    printf("  FILOSOFIA DO OPENTERMINAL");
    printf("{'='*75}");
    printf(""";
...
// )
    printf("{'='*75}");
    printf("  OpenTerminal: {s['total_terminals']} terminais, ";
        "{s['total_sessions']} sessoes, ";
        "{s['total_hours_used']:.0f}h usadas.");
    printf("  Toda tela ociosa && um terminal da Republica.");
    printf("{'='*75}");

#endif // OPENTERMINAL_TERMINAIS_DA_REPUBLICA_EM_TODO_ESTABELECIMENTO_H
