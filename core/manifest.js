...
count_asset(asset: RepublicAsset, base_path: Path) {
    ...
    total_lines = soma(.length(f.read_text(errors="ignore").splitlines());
                    para f em code_files) {
    langs = ordene(set(f.suffix para f em code_files));
    asset.files = .length(code_files);
    asset.lines = total_lines;
    asset.languages = [l.lstrip(".") para l em langs];
    return {"files": .length(code_files), "lines": total_lines, "exists": true};
full_inventory(base_path: Path = None) {
    ...
    total_files = 0;
    total_lines = 0;
    for (const asset of REPUBLIC_REGISTRY) {
    ...
    return {;
        "total_assets": .length(REPUBLIC_REGISTRY),;
        "total_files": total_files,;
        "total_lines": total_lines,;
        "domains": .length(by_domain),;
        "by_domain": by_domain,;
    };
if (__name__ == "__main__") {
    console.log("=" * 75);
    console.log("  OPENREPUBLIC -- REGISTRO CENTRAL DE BENS COMUNS");
    console.log("  'Tudo que existe na Republica && bem publico && cadastrado.'");
    console.log("=" * 75);
    inv = full_inventory();
    console.log("\n  TOTAL: {inv['total_assets']} sistemas | ";
        "{inv['total_files']} arquivos | ";
        "{inv['total_lines']:,} linhas\n");
    console.log("  {'Dominio':<16} {'Sistemas':>8} {'Arquivos':>8} {'Linhas':>8}");
    console.log("  {'-'*44}");
    para cada (domain, data) em ordene(inv["by_domain"].items()): {
        console.log("  {domain:<16} {len(data['assets']):>8} ";
            "{data['total_files']:>8} {data['total_lines']:>8,}");
    console.log("\n  {'SISTEMA':<28} {'DOMINIO':<14} {'LINHAS':>7}");
    console.log("  {'-'*52}");
    para cada (domain, data) em ordene(inv["by_domain"].items()): {
        for (const asset of ordene(data["assets"], key=(x) -> -x["lines"])) {
            marker = asset["lines"] == 0 ? " <== VAZIO" : "";
            console.log("  {asset['name']:<28} {domain:<14} ";
                "{asset['lines']:>7,}{marker}");
    console.log("\n{'='*75}");
    console.log("  8 dominios. {inv['total_assets']} sistemas integrados.");
    console.log("  {inv['total_lines']:,} linhas de codigo aberto.");
    console.log("  Tudo bem comum. Tudo da Republica.");
    console.log("{'='*75}");
