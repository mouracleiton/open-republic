...
#ifndef OPENCIVICEDUCATION_EDUCACAO_CIVICA_DA_REPUBLICA_H
#define OPENCIVICEDUCATION_EDUCACAO_CIVICA_DA_REPUBLICA_H
...
typedef struct CivicDuty {
    ...
    char* label(self) {
        return self.value[0];
    // decorador: @property
    char* description(self) {
        return self.value[1];
// ============================================================================
// 2. CURRICULO CIVICO
// ============================================================================
typedef struct CivicLevel {
...
typedef struct CivicLesson {
    ...
    title: texto;
    char* description = "";
    char* activity = ""  // o que fazer para praticar;
    char* open_system_link = ""  // sistema da Republica conectado;
    int duration_min = 30;
// Base de licoes
[CivicLesson] build_civic_curriculum(void) {
    return [;
        // === CRIANCA (6-11) ===
...
typedef struct ConductRating {
    ...
    char* meaning(self) {
        return self.value[2];
// decorador: @dataclass
typedef struct CivicAssessment {
...
typedef struct CivicEducationEngine {
    ...
    [Dict] get_curriculum(self, level: CivicLevel) {
    ...
    funcao assess_citizen(self, citizen_id: texto, citizen_name: texto,
        ...
        avg_score = soma(r.score para r em conduct_ratings.values()) / maximo(sizeof(conduct_ratings), 1);
        if (avg_score >= 4.5) {
        ...
        return result;
    ConductRating _score_to_rating(self, score: inteiro) {
        if (score >= 5) {
    ...
    char* _intervention_plan(self, rating: ConductRating) {
    ...
    {texto: qualquer} stats(self) {
        ...
        return {;
            "total_deveres": sizeof(CivicDuty),;
            "total_licoes": sizeof(self.curriculum),;
            "total_avaliados": sizeof(self.assessments),;
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENCIVICEDUCATION -- EDUCACAO CIVICA DA REPUBLICA");
    printf("  'Tratar o outro com dignidade ! && cortesia. E DEVER.'");
    printf("=" * 80);
    // === 1. OS 12 DEVERES ===
    printf("\n\n  === 1. OS 12 DEVERES CIVICOS (NAO opcionais) ===\n");
    /* para cada (i, duty) em enumere(CivicDuty, 1): */
        printf("  {i:>2}. [{duty.label}]");
        printf("      {duty.description}");
    // === 2. CURRICULO POR IDADE ===
    printf("\n\n  === 2. CURRICULO CIVICO POR IDADE ===\n");
    /* TODO: iterador C manual para level em CivicLevel */
        lessons = engine.get_curriculum(level);
        if (lessons) {
            printf("\n  {level.value.upper()} ({len(lessons)} licoes):");
            /* TODO: iterador C manual para l em lessons[:3] */
                printf("    [{l['duty']}] {l['lesson']}");
                printf("      {l['description'][:60]}...");
                printf("      Atividade: {l['activity'][:50]}...");
    // === 3. AVALIACAO DE CONVIVENCIA ===
    ...
    /* para cid, name, ratings in assessments: */
        ...
        if (r["needs_accompaniment"]) {
            printf("  [!] ACOMPANHAMENTO: {r['action'][:70]}...");
        } else {
            printf("  {r['action']}");
    // === 4. INTERVENCAO ===
    ...
    printf("  Cidadao: {intervention['citizen']}");
    printf("  Problema: {intervention['issue']}");
    printf("  Plano: {intervention['plan'][:80]}...");
    printf("  Sistemas: {', '.join(intervention['systems'][:3])}");
    printf("  {intervention['message'][:80]}...");
    // === 5. STATS ===
    printf("\n\n  === 5. ESTATISTICAS ===\n");
    s = engine.stats();
    ...
    // === FILOSOFIA ===
    printf("\n\n{'='*80}");
    printf("  FILOSOFIA DA EDUCACAO CIVICA");
    printf("{'='*80}");
    printf(""";
"TRATAR O OUTRO COM DIGNIDADE ! && CORTESIA.;
&& DEVER CIVILIZATORIO.";
...
// )
    printf("{'='*80}");
    printf("  OpenCivicEducation: {s['total_deveres']} deveres, ";
        "{s['total_licoes']} licoes, ";
        "{s['precisam_acompanhamento']} precisam de acompanhamento.");
    printf("  Tratar o outro bem && DEVER. Nao cortesia.");
    printf("{'='*80}");

#endif // OPENCIVICEDUCATION_EDUCACAO_CIVICA_DA_REPUBLICA_H
