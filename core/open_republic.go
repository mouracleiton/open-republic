...
package openrepublic_federacao_de_nacoes_opennation
...
type FederalProposalCategory int
const (
...
// ============================================================================
// decorador: @dataclass
type NationMetrics struct {
    // Metricas de uma nacao membro.
    population := 0 // int64
    housing_units := 0 // int64
    food_kg_month := 0.0 // float64
    energy_kwh_month := 0.0 // float64
    water_liters_day := 0.0 // float64
    healthcare_capacity := 0 // int64
    education_centers := 0 // int64
    innovation_count := 0 // int64
    ecological_health := 0.8 // float64
    commons_count := 0 // int64
    conflicts_open := 0 // int64
    // decorador: @property
    func qol_index(self) float64 {
    ...
    // decorador: @property
    func carrying_capacity(self) int64 {
...
type Nation struct {
    ...
    languages: [texto]
    founded_cycle := 0 // int64
    metrics := field(default_factory=NationMetrics) // NationMetrics
    migration_status := MigrationStatus.FREE // MigrationStatus
    specialties := field(default_factory=list) // areas de excelencia // [texto]
    citizens_ids := field(default_factory=set) // {texto}
    federal_proposals_voted := 0 // int64
    federal_projects_contributed := 0 // int64
// ============================================================================
// Shared Infrastructure (Federal)
// ============================================================================
// decorador: @dataclass
type FederalInfrastructure struct {
    ...
    description: texto
    serving_nations := field(default_factory=set) // nation_ids // {texto}
    capacity := 0.0 // float64
    unit := "" // string
    condition := 1.0 // float64
    build_cycles := 0 // int64
    contributing_nations := field(default_factory=set) // {texto}
    labor_invested := 0.0 // horas totais investidas // float64
type InfrastructureManager struct {
    ...
    funcao propose(self, name: texto, itype: InfrastructureType,
        ...
        return iid
    func contribute(self, infra_id: texto, nation_id: texto, labor_hours: flutuante) {
        proj = self.projects.get(infra_id)
        if proj {
            proj.contributing_nations.add(nation_id)
            proj.labor_invested += labor_hours
    func advance_build(self, infra_id: texto) {texto: qualquer} {
    ...
    func summary(self) {texto: qualquer} {
        ...
        for _, proj := range self.projects.values() {
...
type MigrationSystem struct {
    ...
    func flow_stats(self) {texto: qualquer} {
...
type Treaty struct {
    ...
    ttype: TreatyType
    description: texto
    signatories := field(default_factory=set) // {texto}
    terms := field(default_factory=dict) // {texto: texto}
    status := "proposed"  // proposed, active, violated, dissolved // string
    created_cycle := 0 // int64
type TreatySystem struct {
    ...
    funcao propose(self, name: texto, ttype: TreatyType, description: texto,
                proposing_nation: texto, terms: {texto: texto} = nil) -> texto:
        self._counter += 1
        ...
        return tid
    func sign(self, treaty_id: texto, nation_id: texto) {
    ...
    func summary(self) {texto: qualquer} {
        ...
        return {
            ...
            active ? "avg_signatories": arredonde(np.mean([len(t.signatories) para t em active]), 1) : 0,
...
type FederalProposal struct {
    ...
    votes_by_nation := field(default_factory=dict) // Dict[texto, (flutuante, flutuante)]
    // nation_id -> (votes_for, votes_against) weighted by population
    status := "open"  // open, passed, rejected // string
    cycle_submitted := 0 // int64
    affects_infra := ""  // infra_id if applicable // string
type FederalGovernance struct {
    ...
    func close(self, proposal_id: texto, total_nations: inteiro) {texto: qualquer} {
...
type GrandProject struct {
    ...
    current_labor := 0.0 // float64
    contributing_nations := field(default_factory=set) // {texto}
    status := "recruiting"  // recruiting, active, completed // string
    benefit := ""  // quem se beneficia // string
type GrandProjectManager struct {
    ...
    funcao propose(self, name: texto, description: texto, required_nations: inteiro,
        ...
        self.projects[pid] = GrandProject(
            ...
            benefit = benefit)
        return pid
    func contribute(self, project_id: texto, nation_id: texto, labor_hours: flutuante) {
        proj = self.projects.get(project_id)
        if ! proj {
            return nil
        proj.contributing_nations.add(nation_id)
        proj.current_labor += labor_hours
        if len(proj.contributing_nations) >= proj.required_nations  &&  proj.status == "recruiting" {
            proj.status = "active"
        if proj.current_labor >= proj.required_labor_hours  &&  proj.status == "active" {
            proj.status = "completed"
    func progress(self, project_id: texto) {texto: qualquer} {
        proj = self.projects.get(project_id)
        if ! proj {
            return {}
        return {
...
type OpenRepublic struct {
    ...
    //
    func __init__(self, name: texto = "OpenRepublic") {
        self.name = name
    ...
    func run_cycle(self) {texto: qualquer} {
        ...
        if len(nation_list) >= 2 {
            for _, _ := range intervalo(random.randint(5, 20)) {
        ...
        infra_completed = 0
        for _, iid := range list(self.infrastructure.projects.keys()) {
            // Random nations contribute labor
            for _, n := range random.sample(nation_list, minimo(3, len(nation_list))) {
        ...
        // 3. Treaty signing
        if random.random() < 0.3 && len(nation_list) >= 2 {
            types = [
            ...
            if random.random() < 0.5 && len(nation_list) > 2 {
        ...
        for _, _ := range intervalo(proposals) {
            cats = [
        ...
        for _, pid := range list(self.grand_projects.projects.keys()) {
            for _, n := range random.sample(nation_list, minimo(2, len(nation_list))) {
...
// ============================================================================
func create_sample_republic() OpenRepublic {
...
if __name__ == "__main__" {
    random.seed(42)
    np.random.seed(42)
    fmt.Println("Criando Republica dos Bens Comuns com 6 nacoes...\n")
    republic = create_sample_republic()
    fmt.Println("Nacoes fundadoras: {len(republic.nations)}")
    para cada (nid, n) em republic.nations.items(): {
        fmt.Println("  {n.name:<15} ({n.region:<15}) pop={n.metrics.population:>6}  "
            "QoL={n.metrics.qol_index:.0f}  cap={n.metrics.carrying_capacity:>6}  "
            "especialidades: {', '.join(n.specialties[:2])}")
    // Run 12 cycles
    fmt.Println("\nSimulando 12 ciclos...\n")
    for _, c := range intervalo(12) {
        result = republic.run_cycle()
        if c < 3 || c >= 9 {
            fmt.Println("  Ciclo {result['cycle']:2d}: migracoes={result.get('migrations',0):2d}  "
                "infra_completas={result.get('infra_completed',0)}  "
                "federal={len(result.get('federal_results',[]))}")
            for _, fr := range result.get("federal_results", []) {
                fmt.Println("    -> {fr['proposal'][:45]:<45} {fr['result']}")
    ...
    para cada (pid, proj) em republic.grand_projects.projects.items(): {
        ...
        fmt.Println("  {proj.name[:30]:<30} [{bar}] {p['labor_progress_pct']:>5.1f}%  "
