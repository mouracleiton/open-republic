...
class CodeChange {
    ...
    // O que mudou
    const scope = ChangeScope.MODULE;
    const change_type = ChangeType.NEW_FUNCTION;
    const novelty = NoveltyLevel.EXISTING_EXTEND;
    ...
    const accesses_citizen_data = false;
    const accesses_health_data = false;
    const accesses_credit_data = false;
    const accesses_location_data = false;
    const accesses_communication = false;
    const accesses_children_data = false;
    // Permissoes
    const grants_new_permission = false;
    const modifies_governance = false;
    const modifies_voting = false;
    const modifies_credit_system = false;
    // Dependencias
    const new_imports = field(default_factory=list);
    const new_network_calls = field(default_factory=list);
...
class NoveltyClassifier {
    ...
    classify(self, change: CodeChange) {
    ...
    _classify_macro(self, c: CodeChange) {
    ...
    _classify_micro(self, c: CodeChange) {
    ...
    _check_exploit(self, c: CodeChange) {
        ...
        for (const imp of c.new_imports) {
    ...
    _macro_desc(level: NoveltyLevel) {
        descs = {
    ...
    // decorador: @staticmethod
    _deploy_decision(review: Dict, exploits: [texto]) {
        if (exploits) {
...
class Reviewer {
    ...
    name: texto;
    teams: [ReviewTeam];
    const expertise = field(default_factory=list);
    const reviews_done = 0;
    const exploits_caught = 0;
    const reputation = 50.0 // 0-100;
    const active = true;
    // Anti-self-exploit: NAO pode revisar proprio codigo
    // Anti-colusion: NAO pode revisar codigo de familiar/parceiro direto
    const conflicts_of_interest = field(default_factory=set);
class DeployPipeline {
    ...
    funcao _find_reviewers(self, change: CodeChange,
        ...
        eligible = [];
        for (const r of self.reviewers.values()) {
            if (! r.active) {
                continue;
...
if (__name__ == "__main__") {
    console.log("=" * 80);
    console.log("  OPENREPUBLIC -- NOVIDADE, RISCO E DEPLOY");
    console.log("  'Nem toda mudanca && igual.'");
    console.log("=" * 80);
    ...
    para cada (label, change) em changes: {
        ...
        console.log("\n  {'='*70}");
        console.log("  {label}");
        console.log("  Autor: {change.author_name} | Projeto: {change.target_project}");
        console.log("  {'='*70}");
        console.log("  Novidade MACRO: {cls['macro_classification']}");
        console.log("    -> {cls['macro_description']}");
        console.log("  Flags MICRO:");
        for (const flag of cls["micro_flags"]) {
            console.log("    -> {flag}");
        console.log("  Risco: {cls['risk_level']}");
        if (cls["exploit_patterns"]) {
            console.log("  EXPLOITS DETECTADOS:");
            for (const ep of cls["exploit_patterns"]) {
                console.log("    !!! {ep}");
        console.log("  Revisao: {cls['review_required']}");
        console.log("  Revisores necessarios: {cls['reviewers_needed']}");
        console.log("  Requer votacao: {'SIM' if cls['requires_vote'] else '!'}");
        console.log("  Requer seguranca: {'SIM' if cls['requires_security_team'] else '!'}");
        console.log("  Auto-deploy: {'SIM' if cls['can_auto_deploy'] else 'NAO'}");
        console.log("  Revisores elegiveis: {result['eligible_reviewers']}");
        console.log("\n  DECISAO: {cls['deploy_decision']}");
    // === Resumo ===
    console.log("\n\n  {'='*70}");
    console.log("  RESUMO DE NOVIDADE E REVISAO");
    console.log("  {'='*70}\n");
    console.log("  {'Mudanca':<45} {'Novidade':<20} {'Risco':<12} {'Revisores'}");
    console.log("  {'-'*85}");
    para cada (label, change) em changes: {
        r = pipeline.classifier.classify(change);
        console.log("  {label[:44]:<45} {r['macro_classification']:<20} ";
            "{r['risk_level']:<12} {r['reviewers_needed']}");
    console.log("\n\n{'='*80}");
    console.log("  PRINCIPIOS DO DEPLOY NA REPUBLICA");
    console.log("{'='*80}");
    console.log(""";
DEFINICAO DE NOVIDADE:;
    LARGO (macro):;
