...
public class OpenrepublicClassificacaoDeNovidadeEDeployPipeline {
    ...
    public static class NoveltyLevel {
    ...
    // ============================================================================
    public static class ExploitPattern {
    ...
    public static class CodeChange {
        ...
        // O que mudou
        ChangeScope scope = ChangeScope.MODULE;
        ChangeType change_type = ChangeType.NEW_FUNCTION;
        NoveltyLevel novelty = NoveltyLevel.EXISTING_EXTEND;
        ...
        boolean accesses_citizen_data = false;
        boolean accesses_health_data = false;
        boolean accesses_credit_data = false;
        boolean accesses_location_data = false;
        boolean accesses_communication = false;
        boolean accesses_children_data = false;
        // Permissoes
        boolean grants_new_permission = false;
        boolean modifies_governance = false;
        boolean modifies_voting = false;
        boolean modifies_credit_system = false;
        // Dependencias
        [texto] new_imports = field(default_factory=list);
        [texto] new_network_calls = field(default_factory=list);
        // Hash do codigo para auditoria
        String code_hash = "";
    public static class NoveltyClassifier {
        ...
        public {texto: qualquer} classify(self, change: CodeChange) {
        ...
        public NoveltyLevel _classify_macro(self, c: CodeChange) {
        ...
        public [texto] _classify_micro(self, c: CodeChange) {
        ...
        public [texto] _check_exploit(self, c: CodeChange) {
            ...
            /* TODO: for-each Java para imp em c.new_imports */
        ...
        public String _macro_desc(level: NoveltyLevel) {
            descs = {
        ...
        // decorador: @staticmethod
        public String _deploy_decision(review: Dict, exploits: [texto]) {
            if (exploits) {
    ...
    // ============================================================================
    public static class ReviewTeam {
    ...
    public static class Reviewer {
        ...
        name: texto;
        teams: [ReviewTeam];
        [texto] expertise = field(default_factory=list);
        int reviews_done = 0;
        int exploits_caught = 0;
        double reputation = 50.0 // 0-100;
        boolean active = true;
        // Anti-self-exploit: NAO pode revisar proprio codigo
        // Anti-colusion: NAO pode revisar codigo de familiar/parceiro direto
        {texto} conflicts_of_interest = field(default_factory=set);
    public static class DeployPipeline {
        ...
        public void _init_reviewers(self) {
        ...
        public {texto: qualquer} submit(self, change: CodeChange) {
        ...
        funcao _find_reviewers(self, change: CodeChange,
            ...
            eligible = [];
            /* TODO: for-each Java para r em self.reviewers.values() */
                if (! r.active) {
                    continue;
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENREPUBLIC -- NOVIDADE, RISCO E DEPLOY");
        System.out.println("  'Nem toda mudanca && igual.'");
        System.out.println("=" * 80);
        ...
        /* para cada (label, change) em changes: */
            ...
            System.out.println("  Autor: {change.author_name} | Projeto: {change.target_project}");
            System.out.println("  {'='*70}");
            System.out.println("  Novidade MACRO: {cls['macro_classification']}");
            System.out.println("    -> {cls['macro_description']}");
            System.out.println("  Flags MICRO:");
            /* TODO: for-each Java para flag em cls["micro_flags"] */
                System.out.println("    -> {flag}");
            System.out.println("  Risco: {cls['risk_level']}");
            if (cls["exploit_patterns"]) {
                System.out.println("  EXPLOITS DETECTADOS:");
                /* TODO: for-each Java para ep em cls["exploit_patterns"] */
                    System.out.println("    !!! {ep}");
            System.out.println("  Revisao: {cls['review_required']}");
            System.out.println("  Revisores necessarios: {cls['reviewers_needed']}");
            System.out.println("  Requer votacao: {'SIM' if cls['requires_vote'] else '!'}");
            System.out.println("  Requer seguranca: {'SIM' if cls['requires_security_team'] else '!'}");
            System.out.println("  Auto-deploy: {'SIM' if cls['can_auto_deploy'] else 'NAO'}");
            System.out.println("  Revisores elegiveis: {result['eligible_reviewers']}");
            System.out.println("\n  DECISAO: {cls['deploy_decision']}");
        // === Resumo ===
        System.out.println("\n\n  {'='*70}");
        System.out.println("  RESUMO DE NOVIDADE E REVISAO");
        System.out.println("  {'='*70}\n");
        System.out.println("  {'Mudanca':<45} {'Novidade':<20} {'Risco':<12} {'Revisores'}");
        System.out.println("  {'-'*85}");
        /* para cada (label, change) em changes: */
            r = pipeline.classifier.classify(change);
            System.out.println("  {label[:44]:<45} {r['macro_classification']:<20} ";
                "{r['risk_level']:<12} {r['reviewers_needed']}");
        System.out.println("\n\n{'='*80}");
        System.out.println("  PRINCIPIOS DO DEPLOY NA REPUBLICA");
        System.out.println("{'='*80}");
        System.out.println(""";
    DEFINICAO DE NOVIDADE:;
        LARGO (macro):;
