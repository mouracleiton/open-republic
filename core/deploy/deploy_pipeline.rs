...
struct CodeChange {
    ...
    // O que mudou
    let scope: ChangeScope = ChangeScope.MODULE;
    let change_type: ChangeType = ChangeType.NEW_FUNCTION;
    let novelty: NoveltyLevel = NoveltyLevel.EXISTING_EXTEND;
    let risk: RiskLevel = RiskLevel.MODERATE;
    // Detalhes
    ...
    let accesses_citizen_data: bool = false;
    let accesses_health_data: bool = false;
    let accesses_credit_data: bool = false;
    let accesses_location_data: bool = false;
    let accesses_communication: bool = false;
    let accesses_children_data: bool = false;
    // Permissoes
    let grants_new_permission: bool = false;
    let modifies_governance: bool = false;
    let modifies_voting: bool = false;
    let modifies_credit_system: bool = false;
    // Dependencias
    let new_imports: [texto] = field(default_factory=list);
    let new_network_calls: [texto] = field(default_factory=list);
    // Hash do codigo para auditoria
    let code_hash: String = "";
#[derive(Debug, Clone)]
struct NoveltyClassifier {
    ...
    fn classify(self, change: CodeChange) -> {texto: qualquer} {
    ...
    fn _classify_macro(self, c: CodeChange) -> NoveltyLevel {
    ...
    fn _classify_micro(self, c: CodeChange) -> [texto] {
    ...
    fn _check_exploit(self, c: CodeChange) -> [texto] {
    ...
    fn _macro_desc(level: NoveltyLevel) -> String {
        descs = {
    ...
    // decorador: @staticmethod
    fn _deploy_decision(review: Dict, exploits: [texto]) -> String {
        if exploits {
...
struct Reviewer {
    ...
    name: texto;
    teams: [ReviewTeam];
    let expertise: [texto] = field(default_factory=list);
    let reviews_done: i64 = 0;
    let exploits_caught: i64 = 0;
    let reputation: f64 = 50.0 // 0-100;
    let active: bool = true;
    // Anti-self-exploit: NAO pode revisar proprio codigo
    // Anti-colusion: NAO pode revisar codigo de familiar/parceiro direto
    let conflicts_of_interest: {texto} = field(default_factory=set);
#[derive(Debug, Clone)]
struct DeployPipeline {
    ...
    fn submit(self, change: CodeChange) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    println!("=" * 80);
    println!("  OPENREPUBLIC -- NOVIDADE, RISCO E DEPLOY");
    println!("  'Nem toda mudanca && igual.'");
    println!("=" * 80);
    ...
    para cada (label, change) em changes: {
        ...
        println!("\n  {'='*70}");
        println!("  {label}");
        println!("  Autor: {change.author_name} | Projeto: {change.target_project}");
        println!("  {'='*70}");
        println!("  Novidade MACRO: {cls['macro_classification']}");
        println!("    -> {cls['macro_description']}");
        println!("  Flags MICRO:");
        for flag in cls["micro_flags"] {
            println!("    -> {flag}");
        println!("  Risco: {cls['risk_level']}");
        if cls["exploit_patterns"] {
            println!("  EXPLOITS DETECTADOS:");
            for ep in cls["exploit_patterns"] {
                println!("    !!! {ep}");
        println!("  Revisao: {cls['review_required']}");
        println!("  Revisores necessarios: {cls['reviewers_needed']}");
        println!("  Requer votacao: {'SIM' if cls['requires_vote'] else '!'}");
        println!("  Requer seguranca: {'SIM' if cls['requires_security_team'] else '!'}");
        println!("  Auto-deploy: {'SIM' if cls['can_auto_deploy'] else 'NAO'}");
        println!("  Revisores elegiveis: {result['eligible_reviewers']}");
        println!("\n  DECISAO: {cls['deploy_decision']}");
    // === Resumo ===
    println!("\n\n  {'='*70}");
    println!("  RESUMO DE NOVIDADE E REVISAO");
    println!("  {'='*70}\n");
    println!("  {'Mudanca':<45} {'Novidade':<20} {'Risco':<12} {'Revisores'}");
    println!("  {'-'*85}");
    para cada (label, change) em changes: {
        r = pipeline.classifier.classify(change);
        println!("  {label[:44]:<45} {r['macro_classification']:<20} ";
            "{r['risk_level']:<12} {r['reviewers_needed']}");
    println!("\n\n{'='*80}");
    println!("  PRINCIPIOS DO DEPLOY NA REPUBLICA");
    println!("{'='*80}");
    println!(""";
DEFINICAO DE NOVIDADE:;
    LARGO (macro):;
