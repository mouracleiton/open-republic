...
#ifndef OPENREPUBLIC_CLASSIFICACAO_DE_NOVIDADE_E_DEPLOY_PIPELINE_H
#define OPENREPUBLIC_CLASSIFICACAO_DE_NOVIDADE_E_DEPLOY_PIPELINE_H
...
typedef struct CodeChange {
    ...
    bool accesses_citizen_data = false;
    bool accesses_health_data = false;
    bool accesses_credit_data = false;
    bool accesses_location_data = false;
    bool accesses_communication = false;
    bool accesses_children_data = false;
    // Permissoes
    bool grants_new_permission = false;
    bool modifies_governance = false;
    bool modifies_voting = false;
    bool modifies_credit_system = false;
    // Dependencias
    ...
    char* code_hash = "";
typedef struct NoveltyClassifier {
    ...
    {texto: qualquer} classify(self, change: CodeChange) {
    ...
    NoveltyLevel _classify_macro(self, c: CodeChange) {
    ...
    [texto] _classify_micro(self, c: CodeChange) {
    ...
    [texto] _check_exploit(self, c: CodeChange) {
        ...
        /* TODO: iterador C manual para imp em c.new_imports */
    ...
    char* _macro_desc(level: NoveltyLevel) {
        descs = {
    ...
    // decorador: @staticmethod
    char* _deploy_decision(review: Dict, exploits: [texto]) {
        if (exploits) {
...
typedef struct DeployPipeline {
    ...
    {texto: qualquer} submit(self, change: CodeChange) {
    ...
    funcao _find_reviewers(self, change: CodeChange,
        ...
        eligible = [];
        /* TODO: iterador C manual para r em self.reviewers.values() */
            if (! r.active) {
                continue;
...
if (__name__ == "__main__") {
    printf("=" * 80);
    printf("  OPENREPUBLIC -- NOVIDADE, RISCO E DEPLOY");
    printf("  'Nem toda mudanca && igual.'");
    printf("=" * 80);
    ...
    /* para cada (label, change) em changes: */
        ...
        printf("\n  {'='*70}");
        printf("  {label}");
        printf("  Autor: {change.author_name} | Projeto: {change.target_project}");
        printf("  {'='*70}");
        printf("  Novidade MACRO: {cls['macro_classification']}");
        printf("    -> {cls['macro_description']}");
        printf("  Flags MICRO:");
        /* TODO: iterador C manual para flag em cls["micro_flags"] */
            printf("    -> {flag}");
        printf("  Risco: {cls['risk_level']}");
        if (cls["exploit_patterns"]) {
            printf("  EXPLOITS DETECTADOS:");
            /* TODO: iterador C manual para ep em cls["exploit_patterns"] */
                printf("    !!! {ep}");
        printf("  Revisao: {cls['review_required']}");
        printf("  Revisores necessarios: {cls['reviewers_needed']}");
        printf("  Requer votacao: {'SIM' if cls['requires_vote'] else '!'}");
        printf("  Requer seguranca: {'SIM' if cls['requires_security_team'] else '!'}");
        printf("  Auto-deploy: {'SIM' if cls['can_auto_deploy'] else 'NAO'}");
        printf("  Revisores elegiveis: {result['eligible_reviewers']}");
        printf("\n  DECISAO: {cls['deploy_decision']}");
    // === Resumo ===
    printf("\n\n  {'='*70}");
    printf("  RESUMO DE NOVIDADE E REVISAO");
    printf("  {'='*70}\n");
    printf("  {'Mudanca':<45} {'Novidade':<20} {'Risco':<12} {'Revisores'}");
    printf("  {'-'*85}");
    /* para cada (label, change) em changes: */
        r = pipeline.classifier.classify(change);
        printf("  {label[:44]:<45} {r['macro_classification']:<20} ";
            "{r['risk_level']:<12} {r['reviewers_needed']}");
    printf("\n\n{'='*80}");
    printf("  PRINCIPIOS DO DEPLOY NA REPUBLICA");
    printf("{'='*80}");
    printf(""";
DEFINICAO DE NOVIDADE:;
    LARGO (macro):;
...
#endif // OPENREPUBLIC_CLASSIFICACAO_DE_NOVIDADE_E_DEPLOY_PIPELINE_H
