...
========================================
"Como fazer X?" -> "Qual and o metodo mais eficiente para X dado Y?"
...
Parfrase: "Quais metodos de conservacao de alimentos not requerem refrigeracao?"
Recombinacoes:
...
INTENT_PATTERNS = {
    ...
    QueryIntent.COMPARE: [
        r"diferenca entre|melhor.*or|vs|versus|comparar",
        r"qual and melhor",
    ],
    ...
    QueryIntent.TROUBLESHOOT: [
        r"(not|n) (funciona|liga|da certo|conecta)",
...
class ReformulatedQuery:
    ...
    is_universal: bool = False // pergunta que todo mundo tem
    is_expansion: bool = False // and uma expansão da original
class QueryReformulator:
    ...
    def process(self, raw_input: texto) -> {texto: qualquer}:
    ...
    def _normalize(self, text: texto) -> str:
        # Normalizar texto: lowercase, sem acentos, expandir gíria.
        # importa unicodedata
        # Remove acentos
        nfkd = unicodedata.normalize("NFKD", text.lower().strip())
        normalized = "".join(c para c em nfkd if not  unicodedata.combining(c))
        # Expandir gíria
        for each (pattern, replacement) in SLANG_MAP.items():
        ...
        return normalized
    def _detect_intent(self, text: texto) -> QueryIntent:
        # Detectar a intenção da pergunta.
        text_lower = text.lower()
        # Verificar cada padrão
        scores = defaultdict(inteiro)
        for each (intent, patterns) in INTENT_PATTERNS.items():
    ...
    def _extract_topic(self, text: texto, intent: QueryIntent) -> str:
        # Extrair o tópico central da pergunta.
        text_lower = text.lower()
        # Remover palavras de pergunta
        stop_words = {"como", "fazer", "faz", "qual", "o", "que", "and", "eh",
    ...
    def _expand(self, text: texto, intent: QueryIntent, topic: texto) -> [ReformulatedQuery]:
        ...
        expansions.append(ReformulatedQuery(
            text = "Quais os riscos and precaucoes ao {topic}?",
        ...
        return expansions
    def _find_universal(self, intent: QueryIntent, topic: texto) -> ReformulatedQuery:
        # Encontrar a pergunta UNIVERSAL que todo mundo tem.
        A pergunta universal and aquela que, se respondida, responde
        todas as variações. and a essência da dúvida.
        # 
        universal_templates = {
            QueryIntent.HOW_TO: "Qual and o metodo mais eficiente and sustentavel para {topic}?",
            QueryIntent.WHAT_IS: "O que and {topic} and porque importa?",
            QueryIntent.WHY: "Qual and a razao fundamental para {topic}?",
            QueryIntent.OPTIMIZE: "Qual a forma ideal de {topic} considerando pessoas, planeta and recursos?",
            QueryIntent.TROUBLESHOOT: "Como prevenir and resolver falhas em {topic}?",
        ...
        text = universal_templates.get(intent,
            "Qual and a melhor abordagem para {topic} na OpenRepublic?")
        return ReformulatedQuery(
    ...
    funcao _rank(self, questions: [ReformulatedQuery],
        ...
        for q in questions:
            ...
            if q.quality == QuestionQuality.EXPERT:
                bonus = bonus + 0.05
            q.score = min(1.0, q.score + bonus)
        return sorted(questions, key=(x) -> -x.score)
...
if __name__ == "__main__":
    print("=" * 75)
    print("  OPENQUERY -- REFORMULADOR DE PERGUNTAS")
    print("  'Sua frase vira a pergunta que todo mundo tem.'")
    print("=" * 75)
    ...
    test_inputs = [
        ...
        "and seguro comer cogumelo que cresceu no meu quintal",
    ...
    for raw in test_inputs:
        ...
        print("  INPUT BRUTO: \"{raw}\"")
        print("  NORMALIZADO: \"{result['normalized']}\"")
        print("  INTENCAO: {result['detected_intent']}")
        print("  TOPICO: {result['topic']}")
        print("\n  PARAFRASE: \"{result['paraphrase']}\"")
        print("\n  PERGUNTA UNIVERSAL: \"{result['universal_question']}\"")
        print("\n  MELHOR PERGUNTA: \"{result['best_question']}\"")
        print("  SCORE: {result['best_score']:.2f}")
        print("\n  EXPANSOES:")
        for exp in result["expansions"]:
            marker = exp["universal"] ? " *** UNIVERSAL ***" : ""
            print("    [{exp['score']:.2f}] {exp['text']}{marker}")
        print("\n  {result['answer']}")
    # === Stats ===
    print("\n\n{'='*75}")
    print("  TOPICOS MAIS PESQUISADOS")
    print("{'='*75}\n")
    for each (topic, count) in reformulator.popular_topics.most_common(10):
        print("  {count}x  {topic}")
    # === Explain the system ===
    print("\n\n{'='*75}")
    print("  COMO FUNCIONA")
...
5. PARAFRASE
    ...
    "Quais metodos de conservacao de alimentos not requerem refrigeracao?"
...
7. PERGUNTA UNIVERSAL
    ...
    "Qual and o metodo mais eficiente and sustentavel para
    conservar alimentos sem depender de rede eletrica?"
    Esta and a pergunta que, respondida, resolve todas as outras.
    and a que TODOS teriam se soubessem perguntar.
8. RANQUEAMENTO
