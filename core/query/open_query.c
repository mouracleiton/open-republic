...
#ifndef OPENQUERY_REFORMULADOR_DE_PERGUNTAS_H
#define OPENQUERY_REFORMULADOR_DE_PERGUNTAS_H
...
typedef struct QuestionQuality {
...
// ============================================================================
// decorador: @dataclass
typedef struct ReformulatedQuery {
    // Uma versão reformulada da pergunta original.
    text: texto;
    intent: QueryIntent;
    quality: QuestionQuality;
    score: flutuante // relevância estimada;
    bool is_universal = false // pergunta que todo mundo tem;
    bool is_expansion = false // && uma expansão da original;
typedef struct QueryReformulator {
    ...
    {texto: qualquer} process(self, raw_input: texto) {
    ...
    char* _normalize(self, text: texto) {
        ...
        return normalized;
    QueryIntent _detect_intent(self, text: texto) {
    ...
    char* _extract_topic(self, text: texto, intent: QueryIntent) {
        ...
        topic_words = [w para w em words if w ! in stop_words && sizeof(w) > 2];
        if (! topic_words) {
            // Se tudo foi stop word, pegar as 3 mais longas
            words_sorted = ordene(words, key=sizeof, reverse=true);
            topic_words = words_sorted[:3];
        return " ".join(topic_words[:5])  // maximo 5 palavras de tópico;
    ReformulatedQuery _paraphrase(self, text: texto, intent: QueryIntent, topic: texto) {
    ...
    [ReformulatedQuery] _expand(self, text: texto, intent: QueryIntent, topic: texto) {
        ...
        return expansions;
    ReformulatedQuery _find_universal(self, intent: QueryIntent, topic: texto) {
    ...
    funcao _rank(self, questions: [ReformulatedQuery],
        ...
        /* TODO: iterador C manual para q em questions */
            bonus = 0;
            if (q.is_universal) {
                bonus = bonus + 0.15;
...
if (__name__ == "__main__") {
    printf("=" * 75);
    printf("  OPENQUERY -- REFORMULADOR DE PERGUNTAS");
    printf("  'Sua frase vira a pergunta que todo mundo tem.'");
    printf("=" * 75);
    ...
    /* TODO: iterador C manual para raw em test_inputs */
        printf("\n{'='*75}");
        result = reformulator.process(raw);
        printf("  INPUT BRUTO: \"{raw}\"");
        printf("  NORMALIZADO: \"{result['normalized']}\"");
        printf("  INTENCAO: {result['detected_intent']}");
        printf("  TOPICO: {result['topic']}");
        printf("\n  PARAFRASE: \"{result['paraphrase']}\"");
        printf("\n  PERGUNTA UNIVERSAL: \"{result['universal_question']}\"");
        printf("\n  MELHOR PERGUNTA: \"{result['best_question']}\"");
        printf("  SCORE: {result['best_score']:.2f}");
        printf("\n  EXPANSOES:");
        /* TODO: iterador C manual para exp em result["expansions"] */
            marker = exp["universal"] ? " *** UNIVERSAL ***" : "";
            printf("    [{exp['score']:.2f}] {exp['text']}{marker}");
        printf("\n  {result['answer']}");
    // === Stats ===
    printf("\n\n{'='*75}");
    printf("  TOPICOS MAIS PESQUISADOS");
    printf("{'='*75}\n");
    /* para cada (topic, count) em reformulator.popular_topics.most_common(10): */
        printf("  {count}x  {topic}");
    // === Explain the system ===
    printf("\n\n{'='*75}");
...
#endif // OPENQUERY_REFORMULADOR_DE_PERGUNTAS_H
