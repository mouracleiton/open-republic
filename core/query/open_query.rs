...
struct ReformulatedQuery {
    ...
    let is_universal: bool = false // pergunta que todo mundo tem;
    let is_expansion: bool = false // && uma expansão da original;
#[derive(Debug, Clone)]
struct QueryReformulator {
    ...
    fn process(self, raw_input: texto) -> {texto: qualquer} {
    ...
    fn _normalize(self, text: texto) -> String {
        ...
        return normalized;
    fn _detect_intent(self, text: texto) -> QueryIntent {
    ...
    fn _extract_topic(self, text: texto, intent: QueryIntent) -> String {
    ...
    fn _paraphrase(self, text: texto, intent: QueryIntent, topic: texto) -> ReformulatedQuery {
    ...
    fn _expand(self, text: texto, intent: QueryIntent, topic: texto) -> [ReformulatedQuery] {
        ...
        return expansions;
    fn _find_universal(self, intent: QueryIntent, topic: texto) -> ReformulatedQuery {
...
if __name__ == "__main__" {
    println!("=" * 75);
    println!("  OPENQUERY -- REFORMULADOR DE PERGUNTAS");
    println!("  'Sua frase vira a pergunta que todo mundo tem.'");
    println!("=" * 75);
    ...
    for raw in test_inputs {
        ...
        println!("  INPUT BRUTO: \"{raw}\"");
        println!("  NORMALIZADO: \"{result['normalized']}\"");
        println!("  INTENCAO: {result['detected_intent']}");
        println!("  TOPICO: {result['topic']}");
        println!("\n  PARAFRASE: \"{result['paraphrase']}\"");
        println!("\n  PERGUNTA UNIVERSAL: \"{result['universal_question']}\"");
        println!("\n  MELHOR PERGUNTA: \"{result['best_question']}\"");
        println!("  SCORE: {result['best_score']:.2f}");
        println!("\n  EXPANSOES:");
        for exp in result["expansions"] {
            marker = exp["universal"] ? " *** UNIVERSAL ***" : "";
            println!("    [{exp['score']:.2f}] {exp['text']}{marker}");
        println!("\n  {result['answer']}");
    // === Stats ===
    println!("\n\n{'='*75}");
    println!("  TOPICOS MAIS PESQUISADOS");
    println!("{'='*75}\n");
    para cada (topic, count) em reformulator.popular_topics.most_common(10): {
        println!("  {count}x  {topic}");
    // === Explain the system ===
    println!("\n\n{'='*75}");
