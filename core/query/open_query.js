...
class ReformulatedQuery {
    ...
    const is_universal = false // pergunta que todo mundo tem;
    const is_expansion = false // && uma expansão da original;
class QueryReformulator {
    ...
    _normalize(self, text: texto) {
        ...
        return normalized;
    _detect_intent(self, text: texto) {
    ...
    _extract_topic(self, text: texto, intent: QueryIntent) {
        ...
        topic_words = [w para w em words if w ! in stop_words && .length(w) > 2];
        if (! topic_words) {
            // Se tudo foi stop word, pegar as 3 mais longas
            words_sorted = ordene(words, key=.length, reverse=true);
            topic_words = words_sorted[:3];
        return " ".join(topic_words[:5])  // maximo 5 palavras de tópico;
    _paraphrase(self, text: texto, intent: QueryIntent, topic: texto) {
    ...
    _expand(self, text: texto, intent: QueryIntent, topic: texto) {
        ...
        return expansions;
    _find_universal(self, intent: QueryIntent, topic: texto) {
...
if (__name__ == "__main__") {
    console.log("=" * 75);
    console.log("  OPENQUERY -- REFORMULADOR DE PERGUNTAS");
    console.log("  'Sua frase vira a pergunta que todo mundo tem.'");
    console.log("=" * 75);
    ...
    for (const raw of test_inputs) {
        ...
        console.log("  INPUT BRUTO: \"{raw}\"");
        console.log("  NORMALIZADO: \"{result['normalized']}\"");
        console.log("  INTENCAO: {result['detected_intent']}");
        console.log("  TOPICO: {result['topic']}");
        console.log("\n  PARAFRASE: \"{result['paraphrase']}\"");
        console.log("\n  PERGUNTA UNIVERSAL: \"{result['universal_question']}\"");
        console.log("\n  MELHOR PERGUNTA: \"{result['best_question']}\"");
        console.log("  SCORE: {result['best_score']:.2f}");
        console.log("\n  EXPANSOES:");
        for (const exp of result["expansions"]) {
            marker = exp["universal"] ? " *** UNIVERSAL ***" : "";
            console.log("    [{exp['score']:.2f}] {exp['text']}{marker}");
        console.log("\n  {result['answer']}");
    // === Stats ===
    console.log("\n\n{'='*75}");
    console.log("  TOPICOS MAIS PESQUISADOS");
    console.log("{'='*75}\n");
