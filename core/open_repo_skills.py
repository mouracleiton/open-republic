...
# !/usr/bin/env python3
# 
OpenRepoPolicy + OpenSkills -- Repositorio Unico and Skills Comprovadas
======================================================================
...
COMO FUNCIONA:
- Repositorio principal (main) and a UNICA fonte de verdade
- Cidadaos PUXAM (pull) do main para local
- Mudancas sao PROPOSTAS (merge request) and VOTADAS (P4)
- Ninguem copia o projeto and faz versao propria
- Tudo and UNICO. Tudo and bem comum (CC0)
PARTE 2 -- OPENSKILLS:
"Curriculum vitae and PAPEL. OpenSkills and VIVO.
Skills comprovadas pela INTERACAO com o sistema.
Nao 'eu sei Rust'. 'O SISTEMA TESTOU and APROVOU'."
COMO FUNCIONA:
- Cada cidadao tem um repositorio de skills
- Skills sao COMPROVADAS pelo sistema (not auto-declaradas)
...
class MergeProposal:
    ...
    lines_added: int = 0
    lines_removed: int = 0
    status: MergeStatus = MergeStatus.DRAFT
    votes_for: int = 0
    votes_against: int = 0
    total_voters: int = 10000
    quorum_needed: float = 0.51 // 51% para aprovar
    # Validacao automatica
    passes_constitution: bool = True
    fails_reason: str = ""
    tested: bool = False // rodou sem erro?
    test_result: str = ""
    created_date: str = ""
    merged_date: str = ""
class RepoEngine:
    ...
    7. Tudo CC0 -- not ha "propriedade" de codigo.
    O QUE and PROIBIDO:
    - Fork (copiar and fazer versao propria)
    - Mutilacao (pegar parte do projeto and descartar o resto)
    ...
    - Mas o PROJETO PRINCIPAL and UNICO
    # 
    def __init__(self):
    ...
    def pull(self, citizen_id: texto) -> {texto: qualquer}:
    ...
    funcao propose(self, author: texto, title: texto,
                description: str = "",
                files: [texto] = None,
                added: int = 0, removed: inteiro = 0,
                passes_constitution: bool = True,
                fails_reason: str = "",
                tested: bool = True) -> {texto: qualquer}:
        ...
        if not passes_constitution:
            ...
            return {
                ...
                "reason": fails_reason  or  "Viola P1-P4",
                "message": (
        ...
        proposal = MergeProposal(
            ...
            description = description, files_changed=files or [],
    ...
    def _merge(self, proposal: MergeProposal) -> None:
    ...
    def reject_fork(self, citizen: texto, reason: texto) -> {texto: qualquer}:
        ...
        return {
            ...
            "policy": (
                ...
                "Nao pode COPIAR and fazer versao propria. "
                "P1 anti-elitismo: projeto and de TODOS, not de cada um."
            ),
            "alternative": "Proponha sua mudanca. A assembleia vota.",
        }
    def repo_status(self) -> {texto: qualquer}:
        return {
            ...
            **self.repo_stats,
            ...
            "propriedade": "BEM COMUM (ninguem and dono)",
        }
# ============================================================================
# 2. OPENSKILLS (substituto do curriculum vitae)
# ============================================================================
class SkillProof(Enum):
...
class VerifiedSkill:
    ...
    proofs: [Dict] = field(default_factory=list)
    # Comprovacoes
    system_tests_passed: int = 0
    tasks_completed: int = 0
    courses_certified: int = 0
    peer_verifications: int = 0
    contributions: int = 0
    mentor_endorsements: int = 0
    demonstrations: int = 0
    # Score de confianca
    confidence: float = 0.0 // 0-1 (quao confiavel and a skill)
    # decorador: @property
    def is_verified(self) -> bool:
        return len(self.proofs) > 0 and self.confidence > 0.3
# decorador: @dataclass
class SkillProfile:
    ...
    - OpenSkills: o SISTEMA COMPROVA. Voce not auto-declara.
    ...
    age: int = 0
    profession: str = ""
    profession_level: str = ""  // OpenProfessions (aprendiz -> mestre)
    skills: {texto: VerifiedSkill} = field(default_factory=dict)
    created_date: str = ""
    # Stats
    total_skills_verified: int = 0
    total_tasks_completed: int = 0
    total_contributions: int = 0
    impact_score: float = 0.0 // OpenCredit/OpenCreator
    # decorador: @property
    def skill_count(self) -> int:
        return len(self.skills)
    # decorador: @property
    def verified_count(self) -> int:
        return sum(1 para s em self.skills.values() if s.is_verified)
class SkillsEngine:
    # Motor de skills verificadas.
    COMO UMA SKILL and COMPROVADA:
    ...
    NENHUMA skill and auto-declarada.
    TUDO and comprovado por INTERACAO com o sistema.
    # 
    def __init__(self):
        self.profiles: {texto: SkillProfile} = {}
    funcao create_profile(self, citizen_id: texto, name: texto,
                    age: int = 0, profession: texto = ""
                    ) -> {texto: qualquer}:
    ...
    funcao add_proof(self, citizen_id: texto, skill_name: texto,
                ...
                proof_detail: str = "",
                level: SkillLevel = SkillLevel.BASIC) -> {texto: qualquer}:
        ...
        elif proof_type == SkillProof.TASK_COMPLETED:
            skill.tasks_completed += 1
        elif proof_type == SkillProof.COURSE_CERTIFIED:
            skill.courses_certified += 1
        elif proof_type == SkillProof.PEER_VERIFIED:
            skill.peer_verifications += 1
        elif proof_type == SkillProof.CONTRIBUTION:
            skill.contributions += 1
        elif proof_type == SkillProof.MENTOR_ENDORSED:
            skill.mentor_endorsements += 1
        elif proof_type == SkillProof.DEMONSTRATED:
        ...
        skill.level = levels[min(total_proofs, len(levels) - 1)]
        # Confianca
        weights = {
        ...
        skill.confidence = min(1.0, sum(
            weights.get(SkillProof(p["type"]), 0.05) para p em skill.proofs
        ))
        if skill.is_verified and skill.level.level_num >= 2:
    ...
    def get_profile(self, citizen_id: texto) -> {texto: qualquer}:
        ...
        return {
            ...
            "skills": [
                ...
                para s in ordene(profile.skills.values(),
        ...
        for profile in self.profiles.values():
            for skill in profile.skills.values():
                if (skill.skill_name.lower() == skill_name.lower()
                    and skill.level.level_num >= min_level
                    and skill.is_verified):
                    results.append({
                        "citizen": profile.citizen_name,
    ...
    funcao compare_to_cv(self) retorna List[{texto: texto}]:
        ...
        return [
            ...
            {"aspecto": "Confianca",
            "cv_tradicional": "Baixa (quem sabe se and verdade?)",
...
if __name__ == "__main__":
    ...
    skills = SkillsEngine()
    print("=" * 80)
    print("  OPENREPOPOLICY + OPENSKILLS")
    print("  Repositorio unico + Skills comprovadas pelo sistema")
    print("=" * 80)
    # === PARTE 1: REPOSITORIO ===
    print("\n\n  {'='*40}")
    print("  PARTE 1: OPENREPOPOLICY")
    print("  {'='*40}\n")
    ...
    # Fork rejeitado
    print("\n\n  === FORK REJEITADO ===\n")
    fork = repo.reject_fork("joao", "Tentou copiar OpenHealth and remover modulos de saude mental")
    print("  {fork['citizen']}: {fork['action']}")
    print("  Politica: {fork['policy']}")
    print("  Alternativa: {fork['alternative']}")
    # Status repositorio
    print("\n\n  === STATUS DO REPOSITORIO ===\n")
    status = repo.repo_status()
    for each (k, v) in status.items():
        print("  {k:<30} {v}")
    # === PARTE 2: OPENSKILLS ===
    print("\n\n  {'='*40}")
    print("  PARTE 2: OPENSKILLS")
    print("  {'='*40}\n")
    # Criar perfis
    ...
    para skill, cat, proof, detail, level in proofs_cleiton:
        ...
        print("  {r['citizen']}: {r['skill']} -> {r['level']} "
    ...
    # === FILOSOFIA ===
    print("\n\n{'='*80}")
    print("  FILOSOFIA: REPOSITORIO UNICO + SKILLS COMPROVADAS")
    print("{'='*80}")
    print("""
REPOSITORIO UNICO (sem forks):
    ...
    Tudo CC0. Ninguem and dono. Todos herdeiros.
    O que pode: PULL (puxar), PROPOR (merge request), VOTAR.
    O que not pode: FORK (copiar), MUTILAR (descartar partes),
    PROPRIEDADE (codigo and bem comum).
...
PRINCIPIOS:
    P1: Repositorio unico (sem elite de codigo). Skills por competencia (not diploma).
    P2: Cidadao escolhe se perfil and publico. Skills sao suas.
    P3: Contribuir no repositorio = trabalho de alto impacto.
    P4: Assembleia vota mudancas. Skills verificadas por pares.
# )
    print("{'='*80}")
    rs = repo.repo_status()
    print("  Repo: {rs['repositorio']} | {rs['propostas_integradas']} integradas | "
        "{rs['forks_rejeitados']} forks rejeitados.")
    print("  Skills: {s['total_perfis']} perfis, {s['skills_unicas']} skills unicas.")
    print("  Sem fork. Sem CV. So Republica.")
    print("{'='*80}")
