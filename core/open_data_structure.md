# OpenDataStructure -- Estrutura, Compactacao e Metadados de Pacotes

**Linguagem:** Portugol++ (PPM)

**Arquivo original:** `core/open_data_structure.py`
...
"O Hermes precisa receber dados de 3 fontes:
 ...
 Cada dado precisa ser ESTRUTURADO, COMPACTADO e ROTULADO.
...
classe DataSource herda de Enum:
    KEYBOARD = "keyboard"
    AUDIO = "audio"
    WEBCAM = "webcam"
    SCREEN = "screen"
...
classe DataPriority herda de Enum:
...
classe InputType herda de Enum:
...
// decorador: @dataclass
classe PacketMetadata:
    // Metadados do pacote (quem, quando, o que, prioridade).
    seja packet_id: texto = ""
    seja timestamp: texto = ""
    seja user_id: texto = ""
    seja session_id: texto = ""
    seja sources: [texto] = field(default_factory=list)
    seja compression: texto = "openpack_v1"
    seja priority: DataPriority = DataPriority.NORMAL
    seja input_type: InputType = InputType.COMMAND
    seja context: texto = ""  // o que o usuario esta fazendo
    seja location: texto = ""  // onde (opcional)
    seja device: texto = ""  // smartphone, notebook, terminal
    ...
    funcao create(cls, user_id: texto, sources: [texto],
               ...
               seja input_type: InputType = InputType.COMMAND,
               seja context: texto = "",
               seja device: texto = "notebook") -> "PacketMetadata":
        retorne cls(
...
classe KeyboardData:
    ...
    seja raw_text: texto = ""
    seja compressed_text: texto = ""  // base64(zlib(text))
    seja raw_length: inteiro = 0
    seja compressed_length: inteiro = 0
    seja compression_ratio: flutuante = 0.0
    seja language: texto = "pt-BR"
    seja input_type: texto = "command"  // command, question, context
    seja modifiers: [texto] = field(default_factory=list) // shift, ctrl, etc
    seja typing_speed_wpm: flutuante = 0.0 // palavras por minuto
    seja autocorrect_used: logico = falso
    seja emoji_detected: logico = falso
    seja code_detected: logico = falso // se digitou codigo
    seja url_detected: logico = falso

    // decorador: @classmethod
    funcao from_input(cls, text: texto, language: texto = "pt-BR",
                   seja input_type: texto = "command",
                   seja modifiers: [texto] = nulo) -> "KeyboardData":
...
classe AudioData:
    ...
    // 
    seja compressed_audio: texto = ""  // base64(opus)
    seja channels: [texto] = field(default_factory=() -> ["user_voice"])
    seja duration_sec: flutuante = 0.0
    seja format: texto = "opus_16khz"
    seja sample_rate: inteiro = 16000
    seja bitrate_kbps: inteiro = 24
    seja vad_detected: logico = falso // voice activity detection
    seja diarization: {texto: flutuante} = field(default_factory=dict)
    seja transcription: texto = ""  // texto transcrito (STT)
    seja transcription_confidence: flutuante = 0.0
    seja ambient_type: texto = ""  // rua, escritorio, silencio, musica
    seja noise_level_db: flutuante = 0.0

    // decorador: @classmethod
    funcao from_capture(cls, duration: flutuante = 3.0,
                     seja transcription: texto = "",
                     seja channels: [texto] = nulo,
                     seja vad: logico = verdadeiro,
                     seja ambient: texto = "escritorio",
                     seja noise_db: flutuante = 35.0) -> "AudioData":
        ...
        retorne cls(
            ...
            channels = channels  ou  ["user_voice"],
...
classe WebcamData:
    ...
    FRAMES (nao video continuo):
    - 1-3 frames por pacote (nao 30fps)
    - Redundancia removida (so muda o que mudou)
    - Face detection + expressao
    - Gesto detection (mao, cabeca)
    - Screen share opcional
    // 
    seja compressed_frames: texto = ""
    seja frame_count: inteiro = 0
    seja resolution: texto = "640x480"
    seja fps_captured: inteiro = 1 // 1-3 frames por pacote
    seja face_detected: logico = falso
    seja face_expression: texto = ""  // neutro, feliz, focado, cansado
    seja gesture: texto = "none"  // none, wave, point, thumbs_up
    seja gaze_direction: texto = "screen"  // screen, away, phone
    seja posture: texto = "upright"  // upright, leaning, standing
    seja screen_share: logico = falso
    seja screen_content: texto = ""  // texto detectado na tela (OCR)
    seja objects_detected: [texto] = field(default_factory=list)

    // decorador: @classmethod
    funcao from_capture(cls, face: logico = verdadeiro,
                     seja expression: texto = "focado",
                     seja gesture: texto = "none",
                     seja gaze: texto = "screen",
                     seja posture: texto = "upright",
                     seja screen_share: logico = falso,
                     seja screen_text: texto = "",
                     seja objects: [texto] = nulo) -> "WebcamData":
...
classe OpenDataPacket:
    ...
    seja keyboard: KeyboardData? = nulo
    seja audio: AudioData? = nulo
    seja webcam: WebcamData? = nulo

    // Fusao multimodal
    seja fusion_mode: texto = "single"  // single, combo_multimodal
    seja primary_source: DataSource = DataSource.KEYBOARD
    seja secondary_sources: [DataSource] = field(default_factory=list)

    // decorador: @property
    funcao source_count(self) -> inteiro:
        count = 0
    ...
    // decorador: @property
    funcao is_multimodal(self) -> logico:
        retorne self.source_count >= 2

    // decorador: @property
    funcao total_compressed_size(self) -> inteiro:
        size = 0
    ...
    funcao to_json(self) -> texto:
        ...
        retorne json.dumps({
            ...
            self.keyboard ? "keyboard": asdict(self.keyboard) : nulo,
            self.audio ? "audio": asdict(self.audio) : nulo,
            self.webcam ? "webcam": asdict(self.webcam) : nulo,
            "fusion": {
    ...
    funcao summary(self) -> {texto: qualquer}:
        ...
        retorne {
            ...
            self.keyboard ? "keyboard_text": self.keyboard.decompress()[:50] : nulo,
            self.audio ? "audio_transcription": self.audio.transcription[:50] : nulo,
            self.webcam ? "webcam_expression": self.webcam.face_expression : nulo,
        }
...
classe DataPacketEngine:
    // Motor que cria, compacta e envia pacotes ao Hermes.

    COMO FUNCIONA:
    1. Captura dados de 3 fontes (keyboard, audio, webcam)
    2. Cada fonte ESTRUTURA e COMPACTA seus dados
    3. Motor cria PACOTE UNIFICADO com metadados
    4. Pacote e enviado ao Hermes (JSON compactado)
    5. Hermes recebe, descompacta e processa
    ...
    funcao create_combo_packet(self, user_id: texto,
        ...
        Este e o COMBO REFERENCIAL que o usuario quer:
        ...
        kb = text ? KeyboardData.from_input(text) : nulo
        audio = AudioData.from_capture(
            transcription ? 3.0, transcription, ambient=ambient) : nulo
        cam = WebcamData.from_capture(
            expression = expression, gesture=gesture, gaze=gaze)
    ...
    funcao _register(self, packet: OpenDataPacket) -> None:
        self.packets_sent += 1
    ...
    funcao stats(self) -> {texto: qualquer}:
        ratio = 0.0
        se self.total_bytes_raw > 0 entao:
...
se __name__ == "__main__" entao:
    ...
    imprima("=" * 80)
    imprima("  OPENDATASTRUCTURE -- PACOTE UNIFICADO DE DADOS")
    imprima("  Keyboard + Audio + Webcam em UM pacote compactado")
    imprima("=" * 80)
    ...
    s1 = p1.summary()
    imprima("  Packet ID: {s1['packet_id']}")
    imprima("  Fonte: {s1['sources']}")
    imprima("  Texto: '{s1['keyboard_text']}'")
    imprima("  Bruto: {s1['raw_size']}")
    imprima("  Compactado: {s1['compressed']}")
    imprima("  Compressao: {s1['compression']}")
    ...
    s2 = p2.summary()
    imprima("  Packet ID: {s2['packet_id']}")
    imprima("  Fonte: {s2['sources']}")
    imprima("  Transcricao: '{s2['audio_transcription']}'")
    imprima("  Duracao: {p2.audio.duration_sec}s")
    imprima("  VAD: {p2.audio.vad_detected}")
    imprima("  Diarizacao: {p2.audio.diarization}")
    imprima("  Ambiente: {p2.audio.ambient_type}")
    imprima("  Bruto: {s2['raw_size']}")
    imprima("  Compactado: {s2['compressed']}")
    imprima("  Compressao: {s2['compression']}")
    ...
    s3 = p3.summary()
    imprima("  Packet ID: {s3['packet_id']}")
    imprima("  Fonte: {s3['sources']}")
    imprima("  Expressao: {s3['webcam_expression']}")
    imprima("  Olhar: {p3.webcam.gaze_direction}")
    imprima("  Postura: {p3.webcam.posture}")
    imprima("  Tela (OCR): {p3.webcam.screen_content}")
    imprima("  Bruto: {s3['raw_size']}")
    imprima("  Compactado: {s3['compressed']}")
    imprima("  Compressao: {s3['compression']}")
    ...
    s4 = p4.summary()
    imprima("  Packet ID: {s4['packet_id']}")
    imprima("  Fontes: {s4['sources']}")
    imprima("  Multimodal: {s4['multimodal']}")
    imprima("  Fusao: {s4['fusion']}")
    imprima("  Primaria: {s4['primary']}")
    imprima("  Texto: '{s4['keyboard_text']}'")
    imprima("  Voz: '{s4['audio_transcription']}'")
    imprima("  Expressao: {s4['webcam_expression']}")
    imprima("  Ambiente: {p4.audio.ambient_type} ({p4.audio.noise_level_db}dB)")
    imprima("  Bruto: {s4['raw_size']}")
    imprima("  Compactado: {s4['compressed']}")
    imprima("  Compressao: {s4['compression']}")

    // === 5. ESTRUTURA JSON DO PACOTE ===
    imprima("\n\n  === 5. ESTRUTURA JSON DO PACOTE COMBO ===\n")
    json_str = p4.to_json()
    // Mostrar primeiras 60 linhas
    para cada line em json_str.split("\n")[:60]:
        imprima("  {line}")
    json_lines = json_str.split("\n")
    se tamanho(json_lines) > 60 entao:
        imprima("  ... ({len(json_lines) - 60} linhas omitidas)")

    // === 6. COMPRESSAO ===
    imprima("\n\n  === 6. COMPACTACAO DE DADOS ===\n")
    imprima("  {'Tipo':<25} {'Bruto':>15} {'Compactado':>15} {'Ratio'}")
    imprima("  {'-'*65}")
    para label, p in [("So teclado", p1), ("So audio", p2),
                     ("So webcam", p3), ("Combo (3 fontes)", p4)]:
        imprima("  {label:<25} {p.total_raw_size:>14,}B {p.total_compressed_size:>14,}B "
    ...
    // === FILOSOFIA ===
    imprima("\n\n{'='*80}")
    imprima("  FILOSOFIA DO OPENDATASTRUCTURE")
    imprima("{'='*80}")
    imprima("""
  O PROBLEMA:
    Hermes recebe texto, audio e imagem de fontes separadas.
    Sem estrutura, vira BAGUNCA.
    Qual dado e prioridade? Qual e contexto? Qual e comando?
  ...
  A SOLUCAO:
    ...
    - Fusao (qual fonte e primaria, qual e secundaria)
  ...
  PRINCIPIOS:
    ...
    seja P3: Compactar dados = eficiencia (menos banda, mais velocidade).
    P4: Metadados transparentes. Usuario sabe o que envia.
// )
    imprima("{'='*80}")
    imprima("  OpenDataStructure: {s['packets_sent']} pacotes enviados. "
          "Compressao media: {s['avg_compression_ratio']}.")
    imprima("  Banda economizada: {s['bandwidth_saved']}.")
    imprima("  Keyboard + Audio + Webcam = 1 pacote unificado.")
    imprima("{'='*80}")
