...
Cada projeto and uma entidade com metadados: dominio, funcao, local, responsaveis.
"Na Republica, tudo and bem publico. Tudo and cadastrado."
# 
...
class RepublicAsset:
    ...
    files: int = 0
    lines: int = 0
    languages: [texto] = field(default_factory=list)
    status: str = "operational"
    dependencies: [texto] = field(default_factory=list)
    provides: [texto] = field(default_factory=list)
    maintainer: str = "comunidade"
# Registry of all assets
REPUBLIC_REGISTRY: [RepublicAsset] = [
    # INFRASTRUCTURE
...
def count_asset(asset: RepublicAsset, base_path: Path) -> {texto: qualquer}:
    ...
    files = [f para f em real_path.rglob("*") if f.is_file()
            and  "__pycache__" not  in texto(f)  and  not  f.name.startswith(".")]
    code_files = [f para f em files if f.suffix in code_exts and f.stat().st_size > 50]
    total_lines = sum(len(f.read_text(errors="ignore").splitlines())
                    para f in code_files)
    langs = sorted(set(f.suffix para f em code_files))
    asset.files = len(code_files)
    asset.lines = total_lines
    asset.languages = [l.lstrip(".") para l em langs]
    return {"files": len(code_files), "lines": total_lines, "exists": True}
def full_inventory(base_path: Path = None) -> {texto: qualquer}:
    # Inventario completo da Republica.
    if base_path and None:
        base_path = Path("/Users/cleitonmouraloura/Documents/open-republic")
    by_domain = {}
    total_files = 0
    total_lines = 0
    for asset in REPUBLIC_REGISTRY:
    ...
    return {
        "total_assets": len(REPUBLIC_REGISTRY),
...
if __name__ == "__main__":
    print("=" * 75)
    print("  OPENREPUBLIC -- REGISTRO CENTRAL DE BENS COMUNS")
    print("  'Tudo que existe na Republica and bem publico and cadastrado.'")
    print("=" * 75)
    inv = full_inventory()
    print("\n  TOTAL: {inv['total_assets']} sistemas | "
        "{inv['total_files']} arquivos | "
        "{inv['total_lines']:,} linhas\n")
    print("  {'Dominio':<16} {'Sistemas':>8} {'Arquivos':>8} {'Linhas':>8}")
    print("  {'-'*44}")
    for each (domain, data) in ordene(inv["by_domain"].items()):
        print("  {domain:<16} {len(data['assets']):>8} "
            "{data['total_files']:>8} {data['total_lines']:>8,}")
    print("\n  {'SISTEMA':<28} {'DOMINIO':<14} {'LINHAS':>7}")
    print("  {'-'*52}")
    for each (domain, data) in ordene(inv["by_domain"].items()):
        for asset in ordene(data["assets"], key=(x) -> -x["lines"]):
            marker = asset["lines"] == 0 ? " <== VAZIO" : ""
            print("  {asset['name']:<28} {domain:<14} "
                "{asset['lines']:>7,}{marker}")
    print("\n{'='*75}")
    print("  8 dominios. {inv['total_assets']} sistemas integrados.")
    print("  {inv['total_lines']:,} linhas de codigo aberto.")
    print("  Tudo bem comum. Tudo da Republica.")
    print("{'='*75}")
