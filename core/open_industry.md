# OpenIndustry -- Copiar, Melhorar e Criar Marcas Nacionais Superiores

**Linguagem:** Portugol++ (PPM)

**Arquivo original:** `core/open_industry.py`
...
classe IndustryCategory herda de Enum:
...
classe CopyStatus herda de Enum:
...
classe AdvantageType herda de Enum:
...
classe NationalProduct:
    ...
    // Comparação
    seja foreign_price: texto = "R$ caro"  // preço estrangeiro
    seja national_price: texto = "ZERO"  // preço na República
    seja foreign_quality: inteiro = 8 // 1-10
    seja national_quality: inteiro = 9 // 1-10

    // Vantagens
    seja advantages: [AdvantageType] = field(default_factory=list)
    seja why_better: texto = ""

    // Produção
    seja status: CopyStatus = CopyStatus.IDENTIFIED
    seja fablab_capable: logico = verdadeiro // FabLab nacional produz?
    seja materials_national: flutuante = 0.8 // % de material nacional
    seja production_capacity: texto = ""  // escala

    // Especificações
    seja specs: {texto: texto} = field(default_factory=dict)
...
classe IndustryEngine:
    ...
    POR QUE COPIAR e MELHORAR NÃO É FALTA DE CRIATIVIDADE:
    ...
    funcao list_by_category(self, category: IndustryCategory = None) -> [Dict]:
    ...
    funcao compare(self, product_id: texto) -> {texto: qualquer}:
        // Comparação detalhada: nacional vs estrangeiro.
        p = self.products.get(product_id)
        se nao p entao:
            retorne {"error": "Produto não encontrado"}
    ...
    funcao advance_status(self, product_id: texto) -> {texto: qualquer}:
        // Avança produto para próxima fase.
        p = self.products.get(product_id)
        se nao p entao:
            retorne {"error": "não encontrado"}

        order = list(CopyStatus)
        idx = order.index(p.status)
        se idx + 1 < tamanho(order) entao:
    ...
    funcao dependency_report(self) -> {texto: qualquer}:
        // Relatório de dependência externa (o que ainda importamos).
        by_country = Counter()
        by_category = Counter()
        para cada p em self.products.values():
        ...
        retorne {
            ...
            "message": (
                ...
                "A República já superou {superior} e produz {in_production}. "
                "Meta: SUPERAR TODOS."
            ),
        }
...
se __name__ == "__main__" entao:
    ...
    imprima("=" * 80)
    imprima("  OPENINDUSTRY -- COPIAR, MELHORAR, SUPERAR")
    imprima("  Brasil copia tudo. Cria marcas nacionais. Qualidade SUPERIOR.")
    imprima("=" * 80)

    // === 1. PRODUTOS NACIONAIS ===
    imprima("\n\n  === 1. PRODUTOS NACIONAIS ({len(engine.products)}) ===\n")
    imprima("  {'Nacional':<15} {'Estrangeiro':<25} {'Preço Ext':<18} {'Preço Nac':<10} {'Status'}")
    imprima("  {'-'*85}")
    para cada p em engine.list_by_category():
        imprima("  {p['nacional']:<15} {p['estrangeiro'][:24]:<25} "
    ...
    imprima("  NACIONAL: {comp['nacional']} (qualidade {comp['qualidade_nacional']})")
    imprima("  ESTRANGEIRO: {comp['estrangeiro']} ({comp['empresa_estrangeira']}, {comp['pais']})")
    imprima("  (qualidade {comp['qualidade_estrangeiro']})")
    imprima("\n  PREÇO:")
    imprima("    iPhone: {comp['preco_estrangeiro']}")
    imprima("    OpenPhone: {comp['preco_nacional']}")
    imprima("\n  POR QUE O NACIONAL É SUPERIOR:")
    imprima("    {comp['por_que_melhor']}")
    ...
    imprima("  NACIONAL: {comp2['nacional']}")
    imprima("  ESTRANGEIRO: {comp2['estrangeiro']} ({comp2['pais']})")
    imprima("  PREÇO: {comp2['preco_estrangeiro']} vs {comp2['preco_nacional']}")
    imprima("  POR QUE MELHOR: {comp2['por_que_melhor']}")

    // === 4. COMPARAÇÃO: OpenTrator vs John Deere ===
    imprima("\n\n  === 4. COMPARAÇÃO: OpenTrator vs John Deere ===\n")
    comp3 = engine.compare("IND-TR")
    imprima("  NACIONAL: {comp3['nacional']}")
    imprima("  ESTRANGEIRO: {comp3['estrangeiro']} ({comp3['pais']})")
    imprima("  POR QUE MELHOR: {comp3['por_que_melhor']}")
  ...
  COREIA (1980-2020):
    Copiou eletrônicos. Hoje Samsung > em telas e chips.

  BRASIL (SEMPRE PÔDE):
    Nunca fez por dependência econômica e política.
    A República FAZ. Copia. Melhora. Supera.
...
// )
    ...
    imprima("  Total de produtos: {dep['total_produtos']}")
    imprima("  Já superiores: {dep['ja_superiores']}")
    imprima("  Em produção: {dep['em_producao']}")
    imprima("\n  Dependência por país:")
    para cada (country, count) em dep["dependencia_por_pais"].items():
        imprima("    {country:<15} {count} produtos")
    imprima("\n  Dependência por setor:")
    para cada (cat, count) em dep["dependencia_por_setor"].items():
        imprima("    {cat:<20} {count} produtos")
    ...
    para cada (status, count) em s["status_distribution"].items():
        bar = "#" * count
        imprima("  {status:<25} {count:>2} {bar}")

    // === FILOSOFIA ===
    imprima("\n\n{'='*80}")
    imprima("  FILOSOFIA DO OPENINDUSTRY")
    imprima("{'='*80}")
    imprima("""
  ...
  PRINCÍPIOS:
    ...
    seja P3: Fabricar nacional = trabalho de alto impacto.
    P4: Quais produtos priorizar? Assembleia vota.
// )
    imprima("{'='*80}")
    imprima("  OpenIndustry: {s['total_produtos']} produtos em {s['categorias_cobertas']} setores. "
          "Custo: {s['custo_ao_cidadao']}.")
    imprima("  Copiar. Melhorar. Superar. Tudo nacional. Tudo CC0.")
    imprima("{'='*80}")
