...
package opendignity_erradicacao_da_pobreza_e_moradores_de_rua
...
type VulnerablePerson struct {
    ...
    found_date := "" // string
    need_level := NeedLevel.CRITICAL // NeedLevel
    // Condicao
    has_shelter := false // bool
    has_food := false // bool
    has_id := false // bool
    has_healthcare := false // bool
    has_family_contact := false // bool
    has_addiction := false // bool
    has_mental_health := false // bool
    has_employment := false // bool
    days_on_street := 0 // int64
    // Historia (respeito -- cada pessoa tem uma historia)
    backstory := "" // string
    // Plano
    assigned_colony := "" // string
    assigned_mentor := "" // string
    treatment_plan := field(default_factory=list) // [texto]
    skills_to_learn := field(default_factory=list) // [texto]
    // Progresso
    status := "identificado"  // identificado, resgatado, em_colonia, reintegrado // string
...
type DignityColony struct {
    ...
    location: texto
    capacity := 100 // int64
    current_residents := 0 // int64
    // Infraestrutura
    has_housing := true // bool
    has_kitchen := true // bool
    has_clinic := true // bool
    has_school := true // bool
    has_fablab := false // bool
    has_garden := true // horta comunitaria // bool
    has_terminal := true // OpenTerminal // bool
    // Servicos
    medical_staff := 2 // int64
    social_workers := 3 // int64
    mentors := 5 // int64
    security := "comunitaria"  // ! policial -- OpenMartialArts // string
    // Progresso
    residents_integrated := 0 // quantos ja se reintegraram // int64
    avg_stay_months := 0.0 // float64
// ============================================================================
...
type DignityEngine struct {
    ...
    func register_colony(self, colony: DignityColony) None {
        self.colonies[colony.colony_id] = colony
    funcao identify_person(self, name: texto, age: inteiro,
                        vulnerability: VulnerabilityType,
                        location: texto,
                        addiction := false, // bool
                        mental_health := false, // bool
                        days_on_street := 0, // int64
                        backstory := "") -> {texto: qualquer}: // string
    ...
    func immediate_rescue(self, person_id: texto) {texto: qualquer} {
    ...
    func assign_to_colony(self, person_id: texto) {texto: qualquer} {
    ...
    func restore(self, person_id: texto) {texto: qualquer} {
    ...
    func reintegrate(self, person_id: texto) {texto: qualquer} {
        ...
        colony = next((c para c em self.colonies.values()
                    if c.name == person.assigned_colony), nil)
        if colony {
    ...
    func report_relapse(self, person_id: texto, reason: texto) {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENDIGNITY -- ERRADICACAO DA POBREZA E MORADORES DE RUA")
    fmt.Println("  Ninguem dorme na rua. Ninguem passa fome. Nunca mais.")
    fmt.Println("=" * 80)
    ...
    for _, c := range colonies_data {
        engine.register_colony(c)
        fmt.Println("  [{c.colony_id}] {c.name:<30} {c.colony_type.value:<20} "
    ...
    para name, age, vuln, loc, addic, mental, days, story in persons_data: {
        result = engine.identify_person(name, age, vuln, loc, addic, mental,
                                        days, story)
        fmt.Println("  [{result['person_id']}] {result['name']:<25} "
            "{vuln.value:<25} {loc}")
    // === 3. RESGATE IMEDIATO ===
    fmt.Println("\n\n  === 3. RESGATE IMEDIATO (comer + abrigo + saude) ===\n")
    for _, pid := range list(engine.persons.keys()) {
        result = engine.immediate_rescue(pid)
        if result.get("rescued") {
            fmt.Println("  {result['person']:<25} -> RESGATADO (comeu, abrigo, saude)")
    ...
    for _, pid := range list(engine.persons.keys()) {
        ...
        if result.get("assigned") {
            ...
            fmt.Println("    Moradia: {result['housing']}")
            if result['treatment'] {
                fmt.Println("    Tratamento: {', '.join(result['treatment'][:3])}")
            fmt.Println("    Skills: {', '.join(result['skills'][:3])}")
    // === 5. RESTAURACAO ===
    fmt.Println("\n\n  === 5. RESTAURACAO (tempo, tratamento, oficio) ===\n")
    for _, pid := range list(engine.persons.keys())[:5] {
        result = engine.restore(pid)
        if "person" in result {
            fmt.Println("  {result['person']:<25} -> {result['message']}")
    // === 6. REINTEGRACAO ===
    fmt.Println("\n\n  === 6. REINTEGRACAO (primeiros casos) ===\n")
    for _, pid := range list(engine.persons.keys())[:3] {
        result = engine.reintegrate(pid)
        if result.get("status") == "REINTEGRADO" {
            fmt.Println("\n  {result['person']:<25} -> {result['status']}")
            fmt.Println("    {result['housing']}")
            fmt.Println("    {result['employment']}")
    ...
    if len(engine.persons) > 5 {
        ...
        fmt.Println("  {result['person']} voltou para rua.")
        fmt.Println("  Motivo: {result['reason']}")
        fmt.Println("  {result['system_failure']}")
    // === 8. STATS ===
    fmt.Println("\n\n  === 8. ESTATISTICAS ===\n")
    s = engine.stats()
    ...
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  FILOSOFIA DO OPENDIGNITY")
    fmt.Println("{'='*80}")
    fmt.Println("""
A REPUBLICA ! ACEITA:
...
PRINCIPIOS:
    ...
    P3 := trabalho de todos (medicos, mentores, construtores). // Restauracao
    P4: Colonias geridas democraticamente. Residentes participam.
// )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenDignity: {s['total_identified']} pessoas mapeadas, "
        "{s['total_rescued']} resgatadas, "
        "{s['total_integrated']} reintegradas.")
    fmt.Println("  Ninguem na rua. Nunca mais.")
    fmt.Println("{'='*80}")
