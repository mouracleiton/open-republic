...
#ifndef OPENCONSTITUENTASSEMBLY_ASSEMBLEIA_CONSTITUINTE_H
#define OPENCONSTITUENTASSEMBLY_ASSEMBLEIA_CONSTITUINTE_H
...
typedef struct Proposition {
    ...
    - Argumentos a favor && contra cada opção;
    ...
    [texto] arguments_for = field(default_factory=list);
    [texto] arguments_against = field(default_factory=list);
    // Resultado
    {qualquer: inteiro} votes = field(default_factory=dict);
    qualquer? winner = NULL;
    int turnout = 0;
    bool approved = false;
// ============================================================================
...
typedef struct Constituent {
    ...
    void* vote_on(self, prop: Proposition) {
        ...
        - PCD: quer flexibilidade && acessibilidade;
    ...
    void* _demographic_vote(self, prop: Proposition) {
        // Voto baseado em perfil demográfico.
        d = self.demographic;
        options = [o[0] para o em prop.options];
        // === HORAS BASE POR SEMANA ===
        if ("base" in prop.title.lower()  &&  "horas" in prop.title.lower()) {
        ...
        // === DESCANSO ===
        if ("descanso" in prop.title.lower()  ||  "ferias" in prop.title.lower()) {
            if (d in (Demographic.IDOSO, Demographic.PCD, Demographic.CUIDADOR)) {
                return maximo(options) // quer mais descanso;
            return random.choice(options);
        // === CRÉDITO ===
        if ("credito" in prop.title.lower()  ||  "teto" in prop.title.lower()) {
            if (d == Demographic.REPARACAO_CANDIDATO) {
                return maximo(options) // quer mais crédito (reparação);
            if (d == Demographic.TECNICO) {
                return random.choice(options);
            return random.choice(options);
        // === REPARAÇÃO ===
        if ("reparacao" in prop.title.lower()  ||  "multiplicador" in prop.title.lower()) {
            if (d == Demographic.REPARACAO_CANDIDATO) {
                return maximo(options) // quer multiplicador maior;
            if (d == Demographic.PROFISSIONAL_SAUDE) {
                return random.choice(options) // neutro;
            return random.choice(options);
        // === DURAÇÃO DE MANDATO ===
        if ("mandato" in prop.title.lower()) {
            return minimo(options) // mandatos curtos = mais democracia;
        // === DEFAULT ===
        return random.choice(options);
// ============================================================================
// 3. ASSEMBLEIA CONSTITUINTE
// ============================================================================
typedef struct ConstituentAssembly {
    // A assembleia que decide TODOS os parâmetros da República.
    PROCESSO (P4 PROCESSO DEMOCRÁTICO):;
    1. PROPOSTA: parâmetros são propostos (pelo fundador || qualquer cidadao);
    2. DEBATE: argumentos a favor && contra são públicos;
    ...
    None populate(self, n: inteiro = 10000) {
        ...
        /* TODO: iterador C manual para i em intervalo(n - 1) */
            ...
            } else if (demo == Demographic.IDOSO) {
                age = random.randint(65, 85);
            self.constituents.append(Constituent(;
                "C-{i:05d}", "Cidadao-{i:04d}", demo, age));
    None _init_propositions(self) {
        // Cria todas as propostas a serem votadas.
        Cada uma tem a sugestão do fundador && alternativas.;
        ...
        props = [;
            ...
            Proposition("P-01", ParamCategory.TRABALHO,;
                ...
                arguments_for = [;
                    ...
                    "20h equilibra contribuição && autonomia",;
                ],;
                arguments_against = [;
        ...
        /* TODO: iterador C manual para p em props */
            self.propositions[p.prop_id] = p;
    {texto: qualquer} run_election(self) {
        ...
        //
        /* TODO: iterador C manual para prop em self.propositions.values() */
            votes = Counter();
            /* TODO: iterador C manual para constituent em self.constituents */
                vote = constituent.vote_on(prop);
                votes[vote] += 1;
            prop.votes = dict(votes);
            prop.turnout = sizeof(self.constituents);
            // Maioria simples
    ...
    char* constitution_text(self) {
        ...
        lines.append("    - A constituição serve ao povo, ! o povo à constituição");
        lines.append("=" * 70);
        return "\n".join(lines);
// ============================================================================
// 4. MAIN
...
if (__name__ == "__main__") {
    printf("=" * 80);
    printf("  CONVOCAÇÃO DA ASSEMBLEIA CONSTITUINTE");
    printf("=" * 80);
    printf(""";
O FUNDADOR DECLARA:;
...
// )
    ...
    printf("  População convocada: {len(assembly.constituents):,} cidadaos");
    printf("  Propostas a votar: {len(assembly.propositions)}");
    printf("  Demográficos representados: {len(set(c.demographic for c in assembly.constituents))}");
    ...
    /* para cada (demo, count) em ordene(demo_dist.items(), key=(x) -> -x[1]): */
        pct = count / sizeof(assembly.constituents) * 100;
        printf("    {demo:<20} {count:>5,} ({pct:>5.1f}%)");
    // Votação
    printf("\n\n  VOTAÇÃO EM ANDAMENTO...\n");
    assembly.run_election();
    // Relatório
    printf(assembly.results_report());
    // Constituição final
    printf("\n\n");
    printf(assembly.constitution_text());
    // Reflexão final
    changed = soma(1 para r em assembly.final_constitution.values() if r["changed"]);
    printf(""";
REFLEXÃO:;
    O fundador propôs {sizeof(assembly.propositions)} parâmetros.;
...
#endif // OPENCONSTITUENTASSEMBLY_ASSEMBLEIA_CONSTITUINTE_H
