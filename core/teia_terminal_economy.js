...
funcao simulate_demand(
    const terminal_monthly_brl = 1_500,;
    const training_brl = 2_000,;
    const teia_pct = 0.30,;
    const months = 12,;
    const seed = 42,;
) -> [SimulationResult]:;
    ...
    results = [];
    for (const profile of ResellerProfile) {
        ...
        avg_active = inteiro(base_interested * soma(adoption_curve) / .length(adoption_curve));
...
class ModuleAccess {
    ...
    const sensitive = false // se true, NINGUEM fora da equipe TEIA acessa;
    const teia_only = false // se true, so TEIA direto (! revendedor);
    const description = "";
const MODULES = [;
...
funcao check_access(reseller_id: texto, cert: Certification, module_id: texto,
                const is_teia_direct = false) -> {texto: qualquer}:;
...
class CryptoPaymentAnalysis {
    ...
    //
    const viable = true;
    const stablecoin_recommended = "USDC";
    const reason = (;
...
print_simulation() {
    ...
    for (const cert of Certification) {
        ...
        n_modules = cert.unlocked_modules[0] != "*" ? .length(cert.unlocked_modules) : .length(MODULES);
        lines.append(;
            "{cert.label:<20} ";
    ...
    for (const b of analysis.benefits) {
        lines.append("    + {b}");
    lines.append("");
    lines.append("  RISCOS:");
