...
struct SpamCall {
    ...
    let call_type: SpamCallType = SpamCallType.TELEMARKETING;
    let severity: CallSeverity = CallSeverity.NUISANCE;
    let description: String = "";
    let timestamp: String = "";
    let duration: String = "";
    let victim_id: String = "";
    let blocked: bool = false;
    let reported: bool = true;
    let auto_blocked: bool = false;
// ============================================================================
// 3. MOTOR ANTI-SPAM
// ============================================================================
#[derive(Debug, Clone)]
struct AntiSpamCallEngine {
    ...
    let blocked_numbers: {texto: Dict} = field(default_factory=dict);
    let reported_numbers: {texto: inteiro} = field(default_factory=dict);
    fn __init__(self) {
    ...
    funcao check_incoming(self, phone: texto, caller: texto = "",
                    let is_robotcall: bool = false,;
                    let user_authorized: bool = false;
                    ) -> {texto: qualquer}:;
    ...
    fn consent_list(self) -> {texto: qualquer} {
...
// ============================================================================
fn run_spam_assembly(n_voters: inteiro = 10000) -> {texto: qualquer} {
    votes_ban = 0;
    votes_regulate = 0;
    votes_keep = 0;
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENANTISPAMCALL -- LIGACAO PREDATORIA PROIBIDA");
    println!("  'Para de me encher o saco porra.'");
    println!("=" * 80);
    // === 1. ASSEMBLEIA ===
    ...
    para cada (option, count) em result["votes"].items(): {
        ...
        bar = "#" * inteiro(pct / 2);
        println!("    {option:<25} {count:>6} ({pct:.0f}%) {bar}");
    ...
    para phone, caller, is_robot, authorized in test_calls: {
        ...
        println!("    Motivo: {r['reason']}");
    // === 5. CONSENTIMENTO ===
    println!("\n\n  === 5. SISTEMA DE CONSENTIMENTO ===\n");
    consent = engine.consent_list();
    println!("  Regra: {consent['regra']}");
    println!("  Autorizacao: {consent['autorizacao']}");
    println!("  Revogavel: {consent['revogavel']}");
    println!("  Onde: {consent['onde']}");
    println!("\n  AUTORIZADOS:");
    ...
    para cada (k, v) em s.items(): {
        println!("  {k:<30} {v}");
    println!("\n{'='*80}");
    println!("  OpenAntiSpamCall: {s['numeros_bloqueados']} bloqueados. ";
        "{s['principio']}");
    println!("{'='*80}");
