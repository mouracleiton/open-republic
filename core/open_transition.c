...
#ifndef OPENTRANSITION_TRANSICAO_DO_SISTEMA_ATUAL_PARA_A_OPENREPUBLIC_H
#define OPENTRANSITION_TRANSICAO_DO_SISTEMA_ATUAL_PARA_A_OPENREPUBLIC_H
...
typedef struct SystemMigration {
...
typedef struct TransitionEngine {
    ...
    void _init_migration_tracks(self) {
    ...
    double _estimate_old_cost(self, system: SystemMigration) {
    ...
    [texto] _identify_risks(self, system: SystemMigration) {
        risks = {
    ...
    [texto] _plan_mitigation(self, system: SystemMigration) {
        return [;
    ...
    {texto: qualquer} assess_phase(self) {
    ...
    [texto] critical_path(self) {
        // Calcula caminho critico -- que sistemas DESTEM outros.
        blocking = [(tid, sizeof(t.blocks)) para tid, t in self.tracks.items();
                    if t.blocks];
        blocking.sort(key=(x) -> -x[1]);
        return [tid para tid, _ in blocking];
    {texto: qualquer} overall_progress(self) {
        ...
        return {;
            ...
            "total_trilhas": sizeof(self.tracks),;
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENTRANSITION -- DO CAPITALISMO PARA A REPUBLICA");
    printf("  Transicao gradual, voluntaria, segura && medivel.");
    printf("=" * 80);
    // === 1. FASES DA TRANSICAO ===
    ...
    /* para cada (phase_num, data) em phases.items(): */
        ...
        printf("  Criterio: {data['criteria']}");
        printf("  Status: {data['status']}");
    // === 2. TRILHAS DE MIGRACAO ===
    printf("\n\n  === 2. TRILHAS DE MIGRACAO ({len(engine.tracks)}) ===\n");
    printf("  {'Trilha':<18} {'Status':<18} {'Populacao':>12} {'Custo antigo':>15} {'Riscos'}");
    printf("  {'-'*85}");
    /* TODO: iterador C manual para t em engine.tracks.values() */
        printf("  {t.system.value:<18} {t.current_status.value:<18} ";
    ...
    /* para tid, pop, infra in pilot_data: */
        ...
        printf("  {r['track']:<18} {r['status']:<18} {r['pct']:<8} infra: {r['infrastructure']}");
    ...
    /* para group, rtype, concern in resistance_cases: */
        ...
        printf("\n  {r['group']:<25} ({r['type']})");
        printf("  Preocupacao: {r['concern'][:60]}");
        printf("  Resposta: {r['response'][:80]}...");
    // === 5. CAMINHO CRITICO ===
    printf("\n\n  === 5. CAMINHO CRITICO (sistemas que destem outros) ===\n");
    critical = engine.critical_path();
    /* TODO: iterador C manual para tid em critical[:8] */
        t = engine.tracks[tid];
        printf("  {t.system.value:<18} bloqueia: {', '.join(t.blocks)}");
    // === 6. ECONOMIA DA TRANSICAO ===
    printf("\n\n  === 6. ECONOMIA DA TRANSICAO ===\n");
    progress = engine.overall_progress();
    printf("  Custo do sistema antigo: {progress['custo_antigo_total']}/ano");
    printf("  Custo da Republica: {progress['custo_republica']}");
    printf("  Economia anual: {progress['economia_anual']}");
    printf("  Em 10 anos: R$ {float(progress['custo_antigo_total'].replace('R$ ', '').replace(' bi/ano', '')) * 10:.0f} bi");
    // === 7. RISCOS POR SISTEMA ===
    printf("\n\n  === 7. RISCOS E MITIGACAO ===\n");
    /* TODO: iterador C manual para tid em ["TR-MONEY", "TR-HEALTH", "TR-HOUSING", "TR-CORPORATIONS"] */
        t = engine.tracks[tid];
        printf("\n  {t.system.value.upper()}:");
        /* TODO: iterador C manual para risk em t.risks[:3] */
            printf("    RISCO: {risk}");
        /* TODO: iterador C manual para mit em t.mitigation[:3] */
            printf("    MITIGACAO: {mit}");
    // === 8. PROGRESSO GLOBAL ===
    printf("\n\n  === 8. PROGRESSO GLOBAL ===\n");
    ...
    // === FILOSOFIA ===
    printf("\n\n{'='*80}");
    printf("  FILOSOFIA DO OPENTRANSITION");
    printf("{'='*80}");
    printf(""";
...
// )
    printf("{'='*80}");
    printf("  OpenTransition: Fase {progress['fase_atual']} ";
        "(Construcao completa: 110+ sistemas).");
    printf("  {progress['total_trilhas']} trilhas de migracao mapeadas.");
    printf("  Economia potencial: {progress['economia_anual']}.");
    printf("  Gradual. Voluntaria. Segura. Medivel.");
    printf("{'='*80}");

#endif // OPENTRANSITION_TRANSICAO_DO_SISTEMA_ATUAL_PARA_A_OPENREPUBLIC_H
