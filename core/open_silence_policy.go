...
package opensilencepolicy_alertas_sonoros_de_pressao_proibidos
...
type SoundAlert struct {
    ...
    description := "" // string
    default_sound := ""  // som original // string
    action := AlertAction.DISABLED // AlertAction
    replacement := ""  // o que existe no lugar // string
    pressure_level := 0 // 0-5 (quanto pressiona) // int64
// ============================================================================
...
type SilenceEngine struct {
    ...
    func disable_all_pressure(self) {texto: qualquer} {
        // Desativa TODOS os alertas de pressao.
        disabled = 0
        visual_only = 0
        emergency = 0
        for _, alert := range self.alerts.values() {
    ...
    func user_opt_in(self, user_id: texto, alert_id: texto) {texto: qualquer} {
    ...
    func check_alert(self, user_id: texto, alert_id: texto) {texto: qualquer} {
    ...
    func stats(self) {texto: qualquer} {
        return {
            ...
            "usuarios_opt_in": soma(len(v) para v em self.user_choices.values()),
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENSILENCEPOLICY -- ALERTAS DE PRESSAO DESATIVADOS")
    ...
    fmt.Println("  Som padrao no sistema: {r['som_no_sistema']}")
    // === 2. CATALOGO DE ALERTAS ===
    fmt.Println("\n\n  === 2. CATALOGO ({len(engine.alerts)} alertas) ===\n")
    current_purpose = nil
    for _, alert := range engine.alerts.values() {
        ...
        fmt.Println("  [{action_icon:<10}] {alert.name[:45]:<46} pressao:{alert.pressure_level}")
    // === 3. O QUE SUBSTITUI ===
    fmt.Println("\n\n  === 3. O QUE EXISTE NO LUGAR ===\n")
    replacements = engine.what_replaces_sounds()
    for _, r := range replacements {
        fmt.Println("\n  {r['categoria'].upper()}:")
        fmt.Println("    Antes:  {r['antes']}")
        fmt.Println("    Depois: {r['depois']}")
    // === 4. USUARIO OPT-IN ===
    fmt.Println("\n\n  === 4. USUARIO PODE OPTAR (se quiser) ===\n")
    opt = engine.user_opt_in("cleiton", "AL-H01")
    fmt.Println("  {opt['message']}")
    fmt.Println("  (Pode revogar a qualquer momento. Padrao = desativado.)")
    // === 5. HONRA AUSENCIA ===
    fmt.Println("\n\n  === 5. HONRA OPENSABSENCE ===\n")
    engine.absent_users.append("cleiton")
    check = engine.check_alert("cleiton", "AL-W01")
    fmt.Println("  Cleiton (ausente): {check['action']}")
    fmt.Println("  {check.get('message', check.get('reason', ''))}")
    // === 6. STATS ===
    fmt.Println("\n\n  === 6. ESTATISTICAS ===\n")
    s = engine.stats()
    para cada (k, v) em s.items(): {
        fmt.Println("  {k:<35} {v}")
    fmt.Println("\n{'='*80}")
    fmt.Println("  OpenSilencePolicy: {s['desativados']} desativados, "
