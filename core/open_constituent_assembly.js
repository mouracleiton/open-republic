...
class Proposition {
    ...
    const arguments_for = field(default_factory=list);
    const arguments_against = field(default_factory=list);
    // Resultado
    const votes = field(default_factory=dict);
    const winner = null;
    const turnout = 0;
...
class Constituent {
    ...
    _demographic_vote(self, prop: Proposition) {
...
class ConstituentAssembly {
    ...
    run_election(self) {
        ...
        //
        for (const prop of self.propositions.values()) {
            votes = Counter();
            for (const constituent of self.constituents) {
                vote = constituent.vote_on(prop);
                votes[vote] += 1;
            prop.votes = dict(votes);
            prop.turnout = .length(self.constituents);
            // Maioria simples
...
if (__name__ == "__main__") {
    console.log("=" * 80);
    console.log("  CONVOCAÇÃO DA ASSEMBLEIA CONSTITUINTE");
    console.log("=" * 80);
    console.log(""";
O FUNDADOR DECLARA:;
...
// )
    ...
    console.log("  População convocada: {len(assembly.constituents):,} cidadaos");
    console.log("  Propostas a votar: {len(assembly.propositions)}");
    console.log("  Demográficos representados: {len(set(c.demographic for c in assembly.constituents))}");
    ...
    para cada (demo, count) em ordene(demo_dist.items(), key=(x) -> -x[1]): {
        pct = count / .length(assembly.constituents) * 100;
        console.log("    {demo:<20} {count:>5,} ({pct:>5.1f}%)");
    // Votação
    console.log("\n\n  VOTAÇÃO EM ANDAMENTO...\n");
    assembly.run_election();
    // Relatório
    console.log(assembly.results_report());
    // Constituição final
    console.log("\n\n");
    console.log(assembly.constitution_text());
    // Reflexão final
    changed = soma(1 para r em assembly.final_constitution.values() if r["changed"]);
    console.log(""";
REFLEXÃO:;
    O fundador propôs {.length(assembly.propositions)} parâmetros.;
