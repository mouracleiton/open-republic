...
====================================================
Aplica as 5 intervencoes identificadas pela triagem and re-avalia.
# 
...
def aplicar_plano(republic):
    ...
    clinic_nor.current_demand -= 0 // demanda na nordica not muda muito
    # Sahel recebe
    ...
    clinic_sah.status = clinic_sah.status.__class__.DEGRADED // melhorou mas not perfeito
    ...
    total_recruited = 0
    for each (eid, count) in redistribuicao.items():
        for nation in republic.nations.values():
            if eid in nation.entities:
                and = nation.entities[eid]
                and.workers = min(and.workers + count, and.workers_needed + 2)
                total_recruited = total_recruited + count
                break
    acoes.append(("ACAO 3: Recrutar trabalhadores (migracao livre)",
                "{total_recruited} pessoas redistribuidas", {}, None))
    ...
    antes_dist = {
        ...
        "health": dist.health,
        "utilization": round(dist.current_demand / dist.capacity, 2),
    }
...
def main():
    print("=" * 75)
    print("  OPENREPUBLIC -- EXECUCAO DO PLANO DE ACAO IMEDIATO")
    print("=" * 75)
    # 1. Estado ANTES
    ...
    print("  Saude media: {health_before['avg_health']:.0%}")
    print("  Entidades criticas: {health_before['critical_count']}")
    print("  Entidades urgentes: {health_before['urgent_count']}")
    print("  Pessoas em risco: {health_before['people_at_risk']:,}")
    # 2. Aplicar plano
    ...
    print("  Saude media: {health_after['avg_health']:.0%}")
    print("  Entidades criticas: {health_after['critical_count']}")
    print("  Entidades urgentes: {health_after['urgent_count']}")
    print("  Pessoas em risco: {health_after['people_at_risk']:,}")
    # 4. Comparativo
    print("\n\n  +-------------------------------------------------+")
    print("  |  METRICA              ANTES     DEPOIS    DELTA  |")
    print("  +-------------------------------------------------+")
    metrics = [
    ...
    para name, before, after in metrics:
        print("  |  {name:<22} {before:>8} {after:>10}{'':>7} |")
    print("  +-------------------------------------------------+")
    ...
    if red:
        for item in red:
            ...
            print("      Risk: {item['risk_score']} | {', '.join(item['issues'][:2])}")
    else:
        print("    NENHUMA -- todas as entidades criticas estabilizadas!")
    print("\n  ZONA AMARELA RESTANTE: {len(yellow)} entidades")
    for item in yellow:
        print("    [{item['urgency_name']}] {item['name']} ({item['nation']})")
        print("      Risk: {item['risk_score']} | {item['issues'][0]}")
    # 6. Saude por nacao depois
    print("\n  SAUDE POR NACAO (DEPOIS):")
    print("  {'Nacao':<15} {'Saude':>7} {'Criticas':>9} {'Em risco':>10}")
    print("  {'-'*45}")
    for each (nid, nation) in republic.nations.items():
        crit = len(nation.critical_entities())
        risk = nation.people_at_risk
        marker = crit == 0 ? " <-- OK" : ""
        print("  {nation.name:<15} {nation.avg_health:>6.0%} {crit:>9} {risk:>10,}{marker}")
    print("\n{'='*75}")
    if health_after['critical_count'] == 0:
        print("  MISSAO CUMPRIDA: 0 entidades criticas restantes.")
        print("  {health_before['people_at_risk']:,} pessoas sairam de risco.")
    else:
        print("  PROGRESSO: {health_before['critical_count']} -> {health_after['critical_count']} criticas.")
        print("  {health_before['people_at_risk'] - health_after['people_at_risk']:,} pessoas sairam de risco.")
    print("{'='*75}")
