...
package openlegocode_programacao_modular_em_formato_lego
...
type LegoBlock struct {
    ...
    description := "" // string
    // Conectores
    inputs := field(default_factory=list) // [LegoInput]
    outputs := field(default_factory=list) // [LegoOutput]
    // Execucao (funcao real ou descricao para IA gerar)
    logic := ""  // descricao do que faz (IA implementa) // string
    code_rust := ""  // codigo Rust gerado (producao) // string
    // Metadata
    author := "" // string
    version := "1.0.0" // string
    tested := false // bool
    uses := 0 // quantas cadeias usam este bloco // int64
    color := "blue"  // cor visual do bloco (categoria) // string
    funcao can_connect(self, other: "LegoBlock",
                    my_output_idx := 0, // int64
                    their_input_idx := 0) -> logico: // int64
...
type LegoProgram struct {
    ...
    name: texto
    description := "" // string
    blocks := field(default_factory=dict) // {texto: LegoBlock}
    connections := field(default_factory=list) // [LegoConnection]
    created_by := "" // string
    created_date := "" // string
    // decorador: @property
    func block_count(self) int64 {
        return len(self.blocks)
    // decorador: @property
    func connection_count(self) int64 {
    ...
    // decorador: @property
    func all_inputs_connected(self) bool {
...
// ============================================================================
func build_block_library() {texto: LegoBlock} {
...
type LegoCodeEngine struct {
    ...
    func list_blocks(self, category: texto = None) [Dict] {
    ...
    funcao add_block(self, program_id: texto, block_id: texto,
                instance_name := "") -> {texto: qualquer}: // string
    ...
    func validate_program(self, program_id: texto) {texto: qualquer} {
    ...
    func _has_cycle(self, prog: LegoProgram) bool {
    ...
    funcao execute(self, program_id: texto,
                initial_values := nil) -> {texto: qualquer}: // {texto: qualquer}
    ...
    func _topo_sort(self, prog: LegoProgram) [texto] {
        // Ordenacao topologica dos blocos.
        graph = defaultdict(list)
        in_degree = defaultdict(inteiro)
        for _, inst_id := range prog.blocks {
            in_degree[inst_id] = 0
        for _, conn := range prog.connections {
            graph[conn.from_block].append(conn.to_block)
        ...
        return order
    func generate_rust(self, program_id: texto) {texto: qualquer} {
    ...
    func ai_generate_block(self, description: texto) {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENLEGOCODE -- PROGRAMACAO MODULAR EM FORMATO LEGO")
    fmt.Println("  Escrever codigo && coisa do passado. Programa-se ENCAIXANDO.")
    fmt.Println("=" * 80)
    ...
    para cada (cat, count) em ordene(cats.items(), key=(x) -> -x[1]): {
        ...
        for _, b := range blocks_in_cat {
            in_types = ", ".join("{i.name}:{i.pin_type.value}" para i em b.inputs)
            out_types = ", ".join("{o.name}:{o.pin_type.value}" para o em b.outputs)
            fmt.Println("    [{b.block_id}] {b.name}")
            fmt.Println("      Entradas: ({in_types or 'nenhuma'})")
            fmt.Println("      Saidas: ({out_types or 'nenhuma'})")
    ...
    fmt.Println("  Blocos adicionados: {engine.programs[pid].block_count}")
    ...
    para from_inst, from_out, to_inst, to_in in connections: {
        ...
        fmt.Println("  [{status}] {r.get('from', '?')} -> {r.get('to', '?')}")
    // Validar
    fmt.Println("\n  Validacao:")
    val = engine.validate_program(pid)
    fmt.Println("  Valido: {'SIM' if val['valid'] else 'NAO'}")
    fmt.Println("  Blocos: {val['blocks']}, Conexoes: {val['connections']}")
    if val["issues"] {
        fmt.Println("  Issues: {val['issues']}")
    ...
    fmt.Println("  Valido: {'SIM' if val2['valid'] else 'NAO'}")
    fmt.Println("  Blocos: {val2['blocks']}, Conexoes: {val2['connections']}")
    ...
    for _, desc := range ai_blocks {
        r = engine.ai_generate_block(desc)
        fmt.Println("  [{r['block_id']}] {r['name']}")
        fmt.Println("    Logica: {r['logic'][:60]}...")
        fmt.Println("    Saida: {r['output_type']}")
    ...
    fmt.Println("  Tentativa: texto -> soma")
    fmt.Println("  Resultado: {r.get('error', 'conectado')}")
    fmt.Println("  Mensagem: {r.get('message', '')}")
    fmt.Println("\n  LEGO: pino texto NAO encaixa em buraco numero.")
    fmt.Println("  Sistema IMPEDE erro de tipo antes de executar.")
    // === 6. GERAR CODIGO RUST ===
    fmt.Println("\n\n  === 6. GERAR CODIGO RUST DO PROGRAMA ===\n")
    rust = engine.generate_rust(pid)
    fmt.Println("  Programa: {rust['program']}")
    fmt.Println("  Linguagem: {rust['language']}")
    fmt.Println("  Linhas: {rust['lines']}")
    for _, line := range rust["code"].split("\n")[:10] {
        fmt.Println("  {line}")
    // === 7. STATS ===
    fmt.Println("\n\n  === 7. ESTATISTICAS ===\n")
    s = engine.stats()
    para cada (k, v) em s.items(): {
        if isinstance(v, dict) {
            ...
            para cada (sk, sv) em v.items(): {
                fmt.Println("    {sk:<20} {sv}")
        } else {
            fmt.Println("  {k:<30} {v}")
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  FILOSOFIA: PROGRAMACAO EM LEGO")
    fmt.Println("{'='*80}")
    fmt.Println("""
ESCREVER CODIGO && COISA DO PASSADO:
...
// )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenLegoCode: {s['total_blocos_biblioteca']} blocos na biblioteca, "
        "{s['total_programas']} programas montados.")
    fmt.Println("  Escrever codigo && coisa do passado. Encaixa pecas.")
    fmt.Println("{'='*80}")
