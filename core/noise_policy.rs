...
let NOISE_LIMITS: Dict[(SoundZone, TimeWindow), inteiro] = {;
...
struct NoiseEvent {
    ...
    source: NoiseSource;
    let duration_min: f64 = 0;
    let location: String = "";
    let reporter: String = "";
    let timestamp: f64 = 0;
    let limit_db: i64 = 0;
    let exceeded_by: f64 = 0;
    let severity: NoiseSeverity = NoiseSeverity.NONE;
#[derive(Debug, Clone)]
struct NoiseMonitor {
    ...
    funcao measure(self, db_level: flutuante, zone: SoundZone,
                ...
                source: NoiseSource,;
                let location: String = "",;
                let duration_min: f64 = 0) -> NoiseEvent:;
        ...
        return event;
    fn resolve(self, event: NoiseEvent) -> {texto: qualquer} {
    ...
    fn zone_report(self, zone: SoundZone) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    println!("=" * 75);
    println!("  OPENREPUBLIC -- POLITICA DE POLUICAO SONORA");
    println!("  'Som && vida. Ruido && violencia invisivel.'");
    println!("=" * 75);
    monitor = NoiseMonitor();
    // === Limits Table ===
    println!("\n\n  === LIMITES DE RUIDO dB(A) ===\n");
    println!("  {'Zona':<15} {'Noite':>6} {'Amanh.':>7} {'Dia':>6} {'Anoit.':>7}");
    println!("  {'-'*45}");
    for zone in SoundZone {
        ...
        println!("  {zone.value:<15} {night:>6} {dawn:>7} {day:>6} {eve:>7}");
    ...
    para label, db, zone, tw, source, loc in scenarios: {
        ...
        println!("\n  {label}:");
        println!("    Medido: {event.measured_db:.0f}dB | Limite: {event.limit_db}dB ";
            "| Excesso: +{event.exceeded_by:.0f}dB{flag}");
        println!("    Acao: {resolution['action']}");
        println!("    {resolution['reason']}");
    // === Zone Report ===
    println!("\n\n  === RELATORIO POR ZONA ===\n");
    for zone in SoundZone {
        rep = monitor.zone_report(zone);
        println!("  {rep['zone']:<15} eventos={rep['total_events']} ";
