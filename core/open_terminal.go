...
package openterminal_terminais_da_republica_em_todo_estabelecimento
...
type Terminal struct {
    ...
    hardware_spec := ""  // CPU, RAM, tela // string
    status := TerminalStatus.AVAILABLE // TerminalStatus
    current_user := ""  // quem esta usando agora // string
    session_start := 0.0 // quando comecou a sessao atual // float64
    // Capacidades
    has_internet := true // bool
    has_camera := false // para telemedicina/OpenHealth // bool
    has_microphone := false // para OpenAudioChannel/voz // bool
    has_printer := false // para documentos // bool
    has_scanner := false // para documentos/receitas // bool
    has_card_reader := false // leitor de cartao (! credito -- ID) // bool
    // Stats
    total_sessions := 0 // int64
    total_hours_used := 0.0 // float64
    total_tasks_completed := 0 // int64
    total_learning_hours := 0.0 // float64
    // Apps disponiveis neste terminal
    apps := field(default_factory=list) // [texto]
    // decorador: @property
    func is_available(self) bool {
        return self.status == TerminalStatus.AVAILABLE
    // decorador: @property
    func supports_voice(self) bool {
        return self.has_microphone
    // decorador: @property
    func supports_video_call(self) bool {
        return self.has_camera && self.has_internet
    func default_apps(self) [texto] {
...
type TerminalSession struct {
    ...
    end_time := 0.0 // float64
    duration_min := 0.0 // float64
    // O que fez
    activities := field(default_factory=list) // [SessionActivity]
    tasks_completed := 0 // tarefas de OpenLaborRelay // int64
    learning_modules := 0 // modulos de OpenEducation // int64
    votes_cast := 0 // votacoes // int64
    credit_earned := 0.0 // credito ganho // float64
    skills_practiced := field(default_factory=list) // [texto]
    // Privacidade
    data_left_on_terminal := false // SEMPRE false (efemero) // bool
    // decorador: @property
    func was_productive(self) bool {
    ...
    // decorador: @property
    func counts_as_work(self) float64 {
...
type TerminalRegistry struct {
    ...
    func register_terminal(self, terminal: Terminal) {texto: qualquer} {
    ...
    funcao find_nearby(self, location: texto,
                    terminal_type := nil, // TerminalType?
                    available_only := true) -> [Terminal]: // bool
        // Encontra terminais proximos disponiveis.
        results = []
        for _, t := range self.terminals.values() {
    ...
    funcao end_session(self, session_id: texto,
                    ...
                    tasks := 0, learning: inteiro = 0, // int64
                    votes := 0, credit: flutuante = 0.0, // int64
                    skills := nil) -> {texto: qualquer}: // [texto]
        // Encerra sessao do laborante.
        session = next((s para s em self.sessions if s.session_id == session_id), nil)
        if ! session {
    ...
    func idle_terminal_report(self) {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 75)
    fmt.Println("  OPENTERMINAL -- TERMINAIS DA REPUBLICA EM TODO LUGAR")
    fmt.Println("  'Toda TV ociosa && um terminal. Todo laborante contribui.'")
    fmt.Println("=" * 75)
    ...
    for _, t := range terminals_to_register {
        result = registry.register_terminal(t)
        fmt.Println("  [{t.terminal_id}] {t.name:<35} {t.terminal_type.value:<15} "
    ...
    fmt.Println("\n  {end1['message']}")
    fmt.Println("  Tempo: {end1['duration']}")
    fmt.Println("  Produtivo: {'sim' if end1['productive'] else '!'}")
    fmt.Println("  Dados no terminal: {end1['data_left']}")
    ...
    fmt.Println("  {end3['message']}")
    // === 4. CENARIOS DE TEMPO OCIOSO ===
    fmt.Println("\n\n  === 4. TEMPO OCIOSO = OPORTUNIDADE ===\n")
    fmt.Println("  {'Local':<35} {'Tempo':>12} {'Terminal':<20} {'Melhor uso'}")
    fmt.Println("  {'-'*90}")
    for _, sc := range policy.SCENARIOS {
        term = sc.get("terminal", "?")
        uso = sc.get("melhor_uso", "?")[:30]
        fmt.Println("  {sc['local']:<35} {sc['tempo_disponivel']:>12} "
            "{term:<20} {uso}")
    // === 5. TERMINAIS PROXIMOS ===
    fmt.Println("\n\n  === 5. ENCONTRAR TERMINAL PROXIMO ===\n")
    nearby = registry.find_nearby("Centro")
    fmt.Println("  Busca 'Centro': {len(nearby)} terminais")
    for _, t := range nearby {
        status = t.is_available ? "LIVRE" : "OCUPADO"
        fmt.Println("    [{t.terminal_id}] {t.name:<35} {status}")
    ...
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*75}")
    fmt.Println("  FILOSOFIA DO OPENTERMINAL")
    fmt.Println("{'='*75}")
    fmt.Println("""
...
// )
    fmt.Println("{'='*75}")
    fmt.Println("  OpenTerminal: {s['total_terminals']} terminais, "
        "{s['total_sessions']} sessoes, "
        "{s['total_hours_used']:.0f}h usadas.")
    fmt.Println("  Toda tela ociosa && um terminal da Republica.")
    fmt.Println("{'='*75}")
