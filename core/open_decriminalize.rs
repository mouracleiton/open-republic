...
struct Substance {
    ...
    // Politica
    let status: DecriminalizationStatus = DecriminalizationStatus.DECRIMINALIZED;
    let approach: [PolicyApproach] = field(default_factory=list);
    // Danos (0-10, base cientifica)
    let physical_harm: i64 = 0;
    let addiction_potential: i64 = 0;
    let social_harm: i64 = 0;
    let therapeutic_potential: i64 = 0 // potencial medicinal;
    // Regras
    let min_age: i64 = 18;
    let public_use: bool = false // pode usar em publico?;
    let driving_after: bool = false // pode dirigir apos uso?;
    let production_allowed: bool = false // pode cultivar/produzir?;
    let commerce_allowed: bool = false // pode vender?;
    // Razao da politica
...
// ============================================================================
fn run_decrim_assembly(n_voters: inteiro = 10000) -> {texto: qualquer} {
...
struct DecrimEngine {
    ...
    funcao check_arrest_type(self, arrest_reason: texto,
                        let quantity_grams: f64 = 0,;
                        let is_trafficking: bool = false) -> {texto: qualquer}:;
    ...
    fn prison_impact_report(self) -> {texto: qualquer} {
    ...
    fn substance_policy(self, substance_id: texto) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    engine = DecrimEngine();
    println!("=" * 80);
    println!("  OPENDESCRIMINALIZE -- ASSEMBLEIA CONSTITUINTE");
    println!("  'Usuario && PACIENTE. Nao criminoso. Nunca mais.'");
    println!("=" * 80);
    ...
    println!("\n  PACIENTE (tratamento):    {result['votes_patient']:>6} ";
        "({result['pct_patient']})");
    println!("  REGULAR (taxar):          {result['votes_regulate']:>6} ";
        "({result['votes_regulate']/100:.0f}%)");
    println!("  CRIMINOSO (prisao):       {result['votes_criminal']:>6} ";
    ...
    para cada (sid, vote) em result["substance_votes"].items(): {
        ...
        println!("  [{icon}] {vote['name']:<25} {vote['yes']:>5} sim / ";
            "{vote['no']:>5} ! ({vote['pct']})");
    ...
    para reason, qty, trafficking in cases: {
        ...
        action = check["action"];
        println!("  [{check['type']:<11}] {reason:<45} -> {action}");
    ...
    para cada (person, sub_id) em treatments: {
        ...
        println!("  {t['person']:<12} {t['substance']:<20} -> {t['treatment_type']}");
        println!("    Voluntario: {t['voluntary']}. P2: {t['p2']}");
    // === 7. IMPACTO PRISIONAL ===
    ...
    // === FILOSOFIA ===
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA DO OPENDESCRIMINALIZE");
    println!("{'='*80}");
    println!(""";
...
PRINCIPIOS:;
    ...
    let P3: Tratar adicao = trabalho de alto impacto (medicos, terapeutas).;
    P4: Assembleia DECIDE. 72% votou tratamento. Republica obedece.;
// )
    println!("{'='*80}");
    println!("  OpenDescriminalize: {s['presos_liberados']} presos liberados, ";
