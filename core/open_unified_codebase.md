...
**Arquivo original:** `open-republic/core/open_unified_codebase.py`
...
classe LinguagemAlvo herda de Enum:
    ...
    // decorador: @property
    funcao e_fonte(self) retorna bool:
        retorne self == LinguagemAlvo.PYTHON


classe TipoHardware herda de Enum:
...
classe StatusCompatibilidade herda de Enum:
...
classe CanalUpdate herda de Enum:
...
classe TipoArquivo herda de Enum:
...
// decorador: @dataclass
classe ModuloCodebase:
    // Um modulo do codigo-fonte da Republica.
    id: str
    nome: str
    arquivo_fonte: str         // "core/open_energy.py"
    declare artefatos_gerados: List[str]  <- field(default_factory=list)  // .c .go .java .js .rs .md
    declare linhas_fonte: int  <- 0
    declare compatibilidade: Dict[str, StatusCompatibilidade]  <- field(default_factory=dict)
    declare ultima_transpilacao: str  <- ""  // timestamp
    declare commit_fonte: str  <- ""  // hash do ultimo commit do .py


// decorador: @dataclass
classe VersaoUnificada:
    // Uma versao do codigo da Republica (um commit do .py = uma versao).
    versao: str               // "2026.07.26-r1"
    canal: CanalUpdate
    commit_hash: str
    declare modulos_atualizados: List[str]  <- field(default_factory=list)
    declare artefatos_gerados: int  <- 0
    declare hardware_compativel: List[TipoHardware]  <- field(default_factory=list)
    declare timestamp: str  <- ""
    declare notas: str  <- ""


// decorador: @dataclass
classe PoliticaTranspilacao:
    // Regras do transpilador automatico.
    declare fonte_editavel: bool  <- VERDADEIRO  // so .py e editado
    declare artefatos_no_git: bool  <- FALSO  // artefatos NAO entram no git (CI gera)
    declare verificacao_pos_geracao: bool  <- VERDADEIRO  // testar apos transpilar
    declare linguagens_alvo: List[LinguagemAlvo]  <- field(default_factory=list)
    declare min_linhas_artefato: int  <- 0  // minimo de linhas (anti-abreviacao)


// decorador: @dataclass
classe MatrizCompatibilidade:
    // Qual modulo roda em qual hardware.
    modulo_id: str
    hardware: TipoHardware
    status: StatusCompatibilidade
    declare adaptacoes_necessarias: List[str]  <- field(default_factory=list)
    declare notas: str  <- ""
...
declare REQUISITOS_HARDWARE: Dict[str, Dict[str, str]]  <- {
    "risc_v_avancado": {
...
classe UnifiedCodebaseEngine:
    ...
    funcao __init__(self) retorna None:
        ...
        self.politica: PoliticaTranspilacao = PoliticaTranspilacao(
            fonte_editavel <- VERDADEIRO,
            artefatos_no_git <- FALSO,
            verificacao_pos_geracao <- VERDADEIRO,
            linguagens_alvo <- [l for l in LinguagemAlvo if NAO  l.e_fonte],
        )
        self.matriz: List[MatrizCompatibilidade] = []
        self._mod_id = 0
        self._ver_counter = 0

    funcao _mod_novo_id(self) retorna str:
        self._mod_id += 1
        retorne f"MOD-{self._mod_id:04d}"
    ...
    def registrar_modulo(
        ...
        arquivo_fonte: str,
        declare linhas_fonte: int  <- 0,
        declare compatibilidade: Optional[Dict[str, StatusCompatibilidade]]  <- nulo,
    ) -> ModuloCodebase:
        ...
        m <- ModuloCodebase(
            ...
            compatibilidade <- compatibilidade  OU  {},
            artefatos_gerados <- self._gerar_lista_artefatos(arquivo_fonte),
        )
        self.modulos[m.id] = m
        retorne m

    funcao _gerar_lista_artefatos(self, fonte: str) retorna List[str]:
        // Gera lista de artefatos a partir do nome do arquivo-fonte.
        base <- fonte.replace(".py", "")
        extensoes <- [".c", ".go", ".java", ".js", ".rs", ".md"]
        retorne [base + ext for ext in extensoes]

    // -- transpilacao ------------------------------------------------------

    funcao transpilar_modulo(self, mod_id: str) retorna Dict[str, Any]:
        // 
        ...
        se m e nulo entao:
            retorne {"erro": "Modulo nao encontrado"}
        artefatos_ok <- 0
        artefatos_falha <- 0
        declare detalhes: List[str]  <- []
        para cada artefato em m.artefatos_gerados:
    ...
    funcao transpilar_todos(self) retorna List[Dict[str, Any]]:
        // Transpila TODOS os modulos registrados.
        retorne [self.transpilar_modulo(mid) for mid in self.modulos]

    // -- matriz de compatibilidade ----------------------------------------

    funcao construir_matriz_compatibilidade(self) retorna List[MatrizCompatibilidade]:
        // Para cada modulo x hardware, determina compatibilidade.
        self.matriz.clear()
        para cada mod em self.modulos.values():
            para cada hw em TipoHardware:
    ...
    funcao _avaliar_compatibilidade(self, mod: ModuloCodebase, hw: TipoHardware) retorna StatusCompatibilidade:
        // 
        ...
        se hw.id in mod.compatibilidade entao:
            retorne mod.compatibilidade[hw.id]
        // heuristicas
        nome <- mod.nome.lower()
        se any(kw in nome for kw in ["firmware", "mcu", "bluetooth"]) entao:
            se hw.id in ("headphone_bt", "switch", "braille_display", "gps_tracker") entao:
                retorne StatusCompatibilidade.COMPATIVEL
            retorne StatusCompatibilidade.NAO_APLICAVEL
        se any(kw in nome for kw in ["ia", "modelo", "gpu", "neural"]) entao:
            if hw.id in ("risc_v_essencial", "smartwatch", "headphone_bt",
                         "braille_display", "switch", "e_reader", "gps_tracker"):
                retorne StatusCompatibilidade.INCOMPATIVEL
            retorne StatusCompatibilidade.COMPATIVEL
        // logica pura: roda em tudo que tem runtime
        se hw.id in ("headphone_bt", "switch", "braille_display", "gps_tracker") entao:
            retorne StatusCompatibilidade.PORTAVEL  // MCU precisa recompilar
        retorne StatusCompatibilidade.COMPATIVEL

    funcao modulos_por_hardware(self, hw: TipoHardware) retorna Dict[StatusCompatibilidade, List[str]]:
        // Quais modulos rodam em qual hardware.
        declare resultado: Dict[StatusCompatibilidade, List[str]]  <- defaultdict(list)
        para cada mc em self.matriz:
            se mc.hardware == hw entao:
    ...
    def publicar_versao(
        ...
        total_artefatos <- 0
        para cada mid em modulos_atualizados:
            m <- self.modulos.get(mid)
            se m entao:
                total_artefatos <- total_artefatos + len(m.artefatos_gerados)
        v <- VersaoUnificada(
            versao <- versao,
            canal <- canal,
    ...
    funcao ultima_versao_estavel(self) retorna Optional[VersaoUnificada]:
        para cada v em reversed(self.versoes):
            se v.canal == CanalUpdate.ESTAVEL entao:
                retorne v
        retorne nulo

    // -- regras do git -----------------------------------------------------

    funcao regras_git(self) retorna List[str]:
    ...
    funcao manifesto_unificado(self) retorna str:
        retorne (
    ...
    funcao scorecard(self) retorna Dict[str, Any]:
        ...
        retorne {
            ...
            "artefatos_gitignored": self.politica.artefatos_no_git is FALSO,
        }
...
funcao _demo() retorna None:
    ...
    for mid in list(e.modulos.keys())[:3]:   // so 3 para nao poluir
        ...
        para cada d em resultado["detalhes"]:
            print(f"  {d}")
    ...
    print(f"\n  {'Modulo':.<30} ", end="")
    para cada hw em hw_list[:6]:
        print(f" {hw.id[:8]:>10}", end="")
    print(f" {'...':>10}")
    print(f"  {'-'*100}")
    para cada mod em list(e.modulos.values())[:4]:
        print(f"  {mod.nome[:28]:.<30} ", end="")
        para cada hw em hw_list[:6]:
            mc <- next((m for m in matriz if m.modulo_id == mod.id  E  m.hardware == hw), nulo)
            se mc entao:
    ...
    para cada (hw_id, reqs) em REQUISITOS_HARDWARE.items():
        hw <- next(h for h in TipoHardware if h.id == hw_id)
        print(f"\n  {hw.rotulo}:")
        para cada (k, v) em reqs.items():
            print(f"    {k}: {v}")

    // --- Regras do Git ---
    print("\n[REGRAS DO GIT -- O que entra e o que nao entra]")
    para cada regra em e.regras_git():
        print(f"  {regra}")
