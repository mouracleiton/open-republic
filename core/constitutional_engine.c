...
#ifndef OPENREPUBLIC_SISTEMA_CONSTITUCIONAL_REPRESENTANTES_POR_SISTEMA_H
#define OPENREPUBLIC_SISTEMA_CONSTITUCIONAL_REPRESENTANTES_POR_SISTEMA_H
...
// ============================================================================
typedef struct ConstitutionalPrinciple {
...
typedef struct ViolationSeverity {
...
// decorador: @dataclass
typedef struct ConstitutionalViolation {
    ...
    int detected_at_cycle = 0;
// decorador: @dataclass
typedef struct ComplianceCheck {
...
// ============================================================================
typedef struct ConstitutionalEngine {
    ...
    funcao register_system(self, system_id: texto, name: texto,
                        ...
                        char* created_by = "comunidade",;
                        bool approved_by_vote = false,;
                        int vote_count = 0,;
                        bool affects_bodies = false,;
                        bool collects_data = false,;
                        bool opaque_decisions = false,;
                        bool single_point_of_failure = false,;
                        bool requires_bodily_consent = false,;
                        bool labor_base_respected = true,;
                        bool has_successor = false,;
                        bool public_docs = false,;
    ...
    {texto: qualquer} validate_system(self, system_id: texto) {
        ...
        [ConstitutionalViolation] violations = [];
        // === P1: ANTI-ELITISMO ===
        ...
        avg_score = soma(c.score para c em checks) / sizeof(checks);
        ...
        result = {
            ...
            "violations_count": sizeof(violations),;
        ...
        return result;
    {texto: qualquer} validate_all(self) {
        // Valida TODOS os sistemas registrados.
        results = {};
        /* TODO: iterador C manual para sid em self.systems_registry */
    ...
    ComplianceCheck _check_anti_elitism(self, sys_data: Dict) {
    ...
    ComplianceCheck _check_bodily_autonomy(self, sys_data: Dict) {
    ...
    ComplianceCheck _check_equal_labor(self, sys_data: Dict) {
    ...
    ComplianceCheck _check_democratic_process(self, sys_data: Dict) {
    ...
    char* _status_from_severity(severity: ViolationSeverity) {
...
typedef struct SystemSteward {
    ...
    citizen_id: texto;
    citizen_name: texto;
    StewardRole role = StewardRole.STEWARD;
    double mandate_start = 0.0;
    int mandate_duration_days = 180 // 6 meses;
    int cycles_served = 0;
    int max_consecutive = 2;
    int max_systems_per_person = 3;
    // Demands pipeline
    [Dict] pending_demands = field(default_factory=list);
    [Dict] completed_demands = field(default_factory=list);
    [Dict] rejected_demands = field(default_factory=list);
    // Reporting
    char* last_status_report = "";
    int constitutional_alerts = 0;
    // Successor
    char* apprentice_id = "";
    char* apprentice_name = "";
    // decorador: @property
    bool can_accept_more_systems(self) {
        return self.cycles_served < self.max_consecutive;
    funcao receive_demand(self, source: texto, demand: texto,
                    char* priority = "normal") -> {texto: qualquer}:;
        ...
        return entry;
    {texto: qualquer} execute_demand(self, demand_id: texto) {
        // Marca demanda como executada.
        /* TODO: iterador C manual para d em self.pending_demands */
    ...
    {texto: qualquer} report_status(self) {
        // Reporta o estado do sistema para a Republica.
        return {;
            "system_id": self.system_id,;
            "system_name": self.system_name,;
            "steward": self.citizen_name,;
            "pending_demands": sizeof(self.pending_demands),;
            "completed_demands": sizeof(self.completed_demands),;
...
typedef struct StewardAssignment {
    ...
    funcao founder_batch_demands(self, demands: List[(texto, texto)]
        ...
        /* para cada (sys_id, demand_text) em demands: */
            ...
            results.append(r);
        return results;
    {texto: qualquer} system_status_report(self) {
        ...
        return {;
            ...
            "stewards_assigned": sizeof(self.stewards),;
            "unassigned_systems": sizeof(unassigned),;
            "unassigned_list": unassigned,;
            "steward_reports": steward_reports,;
        };
    [Dict] list_systems_for_demand(self) {
        ...
        /* para cada (sid, sys_data) em self.engine.systems_registry.items(): */
            ...
            result.append({
                ...
                steward ? "pending": sizeof(steward.pending_demands) : 0,;
            });
        return result;
// ============================================================================
// 5. MAIN
...
if (__name__ == "__main__") {
    printf("=" * 80);
    printf("  OPENREPUBLIC -- MOTOR CONSTITUCIONAL + GUARDIOES");
    printf('  "Os principios ! sao adornos. Sao LEIS."');
    printf("=" * 80);
    mgr = StewardAssignment();
    // === 1. OS 4 PRINCIPIOS CODIFICADOS ===
    printf("\n\n  === 1. PRINCIPIOS CONSTITUCIONAIS CODIFICADOS ===\n");
    /* TODO: iterador C manual para p em ConstitutionalPrinciple */
        printf("  P{p.value[0]}: {p.value[1].upper()}");
        printf("  {p.value[2]}");
        printf();
    // === 2. REGISTRO DE SISTEMAS ===
    printf("\n  === 2. SISTEMAS REGISTRADOS: ";
        "{len(mgr.engine.systems_registry)} ===\n");
    printf("  {'ID':<12} {'Sistema':<24} {'Dominio':<16} {'Guardiao'}");
    printf("  {'-'*70}");
    ...
    /* TODO: iterador C manual para s em mgr.list_systems_for_demand() */
        printf("  {s['system_id']:<12} {s['name']:<24} ";
            "{s['domain']:<16} {s['steward']}");
    // === 3. VALIDACAO CONSTITUCIONAL ===
    printf("\n\n  === 3. AUDITORIA CONSTITUCIONAL COMPLETA ===\n");
    audit = mgr.engine.validate_all();
    printf("  Total de sistemas:     {audit['total_systems']}");
    printf("  Conformes:             {audit['fully_compliant']}");
    printf("  Suspensos:             {audit['suspended']}");
    printf("  Banidos:               {audit['banned']}");
    printf("  Taxa de conformidade:  {audit['compliance_rate']}");
    printf("  Precisam revisao:      {audit['needs_review']}");
    ...
    /* para cada (sid, demand) em demands: */
        ...
        if ("error" in r) {
            ...
            printf("  [{r['system']}] -> {r['steward']}");
            printf("    Demanda: {demand}");
            printf("    Status constitucional: {r['constitutional_status']}");
            printf("    Alertas: {r['constitutional_alerts']}");
            printf();
    // === 5. SISTEMAS SEM GUARDIAO ===
    report = mgr.system_status_report();
    printf("\n  === 5. SISTEMAS SEM GUARDIAO ===\n");
    printf("  Designados: {report['stewards_assigned']}/{report['total_systems']}");
    printf("  Sem guardiao: {report['unassigned_systems']}");
    if (report['unassigned_list']) {
        printf("  IDs: {', '.join(report['unassigned_list'][:10])}...");
    // === 6. DETALHE: AUDITORIA DE UM SISTEMA ===
    printf("\n\n  === 6. AUDITORIA DETALHADA: OpenHealth ===\n");
    detail = mgr.engine.validate_system("R-HEA-01");
    printf("  Sistema: {detail['system_name']}");
    printf("  Score geral: {detail['overall_score']}/100");
    printf("  Status: {detail['status']}");
    printf("  Conforme: {'SIM' if detail['fully_compliant'] else 'NAO'}");
    printf("\n  verificacao por principio:");
    /* TODO: iterador C manual para c em detail["checks"] */
        status = c["passed"] ? "PASS" : "FAIL";
        printf("    [{status}] {c['principle']}: {c['score']}/100");
        if (c["notes"] != "Conforme."  &&  c["notes"]) {
            printf("          -> {c['notes'][:100]}");
    // === FILOSOFIA ===
    printf("\n\n{'='*80}");
    printf("  SISTEMA CONSTITUCIONAL OPERACIONAL");
    printf("{'='*80}");
    printf(""";
...
// )
    printf("{'='*80}");
    printf("  ConstitutionalEngine + StewardAssignment operacional.");
    printf("  {len(REPUBLIC_SYSTEMS)} sistemas. 4 principios. 0 exencoes.");
    printf("{'='*80}");

#endif // OPENREPUBLIC_SISTEMA_CONSTITUCIONAL_REPRESENTANTES_POR_SISTEMA_H
