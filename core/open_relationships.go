...
package openrelationships_politica_de_transparencia_relacional_anti_inveja
...
type Relationship struct {
    ...
    status := RelationshipStatus.ACTIVE // RelationshipStatus
    declared_date := "" // string
    ended_date := "" // string
    public := true // TODOS sao publicos (regra) // bool
    children := 0 // int64
    notes := ""  // optional (P2 -- sem detalhar intimidade) // string
    // Anti-inveja
    public_recognition := "relacao_normal"  // ! && "melhor" nem "pior" // string
// ============================================================================
// 2. ANTI-INVEJA
// ============================================================================
type EnvyType int
...
// ============================================================================
func run_relationship_assembly(n_voters: inteiro = 10000) {texto: qualquer} {
...
type RelationshipEngine struct {
    ...
    funcao end_relationship(self, rel_id: texto,
                        mutual := true) -> {texto: qualquer}: // bool
    ...
    func check_status(self, person: texto) {texto: qualquer} {
        // Verifica status relacional de uma pessoa (publico).
        active = []
        ended = []
        for _, rel := range self.relationships.values() {
    ...
    funcao report_envy(self, person: texto, target: texto,
                    envy_type := EnvyType.RELATIONAL, // EnvyType
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENRELATIONSHIPS -- TRANSPARENCIA RELACIONAL + ANTI-INVEJA")
    fmt.Println("  Assembleia Constituinte sobre relacionamentos publicos")
    fmt.Println("=" * 80)
    // === 1. PERGUNTA A ASSEMBLEIA ===
    fmt.Println("\n\n  === 1. PERGUNTA A ASSEMBLEIA ===\n")
    fmt.Println('  "Todo relacionamento deve ser publico para evitar')
    fmt.Println('   invasao de espaco && combater inveja?"')
    // === 2. VOTACAO ===
    fmt.Println("\n\n  === 2. VOTACAO (10.000 cidadaos) ===\n")
    result = run_relationship_assembly(10000)
    fmt.Println("  PUBLICO TOTAL (fato + detalhe): {result['votes_public_total']:>6} "
        "({result['votes_public_total']/100:.0f}%)")
    fmt.Println("  PARCIAL (fato sim, detalhe !): {result['votes_partial_p2']:>6} "
        "({result['votes_partial_p2']/100:.0f}%)")
    fmt.Println("  PRIVADO (cada um decide):       {result['votes_private']:>6} "
    ...
    para a, b, rtype in rels: {
        ...
        fmt.Println("  {a:<12} + {b:<12} -> {rtype.value:<15} "
            "[{result_decl['status']}]")
    // === 5. VERIFICAR STATUS ===
    fmt.Println("\n\n  === 5. VERIFICACAO DE STATUS (anti-invasao) ===\n")
    for _, person := range ["Cleiton", "Carlos", "Joao", "Ana"] {
        status = engine.check_status(person)
        avail = status["available"] ? "DISPONIVEL" : "EM RELACIONAMENTO"
        fmt.Println("  {person:<12} -> {avail}", end="")
        if status["current_relationships"] {
            for _, rel := range status["current_relationships"] {
                fmt.Println(" ({rel['partner']}, {rel['type']})", end="")
        fmt.Println()
    ...
    if rels_list {
        ...
        fmt.Println("  {end_result['public_announcement']}")
        fmt.Println("  {end_result['message']}")
    // === 7. ANTI-INVEJA ===
    fmt.Println("\n\n  === 7. ANTI-INVEJA ===\n")
    envies = [
    ...
    para person, target, etype, desc in envies: {
        ...
        fmt.Println("  {result_envy['treatment'][:80]}")
    // === 8. STATS ===
    fmt.Println("\n\n  === 8. ESTATISTICAS ===\n")
    s = engine.stats()
    ...
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  FILOSOFIA DO OPENRELATIONSHIPS")
    fmt.Println("{'='*80}")
    fmt.Println("""
...
// )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenRelationships: {s['total_relationships']} declarados, "
        "{s['active']} ativos, "
        "{s['envy_cases_treated']} casos de inveja tratados.")
    fmt.Println("  Publico no fato. Privado na intimidade. Sem inveja.")
    fmt.Println("{'='*80}")
