...
#ifndef OPENWEAPONSPOLICY_POLITICA_DE_PORTE_DE_ARMAS_DA_REPUBLICA_H
#define OPENWEAPONSPOLICY_POLITICA_DE_PORTE_DE_ARMAS_DA_REPUBLICA_H
...
typedef struct PermitRequirement {
...
typedef struct WeaponItem {
    ...
    char* description = "";
    char* legitimate_use = ""  // uso legitimo (trabalho, defesa);
    WeaponPermit permit_needed = WeaponPermit.NONE;
    [PermitRequirement] requirements = field(default_factory=list);
    // Restricoes
    int max_per_person = 0 // 0 = proibido, 1 = uma unidade;
    bool open_carry = false // pode portar visivel?;
    bool concealed_carry = false // pode portar escondido?;
    bool home_only = false // so em casa?;
[WeaponItem] WEAPONS_CATALOG = [;
...
// ============================================================================
{texto: qualquer} run_weapons_assembly(n_voters: inteiro = 10000) {
...
//
    ...
    funcao request_permit(self, citizen_id: texto, citizen_name: texto,
                    item_id: texto,;
                    char* has_martial_arts_belt = "",;
                    bool psych_eval_passed = false,;
                    bool has_violent_record = false,;
                    bool assembly_approval = false,;
                    bool community_endorsement = false,;
    ...
    {texto: qualquer} revoke_permit(self, citizen_id: texto, reason: texto) {
    ...
    {texto: qualquer} stats(self) {
        return {;
            "itens_catalogados": sizeof(self.catalog),;
            ...
            "portes_ativos": sizeof(self.permits),;
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENWEAPONSPOLICY -- PORTE DE ARMAS DA REPUBLICA");
    printf("  OpenMartialArts primeiro. Arma de fogo ultimo recurso.");
    printf("=" * 80);
    // === 1. ASSEMBLEIA ===
    ...
    /* para cada (option, count) em result["votes"].items(): */
        ...
        bar = "#" * inteiro(pct / 2);
        printf("    {option:<30} {count:>5} ({pct:.0f}%) {bar}");
    ...
    current_cat = NULL;
    /* TODO: iterador C manual para w em engine.catalog.values() */
        if (w.category != current_cat) {
            current_cat = w.category;
            printf("\n  --- {w.category.value[0].upper()} ---");
        req_count = sizeof(w.requirements);
        location = w.concealed_carry ? w.home_only ? "DOMICILIO" : ("OCULTO" : "N/A");
        printf("  {w.name:<30} max:{w.max_per_person:>2} reqs:{req_count} local:{location}");
    // === 3. PEDIDOS DE PORTE ===
    printf("\n\n  === 3. PEDIDOS DE PORTE ===\n");
    ...
    if ("failed_requirements" in r) {
        printf("  Faltam: {', '.join(r['failed_requirements'])}");
    ...
    printf("  {r['message']}");
    if ("conditions" in r) {
        /* TODO: iterador C manual para c em r["conditions"] */
            printf("    -> {c}");
    ...
    /* TODO: iterador C manual para line em engine.philosophy() */
        printf("  {line}");
    // === 6. STATS ===
    printf("\n\n  === 6. ESTATISTICAS ===\n");
    s = engine.stats();
    /* para cada (k, v) em s.items(): */
        printf("  {k:<30} {v}");
    printf("\n{'='*80}");
    printf("  OpenWeaponsPolicy: {s['proibidos']} proibidos, ";
        "{s['armas_fogo_restritas']} restritas. ";
        "{s['portes_ativos']} portes ativos.");
    printf("  {s['politica']}");
    printf("{'='*80}");

#endif // OPENWEAPONSPOLICY_POLITICA_DE_PORTE_DE_ARMAS_DA_REPUBLICA_H
