...
struct ProtectedAbsence {
    ...
    let duration: AbsenceDuration = AbsenceDuration.FEW_HOURS;
    let status: AbsenceStatus = AbsenceStatus.AUTO_APPROVED;
    let start_date: String = "";
    let end_date: String = "";
    let hours_away: f64 = 0.0;
    // PRIVACIDADE (P2)
    let explanation_provided: bool = false // ! precisa explicar;
    let explanation_kept_private: bool = true // se explicou, && PRIVADO;
    let visible_to_others: String = "ausente"  // so mostra "ausente";
    // Cobertura
    let coverage_assigned: bool = false // OpenLaborRelay cobriu?;
    let coverage_by: String = ""  // quem cobriu;
    // Protecao
    let penalized: bool = false // NUNCA. Lei.;
    let credit_affected: bool = false // NUNCA. Lei.;
    let reputation_affected: bool = false // NUNCA. Lei.;
    // decorador: @property
...
// ============================================================================
fn run_absence_assembly(n_voters: inteiro = 10000) -> {texto: qualquer} {
...
struct AbsenceEngine {
    ...
    funcao request_absence(self, citizen_id: texto, citizen_name: texto,
                        ...
                        let provide_explanation: bool = false;
                        ) -> {texto: qualquer}:;
    ...
    fn check_status(self, citizen_id: texto) -> {texto: qualquer} {
    ...
    fn return_from_absence(self, absence_id: texto) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENABSENCE -- DIREITO DE AUSENCIA PROTEGIDO");
    println!("  Assembleia Constituinte: ausencia && direito. Sem perguntas.");
    println!("=" * 80);
    // === 1. ASSEMBLEIA ===
    println!("\n\n  === 1. ASSEMBLEIA CONSTITUINTE ===\n");
    result = run_absence_assembly(10000);
    println!("  PERGUNTA: {result['question']}");
    println!("\n  PROTEGER (sem condicoes):  {result['votes_protect']:>6} ({result['pct_protect']})");
    println!("  COM CONDICOES:              {result['votes_conditional']:>6} ({result['votes_conditional']/100:.0f}%)");
    println!("  CONTRA:                     {result['votes_against']:>6} ({result['votes_against']/100:.0f}%)");
    println!("\n  RESULTADO: {result['result']}");
    println!("\n  DECISAO DA ASSEMBLEIA:");
    para cada (key, val) em result["decision"].items(): {
        if isinstance(val, logico) {
            println!("    {key:<35} {'SIM' if val else 'NAO'}");
        } else {
            println!("    {key:<35} {val}");
    ...
    para cid, name, atype, dur in absences_data: {
        ...
        println!("\n  {r['citizen']:<12} -> {r['type']:<20} {r['duration']:<15} ";
            "[{r['status']}]");
        println!("    Visivel: '{r['visible_to_others']}'  Penalizada: {r['penalized']}");
        println!("    Cobertura: {r['coverage']['covered']}");
    ...
    for cid in ["C-001", "C-002", "C-005", "C-999"] {
        ...
        println!("  {status.get('citizen', cid):<12} -> {status['status']}");
        if "message" in status  &&  "ausente" in status.get("status", "").lower() {
            println!("    {status['message'][:70]}");
    ...
    for aid in aids {
        ...
        println!("  {r['citizen']:<12} -> {r['status']}");
        println!("    {r['welcome'][:70]}");
    // === 5. PRORROGAR ===
    println!("\n\n  === 5. PRORROGANDO (precisa de mais tempo) ===\n");
    if tamanho(aids) >= 3 {
        r = engine.extend_absence(aids[2], AbsenceDuration.ONE_WEEK);
        println!("  {r['citizen']:<12} -> {r['status']} ({r['new_duration']})");
        println!("    {r['message'][:70]}");
    // === 6. VIOLACAO ===
    ...
    println!("  QUEM: {viol['citizen']}");
    println!("  O QUE: {viol['violation']}");
    println!("  TIPO: {viol['type']}");
    println!("  CONSEQUENCIA: {viol['consequence'][:80]}");
    println!("  LEI: {viol['law'][:80]}");
    // === 7. COMPARACAO ===
    ...
    println!("  {'Aspecto':<20} {'Sistema Atual':<35} {'Republica'}");
    println!("  {'-'*80}");
    para aspect, atual, rep in comparisons: {
        println!("  {aspect:<20} {atual:<35} {rep}");
    // === 8. STATS ===
    println!("\n\n  === 8. ESTATISTICAS ===\n");
    s = engine.stats();
    ...
    // === FILOSOFIA ===
    println!("\n\n{'='*80}");
    println!("  FILOSOFIA: DIREITO DE AUSENCIA");
    println!("{'='*80}");
    println!(""";
...
// )
    println!("{'='*80}");
    println!("  OpenAbsence: {s['total_ausencias']} ausencias protegidas. ";
        "Penalizadas: {s['penalizadas']}. Credito afetado: {s['credito_afetado']}.");
    println!("  Ausencia && direito. Sem perguntas. Sem pressao.");
    println!("{'='*80}");
