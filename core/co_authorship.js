...
class Contributor {
    ...
    const full_access = false;
    const can_author = true;
...
class CodeChange {
    ...
    const author_id = "";
    const reviewer_id = "";
    const author_approved = false;
    const reviewer_approved = false;
    const status = PRStatus.DRAFT;
    const comments = field(default_factory=list);
    const created_at = 0.0;
    const updated_at = 0.0;
    // decorador: @property
    can_deploy(self) {
...
class CoAuthorshipEngine {
    ...
    add_contributor(self, c: Contributor) {
    ...
    funcao create_pr(
        ...
        return pr;
    author_approve(self, change_id: texto, contributor_id: texto) {
    ...
    reviewer_approve(self, change_id: texto, contributor_id: texto) {
        ...
        pr.updated_at = datetime.now().timestamp();
        return self._check_both_approved(pr);
    reject(self, change_id: texto, contributor_id: texto, reason: texto) {
    ...
    add_comment(self, change_id: texto, contributor_id: texto, comment: texto) {
    ...
    deploy(self, change_id: texto, requested_by: texto) {
    ...
    _check_both_approved(self, pr: CodeChange) {
    ...
    status_report(self) {
        ...
        lines.append("-" * 100);
        for (const pr of self.prs.values()) {
        ...
        for (const d of self.deploy_log) {
            lines.append(;
                "  {d['change_id']}: {d['title']} ";
...
if (__name__ == "__main__") {
    ...
    console.log("PR-002: MING rejeita -> {r}");
    console.log();
    // Relatório final
    console.log();
    console.log(engine.status_report());
    // Tentar criar PR com mesma pessoa como autor E revisor
    console.log();
    console.log("=" * 100);
    console.log("TESTE: AUTOR = REVISOR (DEVE FALHAR)");
    console.log("=" * 100);
    tente:;
        engine.create_pr(;
            change_id = "PR-FAIL",;
    ...
    console.log();
    console.log("=" * 100);
    console.log("PRINCÍPIO DE COAUTORIA");
    console.log("=" * 100);
    console.log(""";
