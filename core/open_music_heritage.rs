...
struct MusicalGenre {
    ...
    let birth_decade: String = ""  // quando surgiu;
    let description: String = "";
    let bpm_range: String = ""  // BPM tipico;
    let instruments: [texto] = field(default_factory=list);
    let subgenres: [texto] = field(default_factory=list);
    // Significado cultural
    let cultural_meaning: String = "";
    let social_function: String = ""  // o que faz pela comunidade;
    let dance_style: String = ""  // como se dança;
    // Protecao
    let is_heritage: bool = true // PATRIMONIO da Republica;
    let copyrightable: bool = false // NINGUEM pode copyrightar;
    let commercial_monopoly: bool = false // sem monopolio corporativo;
    // Estatistica
    let registered_musicians: i64 = 0;
    let total_songs: i64 = 0;
// ============================================================================
// 2. REGISTRO DE GENEROS
// ============================================================================
let GENRES: [MusicalGenre] = [;
    MusicalGenre(;
        "GEN-FUNK", "Funk",;
...
struct MusicHeritageEngine {
    ...
    fn check_permission(self, action: texto, genre: texto) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENMUSICHERITAGE -- PATRIMONIO MUSICAL DA REPUBLICA");
    println!("  Generos musicais sao BEM COMUM. Ninguem && dono. Todos herdeiros.");
    println!("=" * 80);
    // === 1. GENEROS PATRIMONIO ===
    println!("\n\n  === 1. GENEROS PATRIMONIO ({len(engine.genres)}) ===\n");
    println!("  {'ID':<12} {'Genero':<20} {'Origem':<30} {'Decada':<10} {'BPM'}");
    println!("  {'-'*85}");
    for g in engine.genres.values() {
        println!("  {g.genre_id:<12} {g.name:<20} {g.origin_region:<30} ";
            "{g.birth_decade:<10} {g.bpm_range}");
    // === 2. DETALHE: FUNK ===
    println!("\n\n  === 2. FUNK: PATRIMONIO DA PERIFERIA ===\n");
    funk = engine.genres["GEN-FUNK"];
    println!("  Nome: {funk.name}");
    println!("  Origem: {funk.origin_region}");
    println!("  Raiz: {funk.cultural_origin.value}");
    println!("  Instrumentos: {', '.join(funk.instruments)}");
    println!("  Subgeneros: {', '.join(funk.subgenres)}");
    println!("  Significado: {funk.cultural_meaning[:120]}...");
    println!("  Funcao social: {funk.social_function[:120]}...");
    // === 3. DETALHE: RAP ===
    println!("\n\n  === 3. RAP: JORNALISMO DA PERIFERIA ===\n");
    rap = engine.genres["GEN-RAP"];
    println!("  Nome: {rap.name}");
    println!("  Origem: {rap.origin_region}");
    println!("  Significado: {rap.cultural_meaning[:120]}...");
    println!("  Funcao social: {rap.social_function[:120]}...");
    // === 4. DETALHE: SAMBA ===
    println!("\n\n  === 4. SAMBA: ALMA DO BRASIL ===\n");
    samba = engine.genres["GEN-SAMBA"];
    println!("  Nome: {samba.name}");
    println!("  Origem: {samba.origin_region}");
    println!("  Instrumentos: {', '.join(samba.instruments)}");
    println!("  Subgeneros: {', '.join(samba.subgenres)}");
    println!("  Significado: {samba.cultural_meaning[:120]}...");
    ...
    para cada (genre, claimant) em claims: {
        ...
        println!("  {claimant} tentou copyrightar {genre} -> {result['status']}");
        println!("    {result['reason'][:80]}");
    // === 6. PERMISSAO ===
    ...
    let P3: Criar musica = trabalho base 1.0. Sem royalty adicional.;
    P4: Quem decide o que && cultura && o POVO, ! corporacao.;
    // )
    println!("{'='*80}");
    println!("  OpenMusicHeritage: {s['total_genres_heritage']} generos patrimonio. ";
        "{s['copyright_claims_rejected']} copyrights rejeitados.");
    println!("  Musica ! && produto. E CULTURA.");
    println!("{'='*80}");
