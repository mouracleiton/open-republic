...
public class OpennightlifeOpenestablishmentupgrade {
    ...
    // 1. NIGHTLIFE
    // ============================================================================
    public static class NightEventType {
    ...
    public static class NightVenue {
    ...
    public static class NightEvent {
        ...
        String organizer = ""  // quem organiza (rotativo);
        int max_attendees = 200;
        int current_attendees = 0;
        String description = "";
        boolean has_food = true;
        boolean has_music = true;
        boolean has_drinks = true // agua/cha/suco (! alcool);
        [NightSafety] safety = field(default_factory=list);
        boolean family_friendly = true;
        double cost = 0.0 // SEMPRE ZERO;
        boolean alcohol_allowed = false;
        boolean broadcast_on_tv = false // transmitido na OpenTV;
        // decorador: @property
        public boolean is_late_night(self) {
            return self.end_hour > 1 || self.end_hour < 6;
    // decorador: @dataclass
    public static class NightZone {
        // Uma zona noturna da Republica.
        zone_id: texto;
        name: texto;
        region: texto;
        [NightVenue] venues = field(default_factory=list);
        [NightEvent] active_events = field(default_factory=list);
        double illumination_level = 0.9 // iluminacao OpenEnergy;
        int patrol_volunteers = 0 // OpenMartialArts voluntarios;
        int open_terminals = 0 // OpenTerminal disponiveis a noite;
        boolean has_food = true;
        boolean has_transport = true // OpenMobility carona a noite;
    public static class NightLifeEngine {
        ...
        funcao create_event(self, name: texto, etype: NightEventType,
                        ...
                        int max_att = 200,;
                        boolean family = true,;
                        boolean broadcast = false) -> {texto: qualquer}:;
            // Cria evento noturno.
        ...
        public [Dict] nightly_schedule(self, day: texto = "sexta") {
            // Grade de eventos noturnos para um dia.
            schedule = [];
            /* TODO: for-each Java para event em self.events.values() */
                schedule.append({
                    "name": event.name,;
        ...
        public {texto: qualquer} night_safety_report(self) {
            return {;
    ...
    // ============================================================================
    public static class EstablishmentStatus {
    ...
    public static class Establishment {
        ...
        location: texto;
        EstablishmentStatus status = EstablishmentStatus.OUTDATED;
        [texto] current_equipment = field(default_factory=list);
        [UpgradeType] needed_upgrades = field(default_factory=list);
        int staff_count = 3;
        int patients_per_day = 50;
        // Custo da modernizacao (em TRABALHO, nao dinheiro)
        double upgrade_work_hours = 0.0;
        double upgrade_credit_cost = 0.0;
        String upgrade_status = "pendente"  // pendente, em_andamento, concluido;
    public static class EstablishmentUpgrader {
        ...
        public {texto: qualquer} assess(self, est: Establishment) {
        ...
        public String _benefit_description(self, est: Establishment) {
            if (est.type == "consultorio_dental") {
        ...
        public {texto: qualquer} execute_upgrade(self, est_id: texto) {
        ...
        public {texto: qualquer} batch_upgrade_report(self) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENNIGHTLIFE + OPENESTABLISHMENTUPGRADE");
        System.out.println("  Noite comunitaria + modernizacao com custo minimo");
        System.out.println("=" * 80);
        // === PARTE 1: NIGHTLIFE ===
        System.out.println("\n\n  {'='*40}");
        System.out.println("  PARTE 1: OPENNIGHTLIFE");
        ...
        /* para name, etype, venue, start, end, org, max_att, family in events_data: */
            ...
            System.out.println("  [{result['time']}] {result['event']:<40} ";
                "{result['venue']:<20} {result['cost']}");
        // Simular comparecimento
        /* TODO: for-each Java para event em nightlife.events.values() */
        ...
        // Grade noturna
        System.out.println("\n\n  === GRADE NOTURNA ===\n");
        System.out.println("  {'Horario':<14} {'Evento':<40} {'Local':<22} {'Publico':<10} {'TV'}");
        System.out.println("  {'-'*90}");
        /* TODO: for-each Java para ev em nightlife.nightly_schedule() */
            tv = ev["on_tv"] ? "SIM" : "";
            System.out.println("  {ev['time']:<14} {ev['name']:<40} {ev['venue']:<22} ";
                "{ev['attendees']:<10} {tv}");
        // Seguranca
        System.out.println("\n\n  === SEGURANCA NOTURNA ===\n");
        ...
        System.out.println("  {'Aspecto':<25} {'Balada atual':<25} {'Republica':<25}");
        ...
        /* para aspect, balada, republica in comparisons: */
            System.out.println("  {aspect:<25} {balada:<25} {republica:<25}");
        // === PARTE 2: ESTABLISHMENT UPGRADE ===
        System.out.println("\n\n  {'='*40}");
        System.out.println("  PARTE 2: MODERNIZACAO DE ESTABELECIMENTOS");
        System.out.println("  {'='*40}\n");
        ...
        /* TODO: for-each Java para est em establishments */
            upgrader.establishments[est.est_id] = est;
        // Avaliar necessidades
        System.out.println("\n  === AVALIACAO DE NECESSIDADES ===\n");
        System.out.println("  {'Estabelecimento':<30} {'Status':<15} {'Horas':>6} {'Custo':<10} {'Beneficio'}");
        System.out.println("  {'-'*95}");
        /* TODO: for-each Java para est em establishments */
            assessment = upgrader.assess(est);
            System.out.println("  {est.name:<30} {est.status.value:<15} ";
        ...
        /* TODO: for-each Java para est em establishments */
            ...
            System.out.println("    Status: {result['new_status']}");
            System.out.println("    Capacidades novas: {', '.join(result['new_capabilities'][:4])}");
            System.out.println("    Trabalho investido: {result['work_invested']:.0f}h");
            System.out.println("    Custo dinheiro: {result['money_cost']}");
            System.out.println("    {result['message'][:70]}");
        // Relatorio
    ...
    PRINCIPIOS:;
        ...
        Modernizar P3 = trabalho. Organizar festa = trabalho.;
    ...
    // )
        ...
        System.out.println("  NightLife: {nl_stats['total_events']} eventos, ";
            "{nl_stats['events_family_friendly']} familiares, ";
            "{nl_stats['events_no_alcohol']} sem alcool.");
        System.out.println("  Upgrade: {up_stats['modernized']}/{up_stats['total_establishments']} ";
