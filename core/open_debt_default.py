...
class Creditor:
    ...
    purchase_price_pct: float = 1.0          # pagou quanto do valor nominal?
    real_risk: str = "baixo"                 # risco REAL para o pais se nao pagar
    bluffs: List[str] = field(default_factory=list)  # o que ameaca
    real_consequence: str = ""               # o que REALMENTE acontece
    can_punish: bool = False                 # tem poder REAL de punir?
...
class YearSimulation:
    ...
    pay_debt_brl: float = 0.0
    pay_interest_brl: float = 0.0
    pay_public_investment_brl: float = 0.0
    pay_gdp_brl: float = 0.0
    pay_gdp_per_capita: float = 0.0
    pay_health_budget: float = 0.0
    pay_education_budget: float = 0.0
    pay_inflation: float = 0.0
    pay_unemployment: float = 0.0
    pay_poverty_pct: float = 0.0

    # Caminho B: Parou de pagar (livre)
    nopay_debt_brl: float = 0.0         # divida estagnada/renegociada
    nopay_interest_brl: float = 0.0     # juros ZERO apos default
    nopay_freed_money_brl: float = 0.0  # dinheiro liberado (ex-juros)
    nopay_public_investment_brl: float = 0.0
    nopay_gdp_brl: float = 0.0
    nopay_gdp_per_capita: float = 0.0
    nopay_health_budget: float = 0.0
    nopay_education_budget: float = 0.0
    nopay_inflation: float = 0.0
    nopay_unemployment: float = 0.0
    nopay_poverty_pct: float = 0.0

    # Diferenca
    gdp_gap: float = 0.0                # quanto o Caminho B esta na frente
    cumulative_freed: float = 0.0       # total liberado acumulado
    winner: str = ""                     # "pagar" ou "nao_pagar"
...
class DefaultSimulator:
    ...
    def simulate(self) -> List[YearSimulation]:
    ...
    def crossover_year(self) -> Optional[int]:
        """Encontra o ano em que nao-pagar ultrapassa pagar."""
        for sim in self.simulations:
            if sim.year > 0 and sim.nopay_gdp_brl > sim.pay_gdp_brl:
                return sim.year_label
        return None

    def final_comparison(self) -> Dict[str, Any]:
...
def render_comparison_chart(simulations: List[YearSimulation]) -> str:
    ...
    for s in simulations:
        ...
        phase_marker = ""
        if s.phase == DefaultPhase.PANIC:
            phase_marker = " [PANICO]"
        elif s.phase == DefaultPhase.SHOCK:
            phase_marker = " [CHOQUE]"
        elif s.phase == DefaultPhase.RECOVERY:
            phase_marker = " [RECUPERANDO]"
        elif s.phase == DefaultPhase.GROWTH:
            phase_marker = " [DISPARANDO]"
        elif s.phase == DefaultPhase.PROSPERITY:
...
def render_poverty_chart(simulations: List[YearSimulation]) -> str:
...
def render_truth_table() -> str:
    ...
    for i, truth in enumerate(AgiotaTruthTable.TRUTHS, 1):
        lines.append("")
...
def render_timeline(simulations: List[YearSimulation]) -> str:
    ...
    for s in simulations:
        ...
        elif s.phase == DefaultPhase.PANIC:
        ...
        elif s.phase == DefaultPhase.ADJUSTMENT:
