...
package openrepublic_politica_de_poluicao_sonora
...
type NoiseEvent struct {
    ...
    source: NoiseSource
    duration_min := 0 // float64
    location := "" // string
    reporter := "" // string
    timestamp := 0 // float64
    limit_db := 0 // int64
    exceeded_by := 0 // float64
    severity := NoiseSeverity.NONE // NoiseSeverity
type NoiseMonitor struct {
    ...
    funcao measure(self, db_level: flutuante, zone: SoundZone,
                ...
                source: NoiseSource,
                location := "", // string
                duration_min := 0) -> NoiseEvent: // float64
        ...
        return event
    func resolve(self, event: NoiseEvent) {texto: qualquer} {
    ...
    func zone_report(self, zone: SoundZone) {texto: qualquer} {
...
if __name__ == "__main__" {
    fmt.Println("=" * 75)
    fmt.Println("  OPENREPUBLIC -- POLITICA DE POLUICAO SONORA")
    fmt.Println("  'Som && vida. Ruido && violencia invisivel.'")
    fmt.Println("=" * 75)
    monitor = NoiseMonitor()
    // === Limits Table ===
    fmt.Println("\n\n  === LIMITES DE RUIDO dB(A) ===\n")
    fmt.Println("  {'Zona':<15} {'Noite':>6} {'Amanh.':>7} {'Dia':>6} {'Anoit.':>7}")
    fmt.Println("  {'-'*45}")
    for _, zone := range SoundZone {
        ...
        fmt.Println("  {zone.value:<15} {night:>6} {dawn:>7} {day:>6} {eve:>7}")
    ...
    para label, db, zone, tw, source, loc in scenarios: {
        ...
        fmt.Println("\n  {label}:")
        fmt.Println("    Medido: {event.measured_db:.0f}dB | Limite: {event.limit_db}dB "
            "| Excesso: +{event.exceeded_by:.0f}dB{flag}")
        fmt.Println("    Acao: {resolution['action']}")
        fmt.Println("    {resolution['reason']}")
    // === Zone Report ===
    fmt.Println("\n\n  === RELATORIO POR ZONA ===\n")
    for _, zone := range SoundZone {
        rep = monitor.zone_report(zone)
        fmt.Println("  {rep['zone']:<15} eventos={rep['total_events']} "
