...
public class OpenpenalrevisionEsvaziarOSistemaPrisionalTransformarEmForcaProdutiva {
    ...
    // ============================================================================
    public static class CrimeCategory {
        ...
        GENOCIDE = "genocidio";
    public static class CrimeSeverity {
    ...
    // decorador: @dataclass
    public static class PenalCase {
        // Um caso carcerario para revisao.
        case_id: texto;
        citizen_id: texto;
        citizen_name: texto;
        int age = 0;
        CrimeCategory crime_category = CrimeCategory.PETTY_THEFT_FOOD;
        String crime_description = "";
        double original_sentence_years = 0.0;
        double years_served = 0.0;
        String victim_id = "";
        String victim_name = "";
        double victim_reparation_due = 0.0 // em horas de trabalho;
        // Contexto (Causa raiz)
        boolean poverty_driven = false // crime por necessidade economica;
        boolean addiction_driven = false // crime por dependencia quimica;
        boolean mental_health_issue = false // saude mental;
        boolean first_offense = true;
        String education_level = "basico";
        // Revisao
        CrimeSeverity severity = CrimeSeverity.NOT_CRIME;
        SentenceStatus status = SentenceStatus.PENDING_REVIEW;
        String new_plan = ""  // plano de transformacao;
        double work_hours_required = 0.0 // trabalho para reparacao;
        [texto] skills_to_learn = field(default_factory=list);
        String treatment_required = "";
        String review_date = "";
        String reviewer_id = "";
        // decorador: @property
        public boolean should_release_immediately(self) {
            // Deve ser libertado AGORA?
            return self.severity == CrimeSeverity.NOT_CRIME;
        // decorador: @property
        public boolean is_productive_eligible(self) {
            // Pode ser convertido em forca produtiva?
            return self.severity in (CrimeSeverity.LOW, CrimeSeverity.MEDIUM,;
                                    CrimeSeverity.HIGH);
        // decorador: @property
        public boolean is_permanently_restricted(self) {
            // Crimes irrecuperaveis (hediondos)?
            return self.severity == CrimeSeverity.HEDIONDOUS;
    // ============================================================================
    // 3. MOTOR DE REVISAO PENAL
    // ============================================================================
    public static class PenalRevisionEngine {
        ...
        public None submit_case(self, case: PenalCase) {
            self.cases[case.case_id] = case;
        public {texto: qualquer} review(self, case_id: texto, reviewer_id: texto) {
        ...
        public {texto: qualquer} _release_immediate(self, case: PenalCase) {
        ...
        public {texto: qualquer} _convert_to_productive(self, case: PenalCase) {
        ...
        public {texto: qualquer} _restrict_permanent(self, case: PenalCase) {
        ...
        public double _calculate_reparation_hours(self, case: PenalCase) {
        ...
        public [texto] _assign_skills(self, case: PenalCase) {
        ...
        public {texto: qualquer} batch_review(self, reviewer_id: texto = "coletivo") {
    ...
    // ============================================================================
    public static class ProductiveForce {
        ...
        public void __init__(self) {
            ...
            self.work_log: [Dict] = [];
        public {texto: qualquer} enroll(self, case: PenalCase) {
        ...
        funcao log_work(self, case_id: texto, hours: flutuante,
            ...
            return entry;
        public {texto: qualquer} reintegrate(self, case_id: texto) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENPENALREVISION");
        System.out.println("  Esvaziar Prisoes, Transformar em Forca Produtiva");
        System.out.println("=" * 80);
        // === 1. POPULACAO PRISIONAL ===
        cases = _seed_cases();
        /* TODO: for-each Java para c em cases */
            engine.submit_case(c);
        System.out.println("\n\n  === 1. POPULACAO PRISIONAL: {len(engine.cases)} CASOS ===\n");
        System.out.println("  {'ID':<8} {'Nome':<18} {'Crime':<20} {'Sentenca':>8} {'Cumprida'}");
        System.out.println("  {'-'*70}");
        /* TODO: for-each Java para c em engine.cases.values() */
            System.out.println("  {c.case_id:<8} {c.citizen_name:<18} ";
        ...
        System.out.println("  Total revisado:           {results['total_reviewed']}");
        System.out.println("  LIBERTAR IMEDIATAMENTE:   {results['released_immediately']}");
        System.out.println("  Convertidos a trabalho:   {results['converted_to_productive']}");
        System.out.println("  Restricao permanente:     {results['permanently_restricted']}");
        System.out.println("  Horas de trabalho geradas:{results['work_hours_generated']:,.0f}h");
        System.out.println("\n  ANTES: {results['prison_population_before']} presos");
        System.out.println("  DEPOIS: {results['prison_population_after']} presos (so hediondos)");
        System.out.println("  ESVAZIAMENTO: {results['emptying_rate']}");
        // === 3. DETALHES DA REVISAO ===
        System.out.println("\n\n  === 3. DETALHES POR CASO ===\n");
        /* TODO: for-each Java para c em engine.cases.values() */
            System.out.println("\n  [{c.case_id}] {c.citizen_name} (idade {c.age})");
            System.out.println("  Crime: {c.crime_description}");
            System.out.println("  Severidade: {c.severity.name}");
            if (c.status == SentenceStatus.OVERTURNED) {
                System.out.println("  ACAO: LIBERTAR IMEDIATAMENTE");
                System.out.println("  Motivo: {c.crime_category.value} ! && crime na Republica");
                System.out.println("  Apoio: moradia + comida + tratamento + oficio");
            } else if (c.status == SentenceStatus.CONVERTED_TO_WORK) {
                System.out.println("  ACAO: CONVERTIDO EM FORCA PRODUTIVA");
                System.out.println("  Trabalho: {c.work_hours_required:.0f}h ";
                    "({c.work_hours_required/920:.1f} anos)");
                System.out.println("  Skills: {', '.join(c.skills_to_learn[:4])}");
                if (c.treatment_required) {
                    System.out.println("  Tratamento: {c.treatment_required}");
                System.out.println("  Vitima: reparada com trabalho");
            } else if (c.status == SentenceStatus.RESTRICTED) {
                System.out.println("  ACAO: RESTRICAO PERMANENTE (hediondo)");
                System.out.println("  Direitos mantidos: sim (sem tortura)");
        ...
        /* TODO: for-each Java para c em engine.cases.values() */
            if (c.status == SentenceStatus.CONVERTED_TO_WORK) {
                ...
                if (result.get("completed")) {
                    System.out.println("  {result['citizen']}: CONCLUIU! {result['progress']}");
        // Mostrar progresso
        System.out.println("\n  Progresso dos transformados:");
        System.out.println("  {'Nome':<18} {'Progresso':>10} {'Horas':>8} {'Status'}");
        System.out.println("  {'-'*55}");
        /* TODO: for-each Java para record em force.transformed.values() */
            System.out.println("  {record['citizen']:<18} {record['progress']:>9.0f}% ";
        ...
        /* para cada (cid, record) em list(force.transformed.items()): */
            if (record["progress"] >= 100) {
                ...
                System.out.println("  {r['citizen']}: {r['status']}");
                System.out.println("  Skills: {r['skills_learned']}");
        ...
        System.out.println("  ANTES (sistema tradicional):");
        System.out.println("    Custo/prsao/ano: R$ {custo_tradicional:,}");
        System.out.println("    Producao dos presos: R$ 0");
        System.out.println("    Reincidencia: ~70%");
        System.out.println("    Custo social: ALTO");
        System.out.println("\n  DEPOIS (Republica):");
        System.out.println("    Custo/prsao/ano: R$ {custo_republica:,} (ZERO)");
        System.out.println("    Producao dos transformados: R$ {valor_trabalho:,}");
        System.out.println("    Reincidencia estimada: <20% (tem oficio)");
        System.out.println("    GANHO social: ALTO");
        // === FILOSOFIA ===
        System.out.println("\n\n{'='*80}");
        System.out.println("  FILOSOFIA DO OPENPENALREVISION");
        System.out.println("{'='*80}");
        System.out.println(""";
    ...
    // )
        System.out.println("{'='*80}");
        System.out.println("  OpenPenalRevision: {results['total_reviewed']} casos revisados.");
        System.out.println("  {results['released_immediately']} libertados. ";
            "{results['converted_to_productive']} transformados. ";
            "{results['permanently_restricted']} restritos.");
        System.out.println("  Prisao esvaziada. Forca produtiva criada.");
        System.out.println("{'='*80}");
}
