...
**Arquivo original:** `open-republic/core/open_sign_language_policy.py`
...
classe PolicyArticle herda de Enum:
...
classe LanguageRole herda de Enum:
...
classe EducationLevel herda de Enum:
...
classe FluencyLevel herda de Enum:
...
// decorador: @dataclass
classe CitizenLanguageProfile:
    // Perfil linguistico de um cidadao da Republica.
    citizen_id: str
    name: str
    declare country: str  <- "Brasil"
    declare country_code: str  <- "BR"

    // Linguas faladas/escritas
    declare mother_tongue_spoken: str  <- "pt"  // lingua materna falada
    declare other_spoken: List[str]  <- field(default_factory=list)  // outras linguas faladas
    declare reading_level: str  <- "alfabetizado"  // alfabetizado, analfabeto, funcional

    // Linguas de sinais
    declare native_sign_language: str  <- ""  // Libras, ASL, etc (se surdo)
    declare lsu_fluency: FluencyLevel  <- FluencyLevel.NONE  // nivel de Lingua de Sinais Universal
    declare other_sign_languages: List[str]  <- field(default_factory=list)

    // Condicao
    declare is_deaf: bool  <- FALSO
    declare is_hard_of_hearing: bool  <- FALSO
    declare is_hearing: bool  <- VERDADEIRO
    declare disability_notes: str  <- ""

    // Educacao LSU
    declare lsu_education_started: bool  <- FALSO
    declare lsu_education_level: str  <- ""
    declare lsu_certified: bool  <- FALSO

    funcao communication_methods(self) retorna List[str]:
        // Metodos de comunicacao que esta pessoa pode usar.
        methods <- []
        se self.is_hearing  E  self.reading_level != "analfabeto" entao:
            methods.append("fala")
            methods.append("escrita")
        se self.native_sign_language entao:
            methods.append(f"sinais_{self.native_sign_language}")
        se self.lsu_fluency != FluencyLevel.NONE entao:
            methods.append("lsu")
        se NAO  methods entao:
            methods.append("gesto_universal")
        retorne methods

    funcao can_communicate_with(self, other: 'CitizenLanguageProfile') retorna bool:
        // Verifica se pode se comunicar diretamente com outra pessoa.
        // Mesma lingua falada
        se self.mother_tongue_spoken == other.mother_tongue_spoken  E  self.is_hearing  E  other.is_hearing entao:
            retorne VERDADEIRO
        // Mesma lingua de sinais regional
        se self.native_sign_language  E  self.native_sign_language == other.native_sign_language entao:
            retorne VERDADEIRO
        // Ambos falam LSU
        se self.lsu_fluency != FluencyLevel.NONE  E  other.lsu_fluency != FluencyLevel.NONE entao:
            retorne VERDADEIRO
        retorne FALSO
...
classe LSUCurriculumUnit:
    ...
    declare assessment_method: str  <- "pratico"  // avaliacao por pratica (nao escrita)


declare LSU_CURRICULUM: List[LSUCurriculumUnit]  <- [
    LSUCurriculumUnit(
...
classe PolicyDetail:
    ...
    declare timeline: str  <- ""  // prazo
    declare responsible: str  <- ""  // quem executa
    declare penalty: str  <- ""  // o que acontece se descumprir


declare POLICY_ARTICLES: List[PolicyDetail]  <- [
    PolicyDetail(
...
classe ImplementationPhase herda de Enum:
...
// decorador: @dataclass
classe ImplementationPlan:
    // Plano de implantacao por fase.
    phase: ImplementationPhase
    year: str
    actions: List[str]
    target_population: str
    declare budget_brl: float  <- 0.0
    declare success_metric: str  <- ""


declare IMPLANTATION_PLAN: List[ImplementationPlan]  <- [
    ImplementationPlan(
...
classe FluencyAssessmentEngine:
    // Avalia fluencia em LSU de cidadaos e instituicoes.

    funcao __init__(self):
        self.assessments: Dict[str, Dict] = {}

    funcao assess_citizen(self, profile: CitizenLanguageProfile) retorna Dict[str, Any]:
        // Avalia o perfil linguistico de um cidadao.
        can_communicate_internationally <- profile.lsu_fluency != FluencyLevel.NONE
        needs_interpreter <- NAO  can_communicate_internationally  E  NAO  (
            profile.is_hearing  E  profile.mother_tongue_spoken == "en"
        )
        ...
        retorne {
            ...
            "native_sign": profile.native_sign_language  OU  "nenhuma",
    ...
    funcao _recommend_education(self, profile: CitizenLanguageProfile) retorna str:
        // Recomenda educacao LSU baseada no perfil.
        se profile.lsu_fluency == FluencyLevel.NONE entao:
            retorne "Curso basico de LSU (40h) -- saudacoes, emergencia, necessidades"
        senao se profile.lsu_fluency == FluencyLevel.BASIC entao:
            retorne "Curso intermediario de LSU (60h) -- conversa cotidiana"
        senao se profile.lsu_fluency == FluencyLevel.INTERMEDIATE entao:
            retorne "Curso avancado de LSU (50h) -- debate, profissional"
        senao se profile.lsu_fluency == FluencyLevel.ADVANCED entao:
            retorne "Curso de fluencia (80h) -- nivel nativo"
        senao se profile.lsu_fluency in (FluencyLevel.FLUENT, FluencyLevel.NATIVE_SIGNER) entao:
            retorne "Ja fluente. Pode se tornar instrutor (120h)"
        retorne "Manter praticar"
    ...
    def assess_institution(self, institution_name: str,
        ...
        compliant <- pct >= 60  E  has_interpreter
...
classe InternationalConversation:
    // 
...
funcao render_comparison_english_vs_lsu() retorna str:
...
funcao demo():
    ...
    print(f"{'=' * 70}")
    para cada pd em POLICY_ARTICLES:
    ...
    print(f"{'=' * 70}")
    para cada unit em LSU_CURRICULUM:
    ...
    print(f"{'=' * 70}")
    total_budget <- 0
    para cada plan em IMPLANTATION_PLAN:
        total_budget <- total_budget + plan.budget_brl
    ...
    citizens <- [
        ...
        CitizenLanguageProfile("c2", "Maria (surda)", is_deaf=VERDADEIRO, is_hearing=FALSO,
    ...
    inst <- engine.assess_institution("Hospital SP", 500, 150, VERDADEIRO, VERDADEIRO)
