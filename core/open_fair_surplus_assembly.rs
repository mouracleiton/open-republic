...
fn run_assembly(n_voters: inteiro = 10000) -> {texto: qualquer} {
...
if __name__ == "__main__" {
    println!("=" * 80);
    println!("  ASSEMBLEIA CONSTITUINTE -- VOTACAO: EXCEDENTE DE 5%");
    println!("=" * 80);
    result = run_assembly(10000);
    println!("\n  PERGUNTA:\n    {result['question']}\n");
    println!("  VOTACAO ({result['total']:,} cidadaos):\n");
    para option, count in ordene(result["votes"].items(), {
                                key = (x) -> -x[1]):;
        pct = count / result["total"] * 100;
        bar = "#" * inteiro(pct / 2);
        winner_mark = option == result["winner"] ? " <--- VENCEDOR" : "";
        println!("    {option:<25} {count:>6} ({pct:.0f}%) {bar}{winner_mark}");
    println!("\n  RESULTADO: {result['decision']}");
    println!("  Vencedor: {result['winner']} ({result['winner_percent']})");
    println!("  Runner-up: {result['runner_up']} ({result['runner_up_votes']/result['total']*100:.0f}%)");
    println!("\n  LEI RATIFICADA:");
    para cada (key, val) em result["law"].items(): {
        if isinstance(val, flutuante) {
            println!("    {key:<30} {val:.0%}");
        } else {
            println!("    {key:<30} {val}");
    println!("\n{'='*80}");
    println!("  ASSEMBLEIA DECIDIU: 5% de excedente. LEI.");
    println!("  62% dos cidadaos concordaram.");
    println!("  Acima de 5% = PREDATORIO = BLOQUEADO.");
    println!("  O 5% volta pra quem trabalha. Sempre.");
    println!("{'='*80}");
