...
class ConstitutionalPattern {
    ...
    description: texto;
    // Onde existe
    const countries_with = field(default_factory=list);
    const countries_without = field(default_factory=list);
    // Dados de impacto
    const public_satisfaction_pct = 0 // % da populacao satisfeita;
    const inequality_index = 0 // 0=igual, 1=extremamente desigual;
    const corruption_index = 0 // 0=limpo, 1=corrupto;
    const human_rights_violations = 0;
    // Analise
    const verdict = LawVerdict.KEEP;
    const priority = Priority.LOW;
    const republic_alternative = "";
    const evidence = "";
    const public_desire = ""  // o que a populacao QUER;
// ============================================================================
...
class ConstitutionalAuditor {
    ...
    stats(self) {
        ...
        avg_satisfaction = soma(p.public_satisfaction_pct para p em self.patterns) / .length(self.patterns);
        total_violations = soma(p.human_rights_violations para p em self.patterns);
        return {;
            "patterns_analyzed": .length(self.patterns),;
...
class Patient {
    ...
    age: inteiro;
    const condition = "";
    const triage = TriageLevel.GREEN;
    // Necessidades
    const needs = field(default_factory=list);
    const urgency_score = 0 // 0-100 (100 = morte iminente);
    const wait_time_h = 0;
    // Priorizacao especial
    const is_child = false;
    const is_pregnant = false;
    const is_elder = false;
    const chronic = false;
class HealthResourceAllocator {
    ...
    _priority_score(patient: Patient) {
    ...
    funcao _try_allocate(self, patient: Patient,
        ...
        allocated = [];
        missing = [];
        for (const need of patient.needs) {
...
if (__name__ == "__main__") {
    console.log("=" * 85);
    console.log("  AUDITORIA CONSTITUCIONAL + ALOCACAO DE SAUDE");
    console.log("  'Engenharia reversa das leis que governam a humanidade.'");
    console.log("=" * 85);
    // === PART 1: CONSTITUTIONAL AUDIT ===
    auditor = ConstitutionalAuditor();
    console.log("\n\n  {'='*80}");
    console.log("  PARTE 1: AUDITORIA CONSTITUCIONAL");
    console.log("  {'='*80}");
    stats = auditor.stats();
    console.log("\n  Padroes analisados: {stats['patterns_analyzed']}");
    console.log("  Satisfacao popular media: {stats['avg_public_satisfaction']}%");
    console.log("  Violacoes de direitos humanos: {stats['total_human_rights_violations']:,}");
    console.log("\n  Por veredito:");
    para cada (v, count) em ordene(stats["by_verdict"].items(), key=(x) -> -x[1]): {
        console.log("    {v:<12} {count}");
    console.log("\n  Por prioridade:");
    para cada (p, count) em ordene(stats["by_priority"].items(), key=(x) -> x[1]): {
        console.log("    {p:<12} {count}");
    // Critical reforms
    console.log("\n\n  === REFORMAS CRITICAS (prioridade maxima) ===\n");
    critical = auditor.critical_reforms();
    console.log("  {'Lei':<35} {'Veredito':<10} {'Satisf.':>8} {'Desejo Popular'}");
    console.log("  {'-'*85}");
    for (const c of critical) {
        console.log("  {c['name']:<35} {c['verdict']:<10} {c['satisfaction']:>8} {c['public_desire'][:30]}");
    // By domain
    console.log("\n\n  === POR DOMINIO ===\n");
    ...
    para cada (domain, patterns) em ordene(domains.items()): {
        ...
        for (const p of patterns) {
            console.log("    [{p.verdict.value:>7}] [{p.priority.name:>8}] {p.name}");
            console.log("      Satisfacao: {p.public_satisfaction_pct}% | Republica: {p.republic_alternative[:50]}...");
    // Priority ranking
    console.log("\n\n  === ORDEM DE REVISAO (prioridade) ===\n");
    ranked = auditor.priority_ranking();
    para cada (i, p) em enumere(ranked, 1): {
        console.log("  {i:>2}. [{p.priority.name:>8}] [{p.verdict.value:>7}] ";
            "{p.domain.value:<18} {p.name}");
    // === PART 2: HEALTH ALLOCATION ===
    console.log("\n\n  {'='*80}");
    console.log("  PARTE 2: ALOCACAO DE RECURSOS DE SAUDE");
    console.log("  {'='*80}");
    ...
    para cada (rt, data) em report.items(): {
        console.log("  {rt:<15} {data['available']:>3}/{data['total']:>3} ";
    ...
    for (const r of results) {
        ...
        console.log("    Triagem: {r['triage']} | Score: {r['priority_score']}");
        console.log("    Alocado: {', '.join(r['allocated']) if r['allocated'] else 'nada'}");
        if (r["missing"]) {
            console.log("    FALTA: {', '.join(r['missing'])} -> {r['action']}");
...
AUDITORIA CONSTITUCIONAL:;
    ...
    {.length(critical)} reformas CRITICAS identificadas.;
