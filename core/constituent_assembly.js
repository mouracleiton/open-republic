...
class Citizen {
    ...
    const pragmatism = 0.5 // quao pragmatico vs idealista;
    const risk_tolerance = 0.5 // tolerancia a risco;
    const equity_focus = 0.5 // foco em igualdade vs meritocracia;
    const autonomy_focus = 0.5 // foco em autonomia individual;
...
class AssemblyProposal {
    ...
    const stances = field(default_factory=dict);
    // Parametros da votacao
    const quorum_pct = 0.50 // 50% + 1 para aprovar;
    const constitutional = false // se true, quorum = 60%;
    // Resultado
    const votes = field(default_factory=list);
    result: 'VoteResult'? = null;
    cast_votes(self, citizens: [Citizen], rng: random.Random) {
...
class VoteResult {
    ...
    total: inteiro;
    margin: texto;
    const by_sector = field(default_factory=dict);
    const note = "";
// ============================================================================
// 3. A ASSEMBLEIA
// ============================================================================
class ConstituentAssembly {
    ...
    submit_proposal(self, proposal: AssemblyProposal) {
    ...
    run_election(self) {
        ...
        constitution = {};
        for (const prop of self.proposals) {
...
if (__name__ == "__main__") {
    ...
    console.log(assembly.print_results());
    console.log();
    console.log("=" * 100);
    console.log("DECISOES APROVADAS PELA ASSEMBLEIA:");
    console.log("=" * 100);
    if (constitution) {
        para cada (title, details) em constitution.items(): {
            console.log("  >> {title}");
            console.log("     Valor: {details['value']}");
            console.log("     Votos: {details['votes']} ({details['yes_pct']:.0f}% sim)");
            console.log();
    } else {
        console.log("  Nenhuma proposta aprovada. O povo rejeitou tudo.");
        console.log("  O fundador precisa ouvir && reformular.");
    console.log("=" * 100);
