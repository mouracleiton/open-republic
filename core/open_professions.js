...
class ProfessionSkill {
    ...
    const description = "";
    const min_level = CareerLevel.TECNICO // nivel minimo para fazer;
// decorador: @dataclass
class Profession {
    ...
    const promotion_criteria = field(default_factory=dict);
    // Distribuicao ideal de carga
    const ideal_load = field(default_factory=dict);
// ============================================================================
// 3. CATALOGO DE PROFISSOES (30+)
// ============================================================================
build_profession_catalog() {
...
class ProfessionEngine {
    ...
    list_professions(self) {
        return [;
            {"id": p.prof_id, "name": p.name, "area": p.area,;
            "skills": .length(p.skills)};
            para p em self.professions.values() {
        ];
    get_career_path(self, prof_id: texto) {
    ...
    stats(self) {
        ...
        return {;
            "total_profissoes": .length(self.professions),;
            "total_skills": soma(.length(p.skills) para p em self.professions.values()),;
            "por_area": dict(by_area),;
            "trabalhadores_reconhecidos": .length(self.workers),;
...
if (__name__ == "__main__") {
    ...
    console.log("=" * 80);
    console.log("  OPENPROFESSIONS -- CARREIRA MODULAR PARA TODAS AS PROFISSOES");
    console.log("  Competencia > Titulo. Cada um faz o que PODE.");
    console.log("=" * 80);
    // === 1. PROFISSOES REGISTRADAS ===
    console.log("\n\n  === 1. PROFISSOES ({len(engine.professions)}) ===\n");
    by_area = defaultdict(list);
    for (const p of engine.professions.values()) {
        by_area[p.area].append(p);
    for (const area of ordene(by_area.keys())) {
        console.log("\n  {area.upper()}:");
        for (const p of by_area[area]) {
            console.log("    [{p.prof_id}] {p.name:<30} {len(p.skills)} skills");
    // === 2. CARREIRA MODULAR (exemplos detalhados) ===
    console.log("\n\n  === 2. TRILHA DE CARREIRA (exemplos) ===\n");
    for (const prof_id of ["PR-MED", "PR-DEV", "PR-CHEF", "PR-BUILD"]) {
        path = engine.get_career_path(prof_id);
        console.log("\n  {path['profession'].upper()} ({path['area']}):");
        para cada (level_label, data) em path["career_path"].items(): {
            skills = data["skills"];
            if (skills) {
                console.log("    [{level_label}] {data['meaning'][:40]}");
                console.log("      Skills: {', '.join(skills[:4])}");
        if (path.get("ideal_load")) {
            console.log("    Distribuicao ideal:");
            para cada (lvl, load) em path["ideal_load"].items(): {
                console.log("      {lvl}: {load}");
    ...
    para name, prof, level, proof in self_taught: {
        ...
        console.log("  {r['name']:<20} -> {r['profession']:<15} ({r['level']})");
    ...
    console.log("  {'Profissao':<25} {'Valor':<18} {'Por que'}");
    console.log("  {'-'*70}");
    for (const c of comparisons) {
        console.log("  {c['profissao']:<25} {c['valor']:<18} {c['por_que']}");
    console.log("\n  SEM 'superior' vs 'inferior'.");
    console.log("  Medico NAO vale mais que lixeiro.");
    console.log("  Advogado NAO vale mais que cozinheiro.");
    console.log("  Todos base 1.0 + impacto.");
    // === 6. STATS ===
    console.log("\n\n  === 6. ESTATISTICAS ===\n");
    s = engine.stats();
    ...
    // === FILOSOFIA ===
    console.log("\n\n{'='*80}");
    console.log("  FILOSOFIA: CARREIRA MODULAR PARA TODOS");
    console.log("{'='*80}");
    console.log(""";
COMPETENCIA > TITULO (P1 anti-elitismo):;
...
// )
    console.log("{'='*80}");
    console.log("  OpenProfessions: {s['total_profissoes']} profissoes, ";
