...
package teia_api_economy_micropagamentos_por_chamada_com_royalty_chain
...
const (
    ...
    func __init__(self, key: texto, description: texto, default_weight: flutuante) {
        self.key = key
...
type Contribution struct {
    ...
    description := "" // string
    // decorador: @property
    func effective_weight(self) float64 {
...
type APIEndpoint struct {
    ...
    // Precificacao
    price_per_call := 0.05 // R$ por chamada // float64
    free_tier_daily := 10 // chamadas gratis por dia // int64
    // Contribuidores (CADEIA DE PROVENIENCIA)
    contributions := field(default_factory=list) // [Contribution]
    // Metricas
    calls_today := 0 // int64
    calls_total := 0 // int64
    revenue_total := 0.0 // float64
    // decorador: @property
    func n_contributors(self) int64 {
        return len(set(c.contributor_id para c em self.contributions))
    // decorador: @property
    func total_weight(self) float64 {
        return soma(c.effective_weight para c em self.contributions)
    func contributor_share(self, contributor_id: texto) float64 {
    ...
    funcao revenue_split(self) retorna List[{texto: qualquer}]:
        ...
        contributor_weights := defaultdict(flutuante) // {texto: flutuante}
        contributor_names := {} // {texto: texto}
        contributor_roles := defaultdict(list) // Dict[texto, [texto]]
        for _, c := range self.contributions {
...
type TEIAAPI struct {
    ...
    func call(self, endpoint_id: texto, n_calls: inteiro = 1) {texto: qualquer} {
...
type RoyaltyTracker struct {
    ...
    func register_calls(self, endpoint_id: texto, n_calls: inteiro) {
        ...
        return result
    func register_daily_traffic(self, calls_per_endpoint: {texto: inteiro}) {
        // Simula um dia de trafego.
        para cada (eid, n) em calls_per_endpoint.items(): {
            self.register_calls(eid, n)
    func earnings_report(self) string {
        ...
        return "\n".join(lines)
    func revenue_per_endpoint_detail(self) string {
