...
class ReplicationCost {
    ...
    const data_collection_hours = 0 // baixar/coletar;
    const data_cleaning_hours = 0 // limpar/normalizar;
    const cross_referencing_hours = 0 // cruzar com outros;
    const model_building_hours = 0 // construir modelo;
    const documentation_hours = 0 // documentar;
    const fact_checking_hours = 0 // verificar;
    const engineering_hours = 0 // construir API/endpoint;
    // Custo/hora de um profissional senior no Brasil (R$)
    const senior_rate_brl = 200.0 // R$200/h (Data Engineer senior);
    // decorador: @property
    total_hours(self) {
...
class MarketFeedback {
    ...
    //
    endpoint: texto;
    const calls_last_30_days = 0;
    const unique_callers = 0;
    const revenue_30_days = 0.0;
    // Para calcular elasticidade (apos mudanca de preco)
    const calls_before_price_change = 0;
    const calls_after_price_change = 0;
    const price_change_pct = 0;
    // Conversao
    const free_tier_users = 0;
    const free_to_paid_conversion = 0 // %;
class DynamicPricer {
...
class CalibratedPrice {
    ...
    const feedback_adjustment = null;
    const feedback_reason = "SEM DADOS AINDA";
    // Camada 4: governance
    const assembly_override = null;
    const assembly_reason = "";
    // decorador: @property
    calibrated_token_cost(self) {
...
print_calibration() {
    ...
    lines.append("-" * 120);
    for (const mc of MARKET_COMPARABLES) {
        lines.append(;
            "{mc.provider:<25} ";
            "{mc.product_type:<30} ";
            "R${mc.price_brl:>9,.0f}  ";
    ...
    lines.append("-" * 120);
    for (const c of ordene(calibrated, key=(x) -> x.calibrated_token_cost, reverse=True)) {
