...
#ifndef OPENSILENCEPOLICY_ALERTAS_SONOROS_DE_PRESSAO_PROIBIDOS_H
#define OPENSILENCEPOLICY_ALERTAS_SONOROS_DE_PRESSAO_PROIBIDOS_H
...
typedef struct SoundAlert {
    ...
    char* description = "";
    char* default_sound = ""  // som original;
    AlertAction action = AlertAction.DISABLED;
    char* replacement = ""  // o que existe no lugar;
...
typedef struct SilenceEngine {
    ...
    {texto: qualquer} disable_all_pressure(self) {
        // Desativa TODOS os alertas de pressao.
        disabled = 0;
        visual_only = 0;
        emergency = 0;
        /* TODO: iterador C manual para alert em self.alerts.values() */
            if (alert.purpose == AlertPurpose.EMERGENCY) {
                ...
                disabled = disabled + 1;
        return {;
            "total_alertas": sizeof(self.alerts),;
    ...
    {texto: qualquer} user_opt_in(self, user_id: texto, alert_id: texto) {
    ...
    {texto: qualquer} check_alert(self, user_id: texto, alert_id: texto) {
    ...
    {texto: qualquer} stats(self) {
        return {;
            "total_alertas_catalogados": sizeof(self.alerts),;
            ...
            "usuarios_opt_in": soma(sizeof(v) para v em self.user_choices.values()),;
            "usuarios_ausentes_protegidos": sizeof(self.absent_users),;
...
if (__name__ == "__main__") {
    ...
    printf("=" * 80);
    printf("  OPENSILENCEPOLICY -- ALERTAS DE PRESSAO DESATIVADOS");
    printf("  'Para de me encher o saco porra.'");
    printf("=" * 80);
    // === 1. DESATIVAR TUDO ===
    printf("\n\n  === 1. DESATIVANDO TODA PRESSAO SONORA ===\n");
    r = engine.disable_all_pressure();
    printf("  {r['message']}");
    printf("  Som padrao no sistema: {r['som_no_sistema']}");
    // === 2. CATALOGO DE ALERTAS ===
    printf("\n\n  === 2. CATALOGO ({len(engine.alerts)} alertas) ===\n");
    current_purpose = NULL;
    /* TODO: iterador C manual para alert em engine.alerts.values() */
        ...
        printf("  [{action_icon:<10}] {alert.name[:45]:<46} pressao:{alert.pressure_level}");
    // === 3. O QUE SUBSTITUI ===
    printf("\n\n  === 3. O QUE EXISTE NO LUGAR ===\n");
    replacements = engine.what_replaces_sounds();
    /* TODO: iterador C manual para r em replacements */
        printf("\n  {r['categoria'].upper()}:");
        printf("    Antes:  {r['antes']}");
        printf("    Depois: {r['depois']}");
    // === 4. USUARIO OPT-IN ===
    printf("\n\n  === 4. USUARIO PODE OPTAR (se quiser) ===\n");
    opt = engine.user_opt_in("cleiton", "AL-H01");
    printf("  {opt['message']}");
    printf("  (Pode revogar a qualquer momento. Padrao = desativado.)");
    // === 5. HONRA AUSENCIA ===
    printf("\n\n  === 5. HONRA OPENSABSENCE ===\n");
    engine.absent_users.append("cleiton");
    check = engine.check_alert("cleiton", "AL-W01");
    printf("  Cleiton (ausente): {check['action']}");
    printf("  {check.get('message', check.get('reason', ''))}");
    // === 6. STATS ===
    printf("\n\n  === 6. ESTATISTICAS ===\n");
    s = engine.stats();
    /* para cada (k, v) em s.items(): */
        printf("  {k:<35} {v}");
    printf("\n{'='*80}");
    printf("  OpenSilencePolicy: {s['desativados']} desativados, ";
        "{s['so_visual']} visual, {s['emergencia']} emergencia.");
    printf("  {s['principio']}");
    printf("{'='*80}");

#endif // OPENSILENCEPOLICY_ALERTAS_SONOROS_DE_PRESSAO_PROIBIDOS_H
