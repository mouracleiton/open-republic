...
let COMMUNITY_STRATEGY: [CommunityPropagation] = [;
...
struct PublicProduct {
    ...
    platform: texto;
    status: texto;
let PUBLIC_PRODUCTS: [PublicProduct] = [;
    PublicProduct(;
...
if __name__ == "__main__" {
    ...
    println!("=" * 80);
    println!("  OPENPROPAGATION -- ESTRATEGIA DE DIVULGACAO MASSIVA");
    println!("  Nao grite. Demonstre. 1 prova > 1000 teorias.");
    println!("=" * 80);
    // === 1. AS 5 FRENTES ===
    println!("\n\n  === AS 5 FRENTES DE PROPAGACAO ===\n");
    println!("  1. DEMONSTRACAO REAL (piloto funcionando) -- A QUE MATRIA");
    println!("  2. CONTEUDO VIRAL (X/Twitter, video, infografico)");
    println!("  3. COMUNIDADES REAIS (CONAQ, MST, G10, ASA)");
    println!("  4. ALIANCAS (artistas, ONGs, jornalistas)");
    println!("  5. PRODUTO PUBLICO (simulador, app, landing page)");
    // === 2. PILOTOS ===
    println!("\n\n  === PILOTOS REAIS ({len(engine.pilots)}) ===\n");
    for p in engine.pilots {
        println!("\n  {p.name} ({p.location})");
        println!("    Demonstra: {p.what_demonstrates}");
        println!("    Impacto: {p.expected_impact}");
        println!("    Custo: {p.cost}");
        println!("    Prazo: {p.timeline}");
        println!("    Viral: {p.viral_potential}");
        println!("    Doc: {p.documentation}");
    // === 3. CONTEUDO ===
    println!("\n\n  === CONTEUDO VIRAL ({len(engine.content)}) ===\n");
    for c in engine.content {
        println!("\n  [{c.format}] {c.topic}");
        println!("    Hook: '{c.hook}'");
        println!("    Plataforma: {c.platform}");
        println!("    CTA: {c.call_to_action}");
        println!("    Esperado: {c.expected_reach}");
    // === 4. COMUNIDADES ===
    println!("\n\n  === EXPANSAO COMUNITARIA ({len(engine.communities)}) ===\n");
    for c in engine.communities {
        println!("  {c.strategy}");
        println!("    Metodo: {c.method}");
        println!("    Fator: {c.expansion_factor}");
    // === 5. ALIANCAS ===
    println!("\n\n  === ALIANCAS ({len(engine.alliances)}) ===\n");
    for a in engine.alliances {
        println!("  [{a.priority}] {a.ally_type}: {a.who}");
        println!("    Eles ganham: {a.what_they_gain[:60]}");
        println!("    Republica ganha: {a.what_republica_gains[:60]}");
    // === 6. PRODUTOS PUBLICOS ===
    println!("\n\n  === PRODUTOS PUBLICOS USAVEIS ({len(engine.products)}) ===\n");
    for p in engine.products {
        println!("  [{p.status}] {p.name}");
        println!("    O que: {p.what}");
        println!("    Viraliza porque: {p.why_viral}");
    // === 7. PLANO DE EXECUCAO ===
    println!("\n\n  === PLANO DE EXECUCAO ===\n");
    ...
    for fase in plan {
        ...
        for acao in fase["acoes"] {
            println!("    -> {acao}");
        println!("    Custo: {fase['custo']}");
        println!("    Resultado: {fase['resultado']}");
    // === 8. CALENDARIO ===
    println!("\n\n  === CALENDARIO DE CONTEUDO (X/Twitter) ===\n");
    cal = engine.content_calendar();
    for day in cal {
        println!("  {day['dia']:<12} {day['tipo']}: {day['exemplo']}");
    // === 9. METRICAS ===
    println!("\n\n  === METRICAS ===\n");
    for m in engine.key_metrics() {
        println!("  {m['metrica']}");
        println!("    3 meses: {m['meta_3meses']}");
        println!("    6 meses: {m['meta_6meses']}");
        println!("    12 meses: {m['meta_12meses']}");
    // === 10. O QUE NAO FAZER ===
    ...
    para cada (k, v) em s.items(): {
        println!("  {k:<30} {v}");
    println!("\n{'='*80}");
    println!("  OpenPropagation: {s['pilotos_planejados']} pilotos, ";
