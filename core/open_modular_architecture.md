...
**Arquivo original:** `core/open_modular_architecture.py`

**Descricao:** ============================================================
"Tudo na Republica e um MODULO.
 Nada e monolito. Tudo encaixa. Tudo troca. Tudo evolui.
 Cada sistema (OpenHealth, OpenGames, OpenMusic, etc) e um modulo.
 ...
 ISTO E O SISTEMA NERVOSO DA REPUBLICA."
CONCEITO:
  O ecossistema OpenRepublic tem 105+ sistemas.
  Se cada um e uma ilha, o ecossistema MORRE.
  Se cada um e um modulo com interface, o ecossistema VIVE.
  ...
  6. Como modulos SAUDE e monitorados (health check)
...
classe ModuleCategory herda de Enum:
...
classe ModuleStatus herda de Enum:
    REGISTERED = "registrado"  // no catalogo mas nao ativo
...
classe InterfaceType herda de Enum:
    ...
    BIDIRECTIONAL = "bidirecional"  // usa e fornece
...
classe ModuleInterface:
    ...
    direction: InterfaceType // fornece ou consome
    seja description: texto = ""
    seja input_schema: Dict = field(default_factory=dict) // parametros que aceita
    seja output_schema: Dict = field(default_factory=dict) // o que retorna
    seja version: texto = "1.0.0"
    seja required: logico = falso // modulo precisa disto para funcionar?
...
classe Module:
    ...
    TODO sistema da Republica e um modulo.
    Todo modulo:
    - Tem ID unico
    - Tem categoria
    ...
    seja version: texto = "1.0.0"
    seja status: ModuleStatus = ModuleStatus.REGISTERED
    seja description: texto = ""
    seja file_path: texto = ""  // onde esta o codigo
    seja lines_of_code: inteiro = 0

    // Interfaces
    seja provides: [ModuleInterface] = field(default_factory=list)
    seja consumes: [ModuleInterface] = field(default_factory=list)

    // Dependencias (outros modulos que precisa)
    seja dependencies: [texto] = field(default_factory=list)
    seja dependents: [texto] = field(default_factory=list) // quem precisa dele

    // Saude
    seja health_score: flutuante = 1.0 // 0-1
    seja last_health_check: texto = ""
    seja uptime_hours: flutuante = 0.0
    seja error_count: inteiro = 0

    // Modularidade
    seja is_replaceable: logico = verdadeiro // pode ser trocado?
    seja is_hot_swappable: logico = verdadeiro // troca sem reiniciar?
    seja replacement_for: texto = ""  // se substituiu outro modulo

    // decorador: @property
    funcao is_healthy(self) -> logico:
        retorne self.health_score >= 0.7 e self.status == ModuleStatus.ACTIVE

    // decorador: @property
    funcao dependency_count(self) -> inteiro:
        retorne tamanho(self.dependencies)

    // decorador: @property
    funcao interface_count(self) -> inteiro:
        retorne tamanho(self.provides) + tamanho(self.consumes)
...
classe Event:
    ...
    seja data: Dict = field(default_factory=dict)
    seja timestamp: texto = field(default_factory=() -> datetime.now().isoformat())
    seja listeners_notified: inteiro = 0
...
classe EventBus:
    ...
    funcao subscribe(self, event_type: texto, module_id: texto) -> None:
        // Modulo se inscreve para ouvir um tipo de evento.
        se module_id nao in self.listeners[event_type] entao:
            self.listeners[event_type].append(module_id)

    funcao emit(self, event_type: texto, source: texto,
             seja data: Dict = nulo) -> Event:
        // Modulo emite evento.
        event = Event(
    ...
    funcao get_listeners(self, event_type: texto) -> [texto]:
        retorne self.listeners.get(event_type, [])

    funcao recent_events(self, n: inteiro = 10) -> [Dict]:
        retorne [
            {
                "type": e.event_type,
                "source": e.source_module,
                "listeners": e.listeners_notified,
                "time": e.timestamp[:19],
            }
...
classe ModuleRegistry:
    ...
    funcao register(self, module: Module) -> {texto: qualquer}:
        // Registra um modulo no catalogo.
        self.modules[module.module_id] = module

        // Auto-inscrever em eventos baseado nas interfaces
        para cada iface em module.consumes:
            se iface.direction == InterfaceType.EVENT_LISTENER entao:
                self.event_bus.subscribe(iface.name, module.module_id)

        // Registrar dependencias reversas
        para cada dep em module.dependencies:
            se dep in self.modules entao:
                se module.module_id nao in self.modules[dep].dependents entao:
    ...
    funcao activate(self, module_id: texto) -> {texto: qualquer}:
        // Ativa modulo (verifica dependencias primeiro).
        mod = self.modules.get(module_id)
        se nao mod entao:
            retorne {"error": "Modulo nao encontrado"}

        // Verificar dependencias
        missing = []
        para cada dep em mod.dependencies:
            dep_mod = self.modules.get(dep)
            se nao dep_mod ou nao dep_mod.is_healthy entao:
                missing.append(dep)
        ...
        self.event_bus.emit("module.activated", "registry",
                            {"module": module_id})
        retorne {"activated": verdadeiro, "module": mod.name}

    funcao health_check(self, module_id: texto) -> {texto: qualquer}:
        ...
        issues = []
        se mod.status == ModuleStatus.OFFLINE entao:
            issues.append("modulo offline")
        se mod.error_count > 10 entao:
            issues.append("{mod.error_count} erros acumulados")
        para cada dep em mod.dependencies:
            dep_mod = self.modules.get(dep)
            se dep_mod e nao dep_mod.is_healthy entao:
                issues.append("dependencia {dep} nao saudavel")
    ...
    funcao hot_swap(self, old_id: texto, new_module: Module) -> {texto: qualquer}:
        ...
        se nao old entao:
            retorne {"error": "Modulo antigo nao encontrado"}
        ...
        se missing_interfaces entao:
            retorne {
                "error": "Novo modulo nao cobre todas as interfaces",
        ...
        para cada dep_id em old.dependents:
            dep_mod = self.modules.get(dep_id)
            se dep_mod entao:
                se old_id in dep_mod.dependencies entao:
                    dep_mod.dependencies.remove(old_id)
                se new_module.module_id nao in dep_mod.dependencies entao:
    ...
    funcao startup_order(self) -> [texto]:
        // Calcula ordem de inicializacao (topological sort).
        visited = set()
        order = []

        funcao visit(mid: texto, path: {texto}):
            se mid in visited entao:
                retorne nulo
            se mid in path entao:
                retorne // circular dependency
    ...
    funcao detect_issues(self) retorna Dict[texto, [texto]]:
        ...
        para cada mod em self.modules.values():
            // Orfaos: fornecem interface mas ninguem consome
            se nao mod.dependents e mod.status == ModuleStatus.ACTIVE entao:
                orphans.append(mod.module_id)

            // Dependencias quebradas
            para cada dep em mod.dependencies:
                se dep nao in self.modules entao:
                    broken.append("{mod.module_id} -> {dep} (nao existe)")
                senao se self.modules[dep].status == ModuleStatus.DEPRECATED entao:
...
funcao build_republic_catalog() -> ModuleRegistry:
...
se __name__ == "__main__" entao:
    ...
    imprima("=" * 80)
    imprima("  OPENMODULARARCHITECTURE -- ESQUELETO MODULAR DA REPUBLICA")
    imprima("  105+ modulos. Tudo encaixa. Tudo troca. Tudo evolui.")
    imprima("=" * 80)

    // === 1. CATALOGO ===
    s = registry.stats()
    imprima("\n\n  === 1. CATALOGO DE MODULOS ===\n")
    imprima("  Total de modulos: {s['total_modules']}")
    imprima("  Linhas de codigo: {s['total_loc']:,}")
    imprima("  Interfaces totais: {s['total_interfaces']}")
    imprima("  Dependencias totais: {s['total_dependencies']}")
    imprima("  Eventos processados: {s['events_processed']}")

    // === 2. POR CATEGORIA ===
    imprima("\n\n  === 2. MODULOS POR CATEGORIA ===\n")
    para cada (cat, count) em ordene(s["by_category"].items(), key=(x) -> -x[1]):
        imprima("  {cat:<20} {count:>3} modulos")

    // === 3. POR STATUS ===
    imprima("\n\n  === 3. STATUS DOS MODULOS ===\n")
    para cada (status, count) em ordene(s["by_status"].items(), key=(x) -> -x[1]):
        imprima("  {status:<20} {count:>3} modulos")
    ...
    para cada (i, mid) em enumere(order[:20]):
        ...
        imprima("  {i+1:>3}. {mid:<30} ({mod_obj.category.value})")
    se tamanho(order) > 20 entao:
        imprima("  ... e mais {len(order)-20} modulos")

    // === 5. GRAFO DE DEPENDENCIAS ===
    imprima("\n\n  === 5. GRAFO DE DEPENDENCIAS (amostra) ===\n")
    graph = registry.dependency_graph()
    para cada mid em list(graph.keys())[:15]:
        deps = graph[mid]
        se deps entao:
            imprima("  {mid:<30} -> {', '.join(deps[:3])}")
        senao:
            imprima("  {mid:<30} -> (sem dependencias)")
    ...
    imprima("  {swap_result.get('message', swap_result.get('error', '?'))}")
    ...
    imprima("  Eventos recentes:")
    para cada ev em registry.event_bus.recent_events(5):
        imprima("    {ev['type']:<35} de {ev['source']:<20} "
              "-> {ev['listeners']} ouvintes")

    // === 8. HEALTH CHECK ===
    imprima("\n\n  === 8. HEALTH CHECK (amostra) ===\n")
    para cada mid em ["open-republic", "open-health-v2", "open-credit", "open-games"]:
        hc = registry.health_check(mid)
        imprima("  {hc['module']:<30} saude: {hc['health']}  "
              "{'OK' if hc['healthy'] else 'PROBLEMA'}")

    // === 9. DETECTAR PROBLEMAS ===
    imprima("\n\n  === 9. DIAGNOSTICO DO ECOSSISTEMA ===\n")
    issues = registry.detect_issues()
    imprima("  Total de issues: {issues['total_issues']}")
    imprima("  Modulos orfaos: {len(issues['orphans'])}")
    imprima("  Dependencias quebradas: {len(issues['broken_dependencies'])}")
    imprima("  Usando deprecated: {len(issues['using_deprecated'])}")
  ...
  6. REPORTAR SAUDE
     health_check() -> {{"healthy": verdadeiro}}

  7. SER HOT-SWAPPABLE
     Pode ser trocado por versao nova sem quebrar nada

  TEMPLATE:
    classe MeuModulo herda de Module:
...
// )
    ...
    // === FILOSOFIA ===
    imprima("\n\n{'='*80}")
    imprima("  FILOSOFIA DA ARQUITETURA MODULAR")
    imprima("{'='*80}")
    imprima("""
  POR QUE MODULAR?
  ...
  1. FALHA ISOLADA
     ...
     Nenhum modulo e "too big to fail".
  ...
  5. EVENT BUS = SISTEMA NERVOSO
     Modulos nao chamam uns aos outros diretamente.
     Eles EMITEM eventos e REAGEM a eventos.
  ...
  6. HEALTH CHECK CONTINUO
     ...
     Modulo degradado e marcado. Dependents avisados.

  7. SEM MONOLITO
     Nenhum modulo e "grande demais para trocar".
     Se OpenSocialNetwork fica obsoleto, troca.
     Se OpenGames precisa de engine nova, troca.
     Nada e permanente. Tudo evolui.
  ...
  PRINCIPIOS:
    ...
    P2: Cada modulo e autonomo (declara o que faz, nao depende de segredos).
    seja P3: Trocar modulo = trabalho (quem faz o hot-swap ganha credito).
    P4: Ordem de inicializacao e democratica (topological sort, nao hierarquia).
// )
    imprima("{'='*80}")
    imprima("  OpenModularArchitecture: {s['total_modules']} modulos, "
          "{s['total_interfaces']} interfaces, "
          "{s['total_dependencies']} dependencias.")
    imprima("  Tudo encaixa. Tudo troca. Tudo evolui.")
    imprima("{'='*80}")
