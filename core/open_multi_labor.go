...
package openmultilabor_calculo_de_perfis_multi_labor_poli_atuacao
...
const (
    ...
    func __init__(self, label: texto, nature: texto, evidence: texto) {
        self.label = label
        self.nature = nature
...
// ============================================================================
func synergy_multiplier(role_a: LaborRole, role_b: LaborRole) float64 {
    SYNERGY_MAP := { // Dict[(texto, texto), flutuante]
...
funcao total_synergy_for_role(
    ...
    weights := nil, // Optional[[flutuante]]
) -> flutuante:
    if ! other_roles {
        return 1.0
    if weights && nil {
        weights = [1.0] * len(other_roles)
    ...
    diversity_bonus = 1.0 + (len(natures) - 1) * 0.02
    return minimo(avg * diversity_bonus, 1.50)
// ============================================================================
// 3. FOCO PENALTY
// ============================================================================
func focus_penalty(n_roles: inteiro) float64 {
    if n_roles <= 1 {
        return 1.0
...
type RoleAssignment struct {
    ...
    // decorador: @property
    func base_rate(self) float64 {
        return self.level.hour_rate_brl
    // decorador: @property
    func base_value_weekly(self) float64 {
        return self.base_rate * self.hours_per_week
// decorador: @dataclass
type MultiLaborProfile struct {
    citizen_id := "" // string
    roles := field(default_factory=list) // [RoleAssignment]
    ...
    func role_synergy(self, idx: inteiro) float64 {
