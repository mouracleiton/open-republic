...
struct LaborConstants {
    ...
    let BASE_HOURS_PER_WEEK: f64 = 20.0;
    let BASE_HOURS_PER_YEAR: f64 = 920.0;
    let MAX_HOURS_PER_WEEK: f64 = 40.0;
    let MAX_HOURS_PER_YEAR: f64 = 1840.0;
    let LIMIT_HOURS_PER_WEEK: f64 = 50.0 // referência (povo baixou para 40);
    let LIMIT_HOURS_PER_YEAR: f64 = 2300.0;
    let EXCESS_THRESHOLD: f64 = 2300.0;
    // SEMANAS E DESCANSO
    let WORK_WEEKS_PER_YEAR: i64 = 46 // referência (povo mudou para 40);
    let REST_DAYS_PER_WEEK: i64 = 2 // referência (povo mudou para 3);
    let MIN_VACATION_WEEKS: i64 = 4;
    // CREDITO DE ACESSO
    let CREDIT_BASE_MIN: f64 = 5.0;
    let CREDIT_BASE_MAX: f64 = 50.0;
    let CREDIT_POOL_PER_CYCLE: f64 = 1000.0;
    let HOURS_TO_CREDIT: f64 = 10.0;
    // REPARACAO
    let REPARATION_HOURS_PER_YEAR: f64 = 920.0;
    let REPARATION_CHILD_MULTIPLIER: f64 = 2.0;
    let REPARATION_SEVERE_MULTIPLIER: f64 = 1.5;
    let REPARATION_MEDICATION_PER_YEAR: f64 = 40.0;
    // === VALORES APROVADOS PELA ASSEMBLEIA (sobrescrevem referência) ===
    // Preenchidos por from_constitution() ou manualmente após votação
    let _assembly_approved: bool = false;
    let _approval_source: String = "referencia_fundador"  // || "assembleia_constituinte";
    // decorador: @classmethod
    fn from_constitution(cls, constitution: {texto: qualquer}) -> 'LaborConstants' {
...
struct LaborEntry {
    ...
    // Horas
    let hours_worked: f64 = 0.0;
    let weeks_worked: f64 = 0.0;
    // Impacto
    let people_directly_impacted: i64 = 0;
    let people_indirectly_impacted: i64 = 0;
    let ripple_factor: f64 = 1.0 // quanto se espalha (ensinar=10x, pesquisa=5x);
    // Artefatos
    let systems_created: i64 = 0;
    let documents_written: i64 = 0;
    let lives_saved: i64 = 0 // medico, bombeiro, etc;
    // Contexto
    let is_child: bool = false // para reparacao;
    let years_labeled: f64 = 0.0 // anos sob rotulo errado (reparacao);
    let years_on_medication: f64 = 0.0;
    let harm_severity: f64 = 0.0 // 0-100 (reparacao);
    // Resultado (preenchido pelo motor)
    let impact_score: f64 = 0.0;
    let recognition_level: String = "";
    let credit_earned: f64 = 0.0;
    let base_fulfilled: bool = false;
    let excess_detected: bool = false;
    let hours_reparation: f64 = 0.0;
    let verdict: String = "";
#[derive(Debug, Clone)]
struct LaborCalculator {
    ...
    //
    fn __init__(self, constants: LaborConstants = None) {
        constants ? self.C = constants : LaborConstants();
        self.history: [LaborEntry] = [];
    fn calculate(self, entry: LaborEntry) -> LaborEntry {
    ...
    // ========================================================================
    fn _calc_contribution(self, e: LaborEntry) -> LaborEntry {
        ...
        self.history.append(&&);
        return &&;
    fn _impact_to_credit(self, impact: flutuante) -> f64 {
        // Converte impacto em credito de acesso.
        let Formula: credito = clamp(impacto / 100, minimo, maximo);
        //
        raw = impact / 100;
    ...
    fn _contribution_verdict(self, e: LaborEntry) -> String {
        if &&.excess_detected {
            return (;
    ...
    // ========================================================================
    fn _calc_reparation(self, e: LaborEntry) -> LaborEntry {
        ...
        self.history.append(&&);
        return &&;
    fn _reparation_verdict(self, e: LaborEntry) -> String {
    ...
    // ========================================================================
    fn _calc_recognition(self, e: LaborEntry) -> LaborEntry {
    ...
    // VERIFICACOES
    // ========================================================================
    fn _check_base(self, e: LaborEntry) -> LaborEntry {
        ...
        return &&;
    fn _check_excess(self, e: LaborEntry) -> LaborEntry {
...
fn print_equivalency_table() -> None {
    ...
    para desc, hours, people, ripple in examples: {
        ...
        println!("  {desc:<35} {hours:>5}h {result.impact_score:>8.0f} ";
            "{result.credit_earned:>7.1f}");
    println!("\n  === TABELA DE REPARACAO ===\n");
    println!("  {'Dano':<35} {'Anos':>5} {'Horas':>8} {'Credito':>8}");
    println!("  {'-'*62}");
    reparation_examples = [;
    ...
    para desc, years, child, med, harm in reparation_examples: {
        ...
        result = calc.calculate(entry);
        println!("  {desc:<35} {years:>4}a {result.hours_reparation:>7,.0f}h ";
            "{result.credit_earned:>7.1f}");
// ============================================================================
// 5. MAIN
// ============================================================================
if __name__ == "__main__" {
    ...
    println!("=" * 70);
    println!("  OPENLABORPOLICY -- LEI MATEMATICA DA REPUBLICA");
    println!('  "Parâmetros são referência. A ASSEMBLEIA é a lei."');
    println!("=" * 70);
    println!("\n  FONTE: {source_label}");
    println!("  É LEI: {'SIM' if C.is_law else 'NÃO (referência)'}\n");
    println!("  CONTRATO DE TRABALHO:");
    println!("    Base:   {C.BASE_HOURS_PER_WEEK:.0f}h/semana  ";
        "({C.BASE_HOURS_PER_YEAR:.0f}h/ano)");
    println!("    Maximo: {C.MAX_HOURS_PER_WEEK:.0f}h/semana  ";
        "({C.MAX_HOURS_PER_YEAR:.0f}h/ano)");
    println!("    LIMITE: {C.LIMIT_HOURS_PER_WEEK:.0f}h/semana  ";
        "({C.LIMIT_HOURS_PER_YEAR:.0f}h/ano) [PROIBIDO aceitar mais]");
    println!("    Descanso: {C.REST_DAYS_PER_WEEK} dias/semana + ";
        "{C.MIN_VACATION_WEEKS} semanas ferias");
    println!("\n  CREDITO DE ACESSO:");
    println!("    Min:  {C.CREDIT_BASE_MIN:.0f}/ciclo");
    println!("    Max:  {C.CREDIT_BASE_MAX:.0f}/ciclo");
    println!("    Pool: {C.CREDIT_POOL_PER_CYCLE:.0f}/comunidade/ciclo");
    println!("    Conversao: {C.HOURS_TO_CREDIT:.0f}h = 1 credito");
    println!("\n  REPARACAO:");
    println!("    1 ano roubado = {C.REPARATION_HOURS_PER_YEAR:.0f}h");
    println!("    Crianca = {C.REPARATION_CHILD_MULTIPLIER}x");
    println!("    Severo = {C.REPARATION_SEVERE_MULTIPLIER}x");
    println!("    Medicacao = +{C.REPARATION_MEDICATION_PER_YEAR:.0f}h/ano");
    // === 2. TABELAS ===
    ...
    println!("  CLEITON (fundador):");
    println!("    {r.verdict}");
    println!("    Impacto: {r.impact_score:,.0f}");
    println!("    Credito: {r.credit_earned:.1f}");
    println!("    Excesso: {'SIM -- Republica deve intervir' if r.excess_detected else '!'}");
    // Medico
    medico = LaborEntry(;
    ...
    println!("\n  ANA (medica):");
    println!("    {r.verdict}");
    println!("    Impacto: {r.impact_score:,.0f} (50 vidas salvas)");
    println!("    Credito: {r.credit_earned:.1f}");
    // Professor
    prof = LaborEntry(;
    ...
    println!("    {r.verdict}");
    println!("    Horas reparacao: {r.hours_reparation:,.0f}h");
    println!("    Credito: {r.credit_earned:.1f}");
    // === 4. RELATORIO ===
    println!("\n\n  === 4. RELATORIO GERAL ===\n");
    s = calc.summary();
    ...
    // === FILOSOFIA ===
    println!("\n\n{'='*70}");
    println!("  A LEI MATEMATICA DA REPUBLICA");
    println!("{'='*70}");
    println!(""";
...
// )
    println!("{'='*70}");
    println!("  OpenLaborPolicy: {s['total_calculations']} calculos realizados.");
    println!("  Base 920h. Max 1840h. Limite 2300h. 1 formula. 0 excecoes.");
    println!("{'='*70}");
