...
package openprohibitedbusiness_proibicao_de_estabelecimentos_nocivos
...
// ============================================================================
// decorador: @dataclass
type HarmfulBusiness struct {
    // Um negocio nocivo identificado.
    business_id: texto
    name: texto // nome do negocio
    business_type: HarmfulBusinessType
    harm_level := HarmLevel.HARMFUL // HarmLevel
    description := "" // string
    victims_estimated := 0 // quantas pessoas afeta // int64
    money_drained_monthly := ""  // quanto suga por mes // string
    // Acao
    action := BusinessAction.PROHIBIT // BusinessAction
    replacement := ""  // open equivalente // string
    conversion_target := ""  // para que converter // string
    // Status
    status := "identificado"  // identificado, votado, fechado, convertido // string
    closed_date := "" // string
// ============================================================================
...
// ============================================================================
func run_business_assembly(n_voters: inteiro = 10000) {texto: qualquer} {
    // Assembleia vota quais negocios fechar.
    business_votes = {}
    for _, biz := range HARMFUL_BUSINESSES {
...
type ProhibitionEngine struct {
    ...
    func list_harmful(self, severity: inteiro = 0) [Dict] {
    ...
    func close_business(self, business_id: texto) {texto: qualquer} {
    ...
    func _convert_space(self, biz: HarmfulBusiness) {texto: texto} {
    ...
    func financial_impact(self) {texto: qualquer} {
...
if __name__ == "__main__" {
    ...
    fmt.Println("=" * 80)
    fmt.Println("  OPENPROHIBITEDBUSINESS -- PROIBICAO DE ESTABELECIMENTOS NOCIVOS")
    fmt.Println("  Quem destroi vida ! tem lugar. Fechado. Convertido. Substituido.")
    fmt.Println("=" * 80)
    // === 1. ASSEMBLEIA ===
    ...
    fmt.Println("  {'ID':<8} {'Negocio':<35} {'Dano':<15} {'Vitimas':>12} {'Acao'}")
    fmt.Println("  {'-'*85}")
    para b em ordene(engine.businesses.values(), {
                    key = (x) -> -x.harm_level.value[1]):
        fmt.Println("  {b.business_id:<8} {b.name[:34]:<35} {b.harm_level.value[0]:<15} "
    ...
    fmt.Println("  {close1['business']}: {close1['status']}")
    fmt.Println("  Vitimas protegidas: {close1['victims_protected']}")
    fmt.Println("  Substituido por: {close1['replacement']}")
    fmt.Println("  Espaco convertido: {close1['space_converted']['new_use']}")
    // === 4. FECHAR BANCOS PREDATORIOS ===
    fmt.Println("\n\n  === 4. FECHANDO: BANCOS PREDATORIOS ===\n")
    close2 = engine.close_business("HB-004")
    fmt.Println("  {close2['business']}: {close2['status']}")
    fmt.Println("  Vitimas: {close2['victims_protected']}")
    fmt.Println("  Substituido: {close2['replacement']}")
    fmt.Println("  Espaco: {close2['space_converted']['new_use']}")
    // === 5. FECHAR TODOS ===
    fmt.Println("\n\n  === 5. FECHANDO TODOS OS NOCIVOS ===\n")
    for _, bid := range list(engine.businesses.keys()) {
        if engine.businesses[bid].status != "fechado" {
            r = engine.close_business(bid)
            fmt.Println("  [FECHADO] {r['business']:<35} -> {r['space_converted']['new_use']}")
    ...
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  FILOSOFIA: PROIBICAO DE NOCIVOS")
    fmt.Println("{'='*80}")
    fmt.Println("""
...
PRINCIPIOS:
    P1 := contra P1 (anti-elitismo). Proibido. // Negocio que destrui vida
...
// )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenProhibitedBusiness: {s['fechados']}/{s['total_identificados']} fechados. "
        "Vitimas protegidas: {s['vitimas_protegidas_total']:,}.")
    fmt.Println("  Quem destroi vida ! tem lugar.")
    fmt.Println("{'='*80}")
