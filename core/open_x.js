...
if (__name__ == "__main__") {
    console.log("=" * 70);
    console.log("  OPENX -- ESTRATEGIA PARA X/TWITTER");
    console.log("  'Um canal. Opensoftware para estender. Sem prisao.'");
    console.log("=" * 70);
    for (const strat of XStrategy) {
        console.log("\n  === {strat.value.upper()} ===\n");
        items = [x para x em X_STRATEGY_PLAN if x.strategy == strat];
        items.sort(key=(x) -> x.priority);
        for (const x of items) {
            console.log("  [{x.id}] P{x.priority} {x.title}");
            console.log("    {x.description}");
            if (x.estimated_effort) {
                console.log("    Esforco: {x.estimated_effort}");
            console.log();
    console.log("\n{'='*70}");
    console.log("  {len(X_STRATEGY_PLAN)} acoes. 3 estrategias. 1 canal.");
    console.log("  X: usar, estender, libertar. Nunca depender.");
    console.log("{'='*70}");
