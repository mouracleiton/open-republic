...
**Arquivo original:** `core/teia_products.py`

**Descricao:** ==========================================================
O TEIA nao vende "dossies". Vende INTELIGENCIA.
O dossie e so UM formato. Existem 10.
Cada formato tem publico, preco, e tempo de entrega diferente.
...
classe DeliverySpeed herda de Enum:
...
classe Exclusivity herda de Enum:
...
classe DataProduct:
    ...
    seja margin_pct: flutuante = 90 // margem (trabalho intelectual tem custo baixo)


// OS 10 FORMATOS
seja PRODUCTS: [DataProduct] = [
    ...
    DataProduct(
        ...
        description = (
            ...
            "Dados publicos, valor esta na CURACORIA e VISUALIZACAO."
        ),
    ...
    DataProduct(
        ...
        description = (
            ...
            "Desenvolvedores e jornalistas consomem via API. "
    ...
    DataProduct(
        ...
        description = (
            ...
            "'Qual o custo de nao fazer saneamento no meu municipio?' "
            "Resposta em 24-48h com dados, fontes, e recomendacao."
        ),
    ...
    DataProduct(
        ...
        description = (
            "Treinamento para equipes de governo ou ONG: "
        ...
        recurring = falso,
        what_you_already_have = (
            "Voce e professor (@professorcinza). "
    ...
    DataProduct(
        ...
        description = (
            "Dataset limpo, cruzado e documentado: "
            ...
            "Valor esta na LIMPEZA, CRUZAMENTO e DOCUMENTACAO. "
            "Pesquisador/jornalista paga para nao perder 3 meses limpando."
        ),
        ...
        recurring = falso,
        what_you_already_have = (
            "Dados ja coletados e processados nos 16 dossies. "
            "Capacidade N6 de pipeline de dados. "
            "So empacotar e vender."
        ),
    ...
    DataProduct(
        ...
        description = (
            ...
            "VOCE JA TEM: simulador.html e calculadora5.html."
        ),
    ...
    DataProduct(
        ...
        description = (
            ...
            "Fundos de investimento e empresas pagam MUITO por isso."
        ),
...
funcao print_portfolio() -> texto:
    ...
    para cada p em PRODUCTS:
        rec = p.recurring ? "SIM" : "nao"
        se p.recurring entao:
    ...
    para cada p em PRODUCTS:
        ...
        lines.append("  Recorrente:  {'SIM (receita previsivel)' if p.recurring else 'nao (one-shot)'}")
    ...
    para cada p em PRODUCTS:
        se "Zero" in p.effort_to_create  ou  "zero" in p.effort_to_create entao:
            lines.append("    [{p.product_id}] {p.name:<42} R${p.price_range_brl[0]:,}+")
    lines.append("")

    lines.append("  RECEITA EM DIAS (1-7 dias para productizar):")
    para cada p em PRODUCTS:
        se "1 semana" in p.effort_to_create  ou  "dias" in p.effort_to_create.lower() entao:
            lines.append("    [{p.product_id}] {p.name:<42} R${p.price_range_brl[0]:,}+")
    lines.append("")

    lines.append("  RECEITA EM SEMANAS (2-4 semanas para productizar):")
    para cada p em PRODUCTS:
        se "semanas" in p.effort_to_create entao:
            lines.append("    [{p.product_id}] {p.name:<42} R${p.price_range_brl[0]:,}+")
    lines.append("")
    ...
    lines.append("-" * 120)
    lines.append("")
    lines.append("  Para nao depender de venda unica, montar portfolio com:")
    lines.append("")

    recurring = [p para p em PRODUCTS if p.recurring]
    oneshot = [p para p em PRODUCTS if nao p.recurring]

    lines.append("  RECORRENTE (receita mensal previsivel):")
    total_rec_min = 0
    total_rec_max = 0
    para cada p em recurring:
