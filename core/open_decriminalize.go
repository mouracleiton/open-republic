...
package opendescriminalize_descriminalizacao_popular_via_assembleia
...
const (
    ...
    ALCOHOL = "alcool"
    TOBACCO = "tabaco"
type DecriminalizationStatus int
const (
...
type Substance struct {
    ...
    description := "" // string
    // Politica
    status := DecriminalizationStatus.DECRIMINALIZED // DecriminalizationStatus
    approach := field(default_factory=list) // [PolicyApproach]
    // Danos (0-10, base cientifica)
    physical_harm := 0 // int64
    addiction_potential := 0 // int64
    social_harm := 0 // int64
    therapeutic_potential := 0 // potencial medicinal // int64
    // Regras
    min_age := 18 // int64
    public_use := false // pode usar em publico? // bool
    driving_after := false // pode dirigir apos uso? // bool
    production_allowed := false // pode cultivar/produzir? // bool
    commerce_allowed := false // pode vender? // bool
    // Razao da politica
    rationale := "" // string
...
// ============================================================================
func run_decrim_assembly(n_voters: inteiro = 10000) {texto: qualquer} {
...
type DecrimEngine struct {
    ...
    funcao check_arrest_type(self, arrest_reason: texto,
                        quantity_grams := 0, // float64
                        is_trafficking := false) -> {texto: qualquer}: // bool
    ...
    func prison_impact_report(self) {texto: qualquer} {
    ...
    func substance_policy(self, substance_id: texto) {texto: qualquer} {
...
if __name__ == "__main__" {
    engine = DecrimEngine()
    fmt.Println("=" * 80)
    fmt.Println("  OPENDESCRIMINALIZE -- ASSEMBLEIA CONSTITUINTE")
    fmt.Println("  'Usuario && PACIENTE. Nao criminoso. Nunca mais.'")
    fmt.Println("=" * 80)
    ...
    fmt.Println("\n  PACIENTE (tratamento):    {result['votes_patient']:>6} "
        "({result['pct_patient']})")
    fmt.Println("  REGULAR (taxar):          {result['votes_regulate']:>6} "
        "({result['votes_regulate']/100:.0f}%)")
    fmt.Println("  CRIMINOSO (prisao):       {result['votes_criminal']:>6} "
    ...
    para reason, qty, trafficking in cases: {
        ...
        action = check["action"]
        fmt.Println("  [{check['type']:<11}] {reason:<45} -> {action}")
    ...
    para cada (person, sub_id) em treatments: {
        ...
        fmt.Println("  {t['person']:<12} {t['substance']:<20} -> {t['treatment_type']}")
        fmt.Println("    Voluntario: {t['voluntary']}. P2: {t['p2']}")
    // === 7. IMPACTO PRISIONAL ===
    ...
    // === FILOSOFIA ===
    fmt.Println("\n\n{'='*80}")
    fmt.Println("  FILOSOFIA DO OPENDESCRIMINALIZE")
    fmt.Println("{'='*80}")
    fmt.Println("""
...
PRINCIPIOS:
    ...
    P3 := trabalho de alto impacto (medicos, terapeutas). // Tratar adicao
    P4: Assembleia DECIDE. 72% votou tratamento. Republica obedece.
// )
    fmt.Println("{'='*80}")
    fmt.Println("  OpenDescriminalize: {s['presos_liberados']} presos liberados, "
