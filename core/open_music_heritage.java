...
public class OpenmusicheritagePatrimonioMusicalDaRepublica {
    ...
    // ============================================================================
    public static class CulturalOrigin {
    ...
    public static class MusicalGenre {
        ...
        String birth_decade = ""  // quando surgiu;
        String description = "";
        String bpm_range = ""  // BPM tipico;
        [texto] instruments = field(default_factory=list);
        [texto] subgenres = field(default_factory=list);
        // Significado cultural
        String cultural_meaning = "";
        String social_function = ""  // o que faz pela comunidade;
        String dance_style = ""  // como se dança;
        // Protecao
        boolean is_heritage = true // PATRIMONIO da Republica;
        boolean copyrightable = false // NINGUEM pode copyrightar;
        boolean commercial_monopoly = false // sem monopolio corporativo;
        // Estatistica
        int registered_musicians = 0;
        int total_songs = 0;
    // ============================================================================
    ...
    // ============================================================================
    public static class MusicHeritageEngine {
        ...
        public [Dict] list_heritage(self) {
            return [;
                {
                    "id": g.genre_id,;
                    "name": g.name,;
        ...
        public {texto: qualquer} check_permission(self, action: texto, genre: texto) {
    ...
    if (__name__ == "__main__") {
        ...
        System.out.println("  OPENMUSICHERITAGE -- PATRIMONIO MUSICAL DA REPUBLICA");
        System.out.println("  Generos musicais sao BEM COMUM. Ninguem && dono. Todos herdeiros.");
        System.out.println("=" * 80);
        // === 1. GENEROS PATRIMONIO ===
        System.out.println("\n\n  === 1. GENEROS PATRIMONIO ({len(engine.genres)}) ===\n");
        System.out.println("  {'ID':<12} {'Genero':<20} {'Origem':<30} {'Decada':<10} {'BPM'}");
        System.out.println("  {'-'*85}");
        /* TODO: for-each Java para g em engine.genres.values() */
            System.out.println("  {g.genre_id:<12} {g.name:<20} {g.origin_region:<30} ";
                "{g.birth_decade:<10} {g.bpm_range}");
        // === 2. DETALHE: FUNK ===
        System.out.println("\n\n  === 2. FUNK: PATRIMONIO DA PERIFERIA ===\n");
        funk = engine.genres["GEN-FUNK"];
        System.out.println("  Nome: {funk.name}");
        System.out.println("  Origem: {funk.origin_region}");
        System.out.println("  Raiz: {funk.cultural_origin.value}");
        System.out.println("  Instrumentos: {', '.join(funk.instruments)}");
        System.out.println("  Subgeneros: {', '.join(funk.subgenres)}");
        System.out.println("  Significado: {funk.cultural_meaning[:120]}...");
        System.out.println("  Funcao social: {funk.social_function[:120]}...");
        // === 3. DETALHE: RAP ===
        System.out.println("\n\n  === 3. RAP: JORNALISMO DA PERIFERIA ===\n");
        rap = engine.genres["GEN-RAP"];
        System.out.println("  Nome: {rap.name}");
        System.out.println("  Origem: {rap.origin_region}");
        System.out.println("  Significado: {rap.cultural_meaning[:120]}...");
        System.out.println("  Funcao social: {rap.social_function[:120]}...");
        // === 4. DETALHE: SAMBA ===
        System.out.println("\n\n  === 4. SAMBA: ALMA DO BRASIL ===\n");
        samba = engine.genres["GEN-SAMBA"];
        System.out.println("  Nome: {samba.name}");
        System.out.println("  Origem: {samba.origin_region}");
        System.out.println("  Instrumentos: {', '.join(samba.instruments)}");
        System.out.println("  Subgeneros: {', '.join(samba.subgenres)}");
        System.out.println("  Significado: {samba.cultural_meaning[:120]}...");
        ...
        /* para cada (genre, claimant) em claims: */
            ...
            System.out.println("  {claimant} tentou copyrightar {genre} -> {result['status']}");
            System.out.println("    {result['reason'][:80]}");
        // === 6. PERMISSAO ===
        System.out.println("\n\n  === 6. PERMISSAO PARA CRIAR ===\n");
        /* TODO: for-each Java para action em ["criar", "tocar", "dançar", "remixar", "ensinar"] */
        ...
        Criar musica P3 = trabalho base 1.0. Sem royalty adicional.;
        P4: Quem decide o que && cultura && o POVO, ! corporacao.;
        // )
        System.out.println("{'='*80}");
        System.out.println("  OpenMusicHeritage: {s['total_genres_heritage']} generos patrimonio. ";
            "{s['copyright_claims_rejected']} copyrights rejeitados.");
        System.out.println("  Musica ! && produto. E CULTURA.");
        System.out.println("{'='*80}");
}
