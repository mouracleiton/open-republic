...
**Arquivo original:** `core/open_prohibited_business.py`

**Descricao:** =================================================================
"Estabelecimento que DESTROI a vida de cidadaos da Republica
 nao tem lugar. Fechou. Convertido. Substituido.
 A Republica PROTEGE. Nao negocia com quem mata."
O QUE ISTO FAZ:
  1. IDENTIFICA estabelecimentos e negocios nocivos
...
classe HarmfulBusinessType herda de Enum:
    ...
    WEAPONS_TRADE = "comercio_armas"  // venda de armas (nao FabLab)
...
classe BusinessAction herda de Enum:
...
// decorador: @dataclass
classe HarmfulBusiness:
    // Um negocio nocivo identificado.
    business_id: texto
    name: texto // nome do negocio
    business_type: HarmfulBusinessType
    seja harm_level: HarmLevel = HarmLevel.HARMFUL
    seja description: texto = ""
    seja victims_estimated: inteiro = 0 // quantas pessoas afeta
    seja money_drained_monthly: texto = ""  // quanto suga por mes

    // Acao
    seja action: BusinessAction = BusinessAction.PROHIBIT
    seja replacement: texto = ""  // open equivalente
    seja conversion_target: texto = ""  // para que converter

    // Status
    seja status: texto = "identificado"  // identificado, votado, fechado, convertido
    seja closed_date: texto = ""
...
seja HARMFUL_BUSINESSES: [HarmfulBusiness] = [
    ...
    HarmfulBusiness(
        ...
        description = (
            ...
            "Pega quem banco nao empresta (vulneravel)."
        ),
    ...
    HarmfulBusiness(
        ...
        description = (
            ...
            "85% perdem dinheiro. Foco em recrutar, nao vender. "
            "Explora amigo/familia. Promessa de liberdade financeira. "
            "Realidade: estoque na garagem e divida."
        ),
        victims_estimated = 5000000,
    ...
    HarmfulBusiness(
        ...
        description = (
            ...
            "Industria sabia que matava e escondeu por decadas. "
    ...
    HarmfulBusiness(
        ...
        HarmLevel.DESTRUCTIVE,
        description = (
            "Vende remedio falso. Mata. "
            "Quem nao tem dinheiro compra mais barato e morre. "
...
funcao run_business_assembly(n_voters: inteiro = 10000) -> {texto: qualquer}:
    // Assembleia vota quais negocios fechar.

    business_votes = {}
    para cada biz em HARMFUL_BUSINESSES:
    ...
    retorne {
        "question": (
            ...
            "e CONVERTIDOS em algo util?"
        ),
        "business_votes": business_votes,
        "total_businesses": tamanho(HARMFUL_BUSINESSES),
...
classe ProhibitionEngine:
    ...
    - Nao e punido (nao sabia que era nocivo, ou precisava)
    ...
    - Se sabia que era nocivo e lucrava: OpenPenalRevision
    ...
    funcao list_harmful(self, severity: inteiro = 0) -> [Dict]:
    ...
    funcao close_business(self, business_id: texto) -> {texto: qualquer}:
        ...
        worker_plan = {
            ...
            "nao_punidos": "Trabalhador nao e punido. Precisava trabalhar.",
        ...
        retorne {
            ...
            "message": (
                ...
                "Trabalhadores: nao punidos. Reinseridos."
            ),
        }

    funcao _convert_space(self, biz: HarmfulBusiness) -> {texto: texto}:
        ...
        retorne conversions.get(biz.business_type, {
            "new_use": "Centro comunitario da Republica",
            "how": "Convertido para uso comunitario.",
        })

    funcao financial_impact(self) -> {texto: qualquer}:
    ...
    funcao stats(self) -> {texto: qualquer}:
        retorne {
            "total_identificados": tamanho(self.businesses),
...
se __name__ == "__main__" entao:
    ...
    imprima("=" * 80)
    imprima("  OPENPROHIBITEDBUSINESS -- PROIBICAO DE ESTABELECIMENTOS NOCIVOS")
    imprima("  Quem destroi vida nao tem lugar. Fechado. Convertido. Substituido.")
    imprima("=" * 80)
    ...
    imprima("  {'ID':<8} {'Negocio':<35} {'Dano':<15} {'Vitimas':>12} {'Acao'}")
    imprima("  {'-'*85}")
    para b em ordene(engine.businesses.values(),
                    key = (x) -> -x.harm_level.value[1]):
        imprima("  {b.business_id:<8} {b.name[:34]:<35} {b.harm_level.value[0]:<15} "
    ...
    imprima("  {close1['business']}: {close1['status']}")
    imprima("  Vitimas protegidas: {close1['victims_protected']}")
    imprima("  Substituido por: {close1['replacement']}")
    imprima("  Espaco convertido: {close1['space_converted']['new_use']}")

    // === 4. FECHAR BANCOS PREDATORIOS ===
    imprima("\n\n  === 4. FECHANDO: BANCOS PREDATORIOS ===\n")
    close2 = engine.close_business("HB-004")
    imprima("  {close2['business']}: {close2['status']}")
    imprima("  Vitimas: {close2['victims_protected']}")
    imprima("  Substituido: {close2['replacement']}")
    imprima("  Espaco: {close2['space_converted']['new_use']}")

    // === 5. FECHAR TODOS ===
    imprima("\n\n  === 5. FECHANDO TODOS OS NOCIVOS ===\n")
    para cada bid em list(engine.businesses.keys()):
        se engine.businesses[bid].status != "fechado" entao:
            r = engine.close_business(bid)
            imprima("  [FECHADO] {r['business']:<35} -> {r['space_converted']['new_use']}")
    ...
    // === FILOSOFIA ===
    imprima("\n\n{'='*80}")
    imprima("  FILOSOFIA: PROIBICAO DE NOCIVOS")
    imprima("{'='*80}")
    imprima("""
  ...
  O QUE ACONTECE COM QUEM TRABALHAVA:
    Trabalhador: nao PUNIDO. Recebe OpenKit + OpenLaborRelay + OpenEducation.
  ...
  IMPACTO FINANCEIRO:
    Fechar nocivos nao custa. ECONOMIZA.
  ...
  A LOGICA:
    ...
    "O predador NAO oferece nada que a Republica nao ofereca MELHOR."

  PRINCIPIOS:
    seja P1: Negocio que destrui vida = contra P1 (anti-elitismo). Proibido.
    P2: Viciar/endividar = violencia contra corpo/mente. P2 protege.
    P3: Trabalhador reinserido (nao punido). Predador: OpenPenalRevision.
    P4: Assembleia vota quais fechar. Povo decide.
// )
    imprima("{'='*80}")
    imprima("  OpenProhibitedBusiness: {s['fechados']}/{s['total_identificados']} fechados. "
          "Vitimas protegidas: {s['vitimas_protegidas_total']:,}.")
    imprima("  Quem destroi vida nao tem lugar.")
    imprima("{'='*80}")
