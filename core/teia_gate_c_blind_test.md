...
**Arquivo original:** `core/teia_gate_c_blind_test.py`
...
ARTEFATOS_PARA_TESTE = [
    ...
    {
        "id": "BLIND-003",
        "tema": "Reforma tributária e impacto em famílias de baixa renda",
        "tempo_leitura_min": 7,
    },
]
...
classe BlindEvaluation:
    ...
    // decorador: @property
    funcao score_total(self) -> flutuante:
        retorne (self.acuracia + self.clareza + self.profundidade +
    ...
    // decorador: @property
    funcao nivel_ministerial(self) -> logico:
        // Score total >= 8.0 = nível ministerial (pronto para governar).
        retorne self.score_total >= 8.0
...
funcao print_evaluation_template() -> texto:
    ...
    lines.append("     (0=dados errados, 10=tudo verificado e correto)")
    lines.append("")
    lines.append("  2. CLAREZA: É legível? Um não-especialista entende?")
    lines.append("     (0=incompreensível, 10=didático e preciso)")
    lines.append("")
    lines.append("  3. PROFUNDIDADE: Tem análise profunda ou é raso?")
    ...
    lines.append("")
    lines.append("  5. FONTES: Fontes são confiáveis e verificáveis?")
    ...
    para cada artefato em ARTEFATOS_PARA_TESTE:
        lines.append("-" * 80)
...
funcao analyze_results(evaluations: [BlindEvaluation], identity_map: {texto: texto}) -> texto:
    ...
    // Agrupar por tema
    temas = set(e.tema para e em evaluations)

    para cada tema em ordene(temas):
        lines.append("-" * 110)
        lines.append("TEMA: {tema}")
        lines.append("-" * 110)
        lines.append("")

        tema_evals = [e para e em evaluations if e.tema == tema]
        // Separar por versão (A vs B)
        a_evals = [e para e em tema_evals if "-A" in e.artefato_id]
        b_evals = [e para e em tema_evals if "-B" in e.artefato_id]

        lines.append("  {'AVALIADOR':<15} {'IDENTIDADE':<12} {'ACUR':>5} {'CLAR':>5} {'PROF':>5} {'ACAO':>5} {'FONT':>5} {'COMP':>5} {'TOTAL':>6} {'GRADE'}")
        lines.append("  " + "-" * 85)

        para cada e em a_evals + b_evals:
            identidade = identity_map.get(e.artefato_id, "???")
            lines.append(
                "  {e.avaliador_id:<15} "
                "{identidade:<12} "
                "{e.acuracia:>4.1f} "
        ...
        // Médias
        a_avg = a_evals ? soma(e.score_total para e em a_evals) / tamanho(a_evals) : 0
        b_avg = b_evals ? soma(e.score_total para e em b_evals) / tamanho(b_evals) : 0
    ...
    teia_evals = [e para e em evaluations if identity_map.get(e.artefato_id) == "TEIA"]
    mercado_evals = [e para e em evaluations if identity_map.get(e.artefato_id) != "TEIA"]

    teia_avg = teia_evals ? soma(e.score_total para e em teia_evals) / tamanho(teia_evals) : 0
    mercado_avg = mercado_evals ? soma(e.score_total para e em mercado_evals) / tamanho(mercado_evals) : 0

    teia_ministerial = soma(1 para e em teia_evals if e.nivel_ministerial)
    mercado_ministerial = soma(1 para e em mercado_evals if e.nivel_ministerial)
    ...
    para crit, attr in [("Acurácia", "acuracia"), ("Clareza", "clareza"),
        ...
        t_avg = teia_evals ? soma(getattr(e, attr) para e em teia_evals) / tamanho(teia_evals) : 0
        m_avg = mercado_evals ? soma(getattr(e, attr) para e em mercado_evals) / tamanho(mercado_evals) : 0
        diff = t_avg - m_avg
...
funcao demo_simulation() -> texto:
    ...
    retorne analyze_results(evaluations, identity_map)
...
se __name__ == "__main__" entao:
    imprima("=" * 110)
    imprima("GATE C: QUALIDADE DO ARTEFATO — PAINEL CEGO")
    imprima("=" * 110)
    imprima()

    // Mostrar template
    imprima("TEMPLATE DE AVALIAÇÃO (para impressão):")
    imprima()
    // print(print_evaluation_template())  // Uncomment para imprimir
    imprima("  [Template disponível no arquivo]")
    imprima()

    // Rodar simulação
    imprima()
    imprima(demo_simulation())

    imprima()
    imprima("=" * 110)
    imprima("COMO EXECUTAR O TESTE REAL:")
    imprima("=" * 110)
    imprima("""
  1. GERAR ARTEFATOS:
  ...
  2. PREPARAR CEGO:
     ...
     - Numerar A e B aleatoriamente (não revelar qual é TEIA)
