...
#ifndef PROPOSTA_DE_POL_TICAS_P_BLICAS_PARA_O_BRASIL_H
#define PROPOSTA_DE_POL_TICAS_P_BLICAS_PARA_O_BRASIL_H
...
if (__name__ == "__main__") {
    printf("=" * 80);
    printf("  POLÍTICAS PÚBLICAS PARA O BRASIL");
    printf("  Baseadas em 116 sistemas | 91.729 linhas | OpenRepublic");
    printf("=" * 80);
    total = 0;
    /* para cada (area, politicas) em POLITICAS.items(): */
        ...
        /* para cada (pid, p) em politicas.items(): */
            total = total + 1;
            printf("  [{pid}] {p['titulo']}");
            printf("    Problema: {p['problema'][:70]}...");
            printf("    Custo: {p['custo_atual']}");
            printf("    Economia: {p['economia']}");
            printf("    Prazo: {p['prazo']}");
            printf("    Sistema: {p['sistema']}");
    printf("\n\n{'='*80}");
    printf("  TOTAL: {total} políticas públicas em {len(POLITICAS)} áreas");
    printf("  Base: 116 sistemas | 91.729 linhas de código | CC0");
    printf("  Brasil: do capitalismo predatório à República do bem comum");
    printf("  20-25 anos. Gradual. Sem trauma. Com dignidade.");
    printf("{'='*80}");

#endif // PROPOSTA_DE_POL_TICAS_P_BLICAS_PARA_O_BRASIL_H
