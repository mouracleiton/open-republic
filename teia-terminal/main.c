...
#ifndef TEIA_TERMINAL_STACK_DEFINITIVA_ESQUELETO_H
#define TEIA_TERMINAL_STACK_DEFINITIVA_ESQUELETO_H
...
typedef struct User {
    ...
    bool is_teia_direct = false;
    bool is_active = true;
    bool vpn_key_active = true;
    bool monthly_paid = true;
    int artifacts_generated = 0;
    double revenue_generated = 0.0;
    double joined_at = field(default_factory=time.time);
...
{texto: qualquer} check_module_access(user: User, module_id: texto) {
...
texto? _current_user_id = "cleiton"  // default: Cleiton (TEIA direto);
...
// ============================================================================
// decorador: @app.get("/")
void dashboard(request: Request) {
...
void view_module(request: Request, module_id: texto) {
...
void simulador(request: Request, modelo: texto) {
...
void api_simular(modelo: texto, valor: flutuante) {
...
void gerar_dossie(request: Request) {
...
void validacao_gate_a(request: Request) {
...
void switch_user(user_id: texto) {
...
if (__name__ == "__main__") {
    // importa uvicorn
    printf("TEIA Terminal v{TEIA_VERSION}");
    printf("Ambiente: {TEIA_ENV}");
    printf("URL: http://localhost:8000");
    printf("Usuários demo: cleiton, demo_base, demo_analista, demo_espec");
    printf("Trocar usuário: /switch-user/<id>");
    printf();
    uvicorn.run(app, host="0.0.0.0", port=8000);

#endif // TEIA_TERMINAL_STACK_DEFINITIVA_ESQUELETO_H
