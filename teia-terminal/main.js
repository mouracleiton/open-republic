...
class User {
    ...
    const is_teia_direct = false;
    const is_active = true;
    const vpn_key_active = true;
    const monthly_paid = true;
    const artifacts_generated = 0;
    const revenue_generated = 0.0;
    const joined_at = field(default_factory=time.time);
...
check_module_access(user: User, module_id: texto) {
...
const _current_user_id = "cleiton"  // default: Cleiton (TEIA direto);
get_current_user() {
...
async view_module(request: Request, module_id: texto) {
...
async simulador(request: Request, modelo: texto) {
...
async api_simular(modelo: texto, valor: flutuante) {
...
async gerar_dossie(request: Request) {
...
async validacao_gate_a(request: Request) {
...
async switch_user(user_id: texto) {
...
if (__name__ == "__main__") {
    // importa uvicorn
    console.log("TEIA Terminal v{TEIA_VERSION}");
    console.log("Ambiente: {TEIA_ENV}");
    console.log("URL: http://localhost:8000");
    console.log("Usuários demo: cleiton, demo_base, demo_analista, demo_espec");
    console.log("Trocar usuário: /switch-user/<id>");
    console.log();
