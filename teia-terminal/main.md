...
**Arquivo original:** `teia-terminal/main.py`
...
POR QUE NAO STREAMLIT:
  Streamlit e prototipo. Recarrega pagina inteira. Sem RBAC granular.
  Sem command palette. Sem UX de terminal. Sem VPN nativa.
POR QUE NAO REACT/SPA:
...
classe CertLevel herda de Enum:
    NONE = 0
    BASE = 1
    ANALISTA = 2
    ESPECIALISTA = 3
...
classe User:
    ...
    seja is_teia_direct: logico = falso
    seja is_active: logico = verdadeiro
    seja vpn_key_active: logico = verdadeiro
    seja monthly_paid: logico = verdadeiro
    seja artifacts_generated: inteiro = 0
    seja revenue_generated: flutuante = 0.0
    seja joined_at: flutuante = field(default_factory=time.time)


// Modulos e requisitos (do teia_terminal_economy.py)
MODULES = {
    "fome":             {"name": "Segurança Alimentar",      "cert": CertLevel.BASE,        "sensitive": falso, "teia_only": falso},
    "saneamento":       {"name": "Saneamento",              "cert": CertLevel.BASE,        "sensitive": falso, "teia_only": falso},
    "emprego":          {"name": "Emprego (CAGED)",          "cert": CertLevel.BASE,        "sensitive": falso, "teia_only": falso},
    "educacao":         {"name": "Educação (INEP)",          "cert": CertLevel.BASE,        "sensitive": falso, "teia_only": falso},
    "impacto_fiscal":   {"name": "Impacto Fiscal (35 pol.)",  "cert": CertLevel.ANALISTA,    "sensitive": falso, "teia_only": falso},
    "simulador":        {"name": "Simulador de Cenários",     "cert": CertLevel.ANALISTA,    "sensitive": falso, "teia_only": falso},
    "negativados":      {"name": "Negativados (SPC)",        "cert": CertLevel.ANALISTA,    "sensitive": falso, "teia_only": falso},
    "due_diligence":    {"name": "Due Diligence",            "cert": CertLevel.ESPECIALISTA,"sensitive": falso, "teia_only": falso},
    "parecer_juridico": {"name": "Parecer Jurídico-Político", "cert": CertLevel.ESPECIALISTA,"sensitive": falso, "teia_only": falso},
    "juros_spread":     {"name": "Juros e Spread Bancário",  "cert": CertLevel.ESPECIALISTA, "sensitive": falso, "teia_only": falso},
    "comunidades_reais":{"name": "Comunidades Reais",        "cert": CertLevel.ESPECIALISTA,"sensitive": verdadeiro,  "teia_only": verdadeiro},
    "saude_individual": {"name": "Saúde Individual",         "cert": CertLevel.ESPECIALISTA,"sensitive": verdadeiro,  "teia_only": verdadeiro},
    "banco_palmas":     {"name": "Dados Banco Palmas",       "cert": CertLevel.ESPECIALISTA,"sensitive": verdadeiro,  "teia_only": verdadeiro},
    "open_credit":      {"name": "OpenCredit",               "cert": CertLevel.ESPECIALISTA,"sensitive": falso, "teia_only": verdadeiro},
}


funcao check_module_access(user: User, module_id: texto) -> {texto: qualquer}:
    // Verifica acesso do usuário a um módulo.
    mod = MODULES.get(module_id)
    se nao mod entao:
        retorne {"allowed": falso, "reason": "Módulo '{module_id}' não existe."}

    se user.is_teia_direct entao:
        retorne {"allowed": verdadeiro, "reason": "TEIA direto."}

    se mod["sensitive"] entao:
        retorne {
            "allowed": falso,
            "reason": "DADO SENSÍVEL. '{mod['name']}' é restrito à equipe TEIA. LGPD/sigilo."
        }

    se mod["teia_only"] entao:
        retorne {
            "allowed": falso,
            "reason": "'{mod['name']}' é operação interna TEIA. Contrate TEIA direto."
        }

    se user.cert_level.value < mod["cert"].value entao:
        retorne {
            "allowed": falso,
            "reason": "'{mod['name']}' requer certificação {mod['cert'].name}. Faça treinamento."
        }

    se nao user.vpn_key_active entao:
        retorne {"allowed": falso, "reason": "VPN inativa. Renove mensalidade."}

    se nao user.monthly_paid entao:
        retorne {"allowed": falso, "reason": "Mensalidade em atraso. Regularize."}

    retorne {"allowed": verdadeiro, "reason": "Acesso autorizado."}
...
USERS = {
    "cleiton": User(
        ...
        cert_level = CertLevel.TEIA_DIRETO, is_teia_direct=verdadeiro,
    ),
    "demo_base": User(
...
TEMPLATES_DIR.mkdir(exist_ok=verdadeiro)
STATIC_DIR.mkdir(exist_ok=verdadeiro)
(STATIC_DIR / "css").mkdir(exist_ok=verdadeiro)
(STATIC_DIR / "js").mkdir(exist_ok=verdadeiro)
...
seja _current_user_id: texto? = "cleiton"  // default: Cleiton (TEIA direto)


funcao get_current_user() -> User:
    se _current_user_id e _current_user_id in USERS entao:
        retorne USERS[_current_user_id]
...
// decorador: @app.get("/")
funcao assincrona dashboard(request: Request):
    ...
    para cada (mid, minfo) em MODULES.items():
        ...
        se access["allowed"] entao:
            modulos_acessiveis.append(entry)
        senao:
            modulos_bloqueados.append(entry)

    retorne templates.TemplateResponse(request, "dashboard.html", {
        "request": request,
        "user": user,
...
funcao assincrona view_module(request: Request, module_id: texto):
    // Visualiza um módulo específico.
    user = get_current_user()
    access = check_module_access(user, module_id)

    se nao  access["allowed"] entao:
        retorne templates.TemplateResponse(request, "blocked.html", {
            "request": request,
            "module_id": module_id,
    ...
    retorne templates.TemplateResponse(request, "modulo.html", {
        "request": request,
        "user": user,
        "module_id": module_id,
...
funcao assincrona simulador(request: Request, modelo: texto):
    ...
    retorne templates.TemplateResponse(request, "simulador.html", {
        "request": request,
        "user": user,
        "modelo": modelo,
        "dados": DADOS,
...
funcao assincrona api_simular(modelo: texto, valor: flutuante):
    // API HTMX para simulação em tempo real.
    user = get_current_user()
    access = check_module_access(user, "simulador")

    se nao  access["allowed"] entao:
        retorne JSONResponse({"error": access["reason"]}, status_code=403)
    ...
    senao se modelo == "saneamento" entao:
        investimento = valor
    ...
    retorne JSONResponse({"error": "Modelo não encontrado"}, status_code=404)


// decorador: @app.get("/gerar/dossie")
funcao assincrona gerar_dossie(request: Request):
    // Gerador de dossiê -- gera, revisa, aprova.
    user = get_current_user()

    retorne templates.TemplateResponse(request, "gerar_dossie.html", {
        "request": request,
        "user": user,
        "dados": DADOS,
        "validacoes": VALIDACOES_GATE_A,
...
funcao assincrona validacao_gate_a(request: Request):
    // Painel de validação Gate A -- mostra dados validados.
    user = get_current_user()

    retorne templates.TemplateResponse(request, "validacao.html", {
        "request": request,
        "user": user,
        "validacoes": VALIDACOES_GATE_A,
    })
...
funcao assincrona switch_user(user_id: texto):
    // Trocar usuário (demo -- em produção seria login JWT).
    // variavel global: _current_user_id
    se user_id in USERS entao:
        _current_user_id = user_id
    retorne RedirectResponse(url="/", status_code=302)


// decorador: @app.get("/health")
funcao assincrona health():
    retorne {"status": "ok", "version": TEIA_VERSION, "env": TEIA_ENV}
...
se __name__ == "__main__" entao:
    // importa uvicorn
    imprima("TEIA Terminal v{TEIA_VERSION}")
    imprima("Ambiente: {TEIA_ENV}")
    imprima("URL: http://localhost:8000")
    imprima("Usuários demo: cleiton, demo_base, demo_analista, demo_espec")
    imprima("Trocar usuário: /switch-user/<id>")
    imprima()
