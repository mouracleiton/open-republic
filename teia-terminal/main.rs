...
struct User {
    ...
    let is_teia_direct: bool = false;
    let is_active: bool = true;
    let vpn_key_active: bool = true;
    let monthly_paid: bool = true;
    let artifacts_generated: i64 = 0;
    let revenue_generated: f64 = 0.0;
    let joined_at: f64 = field(default_factory=time.time);
...
fn check_module_access(user: User, module_id: texto) -> {texto: qualquer} {
...
let _current_user_id: texto? = "cleiton"  // default: Cleiton (TEIA direto);
...
// ============================================================================
// decorador: @app.get("/")
async fn dashboard(request: Request) {
...
async fn view_module(request: Request, module_id: texto) {
...
async fn simulador(request: Request, modelo: texto) {
...
async fn api_simular(modelo: texto, valor: flutuante) {
...
async fn gerar_dossie(request: Request) {
...
async fn validacao_gate_a(request: Request) {
...
async fn switch_user(user_id: texto) {
...
if __name__ == "__main__" {
    // importa uvicorn
    println!("TEIA Terminal v{TEIA_VERSION}");
    println!("Ambiente: {TEIA_ENV}");
    println!("URL: http://localhost:8000");
    println!("Usuários demo: cleiton, demo_base, demo_analista, demo_espec");
    println!("Trocar usuário: /switch-user/<id>");
    println!();
