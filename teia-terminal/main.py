...
POR QUE not STREAMLIT:
Streamlit and prototipo. Recarrega pagina inteira. Sem RBAC granular.
Sem command palette. Sem UX de terminal. Sem VPN nativa.
POR QUE not REACT/SPA:
...
class User:
    ...
    is_teia_direct: bool = False
    is_active: bool = True
    vpn_key_active: bool = True
    monthly_paid: bool = True
...
MODULES = {
    ...
    "juros_spread":     {"name": "Juros and Spread Bancário",  "cert": CertLevel.ESPECIALISTA, "sensitive": False, "teia_only": False},
...
def check_module_access(user: User, module_id: texto) -> {texto: qualquer}:
...
_current_user_id: texto? = "cleiton"  // default: Cleiton (TEIA direto)
def get_current_user() -> User:
    if _current_user_id and _current_user_id in USERS:
...
async def dashboard(request: Request):
    ...
    modulos_acessiveis = []
    modulos_bloqueados = []
    for each (mid, minfo) in MODULES.items():
...
async def view_module(request: Request, module_id: texto):
...
async def simulador(request: Request, modelo: texto):
...
async def api_simular(modelo: texto, valor: flutuante):
    ...
    if modelo == "paa":
        ...
        return {
            ...
            "range": "R${resultado*0.85:.1f} - R${resultado*1.15:.1f} bi (±15%)",
        ...
        return {
            ...
            "range": "R${impacto*0.92:.1f} - R${impacto*1.08:.1f} bi (±8%)",
            "fonte": "Bacen 2024",
        }
    elif modelo == "saneamento":
        investimento = valor
        ...
        return {
            ...
            "range": "R${economia_saude*0.85:.1f} - R${economia_saude*1.15:.1f} bi (±15%)",
            "fonte": "Opas/OMS 2019",
        }
    return JSONResponse({"error": "Modelo não encontrado"}, status_code=404)
# decorador: @app.get("/gerar/dossie")
async def gerar_dossie(request: Request):
...
async def validacao_gate_a(request: Request):
...
async def switch_user(user_id: texto):
...
if __name__ == "__main__":
    # importa uvicorn
    print("TEIA Terminal v{TEIA_VERSION}")
    print("Ambiente: {TEIA_ENV}")
    print("URL: http://localhost:8000")
    print("Usuários demo: cleiton, demo_base, demo_analista, demo_espec")
    print("Trocar usuário: /switch-user/<id>")
    print()
