...
**Arquivo original:** `POLITICAS_PUBLICAS_BRASIL.py`
...
"ECONOMIA": {
    ...
    "P11_INDUSTRY_COPIAR_MELHORAR_SUPERAR": {
        "titulo": "Brasil copia, melhora e supera produtos estrangeiros",
        "problema": (
...
"DIREITOS": {
    ...
    "P21_LIGACAO_PREDATORIA_PROIBIDA": {
        "titulo": "Telemarketing, robocall e spam telefônico PROIBIDOS",
...
"INFRAESTRUTURA": {
    ...
    "P32_REPOSITORIO_UNICO_SEM_FORK": {
        ...
        "custo_atual": "R$ 0",
        "economia": "Evita fragmentação e duplicação",
        "prazo": "Fase 1 (imediato)",
...
se __name__ == "__main__" entao:
    imprima("=" * 80)
    imprima("  POLÍTICAS PÚBLICAS PARA O BRASIL")
    imprima("  Baseadas em 116 sistemas | 91.729 linhas | OpenRepublic")
    imprima("=" * 80)
    ...
    para cada (area, politicas) em POLITICAS.items():
        ...
        para cada (pid, p) em politicas.items():
            total = total + 1
            imprima("  [{pid}] {p['titulo']}")
            imprima("    Problema: {p['problema'][:70]}...")
            imprima("    Custo: {p['custo_atual']}")
            imprima("    Economia: {p['economia']}")
            imprima("    Prazo: {p['prazo']}")
            imprima("    Sistema: {p['sistema']}")

    imprima("\n\n{'='*80}")
    imprima("  TOTAL: {total} políticas públicas em {len(POLITICAS)} áreas")
    imprima("  Base: 116 sistemas | 91.729 linhas de código | CC0")
    imprima("  Brasil: do capitalismo predatório à República do bem comum")
    imprima("  20-25 anos. Gradual. Sem trauma. Com dignidade.")
    imprima("{'='*80}")
